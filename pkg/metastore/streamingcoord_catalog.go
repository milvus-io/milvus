...
//
// It lives in the shared pkg/v3 module (it depends only on streamingpb, no
// model) so the pooled catalog service can reuse the same persistence contract.
// internal/metastore keeps a type-alias shim so existing call sites and the
// internal/metastore/kv/streamingcoord implementation compile unchanged.
