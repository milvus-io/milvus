...
func (kv *RocksdbKV) Load(ctx context.Context, key string) (string, error) {
	...
	if key == "" {
		return "", merr.WrapErrParameterInvalidMsg("rocksdb kv does not support load empty key")
	}
...
func (kv *RocksdbKV) LoadBytes(ctx context.Context, key string) ([]byte, error) {
	...
	if key == "" {
		return nil, merr.WrapErrParameterInvalidMsg("rocksdb kv does not support load empty key")
	}
...
func (kv *RocksdbKV) Save(ctx context.Context, key, value string) error {
	...
	if key == "" {
		return merr.WrapErrParameterInvalidMsg("rocksdb kv does not support empty key")
	}
	if value == "" {
		return merr.WrapErrParameterInvalidMsg("rocksdb kv does not support empty value")
	}
...
func (kv *RocksdbKV) DeleteRange(ctx context.Context, startKey, endKey string) error {
	...
	if startKey >= endKey {
		return merr.WrapErrParameterInvalidMsg("rockskv delete range startkey must < endkey, startkey %s, endkey %s", startKey, endKey)
	}
	writeBatch := gorocksdb.NewWriteBatch()
	defer writeBatch.Destroy()
