...
func TestSplitShardMessage(t *testing.T) {
	header := &message.SplitShardMessageHeader{
		CollectionId:    100,
		SplitTaskId:     1,
		SourceVchannel:  "by-dev-rootcoord-dml_0_100v0",
		TargetVchannels: []string{"by-dev-rootcoord-dml_0_100v1", "by-dev-rootcoord-dml_1_100v2"},
	...
	assert.Equal(t, []string{"by-dev-rootcoord-dml_0_100v1", "by-dev-rootcoord-dml_1_100v2"}, splitMsg.Header().TargetVchannels)
	assert.Equal(t, "by-dev-rootcoord-dml_0_100v0", splitMsg.Header().SourceVchannel)
	...
	assert.Len(t, immutableMsg.Header().TargetVchannels, 2)
