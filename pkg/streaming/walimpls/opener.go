...
func (oo OpenOption) Validate() error {
	if oo.Channel.Name == "" {
		return merr.WrapErrParameterInvalidMsg("channel name is empty")
	}
	if oo.Channel.Term < 0 {
		return merr.WrapErrParameterInvalidMsg("channel term is negative")
	}
	if oo.Channel.AccessMode != types.AccessModeRO && oo.Channel.AccessMode != types.AccessModeRW {
		return merr.WrapErrParameterInvalidMsg("undefined access mode")
	}
	return nil
}
