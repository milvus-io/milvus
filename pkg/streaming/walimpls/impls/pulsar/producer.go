...
func newWALProducer(ctx context.Context, newClient clientFactory, topic string, logger *log.MLogger) (*walProducer, error) {
	...
	if !ok {
		logger.Warn("unknown pulsar producer access mode, use exclusive", zap.String("producerAccessMode", accessModeValue))
	}
	...
	for {
		...
		if err == nil {
			logger.Info("pulsar create producer done", zap.String("producerAccessMode", accessModeValue))
			return &walProducer{client: client, producer: producer}, nil
		}
		logger.RatedWarn(10, "create producer failed", zap.String("producerAccessMode", accessModeValue), zap.Error(err))
		select {
