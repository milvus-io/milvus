...
func (s *ErrSuite) TestWrap() {
	...
	idfCause := WrapErrIoTooManyRequests("bm25/stats", errors.New("throttled"))
	idfErr := WrapErrCollectionRuntimeNotReadyErr(idfCause, "repair reopened segment %d", 10)
	s.ErrorIs(idfErr, ErrCollectionRuntimeNotReady)
	s.ErrorIs(idfErr, ErrIoTooManyRequests)
	s.True(Status(idfErr).GetRetriable())
	s.Equal(Code(ErrCollectionRuntimeNotReady), Status(idfErr).GetCode())
	s.Equal(commonpb.ErrorCode_NotReadyServe, Status(idfErr).GetErrorCode())
	roundTripIDFErr := Error(Status(idfErr))
	s.ErrorIs(roundTripIDFErr, ErrCollectionRuntimeNotReady)
	s.True(IsRetryableErr(roundTripIDFErr))
	// Partition related
