...
func (mr *MetricsRequest) RegisterMetricsRequest(reqType string, action MetricsRequestAction) {
	...
	if ok {
		mlog.Info(context.TODO(), "metrics request type already exists", zap.String("reqType", reqType))
		return
	}
...
func (mr *MetricsRequest) ExecuteMetricsRequest(ctx context.Context, req *milvuspb.GetMetricsRequest) (string, error) {
	...
	if err != nil {
		mlog.Warn(ctx, "failed to parse metric type", zap.Error(err))
		return "", err
	}
	...
	if !ok {
		mr.lock.Unlock()
		mlog.Warn(ctx, "unimplemented metric request type", zap.String("req_type", reqType))
	...
	if err != nil {
		...
		mlog.Warn(ctx, msg, zap.Error(err))
		return "", err
	}
