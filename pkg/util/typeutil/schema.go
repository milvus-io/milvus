...
func ReserveSize(dst, src *schemapb.FieldData, size int64) {
	switch fieldType := src.Field.(type) {
	case *schemapb.FieldData_Scalars:
		if dst == nil || dst.GetScalars() == nil {
		...
		case *schemapb.ScalarField_ArrayData:
			dstScalar.Data = &schemapb.ScalarField_ArrayData{
				ArrayData: &schemapb.ArrayArray{
					Data:        make([]*schemapb.ScalarField, 0, size),
		...
		default:
			log.Error("Not supported field type", zap.String("field type", src.Type.String()))
		}
	case *schemapb.FieldData_Vectors:
		dim := fieldType.Vectors.Dim
		if dst == nil || dst.GetVectors() == nil {
			dst = &schemapb.FieldData{
				Type:      src.Type,
				FieldName: src.FieldName,
				FieldId:   src.FieldId,
