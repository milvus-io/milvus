...
// UpdateArrayFieldByColumnWithPathReplace replaces one existing element in
// each selected Array row. The outer replacement payload remains row-dense:
// every update row carries one non-null immediate element.
func UpdateArrayFieldByColumnWithPathReplace(
	...
	if baseArray.GetElementType() != updateArray.GetElementType() {
		return merr.WrapErrParameterInvalidMsg("PATH_REPLACE element type mismatch: %s vs %s", baseArray.GetElementType().String(), updateArray.GetElementType().String())
	}
	...
	for i, baseIndex := range baseIndices {
		...
		replaced, err := ReplaceArrayRowElement(baseRows[baseIndex], updateRows[updateIndex], index, baseArray.GetElementType())
		if err != nil {
			return err
		}
...
// ReplaceArrayRowElement returns a cloned Array row with one immediate value
// replaced. It never mutates base or update.
func ReplaceArrayRowElement(
	...
	if index < 0 || index >= baseLen {
		return nil, merr.WrapErrParameterInvalidMsg("PATH_REPLACE index %d is out of range for Array length %d", index, baseLen)
	}
	...
	if updateLen != 1 {
		return nil, merr.WrapErrParameterInvalidMsg("PATH_REPLACE operand must contain exactly one Array element")
	}

	out := proto.Clone(base).(*schemapb.ScalarField)
	switch elementType {
	case schemapb.DataType_Bool:
		out.GetBoolData().Data = replaceScalarArrayValue(out.GetBoolData().GetData(), update.GetBoolData().GetData(), index)
	case schemapb.DataType_Int8, schemapb.DataType_Int16, schemapb.DataType_Int32:
		out.GetIntData().Data = replaceScalarArrayValue(out.GetIntData().GetData(), update.GetIntData().GetData(), index)
	case schemapb.DataType_Int64:
		out.GetLongData().Data = replaceScalarArrayValue(out.GetLongData().GetData(), update.GetLongData().GetData(), index)
	case schemapb.DataType_Float:
		out.GetFloatData().Data = replaceScalarArrayValue(out.GetFloatData().GetData(), update.GetFloatData().GetData(), index)
	case schemapb.DataType_Double:
		out.GetDoubleData().Data = replaceScalarArrayValue(out.GetDoubleData().GetData(), update.GetDoubleData().GetData(), index)
	case schemapb.DataType_VarChar, schemapb.DataType_String:
		out.GetStringData().Data = replaceScalarArrayValue(out.GetStringData().GetData(), update.GetStringData().GetData(), index)
	default:
...
func replaceScalarArrayValue[T any](base, update []T, index int) []T {
	out := slices.Clone(base)
	out[index] = update[0]
	return out
}

// ReplaceVectorArrayRowElement returns a cloned ArrayOfVector row with one
// vector replaced. dim is expressed in vector dimensions, not bytes.
func ReplaceVectorArrayRowElement(
	...
	if index < 0 || index >= baseLen {
		return nil, merr.WrapErrParameterInvalidMsg("PATH_REPLACE index %d is out of range for ArrayOfVector length %d", index, baseLen)
	}
	...
	switch elementType {
	case schemapb.DataType_FloatVector:
		out.GetFloatVector().Data = replaceVectorArrayValue(out.GetFloatVector().GetData(), update.GetFloatVector().GetData(), index, int(dim))
	case schemapb.DataType_BinaryVector:
		width := int((dim + 7) / 8)
		out.Data = &schemapb.VectorField_BinaryVector{BinaryVector: replaceVectorArrayValue(out.GetBinaryVector(), update.GetBinaryVector(), index, width)}
	case schemapb.DataType_Float16Vector:
		width := int(dim * 2)
		out.Data = &schemapb.VectorField_Float16Vector{Float16Vector: replaceVectorArrayValue(out.GetFloat16Vector(), update.GetFloat16Vector(), index, width)}
	case schemapb.DataType_BFloat16Vector:
		width := int(dim * 2)
		out.Data = &schemapb.VectorField_Bfloat16Vector{Bfloat16Vector: replaceVectorArrayValue(out.GetBfloat16Vector(), update.GetBfloat16Vector(), index, width)}
	case schemapb.DataType_Int8Vector:
		width := int(dim)
		out.Data = &schemapb.VectorField_Int8Vector{Int8Vector: replaceVectorArrayValue(out.GetInt8Vector(), update.GetInt8Vector(), index, width)}
	default:
		return nil, merr.WrapErrParameterInvalidMsg("PATH_REPLACE does not support ArrayOfVector element type %s", elementType.String())
	}
	return out, nil
}

func replaceVectorArrayValue[T any](base, update []T, index, width int) []T {
	out := slices.Clone(base)
	copy(out[index*width:(index+1)*width], update)
	return out
}

// ScalarArrayRowElementCount validates a scalar Array row and returns its
// physical element count. PATH_REPLACE does not interpret per-element
// valid_data, so any such bitmap is rejected.
func ScalarArrayRowElementCount(row *schemapb.ScalarField, elementType schemapb.DataType) (int, error) {
	...
	case schemapb.DataType_Bool:
		if _, ok := row.GetData().(*schemapb.ScalarField_BoolData); !ok || row.GetBoolData() == nil {
			return 0, merr.WrapErrParameterInvalidMsg("Array row payload does not match element type %s", elementType.String())
		}
	...
	default:
		return 0, merr.WrapErrParameterInvalidMsg("PATH_REPLACE does not support Array element type %s", elementType.String())
	}
}

// VectorArrayRowElementCount validates an ArrayOfVector row and returns its
// physical vector count. PATH_REPLACE does not interpret per-element
// valid_data, so any such bitmap is rejected.
func VectorArrayRowElementCount(row *schemapb.VectorField, elementType schemapb.DataType, dim int64) (int, error) {
	if row == nil || dim <= 0 {
		return 0, merr.WrapErrParameterInvalidMsg("PATH_REPLACE requires a non-nil vector row and a positive dimension")
	}
	if len(row.GetValidData()) != 0 {
		return 0, merr.WrapErrParameterInvalidMsg("PATH_REPLACE does not support ArrayOfVector element valid_data")
	}
	var payloadLen, width int
	switch elementType {
	case schemapb.DataType_FloatVector:
		if _, ok := row.GetData().(*schemapb.VectorField_FloatVector); !ok || row.GetFloatVector() == nil {
			return 0, merr.WrapErrParameterInvalidMsg("ArrayOfVector row payload does not match element type %s", elementType.String())
		}
	...
	default:
		return 0, merr.WrapErrParameterInvalidMsg("PATH_REPLACE does not support ArrayOfVector element type %s", elementType.String())
	}
	if width <= 0 || payloadLen%width != 0 {
		return 0, merr.WrapErrParameterInvalidMsg("vector payload length %d is not divisible by element width %d", payloadLen, width)
	}
