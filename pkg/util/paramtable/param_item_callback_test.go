...
func TestParamItem_FormattedDispatcherEvents(t *testing.T) {
	manager := config.NewManager()
	param := &ParamItem{
	...
	param.RegisterCallback(func(_ context.Context, _ string, oldValue, newValue string) error {
		transitions = append(transitions, [2]string{oldValue, newValue})
		return nil
	})

	formattedKey := "commonsecurityadminauthenabled"
	...
	assert.Equal(t, [][2]string{{"false", "true"}, {"true", "false"}}, transitions)
}

func TestParamItem_DeleteEventUsesEffectiveFallback(t *testing.T) {
	manager := config.NewManager()
	manager.SetConfig("test.param", "primary")
	manager.SetConfig("test.fallback", "fallback")
	param := &ParamItem{
		Key:          "test.param",
		FallbackKeys: []string{"test.fallback"},
		DefaultValue: "default",
	}
	param.Init(manager)

	var oldValue, newValue string
	param.RegisterCallback(func(_ context.Context, _ string, old, new string) error {
		oldValue, newValue = old, new
		return nil
	})

	manager.ResetConfig("test.param")
	manager.Dispatcher.Dispatch(&config.Event{
	...
	assert.Equal(t, "primary", oldValue)
	assert.Equal(t, "fallback", newValue)
}
