...
func recoverConfirmator(etcdCli *clientv3.Client, metaRoot, configRoot string, items []*ParamItem) (*confirmator, error) {
	...
	for _, item := range items {
		...
		if err := vg.registerGate(item.Key, item.VersionGateSwitcher); err != nil {
			log.Warn("version gate: register gate failed, skip", zap.String("key", item.Key), zap.Error(err))
			continue
		}
	}
	if len(vg.gates) == 0 {
	...
	go func() {
		if err := vg.start(context.TODO()); err != nil {
			log.Warn("version gate: start confirmator failed", zap.Error(err))
		}
	}()
	return vg, nil
}
...
func (c *confirmator) start(ctx context.Context) error {
	...
	for _, g := range c.gates {
		...
		if c.gateResolvedLocked(g) {
			g.resolved = true
			log.Info("version gate: gate resolved at startup (explicit config or already flipped)",
	...
	if pending == 0 {
		log.Info("version gate: all gates resolved at startup, confirmator exits",
	...
	for {
		...
		for resp := range wch {
			if err := resp.Err(); err != nil {
				...
				log.Warn("version gate: session watch error, rescan", zap.Error(err))
				break
			}
			...
			if _, err := c.reloadSessions(ctx); err != nil {
				log.Warn("version gate: session rescan failed", zap.Error(err))
			}
		}
		if ctx.Err() != nil {
			return
	...
	for {
		...
		case <-ticker.C:
			if c.processGates(ctx) {
				...
				log.Info("version gate: all gates resolved, confirmator exits")
				c.cancel()
				return
			}
		}
...
func (c *confirmator) processGates(ctx context.Context) bool {
	...
	for _, g := range c.gates {
		...
		case above && !g.wasAbove:
			g.armedAt = now
			log.Info("version gate: cluster above gate version, start stability window",
				zap.String("key", g.key), zap.String("gateVersion", g.switcher.GateVersion),
				zap.String("minOnline", c.minOnline.String()))
		case !above && g.wasAbove:
			g.armedAt = time.Time{}
			log.Warn("version gate: cluster below gate version, reset stability window",
				zap.String("key", g.key), zap.String("gateVersion", g.switcher.GateVersion),
				zap.String("minOnline", c.minOnline.String()))
		}
		g.wasAbove = above
...
func (c *confirmator) flip(ctx context.Context, g *gate) error {
	...
	default:
		...
		log.Info("version gate: explicit etcd config value wins, skip flip",
	...
	if !txnResp.Succeeded {
		...
		if present && cur != g.switcher.EnableAutoSwitchValue {
			log.Info("version gate: concurrent writer resolved the gate, skip flip",
...
func refreshLocalConfig() {
	...
	if err := etcdSource.RefreshConfigurationsLinearizable(); err != nil {
		log.Warn("version gate: refresh local config failed", zap.Error(err))
	}
}
