...
func TestComponentParam(t *testing.T) {
	...
	t.Run("test streamingConfig", func(t *testing.T) {
		...
		assert.Equal(t, int64(64*1024*1024), params.StreamingCfg.IdempotencyRetainedBytesPerVChannel.GetAsSize())
		assert.Equal(t, int64(4*1024*1024), params.StreamingCfg.IdempotencyChunkTargetBytes.GetAsSize())
		assert.Equal(t, 10*time.Second, params.StreamingCfg.IdempotencyPersistInterval.GetAsDurationByParse())
		assert.Equal(t, 5, params.StreamingCfg.IdempotencyManifestChunkInterval.GetAsInt())
		...
		params.Save(params.StreamingCfg.IdempotencyRetainedBytesPerVChannel.Key, "134217728")
		params.Save(params.StreamingCfg.IdempotencyChunkTargetBytes.Key, "8388608")
		params.Save(params.StreamingCfg.IdempotencyPersistInterval.Key, "2s")
		params.Save(params.StreamingCfg.IdempotencyManifestChunkInterval.Key, "8")
		...
		assert.Equal(t, int64(128*1024*1024), params.StreamingCfg.IdempotencyRetainedBytesPerVChannel.GetAsSize())
		assert.Equal(t, int64(8*1024*1024), params.StreamingCfg.IdempotencyChunkTargetBytes.GetAsSize())
		assert.Equal(t, 2*time.Second, params.StreamingCfg.IdempotencyPersistInterval.GetAsDurationByParse())
		assert.Equal(t, 8, params.StreamingCfg.IdempotencyManifestChunkInterval.GetAsInt())
