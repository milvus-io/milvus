...
func TestComponentParam(t *testing.T) {
	...
	t.Run("test proxyConfig", func(t *testing.T) {
		...
		assert.Equal(t, Params.ReplicaSelectionPolicy.GetValue(), "look_aside")
		assert.False(t, Params.QueryTrafficRoutingEnabled.GetAsBool())
		params.Save(Params.QueryTrafficRoutingEnabled.Key, "true")
		assert.True(t, Params.QueryTrafficRoutingEnabled.GetAsBool())
		params.Save(Params.QueryTrafficRoutingEnabled.Key, "false")
		assert.False(t, Params.QueryTrafficRoutingEnabled.GetAsBool())
		assert.Empty(t, Params.QueryTrafficRoutingRules.GetValue())
		params.Save(Params.QueryTrafficRoutingRules.Key, `[{"name":"fallback"}]`)
		assert.Equal(t, `[{"name":"fallback"}]`, Params.QueryTrafficRoutingRules.GetValue())
		params.Save(Params.QueryTrafficRoutingRules.Key, "")
		assert.Empty(t, Params.QueryTrafficRoutingRules.GetValue())
