...
func TestComponentParam(t *testing.T) {
	...
	t.Run("test streamingConfig", func(t *testing.T) {
		...
		assert.Equal(t, int64(64*1024*1024), params.StreamingCfg.IdempotencyMinRetainedBytesPerVChannel.GetAsSize())
		assert.Equal(t, 256, params.StreamingCfg.IdempotencyMaxRetainedChunks.GetAsInt())
		assert.Equal(t, 10*time.Minute, params.StreamingCfg.IdempotencyRetentionTTL.GetAsDurationByParse())
		assert.Equal(t, 10*time.Second, params.StreamingCfg.IdempotencyGCInterval.GetAsDurationByParse())
		...
		params.Save(params.StreamingCfg.IdempotencyMinRetainedBytesPerVChannel.Key, "134217728")
		params.Save(params.StreamingCfg.IdempotencyMaxRetainedChunks.Key, "8")
		params.Save(params.StreamingCfg.IdempotencyRetentionTTL.Key, "30m")
		...
		assert.Equal(t, int64(128*1024*1024), params.StreamingCfg.IdempotencyMinRetainedBytesPerVChannel.GetAsSize())
		assert.Equal(t, 8, params.StreamingCfg.IdempotencyMaxRetainedChunks.GetAsInt())
		assert.Equal(t, 30*time.Minute, params.StreamingCfg.IdempotencyRetentionTTL.GetAsDurationByParse())
		assert.Equal(t, 2*time.Second, params.StreamingCfg.IdempotencyGCInterval.GetAsDurationByParse())
