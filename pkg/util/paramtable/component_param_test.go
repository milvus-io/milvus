...
func TestComponentParam(t *testing.T) {
	...
	t.Run("test streamingConfig", func(t *testing.T) {
		...
		assert.Equal(t, 10*time.Second, params.StreamingCfg.WALAppendBatchCooldownDuration.GetAsDurationByParse())
		...
		params.Save(params.StreamingCfg.WALAppendBatchMaxMessageCount.Key, "8")
		...
		assert.Equal(t, 8, params.StreamingCfg.WALAppendBatchMaxMessageCount.GetAsInt())
		assert.Equal(t, 5*time.Millisecond, params.StreamingCfg.WALAppendBatchMaxDelay.GetAsDurationByParse())
