...
func TestComponentParam(t *testing.T) {
	...
	t.Run("test proxyConfig", func(t *testing.T) {
		...
		assert.Equal(t, int64(4), Params.DQLBackpressureSlowdownMinConcurrency.GetAsInt64())
		assert.Equal(t, 0.5, Params.DQLBackpressureSlowdownRatio.GetAsFloat())
		assert.Equal(t, time.Second, Params.DQLBackpressureSlowdownInterval.GetAsDurationByParse())
		assert.Equal(t, time.Second, Params.DQLBackpressureRecoverInterval.GetAsDurationByParse())
		assert.Equal(t, int64(4), Params.DQLBackpressureRecoverStep.GetAsInt64())
		assert.Equal(t, 5*time.Second, Params.DQLBackpressureRecoverQuietPeriod.GetAsDurationByParse())
		params.Save(Params.DQLBackpressureSlowdownRatio.Key, "1.2")
		assert.Equal(t, 0.5, Params.DQLBackpressureSlowdownRatio.GetAsFloat())
		params.Reset(Params.DQLBackpressureSlowdownRatio.Key)
		params.Save(Params.DQLBackpressureSlowdownInterval.Key, "500ms")
		assert.Equal(t, 500*time.Millisecond, Params.DQLBackpressureSlowdownInterval.GetAsDurationByParse())
		params.Reset(Params.DQLBackpressureSlowdownInterval.Key)
		params.Save(Params.DQLBackpressureRecoverInterval.Key, "1500ms")
		assert.Equal(t, 1500*time.Millisecond, Params.DQLBackpressureRecoverInterval.GetAsDurationByParse())
		params.Reset(Params.DQLBackpressureRecoverInterval.Key)
		params.Save(Params.DQLBackpressureRecoverStep.Key, "0")
		assert.Equal(t, int64(4), Params.DQLBackpressureRecoverStep.GetAsInt64())
		params.Reset(Params.DQLBackpressureRecoverStep.Key)
		params.Save(Params.DQLBackpressureRecoverQuietPeriod.Key, "500ms")
		assert.Equal(t, 500*time.Millisecond, Params.DQLBackpressureRecoverQuietPeriod.GetAsDurationByParse())
		params.Reset(Params.DQLBackpressureRecoverQuietPeriod.Key)
