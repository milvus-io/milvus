...
func TestSensitivePulsarWebAddressConfigLogs(t *testing.T) {
	// 2.6 derives WebAddress from Address and WebPort even when a raw
	// webaddress is supplied. Preserve that behavior and protect the raw input.
	...
	for _, test := range []struct {
		...
		{"explicit HTTPS", "https://private-user:password-canary@private-broker.invalid/admin", fallback},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			t.Cleanup(base.Manager().Close)
			require.NoError(t, base.Save("pulsar.address", "pulsar://broker-0.invalid:6650"))
