	...
	p.CompressionLevel = ParamItem{
		...
		Formatter: func(v string) string {
			...
			case "fastest", "default", "better", "best":
				return normalized
			default:
				log.Warn("Failed to parse grpc.client.compressionLevel, set to default",
					zap.String("role", p.Domain),
					zap.String("grpc.client.compressionLevel", v))
				return DefaultCompressionLevel
			}
		},
	...
	p.CompressionAlgorithm = ParamItem{
		...
		Formatter: func(v string) string {
			...
			case "zstd", "snappy", "s2":
				return normalized
			default:
				log.Warn("Failed to parse grpc.client.compressionAlgorithm, set to default",
					zap.String("role", p.Domain),
					zap.String("grpc.client.compressionAlgorithm", v))
				return DefaultCompressionAlgorithm
			}
		},
...
Change this only after every node in the cluster has been upgraded.`,
	...
	p.CompressionCRC = ParamItem{
		...
		Formatter: func(v string) string {
			...
			_, err := strconv.ParseBool(v)
			if err != nil {
				log.Warn("Failed to parse grpc.client.compressionCRC, set to default",
					zap.String("role", p.Domain),
					zap.String("grpc.client.compressionCRC", v))
				return compressionCRC
			}
	...
	p.CompressionConcurrency = ParamItem{
		...
		Formatter: func(v string) string {
			...
			if err != nil || n < 1 || n > MaxCompressionConcurrency {
				log.Warn("Failed to parse grpc.client.compressionConcurrency, set to default",
					zap.String("role", p.Domain),
					zap.String("grpc.client.compressionConcurrency", v))
				return compressionConcurrency
			}
			return v
		},
