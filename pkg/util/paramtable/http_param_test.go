...
func TestHTTPConfig_Init(t *testing.T) {
	...
	assert.Equal(t, time.Duration(0), cfg.StreamIdleTimeout.GetAsDurationByParse())
...
func TestHTTPConfig_TimeoutOverrides(t *testing.T) {
	...
	base.Save("proxy.http.streamIdleTimeout", "10s")
	base.Save("proxy.http.idleTimeout", "11s")
	...
	assert.Equal(t, 10*time.Second, cfg.StreamIdleTimeout.GetAsDurationByParse())
	assert.Equal(t, 11*time.Second, cfg.IdleTimeout.GetAsDurationByParse())
