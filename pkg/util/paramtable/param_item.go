...
func (pi *ParamItem) gateValue(v string) string {
	...
	if pi.gateReadinessKey != "" {
		// LookupConfig, not GetConfig: the readiness marker is absent until the
		// gate opens, which is the steady state for every cluster that has not
		// switched yet, and GetConfig's miss path allocates a stack-capturing
		// errors.Wrap. These items bypass the typed value cache (see
		// getCachedValue), so this runs on every GetAs* — including per-segment
		// datacoord paths that hold meta.segMu.
		if ready, ok := pi.manager.LookupConfig(pi.gateReadinessKey); ok && ready == "true" {
			return pi.VersionGateSwitcher.TargetValue
		}
	}
