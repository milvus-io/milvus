...
func (pi *ParamItem) handleConfigChange(event *config.Event) {
	...
	if err := pi.callback(context.Background(), pi.Key, oldValue, newValue); err != nil {
		...
		log.Ctx(context.TODO()).Error("param change callback failed",
			zap.String("key", pi.Key),
			zap.String("oldValue", logOldValue),
			zap.String("newValue", logNewValue),
			zap.String("error", config.RedactedValue))
	} else {
		log.Ctx(context.TODO()).Info("param value changed",
			zap.String("key", pi.Key),
			zap.String("oldValue", logOldValue),
			zap.String("newValue", logNewValue))
	}
