...
writeRetryInitialInterval, otherwise the effective cap is raised to twice the initial interval.`,
	...
	p.JSONStatsBuildMaxWorkers = ParamItem{
		...
		Version:      "3.0.1",
		DefaultValue: "4",
		Doc:          "Maximum number of process-wide JSON Stats Build workers. The effective value is capped by the cgroup-aware CPU count. Values less than or equal to 0 use the safe automatic limit min(CPU, 4).",
		Export:       true,
	}
