...
//
// Note: this list is intentionally narrower than the Sensitive list. No
// credential-bearing key belongs here — ProcessImmutableConfigs persists an
// Immutable key's current value into etcd in cleartext and pins it against
// later file/env edits, so marking a secret Immutable would both disclose and
// freeze it. Credentials (minio.accessKeyID/secretAccessKey,
// etcd.auth.password, common.security.defaultRootPassword) are therefore
// Sensitive only, and TestNoCredentialIsImmutable enforces that.
