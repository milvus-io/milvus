...
func ValidateMmapTypeParams(indexType IndexType, typeParams map[string]string) error {
	mmapIndex, ok := typeParams[common.MmapIndexKey]
	if ok {
		enable, err := strconv.ParseBool(mmapIndex)
		if err != nil {
			return fmt.Errorf("invalid %s value: %s, expected: true, false", common.MmapIndexKey, mmapIndex)
		}
	...
	mmapEnable, ok := typeParams[common.MmapEnabledKey]
	if ok {
		enable, err := strconv.ParseBool(mmapEnable)
		if err != nil {
			return fmt.Errorf("invalid %s value: %s, expected: true, false", common.MmapEnabledKey, mmapIndex)
		}
		mmapSupport := indexType == AutoIndex || IsVectorMmapIndex(indexType) || IsScalarMmapIndex(indexType)
		if enable && !mmapSupport {
			return fmt.Errorf("index type %s does not support mmap", indexType)
