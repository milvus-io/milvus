...
func TestInitEtcdServerInvalidConfig(t *testing.T) {
	...
	if os.Getenv(childEnv) != "1" {
		...
		require.NoError(t, err)
		// #nosec G204 -- Re-execute this test binary with fixed arguments to isolate sync.Once.
		cmd := exec.Command(executable, "-test.run=^TestInitEtcdServerInvalidConfig$", "-test.v")
