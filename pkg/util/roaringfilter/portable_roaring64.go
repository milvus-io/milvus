...
func portableBodyError(format string, args ...any) error {
	return merr.WrapErrParameterInvalidMsg("invalid portable roaring64 body: "+format, args...)
}
...
func validatePortableRoaring32(data []byte) (int, uint64, uint64, error) {
	...
	if uint16(cookie) == portableCookieRun {
		...
		if usedBits := containerCount % 8; usedBits != 0 {
			validMask := byte((1 << usedBits) - 1)
			if runBitmap[len(runBitmap)-1]&^validMask != 0 {
				return 0, 0, 0, portableBodyError("run bitmap has bits beyond container count")
			}
		}
		...
		cursor += 4
		if containerCount == 0 || containerCount > 1<<16 {
	...
	for i := uint32(0); i < containerCount; i++ {
		if hasOffsets {
			declaredOffset := binary.LittleEndian.Uint32(
				data[offsetStart+int(i)*4 : offsetStart+int(i+1)*4])
			if uint64(declaredOffset) != uint64(cursor) {
				return 0, 0, 0, portableBodyError(
					"container %d offset %d does not match actual %d", i, declaredOffset, cursor)
			}
		}
...
func validatePortableRunContainer(data []byte, cursor int, cardinality uint32) (int, error) {
	...
	for i := uint32(0); i < runCount; i++ {
		...
		if i > 0 && start <= previousEnd+1 {
			return 0, portableBodyError("run intervals overlap, touch, or are out of order")
		}
		previousEnd = end
