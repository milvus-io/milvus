...
const (
	...
	// SplitShard is the single WAL record of a shard split's write switch. It
	// is broadcast to the source vchannel, the target vchannels and the
	// control channel; each replica plays the role its vchannel has in the
	// header: the source replica fences the source (its time tick is T_switch),
	// a target replica is the genesis of that target vchannel, the control
	// channel replica only orders the ack callback.
	MessageType_SplitShard MessageType = 50
...
type InsertMessageHeader struct {
	...
	IdempotentResult *IdempotentInsertResult `protobuf:"bytes,4,opt,name=idempotent_result,json=idempotentResult,proto3,oneof" json:"idempotent_result,omitempty"`
}
...
func (x *InsertMessageHeader) GetIdempotentResult() *IdempotentInsertResult {
	if x != nil {
...
type IdempotentInsertResult struct {
	...
	RowOffsets []uint32      `protobuf:"varint,1,rep,packed,name=row_offsets,json=rowOffsets,proto3" json:"row_offsets,omitempty"`
	Ids        *schemapb.IDs `protobuf:"bytes,2,opt,name=ids,proto3" json:"ids,omitempty"`
}

func (x *IdempotentInsertResult) Reset() {
	*x = IdempotentInsertResult{}
	if protoimpl.UnsafeEnabled {
...
func (x *IdempotentInsertResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IdempotentInsertResult) ProtoMessage() {}

func (x *IdempotentInsertResult) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use IdempotentInsertResult.ProtoReflect.Descriptor instead.
func (*IdempotentInsertResult) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{12}
}

func (x *IdempotentInsertResult) GetRowOffsets() []uint32 {
	if x != nil {
		return x.RowOffsets
	}
	return nil
...
func (x *IdempotentInsertResult) GetIds() *schemapb.IDs {
	if x != nil {
		return x.Ids
	}
	return nil
...
// SplitShardMessageHeader is the header of the split shard message, the one
// WAL record of a shard split's write switch. One broadcast carries identical
// replicas to the source, the two targets and the control channel; a consumer
// decides what a replica means by where its own vchannel appears:
//   - source_vchannel: the write fence. After this message the source never
//     accepts DML again; its time tick is T_switch.
//   - target_vchannels: the genesis of that target vchannel. The body carries
//     the schema and the header the partition snapshot the target registers
//     with.
//   - the control channel: no effect, it orders the ack callback.
//
// The residues each target owns and the routing modulus are NOT here: the
// body's routing post-image is their only copy.
type SplitShardMessageHeader struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	CollectionId int64 `protobuf:"varint,1,opt,name=collection_id,json=collectionId,proto3" json:"collection_id,omitempty"`
	// the unique split task id allocated by the coordinator,
	// used for idempotency and split task correlation. Never zero.
	SplitTaskId int64 `protobuf:"varint,2,opt,name=split_task_id,json=splitTaskId,proto3" json:"split_task_id,omitempty"`
	// the vchannel this split fences.
	...
	// the vchannels this split creates.
	TargetVchannels []string `protobuf:"bytes,4,rep,name=target_vchannels,json=targetVchannels,proto3" json:"target_vchannels,omitempty"`
	// the growing segments the source streamingnode auto-flushed while
	// processing the SOURCE replica (embedded by the handler, the same way the
	// AlterCollection schema-change path embeds its flushed segments). Empty on
	// every other replica as built. Only a FIRST fence fills it: a same-task
	// re-fence appends the header as received. A secondary clears the field
	// when it remaps a replicated SplitShard, so a re-fence record there never
	// names the primary's segments. No consumer reads the ids off the record;
	// the flusher seals every growing segment of the vchannel instead.
	FlushedSegmentIds []int64 `protobuf:"varint,5,rep,packed,name=flushed_segment_ids,json=flushedSegmentIds,proto3" json:"flushed_segment_ids,omitempty"`
	// the collection's partitions at the time of the split, registered on every
	// target genesis. Exact only because the issuer must start the broadcast
	// under the collection's ExclusiveCollectionName resource key, which keeps
	// collection DDL out of the write switch; nothing in the message enforces
	// that.
	PartitionIds []int64 `protobuf:"varint,6,rep,packed,name=partition_ids,json=partitionIds,proto3" json:"partition_ids,omitempty"`
}

func (x *SplitShardMessageHeader) Reset() {
	*x = SplitShardMessageHeader{}
	if protoimpl.UnsafeEnabled {
...
func (x *SplitShardMessageHeader) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SplitShardMessageHeader) ProtoMessage() {}

func (x *SplitShardMessageHeader) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[19]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use SplitShardMessageHeader.ProtoReflect.Descriptor instead.
func (*SplitShardMessageHeader) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{19}
}

func (x *SplitShardMessageHeader) GetCollectionId() int64 {
	if x != nil {
		return x.CollectionId
	}
	return 0
...
func (x *SplitShardMessageHeader) GetSplitTaskId() int64 {
	if x != nil {
...
func (x *SplitShardMessageHeader) GetSourceVchannel() string {
	if x != nil {
...
func (x *SplitShardMessageHeader) GetTargetVchannels() []string {
	if x != nil {
		return x.TargetVchannels
	}
	return nil
}

func (x *SplitShardMessageHeader) GetFlushedSegmentIds() []int64 {
	if x != nil {
...
func (x *SplitShardMessageHeader) GetPartitionIds() []int64 {
	if x != nil {
...
// SplitShardMessageBody carries what the two consumers that need more than the
// header read from the message.
type SplitShardMessageBody struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// the collection schema for the target genesis, in the shape
	// CreateCollection already uses so the three genesis consumers share one
	// parser (messageutil.MustGetSchemaFromCreateCollectionMessageBody).
	Genesis *msgpb.CreateCollectionRequest `protobuf:"bytes,1,opt,name=genesis,proto3" json:"genesis,omitempty"`
	// the routing post-image the rootcoord ack callback commits to the
	// collection meta once every replica has landed: the grown vchannel list,
	// every shard's state and residues, the modulus and shard_by. Carried whole
	// so a replay never depends on mutable meta.
	Routing *AlterCollectionMessageUpdates `protobuf:"bytes,2,opt,name=routing,proto3" json:"routing,omitempty"`
}

func (x *SplitShardMessageBody) Reset() {
	*x = SplitShardMessageBody{}
	if protoimpl.UnsafeEnabled {
...
func (x *SplitShardMessageBody) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SplitShardMessageBody) ProtoMessage() {}

func (x *SplitShardMessageBody) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[20]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use SplitShardMessageBody.ProtoReflect.Descriptor instead.
func (*SplitShardMessageBody) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{20}
}

func (x *SplitShardMessageBody) GetGenesis() *msgpb.CreateCollectionRequest {
	if x != nil {
		return x.Genesis
	}
	return nil
...
func (x *SplitShardMessageBody) GetRouting() *AlterCollectionMessageUpdates {
	if x != nil {
		return x.Routing
	}
	return nil
...
type AlterCollectionMessageUpdates struct {
	...
	// Shard-split routing commit: the full post-split vchannel topology and routing
	// facts, applied atomically under the shard-split routing field masks. The
	// virtual/physical channel lists are the complete new lists (existing shards plus
	// the split targets), and shard_infos is parallel to virtual_channel_names.
	VirtualChannelNames  []string                        `protobuf:"bytes,10,rep,name=virtual_channel_names,json=virtualChannelNames,proto3" json:"virtual_channel_names,omitempty"`
	PhysicalChannelNames []string                        `protobuf:"bytes,11,rep,name=physical_channel_names,json=physicalChannelNames,proto3" json:"physical_channel_names,omitempty"`
	ShardInfos           []*schemapb.CollectionShardInfo `protobuf:"bytes,12,rep,name=shard_infos,json=shardInfos,proto3" json:"shard_infos,omitempty"`
	// The collection's routing modulus after the split. A split that halves a shard
	// down to its last residue doubles it, which re-expresses every other shard's
	// residues too — so the modulus has to be committed with shard_infos, not apart
	// from it.
	RoutingModulus uint64 `protobuf:"varint,13,opt,name=routing_modulus,json=routingModulus,proto3" json:"routing_modulus,omitempty"`
	// The routing-key expression, back-filled at the first split of a collection
	// created before shard_by existed. Empty leaves it undeclared.
	ShardBy string `protobuf:"bytes,14,opt,name=shard_by,json=shardBy,proto3" json:"shard_by,omitempty"`
	// The shard split this routing commit belongs to. The adoption commit --- the
	// one whose post-image delists a fenced source --- is gated on the LOCAL
	// datacoord reporting that split's sources drained, so every cluster that
	// replays this message can ask its own datacoord about the same task.
	SplitTaskId int64 `protobuf:"varint,15,opt,name=split_task_id,json=splitTaskId,proto3" json:"split_task_id,omitempty"`
}
...
func (x *AlterCollectionMessageUpdates) GetVirtualChannelNames() []string {
	if x != nil {
...
func (x *AlterCollectionMessageUpdates) GetPhysicalChannelNames() []string {
	if x != nil {
...
func (x *AlterCollectionMessageUpdates) GetShardInfos() []*schemapb.CollectionShardInfo {
	if x != nil {
		return x.ShardInfos
	}
	return nil
...
func (x *AlterCollectionMessageUpdates) GetRoutingModulus() uint64 {
	if x != nil {
...
func (x *AlterCollectionMessageUpdates) GetShardBy() string {
	if x != nil {
		return x.ShardBy
	}
	return ""
...
func (x *AlterCollectionMessageUpdates) GetSplitTaskId() int64 {
	if x != nil {
		return x.SplitTaskId
	}
	return 0
...
// ImportIDRangeMessageBody carries the per-file ID ranges allocated from the exact
// post-preimport row counts. The map key is the file's zero-based position in
// ImportMsg.Files / job.Files order. The range size is the primary's exact
// post-preimport row count, so it doubles as the cross-cluster divergence authority
// and no separate row count is carried.
type ImportIDRangeMessageBody struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	IdRanges map[int64]*commonpb.IDRange `protobuf:"bytes,1,rep,name=id_ranges,json=idRanges,proto3" json:"id_ranges,omitempty" protobuf_key:"varint,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}
...
func (x *ImportIDRangeMessageBody) GetIdRanges() map[int64]*commonpb.IDRange {
	if x != nil {
		return x.IdRanges
	}
	return nil
...
// SplitShardExtraResponse is the extra response of the SOURCE replica of a
// SplitShard message: the fence tick (T_switch) recorded for the source
// vchannel. It is the tick of the task's FIRST fence record -- the replica's own
// tick on the first fence, the recorded one on a same-task re-fence -- so a
// re-driven fence never moves T_switch. The source handler also stamps it on
// the WAL record, so a consumer-side ack reports the same value as the append.
type SplitShardExtraResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	SplitTimeTick uint64 `protobuf:"varint,1,opt,name=split_time_tick,json=splitTimeTick,proto3" json:"split_time_tick,omitempty"`
}

func (x *SplitShardExtraResponse) Reset() {
	*x = SplitShardExtraResponse{}
	if protoimpl.UnsafeEnabled {
...
func (x *SplitShardExtraResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SplitShardExtraResponse) ProtoMessage() {}

func (x *SplitShardExtraResponse) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[113]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use SplitShardExtraResponse.ProtoReflect.Descriptor instead.
func (*SplitShardExtraResponse) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{113}
}

func (x *SplitShardExtraResponse) GetSplitTimeTick() uint64 {
	if x != nil {
...
type BroadcastHeader struct {
	...
	// the WAL before any target replica so that a fresh time tick taken on a
	// target pchannel is greater than T_switch. The list is a subset of
	// vchannels; the control channel is never in it.
	AppendFirstVchannels []string `protobuf:"bytes,5,rep,name=append_first_vchannels,json=appendFirstVchannels,proto3" json:"append_first_vchannels,omitempty"`
}
...
func (x *BroadcastHeader) GetAppendFirstVchannels() []string {
	if x != nil {
		return x.AppendFirstVchannels
	}
	return nil
}
...
type RLSPolicyMetadata struct {
	...
	PolicyType  milvuspb.RowPolicyType     `protobuf:"varint,3,opt,name=policy_type,json=policyType,proto3,enum=milvus.proto.milvus.RowPolicyType" json:"policy_type,omitempty"`
	Actions     []milvuspb.RowPolicyAction `protobuf:"varint,4,rep,packed,name=actions,proto3,enum=milvus.proto.milvus.RowPolicyAction" json:"actions,omitempty"`
...
func (x *RLSPolicyMetadata) GetPolicyType() milvuspb.RowPolicyType {
	if x != nil {
		return x.PolicyType
	}
...
func (x *RLSPolicyMetadata) GetActions() []milvuspb.RowPolicyAction {
	if x != nil {
		return x.Actions
	}
	return nil
...
var file_messages_proto_rawDesc = []byte{
	...
	0x6f, 0x1a, 0x08, 0x72, 0x67, 0x2e, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x1a, 0x20, 0x67, 0x6f, 0x6f,
	0x67, 0x6c, 0x65, 0x2f, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x62, 0x75, 0x66, 0x2f, 0x66, 0x69, 0x65,
	0x6c, 0x64, 0x5f, 0x6d, 0x61, 0x73, 0x6b, 0x2e, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x22, 0xb2, 0x01,
	0x0a, 0x07, 0x4d, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x12, 0x18, 0x0a, 0x07, 0x70, 0x61, 0x79,
	0x6c, 0x6f, 0x61, 0x64, 0x18, 0x01, 0x20, 0x01, 0x28, 0x0c, 0x52, 0x07, 0x70, 0x61, 0x79, 0x6c,
	0x6f, 0x61, 0x64, 0x12, 0x4e, 0x0a, 0x0a, 0x70, 0x72, 0x6f, 0x70, 0x65, 0x72, 0x74, 0x69, 0x65,
	0x73, 0x18, 0x02, 0x20, 0x03, 0x28, 0x0b, 0x32, 0x2e, 0x2e, 0x6d, 0x69, 0x6c, 0x76, 0x75, 0x73,
	...
	0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x48, 0x65, 0x61, 0x64, 0x65, 0x72, 0x22, 0x1e, 0x0a, 0x1c,
	0x52, 0x65, 0x63, 0x6f, 0x76, 0x65, 0x72, 0x79, 0x42, 0x61, 0x72, 0x72, 0x69, 0x65, 0x72, 0x4d,
	0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x48, 0x65, 0x61, 0x64, 0x65, 0x72, 0x22, 0x1c, 0x0a, 0x1a,
	0x52, 0x65, 0x63, 0x6f, 0x76, 0x65, 0x72, 0x79, 0x42, 0x61, 0x72, 0x72, 0x69, 0x65, 0x72, 0x4d,
	0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x42, 0x6f, 0x64, 0x79, 0x22, 0
x74, 0x61, 0x64, 0x61, 0x74, 0x61, 0x2a, 0xc7, 0x08, 0x0a, 0x0b, 0x4d, 0x65, 0x73, 0x73, 0x61,
	...
	0x6d, 0x70, 0x6f, 0x72, 0x74, 0x10, 0x2e, 0x12, 0x14, 0x0a, 0x10, 0x41, 0x6c, 0x74, 0x65, 0x72,
	0x52, 0x4c, 0x53, 0x4d, 0x65, 0x74, 0x61, 0x64, 0x61, 0x74, 0x61, 0x10, 0x2f, 0x12, 0x13, 0x0a,
	0x0f, 0x44, 0x72, 0x6f, 0x70, 0x52, 0x4c, 0x53, 0x4d, 0x65, 0x74, 0x61, 0x64, 0x61, 0x74, 0x61,
	0x10, 0x30, 0x12, 0x11, 0x0a, 0x0d, 0x49, 0x6d, 0x70, 0x6f, 0x72, 0x74, 0x49, 0x44, 0x52, 0x61,
	0x6e, 0x67, 0x65, 0x10, 0x31, 0x12, 0x0e, 0x0a, 0x0a, 0x53, 0x70, 0x6c, 0x69, 0x74, 0x53, 0x68,
	0x61, 0x72, 0x64, 0x10, 0x32, 0x12, 0x0d, 0x0a, 0x08, 0x41, 0x6c, 0x74, 0x65, 0x72, 0x57, 0x41,
...
var file_messages_proto_msgTypes = make([]protoimpl.MessageInfo, 141)
var file_messages_proto_goTypes = []interface{}{
	...
	(*IdempotentInsertResult)(nil),                 // 15: milvus.proto.messages.IdempotentInsertResult
	(*PartitionSegmentAssignment)(nil),             // 16: milvus.proto.messages.PartitionSegmentAssignment
	(*SegmentAssignment)(nil),                      // 17: milvus.proto.messages.SegmentAssignment
	(*DeleteMessageHeader)(nil),                    // 18: milvus.proto.messages.DeleteMessageHeader
	(*FlushMessageHeader)(nil),                     // 19: milvus.proto.messages.FlushMessageHeader
	(*CreateSegmentMessageHeader)(nil),             // 20: milvus.proto.messages.CreateSegmentMessageHeader
	(*ManualFlushMessageHeader)(nil),               // 21: milvus.proto.messages.ManualFlushMessageHeader
	(*SplitShardMessageHeader)(nil),                // 22: milvus.proto.messages.SplitShardMessageHeader
	(*SplitShardMessageBody)(nil),                  // 23: milvus.proto.messages.SplitShardMessageBody
	(*CreateCollectionMessageHeader)(nil),          // 24: milvus.proto.messages.CreateCollectionMessageHeader
	(*DropCollectionMessageHeader)(nil),            // 25: milvus.proto.messages.DropCollectionMessageHeader
	(*CreatePartitionMessageHeader)(nil),           // 26: milvus.proto.messages.CreatePartitionMessageHeader
	(*DropPartitionMessageHeader)(nil),             // 27: milvus.proto.messages.DropPartitionMessageHeader
	(*AlterReplicateConfigMessageHeader)(nil),      // 28: milvus.proto.messages.AlterReplicateConfigMessageHeader
	(*AlterReplicateConfigMessageBody)(nil),        // 29: milvus.proto.messages.AlterReplicateConfigMessageBody
	(*BeginTxnMessageHeader)(nil),                  // 30: milvus.proto.messages.BeginTxnMessageHeader
	(*CommitTxnMessageHeader)(nil),                 // 31: milvus.proto.messages.CommitTxnMessageHeader
	(*RollbackTxnMessageHeader)(nil),               // 32: milvus.proto.messages.RollbackTxnMessageHeader
	(*TxnMessageHeader)(nil),                       // 33: milvus.proto.messages.TxnMessageHeader
	(*ImportMessageHeader)(nil),                    // 34: milvus.proto.messages.ImportMessageHeader
	(*SchemaChangeMessageHeader)(nil),              // 35: milvus.proto.messages.SchemaChangeMessageHeader
	(*SchemaChangeMessageBody)(nil),                // 36: milvus.proto.messages.SchemaChangeMessageBody
	(*AlterCollectionMessageHeader)(nil),           // 37: milvus.proto.messages.AlterCollectionMessageHeader
	(*AlterCollectionMessageBody)(nil),             // 38: milvus.proto.messages.AlterCollectionMessageBody
	(*AlterCollectionMessageUpdates)(nil),          // 39: milvus.proto.messages.AlterCollectionMessageUpdates
	(*AlterLoadConfigOfAlterCollection)(nil),       // 40: milvus.proto.messages.AlterLoadConfigOfAlterCollection
	(*AlterLoadConfigMessageHeader)(nil),           // 41: milvus.proto.messages.AlterLoadConfigMessageHeader
	(*AlterLoadConfigMessageBody)(nil),             // 42: milvus.proto.messages.AlterLoadConfigMessageBody
	(*LoadFieldConfig)(nil),                        // 43: milvus.proto.messages.LoadFieldConfig
	(*LoadReplicaConfig)(nil),                      // 44: milvus.proto.messages.LoadReplicaConfig
	(*DropLoadConfigMessageHeader)(nil),            // 45: milvus.proto.messages.DropLoadConfigMessageHeader
	(*DropLoadConfigMessageBody)(nil),              // 46: milvus.proto.messages.DropLoadConfigMessageBody
	(*CreateDatabaseMessageHeader)(nil),            // 47: milvus.proto.messages.CreateDatabaseMessageHeader
	(*CreateDatabaseMessageBody)(nil),              // 48: milvus.proto.messages.CreateDatabaseMessageBody
	(*AlterDatabaseMessageHeader)(nil),             // 49: milvus.proto.messages.AlterDatabaseMessageHeader
	(*AlterDatabaseMessageBody)(nil),               // 50: milvus.proto.messages.AlterDatabaseMessageBody
	(*AlterLoadConfigOfAlterDatabase)(nil),         // 51: milvus.proto.messages.AlterLoadConfigOfAlterDatabase
	(*DropDatabaseMessageHeader)(nil),              // 52: milvus.proto.messages.DropDatabaseMessageHeader
	(*DropDatabaseMessageBody)(nil),                // 53: milvus.proto.messages.DropDatabaseMessageBody
	(*AlterAliasMessageHeader)(nil),                // 54: milvus.proto.messages.AlterAliasMessageHeader
	(*AlterAliasMessageBody)(nil),                  // 55: milvus.proto.messages.AlterAliasMessageBody
	(*DropAliasMessageHeader)(nil),                 // 56: milvus.proto.messages.DropAliasMessageHeader
	(*DropAliasMessageBody)(nil),                   // 57: milvus.proto.messages.DropAliasMessageBody
	(*CreateUserMessageHeader)(nil),                // 58: milvus.proto.messages.CreateUserMessageHeader
	(*CreateUserMessageBody)(nil),                  // 59: milvus.proto.messages.CreateUserMessageBody
	(*AlterUserMessageHeader)(nil),                 // 60: milvus.proto.messages.AlterUserMessageHeader
	(*AlterUserMessageBody)(nil),                   // 61: milvus.proto.messages.AlterUserMessageBody
	(*DropUserMessageHeader)(nil),                  // 62: milvus.proto.messages.DropUserMessageHeader
	(*DropUserMessageBody)(nil),                    // 63: milvus.proto.messages.DropUserMessageBody
	(*AlterRoleMessageHeader)(nil),                 // 64: milvus.proto.messages.AlterRoleMessageHeader
	(*AlterRoleMessageBody)(nil),                   // 65: milvus.proto.messages.AlterRoleMessageBody
	(*DropRoleMessageHeader)(nil),                  // 66: milvus.proto.messages.DropRoleMessageHeader
	(*DropRoleMessageBody)(nil),                    // 67: milvus.proto.messages.DropRoleMessageBody
	(*RoleBinding)(nil),                            // 68: milvus.proto.messages.RoleBinding
	(*AlterUserRoleMessageHeader)(nil),             // 69: milvus.proto.messages.AlterUserRoleMessageHeader
	(*AlterUserRoleMessageBody)(nil),               // 70: milvus.proto.messages.AlterUserRoleMessageBody
	(*DropUserRoleMessageHeader)(nil),              // 71: milvus.proto.messages.DropUserRoleMessageHeader
	(*DropUserRoleMessageBody)(nil),                // 72: milvus.proto.messages.DropUserRoleMessageBody
	(*RestoreRBACMessageHeader)(nil),               // 73: milvus.proto.messages.RestoreRBACMessageHeader
	(*RestoreRBACMessageBody)(nil),                 // 74: milvus.proto.messages.RestoreRBACMessageBody
	(*AlterPrivilegeMessageHeader)(nil),            // 75: milvus.proto.messages.AlterPrivilegeMessageHeader
	(*AlterPrivilegeMessageBody)(nil),              // 76: milvus.proto.messages.AlterPrivilegeMessageBody
	(*DropPrivilegeMessageHeader)(nil),             // 77: milvus.proto.messages.DropPrivilegeMessageHeader
	(*DropPrivilegeMessageBody)(nil),               // 78: milvus.proto.messages.DropPrivilegeMessageBody
	(*AlterPrivilegeGroupMessageHeader)(nil),       // 79: milvus.proto.messages.AlterPrivilegeGroupMessageHeader
	(*AlterPrivilegeGroupMessageBody)(nil),         // 80: milvus.proto.messages.AlterPrivilegeGroupMessageBody
	(*DropPrivilegeGroupMessageHeader)(nil),        // 81: milvus.proto.messages.DropPrivilegeGroupMessageHeader
	(*DropPrivilegeGroupMessageBody)(nil),          // 82: milvus.proto.messages.DropPrivilegeGroupMessageBody
	(*AlterResourceGroupMessageHeader)(nil),        // 83: milvus.proto.messages.AlterResourceGroupMessageHeader
	(*AlterResourceGroupMessageBody)(nil),          // 84: milvus.proto.messages.AlterResourceGroupMessageBody
	(*DropResourceGroupMessageHeader)(nil),         // 85: milvus.proto.messages.DropResourceGroupMessageHeader
	(*DropResourceGroupMessageBody)(nil),           // 86: milvus.proto.messages.DropResourceGroupMessageBody
	(*CreateIndexMessageHeader)(nil),               // 87: milvus.proto.messages.CreateIndexMessageHeader
	(*CreateIndexMessageBody)(nil),                 // 88: milvus.proto.messages.CreateIndexMessageBody
	(*AlterIndexMessageHeader)(nil),                // 89: milvus.proto.messages.AlterIndexMessageHeader
	(*AlterIndexMessageBody)(nil),                  // 90: milvus.proto.messages.AlterIndexMessageBody
	(*DropIndexMessageHeader)(nil),                 // 91: milvus.proto.messages.DropIndexMessageHeader
	(*DropIndexMessageBody)(nil),                   // 92: milvus.proto.messages.DropIndexMessageBody
	(*CreateSnapshotMessageHeader)(nil),            // 93: milvus.proto.messages.CreateSnapshotMessageHeader
	(*CreateSnapshotMessageBody)(nil),              // 94: milvus.proto.messages.CreateSnapshotMessageBody
	(*DropSnapshotMessageHeader)(nil),              // 95: milvus.proto.messages.DropSnapshotMessageHeader
	(*DropSnapshotMessageBody)(nil),                // 96: milvus.proto.messages.DropSnapshotMessageBody
	(*DropSnapshotsByCollectionMessageHeader)(nil), // 97: milvus.proto.messages.DropSnapshotsByCollectionMessageHeader
	(*DropSnapshotsByCollectionMessageBody)(nil),   // 98: milvus.proto.messages.DropSnapshotsByCollectionMessageBody
	(*RestoreSnapshotMessageHeader)(nil),           // 99: milvus.proto.messages.RestoreSnapshotMessageHeader
	(*RestoreSnapshotMessageBody)(nil),             // 100: milvus.proto.messages.RestoreSnapshotMessageBody
	(*AlterWALMessageHeader)(nil),                  // 101: milvus.proto.messages.AlterWALMessageHeader
	(*AlterWALMessageBody)(nil),                    // 102: milvus.proto.messages.AlterWALMessageBody
	(*RefreshExternalCollectionMessageHeader)(nil), // 103: milvus.proto.messages.RefreshExternalCollectionMessageHeader
	(*RefreshExternalCollectionMessageBody)(nil),   // 104: milvus.proto.messages.RefreshExternalCollectionMessageBody
	(*CommitImportMessageHeader)(nil),              // 105: milvus.proto.messages.CommitImportMessageHeader
	(*CommitImportMessageBody)(nil),                // 106: milvus.proto.messages.CommitImportMessageBody
	(*RollbackImportMessageHeader)(nil),            // 107: milvus.proto.messages.RollbackImportMessageHeader
	(*RollbackImportMessageBody)(nil),              // 108: milvus.proto.messages.RollbackImportMessageBody
	(*ImportIDRangeMessageHeader)(nil),             // 109: milvus.proto.messages.ImportIDRangeMessageHeader
	(*ImportIDRangeMessageBody)(nil),               // 110: milvus.proto.messages.ImportIDRangeMessageBody
	(*CacheExpirations)(nil),                       // 111: milvus.proto.messages.CacheExpirations
	(*CacheExpiration)(nil),                        // 112: milvus.proto.messages.CacheExpiration
	(*LegacyProxyCollectionMetaCache)(nil),         // 113: milvus.proto.messages.LegacyProxyCollectionMetaCache
	(*PartialUpdateCAS)(nil),                       // 114: milvus.proto.messages.PartialUpdateCAS
	(*ManualFlushExtraResponse)(nil),               // 115: milvus.proto.messages.ManualFlushExtraResponse
	(*SplitShardExtraResponse)(nil),                // 116: milvus.proto.messages.SplitShardExtraResponse
	(*FlushAllMessageHeader)(nil),                  // 117: milvus.proto.messages.FlushAllMessageHeader
	(*FlushAllMessageBody)(nil),                    // 118: milvus.proto.messages.FlushAllMessageBody
	(*TxnContext)(nil),                             // 119: milvus.proto.messages.TxnContext
	(*RMQMessageLayout)(nil),                       // 120: milvus.proto.messages.RMQMessageLayout
	(*BroadcastHeader)(nil),                        // 121: milvus.proto.messages.BroadcastHeader
	(*ReplicateHeader)(nil),                        // 122: milvus.proto.messages.ReplicateHeader
	(*ResourceKey)(nil),                            // 123: milvus.proto.messages.ResourceKey
	(*CipherHeader)(nil),                           // 124: milvus.proto.messages.CipherHeader
	(*TraceContextHeader)(nil),                     // 125: milvus.proto.messages.TraceContextHeader
	(*TruncateCollectionMessageHeader)(nil),        // 126: milvus.proto.messages.TruncateCollectionMessageHeader
	(*TruncateCollectionMessageBody)(nil),          // 127: milvus.proto.messages.TruncateCollectionMessageBody
	(*BatchUpdateManifestMessageHeader)(nil),       // 128: milvus.proto.messages.BatchUpdateManifestMessageHeader
	(*BatchUpdateManifestMessageBody)(nil),         // 129: milvus.proto.messages.BatchUpdateManifestMessageBody
	(*BatchUpdateManifestItem)(nil),                // 130: milvus.proto.messages.BatchUpdateManifestItem
	(*BatchUpdateManifestV2ColumnGroups)(nil),      // 131: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups
	(*AlterRLSMetadataMessageHeader)(nil),          // 132: milvus.proto.messages.AlterRLSMetadataMessageHeader
	(*AlterRLSMetadataMessageBody)(nil),            // 133: milvus.proto.messages.AlterRLSMetadataMessageBody
	(*RLSPolicyMetadata)(nil),                      // 134: milvus.proto.messages.RLSPolicyMetadata
	(*RLSPrincipalMetadata)(nil),                   // 135: milvus.proto.messages.RLSPrincipalMetadata
	(*DropRLSMetadataMessageHeader)(nil),           // 136: milvus.proto.messages.DropRLSMetadataMessageHeader
	(*DropRLSMetadataMessageBody)(nil),             // 137: milvus.proto.messages.DropRLSMetadataMessageBody
	nil,                                            // 138: milvus.proto.messages.Message.PropertiesEntry
	nil,                                            // 139: milvus.proto.messages.AlterResourceGroupMessageHeader.ResourceGroupConfigsEntry
	nil,                                            // 140: milvus.proto.messages.AlterWALMessageHeader.ConfigEntry
	nil,                                            // 141: milvus.proto.messages.ImportIDRangeMessageBody.IdRangesEntry
	nil,                                            // 142: milvus.proto.messages.RMQMessageLayout.PropertiesEntry
	nil,                                            // 143: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.ColumnGroupsEntry
	(*schemapb.IDs)(nil),                           // 144: milvus.proto.schema.IDs
	(datapb.SegmentLevel)(0),                       // 145: milvus.proto.data.SegmentLevel
	(*msgpb.CreateCollectionRequest)(nil),          // 146: milvus.proto.msg.CreateCollectionRequest
	(*commonpb.ReplicateConfiguration)(nil),        // 147: milvus.proto.common.ReplicateConfiguration
	...
	(commonpb.ConsistencyLevel)(0),                 // 150: milvus.proto.common.ConsistencyLevel
	(*commonpb.KeyValuePair)(nil),                  // 151: milvus.proto.common.KeyValuePair
	(*indexpb.FieldIndex)(nil),                     // 152: milvus.proto.index.FieldIndex
	(*schemapb.CollectionShardInfo)(nil),           // 153: milvus.proto.schema.CollectionShardInfo
	(commonpb.LoadPriority)(0),                     // 154: milvus.proto.common.LoadPriority
	(*milvuspb.UserEntity)(nil),                    // 155: milvus.proto.milvus.UserEntity
	(*internalpb.CredentialInfo)(nil),              // 156: milvus.proto.internal.CredentialInfo
	(*milvuspb.RoleEntity)(nil),                    // 157: milvus.proto.milvus.RoleEntity
	(*milvuspb.RBACMeta)(nil),                      // 158: milvus.proto.milvus.RBACMeta
	(*milvuspb.GrantEntity)(nil),                   // 159: milvus.proto.milvus.GrantEntity
	(*milvuspb.PrivilegeGroupInfo)(nil),            // 160: milvus.proto.milvus.PrivilegeGroupInfo
	(commonpb.WALName)(0),                          // 161: milvus.proto.common.WALName
	(commonpb.MsgType)(0),                          // 162: milvus.proto.common.MsgType
	(*commonpb.MessageID)(nil),                     // 163: milvus.proto.common.MessageID
	(milvuspb.RowPolicyType)(0),                    // 164: milvus.proto.milvus.RowPolicyType
	(milvuspb.RowPolicyAction)(0),                  // 165: milvus.proto.milvus.RowPolicyAction
	(*rgpb.ResourceGroupConfig)(nil),               // 166: milvus.proto.rg.ResourceGroupConfig
	(*commonpb.IDRange)(nil),                       // 167: milvus.proto.common.IDRange
	(*datapb.FieldBinlog)(nil),                     // 168: milvus.proto.data.FieldBinlog
}
var file_messages_proto_depIdxs = []int32{
	138, // 0: milvus.proto.messages.Message.properties:type_name -> milvus.proto.messages.Message.PropertiesEntry
	3,   // 1: milvus.proto.messages.TxnMessageBody.messages:type_name -> milvus.proto.messages.Message
	16,  // 2: milvus.proto.messages.InsertMessageHeader.partitions:type_name -> milvus.proto.messages.PartitionSegmentAssignment
	15,  // 3: milvus.proto.messages.InsertMessageHeader.idempotent_result:type_name -> milvus.proto.messages.IdempotentInsertResult
	144, // 4: milvus.proto.messages.IdempotentInsertResult.ids:type_name -> milvus.proto.schema.IDs
	17,  // 5: milvus.proto.messages.PartitionSegmentAssignment.segment_assignment:type_name -> milvus.proto.messages.SegmentAssignment
	145, // 6: milvus.proto.messages.CreateSegmentMessageHeader.level:type_name -> milvus.proto.data.SegmentLevel
	146, // 7: milvus.proto.messages.SplitShardMessageBody.genesis:type_name -> milvus.proto.msg.CreateCollectionRequest
	39,  // 8: milvus.proto.messages.SplitShardMessageBody.routing:type_name -> milvus.proto.messages.AlterCollectionMessageUpdates
	147, // 9: milvus.proto.messages.AlterReplicateConfigMessageHeader.replicate_configuration:type_name -> milvus.proto.common.ReplicateConfiguration
	148, // 10: milvus.proto.messages.SchemaChangeMessageBody.schema:type_name -> milvus.proto.schema.CollectionSchema
	149, // 11: milvus.proto.messages.AlterCollectionMessageHeader.update_mask:type_name -> google.protobuf.FieldMask
	111, // 12: milvus.proto.messages.AlterCollectionMessageHeader.cache_expirations:type_name -> milvus.proto.messages.CacheExpirations
	39,  // 13: milvus.proto.messages.AlterCollectionMessageBody.updates:type_name -> milvus.proto.messages.AlterCollectionMessageUpdates
	148, // 14: milvus.proto.messages.AlterCollectionMessageUpdates.schema:type_name -> milvus.proto.schema.CollectionSchema
	150, // 15: milvus.proto.messages.AlterCollectionMessageUpdates.consistency_level:type_name -> milvus.proto.common.ConsistencyLevel
	151, // 16: milvus.proto.messages.AlterCollectionMessageUpdates.properties:type_name -> milvus.proto.common.KeyValuePair
	40,  // 17: milvus.proto.messages.AlterCollectionMessageUpdates.alter_load_config:type_name -> milvus.proto.messages.AlterLoadConfigOfAlterCollection
	152, // 18: milvus.proto.messages.AlterCollectionMessageUpdates.bound_field_indexes:type_name -> milvus.proto.index.FieldIndex
	153, // 19: milvus.proto.messages.AlterCollectionMessageUpdates.shard_infos:type_name -> milvus.proto.schema.CollectionShardInfo
	43,  // 20: milvus.proto.messages.AlterLoadConfigMessageHeader.load_fields:type_name -> milvus.proto.messages.LoadFieldConfig
	44,  // 21: milvus.proto.messages.AlterLoadConfigMessageHeader.replicas:type_name -> milvus.proto.messages.LoadReplicaConfig
	154, // 22: milvus.proto.messages.LoadReplicaConfig.priority:type_name -> milvus.proto.common.LoadPriority
	151, // 23: milvus.proto.messages.CreateDatabaseMessageBody.properties:type_name -> milvus.proto.common.KeyValuePair
	151, // 24: milvus.proto.messages.AlterDatabaseMessageBody.properties:type_name -> milvus.proto.common.KeyValuePair
	51,  // 25: milvus.proto.messages.AlterDatabaseMessageBody.alter_load_config:type_name -> milvus.proto.messages.AlterLoadConfigOfAlterDatabase
	155, // 26: milvus.proto.messages.CreateUserMessageHeader.user_entity:type_name -> milvus.proto.milvus.UserEntity
	156, // 27: milvus.proto.messages.CreateUserMessageBody.credential_info:type_name -> milvus.proto.internal.CredentialInfo
	155, // 28: milvus.proto.messages.AlterUserMessageHeader.user_entity:type_name -> milvus.proto.milvus.UserEntity
	156, // 29: milvus.proto.messages.AlterUserMessageBody.credential_info:type_name -> milvus.proto.internal.CredentialInfo
	157, // 30: milvus.proto.messages.AlterRoleMessageHeader.role_entity:type_name -> milvus.proto.milvus.RoleEntity
	155, // 31: milvus.proto.messages.RoleBinding.user_entity:type_name -> milvus.proto.milvus.UserEntity
	157, // 32: milvus.proto.messages.RoleBinding.role_entity:type_name -> milvus.proto.milvus.RoleEntity
	68,  // 33: milvus.proto.messages.AlterUserRoleMessageHeader.role_binding:type_name -> milvus.proto.messages.RoleBinding
	68,  // 34: milvus.proto.messages.DropUserRoleMessageHeader.role_binding:type_name -> milvus.proto.messages.RoleBinding
	158, // 35: milvus.proto.messages.RestoreRBACMessageBody.rbac_meta:type_name -> milvus.proto.milvus.RBACMeta
	159, // 36: milvus.proto.messages.AlterPrivilegeMessageHeader.entity:type_name -> milvus.proto.milvus.GrantEntity
	159, // 37: milvus.proto.messages.DropPrivilegeMessageHeader.entity:type_name -> milvus.proto.milvus.GrantEntity
	160, // 38: milvus.proto.messages.AlterPrivilegeGroupMessageHeader.privilege_group_info:type_name -> milvus.proto.milvus.PrivilegeGroupInfo
	160, // 39: milvus.proto.messages.DropPrivilegeGroupMessageHeader.privilege_group_info:type_name -> milvus.proto.milvus.PrivilegeGroupInfo
	139, // 40: milvus.proto.messages.AlterResourceGroupMessageHeader.resource_group_configs:type_name -> milvus.proto.messages.AlterResourceGroupMessageHeader.ResourceGroupConfigsEntry
	152, // 41: milvus.proto.messages.CreateIndexMessageBody.field_index:type_name -> milvus.proto.index.FieldIndex
	152, // 42: milvus.proto.messages.AlterIndexMessageBody.field_indexes:type_name -> milvus.proto.index.FieldIndex
	161, // 43: milvus.proto.messages.AlterWALMessageHeader.target_wal_name:type_name -> milvus.proto.common.WALName
	140, // 44: milvus.proto.messages.AlterWALMessageHeader.config:type_name -> milvus.proto.messages.AlterWALMessageHeader.ConfigEntry
	141, // 45: milvus.proto.messages.ImportIDRangeMessageBody.id_ranges:type_name -> milvus.proto.messages.ImportIDRangeMessageBody.IdRangesEntry
	112, // 46: milvus.proto.messages.CacheExpirations.cache_expirations:type_name -> milvus.proto.messages.CacheExpiration
	113, // 47: milvus.proto.messages.CacheExpiration.legacy_proxy_collection_meta_cache:type_name -> milvus.proto.messages.LegacyProxyCollectionMetaCache
	162, // 48: milvus.proto.messages.LegacyProxyCollectionMetaCache.msg_type:type_name -> milvus.proto.common.MsgType
	142, // 49: milvus.proto.messages.RMQMessageLayout.properties:type_name -> milvus.proto.messages.RMQMessageLayout.PropertiesEntry
	123, // 50: milvus.proto.messages.BroadcastHeader.Resource_keys:type_name -> milvus.proto.messages.ResourceKey
	163, // 51: milvus.proto.messages.ReplicateHeader.message_id:type_name -> milvus.proto.common.MessageID
	163, // 52: milvus.proto.messages.ReplicateHeader.last_confirmed_message_id:type_name -> milvus.proto.common.MessageID
	2,   // 53: milvus.proto.messages.ResourceKey.domain:type_name -> milvus.proto.messages.ResourceDomain
	130, // 54: milvus.proto.messages.BatchUpdateManifestMessageBody.items:type_name -> milvus.proto.messages.BatchUpdateManifestItem
	131, // 55: milvus.proto.messages.BatchUpdateManifestItem.v2_column_groups:type_name -> milvus.proto.messages.BatchUpdateManifestV2ColumnGroups
	143, // 56: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.column_groups:type_name -> milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.ColumnGroupsEntry
	111, // 57: milvus.proto.messages.AlterRLSMetadataMessageHeader.cache_expirations:type_name -> milvus.proto.messages.CacheExpirations
	134, // 58: milvus.proto.messages.AlterRLSMetadataMessageBody.policy:type_name -> milvus.proto.messages.RLSPolicyMetadata
	135, // 59: milvus.proto.messages.AlterRLSMetadataMessageBody.principal:type_name -> milvus.proto.messages.RLSPrincipalMetadata
	164, // 60: milvus.proto.messages.RLSPolicyMetadata.policy_type:type_name -> milvus.proto.milvus.RowPolicyType
	165, // 61: milvus.proto.messages.RLSPolicyMetadata.actions:type_name -> milvus.proto.milvus.RowPolicyAction
	111, // 62: milvus.proto.messages.DropRLSMetadataMessageHeader.cache_expirations:type_name -> milvus.proto.messages.CacheExpirations
	166, // 63: milvus.proto.messages.AlterResourceGroupMessageHeader.ResourceGroupConfigsEntry.value:type_name -> milvus.proto.rg.ResourceGroupConfig
	167, // 64: milvus.proto.messages.ImportIDRangeMessageBody.IdRangesEntry.value:type_name -> milvus.proto.common.IDRange
	168, // 65: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.ColumnGroupsEntry.value:type_name -> milvus.proto.data.FieldBinlog
...
func file_messages_proto_init() {
	...
	if !protoimpl.UnsafeEnabled {
		...
		file_messages_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*IdempotentInsertResult); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
		...
		file_messages_proto_msgTypes[19].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SplitShardMessageHeader); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
		...
		file_messages_proto_msgTypes[20].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SplitShardMessageBody); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
		...
		file_messages_proto_msgTypes[113].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SplitShardExtraResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
	...
	file_messages_proto_msgTypes[109].OneofWrappers = []interface{}{
		(*CacheExpiration_LegacyProxyCollectionMetaCache)(nil),
	}
	file_messages_proto_msgTypes[130].OneofWrappers = []interface{}{
		(*AlterRLSMetadataMessageBody_Policy)(nil),
		(*AlterRLSMetadataMessageBody_Principal)(nil),
	}
	file_messages_proto_msgTypes[134].OneofWrappers = []interface{}{
