...
type AlterAliasMessageHeader struct {
	...
	// The collection the alias pointed to BEFORE this alter, 0 if none
	// (CreateAlias) or unknown (older producer). Carried in the header --
	// computed once under the broadcaster's database lock -- so cache
	// expiration can evict the old target by id even when a concurrent
	// describe has already re-pointed the proxy's alias resolution to the
	// new target, and so replaying the message stays deterministic.
