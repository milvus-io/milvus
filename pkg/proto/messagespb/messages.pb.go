...
const (
	...
	MessageType_SplitShard                MessageType = 49
	// CreateVChannel is the genesis entry of a shard split target vchannel.
	// It carries BarrierTimeTick = T_switch, so the new WAL is born strictly
	// after the fence and creation doubles as activation.
	MessageType_CreateVChannel MessageType = 50
	// DropVChannel retires a vchannel a shard split left behind. It is the
	// inverse of CreateVChannel: the streamingnode tears down the vchannel's
	// shard-manager entry, recovery info and flusher, so the coordinator can
	// drop it from the collection and give the pchannel slot back.
	MessageType_DropVChannel MessageType = 51
...
// SplitShardTarget describes one target shard of a shard split.
type SplitShardTarget struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// the vchannel name of the target shard.
	Vchannel string `protobuf:"bytes,1,opt,name=vchannel,proto3" json:"vchannel,omitempty"`
	// the routing residues owned by the target shard after the split, taken
	// against the routing_modulus of the message carrying this target.
	Routing *schemapb.HashRouting `protobuf:"bytes,2,opt,name=routing,proto3" json:"routing,omitempty"`
}

func (x *SplitShardTarget) Reset() {
	*x = SplitShardTarget{}
	if protoimpl.UnsafeEnabled {
...
func (x *SplitShardTarget) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SplitShardTarget) ProtoMessage() {}

func (x *SplitShardTarget) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[16]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use SplitShardTarget.ProtoReflect.Descriptor instead.
func (*SplitShardTarget) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{16}
}

func (x *SplitShardTarget) GetVchannel() string {
	if x != nil {
		return x.Vchannel
	}
	return ""
...
func (x *SplitShardTarget) GetRouting() *schemapb.HashRouting {
	if x != nil {
		return x.Routing
	}
	return nil
...
// SplitShardMessageHeader is the header of the split shard message.
// The message is appended to the source shard's vchannel as the write fence:
// its time tick is T_switch, after this message the source vchannel never
// accepts new DML messages, and the new writes of the source key range are
// served by the target shards.
type SplitShardMessageHeader struct {
	...
	// the target shards that the source shard splits into.
	// the residues of all targets are disjoint and their union is exactly
	// the residues owned by the source shard.
	Targets []*SplitShardTarget `protobuf:"bytes,3,rep,name=targets,proto3" json:"targets,omitempty"`
	// the growing segments the source streamingnode auto-flushed while
	// processing this message (embedded by the handler, the same way the
	// AlterCollection schema-change path embeds its flushed segments).
	FlushedSegmentIds []int64 `protobuf:"varint,4,rep,packed,name=flushed_segment_ids,json=flushedSegmentIds,proto3" json:"flushed_segment_ids,omitempty"`
	// the collection's routing modulus the targets' residues are taken against.
	// Recorded here because the record is permanent while the collection's
	// modulus moves: a later split doubling it would leave these residues
	// uninterpretable on their own.
	RoutingModulus uint64 `protobuf:"varint,5,opt,name=routing_modulus,json=routingModulus,proto3" json:"routing_modulus,omitempty"`
}
...
func (x *SplitShardMessageHeader) GetTargets() []*SplitShardTarget {
	if x != nil {
		return x.Targets
	}
	return nil
...
func (x *SplitShardMessageHeader) GetRoutingModulus() uint64 {
	if x != nil {
...
// SplitShardMessageBody is the body of the split shard message.
...
// CreateVChannelMessageHeader is the header of the create-vchannel message,
// the genesis entry of a shard split target vchannel. It reuses the
// CreateCollection body shape (schema), so consumers share the existing
// schema parser, and adds the split correlation and the key range the target
// owns. The append carries BarrierTimeTick = T_switch, so the new WAL is born
// strictly after the fence and creation doubles as activation.
type CreateVChannelMessageHeader struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
	...
	PartitionIds []int64 `protobuf:"varint,2,rep,packed,name=partition_ids,json=partitionIds,proto3" json:"partition_ids,omitempty"`
	DbId         int64   `protobuf:"varint,3,opt,name=db_id,json=dbId,proto3" json:"db_id,omitempty"`
	// the unique split task id allocated by the coordinator, for idempotency
	// and split task correlation.
	SplitTaskId int64 `protobuf:"varint,4,opt,name=split_task_id,json=splitTaskId,proto3" json:"split_task_id,omitempty"`
	// the fenced source vchannels of the split. More than one when the
	// collection is rehashed to an arbitrary shard count, where the target is
	// carved out of every source at once. Provenance recorded permanently in
	// the WAL, so it lists every source rather than a representative one.
	SplitSourceVchannels []string `protobuf:"bytes,5,rep,name=split_source_vchannels,json=splitSourceVchannels,proto3" json:"split_source_vchannels,omitempty"`
	// the routing residues this target vchannel owns, taken against
	// routing_modulus below.
	Routing *schemapb.HashRouting `protobuf:"bytes,6,opt,name=routing,proto3" json:"routing,omitempty"`
	// the collection's routing modulus at the time of the split, for the same
	// reason SplitShardMessageHeader records it.
	RoutingModulus uint64 `protobuf:"varint,7,opt,name=routing_modulus,json=routingModulus,proto3" json:"routing_modulus,omitempty"`
}

func (x *CreateVChannelMessageHeader) Reset() {
	*x = CreateVChannelMessageHeader{}
	if protoimpl.UnsafeEnabled {
...
func (x *CreateVChannelMessageHeader) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateVChannelMessageHeader) ProtoMessage() {}

func (x *CreateVChannelMessageHeader) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[20]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use CreateVChannelMessageHeader.ProtoReflect.Descriptor instead.
func (*CreateVChannelMessageHeader) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{20}
}

func (x *CreateVChannelMessageHeader) GetCollectionId() int64 {
	if x != nil {
		return x.CollectionId
	}
	return 0
...
func (x *CreateVChannelMessageHeader) GetPartitionIds() []int64 {
	if x != nil {
		return x.PartitionIds
	}
	return nil
...
func (x *CreateVChannelMessageHeader) GetDbId() int64 {
	if x != nil {
		return x.DbId
	}
	return 0
...
func (x *CreateVChannelMessageHeader) GetSplitTaskId() int64 {
	if x != nil {
		return x.SplitTaskId
	}
	return 0
...
func (x *CreateVChannelMessageHeader) GetSplitSourceVchannels() []string {
	if x != nil {
		return x.SplitSourceVchannels
	}
	return nil
}

func (x *CreateVChannelMessageHeader) GetRouting() *schemapb.HashRouting {
	if x != nil {
		return x.Routing
	}
	return nil
...
func (x *CreateVChannelMessageHeader) GetRoutingModulus() uint64 {
	if x != nil {
		return x.RoutingModulus
	}
	return 0
...
// DropVChannelMessageHeader is the header of the drop-vchannel message, the
// inverse of CreateVChannel. It retires ONE vchannel of a collection rather
// than the whole collection, which is what a shard split needs: the split's
// retired source is dead as a shard long before the collection is.
//
// Without it a retired source lives forever on the streamingnode. That is not
// merely untidy once the coordinator reclaims the slot: the shard manager
// indexes by collection id, one entry per pchannel, so a later vchannel of the
// SAME collection landing on that pchannel finds the entry already there,
// skips its own creation, and inherits the retired shard's SPLITTED state --
// leaving the new shard permanently unwritable.
type DropVChannelMessageHeader struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
	...
	// the split task that retired this vchannel, for correlation with the
	// CreateVChannel that created it.
	SplitTaskId int64 `protobuf:"varint,3,opt,name=split_task_id,json=splitTaskId,proto3" json:"split_task_id,omitempty"`
}

func (x *DropVChannelMessageHeader) Reset() {
	*x = DropVChannelMessageHeader{}
	if protoimpl.UnsafeEnabled {
...
func (x *DropVChannelMessageHeader) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DropVChannelMessageHeader) ProtoMessage() {}

func (x *DropVChannelMessageHeader) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[21]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use DropVChannelMessageHeader.ProtoReflect.Descriptor instead.
func (*DropVChannelMessageHeader) Descriptor() ([]byte, []int) {
	return file_messages_proto_rawDescGZIP(), []int{21}
}

func (x *DropVChannelMessageHeader) GetCollectionId() int64 {
	if x != nil {
		return x.CollectionId
	}
	return 0
...
func (x *DropVChannelMessageHeader) GetDbId() int64 {
	if x != nil {
		return x.DbId
	}
	return 0
...
func (x *DropVChannelMessageHeader) GetSplitTaskId() int64 {
	if x != nil {
		return x.SplitTaskId
	}
	return 0
...
// DropVChannelMessageBody is the body of the drop-vchannel message. Empty: the
// header names everything the teardown needs.
type DropVChannelMessageBody struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *DropVChannelMessageBody) Reset() {
	*x = DropVChannelMessageBody{}
	if protoimpl.UnsafeEnabled {
...
func (x *DropVChannelMessageBody) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DropVChannelMessageBody) ProtoMessage() {}

func (x *DropVChannelMessageBody) ProtoReflect() protoreflect.Message {
	mi := &file_messages_proto_msgTypes[22]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use DropVChannelMessageBody.ProtoReflect.Descriptor instead.
func (*DropVChannelMessageBody) Descriptor() ([]byte, []int) {
...
var file_messages_proto_rawDesc = []byte{
	...
	,
	...
	,
	...
	0x70, 0x61, 0x72, 0x74, 0x69, 0x74, 0x69, 0x6f, 0x6e, 0x5f, 0x69, 0x64, 0x73, 0x18, 0x02, 0x20,
	0x03, 0x28, 0x03, 0x52, 0x0c, 0x70, 0x61, 0x72, 0x74, 0x69, 0x74, 0x69, 0x6f, 0x6e, 0x49, 0x64,
	0x73, 0x12, 0x13, 0x0a, 0x05, 0x64, 0x62, 0x5f, 0x69, 0x64, 0x18, 0x03, 0x20, 0x01, 0x28, 0x03,
	0x52, 0x04, 0x64, 0x62, 0x49, 0x64, 0x22, 0xbb, 0x02, 0x0a, 0x1b, 0x43, 0x72, 0x65, 0x61, 0x74,
	0x65, 0x56, 0x43, 0x68, 0x61, 0x6e, 0x6e, 0x65, 0x6c, 0x4d, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65,
	...
	,
	...
	0x02, 0x20, 0x01, 0x28, 0x03, 0x52, 0x04, 0x64, 0x62, 0x49, 0x64, 0x12, 0x22, 0x0a, 0x0d, 0x73,
	0x70, 0x6c, 0x69, 0x74, 0x5f, 0x74, 0x61, 0x73, 0x6b, 0x5f, 0x69, 0x64, 0x18, 0x03, 0x20, 0x01,
	0x28, 0x03, 0x52, 0x0b, 0x73, 0x70, 0x6c, 0x69, 0x74, 0x54, 0x61, 0x73, 0x6b, 0x49, 0x64, 0x22,
	0x19, 0x0a, 0x17, 0x44, 0x72, 0x6f, 0x70, 0x56, 0x43, 0x68, 0x61, 0x6e, 0x6e, 0x65, 0x6c, 0x4d,
	0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x42, 0x6f, 0x64, 0x79, 0x22, 0x57, 0x0a, 0x1b, 0x44, 0x72,
	...
	0x64, 0x22, 0xfc, 0x01, 0x0a, 0x21, 0x41, 0x6c, 0x74, 0x65, 0x72, 0
	(*SplitShardMessageHeader)(nil),                // 20: milvus.proto.messages.SplitShardMessageHeader
	(*SplitShardMessageBody)(nil),                  // 21: milvus.proto.messages.SplitShardMessageBody
	(*CreateCollectionMessageHeader)(nil),          // 22: milvus.proto.messages.CreateCollectionMessageHeader
	(*CreateVChannelMessageHeader)(nil),            // 23: milvus.proto.messages.CreateVChannelMessageHeader
	(*DropVChannelMessageHeader)(nil),              // 24: milvus.proto.messages.DropVChannelMessageHeader
	(*DropVChannelMessageBody)(nil),                // 25: milvus.proto.messages.DropVChannelMessageBody
	(*DropCollectionMessageHeader)(nil),            // 26: milvus.proto.messages.DropCollectionMessageHeader
	(*CreatePartitionMessageHeader)(nil),           // 27: milvus.proto.messages.CreatePartitionMessageHeader
	(*DropPartitionMessageHeader)(nil),             // 28: milvus.proto.messages.DropPartitionMessageHeader
	(*AlterReplicateConfigMessageHeader)(nil),      // 29: milvus.proto.messages.AlterReplicateConfigMessageHeader
	(*AlterReplicateConfigMessageBody)(nil),        // 30: milvus.proto.messages.AlterReplicateConfigMessageBody
	(*BeginTxnMessageHeader)(nil),                  // 31: milvus.proto.messages.BeginTxnMessageHeader
	(*CommitTxnMessageHeader)(nil),                 // 32: milvus.proto.messages.CommitTxnMessageHeader
	(*RollbackTxnMessageHeader)(nil),               // 33: milvus.proto.messages.RollbackTxnMessageHeader
	(*TxnMessageHeader)(nil),                       // 34: milvus.proto.messages.TxnMessageHeader
	(*ImportMessageHeader)(nil),                    // 35: milvus.proto.messages.ImportMessageHeader
	(*SchemaChangeMessageHeader)(nil),              // 36: milvus.proto.messages.SchemaChangeMessageHeader
	(*SchemaChangeMessageBody)(nil),                // 37: milvus.proto.messages.SchemaChangeMessageBody
	(*AlterCollectionMessageHeader)(nil),           // 38: milvus.proto.messages.AlterCollectionMessageHeader
	(*AlterCollectionMessageBody)(nil),             // 39: milvus.proto.messages.AlterCollectionMessageBody
	(*AlterCollectionMessageUpdates)(nil),          // 40: milvus.proto.messages.AlterCollectionMessageUpdates
	(*AlterLoadConfigOfAlterCollection)(nil),       // 41: milvus.proto.messages.AlterLoadConfigOfAlterCollection
	(*AlterLoadConfigMessageHeader)(nil),           // 42: milvus.proto.messages.AlterLoadConfigMessageHeader
	(*AlterLoadConfigMessageBody)(nil),             // 43: milvus.proto.messages.AlterLoadConfigMessageBody
	(*LoadFieldConfig)(nil),                        // 44: milvus.proto.messages.LoadFieldConfig
	(*LoadReplicaConfig)(nil),                      // 45: milvus.proto.messages.LoadReplicaConfig
	(*DropLoadConfigMessageHeader)(nil),            // 46: milvus.proto.messages.DropLoadConfigMessageHeader
	(*DropLoadConfigMessageBody)(nil),              // 47: milvus.proto.messages.DropLoadConfigMessageBody
	(*CreateDatabaseMessageHeader)(nil),            // 48: milvus.proto.messages.CreateDatabaseMessageHeader
	(*CreateDatabaseMessageBody)(nil),              // 49: milvus.proto.messages.CreateDatabaseMessageBody
	(*AlterDatabaseMessageHeader)(nil),             // 50: milvus.proto.messages.AlterDatabaseMessageHeader
	(*AlterDatabaseMessageBody)(nil),               // 51: milvus.proto.messages.AlterDatabaseMessageBody
	(*AlterLoadConfigOfAlterDatabase)(nil),         // 52: milvus.proto.messages.AlterLoadConfigOfAlterDatabase
	(*DropDatabaseMessageHeader)(nil),              // 53: milvus.proto.messages.DropDatabaseMessageHeader
	(*DropDatabaseMessageBody)(nil),                // 54: milvus.proto.messages.DropDatabaseMessageBody
	(*AlterAliasMessageHeader)(nil),                // 55: milvus.proto.messages.AlterAliasMessageHeader
	(*AlterAliasMessageBody)(nil),                  // 56: milvus.proto.messages.AlterAliasMessageBody
	(*DropAliasMessageHeader)(nil),                 // 57: milvus.proto.messages.DropAliasMessageHeader
	(*DropAliasMessageBody)(nil),                   // 58: milvus.proto.messages.DropAliasMessageBody
	(*CreateUserMessageHeader)(nil),                // 59: milvus.proto.messages.CreateUserMessageHeader
	(*CreateUserMessageBody)(nil),                  // 60: milvus.proto.messages.CreateUserMessageBody
	(*AlterUserMessageHeader)(nil),                 // 61: milvus.proto.messages.AlterUserMessageHeader
	(*AlterUserMessageBody)(nil),                   // 62: milvus.proto.messages.AlterUserMessageBody
	(*DropUserMessageHeader)(nil),                  // 63: milvus.proto.messages.DropUserMessageHeader
	(*DropUserMessageBody)(nil),                    // 64: milvus.proto.messages.DropUserMessageBody
	(*AlterRoleMessageHeader)(nil),                 // 65: milvus.proto.messages.AlterRoleMessageHeader
	(*AlterRoleMessageBody)(nil),                   // 66: milvus.proto.messages.AlterRoleMessageBody
	(*DropRoleMessageHeader)(nil),                  // 67: milvus.proto.messages.DropRoleMessageHeader
	(*DropRoleMessageBody)(nil),                    // 68: milvus.proto.messages.DropRoleMessageBody
	(*RoleBinding)(nil),                            // 69: milvus.proto.messages.RoleBinding
	(*AlterUserRoleMessageHeader)(nil),             // 70: milvus.proto.messages.AlterUserRoleMessageHeader
	(*AlterUserRoleMessageBody)(nil),               // 71: milvus.proto.messages.AlterUserRoleMessageBody
	(*DropUserRoleMessageHeader)(nil),              // 72: milvus.proto.messages.DropUserRoleMessageHeader
	(*DropUserRoleMessageBody)(nil),                // 73: milvus.proto.messages.DropUserRoleMessageBody
	(*RestoreRBACMessageHeader)(nil),               // 74: milvus.proto.messages.RestoreRBACMessageHeader
	(*RestoreRBACMessageBody)(nil),                 // 75: milvus.proto.messages.RestoreRBACMessageBody
	(*AlterPrivilegeMessageHeader)(nil),            // 76: milvus.proto.messages.AlterPrivilegeMessageHeader
	(*AlterPrivilegeMessageBody)(nil),              // 77: milvus.proto.messages.AlterPrivilegeMessageBody
	(*DropPrivilegeMessageHeader)(nil),             // 78: milvus.proto.messages.DropPrivilegeMessageHeader
	(*DropPrivilegeMessageBody)(nil),               // 79: milvus.proto.messages.DropPrivilegeMessageBody
	(*AlterPrivilegeGroupMessageHeader)(nil),       // 80: milvus.proto.messages.AlterPrivilegeGroupMessageHeader
	(*AlterPrivilegeGroupMessageBody)(nil),         // 81: milvus.proto.messages.AlterPrivilegeGroupMessageBody
	(*DropPrivilegeGroupMessageHeader)(nil),        // 82: milvus.proto.messages.DropPrivilegeGroupMessageHeader
	(*DropPrivilegeGroupMessageBody)(nil),          // 83: milvus.proto.messages.DropPrivilegeGroupMessageBody
	(*AlterResourceGroupMessageHeader)(nil),        // 84: milvus.proto.messages.AlterResourceGroupMessageHeader
	(*AlterResourceGroupMessageBody)(nil),          // 85: milvus.proto.messages.AlterResourceGroupMessageBody
	(*DropResourceGroupMessageHeader)(nil),         // 86: milvus.proto.messages.DropResourceGroupMessageHeader
	(*DropResourceGroupMessageBody)(nil),           // 87: milvus.proto.messages.DropResourceGroupMessageBody
	(*CreateIndexMessageHeader)(nil),               // 88: milvus.proto.messages.CreateIndexMessageHeader
	(*CreateIndexMessageBody)(nil),                 // 89: milvus.proto.messages.CreateIndexMessageBody
	(*AlterIndexMessageHeader)(nil),                // 90: milvus.proto.messages.AlterIndexMessageHeader
	(*AlterIndexMessageBody)(nil),                  // 91: milvus.proto.messages.AlterIndexMessageBody
	(*DropIndexMessageHeader)(nil),                 // 92: milvus.proto.messages.DropIndexMessageHeader
	(*DropIndexMessageBody)(nil),                   // 93: milvus.proto.messages.DropIndexMessageBody
	(*CreateSnapshotMessageHeader)(nil),            // 94: milvus.proto.messages.CreateSnapshotMessageHeader
	(*CreateSnapshotMessageBody)(nil),              // 95: milvus.proto.messages.CreateSnapshotMessageBody
	(*DropSnapshotMessageHeader)(nil),              // 96: milvus.proto.messages.DropSnapshotMessageHeader
	(*DropSnapshotMessageBody)(nil),                // 97: milvus.proto.messages.DropSnapshotMessageBody
	(*DropSnapshotsByCollectionMessageHeader)(nil), // 98: milvus.proto.messages.DropSnapshotsByCollectionMessageHeader
	(*DropSnapshotsByCollectionMessageBody)(nil),   // 99: milvus.proto.messages.DropSnapshotsByCollectionMessageBody
	(*RestoreSnapshotMessageHeader)(nil),           // 100: milvus.proto.messages.RestoreSnapshotMessageHeader
	(*RestoreSnapshotMessageBody)(nil),             // 101: milvus.proto.messages.RestoreSnapshotMessageBody
	(*AlterWALMessageHeader)(nil),                  // 102: milvus.proto.messages.AlterWALMessageHeader
	(*AlterWALMessageBody)(nil),                    // 103: milvus.proto.messages.AlterWALMessageBody
	(*RefreshExternalCollectionMessageHeader)(nil), // 104: milvus.proto.messages.RefreshExternalCollectionMessageHeader
	(*RefreshExternalCollectionMessageBody)(nil),   // 105: milvus.proto.messages.RefreshExternalCollectionMessageBody
	(*CommitImportMessageHeader)(nil),              // 106: milvus.proto.messages.CommitImportMessageHeader
	(*CommitImportMessageBody)(nil),                // 107: milvus.proto.messages.CommitImportMessageBody
	(*RollbackImportMessageHeader)(nil),            // 108: milvus.proto.messages.RollbackImportMessageHeader
	(*RollbackImportMessageBody)(nil),              // 109: milvus.proto.messages.RollbackImportMessageBody
	(*CacheExpirations)(nil),                       // 110: milvus.proto.messages.CacheExpirations
	(*CacheExpiration)(nil),                        // 111: milvus.proto.messages.CacheExpiration
	(*LegacyProxyCollectionMetaCache)(nil),         // 112: milvus.proto.messages.LegacyProxyCollectionMetaCache
	(*PartialUpdateCAS)(nil),                       // 113: milvus.proto.messages.PartialUpdateCAS
	(*ManualFlushExtraResponse)(nil),               // 114: milvus.proto.messages.ManualFlushExtraResponse
	(*FlushAllMessageHeader)(nil),                  // 115: milvus.proto.messages.FlushAllMessageHeader
	(*FlushAllMessageBody)(nil),                    // 116: milvus.proto.messages.FlushAllMessageBody
	(*TxnContext)(nil),                             // 117: milvus.proto.messages.TxnContext
	(*RMQMessageLayout)(nil),                       // 118: milvus.proto.messages.RMQMessageLayout
	(*BroadcastHeader)(nil),                        // 119: milvus.proto.messages.BroadcastHeader
	(*ReplicateHeader)(nil),                        // 120: milvus.proto.messages.ReplicateHeader
	(*ResourceKey)(nil),                            // 121: milvus.proto.messages.ResourceKey
	(*CipherHeader)(nil),                           // 122: milvus.proto.messages.CipherHeader
	(*TraceContextHeader)(nil),                     // 123: milvus.proto.messages.TraceContextHeader
	(*TruncateCollectionMessageHeader)(nil),        // 124: milvus.proto.messages.TruncateCollectionMessageHeader
	(*TruncateCollectionMessageBody)(nil),          // 125: milvus.proto.messages.TruncateCollectionMessageBody
	(*BatchUpdateManifestMessageHeader)(nil),       // 126: milvus.proto.messages.BatchUpdateManifestMessageHeader
	(*BatchUpdateManifestMessageBody)(nil),         // 127: milvus.proto.messages.BatchUpdateManifestMessageBody
	(*BatchUpdateManifestItem)(nil),                // 128: milvus.proto.messages.BatchUpdateManifestItem
	(*BatchUpdateManifestV2ColumnGroups)(nil),      // 129: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups
	(*AlterRLSMetadataMessageHeader)(nil),          // 130: milvus.proto.messages.AlterRLSMetadataMessageHeader
	(*AlterRLSMetadataMessageBody)(nil),            // 131: milvus.proto.messages.AlterRLSMetadataMessageBody
	(*RLSPolicyMetadata)(nil),                      // 132: milvus.proto.messages.RLSPolicyMetadata
	(*RLSPrincipalMetadata)(nil),                   // 133: milvus.proto.messages.RLSPrincipalMetadata
	(*DropRLSMetadataMessageHeader)(nil),           // 134: milvus.proto.messages.DropRLSMetadataMessageHeader
	(*DropRLSMetadataMessageBody)(nil),             // 135: milvus.proto.messages.DropRLSMetadataMessageBody
	nil,                                            // 136: milvus.proto.messages.Message.PropertiesEntry
	nil,                                            // 137: milvus.proto.messages.AlterResourceGroupMessageHeader.ResourceGroupConfigsEntry
	nil,                                            // 138: milvus.proto.messages.AlterWALMessageHeader.ConfigEntry
	nil,                                            // 139: milvus.proto.messages.RMQMessageLayout.PropertiesEntry
	nil,                                            // 140: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.ColumnGroupsEntry
	(datapb.SegmentLevel)(0),                       // 141: milvus.proto.data.SegmentLevel
	(*schemapb.HashRouting)(nil),                   // 142: milvus.proto.schema.HashRouting
	(*commonpb.ReplicateConfiguration)(nil),        // 143: milvus.proto.common.ReplicateConfiguration
	...
	(commonpb.ConsistencyLevel)(0),                 // 146: milvus.proto.common.ConsistencyLevel
	(*commonpb.KeyValuePair)(nil),                  // 147: milvus.proto.common.KeyValuePair
	(*indexpb.FieldIndex)(nil),                     // 148: milvus.proto.index.FieldIndex
	(*schemapb.CollectionShardInfo)(nil),           // 149: milvus.proto.schema.CollectionShardInfo
	(commonpb.LoadPriority)(0),                     // 150: milvus.proto.common.LoadPriority
	(*milvuspb.UserEntity)(nil),                    // 151: milvus.proto.milvus.UserEntity
	(*internalpb.CredentialInfo)(nil),              // 152: milvus.proto.internal.CredentialInfo
	(*milvuspb.RoleEntity)(nil),                    // 153: milvus.proto.milvus.RoleEntity
	(*milvuspb.RBACMeta)(nil),                      // 154: milvus.proto.milvus.RBACMeta
	(*milvuspb.GrantEntity)(nil),                   // 155: milvus.proto.milvus.GrantEntity
	(*milvuspb.PrivilegeGroupInfo)(nil),            // 156: milvus.proto.milvus.PrivilegeGroupInfo
	(commonpb.WALName)(0),                          // 157: milvus.proto.common.WALName
	(commonpb.MsgType)(0),                          // 158: milvus.proto.common.MsgType
	(*commonpb.MessageID)(nil),                     // 159: milvus.proto.common.MessageID
	(milvuspb.RowPolicyType)(0),                    // 160: milvus.proto.milvus.RowPolicyType
	(milvuspb.RowPolicyAction)(0),                  // 161: milvus.proto.milvus.RowPolicyAction
	(*rgpb.ResourceGroupConfig)(nil),               // 162: milvus.proto.rg.ResourceGroupConfig
	(*datapb.FieldBinlog)(nil),                     // 163: milvus.proto.data.FieldBinlog
}
var file_messages_proto_depIdxs = []int32{
	136, // 0: milvus.proto.messages.Message.properties:type_name -> milvus.proto.messages.Message.PropertiesEntry
	...
	141, // 4: milvus.proto.messages.CreateSegmentMessageHeader.level:type_name -> milvus.proto.data.SegmentLevel
	142, // 5: milvus.proto.messages.SplitShardTarget.routing:type_name -> milvus.proto.schema.HashRouting
	19,  // 6: milvus.proto.messages.SplitShardMessageHeader.targets:type_name -> milvus.proto.messages.SplitShardTarget
	142, // 7: milvus.proto.messages.CreateVChannelMessageHeader.routing:type_name -> milvus.proto.schema.HashRouting
	143, // 8: milvus.proto.messages.AlterReplicateConfigMessageHeader.replicate_configuration:type_name -> milvus.proto.common.ReplicateConfiguration
	144, // 9: milvus.proto.messages.SchemaChangeMessageBody.schema:type_name -> milvus.proto.schema.CollectionSchema
	145, // 10: milvus.proto.messages.AlterCollectionMessageHeader.update_mask:type_name -> google.protobuf.FieldMask
	110, // 11: milvus.proto.messages.AlterCollectionMessageHeader.cache_expirations:type_name -> milvus.proto.messages.CacheExpirations
	40,  // 12: milvus.proto.messages.AlterCollectionMessageBody.updates:type_name -> milvus.proto.messages.AlterCollectionMessageUpdates
	144, // 13: milvus.proto.messages.AlterCollectionMessageUpdates.schema:type_name -> milvus.proto.schema.CollectionSchema
	146, // 14: milvus.proto.messages.AlterCollectionMessageUpdates.consistency_level:type_name -> milvus.proto.common.ConsistencyLevel
	147, // 15: milvus.proto.messages.AlterCollectionMessageUpdates.properties:type_name -> milvus.proto.common.KeyValuePair
	41,  // 16: milvus.proto.messages.AlterCollectionMessageUpdates.alter_load_config:type_name -> milvus.proto.messages.AlterLoadConfigOfAlterCollection
	148, // 17: milvus.proto.messages.AlterCollectionMessageUpdates.bound_field_indexes:type_name -> milvus.proto.index.FieldIndex
	149, // 18: milvus.proto.messages.AlterCollectionMessageUpdates.shard_infos:type_name -> milvus.proto.schema.CollectionShardInfo
	44,  // 19: milvus.proto.messages.AlterLoadConfigMessageHeader.load_fields:type_name -> milvus.proto.messages.LoadFieldConfig
	45,  // 20: milvus.proto.messages.AlterLoadConfigMessageHeader.replicas:type_name -> milvus.proto.messages.LoadReplicaConfig
	150, // 21: milvus.proto.messages.LoadReplicaConfig.priority:type_name -> milvus.proto.common.LoadPriority
	147, // 22: milvus.proto.messages.CreateDatabaseMessageBody.properties:type_name -> milvus.proto.common.KeyValuePair
	147, // 23: milvus.proto.messages.AlterDatabaseMessageBody.properties:type_name -> milvus.proto.common.KeyValuePair
	52,  // 24: milvus.proto.messages.AlterDatabaseMessageBody.alter_load_config:type_name -> milvus.proto.messages.AlterLoadConfigOfAlterDatabase
	151, // 25: milvus.proto.messages.CreateUserMessageHeader.user_entity:type_name -> milvus.proto.milvus.UserEntity
	152, // 26: milvus.proto.messages.CreateUserMessageBody.credential_info:type_name -> milvus.proto.internal.CredentialInfo
	151, // 27: milvus.proto.messages.AlterUserMessageHeader.user_entity:type_name -> milvus.proto.milvus.UserEntity
	152, // 28: milvus.proto.messages.AlterUserMessageBody.credential_info:type_name -> milvus.proto.internal.CredentialInfo
	153, // 29: milvus.proto.messages.AlterRoleMessageHeader.role_entity:type_name -> milvus.proto.milvus.RoleEntity
	151, // 30: milvus.proto.messages.RoleBinding.user_entity:type_name -> milvus.proto.milvus.UserEntity
	153, // 31: milvus.proto.messages.RoleBinding.role_entity:type_name -> milvus.proto.milvus.RoleEntity
	69,  // 32: milvus.proto.messages.AlterUserRoleMessageHeader.role_binding:type_name -> milvus.proto.messages.RoleBinding
	69,  // 33: milvus.proto.messages.DropUserRoleMessageHeader.role_binding:type_name -> milvus.proto.messages.RoleBinding
	154, // 34: milvus.proto.messages.RestoreRBACMessageBody.rbac_meta:type_name -> milvus.proto.milvus.RBACMeta
	155, // 35: milvus.proto.messages.AlterPrivilegeMessageHeader.entity:type_name -> milvus.proto.milvus.GrantEntity
	155, // 36: milvus.proto.messages.DropPrivilegeMessageHeader.entity:type_name -> milvus.proto.milvus.GrantEntity
	156, // 37: milvus.proto.messages.AlterPrivilegeGroupMessageHeader.privilege_group_info:type_name -> milvus.proto.milvus.PrivilegeGroupInfo
	156, // 38: milvus.proto.messages.DropPrivilegeGroupMessageHeader.privilege_group_info:type_name -> milvus.proto.milvus.PrivilegeGroupInfo
	137, // 39: milvus.proto.messages.AlterResourceGroupMessageHeader.resource_group_configs:type_name -> milvus.proto.messages.AlterResourceGroupMessageHeader.ResourceGroupConfigsEntry
	148, // 40: milvus.proto.messages.CreateIndexMessageBody.field_index:type_name -> milvus.proto.index.FieldIndex
	148, // 41: milvus.proto.messages.AlterIndexMessageBody.field_indexes:type_name -> milvus.proto.index.FieldIndex
	157, // 42: milvus.proto.messages.AlterWALMessageHeader.target_wal_name:type_name -> milvus.proto.common.WALName
	138, // 43: milvus.proto.messages.AlterWALMessageHeader.config:type_name -> milvus.proto.messages.AlterWALMessageHeader.ConfigEntry
	111, // 44: milvus.proto.messages.CacheExpirations.cache_expirations:type_name -> milvus.proto.messages.CacheExpiration
	112, // 45: milvus.proto.messages.CacheExpiration.legacy_proxy_collection_meta_cache:type_name -> milvus.proto.messages.LegacyProxyCollectionMetaCache
	158, // 46: milvus.proto.messages.LegacyProxyCollectionMetaCache.msg_type:type_name -> milvus.proto.common.MsgType
	139, // 47: milvus.proto.messages.RMQMessageLayout.properties:type_name -> milvus.proto.messages.RMQMessageLayout.PropertiesEntry
	121, // 48: milvus.proto.messages.BroadcastHeader.Resource_keys:type_name -> milvus.proto.messages.ResourceKey
	159, // 49: milvus.proto.messages.ReplicateHeader.message_id:type_name -> milvus.proto.common.MessageID
	159, // 50: milvus.proto.messages.ReplicateHeader.last_confirmed_message_id:type_name -> milvus.proto.common.MessageID
	2,   // 51: milvus.proto.messages.ResourceKey.domain:type_name -> milvus.proto.messages.ResourceDomain
	128, // 52: milvus.proto.messages.BatchUpdateManifestMessageBody.items:type_name -> milvus.proto.messages.BatchUpdateManifestItem
	129, // 53: milvus.proto.messages.BatchUpdateManifestItem.v2_column_groups:type_name -> milvus.proto.messages.BatchUpdateManifestV2ColumnGroups
	140, // 54: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.column_groups:type_name -> milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.ColumnGroupsEntry
	132, // 55: milvus.proto.messages.AlterRLSMetadataMessageBody.policy:type_name -> milvus.proto.messages.RLSPolicyMetadata
	133, // 56: milvus.proto.messages.AlterRLSMetadataMessageBody.principal:type_name -> milvus.proto.messages.RLSPrincipalMetadata
	160, // 57: milvus.proto.messages.RLSPolicyMetadata.policy_type:type_name -> milvus.proto.milvus.RowPolicyType
	161, // 58: milvus.proto.messages.RLSPolicyMetadata.actions:type_name -> milvus.proto.milvus.RowPolicyAction
	162, // 59: milvus.proto.messages.AlterResourceGroupMessageHeader.ResourceGroupConfigsEntry.value:type_name -> milvus.proto.rg.ResourceGroupConfig
	163, // 60: milvus.proto.messages.BatchUpdateManifestV2ColumnGroups.ColumnGroupsEntry.value:type_name -> milvus.proto.data.FieldBinlog
...
func file_messages_proto_init() {
	...
	if !protoimpl.UnsafeEnabled {
		...
		file_messages_proto_msgTypes[20].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateVChannelMessageHeader); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
		...
		file_messages_proto_msgTypes[21].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DropVChannelMessageHeader); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
		...
		file_messages_proto_msgTypes[22].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DropVChannelMessageBody); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
	...
	file_messages_proto_msgTypes[108].OneofWrappers = []interface{}{
	...
	file_messages_proto_msgTypes[132].OneofWrappers = []interface{}{
