...
type GetShardLeadersRequest struct {
	...
	// resource_group restricts the answer to the leaders that sit in replicas
	// whose own resource group is this one. The empty string is the absence of
	// a filter, not a filter that matches nothing, so a caller that does not
	// set it gets every replica of the collection - the only answer this
	// request could give before the field existed.
	//
	// The filter cannot be applied by the caller afterwards: the response
	// flattens every replica into one list per channel, and a replica may
	// borrow nodes from another resource group, so node-set membership is not
	// replica membership. Only the coordinator still holds the replica each
	// leader belongs to.
...
func (x *GetShardLeadersRequest) GetResourceGroup() string {
	if x != nil {
...
var file_query_coord_proto_rawDesc = []byte{
	...
	0x6e, 0x66, 0x6f, 0x52, 0x05, 0x69, 0x6e, 0x66, 0x6f, 0x73, 0x22, 0xd1, 0x01, 0x0a, 0x16, 0x47,
	...
	0x65, 0x53, 0x68, 0x61, 0x72, 0x64, 0x73, 0x12, 0x25, 0x0a, 0x0e, 0x72, 0x65, 0x73, 0x6f, 0x75,
	0x72, 0x63, 0x65, 0x5f, 0x67, 0x72, 0x6f, 0x75, 0x70, 0x18, 0x04, 0x20, 0x01, 0x28, 0x09, 0x52,
	0x0d, 0x72, 0x65, 0x73, 0x6f, 0x75, 0x72, 0x63, 0x65, 0x47, 0x72, 0x6f, 0x75, 0x70, 0x22, 0x8c,
	...
	0x8a, 0x0d, 0x0a, 0x0f, 0x53, 0x65, 0x67, 0x6d, 0x65, 0x6e, 0x74, 0x4c, 0x6f, 0x61, 0x64, 0x49,
	...
	0x72, 0x73, 0x69, 0x6f, 0x6e, 0x49, 0x44, 0x22, 0xaf, 0x05, 0x0a, 0x13, 0x4c, 0x6f, 0x61, 0x64,
