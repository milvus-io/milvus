...
func TestFileSourceFailureLogsProtectConfig(t *testing.T) {
	// 2.6 shares one named rate limiter across every refresher. Other tests may
	// leave a failing background source that consumes its credit, so exercise
	// the actual periodic failure in a fresh process with no competing sources.
	const childEnv = "MILVUS_TEST_FILE_SOURCE_FAILURE"
	if os.Getenv(childEnv) != "1" {
		executable, err := os.Executable()
		require.NoError(t, err)
		cmd := exec.Command(executable, "-test.run=^TestFileSourceFailureLogsProtectConfig$") // #nosec G204 -- Re-exec this test with a fixed filter.
...
func TestYAMLFlattenFailureLogsProtectConfig(t *testing.T) {
	...
	// 2.6 skips unsupported list elements and retains the empty list value.
	assert.Equal(t, map[string]string{canary: ""}, values)
