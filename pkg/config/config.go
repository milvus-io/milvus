...
func Init(opts ...Option) (*Manager, error) {
	...
	if o.FileInfo != nil {
		...
		if err != nil {
			...
			log.Ctx(context.TODO()).Error("failed to add FileSource config", zap.String("error", RedactedValue))
			os.Exit(1)
		}
	}
...
// normalize caller-supplied keys on the management server. An unbounded memo therefore lets
...
func flattenAndMergeMap(prefix string, m map[string]interface{}, result map[string]string) {
	for k, v := range m {
		...
		default:
			...
			if err != nil {
				log.Ctx(context.TODO()).Warn("cast configuration to string failed", zap.String("error", RedactedValue))
				continue
			}
			result[lowerKey(fullKey)] = str
			result[formatKey(fullKey)] = str
