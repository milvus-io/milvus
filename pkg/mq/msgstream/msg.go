...
func convertToByteArray(input interface{}) ([]byte, error) {
	...
	case []byte:
		return output, nil
	default:
		return nil, merr.WrapErrParameterInvalidMsg("Cannot convert interface{} to []byte")
	}
}
...
func (it *InsertMsg) CheckAligned() error {
	numRowsOfFieldDataMismatch := func(fieldName string, fieldNumRows, passedNumRows uint64) error {
		return merr.WrapErrDataIntegrityMsg("the num_rows(%d) of %sth field is not equal to passed NumRows(%d)", fieldNumRows, fieldName, passedNumRows)
	}
	rowNums := it.NRows()
	if it.IsColumnBased() {
	...
	if len(it.GetRowIDs()) != len(it.GetTimestamps()) {
		return merr.WrapErrDataIntegrityMsg("the num_rows(%d) of rowIDs  is not equal to the num_rows(%d) of timestamps", len(it.GetRowIDs()), len(it.GetTimestamps()))
	}

	if uint64(len(it.GetRowIDs())) != it.NRows() {
		return merr.WrapErrDataIntegrityMsg("the num_rows(%d) of rowIDs  is not equal to passed NumRows(%d)", len(it.GetRowIDs()), it.NRows())
	}
...
func (dt *DeleteMsg) CheckAligned() error {
	...
	if numRows != numPks {
		return merr.WrapErrDataIntegrityMsg("the num_rows(%d) of pks is not equal to passed NumRows(%d)", numPks, numRows)
	}
