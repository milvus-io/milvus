...
func NewKafkaClientInstanceWithConfigMap(config kafka.ConfigMap, extraConsumerConfig kafka.ConfigMap, extraProducerConfig kafka.ConfigMap) *kafkaClient {
	...
	log.Ctx(context.TODO()).Info("init kafka config",
		zap.String("commonConfigKeys", ConfigKeysString(config)),
		zap.String("extraConsumerConfigKeys", ConfigKeysString(extraConsumerConfig)),
		zap.String("extraProducerConfigKeys", ConfigKeysString(extraProducerConfig)),
	)
...
func (kc *kafkaClient) specialExtraConfig(current *kafka.ConfigMap, special kafka.ConfigMap) {
	for k, v := range special {
		if existingConf, _ := current.Get(k, nil); existingConf != nil {
			...
			log.Ctx(context.TODO()).Warn("special kafka config overrides existing config", zap.String("key", k))
		}
