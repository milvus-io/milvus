...
func (ms *mqMsgStream) getTsMsgFromConsumerMsg(msg mqwrapper.Message) (TsMsg, error) {
	...
	// overwriteTsForUsingLogService will overwrite tsMsg's beginTs and endTs and all timestamps in insertMsg if using lognode.
	overwriteTsForUsingLogService(msg, tsMsg)
...
// overwriteTsForUsingLogService using lognode generated timestamp if message is generated by lognode.
func overwriteTsForUsingLogService(msg mqwrapper.Message, tsMsg TsMsg) {
	// timestamp is now generated on lognode.
	// overwrite the timestamp.
	// TODO: remove the old timestamp allocation.
	newMsgLayout := message.NewImmutableMessageFromMQConsumedMessage(msg)
	switch newMsgLayout.Version() {
	case message.VersionOld:
		// do nothing.
	case message.VersionV1:
		tsMsg.SetTs(newMsgLayout.TimeTick())
	default:
