...
func InsertRepackFunc(tsMsgs []TsMsg, hashKeys [][]int32) (map[int32]*MsgPack, error) {
	...
	for i, request := range tsMsgs {
		if request.Type() != commonpb.MsgType_Insert {
			return nil, merr.WrapErrParameterInvalidMsg("msgs must be Insert")
		}
		...
		if insertRequest.NRows() != uint64(keysLen) {
			return nil, merr.WrapErrParameterInvalidMsg("the length of hashValue, timestamps, rowIDs, RowData are not equal")
		}
...
func DeleteRepackFunc(tsMsgs []TsMsg, hashKeys [][]int32) (map[int32]*MsgPack, error) {
	...
	for i, request := range tsMsgs {
		if request.Type() != commonpb.MsgType_Delete {
			return nil, merr.WrapErrParameterInvalidMsg("msgs must be Delete")
		}
		...
		if keysLen != timestampLen || int64(keysLen) != deleteRequest.NumRows {
			return nil, merr.WrapErrParameterInvalidMsg("the length of hashValue, timestamps, primaryKeys are not equal")
		}
