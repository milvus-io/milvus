...
func GetMsgTypeFromRaw(payload []byte, properties map[string]string) (commonpb.MsgType, error) {
	...
	if msgType == commonpb.MsgType_Undefined {
		...
		if payload == nil {
			return msgType, merr.WrapErrDataIntegrityMsg("failed to unmarshal message header, payload is empty")
		}
		err := proto.Unmarshal(payload, &header)
		if err != nil {
			return msgType, merr.WrapErrDataIntegrity(err, "failed to unmarshal message header")
		}
		if header.Base == nil {
			return msgType, merr.WrapErrDataIntegrityMsg("failed to unmarshal message, header is uncomplete")
		}
