...
class HelmEnv(BaseEnv):
    ...
    def start_up(self, helm_path, helm_install_params):
        ...
        server_tag = (
            helm_install_params["server_tag"]
            if "server_tag" in helm_install_params
            else None
        )
        server_config = (
            helm_install_params["server_config"]
            if "server_config" in helm_install_params
            else None
        )
        ...
        server_config = helm_utils.update_server_config(
