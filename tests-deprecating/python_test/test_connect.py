...
class TestConnect:
    ...
    def test_connect_uri(self, args):
        ...
        milvus = get_milvus(
            args["ip"], args["port"], uri=uri_value, handler=args["handler"]
        )
    ...
    def test_connect_with_multiprocess(self, args):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            future_results = {executor.submit(connect): i for i in range(100)}
...
class TestConnectIPInvalid(object):
    ...
    @pytest.fixture(scope="function", params=gen_invalid_ips())
...
class TestConnectURIInvalid(object):
    ...
    @pytest.fixture(scope="function", params=gen_invalid_uris())
