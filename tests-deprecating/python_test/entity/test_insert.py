...
class TestInsertBase:
    ...
    ******************************************************************
    ...
    def test_insert_default_partition(self, connect, collection):
        ...
        """
        ids = connect.insert(
            collection, default_entities, partition_name=default_partition_name
        )
        assert len(ids) == default_nb
        connect.flush([collection])
        stats = connect.get_collection_stats(collection)
...
class TestInsertAsync:
    ...
    def test_insert_async_callback(self, connect, collection, insert_count):
        ...
        future = connect.insert(
            collection, gen_entities(nb), _async=True, _callback=self.check_result
        )
        future.done()
        ids = future.result()
        assert len(ids) == nb
...
class TestInsertInvalid(object):
    ...
    @pytest.fixture(scope="function", params=gen_invalid_vectors())
