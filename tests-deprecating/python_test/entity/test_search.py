...
default_query, default_query_vecs = gen_query_vectors(
    field_name, entities, default_top_k, nq
)
default_binary_query, default_binary_query_vecs = gen_query_vectors(
    binary_field_name, binary_entities, default_top_k, nq
)
...
class TestSearchBase:
    ...
    @pytest.fixture(scope="function", params=gen_index())
    ...
    @pytest.fixture(scope="function", params=[1, 10, 1100])
    ...
    def _test_search_field(self, connect, collection, get_top_k, get_nq):
        ...
        if top_k <= max_top_k:
            ...
            for i in range(nq):
                assert entities[1]["values"][:nq][i] in [
                    r.entity.get("float") for r in res[i]
                ]
        else:
    ...
    def _test_search_after_delete(self, connect, collection, get_top_k, get_nq):
        ...
        query, vecs = gen_query_vectors(
            field_name, entities, top_k, nq, search_params=search_param
        )
        vecs[:] = []
        ...
        if top_k > 1:
            ...
            assert res2[0][0].entity.get(
                "int64") == res[0][1].entity.get("int64")

    @pytest.mark.tags(CaseLabel.L2)
    def test_search_after_index(
        self, connect, collection, get_simple_index, get_top_k, get_nq
    ):
        """
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_after_index_different_metric_type(
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_index_empty_partition(
        self, connect, collection, get_simple_index, get_top_k, get_nq
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
        ...
        if top_k > max_top_k:
            ...
            res = connect.search(
                collection, query, partition_names=[default_tag])
            assert len(res[0]) == 0

    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.timeout(600)
    def test_search_index_partition(
        self, connect, collection, get_simple_index, get_top_k, get_nq
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
        ...
        entities, ids = init_data(
            connect, collection, partition_names=default_tag)
        connect.create_index(collection, field_name, get_simple_index)
        search_param = get_search_param(index_type)
        query, vecs = gen_query_vectors(
            field_name, entities, top_k, nq, search_params=search_param
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_index_partition_not_existed(
        self, connect, collection, get_top_k, get_nq, get_simple_index
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
        ...
        if top_k > max_top_k:
            with pytest.raises(Exception) as e:
                res = connect.search(
                    collection, query, partition_names=["new_tag"])
        else:
            connect.load_collection(collection)
            with pytest.raises(Exception) as e:
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_index_partitions(
        self, connect, collection, get_simple_index, get_top_k
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
        ...
        entities, ids = init_data(
            connect, collection, partition_names=default_tag)
        new_entities, new_ids = init_data(
            connect, collection, nb=6001, partition_names=new_tag
        )
        connect.create_index(collection, field_name, get_simple_index)
        search_param = get_search_param(index_type)
        query, vecs = gen_query_vectors(
    ...
    def test_search_index_partitions_B(
        ...
        print(
            f'new_entities[-1]["values"][:1]: {new_entities[-1]["values"][:1]}')
        query, vecs = gen_query_vectors(
            field_name,
            new_entities,
            top_k,
            ...
            search_params=search_param,
            replace_vecs=[
                entities[-1]["values"][:1][0],
                new_entities[-1]["values"][:1][0],
            ],
        )
        if top_k > max_top_k:
            with pytest.raises(Exception) as e:
            ...
            connect.load_collection(collection)
            res = connect.search(
                collection, query, partition_names=["(.*)tag"])
            assert check_id_result(res[0], ids[0])
            assert check_id_result(res[0], new_ids[0])
            assert res[0]._distances[0] < epsilon
            assert res[1]._distances[0] < epsilon
            res = connect.search(
                collection, query, partition_names=["new(.*)"])
    ...
    def test_search_ip_flat(
        ...
        entities, ids = init_data(connect, collection)
        query, vecs = gen_query_vectors(
            field_name, entities, top_k, nq, metric_type="IP"
        )
        connect.load_collection(collection)
        res = connect.search(collection, query)
        assert len(res[0]) == top_k
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_ip_after_index(
        self, connect, collection, get_simple_index, get_top_k, get_nq
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_ip_index_empty_partition(
        self, connect, collection, get_simple_index, get_top_k, get_nq
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
        ...
        if top_k > max_top_k:
            ...
            assert len(res) == nq
            assert len(res[0]) >= top_k
            assert res[0]._distances[0] >= 1 - \
                gen_inaccuracy(res[0]._distances[0])
            assert check_id_result(res[0], ids[0])
            res = connect.search(
                collection, query, partition_names=[default_tag])
            assert len(res[0]) == 0
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_ip_index_partitions(
        self, connect, collection, get_simple_index, get_top_k
    ):
        """
        target: test basic search function, all the search params is correct, test all index params, and build
    ...
    def test_search_distance_l2(self, connect, collection):
        ...
        inside_query, inside_vecs = gen_query_vectors(
            field_name, entities, default_top_k, nq, search_params=search_param
        )
        ...
        res = connect.search(collection, query)
        assert abs(
            np.sqrt(res[0]._distances[0]) - min(distance_0, distance_1)
        ) <= gen_inaccuracy(res[0]._distances[0])

    @pytest.mark.tags(CaseLabel.L2)
    def test_search_distance_l2_after_index(
        self, connect, id_collection, get_simple_index
    ):
        """
        target: search collection, and check the result: distance
    ...
    def test_search_distance_ip(self, connect, collection):
        ...
        assert abs(res[0]._distances[0] -
                   max(distance_0, distance_1)) <= epsilon

    @pytest.mark.tags(CaseLabel.L2)
    def test_search_distance_ip_after_index(
        self, connect, id_collection, get_simple_index
    ):
        """
        target: search collection, and check the result: distance
    ...
    def test_search_distance_jaccard_flat_index(self, connect, binary_collection):
        ...
        """
        nq = 1
        int_vectors, entities, ids = init_binary_data(
            connect, binary_collection, nb=2)
        query_int_vectors, query_entities, tmp_ids = init_binary_data(
            connect, binary_collection, nb=1, insert=False
        )
        distance_0 = jaccard(query_int_vectors[0], int_vectors[0])
        distance_1 = jaccard(query_int_vectors[0], int_vectors[1])
        query, vecs = gen_query_vectors(
            binary_field_name, query_entities, default_top_k, nq, metric_type="JACCARD"
        )
    ...
    def test_search_binary_flat_with_L2(self, connect, binary_collection):
        ...
        query, vecs = gen_query_vectors(
            binary_field_name, query_entities, default_top_k, nq, metric_type="L2"
        )
        with pytest.raises(Exception) as e:
    ...
    def test_search_distance_hamming_flat_index(self, connect, binary_collection):
        ...
        query, vecs = gen_query_vectors(
            binary_field_name, query_entities, default_top_k, nq, metric_type="HAMMING"
        )
        connect.load_collection(binary_collection)
        res = connect.search(binary_collection, query)
        assert (
            abs(res[0][0].distance - min(distance_0, distance_1).astype(float))
            <= epsilon
        )
    ...
    def test_search_distance_substructure_flat_index_B(
        self, connect, binary_collection
    ):
        """
        target: search binary_collection, and check the result: distance
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_distance_superstructure_flat_index_B(
        self, connect, binary_collection
    ):
        """
        target: search binary_collection, and check the result: distance
        ...
        int_vectors, entities, ids = init_binary_data(
            connect, binary_collection, nb=2)
        query_int_vectors, query_vecs = gen_binary_super_vectors(
            int_vectors, 2)
        query, vecs = gen_query_vectors(
            binary_field_name,
            entities,
    ...
    def test_search_distance_tanimoto_flat_index(self, connect, binary_collection):
        ...
        query, vecs = gen_query_vectors(
            binary_field_name, query_entities, default_top_k, nq, metric_type="TANIMOTO"
        )
        connect.load_collection(binary_collection)
        res = connect.search(binary_collection, query)
    ...
    def _test_query_entities_with_field_less_than_top_k(self, connect, id_collection):
        ...
        top_k = 300
        default_query, default_query_vecs = gen_query_vectors(
            field_name, entities, top_k, nq, search_params={"nprobe": 1}
        )
...
class TestSearchDSL(object):
    ...
    ******************************************************************
    ...
    def test_query_no_vector_range_only(self, connect, collection):
        ...
        """
        # entities, ids = init_data(connect, collection)
        expr = {"must": [gen_default_range_expr]}
        query = update_query_expr(default_query, keep_old=False, expr=expr)
        with pytest.raises(Exception) as e:
            res = connect.search(collection, query)
    ...
    def test_query_wrong_format(self, connect, collection):
        ...
        """
        # entities, ids = init_data(connect, collection)
        expr = {"must1": [gen_default_term_expr]}
        query = update_query_expr(default_query, keep_old=False, expr=expr)
        with pytest.raises(Exception) as e:
            res = connect.search(collection, query)
    ...
    ******************************************************************
    ...
    def test_query_term_value_not_in(self, connect, collection):
        ...
        expr = {
            "must": [
                ...
                gen_default_term_expr(values=[100000]),
            ]
        }
        query = update_query_expr(default_query, expr=expr)
        connect.load_collection(collection)
    ...
    def test_query_term_value_all_in(self, connect, collection):
        ...
        expr = {
            "must": [
                gen_default_vector_expr(default_query),
                gen_default_term_expr(values=[1]),
            ]
        }
        query = update_query_expr(default_query, expr=expr)
        connect.load_collection(collection)
    ...
    def test_query_term_values_all_in(self, connect, collection):
        ...
        entities, ids = init_data(connect, collection)
        expr = {
            "must": [gen_default_vector_expr(default_query), gen_default_term_expr()]
        }
        query = update_query_expr(default_query, expr=expr)
        connect.load_collection(collection)
        res = connect.search(collection, query)
        ...
        expr = {
            "must": [
                ...
                gen_default_term_expr(
                    values=list(
                        range(default_nb // 2, default_nb + default_nb // 2))
                ),
            ]
        }
        query = update_query_expr(default_query, expr=expr)
    ...
    def test_query_term_values_repeat(self, connect, collection):
        ...
        expr = {
            "must": [
                gen_default_vector_expr(default_query),
                gen_default_term_expr(
                    values=[1 for i in range(1, default_nb)]),
            ]
        }
        query = update_query_expr(default_query, expr=expr)
        connect.load_collection(collection)
    ...
    ******************************************************************
    ...
    @pytest.fixture(scope="function", params=gen_invalid_term())
    ...
    def test_query_term_field_named_term(self, connect, collection):
        ...
        term_param = {
            "term": {"term": {"values": list(range(default_nb // 2))}}}
    ...
    ******************************************************************
    ...
    @pytest.fixture(scope="function", params=gen_invalid_range())
    ...
    @pytest.fixture(scope="function", params=gen_valid_ranges())
    ...
    ************************************************************************
    ...
    def test_query_multi_term_has_common(self, connect, collection):
        ...
        term_second = gen_default_term_expr(
            values=list(range(default_nb // 3)))
        expr = {
    ...
    def test_query_multi_term_different_fields(self, connect, collection):
        ...
        term_first = gen_default_term_expr()
        term_second = gen_default_term_expr(
            field="float", values=[float(i) for i in range(default_nb // 2, default_nb)]
        )
        expr = {
            "must": [gen_default_vector_expr(default_query), term_first, term_second]
        }
    ...
    def test_query_multi_range_has_common(self, connect, collection):
        ...
        range_two = gen_default_range_expr(
            ranges={"GT": 1, "LT": default_nb // 3})
        expr = {"must": [gen_default_vector_expr(
            default_query), range_one, range_two]}
        query = update_query_expr(default_query, expr=expr)
        connect.load_collection(collection)
        res = connect.search(collection, query)
        assert len(res) == nq
    ...
    def test_query_multi_range_no_common(self, connect, collection):
        ...
        range_one = gen_default_range_expr()
        range_two = gen_default_range_expr(
            ranges={"GT": default_nb // 2, "LT": default_nb}
        )
        expr = {"must": [gen_default_vector_expr(
            default_query), range_one, range_two]}
        query = update_query_expr(default_query, expr=expr)
    ...
    def test_query_multi_range_different_fields(self, connect, collection):
        ...
        range_second = gen_default_range_expr(
            field="float", ranges={"GT": default_nb // 2, "LT": default_nb}
        )
        expr = {
            "must": [gen_default_vector_expr(default_query), range_first, range_second]
        }
        query = update_query_expr(default_query, expr=expr)
        connect.load_collection(collection)
        res = connect.search(collection, query)
    ...
    def test_query_single_range_multi_fields(self, connect, collection):
        ...
        range_second = {
            "float": {"GT": default_nb / 2, "LT": float(default_nb)}}
...
class TestSearchInvalid(object):
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_with_invalid_partition(
        self, connect, collection, get_invalid_partition
    ):
        # tag = " "
        tag = get_invalid_partition
        with pytest.raises(Exception) as e:
            res = connect.search(
                collection, default_query, partition_names=tag)

    @pytest.mark.tags(CaseLabel.L2)
    def test_search_with_invalid_field_name(
        self, connect, collection, get_invalid_field
    ):
    ...
    @pytest.fixture(scope="function", params=gen_invaild_search_params())
    ...
    def test_search_with_invalid_params(
        self, connect, collection, get_simple_index, get_search_params
    ):
        """
    ...
    # #1464
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_with_empty_params(
        self, connect, collection, args, get_simple_index
    ):
        """
        ...
        connect.load_collection(collection)
        query, vecs = gen_query_vectors(
            field_name, entities, default_top_k, 1, search_params={}
        )
        with pytest.raises(Exception) as e:
            res = connect.search(collection, query)
    ...
    def test_search_with_empty_vectors(self, connect, collection):
        ...
        connect.load_collection(collection)
        query, vecs = gen_query_vectors(
            field_name, entities, default_top_k, nq=0)
        with pytest.raises(Exception) as e:
            res = connect.search(collection, query)
...
class TestSearchWithExpression(object):
    ...
    @pytest.mark.tags(CaseLabel.tags_smoke)
    def test_search_with_expression(
        self, connect, collection, index_param, search_params, limit, expression
    ):
        entities, ids = init_data(connect, collection)
        assert len(ids) == default_nb
        connect.create_index(
            collection, default_float_vec_field_name, index_param)
        connect.load_collection(collection)
        nq = 10
        query_data = entities[2]["values"][:nq]
        res = connect.search_with_expression(
            collection,
            query_data,
