# Summary L0 Materializer Design (Retained for QueryView)
...
**Status:** Implemented and tested in `vchannel/l0materializer/window.go`, but
not connected to the current RecoveryStorage runtime. This is the target
materializer after QueryView replaces legacy channel-checkpoint recovery.
The currently wired consumer is [WAL L0 Materializer](l0_materializer.md).
The no-age-timer and L1-final-commit rules below apply to this retained implementation.
