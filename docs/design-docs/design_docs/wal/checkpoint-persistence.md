...
If snapshot persistence exits with a fatal error while the WAL is still open,
the owner marks the WAL unavailable. It must not leave a writable WAL whose
checkpoint publisher has stopped. The frozen failed batch remains pending and
the published checkpoint is unchanged; normal Close cancellation is excluded.
