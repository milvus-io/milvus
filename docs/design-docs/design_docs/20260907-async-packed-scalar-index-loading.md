...
1. `AsyncIndexEntryReader::Open` reads the object's directory and index metadata.
   The resulting **catalog** describes what is stored, without loading every entry.
...
3. Call `reader->ReadEntriesAsync(plan)` to prepare destinations and read slices
...
The call chain is `Open` → `PlanLoad` → `ReadEntriesAsync` →
...
| Open the input stream; parse directory and metadata; choose destinations | Shared `AsyncLoadExecutor` |
...
| Native remote range read | Storage backend asynchronous I/O; completion resumes the coroutine |
| Generic Arrow range read | Arrow `ReadAsync`; copy the returned buffer into the destination |
| Non-Arrow `InputStream` read | Synchronous read on the shared async worker |
...
Input stream opening and non-Arrow fallback reads can still block an async worker.
Engine restoration is also a synchronous call on that worker; async transport does
not make a parser or engine call itself asynchronous.
...
The range reader retains Arrow status details for its existing bounded retry
classification. Terminal statuses pass through the storage error converter.
...
completion, shared admission with one async worker, cancellation/draining,
