...
- **Empty literals** (`LIKE '%'` / `LIKE '%%'`, lowered to an anchored op with
  ...
  `ShouldUseOp` returns true for an empty pattern **on the anchored ops**. Null
  rows stay excluded, so `NULL LIKE '%'` is not a match. `Match` is the
  exception: an empty pattern leaves phase 1 with no literal fragment to search
  for, so it declines exactly like the other empty-fragment patterns (`%`,
  `%_%`) and the executor runs the scan. The planner never produces it anyway —
  a wildcard-free `LIKE` lowers to `Equal` (`optimizeLikePattern`).
...
(`MatchGuardAccepts`). Locate cost is `occ × sa_sample_rate`. Phase 2 reads
those candidate rows back from sealed VARCHAR via `ProcessDataByOffsets`
(ascending offsets, views fetched per same-chunk run). Its candidate-byte cost
is NOT priced by the bound, which stays locate-only
(`occ × sa_sample_rate < ratio × tokens`) — a known approximation whose worst
case is long average rows × unselective fragments, where phase 2 approaches a
full column read on top of the locate. Recalibrating (or adding a
candidate-bytes term) is deferred until end-to-end measurements across row
length and selectivity are available. A pattern with no usable factor
(`LIKE '%'`, `LIKE '%_%'`) declines to the scan. ISA is not built at load;
Match does not call `Extract`.
...
(accepted for the anchored ops, declined for `Match`):
