...
- An exact `uint64` model: `uint64 > INT64_MAX` continues to use the legacy
  uint64-to-double comparison contract on raw, stats, Flat and a `DOUBLE`
  Path index. An `INT64` Path index stores such values as null, so the
  predicate is `UNKNOWN` there; see "Query time: exact int64" above.
