...
internal/streamingnode/server/wal/interceptors/pkindex   the interceptor (a thin shell)
        ...
        ▼
internal/pkindex/authority/decider                       the write path decisions
...
- The interceptor guarantees the sequence `Decide`, the WAL write, then exactly
  one of `Apply` and `Discard`. A deferred guard discards a decision that a
  panic left unresolved.
...
`test` build tag. A production binary has no engine, the interceptor logs one
warning and stays a pass-through, so the switch has no effect there.
...
- A delete by expression (`DeleteRequest.SerializedExprPlan`) carries no keys
  and passes through. No producer sets that field yet.
- Every redo of an upgraded insert writes one BeginTxn and one RollbackTxn.
- An index failure makes the append fail with a retriable error. The producer
  retries such errors without a limit, so a permanently broken engine stalls
  the writes of that collection instead of failing them.
