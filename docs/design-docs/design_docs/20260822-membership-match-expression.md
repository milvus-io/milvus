...
7. **Proxy config**: the old kind-specific names converge while preserving two
   ...
   `proxy.maxMembershipFilterPlanSize` (default 128 MiB) independently limits
   aggregate serialized membership-bearing plan bytes and aggregate estimated
   decoded bytes for one request, and falls back to
   `proxy.maxBloomFilterPlanSize`. The parser uses that same aggregate ceiling
   for conservative body-occurrence and decoded-memory preflights shared by all
   HybridSearch sub-requests and scorer filters; exact `proto.Size` accounting
   remains the final serialized-size gate. Keeping the two configurations separate prevents a legacy plan
   setting from silently widening the per-blob admission limit. The fixed MRB1
   admissions (262,144 high containers and a 64 MiB decoded estimate per
