...
AdmissionTicket ObserveMiss(const Key& key, int64_t active_count);
...
`memory.maxBytes` counts compressed `EntryPool` payloads, while
`disk.maxBytes` counts disk slots. Both modes therefore use the independent
`materialization.maxBytes` heap budget.

Before a backend allocates or reads a cache-hit value, the manager reserves
`2 * ceil(active_count / 64) * 8` bytes for its result/validity pair. The
reservation is non-blocking. If it would exceed the configured limit,
`GetWithStatus` returns `ResourceLimit` without calling either backend. A
successful lookup returns two aliasing `shared_ptr`s backed by one per-lookup
holder. The holder owns both bitmaps and the reservation, so the bytes remain
charged until the last result or validity reference is released, including
...
Memory-mode lookup keeps the `EntryPool` shared lock while decompressing the
entry. This lets concurrent reads proceed and prevents eviction from
invalidating the payload without first copying the full compressed buffer.
`Put` and erase operations wait for active decodes.
...
and active count) and the manager's config epoch. `SetConfig()` increments the
epoch and resets the frequency tracker; `PutAdmitted()` rejects a stale ticket
...
through `GetMaterializationBytes()` for tests and diagnostics; it is not added
to either storage gauge.
...
- `EntryPool` retains a shared index lock through decompression; puts and
  erases take the exclusive lock.
