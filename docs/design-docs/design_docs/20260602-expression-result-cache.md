...
| `queryNode.exprCache.admissionThreshold` | `2` | yes | Per-cache-key frequency admission threshold shared by memory and disk modes. `1` disables frequency admission. |
...
- **Frequency admission:** cache only after the same segment id and expression signature have been observed at least `queryNode.exprCache.admissionThreshold` times.
...
Tickets are bound to the full cache-key hash (segment id plus expression
signature) and the manager's config epoch. `SetConfig()` increments the epoch
and resets the frequency tracker; `PutAdmitted()` rejects a stale ticket or one
issued for a different segment or signature. The active count is deliberately
not part of the admission hash because it versions the same logical cache
entry. Scoping frequency to the full cache key prevents one query that fans the
same expression out across many segments from satisfying the threshold for
otherwise one-off per-segment entries. Latency admission cannot be decided
...
- **Admission:** threshold 2 observes once per expression instance rather than
  once per batch; frequencies are isolated by segment id and signature;
  never-evaluated `MoveCursor()` instances do not count; tickets reject a
  different segment, signature, or config epoch; latency tests use synthetic
