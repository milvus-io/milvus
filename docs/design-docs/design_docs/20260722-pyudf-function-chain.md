...
This is the single design and operational reference for PyUDF, including the
worker pool, protocol, configuration, deployment, verification, and benchmarks.
Current behavior is described in sections 1–13; section 14 separates acceptance
requirements from recorded results and historical implementation milestones.
The implementation links are the references for configuration, protocol
generation, and tests.

## Contents

- [Scope and decisions](#1-scope-and-decisions)
- [Architecture and ownership](#2-architecture-and-ownership)
- [Startup and process management](#3-startup-and-process-management)
- [Go client and connection reuse](#4-go-client-and-connection-reuse)
- [Concurrency and UDF execution](#5-concurrency-and-udf-execution)
- [Protocol and Arrow IPC](#6-independent-protocol-and-arrow-ipc)
- [UDF loading and wheel updates](#7-udf-loading-caching-and-wheel-updates)
- [FileResource boundary](#8-fileresource-boundary)
- [Timeout, retry, and health](#9-timeout-retry-and-health)
- [Errors and propagation](#10-errors-and-propagation)
- [Shutdown and recovery](#11-shutdown-abnormal-exit-and-recovery)
- [Configuration](#12-configuration)
- [Packaging and generated files](#13-packaging-and-generated-files)
- [Verification, implementation history, and benchmarks](#14-implementation-references-and-verification)
- [Follow-up work](#15-follow-up-work)
...
`group_by` also puts numeric scores before NaNs for both within-group trimming
and cross-group ranking, in either direction. NaN ties retain the existing
rules: ID ascending within a group; group size descending, then first ID
ascending, between groups. Sum/average aggregation can itself produce NaN
(for example, from positive and negative infinity); that group ranks after
numeric group scores.
When exporting to `SearchResultData.Scores`, the final `$score` must be Float32,
non-null, and finite. L0 scores satisfy the same constraint before entering
QueryNode heap merge, while their null bitmap is still available. Validation
runs before heap comparisons and topK selection; invalid candidates are rejected,
not silently filtered or converted to zero. Other output columns may retain NaN
and infinity.
...
### Factory and instance contract

The wheel's `entry_points.txt` must declare exactly one entry in the
`milvus.pyudf` group, using `module:factory` with a callable factory. For example:

```toml
[project.entry-points."milvus.pyudf"]
main = "example_udf:factory"
```

The factory receives an immutable `PyUDFContext` containing `resource_name`,
canonical `wheel_path`, `stage`, a logger, and immutable `runtime_info` with the
Python implementation, version, and executable. It returns an object implementing
exactly one callable `transform` or `transform_query`. The current worker Execute
path requires `transform_query`; an object implementing only `transform` can be
loaded but cannot execute this path.

`transform_query(params, columns)` receives an immutable parameter mapping and
a tuple of `pyarrow.Array` objects. It returns a sequence of Arrow arrays under
the output contract in section 6. An optional `close` attribute must be callable.
If wrapper validation fails after factory creation, the loader attempts `close`
without replacing the original failure if cleanup also fails. Cached instances
are not closed per request or during process shutdown.

### Wheel loading and cache ownership
...
The protocol enum assigns 0 to unspecified, 1 to `UDF_FAILED`, 2–4 to the three
resource I/O categories in table order, 5 to `OUT_OF_MEMORY`, 6 to `UNSUPPORTED`,
7 to `INTERNAL`, and 8 to `INVALID_ARGUMENT`. These numbers are not Search status
codes. `INVALID_ARGUMENT` is an InputError; the other defined worker categories
retain their existing System classification. All defined structured worker error
categories are non-retriable; transport errors have their own mappings.
...
| Empty worker resource name or invalid internal wheel path | ServiceInternal (5) |
| Protobuf decoding failure before handler dispatch | gRPC INTERNAL, mapped to ServiceInternal (5) |
...
For source-to-consumer audits, follow these boundaries:

| Boundary | Audit references and required checks |
|---|---|
| Python source to response | [loader.py](../../../internal/util/function/pyudf/python/milvus_pyudf_runtime/loader.py), [instance.py](../../../internal/util/function/pyudf/python/milvus_pyudf_runtime/instance.py), and [worker.py](../../../internal/util/function/pyudf/python/milvus_pyudf_runtime/worker.py): distinguish resource I/O, user exceptions, parameter errors, and runtime failures at their origin |
| Response to expression | [client_errors.go](../../../internal/util/function/pyudf/client_errors.go), [client_ipc.go](../../../internal/util/function/pyudf/client_ipc.go), and [pyudf_expr.go](../../../internal/util/function/chain/expr/pyudf_expr.go): preserve typed errors and cancellation; malformed successful responses are internal failures |
| Expression to FunctionChain | [operator_map.go](../../../internal/util/function/chain/operator_map.go) and [chain.go](../../../internal/util/function/chain/chain.go): preserve execution errors and distinguish later business output validation |
| FunctionChain to Search | [search_pipeline.go](../../../internal/proxy/search_pipeline.go), [task_search.go](../../../internal/proxy/task_search.go), [task_scheduler.go](../../../internal/proxy/scheduler/task_scheduler.go), and [impl.go](../../../internal/proxy/impl.go): trace task completion through `merr.Status` without publishing partial results |
| Status to SDK | [merr/utils.go](../../../pkg/util/merr/utils.go) and the supported SDK's status decoder: assert Code, Retriable, ExtraInfo, and compatible/input-error fields where exposed, not only message text |

Audit every construction and wrapping site for a category before claiming that
it survives this path. Trace or inject each real failure, including resource
I/O, invalid output, memory allocation, cancellation, and timeout. A real OOM
kill or native crash is a transport/process failure, not evidence that Python
returned `OUT_OF_MEMORY`. SDK retries and user resubmissions must be evaluated
separately from the Go client's no-replay contract.
...
Check generated products without changing them with
`make check-pyudf-proto-product PYTHON=/path/to/codegen/environment/bin/python`.
Runtime installation and code generation are separate: production only needs
the runtime dependencies, while development codegen requires its pinned tools.
...
Use a controlled Go gRPC service to verify that the Go client imposes no local
admission limit. A real worker may reject calls at its configured gRPC limit;
the test must not require every submitted call to enter user code. Test shutdown
while the UDF remains blocked, and prove process exit with wait/reap results.
Releasing the UDF first or relying on test-harness cleanup does not prove product
shutdown. An ordinary RPC timeout must be tested while Milvus remains running.
Arbitrary UDF child processes, uninterruptible kernel waits, and abnormal exits
that bypass signal handling remain deployment limitations.

Before claiming a behavioral change complete, audit upstream value construction,
trace or inject each relevant failure through its consumer, and perform an
adversarial review of untested paths. Changes to wire projections, old-code
mappings, or metric labels require the merr guard tests and full `make test-go`.

### Implementation milestones and recorded verification

The worker-pool implementation used the following milestones. These names are
retained to explain historical results; they are not separate design documents
or a claim that every deployment acceptance item is complete.

| Milestone | Delivered scope / acceptance boundary |
|---|---|
| P0: Contract | Configuration, unary protocol, error origins, lifecycle, and acceptance requirements, now maintained in this document |
| P1: Protocol and packaging | Generated protocol, Go configuration/argv, Python argument parsing, standard health service, pinned dependencies, and installable runtime wheel |
| P2: Single worker | Real gRPC Execute, IPC, shared-instance loading, cancellable lock waits, output validation, source-specific errors, and signal exit |
| P3: Process lifecycle | Go startup/stop ownership, Python fork/reap/replacement, backoff, shared-port behavior, and TERM/KILL shutdown |
| P4: Go client | Shared fixed connection pool, synchronous Arrow conversion, complete-call deadline, error translation, reference cleanup, and no application replay |
| P5: Proxy integration | Lifecycle wiring, FileResource path resolution, FunctionChain Execute, connection cleanup, and removal of Runtime/Lease and embedded execution |
| P6: Deployment and CI | Build/install targets and Python CI integration exist; clean-image and supported-deployment validation remain acceptance requirements |
| P7: Delivery and performance | Standalone integration and local benchmark records exist; cluster, native macOS, and broader combined-failure evidence must be established separately |

Implementation dependencies were P0 → P1; P2/P3/P4 built on P1, with real-worker
lifecycle and client validation depending on P2; P5 integrated P2–P4; deployment
and final acceptance followed P5. Module failures were tested during each stage,
rather than deferred to deployment testing.

Historical validation records from September 15–16, 2026 reported:

- Protocol generation reproducibility, wheel build/install checks, Python
  formatting, Go configuration and lifecycle tests, and worker/client tests.
  Python suite counts varied from 38 to 57 as the implementation evolved; these
  counts do not describe the current suite. Early wheels included schema/common
  descriptors; the current protocol is independent, as described in section 6.
- Concurrent instance creation, same-instance execution, cancellation during
  loading, import failure recovery, package conflicts, zero-row and multi-query
  IPC, dictionary/compressed input, message limits, and output contract errors.
  Historical cross-language stress cases included 2,048 columns and 16,385 query
  batches; these are test cases, not protocol limits.
- Linux fork/init failure recovery, bind failures, duplicate pools at one port,
  blocked-handler shutdown, ignored TERM followed by KILL, reaping, and continued
  service after worker replacement. Go lifecycle code/tests cross-compiled for
  Darwin arm64 and amd64; native macOS behavior was not established by that check.
- Go client race tests, connection reuse and partial-creation rollback,
  concurrent admission, error categories, malformed responses, and no replay
  after a test service had begun processing. Go unit tests now use Go services
  and helper processes; historical Go-to-Python tests are not retained as a
  prerequisite for running Go UT.
- Six standalone Search tests passed with real wheel upload/download, multiple
  UDFs, derived columns, recursive parameters, output failures, and recovery.
  Separate SDK fault injections reported absent-resource code 1100, a missing
  wheel code 1000, recovery scores `[20, 40, 60]`, and deadline code 10001.
  A blocked-UDF shutdown test reported process reaping in about 4.31 s without
  running the UDF's `finally`. Those experiments used `rpcTimeout=2s` and
  `shutdownTimeout=3s`, values outside today's validation ranges; use the current
  configuration table for new runs.
- A full Go regression attempt stopped after a port-22125 conflict in
  `internal/proxy.TestProxy`. It did not establish a passing full repository
  suite. Standalone evidence did not establish cluster, native macOS, production
  image, or all combined resource/transport failure behavior.

The September 20, 2026 worker review ran the Python suite on Python 3.12 with
the pinned gRPC/Arrow dependencies and local socket access: 72 tests passed and
the installed-wheel test was skipped. That run did not repeat full Go or Search
E2E validation. Earlier parameter-error records used INTERNAL/5; the current
worker returns INVALID_ARGUMENT/1100 for unset values and detected logical-depth
violations, while protobuf decoding failures remain gRPC INTERNAL/5.

READY pipes, shared readiness state, parent-PID monitoring, and the embedded
runtime are superseded designs. Their historical experiments are not evidence
for the current signal-driven lifecycle. Keep new verification records scoped
to the actual implementation, dependency versions, and deployment tested.
...
The [standalone worker benchmark](../../../internal/util/function/pyudf/python/benchmarks/bench_worker.py)
...
Run the script with `--help` for its options. Its `--startup-timeout` is a
benchmark-only channel-readiness budget, not a production PyUDF startup setting.
