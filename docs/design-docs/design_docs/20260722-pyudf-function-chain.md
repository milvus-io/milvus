...
The expression package owns a process-global Production Runtime, following the existing XGBoost expression ownership pattern. Package initialization registers the expression and a FileResource listener. Each `PyUDFExpr` construction checks the process-wide enable gate before selecting any backend; the first enabled expression construction reads the immutable configuration, validates build capability, constructs the global runtime, and binds the expression to it without component-specific FunctionChain injection. Embedded CPython is initialized lazily and exactly once when the enabled runtime handles its first `Acquire`.
...
When constructed directly with disabled configuration, Production Runtime returns an unavailable runtime and never initializes CPython. Normal expression construction rejects the process-wide disabled configuration before selecting a runtime or backend.

The expression package creates Production Runtime as a process-global object:

- the first request-level expression construction reads the non-refreshable configuration and constructs the final Production Runtime with a process-lifetime context, matching the XGBoost expression ownership pattern;
- when enabled, construction validates build capability and prepares the cache, but does not initialize CPython; configuration, capability, or cache construction failure is returned to the expression builder and cached by the global runtime provider;
...
- when Production Runtime is constructed with disabled configuration, it installs an unavailable Runtime without checking native capability or initializing CPython;
- package initialization registers process-global resource metadata as FileResource listener `pyudf`; the runtime cache subscribes to it and receives the current ready snapshot by replay;
...
- `enabled` is checked by every `PyUDFExpr` construction before selecting any backend, including an explicitly supplied Runtime;
- Production Runtime independently keeps a defensive disabled check for direct callers; enabled construction validates native build capability and creates the cache, but embedded CPython initialization is deferred until the first `Acquire`;
- a deferred CPython initialization failure is returned by the first `Acquire` and cached for subsequent acquisitions.

Configuration is non-refreshable. Runtime selection does not silently fall back.

### Configuration initialization ordering

PyUDF `NewConfig` is the single adapter that reads the PyUDF-owned fields from `paramtable.Get().FunctionCfg`; both `CheckEnabled` and the global runtime provider use that adapter. The first call occurs during `PyUDFExpr` construction. Every process mode, including Embedded Milvus, must therefore initialize its intended parameter table before building a FunctionChain containing `py_udf`. Importing the FunctionChain expression package alone only registers the expression and FileResource listener and does not read the PyUDF configuration.
...
- lazy package-global runtime construction, expression-level enable gating, resource-listener replay, and request-context separation;
...
3. add Embedded Milvus coverage proving its parameter table is initialized before the first `py_udf` FunctionChain is built.
