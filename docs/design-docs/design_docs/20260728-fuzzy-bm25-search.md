...
Each sealed-Worker expansion response and local growing expansion result uses
the existing `quotaAndLimits.limits.maxOutputSize` limit shared by Search and
Query. Milvus measures the complete expansion protobuf result; a result equal
to the limit is accepted, while a larger result fails that expansion subtask
with a dedicated size error. Milvus never truncates candidates to fit the
limit, and the existing partial-result policy decides whether another Worker's
failure can be tolerated.

`fuzzy_max_expansions` is not an aggregate admission limit. Before Phase 1 is
generally enabled, QueryNode must also bound total term/component work, RPC
concurrency, and in-flight native traversals. Static request excess is an
input-size error; runtime saturation is a retriable resource error. The system
rejects excess work rather than globally clipping candidates and changing
query semantics. The per-response size limit is a final output guard, not a
substitute for work admission or temporary-memory control.
...
- corrupt/missing/stale artifacts, generation retry, Worker partial failure,
  expansion output limits, aggregate admission, and cancellation;
