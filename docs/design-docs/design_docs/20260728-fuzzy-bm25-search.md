...
enable fuzzy search. The first release rejects positive `fuzziness` with a
Search Iterator, including an iterator on a hybrid-search subrequest, because
the iterator token does not preserve the rewritten sparse query or Segment FST
generations. Zero or omitted `fuzziness` remains exact BM25 and follows the
...
generation. A generation change is a transient segment failure; when the
partial-result policy cannot accept the reduced target, Milvus restarts the
complete two-phase operation rather than reusing stale candidates. Every target
receives the same final vector and naturally ignores dimensions it does not
contain.
...
reduced target. Sealed expansion uses the same bounded Worker retry context as
Search. A node-not-found or Worker identity mismatch marks all segments in that
request offline before partial-result evaluation, so later requests do not keep
using stale routing.

The existing `quotaAndLimits.limits.maxOutputSize` limit shared by Search and
Query bounds each Worker response, the Delegator's merged expansion, and the
rewritten sparse query. Admission happens before retaining or multiplying
candidates, followed by a final serialized-size check. Milvus never truncates
candidates to fit the limit, and the existing partial-result policy decides
whether another Worker's failure can be tolerated.

RPC and native-expansion semaphores are shared by all shard Delegators on one
QueryNode and use the existing maximum read concurrency as their capacity.
Concurrency saturation is a retriable resource error. The response-size limit
remains the shared guard for materialized expansion results; fuzzy expansion
does not introduce a separate logical work-accounting protocol.

Before native DFA construction, the deduplicated input is limited to 512 terms,
256 Unicode code points per term, and 512 code points in total. Oversized input
fails as a parameter-too-large error before any native allocation.

Context cancellation stops orchestration and remote RPCs. Cooperative
cancellation within one native FST/Trie traversal is follow-up work. The initial
release relies on the shared native-expansion concurrency limit to bound the
number of simultaneous traversals.
...
- Reuse existing output-size and read-concurrency controls for expansion.
...
- schema/search validation, exact `fuzziness = 0`, edit distance, transposition,
  Unicode prefix, deterministic limit ordering, multi-analyzer selection, and
  normal/hybrid iterator rejection;
...
- corrupt/missing/stale artifacts, generation retry, bounded Worker retry,
  stale-route invalidation, Worker partial failure, expansion output limits,
  concurrency saturation, and boundary cancellation;
- Global FST publication, coverage fallback, and `global_only` accuracy loss.

Initial troubleshooting uses existing QueryNode request/error metrics and logs
that identify collection, field, segment, generation, load mode, and failure
class without logging terms. Dedicated partial-expansion and Global FST coverage
metrics are follow-up observability work.
