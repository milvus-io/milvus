...
它们保护升级期间的既有流量;它们刻意**不**让 schema DDL 在该窗口安全——一个旧组件无法遵守它不知道的语义。因此运营契约是:升级顺序 **coordinator → querynode → proxy → SDK**,且**集群混合版本期间不做任何 schema-change DDL**。关闭这些逃生舱(或在 cluster-min-version 检查下自动拒绝 schema DDL)是可能的后续硬化,但**契约、而非逃生舱,才是安全边界**。
...
- **[P1] 滚动升级期间做 schema DDL 不安全,且服务端不强制阻止。** 三个升级可用性 escape hatch 均在:旧 QN 无 `ReadySchemaVersion` 记为 ready(`internal/querycoordv2/services.go`)、legacy read `version==0` 跳读闸门(`internal/querynodev2/delegator/delegator.go`)、legacy insert 无 version 跳写闸门(`shard_manager_collection.go`)。§9.2 只写了"mixed-version 禁 DDL"的**契约**,但**无 cluster-min-version/capability gate 在代码里强制**。旧 QN `UpdateSchema` 重试耗尽是 `mlog.Fatal()` **panic/crash-loop**(`delete_node.go`,fail-stop、可观测,非静默继续推 tsafe);新字段静默丢失走另一路径——`internal/querynodev2/pipeline/insert_node.go` 把当前 schema 里没有的字段 skip 掉(带 warning+metric),在旧 QN 尚未 UpdateSchema 且 insert 携带新列时可达。修法:加 cluster-min-version gate,mixed-version 期间拒绝 schema-change DDL。
