...
5. **JSON paths are STRICTLY TYPED.** The hash domain has exactly two kinds — int64
   ...
   - **Type domains stay distinct**, and by construction rather than by luck. The two domains
     hash into one XXH64 space, so byte-identical encodings collide with probability 1 (an
     8-byte string vs. the int64 with the same byte image — see "Value domains" below). What
     keeps them apart is the envelope's `domains` bitmask: a filter built only from int64s is
     never probed with the UTF-8 hash, so JSON int `5` cannot alias the string `"5"`, and no
     JSON string can alias an int64 member, whatever their byte encodings. A blob that
     deliberately declares BOTH domains opts back into one shared space, where a cross-type
     collision is possible and is an ordinary false positive bounded by the configured fpr.
...
`body_len == num_blocks * 32`, `num_blocks` power-of-two per SBBF spec, no unknown `domains`
bit, zero `reserved`, total size within the C++ SBBF 128 MB ceiling. Any mismatch → request
rejected with a parameter error.

### Value domains (`domains`)

The two hash domains share **one** XXH64 output space. An 8-byte string and the int64 with
the same byte image therefore hash identically — e.g. `int64 0x4847464544434241`
(= 5208208757389214273) has the little-endian bytes `41 42 43 44 45 46 47 48`, exactly the
UTF-8 bytes of `"ABCDEFGH"`. Nothing in the SBBF body can tell the two apart.

`domains` records which domains the builder actually inserted, and **every probe is gated on
it**: a value can only match a filter built from its own domain. This is what makes the
one-sided guarantee ("a true member is never missed") hold unconditionally rather than only
when the caller happens to build from the right domain, and it is enforced in one place — the
prober (`SplitBlockBloomFilterView` / `sbbf.Filter`) — so the typed-scalar path, the
index reverse-lookup path and the per-row JSON path all inherit it.

Two consequences:

- **Typed scalar fields** (INT8/16/32/64, VARCHAR) have exactly one domain, so a blob that
  declares a domain the field cannot probe is a **caller error**: the proxy rejects it with a
  parameter error (`ErrParameterInvalid`) instead of running a query that silently returns
  fewer rows. The realistic trigger is IDs stringified by a JSON/JS layer — a `[]string`
  filter aimed at an INT64 field. A blob declaring *both* domains is accepted (the extra
  domain only costs false positives).
- **JSON paths** are typed per row, not per field, so no domain is required and nothing is
  rejected: a domain the blob never recorded simply never matches, which is the correct
  answer. This is also what makes the type-domain separation in Semantics 5 real rather than
  incidental.

`domains == 0` means the filter recorded no value at all (an empty membership set). It is a
legal filter that matches nothing, and it is never rejected.
...
- **`proxy.maxMembershipFilterPlanSize`** (default 128 MiB, refreshable): request-wide
  membership resource budget. Proxy independently checks both the aggregate serialized size
  of every membership-filter-bearing plan and the aggregate estimated decoded bytes against
  this value; the two measurements are not added together. Serialized size is enforced with
  `proto.Size(plan)` before `proto.Marshal`. Every embedded membership blob consumes the
  serialized budget; each MRB1 occurrence additionally consumes the decoded budget. Hybrid
  search accumulates all
  membership-filter-bearing sub-plans against the same request budget, and scorer filters are
  included: a membership expression in a scorer filter is charged like one in the main predicate.
