...
blob independently, parses it once into shared immutable membership state, and
probes raw field data or a bounded index-only reverse-lookup fallback.
...
- The sum of that estimate across every `roaring_match` occurrence in one
  logical request/plan is at most 64 MiB. Repeated references are charged once
  per occurrence even when their immutable decode is shared.
...
| --- | --- | --- |
| `proxy.maxRoaringFilterSize` | 32 MiB | One MRB1 body. The fixed 32-byte header is allowed on top, matching `proxy.maxBloomFilterSize`. |
| `proxy.maxBloomFilterPlanSize` | 128 MiB | The aggregate serialized size of every filter-bearing plan in one Search, HybridSearch, Query, or complex Delete. |
...
| MRB1 aggregate decoded admission | 64 MiB | Sum of the decoded estimate across every `roaring_match` occurrence in one logical request/plan. Proxy enforces it before materialization and across HybridSearch sub-plans; QueryNode enforces it independently per logical plan before decode. |
...
The aggregate budget is deliberately **shared with `bloom_match`** rather than
given its own key. The two expressions consume one resource — a client-built
blob embedded in the serialized plan and fanned out to every QueryNode — so a
request carrying one of each must not be allowed to spend twice what either
alone may. The key keeps its historical bloom-specific name to avoid breaking
deployments that already set it.

Before template materialization, Proxy MUST charge each syntactic
`bloom_match`/`roaring_match` occurrence by its referenced blob length. A
template referenced five times is charged five times, even though its Roaring
structure is validated only once per template name. This lower-bound preflight
must run before structural validation so a request that is already over budget
cannot force repeated work. The existing exact `proto.Size` gate remains after
materialization and catches protobuf overhead and HybridSearch accumulation.
After unique-template structural validation reports the decoded estimate,
Proxy MUST also multiply that estimate by the template's occurrence count and
reject an aggregate over 64 MiB before materialization. Materialized scorer
filters and HybridSearch sub-plans are re-accounted at the request boundary so
all plans in one request share the same decoded budget.
...
value distribution: at 32 MiB the same budget holds roughly 16M members spread
uniformly over the whole `INT32` domain, but tens of millions when the values
are dense or contiguous. The wire byte budget alone could hold about 1.5M
`INT64` members spread uniformly over the full 64-bit range (~22 bytes per
member, the worst case, where each value lands alone in its own 2^32 high
container), but such full-range sparse input reaches the fixed high-container
admission first: approximately 262K
distinct high-32 buckets is the hard ceiling even when the serialized body is
well below 32 MiB. Operators should use `bloom_match` for that distribution and
...
Proxy parser and template materialization
  ...
  5. Charge every AST occurrence against the shared plan budget.
  6. Validate each unique MRB1 template in linear time without decoding and
     charge its decoded estimate once per AST occurrence.
  7. Reject an aggregate decoded estimate above 64 MiB, then materialize
     planpb.RoaringFilterExpr {column_info, bitmap_blob}.
        |
        v
QueryNode / segcore
  8. Scan every MRB1 occurrence and enforce the same aggregate admission before
     any CRoaring allocation.
  9. Decode each distinct blob once into immutable RoaringMembership.
 10. Share that membership across per-segment PhyRoaringFilterExpr objects.
 11. Probe raw values or the bounded index-only reverse-lookup fallback.
```
...
If it has not already landed through the Bloom or foundation work, it must land
before `roaring_match`. This is a transport dependency, not a dependency on
Bloom semantics or MBF1.
...
QueryNode parses each distinct blob once per logical plan, not once per AST
occurrence, segment, batch, or row. Repeated logical occurrences and
per-segment physical expressions share a `const`/immutable membership object.
Scorer definitions are parsed into the C++ plan at admission time; segment
rerank calls pass only the scorer ordinal, so they neither marshal the scorer
protobuf nor decode its membership again per segment.
...
`roaring_match` is a row-level expression and MUST be rejected inside
`element_filter(...)`. Element-level execution supplies global element IDs,
while the physical membership expression consumes row offsets; accepting the
combination could read a different row or go out of bounds. A document-level
`roaring_match(...) && element_filter(...)` conjunction remains valid because
the two expressions are siblings evaluated in their respective domains.
...
parameters, and configured gRPC receive limits also apply. In particular, a
current default Proxy gRPC receive limit of 64 MiB is lower than the MRB1 body
cap and can reject the request before MRB1 validation once request framing is
included. Practical capacity is therefore bounded by transport limits, the
128 MiB body cap, the high-container cap, and the 64 MiB decoded estimate; the
tightest bound is distribution-sensitive.
...
- Repeated uses of one template are validated once by template name, while each
  AST occurrence is charged for both wire bytes and decoded-memory estimate
  before materialization.
- HybridSearch sub-plans share the same 64 MiB decoded budget at Proxy.
- QueryNode scans the complete logical plan first, admits aggregate decoded
  cost, then parses each distinct blob once and shares the immutable decoded
  bitmap across main predicates, scorer filters, logical occurrences, and
  segment expressions. Segment scorer execution references the pre-parsed
  scorer by ordinal instead of remarshal/reparse.
...
- Decoded bitmap memory lives for the query. Sharing avoids duplicate logical
  and per-segment copies, while the fixed 64 MiB aggregate estimate bounds one
  logical request/plan. Concurrent requests can still create tenant or
  QueryNode-wide pressure, so concurrency testing must determine whether a
  lower operational limit or admission controller is required.
...
- Never log, trace, or include raw bitmap bytes or member values in error
  messages.
...
- Unsupported-expression errors MUST be bounded and redacted. An old QueryNode
  ...
  error in Milvus commit `9f6bfcc9b7` protects only binaries that contain that
  change; it does not protect unpatched old QueryNodes.
...
Metrics and logs MUST NOT carry raw member values or blob contents.
...
| New Proxy to patched old QueryNode | The old generated API sees the expression oneof as unset while protobuf preserves unknown field 23; the bounded, redacted parser default rejects the plan without printing the payload. |
| New Proxy to unpatched old QueryNode | The parser still rejects the unset expression, preserving correctness, but the legacy TextFormat error path may expose the preserved MRB1 payload. Routing this combination is prohibited. |
| Mixed old and new QueryNodes | Unsupported workers must have the redacted error backport or be excluded by a Proxy, routing, or version gate. No worker may execute without the predicate. |
...
error path must separately prevent disclosure of the retained payload. Mixed
execution remains correctness-safe but is not availability-safe, and an
unpatched legacy error path is not privacy-safe.
...
1. Backport the bounded, redacted unsupported-expression error to every old
   QueryNode that can receive requests during the upgrade; or
2. Enforce a Proxy, routing, or version gate that prevents field-23 plans from
   reaching every unpatched QueryNode.
...
all field-23 requests before any unpatched old QueryNode becomes eligible for
routing. Proxy and QueryNode can then be rolled back in the normal order.
...
- Reject valid bodies that exceed either decoded-resource admission limit.
...
- Repeat one template across multiple AST leaves and prove occurrence budgeting
  rejects before validation while unique-template validation is cached.
- Use a valid compact bitmap whose per-expression estimate is admitted but
  whose repeated occurrence estimate exceeds 64 MiB; prove Proxy rejects it
  before materialization and HybridSearch sub-plans share the same budget.
...
- Verify one parse per logical plan, shared membership across physical segment
  expressions, one decode per distinct blob across repeated logical
  occurrences and scorer filters, and no per-segment scorer marshal, blob copy,
  or reparse.
- Prove the complete plan is scanned before decode and that individually valid
  occurrences whose aggregate estimate exceeds 64 MiB are rejected.
...
- Test old Proxy/new client, old QueryNode/new Proxy, and mixed QueryNode cases;
  ...
  success.
- Test both privacy-safe upgrade strategies: the redacted-error backport and a
  Proxy/routing/version gate that prevents dispatch to unpatched old workers.
- For every mixed-version rejection, assert returned errors and component logs
  contain no `MRB1` magic, serialized or escaped blob bytes, or member values.
- Assert Proxy plan-log redaction covers pure Roaring, mixed Bloom plus Roaring,
  and scorer-filter predicates without mutating the original plan.
...
- Exercise concurrent large blobs to establish a QueryNode/tenant-wide
  operational admission policy beyond the fixed per-request/plan 64 MiB
  aggregate budget before broad enablement.
...
2. Land the additive proto field 23 together with Proxy and QueryNode support,
   ...
   client use opt-in.
3. Before mixed-version routing, either backport that redacted error to every
   old QueryNode eligible during the rollout or deploy a Proxy/routing/version
   gate that prevents field-23 plans from reaching unpatched nodes.
...
7. Monitor blob sizes, validation failures and latency, decoded memory,
   index-only fallback use, slow-index rejection, and query p99. Tighten the
   recommended QueryNode/tenant-wide operational admission if concurrency
   results require it, without changing the fixed per-request/plan budget or
   the MRB1 version 1 format limit.
