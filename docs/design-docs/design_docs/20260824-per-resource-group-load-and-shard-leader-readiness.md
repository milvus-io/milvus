...
- `ChannelWorkload.ResourceGroup` / `CollectionWorkLoad.ResourceGroup` is the scope seam. `LBPolicyImpl.selectNode` applies the filter to the candidates of **one channel**, before its exclusion logic, so "every candidate excluded" is judged on the scoped set. Nothing on the proxy's request path sets the field yet; with it empty, routing is byte-for-byte what it was.
...
- The two `utils` surfaces now refuse an unknown group name with `ErrResourceGroupNotFound`; they are in-process only and have no caller before this PR.
