...
- The two `utils` surfaces refuse an unknown group name with `ErrResourceGroupNotFound`. The per-group figure is reachable on the wire through `ShowLoadCollectionsRequest.resource_group`: set, `inMemory_percentages` is the progress of the replicas in that group, `-1` for a collection with no replica there; empty keeps the collection-wide figure, so `Proxy.GetLoadingProgress` and every existing caller are unchanged.
