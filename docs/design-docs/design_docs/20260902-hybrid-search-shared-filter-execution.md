...
predicate and hashing it, repeated on every shard the request touches. Measured on a 6.4 KB plan of the
shape this design targets (`BenchmarkSharedFilterKeyOf`):
...
175 µs per sub-request per shard is not a rounding error against a query budget; for a two-branch search
over eight shards it is several milliseconds of pure bookkeeping.

The proxy already parses each sub-request's `(Dsl, expr_template_values)` into a plan
(`task_search.go`, the `tryGeneratePlan` loop), so it can recognise equality for the cost of a string
compare and publish the answer as `SubSearchRequest.filter_sharing_group`: equal non-zero values mean
identical predicates, and 0 means the sub-request cannot share a filter at all — no predicate, or an
iterative filter, which applies the predicate after the vector search and leaves no prefix subtree.
...
conservative — semantically equivalent filters spelled differently will not group — and it is exact,
with no digest collisions to handle. Singletons are numbered too rather than folded into 0, so the query
node can still separate "nothing to share" from "nothing matched" in its fallback metric.
...
1. Check the O(1) invariants: bitset_result->segment_id == this segment, and
   the four snapshot fields the bitset is a function of -- active_count,
   timestamp, collection_ttl, entity_ttl_physical_time_us -- all equal this
   branch's. On a mismatch, skip steps 2-3 and evaluate this branch's own
   filter instead.
...
The snapshot checks should never fire: both phases pass the same MVCC timestamp and TTL context, and
`get_active_count` on growing is an `upper_bound` over insert timestamps, so rows inserted between the
phases carry larger timestamps and fall outside. A mismatch **degrades rather than throws**: the branch
evaluates its own filter, costing the sharing but never the query.
...
- *Structural (P1).* Make the prefix output self-describing: carry these fields next to the bitset in the
  ...
  own.
- *A test that fails instead of a comment that is read.* `QueryContext` has 12 setters, of which the 5
  filter-subtree ones are captured. Recording which setters fired during phase 1 and asserting that set
  is a subset of {captured} ∪ {an explicit allowlist of the 7 that are not filter state} would force a
  new setter to be classified rather than forgotten. Not implemented; the current tests enumerate the
  known fields by hand and share the weakness described above.
...
2. **Identical bitset by construction.** Byte-identical predicate + identical segment + identical MVCC
   ...
   Segcore additionally compares those snapshot fields in phase 2 (step 1 above). That comparison is
   redundant for the hybrid path — the proto shape already makes a mismatch unrepresentable — but
   `SearchWithBitset` is a segcore-level entry point, and a future caller outside this path gets no
   protection from the proto. Note that `active_count` alone would not be enough to detect a snapshot
   mismatch: on sealed segments `get_active_count` returns `get_row_count()` and ignores the timestamp
   entirely (`ChunkedSegmentSealedImpl.cpp:6500`), so a branch reading a different timestamp's
   delete/TTL mask would pass an active-count-only check.
...
branch does, since the shared stage-1 pass makes an extra branch free.
...
None of them can fire on the hybrid path: the snapshot fields are
unrepresentable per-branch in the proto, and the predicate is compared by the
delegator. They are kept for two reasons. `active_count` guards something
memory-unsafe rather than merely wrong — the scan is bounded by the branch's
own active count while the bitset is viewed at its own size, so a larger count
would read past the end of the buffer. The timestamp and TTL fields cost three
integer compares and make `SearchWithBitset` safe for a caller outside this
path, where the proto guarantee does not apply; without them `active_count`
alone proves nothing on sealed segments, whose `get_active_count` ignores the
timestamp. On a mismatch the branch evaluates its own filter instead, costing
the sharing rather than failing the search.
