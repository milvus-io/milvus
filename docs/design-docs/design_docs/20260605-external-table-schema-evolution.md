...
segments. Added source-backed fields may be readable earlier when the loaded
external manifest can project the referenced source column, but refresh is
still required to commit durable coverage metadata used by index and target
readiness checks. Exact read-path behavior for schema-visible but uncovered
fields is handled separately by
...
3. **Metadata first, materialization later**: schema mutation succeeds after
   ...
   generated function outputs and for DataCoord/QueryCoord coverage metadata.
...
|-----------|-----|---------------|------------------------|-----------------------|
| Add source-backed field | `AlterCollectionSchema(AddRequest)` | Adds a nullable source-backed field with `external_field`. | Refresh patches or rebuilds external manifests so active segments carry explicit coverage for the new source column. | Existing queries on old fields continue. The new field may be readable before refresh if the loaded manifest can project the external source column; refresh is still required for durable coverage and index readiness. |
| Add function | `AlterCollectionSchema(AddRequest)` | Adds exactly one built-in function and all new output fields. | Refresh computes generated output columns and writes them into external manifests. | Existing queries continue. Output fields are expected to become usable after refresh and target reload; pre-refresh read error normalization is tracked by [#50416](https://github.com/milvus-io/milvus/issues/50416). |
...
The shared coverage resolver is used by DataCoord index validation and
QueryCoord index/reopen scheduling. Read-path validation is intentionally left
to [#50416](https://github.com/milvus-io/milvus/issues/50416); this PR should
not add another independent materialization gate in Proxy or QueryNode.
...
6. Source-backed fields may be readable before refresh when the loaded external
   manifest can project the source column. This is an implementation detail,
   not a durable materialization guarantee.
...
| Query output field | May succeed for source-backed columns that the loaded manifest can project; otherwise uses existing read-path errors until [#50416](https://github.com/milvus-io/milvus/issues/50416). | Fails normal field-not-found validation. |
| Filter expression | Same as query output fields; final error normalization is tracked by [#50416](https://github.com/milvus-io/milvus/issues/50416). | Fails normal field-not-found validation. |
...
5. The new field is schema-visible. Existing loaded fields remain queryable;
   the new source-backed field may also be readable immediately if the loaded
   external manifest can project its `external_field`.
...
7. Existing fields remain queryable. Added source-backed fields may be readable
   immediately if the loaded manifest can project the source column. Generated
   function outputs become reliably queryable after refresh and loaded-target
   switch finish. Dropped fields become invalid for new plans after schema
   propagation.
...
5. Use field-level serving materialization state for index/reopen scheduling.
   Read-path rejection for uncovered fields is tracked by
   [#50416](https://github.com/milvus-io/milvus/issues/50416).
...
| E2E | Load external collection, add source-backed field, query existing fields succeeds, query the added field when it is immediately projectable, refresh, wait target ready, query the added field succeeds. |
