...
7. **No field ID reuse**: every drop keeps `max_field_id` pinned to the
   historical high-water mark. For `milvus-table`, target-only IDs are also
   persisted in `milvus_table_target_only_field_ids` and remain reserved after
   drop, because source and generated manifest columns share the numeric field
   ID namespace.
...
For `milvus-table`, source-backed fields reuse the source snapshot field ID so
numeric manifest columns remain addressable. Virtual primary keys and target
function outputs receive target-only IDs above the current source/target high
water marks. Those target-only IDs are recorded permanently; a later source
snapshot using any reserved ID is rejected during refresh and schema alter,
including after the target-only field has been dropped.
...
For `milvus-table` identity checks, a non-nullable source field may therefore
map to a nullable target field. The reverse nullable-to-non-nullable narrowing
remains invalid; all other field attributes must still match.
...
2. Proxy validates:
   ...
   - the `external_field` value does not collide with generated numeric field-ID
     column names.
3. RootCoord assigns the field ID, updates `max_field_id`, increments schema
   version, and broadcasts the schema update. `milvus-table` source-backed
   fields reuse the source field ID after verifying that it is not in the
   durable target-only reservation; other formats allocate a new target ID.
...
External schema and materialization state advance together only within the
primary cluster. Replicating the schema mutation without its refresh and
manifest updates would leave the secondary with a new schema backed by stale
external segment metadata. To avoid that half-replicated state, the following
WAL messages are marked unreplicable and are skipped by CDC:

1. External-collection schema mutations emitted by `AlterCollectionSchema`,
   including add and drop operations.
2. The legacy `AddCollectionField` compatibility path when it mutates an
   external collection.
3. `RefreshExternalCollection` messages.
4. `BatchUpdateManifest` messages that publish refreshed external manifests.

The secondary therefore retains its previous schema and materialization state.
These external schema changes are not preserved by CDC-based failover and must
be executed again on the promoted cluster when needed. Internal collection
schema evolution remains replicable. External collection property-only alters
also keep their existing replication behavior because they do not use the
schema-mutation broadcast paths above.
...
2. **RootCoord schema builder**
   ...
   - Persist `milvus-table` target-only field IDs and reject source snapshot
     fields that reuse them.
...
| RootCoord unit | Add source-backed field assigns the format-appropriate field ID and updates `max_field_id`. |
| RootCoord/DataCoord unit | `milvus-table` source fields cannot reuse a persisted target-only field ID after drop. |
