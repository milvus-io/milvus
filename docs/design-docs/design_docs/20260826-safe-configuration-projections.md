...
> **2.6 backport scope:** This design originated on master in #52579. The 2.6
> backport uses `pkg/v2` and the existing `pkg/log` API. Master-only snapshot,
> Woodpecker quorum, and additional S3 transport settings are not introduced.
> Management authentication (#52580), including `adminAuthEnabled` and its
> write protection, is a separate change and is not included here. Existing
> 2.6 defaults and write contracts are retained; storage-normalized `mq.type`
> spellings are guarded consistently. Native plugin failures preserve 2.6's
> existing typed-code / untyped-`UnexpectedError` classification.
