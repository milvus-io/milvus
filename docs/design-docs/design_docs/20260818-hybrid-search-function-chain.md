...
- Let the public Function Chain own final ordering and score transformation. An explicit L2 Limit owns pagination; otherwise append the Hybrid request limit and offset.
...
`rank_params` as a whole is not mutually exclusive with `function_chains`. Existing clients and REST conversion may still send `limit`, `offset`, and `round_decimal`. The public Hybrid L2 Function Chain falls back to the request `limit` and `offset` only when it contains no Limit operator. Request `round_decimal` remains unapplied; users express score rounding explicitly in the chain. Grouping keys remain valid ANN recall controls with the final-result limitation described below. Only meaningful legacy `strategy` and `params` values select a conflicting rerank source.
...
HybridSearchRequest
  ...
  -> QueryNode executes each sub-search independently
       prepare and validate PlanNode[i].querynode_function_chains
       -> ANN
       -> L0 per segment
...
QueryNode prepares and validates their static semantics before ANN search,
including L0/L1 function construction, registered argument validators, stage
compatibility, and required schema fields. Errors from these checks are returned
before `SearchHistorical` or `SearchStreaming`, regardless of whether local
segments or matching rows exist.
When `len(results) == 0`, QueryNode still returns an encoded empty result without
executing the prepared chains. Model execution and data-dependent checks remain
in the execution phase. Internal filter-only requests collect statistics for
two-stage search and skip chain preparation; the subsequent ANN request validates
the chain before searching.
...
Public Hybrid L2 `function_chains` execute in their declared order. If the L2 chain contains a Limit operator anywhere, Proxy does not append a global limit. Otherwise, Proxy appends a Limit using the Hybrid request `limit` and `offset` after all user operators, applying it independently to each query. This includes limit values supplied by SDK defaults. Nested L0/L1 Limit operators do not suppress this fallback. Ordinary Search L2 behavior is unchanged. Proxy does not append an implicit Sort or RoundDecimal operation.

Hybrid `rank_params.limit`, `offset`, and `round_decimal` continue to be parsed and are not treated as selection of the legacy ranker. The `limit` and `offset` values are used together as the fallback described above; when the L2 chain has an explicit Limit, neither request value alters public-chain output. Request-level `round_decimal` remains unapplied; score rounding is expressed with a `map` operator using the `round_decimal` expression.
...
6. Execute the public chain and append the request limit with its offset only when the L2 chain contains no Limit operator. Do not append a server-owned Sort or RoundDecimal tail.
...
7. Prepare and validate QueryNode L0/L1 chains before ANN, including static
   map-function checks for both stages. Reuse the prepared state for execution.
...
5. Keep `limit`, `offset`, and `round_decimal` accepted for request compatibility. Document that `limit` and `offset`, including SDK defaults, apply only when the Hybrid L2 chain contains no Limit, while request `round_decimal` remains unapplied. Keep output fields, consistency, and partition controls available.
...
- Request-level limit and offset apply after the full Hybrid L2 chain only when it contains no Limit, including when nested L1 has a Limit. Cover per-query pagination, fewer candidates than the limit, an offset beyond available candidates, and a downstream sort before the fallback.
- An explicit Limit anywhere in the L2 chain overrides the request limit and offset. Request score rounding remains unapplied.
...
- Nested chains are statically validated before ANN, including requests with no
  local segments or matching rows. Valid empty sub-search results retain the
  existing shortcut without executing the chains.
...
- L1 Sort and Limit change only that sub-search's candidate contribution before
  fusion; final pagination follows the top-level L2 Limit or the Hybrid request limit/offset fallback.
...
- Nested chain static semantic validation remains at QueryNode and runs before
  ANN search, independently of candidate availability.
