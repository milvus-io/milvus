...
class FieldChunkMetrics {
 public:
    ...
    virtual bool CanSkipPreparedIn(const PreparedInQuery& query) const;
...
values. For `IN`, prefetch and scan reuse one `PreparedInQuery` per expression,
containing the converted values and their precomputed inclusive min/max range.
Both are initialized together with `std::call_once`, after type conversion and
integer-overflow filtering. Range-only integer, floating-point and string metrics
reuse these bounds through `CanSkipBinaryRange`; metrics with Bloom filters keep
the full-list `CanSkipIn` predicate. Other providers retain that predicate by default.
String entries borrow immutable query literals retained by the expression, so per-cell
checks do not copy string contents, allocate temporary value vectors, or recompute
the query range. Empty or incompatible value lists retain the existing fallback,
and an empty string remains a valid bound. Footer string bounds remain owning.
