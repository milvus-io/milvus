...
- A Flat index mounted at `/a` covers `/a` and `/a/b`, but never `/ab` or
  ...
  including `EXISTS` with offset input, strip the mount prefix before querying
  the indexed sub-document. An exact mount match uses the empty relative path;
  Flat's existing empty-container limitations still apply.
...
- QueryNodes and DataNodes advertise default scalar version V5 and maximum
  readable/writable version V6. `dataCoord.targetScalarIndexVersion=-1` keeps
  the default build version until every online session, including standby
  coordinators, reaches **3.0.2** for one minute. The version gate persists
  target V6; all existing build-request paths use that resolved target.
...
JSON unary comparisons, `IN`/`NOT IN`, `EXISTS`, `CONTAINS`, `CONTAINS_ANY`,
and `CONTAINS_ALL` keep the selected index when evaluating candidate row
offsets. They cache the same segment-wide result
and validity bitmaps as ordinary execution, then gather both by the requested
row IDs. Offset input must not switch a typed Path projection back to raw
comparison. `JsonIndexTest.TestJsonContains` covers repeated offset batches,
empty input, unordered/duplicate rows, negation, and the large-int64 alias pair,
both with raw JSON loaded and with only the Path index available.
`JsonNumericCastTest` checks all six unary comparisons, membership and negation
through DOUBLE/INT64 projections on INVERTED/STL_SORT. `JsonIndexTestFixture`
also covers BOOL and VARCHAR membership, including the BOOL specialization.
`JsonIndexExistsTest` checks both a named path and the root path without raw
JSON; EXISTS gathers the persisted presence bitmap, not typed-cast validity.
The offset tests reuse one expression across batches and verify that gathering
does not advance its sequential cursor. Short-circuiting a composite expression
with offsets must also leave unevaluated children's sequential cursors unchanged;
`JsonNumericCastTest` covers this with split numeric `IN` and its negation.
`JsonFlatIndexExprTest.NestedExistsUsesRelativePathWithOffsets` checks nested
Flat `EXISTS` and negation against raw results, with and without raw data loaded.
...
### 7. Historical V3 stats retain pre-migration differences
...
The historical numeric writer could also persist an integer such as
`18446744073709551616` as DOUBLE via `std::stod`, while the current raw
`get_number()` parser rejects that integer token. An old artifact may therefore
...
Tests that build V3 with the current writer are not historical-writer fixtures
and do not establish numeric parity for those persisted artifacts.
...
call, including with offsets, while `CONTAINS_ALL` keeps per-candidate queries.
...
sessions, and persists format 4 in the config center. An old standby therefore
blocks publication even after all QueryNodes and DataNodes have upgraded.
The persisted switch survives coordinator failover; no manual format switch
...
while a force flag with an explicit target uses `!=`. Fake-finished indexes
