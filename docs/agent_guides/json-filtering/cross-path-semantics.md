...
| Path index | User-selected path and fixed `json_cast_type` | One configured typed projection | Answers within the configured projection, which is `DOUBLE` by default or `INT64` when an integer cast is configured |
...
V3 stats remain readable and queryable until their segments migrate to V4.
Their persisted numeric values and typed-column empty-string null sentinel
retain historical representation and can differ from current raw semantics.
These migration-window differences are accepted; loading V3 does not repair or
reinterpret old artifacts. Rebuilding from source JSON as V4 applies the current
semantics. See case 7 below.
...
Historical V3 artifacts retain the explicitly accepted migration-window
exceptions in case 7; this contract does not claim they already have V4 parity.
...
The planner splits mixed JSON `IN` lists by concrete literal type and combines
the predicates with `OR`; `NOT IN` negates that combined membership. Each
`TermExpr` is homogeneous, and segcore rejects an unnormalized mixed list.
Integer and floating-point branches select their execution paths independently:
an `INT64` Path index can serve the integer branch, while the floating-point
branch uses a compatible index, stats, or raw data. Both branches retain exact
cross-type numeric comparison on Raw, Stats, and Flat.
...
- A Flat index mounted at `/a` covers `/a` and `/a/b`, but never `/ab` or
  `/a~1b`. Index selection matches complete JSON Pointer components. Executors
  strip the mount prefix before querying the indexed sub-document. An exact
  mount match uses the empty relative path; Flat's existing empty-container
...
Path lookup in raw JSON and typed Path indexes matches the original spelling
of object keys, not their JSON-unescaped form. Stats must preserve that spelling
when sampling paths and building both typed and shared columns. For example,
`{"\u006e":7}` has no match at `/n`, and `{"n":8,"\u006e":7}` must retain two
distinct paths. JSON Pointer escaping (`~0` and `~1`) still applies to each path
component. String **values** are decoded normally; this rule only concerns keys.

`JsonCrossPathContractTest.RawStatsPathAndFlatAgreeUnlessDocumented` checks
equality, negation, and `EXISTS` against raw results and validity for V3/V4 stats,
including shared-only stats, with raw field data removed from stats segments.
...
- A typed `INT64` path index compares as exact signed integers. Build-time
  strict cast leaves unsigned values above `INT64_MAX` and non-integral
  numbers null rather than truncating; see the documented difference below.
  A literal empty `IN []` has the unconditional false/all-valid contract.
- Milvus's current `uint64 > INT64_MAX` behavior uses the legacy
  ...
  Path index; it is not an exact uint64 model. An `INT64` Path index does not
  share that contract: the value is null in its projection, so the predicate
  is `UNKNOWN` there.
...
- QueryNodes and DataNodes advertise default scalar version V5 and maximum
  readable/writable version V6. `dataCoord.targetScalarIndexVersion=auto` keeps
  the default build version until every online session, including standby
  coordinators, reaches **3.0.2** for one minute. It then resolves to this
  release's latest supported version, V6; all existing build-request paths use
  that resolved target. The configured value remains `auto`, just like leaving
  the option unset. Only gate readiness is persisted, separately from the
  configured value and identified by the config key, gate version, and target.
  Later releases can therefore advance the automatic target without confusing
  it with a manually configured number. Readiness is recorded even while a
  manual value is set, so returning to `auto` uses the confirmed capability.
  An explicit `-1` follows the engine's current version without participating
  in this automatic switch or setting a forced rebuild target.
  DataCoord starts gradual one-input compaction only after this switch and all
  ...
  Bound workers skip the DN capability checks; their deployment owns writer
  compatibility, while QueryNode reader checks and the version gates remain.
...
- Automatic migration skips a segment if an active artifact is newer than its
  rebuild target or an active index build is unfinished. An explicit scalar or
  vector force-rebuild setting permits a supported downgrade only for that
  index family; it does not permit a JSON stats format downgrade.
...
use reader capabilities, not the scalar build-version override. Introducing
V4 stats also requires the JSON stats format gate to be open: copied files can
otherwise trigger an old standby coordinator's format-directory GC.
...
Iterative-filter offset evaluation retains the legacy raw-JSON path for JSON
unary and binary comparisons, `IN`/`NOT IN`, `EXISTS`, `CONTAINS`,
`CONTAINS_ANY`, and `CONTAINS_ALL`. Do not infer cross-mode parity from the
ordinary Path-index contract above.

TODO: remove iterative filtering and its native offset-input expression path
instead of expanding JSON index execution into that mode.

`JsonFlatIndexExprTest.NestedExistsUsesRelativePath` checks that ordinary
nested Flat `EXISTS` and negation strip the mount prefix before querying the
indexed sub-document.
`JsonFlatIndexExprTest.IndexSelectionRequiresPathComponentBoundary` covers root,
empty-key and escaped-component mounts, and rejects sibling prefix collisions.

When Reopen or an index replacement publishes a new sealed-segment state, it
invalidates that segment's expression-result cache inside the publication
barrier, after existing readers have drained and before new readers can enter.
Segment ID, expression signature, and row count alone do not distinguish an
old typed projection from its replacement. Cancelled publication preserves the
old state and cache; other segments' cache entries are unaffected.
`SealedSegmentReopenExprCacheTest` covers both cache backends, reader drain,
cancellation, and DOUBLE-to-INT64 index replacement.
...
| `uint64 > INT64_MAX` numeric lookup | Direct with legacy uint64-to-double comparison | Direct | Direct | `DOUBLE` cast direct in double; `INT64` cast stores null and answers `UNKNOWN` | Direct | Expected Path difference for the `INT64` cast |
...
| Historical V3 oversized integer stored as DOUBLE | Current numeric parser may reject the source | Retains persisted DOUBLE | Retains persisted DOUBLE | Not applicable | Not applicable | Expected V3 difference until rebuilt as V4 |
...
during the pre-V6-to-V6 Path migration, case 7 covers accepted historical V3
differences that V4 migration closes, and case 8 records the
...
### 7. V3 reads and writes retain pre-migration numeric behavior
...
The V3 numeric writer continues to persist an integer such as
`18446744073709551616` as DOUBLE via `std::stod`, while the current raw
`get_number()` parser rejects that integer token. A V3 artifact may therefore
match `n == 1.8446744073709552e19` and report `EXISTS(n)=true`, while current raw
execution reports UNKNOWN and absent respectively. This difference is accepted
until the segment is rebuilt as V4, where the rejected number becomes null.
The writer chooses the numeric policy before parsing, during both path sampling
and value materialization. V3 uses the legacy exact-int64 probe followed by
`std::stod` for scalar fields, and the legacy simdjson DOM parser for arrays.
It does not send oversized scalar integers through V4's `get_number()` first.
Values rejected by the legacy converter (such as scalar `1e400`, and subnormal
or underflowing scalar tokens rejected by `std::stod`) still fail V3 builds,
with `DataFormatBroken` rather than a retryable error. Array numeric acceptance
continues to follow the DOM parser, including its support for subnormals.
V4 keeps strict numeric parsing and null normalization. Consequently the
subnormal scalar fix takes effect when stats are built as V4.

`JsonKeyStatsUploadLoadTest.NumericWritesFollowTheirFormatAfterReload` checks
V3 DOUBLE versus V4 null after upload and fresh-instance load, for both typed
Parquet and shared BSON. These are current-writer round trips, not historical
artifact fixtures.

### 8. A numeric Path index is a typed projection

The numeric cast types are `DOUBLE`/`ARRAY_DOUBLE` and `INT64`. A `DOUBLE` Path index stores
...
`PhyJsonContainsFilterExpr::Eval`). An `INT64` Path index stores the value only
when it is an integral, in-range number; out-of-range and non-integral values
are null (build-time strict cast).
...
Validity is unaffected by a `DOUBLE` projection: every number reaches some
double, so that projection changes which rows match, never which rows are
`UNKNOWN`. An `INT64` projection also changes validity. Its build-time strict
cast stores null for non-integral numbers and for integers outside the signed
64-bit range, and those rows answer `UNKNOWN` where raw answers a known result:

| Data | Query | Raw / stats | `INT64` Path index |
|---|---|---|---|
| `{"n": 2.5}` | `n < 100` | true | **UNKNOWN** |
| `{"n": 18446744073709551615}` | `n > 100` | true | **UNKNOWN** |

The path itself stays present, so `EXISTS` is true in both. This is the
validity half of the same user-configured projection, and it survives `NOT`.
`JsonNumericCastTest.IntegerAndDoubleSourcesPreserveProjectionValidity`
asserts both bitmaps, and the negated form, for fractional and out-of-range
sources.
...
sources through DOUBLE and INT64 projections, using INVERTED and
...
`CONTAINS_ALL` never drops an impossible out-of-range requirement. It also checks
that `CONTAINS_ANY` submits all distinct representable candidates in one `In()`
call, while `CONTAINS_ALL` keeps per-candidate queries.
...
- [Migration compaction checker](../../../internal/datacoord/compaction_checker_migration.go)
...
## Default JSON stats format and rolling upgrades

`dataCoord.jsonStatsFormatVersion` defaults to `auto`, effectively V3 until
every online session reaches **3.0.2** for one minute. The common version-gate
confirmator scans the entire session prefix, including `mixcoord-<id>` standby
sessions, and persists gate readiness in the config center. Once ready, `auto`
resolves to this release's latest supported format, V4, while the configured
value stays `auto`. An old standby therefore
blocks publication even after all QueryNodes and DataNodes have upgraded.
The persisted readiness survives coordinator failover; no manual format switch
or per-segment rebuild is needed. Explicit 3 pins V3; explicit 4 bypasses the
version gate and must not be forced during a rolling upgrade.

New tasks persist the effective target and their requested JSON field IDs.
A V4 task, including one recovered from metadata, waits for this gate and every
online QueryNode to advertise the V6 reader capability. In normal discovery
mode, it runs only on an individual DataNode advertising the corresponding
writer capability; unknown capabilities fail closed. The automatic binary
version gate waits for all online roles, whereas this additional writer check
only concerns the selected worker (also when format 4 is explicitly forced).

Disabling `common.enabledJSONShredding` (or the deprecated
`dataCoord.jsonShreddingTriggerCount=0` kill switch) pauses queued/retrying JSON
stats tasks, including recovered tasks, without rejecting their writers. Enabling
it again releases those same tasks. Request preparation snapshots enablement:
disabled JSON attempts wait locally, while admitted requests always enable
shredding and may finish under the same strict output contract after a toggle.
New JSON stats submissions also check both switches at admission, including
direct `SubmitStatsTask` callers and trigger loops. The pending limit is applied
separately to each stats subjob: paused JSON tasks (including V4 tasks waiting
under a format-3 override) do not consume TextIndex admission capacity. Retry
backoff still counts against the same subjob's limit. Existing JSON tasks stay
queued for resumption, and admitted in-flight attempts may complete normally.
Sort/text jobs and local completion/cleanup of empty/dropped segments are unaffected.
`Test_statsInspectorSuite` covers disabled admission, switch changes during
discovery, pause/resume, and TextIndex submission with a full JSON backlog using
the real scheduler queue.

With `indexCoord.bindIndexNodeMode.enable=true`, the configured bound worker is
assumed to support V4 without a session capability. Its deployment owns writer
compatibility; QueryNode reader checks and output validation remain mandatory.
Invalid output still rejects the bound worker, whose fixed node ID remains
rejected until the coordinator restarts. Whole-segment migration also trusts
bound writers without advertised scalar/vector ranges, while retaining the
version gates, QueryNode reader checks and existing-artifact version protection.

Published V3 stats remain readable and keep serving until their segment is
replaced. Running V3 tasks retain their original format and can finish normally.
Once the format gate, V4 readers, and the selected writer are ready, an idle/retrying
background V3 task is retired instead of retrying the old parser forever. DataCoord first
drops any previous worker attempt, then removes only the task record. Cleanup
failure leaves it retryable. The next inspector tick creates a fresh V4 task ID
for missing stats; segments with existing V3 stats are left to gradual
Compaction V2 migration. No existing artifact is relabeled or deleted by task
retirement. Legacy records without a format still mean V3, not the new default.

DataCoord rechecks the format and reader gates before publishing results and
rejects V3, missing-field, wrong-build, or empty-file results for a nonempty V4 task. An
incompatible result leaves the task retryable and excludes that worker session
from further V4 builds. Empty segments need no stats artifacts.

Before retrying invalid output for an external Storage V3 segment, DataCoord
cleans only files whose task, attempt, requested field and segment base match.
Current Stats references and snapshot-protected segments are preserved; files
with unverified ownership are left untouched. Cleanup holds the segment manifest
lock, and a deletion failure retains the completed result for another attempt.
Remote keys retain the exact manifest-base and file-name spelling during
ownership checks, reference protection, and deletion, including repeated
separators and dot components. Only local filesystem paths are normalized
before checking that they remain inside the field's stats directory.

Migration respects explicit operator settings: disabling compaction or JSON
shredding, setting the migration quota to zero, or pinning stats format 3 does
not get overridden. Snapshot-protected segments wait for protection to end.
External collection segments are not compacted and retain V3 until refreshed
or replaced. A replacement that is not yet sorted can serve through raw scans
until it becomes eligible for stats construction.

`MigrationCompactionChecker` also owns `autoUpgradeSegmentIndex` and both
scalar/vector force-rebuild triggers. V1 no longer selects segments because of
index versions. The checker compares each segment's active, finished indexes
with the resolved scalar/vector build targets: automatic upgrades use `<`,
while a force flag with a resolved target uses `!=`, including an activated
`auto` target. Explicit `-1` supplies no force target. Fake-finished indexes
without files do not trigger a rewrite. Collections without JSON participate
in these generic rebuilds, which retain the collection/global auto-compaction
switch. JSON format migrations keep their independent version gates and remain
eligible when generic index upgrades are disabled.

All reasons share the existing migration quota, artifact safety checks and
one-input compaction submission: multiple outdated indexes and stats on one
segment produce a single rewrite. Explicit scalar force rebuilds also allow
text indexes rebuilt inline to align to the scalar target; vector force rebuilds
do not relax scalar/text protection. Generic rebuilds wait if the migration
writer ceiling differs from the versions the build-request paths resolve,
preventing repeated rewrites toward a version those paths never request.
Bound workers use the resolved build targets without DN capability checks.
`compaction_checker_migration_index_test.go` covers generic triggers,
force downgrade scope, target clamping and convergence, no-JSON collections,
combined reasons, switch/quota guards, and dispatch through the registered V2
policy to the compaction inspector.

Migration discovery stops after finding as many eligible segments as the
remaining quota permits. Index safety and rebuild decisions share a snapshot
of scalar metadata fields, including whether artifact files exist; discovery
does not copy index file-key lists. If fewer candidates remain, including after
migration converges, discovery still scans the collection to find them. The
quota bounds selected candidates, not the number of nonmatching segments read.

The version gate checks binary release versions, not only artifact capability
numbers. Do not bump the advertised binary version just to open the gate;
pre-3.0.2 builds (including 3.0.2 prereleases) intentionally keep it closed.
Embedded-etcd standalone resolves locally once its sole binary reaches 3.0.2.
This is a one-shot switch following the monotonic rolling-upgrade contract:
once V4 artifacts are published, incompatible readers and coordinators must
not rejoin or be used for rollback. Switching the configured write format back
to 3 does not convert existing V4 artifacts. This change neither enables nor
replaces Storage V3 stats attempt-directory isolation.
