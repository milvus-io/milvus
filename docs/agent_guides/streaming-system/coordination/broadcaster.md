...
A broadcast message carrying the `_ik` idempotency key property is additionally indexed by that key. A later broadcast presenting the same key short-circuits: it creates no task and returns the ORIGINAL broadcast's message in `BroadcastAppendResult.Duplicated`. `AppendResults` is still filled in, rebuilt from the original's persisted per-vchannel checkpoints, so a caller that only reads append results cannot tell a duplicate from a fresh broadcast; it is nil only when the original has not finished being acked. The lookup happens inside `broadcasterWithRK.Broadcast`, which only exists after the resource keys are held — so two concurrent same-key requests are serialized by the resource lock rather than both missing. **This serialization holds only for exclusive resource keys.** Under a shared key both requests take a read lock, both can miss, and each creates a broadcast task; the index keeps the first `broadcastID` while the second has already reached the WAL and triggered its callbacks. The only keyed caller today (import) holds `ExclusiveCollectionName`, so this constrains a future shared-key adopter rather than anything shipping now.

**The scope is built from names, not identities.** It is `(messageType, sorted resource keys, clientKey)`, and the resource keys carry a dbName and a collectionName resolved at broadcast time -- never a collectionID. That leaves two mirror-image holes, and only one of them is closable by the caller. Drop `db1.c1` and recreate a same-named `db1.c1`: the scope is unchanged, so a reused key resolves to the dropped incarnation's task -- a false HIT, which the caller can catch because it does get the duplicate handed to it (import compares the decoded `collectionID` and rejects the mismatch). Rename `db1.c1` to `db1.c2` instead, which `RenameCollection` does while keeping the collectionID, and the scope changes: the lookup MISSES, a fresh task is created on the normal path, and **no caller check ever runs, because a MISS is not routed through the duplicate branch at all**. So a rename inside the window silently ends it early -- for import, the same key imports the same files a second time, with no rejection or warning anywhere. A caller adopting `.WithIdempotencyKey(...)` must treat a rename of any resource it names as ending the window.
