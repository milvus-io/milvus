...
**Persisting the first group before appending the rest is load-bearing beyond this cluster.** A secondary's replicate streams carry no order between PChannels and reproduce it by holding every non-append-first replica until the append-first ones have been acked *there*; the liveness argument for that gate assumes every append-first replica's TimeTick is strictly below every other replica's, which is exactly what this two-phase `Execute` guarantees. Appending the rest concurrently with the first group would leave every test in this package green and wedge a secondary's replication. See [Replication](../replication/replicate.md).

## `WaitVChannelsAcked`

`WaitVChannelsAcked(broadcastID, vchannels)` blocks until every named VChannel of that broadcast has a recorded **ack in this cluster**, or the caller's context ends. "Acked" is either half of what a task can carry: the acked-VChannel bitmap bit (what tasks persisted before 2.6.1 have) or a non-zero acked checkpoint (what the ack path writes today) — reading only the checkpoint would call an older task's VChannel unacked forever, which for the append gate means a wait that never ends. It is the server half of the secondary's append gate, and the wait covers the broadcast task's *creation* as well as its acks — a secondary learns of a broadcast only from whichever replica its streams deliver first, so a waiter routinely arrives before the task does (`taskCreationWaiters`).

Both halves are unbounded except by the caller's context, because there is no local answer to give while the replicas are simply still in flight; the caller owns the deadline. The caller is another process (the secondary's proxy, on a replicate stream with no deadline), so the manager keeps a `closing` channel that `Close()` shuts **before** waiting on its lifetime — otherwise one gated replica would hold `lifetime.Wait()` until the pod is killed. That shut is guarded by a `sync.Once`, so `Close()` is idempotent rather than a double-close panic. A wait that outlives `waitVChannelsAckedSlowThreshold` (30s) logs one Warn naming the broadcast and the VChannels, since head-of-line blocking on the caller's stream is otherwise indistinguishable from wedged replication.
