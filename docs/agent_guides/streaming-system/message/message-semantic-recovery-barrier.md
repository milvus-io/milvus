...
2. It establishes a per-VChannel query-resource baseline during recovery without
   storing per-VChannel MVCC snapshots in the recovery checkpoint.

At recovery start, after the persisted recovery snapshot has been loaded but
before stream replay begins, the node appends one persisted `RecoveryBarrier`
message to the recovered PChannel. The append must succeed before the node can
continue recovery or serve the PChannel; failure means this node is fenced or
otherwise does not own a writable WAL. The ability to prevent an old owner from
writing after a newer recovery owner has written the barrier depends on the WAL
backend; currently this is provided by Woodpecker writer fencing. Recovery then
replays WAL messages from the checkpoint through the `RecoveryBarrier`. When the
barrier is consumed, all earlier replayed messages have rebuilt VChannel
metadata, growing runtime state, transform-log state, segment state, and other
in-memory resources. The barrier TimeTick becomes the initial query MVCC
baseline for every live VChannel recovered on that PChannel.
...
role, it establishes the recovered query-resource baseline.
...
When a `RecoveryBarrier` with TimeTick `T` is applied during recovery replay,
every live VChannel on the PChannel must observe the barrier as a query-resource
baseline:
...
- Query resource modules for each live VChannel receive a recovery-baseline
  event at `T`. Runtime-specific behavior is defined by
...
This makes future QueryView query plans safe to use the barrier TimeTick as the
starting per-VChannel MVCC even when no Insert or Delete message has occurred on
that VChannel after recovery.
...
- Successful append of the barrier proves the recovering node can write the
  ...
  the PChannel.
- The barrier must be consumed before the recovered WAL is considered query-ready.
