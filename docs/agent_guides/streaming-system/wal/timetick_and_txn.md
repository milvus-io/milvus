...
3. **Commit**: Mark the transaction as committed. The transaction's messages become visible to consumers as a single atomic group at the assigned TimeTick. **A producer must wait for the append result of every body before it appends the CommitTxn.** The TimeTick interceptor tolerates a body that is still in flight when the CommitTxn arrives (it waits for the body), but an interceptor above it that decides on the transaction at CommitTxn time (the primary key index) has already taken its snapshot of the bodies by then. That interceptor ends the process when a body is applied after its commit decision. The streaming client (`internal/distributed/streaming`) and `txn.AppendInTxn` obey this rule.
