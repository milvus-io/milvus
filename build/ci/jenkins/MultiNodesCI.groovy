...
String cron_string = BRANCH_NAME == "master" ? "00 13 * * 0 " : ""
...
//                         'pymilvus-orm'
                ...
                agent {
                    kubernetes {
                        label "milvus-e2e-test-kind-multi-nodes"
                        inheritFrom 'default'
                                ...
                                dir ('tests/scripts') {
                                    script {
                                        ...
                                        if ("${MILVUS_CLIENT}" == "pymilvus") {
                                            ...
                                            --install-extra-arg "--set etcd.enabled=false --set externalEtcd.enabled=true \
                                                                 --set externalEtcd.endpoints={\$KRTE_POD_IP:2379} \
                        ...
                        container('main') {
                            script {
                                ...
                                withCredentials([usernamePassword(credentialsId: "${env.DOCKER_CREDENTIALS_ID}", usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD')]) {
                                    ...
                                    sh """
                                        docker tag localhost:5000/milvus:latest ${TARGET_REPO}/milvus-multi-nodes:${env.BRANCH_NAME}-${date}-${gitShortCommit}
                                        docker tag localhost:5000/milvus:latest ${TARGET_REPO}/milvus-multi-nodes:${env.BRANCH_NAME}-latest
                                        docker push ${TARGET_REPO}/milvus-multi-nodes:${env.BRANCH_NAME}-${date}-${gitShortCommit}
                                        docker push ${TARGET_REPO}/milvus-multi-nodes:${env.BRANCH_NAME}-latest
                                    """
                                    sh 'docker logout'
                                }
                            }
                    ...
                    always {
                        container('main') {
                            script {
                                ...
                                dir("${env.ARTIFACTS}") {
                                    sh "find ./kind -path '*/history/*' -type f | xargs tar -zcvf artifacts-${PROJECT_NAME}-${MILVUS_SERVER_TYPE}-${SEMVER}-${env.BUILD_NUMBER}-e2e-multi-nodes-logs.tar.gz --transform='s:^[^/]*/[^/]*/[^/]*/[^/]*/::g' || true"
                                    if ("${MILVUS_CLIENT}" == "pymilvus") {
