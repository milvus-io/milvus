...
class TestQueryBase(TestcaseBase):
    ...
    def test_query(self):
        ...
        collection_w, vectors, _, = self.init_collection_general(prefix, init_data=True)
    ...
    def test_query_binary_expr_single_term_array(self):
        ...
        expected: query result is one entity
        """
        collection_w, vectors, _, = self.init_collection_general(prefix, init_data=True, is_binary=True)
        term_expr = f'{ct.default_int_field_name} in [0]'
        res, _ = collection_w.query(term_expr)
        assert len(res) == 1
    ...
    def test_query_expr_half_term_array(self):
        ...
        df_default = cf.gen_default_dataframe_data(half)
        collection_w.insert(df_default)
        df_partition = cf.gen_default_dataframe_data(half, start=half)
