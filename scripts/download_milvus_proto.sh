...
# Use the last field to handle replace directive (e.g., "... => ... v2.0.0-xxx")
API_VERSION=$(go list -m github.com/milvus-io/milvus-proto/go-api/v2 | awk '{print $NF}')

if [ ! -d "$THIRD_PARTY_DIR/milvus-proto" ]; then
  mkdir -p $THIRD_PARTY_DIR
  pushd $THIRD_PARTY_DIR
  git clone https://github.com/SpadeA-Tang/milvus-proto.git
  cd milvus-proto
  # try tagged version first
  COMMIT_ID=$(git ls-remote https://github.com/SpadeA-Tang/milvus-proto.git refs/tags/${API_VERSION} | cut -f 1)
  if [[ -z $COMMIT_ID ]]; then
