...
for d in $(go list ./tests/integration/...); do
    ...
    if [[ $d == *"coordrecovery"* ]]; then
        ...
        go test -tags dynamic,test -v -coverprofile=profile.out -covermode=atomic "$d" -caseTimeout=20m -timeout=30m
    else
        go test -race -tags dynamic,test -v -coverpkg=./... -coverprofile=profile.out -covermode=atomic "$d" -caseTimeout=15m -timeout=30m
    fi
