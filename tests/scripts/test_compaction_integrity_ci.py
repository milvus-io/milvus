...
@pytest.mark.parametrize("line_number,expected", [("42", 42), ("unknown", None)])
def test_error_info_preserves_traceback_when_line_number_is_invalid(log_plugin, tmp_path, line_number, expected):
    handler = log_plugin.ConditionalLogHandler(SimpleNamespace(log_path=str(tmp_path)))
    traceback = f"test_example.py:{line_number} > assertion\nE       AssertionError: mismatch"
    info = handler._extract_error_info(SimpleNamespace(longreprtext=traceback))
    assert info == {
        "type": "AssertionError",
        "message": "AssertionError: mismatch",
        "traceback": traceback,
...
def test_error_info_preserves_legacy_conversion_fallback(log_plugin, monkeypatch, tmp_path, error_type):
    # Keep this lint-only change equivalent to 3.0's original bare except.
    def failed_conversion(value):
        raise error_type("line-number conversion failed")

    monkeypatch.setattr(log_plugin, "int", failed_conversion, raising=False)
    handler = log_plugin.ConditionalLogHandler(SimpleNamespace(log_path=str(tmp_path)))
    traceback = "test_example.py:42 > assertion\nE       AssertionError: mismatch"
    info = handler._extract_error_info(SimpleNamespace(longreprtext=traceback))
    assert info["line_number"] is None
    assert info["file"] == "test_example.py"
    assert info["message"] == "AssertionError: mismatch"
...
def test_sdk_helpers_are_isolated_and_all_oracle_units_are_selectable():
    ...
    helper = directory / "compaction_integrity_helpers.py"
    assert helper.is_file()
    assert not (directory / "test_compaction_integrity_helpers.py").exists()
