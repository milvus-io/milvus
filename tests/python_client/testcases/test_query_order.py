...
class TestQueryOrderErrorV2(TestMilvusClientV2Base):
    ...
    def test_order_by_vector_field_error(self):
        ...
        error = {ct.err_code: 1100, ct.err_msg: "order_by"}
        self.query(
            client,
            collection_name,
            filter="",
...
class TestQueryOrderNullableV2(TestMilvusClientV2Base):
    ...
    @pytest.mark.tags(CaseLabel.L1)
    def test_order_by_nullable_field_desc(self):
        """
        target: test ORDER BY DESC on a nullable field
        method: create collection with nullable Int32 field, insert data with NULLs,
                query with order_by=["score:desc"]
        expected: results sorted descending, NULL values at the beginning
                  (NULLS FIRST is default for DESC per PostgreSQL convention)
        """
        client = self._client()
        ...
        # NULLs should be at the beginning (NULLS FIRST is default for DESC)
        for i in range(null_count):
            assert results[i]["score"] is None, \
                f"Expected NULL at index {i}, got {results[i]['score']}"

        # Non-null values should be sorted descending after the NULLs
        non_null_scores = [r["score"] for r in results if r["score"] is not None]
        for i in range(len(non_null_scores) - 1):
            assert non_null_scores[i] >= non_null_scores[i + 1], \
                f"Non-null scores not in descending order at index {i}: " \
                f"{non_null_scores[i]} < {non_null_scores[i+1]}"

        self.drop_collection(client, collection_name)
        log.info(f"test_order_by_nullable_field_desc passed: {null_count} NULLs at front")

    @pytest.mark.tags(CaseLabel.L1)
    def test_order_by_nullable_multi_type(self):
        """
        target: test ORDER BY on nullable fields of different data types
        method: create collection with nullable Double and VarChar fields,
                query with order_by on each nullable field
        expected: correct sorting with NULLs handled properly for each type
        """
        client = self._client()
        ...
        schema.add_field("tag", DataType.VARCHAR, max_length=100, nullable=True)
        schema.add_field("vec", DataType.FLOAT_VECTOR, dim=8)
        ...
        for i in range(60):
            row = {
                "price": None if i % 4 == 0 else round(float(np.random.uniform(1.0, 100.0)), 2),
                "tag": None if i % 6 == 0 else np.random.choice(tags),
                "vec": [float(x) for x in np.random.random(8)],
            }
            rows.append(row)
        self.insert(client, collection_name, data=rows)
        ...
        # Test 1: nullable Double ASC (NULLS LAST)
        results, _ = self.query(
            client,
            collection_name,
            filter="",
            output_fields=["price", "tag"],
            limit=60,
            order_by=["price:asc"],
        )
        assert len(results) == 60

        non_null_prices = [r["price"] for r in results if r["price"] is not None]
        for i in range(len(non_null_prices) - 1):
            assert non_null_prices[i] <= non_null_prices[i + 1], \
                f"Double not in ascending order at {i}: {non_null_prices[i]} > {non_null_prices[i+1]}"

        # NULLs at end for ASC
        first_null = next((i for i, r in enumerate(results) if r["price"] is None), None)
        if first_null is not None:
            for i in range(first_null, len(results)):
                assert results[i]["price"] is None, \
                    f"Non-NULL price found after NULL at index {i}"

        # Test 2: nullable VarChar ASC (NULLS LAST)
        results, _ = self.query(
            client,
            collection_name,
            filter="",
        ...
        non_null_tags = [r["tag"] for r in results if r["tag"] is not None]
        for i in range(len(non_null_tags) - 1):
            assert non_null_tags[i] <= non_null_tags[i + 1], \
                f"VarChar not in ascending order at {i}: '{non_null_tags[i]}' > '{non_null_tags[i+1]}'"

        # NULLs at end for ASC
        first_null = next((i for i, r in enumerate(results) if r["tag"] is None), None)
        if first_null is not None:
            for i in range(first_null, len(results)):
                assert results[i]["tag"] is None, \
                    f"Non-NULL tag found after NULL at index {i}"

        self.drop_collection(client, collection_name)
        log.info("test_order_by_nullable_multi_type passed")

    @pytest.mark.tags(CaseLabel.L1)
    def test_order_by_nullable_cross_segment(self):
        """
        target: test ORDER BY nullable field with data across multiple segments
        method: insert nullable data in 3 batches with flush between each,
                query with ORDER BY to verify global sorting correctness
        expected: NULLs from all segments correctly positioned, non-NULLs globally sorted
        """
        client = self._client()
        ...
        for batch_idx in range(3):
            rows = []
            for i in range(100):
                score = None if i % 7 == 0 else int(np.random.randint(1, 500))
                rows.append({
                    "score": score,
                    "vec": [float(x) for x in np.random.random(8)],
                })
        ...
        # Query with limit to get top-K sorted results
        limit = 50
        results, _ = self.query(
            client,
            collection_name,
        ...
        # Build ground truth: sort non-null ASC, then nulls
        non_null_gt = sorted([s for s in all_scores if s is not None])
        null_count_gt = sum(1 for s in all_scores if s is None)
        expected = non_null_gt + [None] * null_count_gt

        # Verify first `limit` results match ground truth
        for i in range(limit):
            result_score = results[i]["score"]
            expected_score = expected[i]
            assert result_score == expected_score, \
                f"Cross-segment mismatch at index {i}: got {result_score}, expected {expected_score}"

        self.drop_collection(client, collection_name)
        log.info("test_order_by_nullable_cross_segment passed")

    @pytest.mark.tags(CaseLabel.L2)
    def test_order_by_nullable_all_null(self):
        """
        target: test ORDER BY when all values in the sort field are NULL
        method: create collection where nullable field is always NULL, query with order_by
        expected: all results returned with NULL values, no crash
        """
        client = self._client()
        ...
        for i, r in enumerate(results):
            assert r["score"] is None, \
                f"Expected NULL at index {i}, got {r['score']}"

        self.drop_collection(client, collection_name)
        log.info("test_order_by_nullable_all_null passed")
