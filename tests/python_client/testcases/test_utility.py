...
class TestUtilityParams(TestcaseBase):
    ...
    @pytest.fixture(
        scope="function", params=["JACCARD", "Superstructure", "Substructure"]
    )
    ...
    ******************************************************************
    ...
    def test_loading_progress_invalid_collection_name(self, invalid_c_name):
        ...
        self.collection_wrap.construct_from_dataframe(
            c_name, df, primary_field=ct.default_int64_field_name
        )
        self.collection_wrap.load()
        error = {
            ct.err_code: 1,
            ct.err_msg: "Invalid collection name: {}".format(invalid_c_name),
        }
        self.utility_wrap.loading_progress(
            invalid_c_name, check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_loading_progress_not_existed_collection_name(self):
        ...
        error = {
            ct.err_code: 1,
            ct.err_msg: "describe collection failed: can't find collection",
        }
        self.utility_wrap.loading_progress(
            "not_existed_name", check_task=CheckTasks.err_res, check_items=error
        )

    @pytest.mark.tag(CaseLabel.L1)
    @pytest.mark.xfail(reason="pymilvus issue #677")
    def test_loading_progress_invalid_partition_names(
        self, get_invalid_partition_names
    ):
        """
        ...
        err_msg = {
            ct.err_code: 0,
            ct.err_msg: "`partition_name_array` value {} is illegal".format(
                partition_names
            ),
        }
        collection_w.load()
    ...
    @pytest.mark.parametrize(
        "partition_names",
        [[ct.default_tag], [ct.default_partition_name, ct.default_tag]],
    )
    def test_loading_progress_not_existed_partitions(self, partition_names):
        ...
        err_msg = {
            ct.err_code: 1,
            ct.err_msg: f"partitionID of partitionName:{ct.default_tag} can not be found",
        }
        self.utility_wrap.loading_progress(
            collection_w.name,
            partition_names,
    ...
    def test_drop_collection_not_existed(self):
        ...
        self.utility_wrap.drop_collection(
            c_name, check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_calc_distance_invalid_metric_type(
        self, get_support_metric_field, get_invalid_metric_type
    ):
        """
        ...
        self.utility_wrap.calc_distance(
            ...
            check_items={
                "err_code": 1,
                "err_msg": "params value {{'metric': {}}} " "is illegal".format(metric),
            },
        )

    @pytest.mark.tags(CaseLabel.L2)
    def test_calc_distance_invalid_metric_value(
        self, get_support_metric_field, get_invalid_metric_value
    ):
        """
        target: test calculated distance with invalid metric
        method: input invalid metric
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_calc_distance_not_support_metric(
        self, get_support_metric_field, get_not_support_metric
    ):
        """
        target: test calculated distance with invalid metric
        method: input invalid metric
    ...
    def test_calc_distance_invalid_using(self, get_support_metric_field):
        ...
        self.utility_wrap.calc_distance(
            ...
            using=using,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": "should create connect"},
        )
    ...
    def test_calc_distance_collection_before_load(self, get_support_metric_field):
        ...
        self._connect()
        nb = 10
        collection_w, vectors, _, insert_ids, _ = self.init_collection_general(
            prefix, True, nb, is_index=True
        )
...
class TestUtilityBase(TestcaseBase):
    ...
    def test_loading_progress_with_partition(self):
        ...
        res = self.utility_wrap.loading_progress(
            collection_w.name, partition_names=[partition_w.name]
        )[0]
    ...
    def test_calc_distance_default(self):
        ...
        self.utility_wrap.calc_distance(
            ...
            check_items={"vectors_l": vectors_l, "vectors_r": vectors_r},
        )
    ...
    def test_calc_distance_from_collections(self, metric_field, metric, sqrt):
        ...
        collection_w_1, vectors_1, _, insert_ids_1, _ = self.init_collection_general(
            prefix_1, True, nb
        )
        ...
        op_r = {
            "ids": insert_ids_1,
            "collection": collection_w_1.name,
            "field": default_field_name,
        }
        params = {metric_field: metric, "sqrt": sqrt}
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_calc_distance_left_vector_and_collection_ids(
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_calc_distance_left_vectors_and_partition_ids(
        self, metric_field, metric, sqrt
    ):
        """
