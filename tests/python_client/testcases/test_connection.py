    ...
    def test_connection_add_connection_kwargs_invalid_host_port(self):
        ...
        for data in [{"port": "port"}, {"port": ["192.168.1.1"]}]:
            self.connection_wrap.add_connection(
                ...
                check_items={ct.err_code: ct.ANY_CODE, ct.err_msg: "Type of 'port' must be str or int"},
            )
        for data in [{"host": -1}]:
            self.connection_wrap.add_connection(
                _kwargs=data,
        ...
        self.connection_wrap.add_connection(
            _kwargs=data,
            check_task=ct.CheckTasks.err_res,
            check_items={ct.err_code: ct.ANY_CODE, ct.err_msg: "port number -1 out of range, valid range [0, 65535)"},
        )
...
class TestConnectionOperation(TestcaseBase):
    ...
    def test_connection_connect_default_alias_invalid(self, port):
        ...
        self.connection_wrap.connect(
            alias=DefaultConfig.DEFAULT_USING,
            check_task=ct.CheckTasks.err_res,
            check_items={ct.err_code: ct.ANY_CODE, ct.err_msg: "illegal connection params or server unavailable"},
        )
    ...
    def test_connection_init_collection_invalid_connection(self):
        ...
        self.collection_wrap.init_collection(
            ...
            schema=schema,
            check_task=ct.CheckTasks.err_res,
            check_items={ct.err_code: ct.ANY_CODE, ct.err_msg: cem.ConnectFirst},
            _using=ct.Not_Exist,
        )
