...
from utils.utils import gen_invalid_ints, gen_invalid_ips, gen_invalid_uris, get_milvus
    ...
    def test_connection_add_connection_kwargs_param_check(self, data):
        ...
        self.connection_wrap.list_connections(
            ...
            check_items={ct.list_content: [
                (DefaultConfig.DEFAULT_USING, None)]},
        )
    ...
    def test_connection_connect_alias_param_check(self, alias):
        ...
        self.connection_wrap.connect(
            ...
            check_items={ct.err_code: 0,
                         ct.err_msg: cem.AliasType % type(alias)},
        )
...
class TestConnectionOperation(TestcaseBase):
    ...
    @pytest.mark.parametrize(
        "data, err_msg",
        [
            (ct.get_wrong_format_dict[0], cem.PortType),
    ...
    def test_connection_add_default(self):
        ...
        self.connection_wrap.add_connection(
            default={"port": "19530", "host": "localhost"}
        )
    ...
    def test_connection_add_cover_default(self):
        ...
        # add connections
        self.connection_wrap.add_connection(
            default={"host": "192.168.1.1", "port": "12345"}
        )
    ...
    def test_connection_add_after_connect(self, host, port):
        ...
        self.connection_wrap.connect(
            alias="test_alias_name", host=host, port=port, check_task=ct.CheckTasks.ccr
        )
        ...
        self.connection_wrap.add_connection(
            test_alias_name={"host": host, "port": port}
        )
    ...
    def test_connection_connect_default_alias_effective(self, host, port):
        ...
        self.connection_wrap.connect(
            alias=DefaultConfig.DEFAULT_USING, check_task=ct.CheckTasks.ccr
        )
        ...
        self.connection_wrap.list_connections(
            ...
            check_items={
                ct.list_content: [
                    (DefaultConfig.DEFAULT_USING, ct.Connect_Object_Name)]
            },
        )
    ...
    def test_connection_connect_repeat(self, host, port, connect_name):
        ...
        # successfully created default connection
        self.connection_wrap.connect(
            alias=connect_name, check_task=ct.CheckTasks.ccr)

        # get the object of alias
        res_obj1 = self.connection_wrap.get_connection(
            alias=connect_name,
            check_task=ct.CheckTasks.ccr,
        ...
        self.connection_wrap.connect(
            alias=connect_name, host=host, port=port, check_task=ct.CheckTasks.ccr
        )

        # get the object of alias
        res_obj2 = self.connection_wrap.get_connection(
            alias=connect_name,
            check_task=ct.CheckTasks.ccr,
            check_items={ct.value_content: ct.Connect_Object_Name},
        )[0]
    ...
    def test_connection_connect_params(self, host, port, connect_name):
        ...
        # list all connections and check the response
        list_content = (
            [(connect_name, ct.Connect_Object_Name)]
        ...
        self.connection_wrap.list_connections(
            check_task=ct.CheckTasks.ccr, check_items={
                ct.list_content: list_content}
        )
    ...
    @pytest.mark.tags(ct.CaseLabel.L2)
    @pytest.mark.parametrize(
        "connect_name", [DefaultConfig.DEFAULT_USING, ct.Not_Exist]
    )
    ...
    @pytest.mark.tags(ct.CaseLabel.L1)
    @pytest.mark.parametrize(
        "connect_name", [DefaultConfig.DEFAULT_USING, "test_alias_name"]
    )
    def test_connection_remove_after_connect(self, host, port, connect_name):
        ...
        list_content = (
            []
            if connect_name == DefaultConfig.DEFAULT_USING
            else [(DefaultConfig.DEFAULT_USING, None)]
        )
        self.connection_wrap.list_connections(
            check_task=ct.CheckTasks.ccr, check_items={
                ct.list_content: list_content}
    ...
    def test_connection_init_collection_connection(self, host, port):
        ...
        schema = cf.gen_default_collection_schema()
        self.collection_wrap.init_collection(
            name=collection_name, schema=schema, _using=DefaultConfig.DEFAULT_USING
        )
        ...
        self.collection_wrap.drop(
            check_task=ct.CheckTasks.err_res,
            check_items={ct.err_code: 0,
                         ct.err_msg: "should create connect first"},
        )
