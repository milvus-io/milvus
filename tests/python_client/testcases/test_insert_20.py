...
class TestInsertParams(TestcaseBase):
    ...
    def test_insert_non_data_type(self, get_non_data_type):
        ...
        collection_w.insert(
            data=get_non_data_type, check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_insert_empty_data(self, data):
        ...
        error = {
            ct.err_code: 0,
            ct.err_msg: "The data fields number is not match with schema",
        }
        collection_w.insert(
    ...
    def test_insert_numpy_data(self):
        ...
        error = {ct.err_code: 0,
                 ct.err_msg: "Data type not support numpy.ndarray"}
        collection_w.insert(
            data=data, check_task=CheckTasks.err_res, check_items=error)
...
class TestInsertOperation(TestcaseBase):
    ...
    ******************************************************************
    ...
    @pytest.mark.parametrize(
        "vec_fields",
        [
            [cf.gen_float_vec_field(name="float_vector1")],
    ...
    def test_insert_multi_times(self, dim):
        ...
        for _ in range(nb // step):
            ...
            assert (
                mutation_res.primary_keys
                == df[ct.default_int64_field_name].values.tolist()
            )
...
class TestInsertAsync(TestcaseBase):
    ...
    ******************************************************************
    ...
    def test_insert_async_callback_timeout(self):
        ...
        df = cf.gen_default_dataframe_data(nb)
        future, _ = collection_w.insert(
            data=df, _async=True, _callback=assert_mutation_result, timeout=1
        )
