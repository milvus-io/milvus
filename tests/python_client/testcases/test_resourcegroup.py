...
class TestTransferNode(TestcaseBase):
    ...
    def test_transfer_node_with_wrong_num_node(self, num_node):
        ...
        # transfer replicas
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "resource group node not enough"}
        if num_node in [0, -1]:
            error = {
                ct.err_code: ct.ANY_CODE,
    ...
    def test_transfer_replica_with_wrong_num_replica(self, num_replica):
        ...
        if num_replica == 99:
            error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "NumReplica not greater than the number of replica"}
    ...
    def test_load_collection_with_multi_replicas_multi_rgs(self, replicas):
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "resource group num can only be 0, 1 or same as replica number"}
        collection_w.load(
