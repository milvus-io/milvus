...
class TestResourceGroupMultiNodes(TestcaseBase):
    ...
    def test_transfer_replica_into_same_rg(self, with_growing):
        ...
        error = {ct.err_code: 999,
                 ct.err_msg: 'found [1] replicas of same collection in target resource group[{}], dynamically increase replica num is unsupported'.format(rgA_name)}
        self.utility_wrap.transfer_replica(source=rgB_name, target=rgA_name,
                                           collection_name=collection_w.name, num_replica=1,
                                           check_task=CheckTasks.err_res,
                                           check_items=error)
        error = {ct.err_code: 999,
                 ct.err_msg: 'found [1] replicas of same collection in target resource group[{}], dynamically increase replica num is unsupported'.format(rgB_name)}
