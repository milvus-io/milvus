...
class TestFieldPartialLoad(TestcaseBase):
    ...
    def test_skip_load_dynamic_field(self):
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "field color cannot be returned since dynamic field not loaded"}
        collection_w.search(
            data=search_vectors,
            anns_field=vector_field.name,
            param=search_params,
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "field color is dynamic but dynamic field is not loaded"}
        collection_w.search(
            data=search_vectors,
            anns_field=vector_field.name,
            param=search_params,
    ...
    def test_partial_load_with_partition(self):
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "can't change the load field list for loaded collection"}
        p2.load(
            load_fields=[pk_field.name, vector_field.name, not_load_string_field.name, not_load_int64_field.name],
            check_task=CheckTasks.err_res,
            check_items=error,
...
class TestFieldPartialLoadInvalid(TestcaseBase):
    ...
    def test_skip_load_on_pk_field_or_vector_field(self):
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "does not contain vector field"}
        collection_w.load(
    ...
    def test_skip_load_on_partition_key_field(self):
        ...
        error = {
            ...
            ct.err_msg: f"does not contain partition key field {partition_key_field.name}",
        }
        collection_w.load(
    ...
    def test_skip_load_on_clustering_key_field(self):
        ...
        error = {
            ct.err_code: ct.ANY_CODE,
            ct.err_msg: f"does not contain clustering key field {clustering_key_field.name}",
        }
        collection_w.load(
            load_fields=[vector_field.name, pk_field.name], check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_one_of_dynamic_fields_in_load_fields_list(self):
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "failed to get field schema by name: fieldName(color) not found"}
        collection_w.load(
        ...
        error = {
            ct.err_code: ct.ANY_CODE,
            ct.err_msg: "failed to get field schema by name: fieldName(not_existing) not found",
        }
        collection_w.load(
    ...
    def test_search_on_not_loaded_fields(self):
        ...
        error = {ct.err_code: ct.ANY_CODE, ct.err_msg: "cannot parse expression"}
        collection_w.search(
            data=search_vectors,
            anns_field=vector_field.name,
            param=search_params,
