...
@pytest.mark.xdist_group("TestGroupSearchNewHNSWIndex")
class TestGroupSearchNewHNSWIndex(TestCaseClassBase):
    """
    Testing group search scenarios after new HNSW indexes
    1. collection schema:
        int64_pk(auto_id), varchar,
        float16_vector, float_vector, bfloat16_vector, sparse_vector,
        inverted_varchar
    ...
    def setup_class(self):
        ...
        # create a collection with fields
        self.collection_wrap.init_collection(
            name=cf.gen_unique_str("TestGroupSearchNewHNSWIndex"),
            schema=cf.set_collection_schema(
                fields=[self.primary_field, DataType.VARCHAR.name, DataType.FLOAT16_VECTOR.name,
                        DataType.FLOAT_VECTOR.name, DataType.BFLOAT16_VECTOR.name, DataType.BFLOAT16_VECTOR.name+"new",
                        DataType.INT8.name, DataType.INT64.name, DataType.BOOL.name,
                        self.inverted_string_field],
                field_params={
                    self.primary_field: FieldParams(is_primary=True).to_dict,
                    DataType.FLOAT16_VECTOR.name: FieldParams(dim=128).to_dict,
                    DataType.FLOAT_VECTOR.name: FieldParams(dim=128).to_dict,
                    DataType.BFLOAT16_VECTOR.name: FieldParams(dim=128).to_dict,
                    DataType.BFLOAT16_VECTOR.name+"new": FieldParams(dim=128).to_dict
                },
                auto_id=True
            )
        )
        ...
        self.vector_fields = [DataType.FLOAT16_VECTOR.name, DataType.FLOAT_VECTOR.name,
                              DataType.BFLOAT16_VECTOR.name, DataType.BFLOAT16_VECTOR.name]
        self.dims = [128, 128, 128, 128]
        self.index_types = [cp.IndexName.HNSW_FLAT, cp.IndexName.HNSW_SQ, cp.IndexName.HNSW_PQ, cp.IndexName.HNSW_PRQ]
    ...
    def prepare_data(self):
        ...
        # build index for each vector field
        index_params = {
            **DefaultVectorIndexParams.HNSW_FLAT(DataType.FLOAT16_VECTOR.name, metric_type=MetricType.L2),
            **DefaultVectorIndexParams.HNSW_SQ(DataType.FLOAT_VECTOR.name, metric_type=MetricType.IP),
            **DefaultVectorIndexParams.HNSW_PQ(DataType.BFLOAT16_VECTOR.name, metric_type=MetricType.COSINE),
            **DefaultVectorIndexParams.HNSW_PRQ(DataType.BFLOAT16_VECTOR.name+"new", metric_type=MetricType.IP),
            # index params for varchar field
            **DefaultScalarIndexParams.INVERTED(self.inverted_string_field)
        }

        self.build_multi_index(index_params=index_params)
        for n in self.collection_wrap.indexes:
            log.debug(n.field_name)
        assert sorted([n.field_name for n in self.collection_wrap.indexes]) == sorted(index_params.keys())
    ...
    @pytest.mark.tags(CaseLabel.L0)
    # @pytest.mark.skip(reason="issue #37602")
    @pytest.mark.parametrize("group_by_field", [DataType.VARCHAR.name, "varchar_inverted"])
    ...
    @pytest.mark.tags(CaseLabel.L0)
    def test_hybrid_search_group_size_new_hnsw_index(self):
        """
        hybrid search group by on 4 different float vector fields with group by varchar field with group size
        verify results returns with de-dup group values and group distances are in order as rank_group_scorer
        """
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_hybrid_search_group_by_new_hnsw_index(self):
        """
        verify hybrid search group by works with different Rankers
        """
        # 3. prepare search params
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_pagination_group_by_new_hnsw_index(self):
        """
        verify search group by works with pagination
        """
        limit = 10
    ...
    @pytest.mark.tags(CaseLabel.L0)
    @pytest.mark.skip(reason="issue #37601")
    def test_search_pagination_group_size_new_hnsw_index(self):
        limit = 10
        group_size = 5
        page_rounds = 3
        search_param = {}
