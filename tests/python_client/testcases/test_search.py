...
class TestCollectionSearchInvalid(TestcaseBase):
    ...
    ******************************************************************
    ...
    def test_search_index_partition_not_existed(self):
        ...
        # 1. initialize with data
        collection_w = self.init_collection_general(prefix, True)[0]
        # 2. search the non exist partition
...
class TestCollectionSearch(TestcaseBase):
    ...
    ******************************************************************
    ...
    def test_search_without_metric_type_multi_querynodes(self):
        """
        target: test search without metric type multi querynodes
    ...
    def test_search_normal_default_params(self, dim, auto_id, search_params, _async, enable_dynamic_field):
        ...
        collection_w = self.init_collection_general(auto_id=auto_id, is_index=False,
                                                    dim=dim, name=new_collection_name,
                                                    enable_dynamic_field=enable_dynamic_field)[0]
        # 3. search
    ...
    def test_search_HNSW_index_with_redundant_param(self, M, efConstruction, limit, auto_id,
        ...
        collection_w.create_index("float_vector", HNSW_index)
        collection_w.load()
        search_param = {"metric_type": "L2", "params": {"ef": ef, "nprobe": 10}}  # nprobe is of no use
        vectors = [[random.random() for _ in range(dim)] for _ in range(default_nq)]
        collection_w.search(vectors[:default_nq], default_search_field,
                            search_param, limit,
                            default_search_exp, _async=_async,
    ...
    def test_search_with_expression_auto_id(self, dim, expression, _async, enable_dynamic_field):
        ...
        # 2. search with different expressions
    ...
    def test_search_with_expression_json_contains(self, enable_dynamic_field):
        ...
        for expression in expressions:
            collection_w.search(vectors[:default_nq], default_search_field,
                                ...
                                check_items={"nq": default_nq,
                                             "limit": 3})
        expressions = ["json_contains('list', 100)", "JSON_CONTAINS('list', 100)"]
        for expression in expressions:
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.skip(reason="issue #25487")
    ...
    def test_search_with_expression_large_two(self, dim, enable_dynamic_field):
        ...
        # 1. initialize with data
        nb = 10000
        collection_w, _, _, insert_ids = self.init_collection_general(prefix, True, nb, dim=dim,
    ...
    @pytest.mark.parametrize("partition_name", ["_PartiTi0n", "pArt1_ti0n"])
    @pytest.mark.parametrize("index_name", ["_1ndeX", "In_0"])
    def test_search_partition_naming_rules_with_index(self, nq, dim, auto_id, partition_name,
                                                      index_name, enable_dynamic_field):
        """
        target: test search collection naming rules
        method: 1. Connect milvus
                2. Create a collection
    ...
    ******************************************************************
    ...
    def test_search_invalid_params_with_diskann_A(self, dim, auto_id, search_list, limit):
        ...
        method: 1.create collection , insert data, primary_field is int field
                ...
                3.search with invalid params, where  topk <=20, search list [0, 200]
    ...
    def test_search_invalid_params_with_diskann_B(self, dim, auto_id, search_list, limit):
        ...
        method: 1.create collection , insert data, primary_field is int field
                2.create  diskann index 
                3.search with invalid params, [0, 200] when k <= 20
        expected: search report an error
        """
        # 1. initialize with data
        collection_w, _, _, insert_ids = \
    ...
    def test_search_invalid_params_with_diskann_C(self, dim, auto_id, search_list, limit):
        ...
        method: 1.create collection , insert data, primary_field is int field
                2.create diskann index 
                3.search with invalid params , [0, min( 10 * topk, 65535)] when k > 20
        expected: search report an error
        """
        # 1. initialize with data
        collection_w, _, _, insert_ids = \
