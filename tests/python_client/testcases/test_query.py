...
class TestQueryParams(TestcaseBase):
    ...
    def test_query_expr_json_contains(self, enable_dynamic_field):
        ...
        expressions = ["json_contains('list', 1000)", "JSON_CONTAINS('list', 1000)"]
        for expression in expressions:
            msg = f"cannot parse expression: {expression}, error: json_contains operation are only supported " \
                  f"on json fields now, got: 'list'"
            collection_w.query(expression, check_task=CheckTasks.err_res, check_items={ct.err_code: 1, ct.err_msg: msg})
    ...
    def test_query_invalid_ignore_growing_param(self, ignore_growing):
        ...
        expected: run as ignore_growing=False
        """
        if ignore_growing == 1:
