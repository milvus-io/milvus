...
class TestQueryParams(TestcaseBase):
    ...
    def test_query_auto_id_collection(self):
        ...
        insert_res, _, = self.collection_wrap.construct_from_dataframe(
        ...
        df = pd.DataFrame(
            {
                ...
                ct.default_float_field_name: pd.Series(
                    data=[float(i) for i in range(ct.default_nb)], dtype="float32"
                ),
    ...
    def test_query_expr_by_bool_field(self):
        ...
        bool_values = pd.Series(
            data=[i % 2 == 0 for i in range(ct.default_nb)], dtype="bool"
        )
        ...
        res, _ = self.collection_wrap.query(
            term_expr, output_fields=[ct.default_bool_field_name]
        )
    ...
    def test_query_output_fields_simple_wildcard(self):
        ...
        collection_w, df = self.init_multi_fields_collection_wrap(
            cf.gen_unique_str(prefix)
        )
        collection_w.load()
    ...
    def test_query_not_existed_partition(self):
        ...
        error = {
            ct.err_code: 1,
            ct.err_msg: f"PartitonName: {partition_names} not found",
        }
        collection_w.query(
            default_term_expr,
