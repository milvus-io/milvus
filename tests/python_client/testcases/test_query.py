...
class TestQueryTextMatch(TestcaseBase):
    ...
    ******************************************************************
    ...
    def test_query_text_match_with_addition_inverted_index(self):
        ...
        res = utility.list_indexes(
            collection_name=collection_w.name
        )
        log.info(f"index {res}")
        collection_w.load()
        df = pd.DataFrame(data)
        word_word_freq = cf.analyze_documents(df["word"].tolist())
        sentence_word_freq = cf.analyze_documents(df["sentence"].tolist())
        paragraph_word_freq = cf.analyze_documents(df["paragraph"].tolist())
        text_word_freq = cf.analyze_documents(df["text"].tolist())
        wf_map = {
            "word": word_word_freq,
            "sentence": sentence_word_freq,
            "paragraph": paragraph_word_freq,
            "text": text_word_freq,
        }
        text_fields = ["word", "sentence", "paragraph", "text"]
        # query single field for one word
        for field in text_fields:
            word = list(wf_map[field].keys())[0]
            expr = f"TextMatch({field}, '{word}')"
            log.info(f"expr: {expr}")
            res, _ = collection_w.query(
                expr=expr,
                output_fields=["id", field]
    ...
    def test_query_text_match_vs_like(self):
        ...
        for field in text_fields:
            word = list(wf_map[field].keys())[0]
            text_match_expr = f"TextMatch({field}, '{word}')"
            cap_word = word.capitalize()
            decap_word = cap_word[0].lower() + cap_word[1:]
            like_match_expr = f"{field} like '%{cap_word}%' or {field} like '%{decap_word}%'"
            ...
            tt = time.time() - t0
            log.info(f"text match query cost {tt} res len {len(res)}")
            ...
            tt = time.time() - t0
            log.info(f"like match query cost {tt} res len {len(res)}")
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_query_text_match_vs_like_use_random_dataset(self):
        """
        target: test query iterator normal
        method: 1. query iterator
                2. check the result, expect pk
    ...
    @pytest.mark.tags(CaseLabel.L0)
    def test_query_text_match_with_multi_bool_logic_one_field(self):
        """
        target: test query iterator normal
        method: 1. query iterator
                2. check the result, expect pk
        ...
        cast_word_freq = cf.analyze_documents(df["cast"].tolist())
        wf_map = {
            "title": dict(title_word_freq),
            "overview": dict(overview_word_freq),
            "genres": dict(genres_word_freq),
            "producer": dict(producer_word_freq),
            "cast": dict(cast_word_freq)
        }
        text_fields = ["title", "overview", "genres", "producer", "cast"]
        for i in range(10):

            _, expr = cf.generate_random_query_from_freq_dict(wf_map, min_freq=3, max_terms=5, p_not=0.2)
            log.info(f"expr: {expr}")
            res, _ = collection_w.query(
                expr=expr,
    ...
    @pytest.mark.tags(CaseLabel.L0)
    def test_query_text_match_with_single_bool_logic_one_field(self):
        """
        target: test query iterator normal
        method: 1. query iterator
                2. check the result, expect pk
