...
class TestQueryParams(TestcaseBase):
    ...
    def test_query_no_collection(self):
        ...
        collection_w.query(
            ...
            check_items={"err_code": 1, "err_msg": "collection not found"},
        )
    ...
    def test_query_with_dup_primary_key(self, dim, dup_times):
        ...
        """
        nb = ct.default_nb
        collection_w, insert_data, _, _ = self.init_collection_general(
            prefix, True, nb, dim=dim
        )[0:4]
        ...
        df = pd.DataFrame(
            {
                ...
                ct.default_int16_field_name: pd.Series(
                    data=[np.int16(i) for i in range(ct.default_nb)], dtype="int16"
                ),
                ct.default_float_field_name: pd.Series(
                    data=[np.float32(i) for i in range(ct.default_nb)], dtype="float32"
                ),
                ct.default_double_field_name: pd.Series(
                    data=[np.double(i) for i in range(ct.default_nb)], dtype="double"
                ),
                ct.default_string_field_name: pd.Series(
                    data=[str(i) for i in range(ct.default_nb)], dtype="string"
                ),
                ct.default_float_vec_field_name: cf.gen_vectors(
    ...
    def test_query_expr_by_bool_field(self):
        ...
        res, _ = self.collection_wrap.query(
            default_term_expr, output_fields=[ct.default_bool_field_name]
        )
        ...
        for bool_value in [True, False]:
            exprs = [
                f"{ct.default_bool_field_name} in [{bool_value}]",
                f"{ct.default_bool_field_name} == {bool_value}",
            ]
            for expr in exprs:
                res, _ = self.collection_wrap.query(
                    expr, output_fields=[ct.default_bool_field_name]
                )
    ...
    def test_query_expr_by_int64(self):
        ...
        expr_list = [
            f"{ct.default_int64_field_name} > 8192 && {ct.default_int64_field_name} < 8194",
            f"{ct.default_int64_field_name} > 16384 && {ct.default_int64_field_name} < 16386",
        ]
        for expr in expr_list:
            res, _ = self.collection_wrap.query(
                expr, output_fields=[ct.default_int64_field_name]
            )
            assert len(res) == 1
    ...
    def test_query_expr_by_int8_field(self):
        ...
        int8_values = pd.Series(
            data=[np.int8(i) for i in range(ct.default_nb)], dtype="int8"
        )
        ...
        self.collection_wrap.query(
            term_expr,
            output_fields=["float", "int64", "int8", "varchar"],
            check_task=CheckTasks.check_query_results,
            check_items={exp_res: res},
        )
    ...
    def test_query_with_expression(self, get_normal_expr, enable_dynamic_field):
        ...
        collection_w, _vectors, _, insert_ids = self.init_collection_general(
            prefix, True, nb, enable_dynamic_field=enable_dynamic_field
        )[0:4]
    ...
    def test_query_expr_wrong_term_keyword(self):
        ...
        error_1 = {
            ct.err_code: 65535,
            ct.err_msg: "cannot parse expression: int64 inn [1, 2], "
    ...
    @pytest.mark.parametrize(
        "field", [ct.default_int64_field_name, ct.default_float_field_name]
    )
    def test_query_expr_not_in_term(self, field):
        ...
        self.collection_wrap.query(
            term_expr,
            output_fields=["float", "int64", "varchar"],
            check_task=CheckTasks.check_query_results,
            check_items={exp_res: res},
        )
    ...
    def test_query_expr_non_array_term(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: "cannot parse expression: int64 in 1, "
    ...
    def test_query_expr_inconsistent_mix_term_array(self):
        ...
        for values in int_values:
            ...
            collection_w.query(
                term_expr, check_task=CheckTasks.err_res, check_items=error
            )
    ...
    @pytest.mark.parametrize(
        "expr_prefix",
        ["json_contains", "JSON_CONTAINS", "array_contains", "ARRAY_CONTAINS"],
    )
    def test_query_expr_json_contains(self, enable_dynamic_field, expr_prefix):
        ...
        collection_w = self.init_collection_general(
            prefix, enable_dynamic_field=enable_dynamic_field
        )[0]
    ...
    def test_query_expr_list_json_contains(self, expr_prefix):
        ...
        """
        # 1. initialize with data
        collection_w = self.init_collection_general(prefix, enable_dynamic_field=True)[
            0
        ]
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("expr_prefix", ["json_contains", "JSON_CONTAINS"])
    def test_query_expr_json_contains_combined_with_normal(
        self, enable_dynamic_field, expr_prefix
    ):
        """
        target: test query with expression using json_contains
        method: query with expression using json_contains
        ...
        for i in range(ct.default_nb):
            array[i][ct.default_json_field_name] = {
                "number": i,
                "list": [m for m in range(i, i + limit)],
            }
        ...
        tar = 1000
        expression = (
            f"{expr_prefix}({json_field}['list'], {tar}) && float > {tar - limit // 2}"
        )
        res = collection_w.query(expression)[0]
    ...
    def test_query_expr_all_datatype_json_contains_all(
        self, enable_dynamic_field, expr_prefix
    ):
        """
        target: test query with expression using json_contains
    ...
    def test_query_expr_list_all_datatype_json_contains_all(self, expr_prefix):
        ...
        for i in range(ct.default_nb):
            ...
            array[i]["listStr"] = [
                str(m) for m in range(i, i + limit)
            ]  # test for string
            array[i]["listFlt"] = [
                m * 1.0 for m in range(i, i + limit)
            ]  # test for float
    ...
    def test_query_expr_all_datatype_json_contains_any(
        self, enable_dynamic_field, expr_prefix
    ):
        """
        target: test query with expression using json_contains
    ...
    def test_query_expr_list_all_datatype_json_contains_any(self, expr_prefix):
        ...
        expression = f"{expr_prefix}(listInt, {ids})"
        res = collection_w.query(expression)[0]
        assert [entity[pk_field] for entity in res] == cf.assert_json_contains(
            expression, int_data
        )
    ...
    def test_query_expr_json_contains_list_in_list(
        self, expr_prefix, enable_dynamic_field
    ):
        """
        target: test query with expression using json_contains_any
        method: query with expression using json_contains_any
    ...
    def test_query_expr_json_contains_invalid_type(
        self, expr_prefix, enable_dynamic_field, not_list
    ):
        """
        target: test query with expression using json_contains_any
        method: query with expression using json_contains_any
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("expr_prefix", ["json_contains", "JSON_CONTAINS"])
    def test_query_expr_json_contains_pagination(
        self, enable_dynamic_field, expr_prefix
    ):
        """
        target: test query with expression using json_contains
    ...
    def test_query_expr_array_length(self, array_length, op, enable_dynamic_field):
        ...
        collection_w = self.init_collection_wrap(
            schema=schema, enable_dynamic_field=enable_dynamic_field
        )
    ...
    def test_query_expr_empty_without_limit(self):
        ...
        error = {
            ct.err_code: 1,
            ct.err_msg: "empty expression should be used with limit",
        }
        collection_w.query("", check_task=CheckTasks.err_res, check_items=error)

        # 3. query with offset but no limit
        collection_w.query(
            "", offset=1, check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_query_expr_empty_pk_string(self):
        ...
        # 1. initialize with data
        collection_w, _, _, insert_ids = self.init_collection_general(
            prefix, True, primary_field=ct.default_string_field_name
        )[0:4]
    ...
    def test_query_expr_empty_with_random_pk(self, limit, offset):
        ...
        for ids in exp_ids:
            res.append(
                {
                    ct.default_int64_field_name: ids,
                    ct.default_string_field_name: str(ids),
                }
            )
    ...
    def test_query_expr_with_limit_offset_out_of_range(self):
        ...
        collection_w.query(
            "", limit=16385, check_task=CheckTasks.err_res, check_items=error
        )

        # 3. query with offset + limit > 16384
        collection_w.query(
            "", limit=1, offset=16384, check_task=CheckTasks.err_res, check_items=error
        )
        collection_w.query(
            "", limit=16384, offset=1, check_task=CheckTasks.err_res, check_items=error
        )
        ...
        collection_w.query(
            "", limit=2, offset=-1, check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_query_expr_out_of_range(self, expression):
        ...
        _vectors["int16"] = pd.Series(
            data=[np.int16(i * 40) for i in range(start, start + ct.default_nb)],
            dtype="int16",
        )
        _vectors["int32"] = pd.Series(
            data=[np.int32(i * 2200000) for i in range(start, start + ct.default_nb)],
            dtype="int32",
        )
    ...
    def test_query_output_one_field(self, enable_dynamic_field):
        ...
        res, _ = collection_w.query(
            default_term_expr, output_fields=[ct.default_float_field_name]
        )
        assert set(res[0].keys()) == {
            ct.default_int64_field_name,
            ct.default_float_field_name,
    ...
    def test_query_output_all_fields(self, enable_dynamic_field, random_primary_key):
        ...
        # 1. initialize with data
        collection_w, df, _, insert_ids = self.init_collection_general(
            prefix,
            True,
            nb=10,
        ...
        log.info(res)
        collection_w.load()
        actual_res, _ = collection_w.query(
            default_term_expr,
    ...
    def test_query_output_float_vec_field(self):
        ...
        res = df.loc[
            :1, [ct.default_int64_field_name, ct.default_float_vec_field_name]
        ].to_dict("records")
        collection_w.create_index(
            ct.default_float_vec_field_name, index_params=ct.default_flat_index
        )
    ...
    @pytest.mark.parametrize(
        ...
        [["*"], ["*", default_float_field_name], ["*", default_int_field_name]],
    )
    def test_query_output_field_wildcard(self, wildcard_output_fields):
        ...
        collection_w.insert(df)
        assert collection_w.num_entities == ct.default_nb
        output_fields = cf.get_wildcard_output_field_names(
            collection_w, wildcard_output_fields
        )
        ...
        actual_res = collection_w.query(
            default_term_expr, output_fields=wildcard_output_fields
        )[0]
    ...
    @pytest.mark.parametrize(
        "vec_fields", [[cf.gen_float_vec_field(name="float_vector1")]]
    )
    ...
    def test_query_output_binary_vec_field(self):
        ...
        """
        collection_w, vectors = self.init_collection_general(
            prefix, insert_data=True, is_binary=True
        )[0:2]
        fields = [
    ...
    def test_query_output_primary_field(self):
        ...
        res, _ = collection_w.query(
            default_term_expr, output_fields=[ct.default_int64_field_name]
        )
        assert res[0].keys() == {ct.default_int64_field_name}
    ...
    def test_query_invalid_output_fields(self):
        ...
        error = {
            ct.err_code: 0,
            ct.err_msg: "Invalid query format. 'output_fields' must be a list",
        }
        for fields in output_fields:
            collection_w.query(
                default_term_expr,
    ...
    def test_query_empty_partition(self):
        ...
        partition_w.load()
        res, _ = collection_w.query(
            default_term_expr, partition_names=[partition_w.name]
        )
        assert len(res) == 0
    ...
    def test_query_not_existed_partition(self):
        ...
        error = {
            ct.err_code: 65535,
            ct.err_msg: f"partition name {partition_names} not found",
        }
        collection_w.query(
            default_term_expr,
    ...
    def test_query_pagination(self, offset):
        ...
        pos = 10
        term_expr = (
            f"{ct.default_int64_field_name} in {int_values[offset: pos + offset]}"
        )
        res = vectors[0].iloc[offset : pos + offset, :1].to_dict("records")
        query_params = {"offset": offset, "limit": 10}
        query_res = collection_w.query(
            term_expr,
            params=query_params,
            check_task=CheckTasks.check_query_results,
            check_items={exp_res: res},
    ...
    def test_query_pagination_with_invalid_limit_type(self, limit):
        ...
        collection_w.query(
            ...
            limit=limit,
            check_task=CheckTasks.err_res,
            check_items={ct.err_code: 1, ct.err_msg: "limit [%s] is invalid" % limit},
        )
    ...
    def test_query_pagination_with_invalid_offset_type(self, offset):
        ...
        collection_w.query(
            ...
            limit=10,
            check_task=CheckTasks.err_res,
            check_items={ct.err_code: 1, ct.err_msg: "offset [%s] is invalid" % offset},
        )
    ...
    def test_mmap_query_expr_empty_pk_string(self):
        ...
        # 1. initialize with data
        collection_w, _, _, insert_ids = self.init_collection_general(
            prefix, True, is_index=False, primary_field=ct.default_string_field_name
        )[0:4]
    ...
    @pytest.mark.tags(CaseLabel.L1)
    def test_enable_mmap_query_with_expression(
        self, get_normal_expr, enable_dynamic_field
    ):
        """
        ...
        nb = 1000
        collection_w, _vectors, _, insert_ids = self.init_collection_general(
            prefix, True, nb, is_index=False, enable_dynamic_field=enable_dynamic_field
        )[0:4]
    ...
    def test_mmap_query_string_field_not_primary_is_empty(self):
        ...
        # 1.  create a collection
        collection_w, vectors = self.init_collection_general(
            prefix, insert_data=False, is_index=False
        )[0:2]
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize(
        "expression", cf.gen_normal_string_expressions([default_string_field_name])
    )
    def test_mmap_query_string_is_primary(self, expression):
        ...
        collection_w.load()
        res, _ = collection_w.query(
            expression, output_fields=[ct.default_string_field_name]
        )
    ...
    ******************************************************************
    ...
    def test_query_without_connection(self):
        ...
        self.connection_wrap.list_connections(
            check_task=ct.CheckTasks.ccr, check_items={ct.list_content: []}
        )
    ...
    def test_query_expr_single_term_array(self, term_expr):
        ...
        collection_w, vectors, binary_raw_vectors = self.init_collection_general(
    ...
    def test_query_to_get_latest_entity_with_dup_ids(self, with_growing):
        ...
        collection_w = self.init_collection_general(
            ...
            insert_data=False,
            is_index=False,
            vector_data_type=ct.float_type,
            with_json=False,
        )[0]
        nb = 50
        rounds = 10
        for i in range(rounds):
            df = cf.gen_default_dataframe_data(
                dim=16, nb=nb, start=i * nb, with_json=False
            )
        ...
        if not with_growing:
            collection_w.flush()
        collection_w.create_index(
            ct.default_float_vec_field_name, index_params=ct.default_index
        )
        collection_w.load()
    ...
    def test_query_after_search(self):
        ...
        vectors_s = [
            [random.random() for _ in range(ct.default_dim)]
            for _ in range(ct.default_nq)
        ]
        collection_w.search(
            vectors_s[: ct.default_nq],
            ct.default_float_vec_field_name,
    ...
    def test_query_output_binary_vec_field_after_index(self):
        ...
        """
        collection_w, vectors = self.init_collection_general(
            prefix, insert_data=True, is_binary=True, is_index=False
        )[0:2]
        ...
        collection_w.load()
        res, _ = collection_w.query(
            default_term_expr, output_fields=[ct.default_binary_vec_field_name]
        )
        assert res[0].keys() == set(fields)

    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize(
        "vector_data_type", ["FLOAT_VECTOR", "FLOAT16_VECTOR", "BFLOAT16_VECTOR"]
    )
    def test_query_output_all_vector_type(self, vector_data_type):
        ...
        """
        collection_w, vectors = self.init_collection_general(
            prefix, True, vector_data_type=vector_data_type
        )[0:2]
        fields = [ct.default_int64_field_name, ct.default_float_vec_field_name]
        res, _ = collection_w.query(
            default_term_expr, output_fields=[ct.default_float_vec_field_name]
        )
        assert res[0].keys() == set(fields)
    ...
    def test_query_partition_repeatedly(self):
        ...
        # query twice
        res_one, _ = collection_w.query(
            default_term_expr, partition_names=[partition_w.name]
        )
        res_two, _ = collection_w.query(
            default_term_expr, partition_names=[partition_w.name]
        )
    ...
    def test_query_multi_partitions_multi_results(self):
        ...
        res, _ = collection_w.query(
            term_expr, partition_names=[ct.default_partition_name, partition_w.name]
        )
        assert len(res) == 2
    ...
    def test_query_multi_logical_exprs(self):
        ...
        _, check_res = collection_w.query(
            multi_exprs, output_fields=[f"{default_int_field_name}"]
        )
...
class TestQueryString(TestcaseBase):
    ...
    ******************************************************************
    ...
    def test_query_string_expr_with_binary(self):
        ...
        assert collection_w.has_index()[0]
        res, _ = collection_w.query(
            default_string_term_expr, output_fields=[ct.default_binary_vec_field_name]
        )
        assert len(res) == 2
    ...
    def test_query_with_create_diskann_index(self):
        ...
        collection_w.create_index(
            ct.default_float_vec_field_name, ct.default_diskann_index
        )
        assert collection_w.has_index()[0]
...
class TestQueryCount(TestcaseBase):
    ...
    @pytest.mark.parametrize(
        "consistency_level",
        [CONSISTENCY_BOUNDED, CONSISTENCY_STRONG, CONSISTENCY_EVENTUALLY],
    )
    def test_count_consistency_level(self, consistency_level):
        ...
        """
        collection_w = self.init_collection_wrap(
            name=cf.gen_unique_str(prefix), consistency_level=consistency_level
        )
        # load collection
        collection_w.create_index(
            ct.default_float_vec_field_name, index_params=ct.default_flat_index
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize(
        "invalid_output_field", ["count", "count(int64)", "count(**)"]
    )
    ...
    def test_count_without_loading(self):
        ...
        collection_w.query(
            ...
            output_fields=[ct.default_count_output],
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "collection not loaded"},
        )
    ...
    def test_count_multi_partitions(self):
        ...
        half = ct.default_nb // 2
        # insert [0, half) into partition_w, [half, nb) into _default
        collection_w, p1, _, _ = self.insert_entities_into_two_partitions_in_half(
            half=half
        )
    ...
    def test_count_during_handoff(self):
        ...
        t_count = threading.Thread(
            target=collection_w.query,
            args=(default_expr,),
            kwargs={
                "output_fields": [ct.default_count_output],
                "check_task": CheckTasks.check_query_results,
    ...
    def test_count_delete_insert_duplicate_ids(self):
        ...
        collection_w.delete(
            f"{ct.default_int64_field_name} in {[i for i in range(ct.default_nb)]}"
        )
        collection_w.query(
            expr=default_expr,
            output_fields=[ct.default_count_output],
    ...
    def test_count_compact_delete(self):
        ...
        # create -> index -> insert
        collection_w = self.init_collection_wrap(
            cf.gen_unique_str(prefix), shards_num=1
        )
        collection_w.create_index(
            ct.default_float_vec_field_name, index_params=ct.default_flat_index
        )
        ...
        half_expr = (
            f"{ct.default_int64_field_name} in {[i for i in range(ct.default_nb // 2)]}"
        )
    ...
    def test_query_count_expr_json(self):
        ...
        # 1. initialize with data
        collection_w = self.init_collection_general(
            prefix, enable_dynamic_field=True, with_json=True
        )[0]
    ...
    def test_json_expr_on_search_n_query(self):
        ...
        fields = [
            ...
            cf.gen_json_field(json_int),
            cf.gen_json_field(json_float),
            cf.gen_json_field(json_string),
            cf.gen_json_field(json_bool),
            cf.gen_json_field(json_array),
            cf.gen_json_field(json_embedded_object),
            cf.gen_json_field(json_objects_array),
        ]
        schema = cf.gen_collection_schema(
            fields=fields, primary_field=ct.default_int64_field_name, auto_id=True
        )
        ...
        for i in range(10):
            data = [
                ...
                cf.gen_json_data_for_diff_json_types(
                    nb=nb, start=i * nb, json_type=json_int
                ),
                cf.gen_json_data_for_diff_json_types(
                    nb=nb, start=i * nb, json_type=json_float
                ),
                cf.gen_json_data_for_diff_json_types(
                    nb=nb, start=i * nb, json_type=json_string
                ),
                cf.gen_json_data_for_diff_json_types(
                    nb=nb, start=i * nb, json_type=json_bool
                ),
                cf.gen_json_data_for_diff_json_types(
                    nb=nb, start=i * nb, json_type=json_array
                ),
                cf.gen_json_data_for_diff_json_types(
        ...
        # 3. build index and load
        collection_w.create_index(
            ct.default_float_vec_field_name, index_params=default_index_params
        )
        collection_w.load()
    ...
    def test_count_with_pagination_param(self):
        ...
        collection_w.query(
            ...
            check_items={
                ct.err_code: 1,
                ct.err_msg: "count entities with pagination is not allowed",
            },
        )
    ...
    def test_count_alias_insert_delete_drop(self):
        ...
        collection_w_alias.insert(
            cf.gen_default_dataframe_data(start=ct.default_nb), partition_name=p_name
        )
        collection_w_alias.query(
            expr=default_expr,
            output_fields=[ct.default_count_output],
            check_task=CheckTasks.check_query_results,
            check_items={exp_res: [{count: ct.default_nb * 2}]},
        ...
        collection_w_alias.drop_partition(
            p_name,
            check_task=CheckTasks.err_res,
            check_items={
                ct.err_code: 65535,
    ...
    def test_count_bool_expressions(self, bool_type):
        ...
        # create -> insert -> index -> load
        collection_w, _vectors, _, insert_ids = self.init_collection_general(
            insert_data=True, is_all_data_type=True
        )[0:4]
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize(
        "expression", cf.gen_normal_expressions_field(default_float_field_name)
    )
    ...
    def test_counts_expression_sparse_vectors(self, index):
        ...
        collection_w.create_index(
            ct.default_sparse_vec_field_name, index_params, index_name=index
        )
        collection_w.load()
        collection_w.query(
            expr=default_expr,
        ...
        collection_w.query_iterator(
            ...
            check_items={"count": ct.default_nb, "batch_size": batch_size},
        )
...
class TestQueryIterator(TestcaseBase):
    ...
    ******************************************************************
    ...
    def test_query_iterator_normal(self):
        ...
        collection_w.create_index(
            ct.default_float_vec_field_name, {"metric_type": "L2"}
        )
        collection_w.load()
        # 2. search iterator
        expr = "int64 >= 0"
    ...
    def test_query_iterator_using_default_batch_size(self):
        ...
        collection_w.query_iterator(
            check_task=CheckTasks.check_query_iterator,
            check_items={"count": ct.default_nb, "batch_size": ct.default_batch_size},
        )
    ...
    def test_query_iterator_with_offset(self, offset):
        ...
        collection_w.query_iterator(
            ...
            offset=offset,
            check_task=CheckTasks.check_query_iterator,
            check_items={"count": ct.default_nb - offset, "batch_size": batch_size},
        )
    ...
    def test_query_iterator_with_different_limit(self, limit, offset):
        ...
        collection_w.query_iterator(
            ...
            offset=offset,
            check_task=CheckTasks.check_query_iterator,
            check_items={"count": max(Count, 0), "batch_size": ct.default_batch_size},
        )
    ...
    def test_query_iterator_invalid_batch_size(self):
        ...
        collection_w.query_iterator(
            batch_size=-1, expr=expr, check_task=CheckTasks.err_res, check_items=error
        )
    ...
    def test_query_iterator_expr_empty_with_random_pk_pagination(
        ...
        # 1. initialize with data
        collection_w, _, _, insert_ids = self.init_collection_general(
            prefix, True, random_primary_key=True
        )[0:4]
    ...
    ******************************************************************
    ...
    def test_query_text_match_with_combined_expression_for_multi_field(self):
        ...
        for i in range(2):
            query, text_match_expr, pandas_expr = (
                cf.generate_random_query_from_freq_dict_v1(
    ...
    def test_query_text_match_with_some_empty_string(self):
        ...
        for i in range(0, len(df), batch_size):
            ...
            collection_w.flush()
        collection_w.create_index("emb", {"index_type": "FLAT", "metric_type": "L2"})
        collection_w.load()
        # analyze the croup and get the tf-idf, then base on it to crate expr and ground truth
