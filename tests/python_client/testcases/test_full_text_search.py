    ...
    def test_search_for_full_text_search_with_invalid_search_data(
        ...
        if invalid_search_data == "sparse_vector":
            search_data = cf.gen_vectors(
                nb=nq, dim=1000, vector_data_type="SPARSE_FLOAT_VECTOR"
            )
        else:
            search_data = cf.gen_vectors(
                nb=nq, dim=1000, vector_data_type="FLOAT_VECTOR"
            )
        log.info(f"search data: {search_data}")
        error = {
            ct.err_code: 1100,
    ...
    ******************************************************************
    ...
    def test_hybrid_search_with_full_text_search(
        ...
        data = [
            {
                ...
                "neural_sparse_emb": cf.gen_vectors(
                    nb=1, dim=1000, vector_data_type="SPARSE_FLOAT_VECTOR"
                )[0],
            }
            for i in range(data_size)
        ]
        ...
        sparse_search = AnnSearchRequest(
            data=cf.gen_vectors(
                nb=nq, dim=dim, vector_data_type="SPARSE_FLOAT_VECTOR"
            ),
