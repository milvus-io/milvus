...
class TestIssues(TestcaseBase):
    ...
    def test_issue_30607(self, par_key_field, use_upsert):
        ...
        schema = cf.gen_collection_schema(
            fields=[pk_field, int64_field, string_field, vector_field], auto_id=False, partition_key_field=par_key_field
        )
        ...
        for index_on_par_key_field in [False, True]:
            ...
            for i in range(len(res)):
                ...
                res_parkey, _ = collection_w.query(
                    expr=f"{par_key_field}=={parkey_value} and pk=='{pk}'", output_fields=["pk", par_key_field]
                )
                if len(res_parkey) != 1:
