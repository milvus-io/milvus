...
class HelmEnv:
    ...
    def __init__(self, release_name=None, **kwargs):
        self.release_name = (
            release_name
            if release_name
            else cf.gen_unique_str(constants.DEFAULT_RELEASE_PREFIX)
        )
    ...
    def helm_install_cluster_milvus(self, image_pull_policy=constants.IF_NOT_PRESENT):
        ...
        """
        install_cmd = (
            f"helm install --wait --timeout 360s "
    ...
    def helm_upgrade_cluster_milvus(self, **kwargs):
        ...
        upgrade_cmd = (
            f"helm upgrade {self.release_name} . "
    ...
    def get_service_ip(self):
        ...
        service = v1.read_namespaced_service(
            f"{self.release_name}-milvus", constants.NAMESPACE
        )
    ...
    def export_all_logs(self):
        ...
        for pod in pods:
            os.system(
                f"kubectl logs {pod} > {constants.MILVUS_LOGS_PATH}/{pod}.log 2>&1"
            )
    ...
    def list_all_pods(self):
        ...
        ret = v1.list_namespaced_pod(
            namespace=constants.NAMESPACE, label_selector=label_selector
        )
        pods = []
        for i in ret.items:
