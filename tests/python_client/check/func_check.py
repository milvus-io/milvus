...
class ResponseChecker:
    ...
    def check_collection_property(res, func_name, check_items):
        ...
        if check_items.get("num_shards", None):
            assert collection.num_shards == check_items.get("num_shards")
        return True
    ...
    def check_search_results(search_res, func_name, check_items):
        ...
        for hits in search_res:
            ...
            if (len(hits) != check_items["limit"]) \
                ...
                assert len(ids) == check_items["limit"], f"expect {check_items['limit']}, but got {len(ids)}"
            else:
                if check_items.get("ids", None) is not None:
                    ids_match = pc.list_contain_check(list(ids),
