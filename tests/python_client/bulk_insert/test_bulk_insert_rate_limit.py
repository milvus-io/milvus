...
class TestChaos(TestChaosBase):
    ...
    def test_bulk_insert_rate_limit(self, file_type, create_index, is_loaded, nb, dim, use_same_collection):
        ...
        if create_index == "create_index":
            ...
            create_index = False
        is_loaded = True if is_loaded == "loaded" else False
        if not create_index and is_loaded:
            raise Exception("can not load data without creating index")
        if use_same_collection == "true":
            collection_name = cf.gen_unique_str("Checker_")
        else:
            collection_name = None
        self.init_health_checkers(collection_name=collection_name, create_index=create_index, dim=int(dim))
        if nb == "None":
            nb = 3000
            if file_type == "json":
                nb = 13800
