...
def phase_report(phase, outcome):
    ...
    return TestReport(
        ...
        when=phase,
        duration={"setup": 0.1, "call": 0.2, "teardown": 0.4}[phase],
    )
...
def test_integrity_report_waits_for_teardown_without_changing_ordinary_cases(
    ...
    expected_duration = (
        sum(duration for duration, outcome in zip((0.1, 0.2, 0.4), outcomes) if outcome is not None) if marked else 0.2
    )
    ...
    assert persisted["duration"] == pytest.approx(expected_duration)
    if marked:
