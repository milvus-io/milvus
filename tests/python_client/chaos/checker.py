...
def assert_statistic(checkers, expectations={}):
    for k in checkers.keys():
        ...
        # rsp_times = checkers[k].rsp_times
        ...
        # checkers_result = f"{str(k)} succ_rate: {succ_rate:.4f}, total: {total}, average_time: {average_time:.4f}, \
        #                     max_time: {max_time:.4f}, min_time: {min_time:.4f}"
        checkers_result = k.check_result()

        if expectations.get(k, '') == constants.FAIL:
            log.info(f"Expect Fail: {str(k)} {checkers_result}")
            expect(succ_rate < 0.49 or total < 2,
                   f"Expect Fail: {str(k)} {checkers_result}")
        else:
            log.info(f"Expect Succ: {str(k)} {checkers_result}")
            expect(succ_rate > 0.90 or total > 2,
                   f"Expect Succ: {str(k)} {checkers_result}")
