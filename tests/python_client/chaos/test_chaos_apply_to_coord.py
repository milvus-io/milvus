...
class TestChaosApply:
    ...
    def test_chaos_apply(self, chaos_type, role_type, target_component, chaos_duration, chaos_interval):
        ...
        activate_pod_list, standby_pod_list = find_activate_standby_coord_pod(
            self.milvus_ns, self.release_name, target_component
        )
        ...
        time.sleep(30)
        activate_pod_list_after_chaos, standby_pod_list_after_chaos = find_activate_standby_coord_pod(
            self.milvus_ns, self.release_name, target_component
        )
        log.info(
            f"activated pod list: {activate_pod_list_after_chaos}, standby pod list: {standby_pod_list_after_chaos}"
        )
        if role_type == "standby":
