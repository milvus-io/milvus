...
def gen_data_by_data_field(
    ...
    use_utf8_data = kwargs.get("use_utf8_data", False)
    ...
    if rows > 0:
        if "vec" in data_field:
            ...
            if not nullable:
                if use_utf8_data:
                    data = [f"{i}_{gen_utf8_string(i)}" for i in range(start, rows + start)]
                else:
                    data = [gen_unique_str(str(i)) for i in range(start, rows + start)]
            else:
            ...
            if not nullable:
                if use_utf8_data:
                    data = [f"{gen_utf8_string(i)} milvus UTF-8 text" for i in range(start, rows + start)]
                else:
                    data = [fake.text() + " milvus " for i in range(start, rows + start)]
            else:
                if use_utf8_data:
                    data = [
                        None if random.random() < 0.5 else f"{gen_utf8_string(i)} milvus UTF-8 text"
                        for i in range(start, rows + start)
                    ]
                else:
                    data = [
                        None if random.random() < 0.5 else fake.text() + " milvus " for _ in range(start, rows + start)
                    ]
                ...
                if use_utf8_data:
                    data = pd.Series(
                        [
                            json.dumps(
                                {
                                    gen_utf8_string(i): random.randint(-999999, 9999999),
                                    "name": True,
                                    "address": gen_utf8_string(i + 1),
                                    "number": i,
                                    "tags": [gen_utf8_string(i + 2), gen_utf8_string(i + 3)],
                                },
                                ensure_ascii=False,
                            )
                            for i in range(start, rows + start)
            ...
            if not nullable:
                if use_utf8_data:
                    utf8_array_length = max(1, array_length)
                    data = pd.Series(
                        [
                            np.array(
                                [f"{i}_{gen_utf8_string(i + j)}" for j in range(utf8_array_length)],
                                dtype=np.dtype("str"),
                            )
                            for i in range(start, rows + start)
                        ]
