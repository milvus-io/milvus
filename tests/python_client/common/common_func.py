...
from common.common_params import Expr, ExprCheckParams
...
def gen_geometry_field(name="geo", description=ct.default_desc, is_primary=False, **kwargs):  # noqa: F811
    return gen_scalar_field(DataType.GEOMETRY, name=name, description=description, is_primary=is_primary, **kwargs)
...
def prepare_bulk_insert_data(
    ...
    _ = get_dim_by_schema(schema=schema)
...
def get_hybrid_search_base_results(search_res_dict_array, weights, metric_types, round_decimal=-1):
    ...
    for i, result in enumerate(search_res_dict_array, 0):
        ...
        for key, distance in result.items():
            ...
            weight = weights[i]
            search_res_dict_merge[key] = search_res_dict_merge.get(key, 0) + activate_distance * weight
