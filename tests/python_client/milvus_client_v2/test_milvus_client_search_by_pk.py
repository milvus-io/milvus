...
class TestSearchByPkIndependent(TestMilvusClientV2Base):
    ...
    def test_search_by_pk_after_partition_release(self):
        ...
        search_res1, _ = self.search(
            ...
            limit=limit,
            check_task=CheckTasks.check_search_results,
            check_items={"enable_milvus_client_api": True, "nq": len(ids_to_search), "pk_name": "id", "limit": limit},
        )
    ...
    def test_search_by_pk_repeatedly_with_different_index(self, index):
        ...
        ids_to_search = [i for i in range(ct.default_nq)]
        res1 = self.search(
            client, collection_name, ids=ids_to_search, anns_field="vector", search_params=search_params, limit=limit
        )[0]
        res2 = self.search(
            client,
            collection_name,
