...
class TestSearchInvalidShared(TestMilvusClientV2Base):
    ...
    def test_search_param_missing(self):
        ...
        self.search(
            ...
            check_items={"err_code": 1, "err_msg": "Either ids or data must be provided"},
        )
    ...
    def test_search_param_invalid_dim(self):
        ...
        self.search(
            client,
            self.collection_name,
            data=wrong_vectors[:default_nq],
            anns_field=default_search_field,
            search_params=default_search_params,
            limit=default_limit,
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "vector dimension mismatch"},
        )
    ...
    def test_search_param_invalid_metric_type(self, invalid_metric):
        ...
        if isinstance(invalid_metric, dict):
            self.search(
                ...
                filter=default_search_exp,
                check_task=CheckTasks.err_res,
                check_items={"err_code": 1, "err_msg": "Dict key must be str"},
            )
        else:
            self.search(
                client,
                ...
                filter=default_search_exp,
                check_task=CheckTasks.err_res,
                check_items={"err_code": 65535, "err_msg": "metric type not match"},
            )
    ...
    def test_search_param_metric_type_not_match(self):
        ...
        self.search(
            ...
            check_items={
                "err_code": 65535,
                "err_msg": "metric type not match: invalid parameter[expected=COSINE][actual=L2]",
            },
        )

    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("invalid_limit", [p for p in ct.get_invalid_ints if not (isinstance(p, int) and p >= 0)])
    ...
    @pytest.mark.parametrize(
        "expression", [f"{ct.default_int64_field_name} like 33", f"{ct.default_float_field_name} LIKE 33"]
    )
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize(
        "non_existing_output_fields", [["non_existing"], [ct.default_int64_field_name, "non_existing"]]
    )
    ...
    def test_search_output_field_invalid_wildcard(self, output_fields):
        ...
        self.search(
            ...
            output_fields=output_fields,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": f"field {output_fields[-1]} not exist"},
        )

    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize(
        "invalid_guarantee_time", [p for p in ct.get_invalid_ints if p != 9999999999 and p is not None]
    )
    ...
    def test_search_invalid_round_decimal(self, round_decimal):
        ...
        self.search(
            ...
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": f"`round_decimal` value {round_decimal} is illegal"},
        )
    ...
    def test_search_filter_division_by_zero_with_expr_params(self, expr, expr_params):
        ...
        client = self._client(alias=self.shared_alias)
        log.info(
            f"test_search_filter_division_by_zero_with_expr_params: searching with expr: {expr}, params: {expr_params}"
        )
        self.search(
            client,
            self.collection_name,
    ...
    ******************************************************************
    ...
    def test_search_no_connection(self):
        ...
        self.search(
            ...
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": "should create connection first"},
        )
        ...
        for invalid_search_param in invalid_search_params:
            if index == invalid_search_param["index_type"]:
                ...
                self.search(
                    ...
                    filter=default_search_exp,
                    check_task=CheckTasks.err_res,
                    check_items={"err_code": 999, "err_msg": "fail to search on QueryNode"},
                )
    ...
    def test_search_param_invalid_annoy_index(self, search_k):
        ...
        idx = self.prepare_index_params(client)[0]
        idx.add_index(
            field_name=ct.default_float_vec_field_name, index_type="ANNOY", metric_type="L2", params={"n_trees": 512}
        )
        self.create_index(client, collection_name, index_params=idx)
        self.load_collection(client, collection_name)
        ...
        self.search(
            ...
            data=vectors[:default_nq],
            anns_field=default_search_field,
            search_params=annoy_search_param,
            limit=default_limit,
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": "Search params check failed"},
        )
    ...
    def test_search_with_expression_join_two_fields(self, expression):
        ...
        # 2. insert data
        data = [
            {"int64_1": i, "int64_2": i, ct.default_float_vec_field_name: [random.random() for _ in range(dim)]}
            for i in range(nb)
        ]
        self.insert(client, collection_name, data=data)
        self.flush(client, collection_name)
    ...
    def test_search_param_invalid_expr_bool(self, invalid_expr_bool_value):
        ...
        self.search(
            ...
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": "failed to create query plan"},
        )
    ...
    def test_search_with_empty_collection(self, vector_data_type):
        ...
        self.search(
            ...
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 101, "err_msg": err_msg},
        )

        # 3. search collection without data after load
        if vector_data_type == DataType.INT8_VECTOR:
            idx = self.prepare_index_params(client)[0]
            idx.add_index(field_name=ct.default_float_vec_field_name, index_type="HNSW", metric_type="L2")
            self.create_index(client, collection_name, index_params=idx)
        else:
            idx = self.prepare_index_params(client)[0]
    ...
    def test_search_partition_deleted(self):
        ...
        self.search(
            ...
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "partition name search_partition_0 not found"},
        )
    ...
    def test_search_scann_with_invalid_reorder_k(self, reorder_k):
        ...
        idx = self.prepare_index_params(client)[0]
        idx.add_index(
            field_name=ct.default_float_vec_field_name, index_type="SCANN", metric_type="L2", params={"nlist": 1024}
        )
        self.create_index(client, collection_name, index_params=idx)
        self.load_collection(client, collection_name)
        ...
        self.search(
            ...
            limit=reorder_k + 1,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "reorder_k(100) should be larger than k(101)"},
        )
    ...
    def test_search_param_invalid_binary(self):
        ...
        self.search(
            ...
            search_params=wrong_search_params,
            limit=default_limit,
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "metric type not match"},
    ...
    def test_search_binary_flat_with_L2(self):
        ...
        self.search(
            ...
            check_items={
                "err_code": 65535,
                "err_msg": "metric type not match: invalid parameter[expected=JACCARD][actual=L2]",
            },
        )
    ...
    def test_search_with_output_fields_not_exist(self, output_fields, enable_dynamic):
        ...
        if enable_dynamic and output_fields == "int63":
            ...
            self.search(
                ...
                output_fields=[output_fields],
                check_task=CheckTasks.err_res,
                check_items={ct.err_code: 65535, ct.err_msg: "field int63 not exist"},
            )
    ...
    def test_search_output_field_vector_after_gpu_index(self, index):
        ...
        self.search(
            ...
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": "not supported"},
        )
    ...
    def test_search_invalid_ignore_growing_param(self, ignore_growing):
        ...
        self.search(
            client,
            collection_name,
            data=vector[:default_nq],
            anns_field=default_search_field,
            search_params=search_params,
            limit=default_limit,
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 999, "err_msg": "parse ignore growing field failed"},
        )
    ...
    def test_range_search_invalid_radius_range_filter_L2(self):
        ...
        # 2. create index
        idx = self.prepare_index_params(client)[0]
        idx.add_index(field_name=ct.default_float_vec_field_name, index_type="FLAT", metric_type="L2")
        self.create_index(client, collection_name, index_params=idx)
        # 3. load
        self.load_collection(client, collection_name)
        ...
        self.search(
            ...
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "must be less than radius"},
        )
    ...
    def test_range_search_invalid_radius_range_filter_IP(self):
        ...
        # 2. create index
        idx = self.prepare_index_params(client)[0]
        idx.add_index(field_name=ct.default_float_vec_field_name, index_type="FLAT", metric_type="IP")
        self.create_index(client, collection_name, index_params=idx)
        # 3. load
        self.load_collection(client, collection_name)
        ...
        self.search(
            ...
            filter=default_search_exp,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 65535, "err_msg": "must be greater than radius"},
        )
    ...
    def test_search_dynamic_compare_two_fields(self):
        ...
        self.search(
            ...
            filter=expr,
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1100, "err_msg": "error: two column comparison with JSON type is not supported"},
        )
