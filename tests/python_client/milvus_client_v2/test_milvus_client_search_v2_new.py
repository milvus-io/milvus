...
class TestSearchV2Independent(TestMilvusClientV2Base):
    ...
    def test_search_with_duplicate_primary_key(self):
        ...
        data = [
            {
                ct.default_primary_key_field_name: i if i % 2 == 0 else i + 1,
                ct.default_vector_field_name: all_vectors[i],
            }
            for i in range(default_nb)
        ]
        self.insert(client, collection_name, data)
    ...
    def test_search_after_release_load(self, flush):
        ...
        data = [
            {ct.default_primary_key_field_name: i, ct.default_vector_field_name: all_vectors[i]}
            for i in range(default_nb)
        ]
        self.insert(client, collection_name, data)
        if flush:
    ...
    def test_search_after_partition_release(self):
        ...
        for j in range(insert_times):
            ...
            for i in range(ct.default_nb):
                ...
                row = {ct.default_primary_key_field_name: pk, ct.default_vector_field_name: float_vectors[pk]}
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize(
        "index", [i for i in ct.all_dense_float_index_types if i != "DISKANN"]
