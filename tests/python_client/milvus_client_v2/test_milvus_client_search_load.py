...
class TestSearchLoadIndependent(TestMilvusClientV2Base):
    ...
    def _create_collection_with_partitions_and_data(
        self, client, nb=200, partition_num=1, is_index=False, dim=default_dim
    ):
        """
        ...
        if is_index:
            ...
            idx.add_index(
                field_name=ct.default_float_vec_field_name, metric_type="COSINE", index_type="FLAT", params={}
            )
    ...
    def _do_search(self, client, collection_name, partition_names, limit, expected):
        ...
        if expected == NOT_LOADED:
            self.search(
                client,
                collection_name,
                data=cf.gen_vectors(1, default_dim),
                anns_field=field_name,
                search_params=default_search_params,
                limit=limit,
                partition_names=partition_names,
                check_task=CheckTasks.err_res,
                check_items={ct.err_code: 1, ct.err_msg: "not loaded", "enable_milvus_client_api": True},
            )
            ...
            self.search(
                ...
                partition_names=partition_names,
                check_task=CheckTasks.err_res,
                check_items={ct.err_code: 1, ct.err_msg: "not found", "enable_milvus_client_api": True},
            )
            ...
            self.search(
                ...
                partition_names=partition_names,
                check_task=CheckTasks.err_res,
                check_items={ct.err_code: 65535, ct.err_msg: "collection not loaded", "enable_milvus_client_api": True},
            )
        else:
    ...
    @pytest.mark.parametrize(
        "ops,expected_coll,expected_p1,expected_p2,coll_search_parts",
        [
    ...
    def test_search_after_delete_ops_l1(self, ops, expected_coll, expected_p1, expected_p2, coll_search_parts):
        """Parametrized L1 tests: delete + load/release operation sequences."""
        client = self._client()
        collection_name, partition_names, _ = self._create_collection_with_partitions_and_data(
            client, nb=200, partition_num=1, is_index=False
        )
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_after_delete_ops_l2(self, ops, expected_coll, expected_p1, expected_p2, coll_search_parts):
    ...
    def test_load_partition_delete_release_collection(self):
        ...
        self.query(
            ...
            check_items={"exp_res": [{ct.default_count_output: 50}], "enable_milvus_client_api": True},
        )
        self.query(
            client,
            collection_name,
    ...
    def _setup_compact_test(self, client):
        ...
        collection_name, partition_names, _ = self._create_collection_with_partitions_and_data(
            client, nb=0, partition_num=1, is_index=True
        )
        p1_name, p2_name = partition_names[0], partition_names[1]
        self.release_collection(client, collection_name)
    ...
    def test_search_after_compact_ops_l1(self, ops, expected_coll, expected_p1, expected_p2, coll_search_parts):
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_after_compact_ops_l2(self, ops, expected_coll, expected_p1, expected_p2, coll_search_parts):
    ...
    def test_search_after_flush_ops_l1(self, ops, expected_coll, expected_p1, expected_p2, coll_search_parts):
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_search_after_flush_ops_l2(self, ops, expected_coll, expected_p1, expected_p2, coll_search_parts):
    ...
    def test_load_release_collection_multi_times(self):
        ...
        for _ in range(5):
            self.release_collection(client, collection_name)
            self.load_partitions(client, collection_name, partition_names=[p2_name])
        self._run_search_assertions(client, collection_name, p1_name, p2_name, 200, 100, NOT_LOADED, 100)
    ...
    def test_search_load_collection_create_partition(self):
        ...
        """
        client = self._client()
        collection_name, _, _ = self._create_collection_with_partitions_and_data(
            client, nb=200, partition_num=1, is_index=False
        )
        self._create_index_flat(client, collection_name)
        self.load_collection(client, collection_name)
