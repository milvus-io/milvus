...
# Re-exported imports (random/time/np/DataType/MilvusClient/log/preprocessing)
# are kept because existing tests rely on them being available after `import *`.
__all__ = [
    # re-exported imports
    "random",
    "time",
    ...
    # functions / classes
    "get_milvus",
    "reset_build_index_threshold",
    "disable_flush",
    "enable_flush",
    "gen_inaccuracy",
    "gen_vectors",
    "gen_binary_vectors",
    "gen_binary_sub_vectors",
    "gen_binary_super_vectors",
    "gen_int_attr",
    "gen_float_attr",
    "gen_unique_str",
    "gen_primary_field",
    "gen_single_filter_fields",
    ...
    "gen_entities_rows",
    "gen_binary_entities",
    "gen_binary_entities_new",
    "gen_binary_entities_rows",
    "gen_entities_by_fields",
    "assert_equal_entity",
    "gen_search_vectors_params",
    "update_query_expr",
    "gen_default_vector_expr",
    ...
    "update_field_type_row",
    "gen_segment_row_limits",
    "gen_invalid_ips",
    "gen_invalid_uris",
    "gen_invalid_strs",
    "gen_invalid_field_types",
    "gen_invalid_metric_types",
    "gen_invalid_ints",
    "gen_invalid_params",
    "gen_invalid_vectors",
    "gen_invalid_index",
...
def gen_entities_rows(nb, is_normal=False, _id=True):
    ...
    if not _id:
        for i in range(nb):
            entity = {"_id": i, "int64": i, "float": float(i), default_float_vec_field_name: vectors[i]}
            entities.append(entity)
    else:
        for i in range(nb):
            entity = {"int64": i, "float": float(i), default_float_vec_field_name: vectors[i]}
            entities.append(entity)
    return entities
...
def gen_binary_entities_rows(nb, _id=True):
    ...
    if not _id:
        for i in range(nb):
            entity = {"_id": i, "int64": i, "float": float(i), default_binary_vec_field_name: vectors[i]}
            entities.append(entity)
    else:
        for i in range(nb):
            entity = {"int64": i, "float": float(i), default_binary_vec_field_name: vectors[i]}
            entities.append(entity)
    return raw_vectors, entities
...
def add_field(entities, field_name=None):
    ...
    if field_name is None:
        field_name = gen_unique_str()
    field = {"name": field_name, "type": DataType.INT64, "values": [i for i in range(nb)]}
...
def gen_index():
    ...
    for index_type in all_index_types:
        if index_type in ["FLAT", "BIN_FLAT", "BIN_IVF_FLAT"]:
            ...
            ivf_params = [{"index_type": index_type, "index_param": {"nlist": nlist}} for nlist in nlists]
            index_params.extend(ivf_params)
        elif index_type == "IVF_PQ":
            IVFPQ_params = [
                {"index_type": index_type, "index_param": {"nlist": nlist, "m": m}} for nlist in nlists for m in pq_ms
            ]
            index_params.extend(IVFPQ_params)
        elif index_type in ["HNSW"]:
            hnsw_params = [
                {"index_type": index_type, "index_param": {"M": M, "efConstruction": efConstruction}}
                for M in Ms
