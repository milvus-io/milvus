...
class TestMilvusClientV2Base(Base):
    ...
    @trace()
    def create_database(
        self, client, db_name, properties: Optional[dict] = None, check_task=None, check_items=None, **kwargs
    ):
        func_name = sys._getframe().f_code.co_name
    ...
    def drop_snapshot(self, client, snapshot_name, timeout=None, check_task=None, check_items=None, **kwargs):
        ...
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, check, snapshot_name=snapshot_name, **kwargs
        ).run()
        return res, check_result
