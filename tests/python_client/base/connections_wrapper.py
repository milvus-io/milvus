...
class ApiConnectionsWrapper:
    ...
    def add_connection(self, check_task=None, check_items=None, **kwargs):
        ...
        check_result = ResponseChecker(
            response, func_name, check_task, check_items, is_succ, **kwargs
        ).run()
    ...
    def disconnect(self, alias, check_task=None, check_items=None):
        ...
        check_result = ResponseChecker(
            response, func_name, check_task, check_items, is_succ, alias=alias
        ).run()
        return response, check_result
    ...
    def connect(
        ...
        **kwargs
        ...
        check_result = ResponseChecker(
            response, func_name, check_task, check_items, succ, alias=alias, **kwargs
        ).run()
        return response, check_result

    def get_connection(
        self, alias=DefaultConfig.DEFAULT_USING, check_task=None, check_items=None
    ):
        func_name = sys._getframe().f_code.co_name
    ...
    def list_connections(self, check_task=None, check_items=None):
        ...
        check_result = ResponseChecker(
            response, func_name, check_task, check_items, is_succ
        ).run()
        return response, check_result
