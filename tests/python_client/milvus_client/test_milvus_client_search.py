...
class TestMilvusClientSearchInvalid(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    def test_milvus_client_search_null_expr_json_key(self, nullable,  null_expr_op, supported_varchar_scalar_index,
                                                     supported_json_cast_type, is_flush):
        """
        ...
        if null_expr_op in ["is null", "IS NULL"]:
            limit = default_limit
        else:
            limit = 0
        ...
        self.drop_index(client, collection_name, index_name=default_vector_field_name)
        index_params = self.prepare_index_params(client)[0]
        index_params.add_index(default_vector_field_name, metric_type="COSINE")
        index_params.add_index(field_name=nullable_field_name, index_type=supported_varchar_scalar_index,
                               params={"json_cast_type": supported_json_cast_type, "json_path": f"{nullable_field_name}['a']['b']"})
        index_params.add_index(field_name=nullable_field_name,
                               index_type=supported_varchar_scalar_index,
                               params={"json_cast_type": supported_json_cast_type,
                                       "json_path": f"{nullable_field_name}['a']"})
        index_params.add_index(field_name=nullable_field_name,
                               index_type=supported_varchar_scalar_index,
                               params={"json_cast_type": supported_json_cast_type,
                                       "json_path": f"{nullable_field_name}"})
        index_params.add_index(field_name=nullable_field_name,
                               index_type=supported_varchar_scalar_index,
                               params={"json_cast_type": supported_json_cast_type,
                                       "json_path": f"{nullable_field_name}['b']"})
        index_params.add_index(field_name=nullable_field_name,
                               index_type=supported_varchar_scalar_index,
                               params={"json_cast_type": supported_json_cast_type,
                                       "json_path": f"{nullable_field_name}['a'][0]['b']"})
        index_params.add_index(field_name=nullable_field_name,
                               index_type=supported_varchar_scalar_index,
                               params={"json_cast_type": supported_json_cast_type,
                                       "json_path": f"{nullable_field_name}['a'][0]"})
        if is_flush:
