...
# Existing, non-nullable ordinary fields with written values and no index/function dependencies.
COMPACTION_INTEGRITY_V2_DDL_DROP_CANDIDATES = (
    "int8_value",
    "int16_value",
    "int32_value",
    "int64_value",
...
def _compaction_integrity_float32(logical_pk, explicit_test_ts, field_id, element_index=0):
    # Adjacent elements stay distinct after FLOAT encoding within this bounded workload.
    value = logical_pk * 256 + explicit_test_ts * 8 + field_id + element_index
    assert 0 <= value < 2**24, f"FLOAT fingerprint exceeds exact integer range: {value}"
    return float(value)


def _validate_compaction_integrity_float32_range(max_pk, max_test_ts):
    # Cover scalar IDs, sparse indices and all nested vector element offsets.
    max_field_id = max(COMPACTION_INTEGRITY_STRUCT_SUBFIELD_IDS.values())
    max_element_index = max(127, COMPACTION_INTEGRITY_STRUCT_ARRAY_MAX_CAPACITY * COMPACTION_INTEGRITY_VECTOR_DIM - 1)
    _compaction_integrity_float32(max_pk, max_test_ts, max_field_id, max_element_index)
    ...
    for row_offset, actual in enumerate(batch):
        ...
        for field_name in output_fields:
            ...
            try:
                ...
                # Malformed returned cells are corruption evidence, not a reason
                # to stop checking the remaining fields, rows, or batches.
                record_issue(
                    row_offset,
                    "canonical_encoding_error",
                    physical_pk=physical_pk,
                    field_name=field_name,
                    actual_type=type(actual[field_name]).__name__,
...
def _snapshot_compaction_integrity_tasks(tasks):
    return {
        task.task_id: {
            ...
            "type": task.compaction_type.name,
            # Keep the checkpoint's existing state names at the SDK enum boundary.
            "state": "meta_saved" if task.state.name == "MetaSaved" else task.state.name.lower(),
...
def _select_compaction_integrity_v2_ddl_drop_field(schema_fields, output_fields, seed):
    candidates = [
        name for name in COMPACTION_INTEGRITY_V2_DDL_DROP_CANDIDATES if name in schema_fields and name in output_fields
    ]
    assert candidates, "V2 DDL requires an existing data-bearing ordinary field to drop"
    # A local RNG keeps unrelated test data generation unchanged.
    return random.Random(seed).choice(candidates)


def _assert_compaction_integrity_v2_ddl_checkpoint(checkpoint, before_checkpoint=None):
    assert checkpoint["storage_versions"] == {2}, "V2 DDL checkpoint contains non-V2 active segments"
    bump_tasks = {
        task_id: task
        for task_id, task in checkpoint["tasks"].items()
        if task["type"] == COMPACTION_INTEGRITY_BUMP_TASK_TYPE
    }
    assert not bump_tasks, f"schema-bump tasks observed with bump disabled: {bump_tasks}"
    if before_checkpoint is None:
        return set()
    edges = _assert_compaction_integrity_graph_transition(before_checkpoint, checkpoint, require_new_inputs=False)
    unchanged_sources = set(before_checkpoint["active"]).intersection(checkpoint["active"])
    assert not unchanged_sources, f"V2 DDL did not rewrite all pre-DDL active segments: {unchanged_sources}"
    mix_tasks = _compaction_integrity_successful_new_tasks(before_checkpoint, checkpoint, task_types={"MixCompaction"})
    assert any(_compaction_integrity_task_reaches_active_frontier(task, checkpoint) for task in mix_tasks.values()), (
        "V2 DDL has no successful new MixCompaction reaching the active frontier"
    )
    return edges
...
def _assert_compaction_integrity_bm25(
    ...
    for field_name in anns_fields:
        for batch_start in range(0, len(token_items), COMPACTION_INTEGRITY_BM25_QUERY_BATCH_SIZE):
            ...
            for offset, (token, expected_pk) in enumerate(batch_items):
                ...
                if not score_finite:
                    record_failure("non_finite_score", field=field_name, token=token, score=str(score))
                ...
                if baseline_by_token is not None:
                    ...
                    if baseline is None:
                        ...
                        baseline_finite = math.isfinite(baseline_score)
                        if not baseline_finite:
                            record_failure(
                                "non_finite_baseline", field=field_name, token=token, score=str(baseline_score)
                            )
                        ...
                        if score_finite and baseline_finite and abs(score - baseline_score) >= 1e-6:
    ...
    for field_name in anns_fields[1:]:
        for token in token_to_pk:
            ...
            if (
                math.isfinite(actual_score)
                and math.isfinite(control_score)
                and abs(actual_score - control_score) >= 1e-6
            ):
                record_score_mismatch(
...
def _assert_compaction_integrity_fenced_dataset(
    ...
    while time.time() < deadline:
        ...
        except (Exception, pytest.fail.Exception) as error:  # Retry only if the frontier changed.
        ...
        try:
            ...
            _log_compaction_integrity_evidence(
                "fenced_validation_checkpoint_failed",
                collection=collection_name,
                attempt=attempt,
                ...
                checkpoint_error=repr(checkpoint_error),
                # No post-checkpoint means the result cannot be attributed to
                # a stable frontier, even if Retrieve itself succeeded.
                frontier_verified=False,
            )
            if validation_error is not None:
                raise validation_error from checkpoint_error
            raise
...
@pytest.mark.tags("CompactionIntegrityUnit")
...
def test_compaction_integrity_empty_round_start_is_a_stable_checkpoint(monkeypatch):
    ...
    class EmptyClient:
        ...
        def list_loaded_segments(self, collection_name):
            return ()
...
class TestMilvusClientCompactionDataIntegrity(TestMilvusClientV2Base):
    ...
    def compaction_integrity_storage_config(
        ...
        with MilvusEtcdConfigController(
            ...
            with self._preserve_compaction_integrity_storage_config(
                ...
                observed = config_controller.read_config(COMPACTION_INTEGRITY_STORAGE_VERSION_CONFIG)
                assert observed.value == configured.value, (
                    f"failed to set {COMPACTION_INTEGRITY_STORAGE_VERSION_CONFIG}: got {observed.value!r}"
                )
                assert observed.mod_revision == configured.mod_revision, (
                    f"concurrent update detected for {COMPACTION_INTEGRITY_STORAGE_VERSION_CONFIG}: "
    ...
    def _create_compaction_integrity_collection(
        ...
        if include_text:
            deadline = time.monotonic() + COMPACTION_INTEGRITY_V3_ADOPTION_TIMEOUT
            attempts = 0
            while True:
                ...
                if remaining <= 0:
                    raise AssertionError(
                        ...
                        f"after {attempts} create attempts"
                    )
                attempts += 1
                response, created = self.create_collection(
                    client,
                    collection_name,
                    schema=schema,
                    consistency_level="Strong",
                ...
                if created:
                    _log_compaction_integrity_evidence(
                        "workload_storage_proxy_adopted",
                        collection=collection_name,
                        attempts=attempts,
                        expected_storage_version=3,
                    )
                    break
                if response.code != 1100 or "TEXT field requires StorageV3" not in response.message:
                    raise AssertionError(f"failed to create {collection_name}: {response}")
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    raise AssertionError(
                        f"Proxy did not adopt StorageV3 within {COMPACTION_INTEGRITY_V3_ADOPTION_TIMEOUT}s "
                        f"after {attempts} create attempts: {response}"
                    )
    ...
    def _run_compaction_integrity_rounds(
        ...
        if ingress_type == "import":
            max_test_ts = round_count
            rows_per_batch = COMPACTION_INTEGRITY_IMPORT_ROWS_PER_ROUND
        else:
            max_test_ts = round_count * COMPACTION_INTEGRITY_INSERT_BATCHES_PER_ROUND
            rows_per_batch = COMPACTION_INTEGRITY_INSERT_ROWS_PER_BATCH
        _validate_compaction_integrity_float32_range((max_test_ts + 1) * rows_per_batch - 1, max_test_ts)
        expected_by_pk = {}
        empty_checkpoint = {
            "all": {},
            "active": {},
    ...
    def _ingest_ddl_compaction_integrity_dataset(
        ...
        _validate_compaction_integrity_float32_range(
            (COMPACTION_INTEGRITY_DDL_BATCHES + 1) * COMPACTION_INTEGRITY_DDL_ROWS_PER_BATCH - 1,
            COMPACTION_INTEGRITY_DDL_BATCHES,
        )
        expected_by_pk = {}
        token_to_pk = {}
        for batch_index in range(COMPACTION_INTEGRITY_DDL_BATCHES):
            ...
            for logical_pk in range(pk_start, pk_start + COMPACTION_INTEGRITY_DDL_ROWS_PER_BATCH):
                row, expected = _build_compaction_integrity_row(
                    ...
                    include_bm25_control=include_bm25_control,
                    include_struct_array=False,
                )
                physical_pk = row["id"]
                rows.append(row)
                ...
                if include_bm25_control:
                    batch_tokens[_compaction_integrity_bm25_token(logical_pk)] = physical_pk
    ...
    def _wait_for_ddl_schema_transition(
        ...
        return _wait_for_compaction_integrity_checkpoint(
            ...
            required_task_types=(
                COMPACTION_INTEGRITY_SCHEMA_REWRITE_TASK_TYPES if required_task_types is None else required_task_types
            ),
            expected_storage_version=expected_storage_version,
            timeout=timeout,
        )
    ...
    def test_v2_add_drop_field_mix_compaction_preserves_all_rows_and_fields(
        self,
        compaction_integrity_storage_config,
    ):
        """
        target: verify V2 add/drop field reconciliation through MixCompaction with schema bump disabled
        method: persist one dataset, add X, drop existing Y, then drop X, compacting and checking after each DDL
        expected: C0-C3 serve V2 data, every DDL has Mix lineage, and all retained cells including X at C2 match
        """
        client = self._client()
        collection_name = cf.gen_unique_str("v2_ddl_mix_compaction_integrity")
        primary_key_type = DataType.VARCHAR
        assert compaction_integrity_storage_config["storage_version"] == 2
        config_controller = compaction_integrity_storage_config["controller"]
        config_key = COMPACTION_INTEGRITY_BUMP_SCHEMA_VERSION_CONFIG
        original_config = config_controller.read_config(config_key)
        with self._preserve_compaction_integrity_runtime_config(
            config_controller, config_key, original_config, evidence_prefix="v2_ddl_bump_schema"
        ):
            configured = config_controller.set_config(config_key, "false", settle_after_write=True)
            observed = config_controller.read_config(config_key)
            assert observed.value == b"false" and observed.mod_revision == configured.mod_revision
            _log_compaction_integrity_evidence(
                "v2_ddl_bump_schema_config_verified",
                collection=collection_name,
                config_key=config_key,
                value=observed.value.decode(),
                mod_revision=observed.mod_revision,
            ...
            try:
                ...
                created = True
                expected_by_pk, _ = self._ingest_ddl_compaction_integrity_dataset(
                    client,
                    collection_name,
                    output_fields,
                    primary_key_type,
                ...
                empty_checkpoint = {"all": {}, "active": {}, "serving": {}, "tasks": {}, "storage_versions": set()}
                _wait_for_compaction_integrity_checkpoint(
                    client,
                    collection_name,
                    expected_rows=expected_rows,
                    before_checkpoint=empty_checkpoint,
                ...
                schema_snapshot = _compaction_integrity_schema_snapshot(client, collection_name)
                _assert_compaction_integrity_v2_ddl_checkpoint(checkpoint)
                field_name = COMPACTION_INTEGRITY_ADDED_DEFAULT_FIELD
                drop_seed = os.getenv("MILVUS_COMPACTION_INTEGRITY_DDL_DROP_SEED", collection_name)
                existing_field_name = _select_compaction_integrity_v2_ddl_drop_field(
                    schema_snapshot["fields"], output_fields, drop_seed
                )
                dropped_fields = []
                _log_compaction_integrity_checkpoint(
                    ...
                    selected_existing_field=existing_field_name,
                    drop_seed=drop_seed,
                )

                def verify_dropped_fields():
                    # Dynamic fields are enabled, so an absent name may legally
                    # resolve to an empty dynamic key rather than raise an error.
                    for dropped_field in dropped_fields:
                        result = client.query(
                            collection_name,
                            filter=f"exists {dropped_field}",
                            output_fields=["count(*)"],
                            consistency_level="Strong",
                        )
                        assert result == [{"count(*)": 0}], (
                            f"dropped field {dropped_field} still has readable values: {result}"
                        )

                for stage, changed_field in (
                    ("C1_add_field", field_name),
                    ("C2_drop_existing_field", existing_field_name),
                    ("C3_drop_added_field", field_name),
                ):
                    before_checkpoint = checkpoint
                    before_schema_version = schema_snapshot["schema_version"]
                    if stage == "C1_add_field":
                        ...
                        default_bytes = _canonical_compaction_integrity_cell(
                            field_name, COMPACTION_INTEGRITY_ADDED_DEFAULT_VALUE, primary_key_type
                        )
                        for expected in expected_by_pk.values():
                            expected[field_name] = default_bytes
                    else:
                        self.drop_collection_field(client, collection_name, field_name=changed_field)
                        dropped_fields.append(changed_field)
                        output_fields = [name for name in output_fields if name != changed_field]
                        for expected in expected_by_pk.values():
                            expected.pop(changed_field)
                    schema_snapshot = _compaction_integrity_schema_snapshot(client, collection_name)
                    assert schema_snapshot["schema_version"] > before_schema_version
                    assert (field_name in schema_snapshot["fields"]) == (stage != "C3_drop_added_field")
                    assert (existing_field_name in schema_snapshot["fields"]) == (stage == "C1_add_field")
                    assert not set(dropped_fields).intersection(schema_snapshot["fields"])
                    # The job is diagnostic: an automatic Mix may perform the rewrite first.
                    ...
                    self._wait_for_ddl_schema_transition(
                        ...
                        before_checkpoint,
                        transition_policy="lineage",
                        expected_schema_version=schema_snapshot["schema_version"],
                        expected_storage_version=2,
                    ...
                    checkpoint, _, _ = _assert_compaction_integrity_fenced_dataset(
                        ...
                        output_fields,
                        primary_key_type,
                        additional_validator=verify_dropped_fields if dropped_fields else None,
                        expected_storage_version=2,
                        timeout=600,
                    )
                    edges = _assert_compaction_integrity_v2_ddl_checkpoint(checkpoint, before_checkpoint)
                    observed = config_controller.read_config(config_key)
                    assert observed.value == b"false" and observed.mod_revision == configured.mod_revision
                    _log_compaction_integrity_checkpoint(
                        stage,
                        ...
                        rows=expected_rows,
                        ordinary_field_count=len(output_fields),
                        expected_cell_count=expected_rows * len(output_fields),
                        expected_pk_digest=_compaction_integrity_pk_digest(expected_by_pk, primary_key_type),
        ...
        with self._preserve_compaction_integrity_runtime_config(
            ...
            try:
                ...
                def verify_dropped_index():
                    assert dropped_field not in client.list_indexes(collection_name)
    ...
    def test_compaction_storage_version_transitions_preserve_all_rows_and_fields(
        ...
        method: atomically switch the storage config and observe both storage-version lineage rewrites on one collection
        expected: both transitions hand off a complete serving set with every PK, field, null/default, and cell byte intact
        """
        max_test_ts = round_count * COMPACTION_INTEGRITY_INSERT_BATCHES_PER_ROUND
        _validate_compaction_integrity_float32_range(
            (max_test_ts + 1) * COMPACTION_INTEGRITY_INSERT_ROWS_PER_BATCH - 1, max_test_ts
        )
        client = self._client()
        ...
        with MilvusEtcdConfigController(
            ...
            with self._preserve_compaction_integrity_storage_config(
                ...
                try:
                    v2_config = config_controller.set_config(
                        COMPACTION_INTEGRITY_STORAGE_VERSION_CONFIG,
                        "false",
                        settle_after_write=True,
                    )
                    ...
                    v3_config = config_controller.set_config(
                        COMPACTION_INTEGRITY_STORAGE_VERSION_CONFIG,
                        "true",
                        settle_after_write=True,
                    )
