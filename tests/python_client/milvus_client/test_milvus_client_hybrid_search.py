...
# Shared collection for TestMilvusClientHybridSearchInvalid — unique suffix per run to avoid parallel-run conflicts
INVALID_COLLECTION_NAME = "test_hybrid_search_invalid_" + cf.gen_unique_str("_")


@pytest.mark.xdist_group("TestMilvusClientHybridSearchInvalid")
class TestMilvusClientHybridSearchInvalid(TestMilvusClientV2Base):
    ...
    def prepare_invalid_collection(self, request):
        """Create one minimal collection shared across all invalid-input tests.
        Tests only need a valid collection to target; they assert on client/server
        validation errors (limit, output_fields, partition, vector field name),
        not on data content — so no inserts or data-dependent state is needed."""
        client = self._client()
        ...
        def teardown():
            try:
                if self.has_collection(client, INVALID_COLLECTION_NAME):
                    self.drop_collection(client, INVALID_COLLECTION_NAME)
            except Exception:
                pass
    ...
    ******************************************************************
    ...
    def test_milvus_client_hybrid_search_invalid_collection_name_string(self, name):
        ...
        method: hybrid search with invalid collection name — no collection needed
    ...
    def test_milvus_client_hybrid_search_invalid_limit(self, invalid_limit):
        ...
        method: hybrid search on shared collection with invalid limit
        expected: Raise exception
        """
        client = self._client()
        rng = np.random.default_rng(seed=19530)
        ...
        self.hybrid_search(client, INVALID_COLLECTION_NAME, [sub_search1, sub_search1], ranker, limit=invalid_limit,
                    check_task=CheckTasks.err_res, check_items=error)
    ...
    def test_milvus_client_hybrid_search_invalid_output_fields(self, invalid_output_fields):
        ...
        method: hybrid search on shared collection with invalid output_fields
        expected: Raise exception
        """
        client = self._client()
        rng = np.random.default_rng(seed=19530)
    ...
    def test_milvus_client_hybrid_search_not_exist_partition_names(self, invalid_partition_names):
        ...
        method: hybrid search on shared collection with not exist partition names
        expected: Raise exception
        """
        client = self._client()
        rng = np.random.default_rng(seed=19530)
    ...
    def test_milvus_client_hybrid_search_not_exist_vector_name(self):
        """
        target: test hybrid search with not exist vector field name
        method: hybrid search on shared collection using a vector field that doesn't exist
        expected: Raise exception with fieldName not found error
        """
        client = self._client()
        rng = np.random.default_rng(seed=19530)
        vectors_to_search = rng.random((1, 8))
