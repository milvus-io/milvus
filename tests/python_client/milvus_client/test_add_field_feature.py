...
class TestMilvusClientAddFieldFeature(TestMilvusClientV2Base):
    ...
    def test_milvus_client_collection_add_field(self):
        ...
        schema.add_field(
            "id_string", DataType.VARCHAR, max_length=64, is_primary=True, auto_id=False
        )
        schema.add_field("embeddings", DataType.FLOAT_VECTOR, dim=dim)
        schema.add_field(
            "title", DataType.VARCHAR, max_length=64, is_partition_key=True
        )
        schema.add_field(
            "nullable_field", DataType.INT64, nullable=True, default_value=10
        )
        schema.add_field(
            "array_field",
            DataType.ARRAY,
    ...
    @pytest.mark.parametrize(
        "index_type",
        ["HNSW", "IVF_FLAT", "IVF_SQ8", "IVF_RABITQ", "AUTOINDEX", "DISKANN"],
    )
    def test_milvus_client_add_vector_field(self, index_type):
        ...
        index_params.add_index(
            vec_field_name, index_type=index_type, metric_type="COSINE"
        )
        ...
        rows = [
            {
                ...
                vec_field_name: cf.gen_vectors(
                    1, dim, vector_data_type=DataType.FLOAT_VECTOR
                )[0],
        ...
        vectors_to_search = cf.gen_vectors(
            ct.default_nq, dim, vector_data_type=DataType.FLOAT_VECTOR
        )
        self.search(
            client,
            collection_name,
        ...
        basic_rows_with_null_vector = [
            row for row in basic_rows if row[pk_name] in query_pks
        ]
        ...
        self.load_collection(
            client, collection_name, check_task=CheckTasks.err_res, check_items=error
        )
        # create index and reload the collection
        index_params = self.prepare_index_params(client)[0]
        index_params.add_index(
            new_vec_field_name, index_type=index_type, metric_type="COSINE"
        )
        ...
        rows_without_nullable_vector = cf.gen_row_data_by_schema(
            nb=ct.default_nb, schema=schema, start=ct.default_nb
        )
    ...
    def test_milvus_client_add_vector_field_build_index_before_insert(self, index_type):
        ...
        rows = cf.gen_row_data_by_schema(
            nb=ct.default_nb, schema=new_collection_info, start=ct.default_nb
        )
        self.insert(client, collection_name, rows)
        # 4. add one more vector field and index data
        ...
        index_params = self.prepare_index_params(client)[0]
        index_params.add_index(
            new_vec_field_name_2, index_type=index_type, metric_type="COSINE"
        )
        self.create_index(client, collection_name, index_params)
        self.wait_for_index_ready(client, collection_name, new_vec_field_name_2)
        new_collection_info = self.describe_collection(client, collection_name)[0]
        rows = cf.gen_row_data_by_schema(
            nb=ct.default_nb, schema=new_collection_info, start=ct.default_nb * 2
        )
        self.insert(client, collection_name, rows)
        # 6. load collection and search on the new added vector field
        self.load_collection(client, collection_name)
    ...
    def test_milvus_client_compact_with_added_field(self):
        ...
        vectors = cf.gen_vectors(
            default_nb, dim, vector_data_type=DataType.FLOAT_VECTOR
        )
    ...
    def test_milvus_client_insert_with_old_and_added_field(self):
        ...
        vectors = cf.gen_vectors(
            default_nb * 3, dim, vector_data_type=DataType.FLOAT_VECTOR
        )
        rows = [
            {
                default_primary_key_field_name: i,
    ...
    def test_milvus_client_upsert_with_added_field(self):
        ...
        # 1. create collection
        self.create_collection(
            client, collection_name, default_dim, consistency_level="Strong"
        )
        collections = self.list_collections(client)[0]
        assert collection_name in collections
        self.describe_collection(
        ...
        # 2. insert before add field
        vectors = cf.gen_vectors(
            default_nb * 3, default_dim, vector_data_type=DataType.FLOAT_VECTOR
        )
        rows = [
            {
                default_primary_key_field_name: i,
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize(
        "new_field_name", [default_dynamic_field_name, "new_field"]
    )
    def test_milvus_client_search_query_enable_dynamic_and_add_field(
        self, new_field_name
    ):
        """
        target: test search (high level api) normal case
    ...
    def test_milvus_client_add_field_and_update_existing_data(self):
        ...
        vectors = cf.gen_vectors(
            nb_entries, dim, vector_data_type=DataType.FLOAT_VECTOR
        )
        rows = [
            {
                default_primary_key_field_name: i,
    ...
    def test_milvus_client_add_vector_field_all_types(
        self, vector_type, index_type, metric_type
    ):
        """
        ...
        # 3. add new nullable vector field of the parametrized type
        add_kwargs = dict(
            field_name=new_vec_field, data_type=vector_type, nullable=True
        )
        if not is_sparse:
        ...
        all_base_vecs = cf.gen_vectors(
            half_nb, base_dim, vector_data_type=DataType.FLOAT_VECTOR
        )
        new_rows = [
            {
                pk_name: i + half_nb,
        ...
        search_vecs = cf.gen_vectors(
            ct.default_nq, search_dim, vector_data_type=vector_type
        )
        ...
        new_index_params.add_index(
            new_vec_field, index_type=index_type, metric_type=metric_type
        )
        ...
        if not is_sparse:
            ...
            assert all(len(r) == 0 for r in res), (
                "null vector must return empty search result"
            )
            res = self.search(
                client,
                collection_name,
            ...
            assert all(len(r) > 0 for r in res), (
                "non-null vector must return non-empty search result"
            )
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize(
        "insert_order", ["insert_before_index", "index_before_insert"]
    )
    @pytest.mark.parametrize(
        "vector_type,index_type,metric_type",
        [
    ...
    def test_milvus_client_add_vector_field_index_insert_order(
        ...
        method:
            ...
            index_before_insert — add_field → create_index →
        ...
        def gen_new_rows(start_pk, nb):
            ...
            base_vecs = cf.gen_vectors(
                nb, base_dim, vector_data_type=DataType.FLOAT_VECTOR
            )
            return [
                {
                    pk_name: start_pk + i,
        ...
        base_vecs = cf.gen_vectors(
            ct.default_nq, base_dim, vector_data_type=DataType.FLOAT_VECTOR
        )
        self.search(
            client,
            collection_name,
...
class TestMilvusClientAddFieldFeatureInvalid(TestMilvusClientV2Base):
    ...
    def test_milvus_client_collection_add_field_as_auto_id(self):
        ...
        error = {
            ct.err_code: 1,
            ct.err_msg: "The auto_id can only be specified on the primary key field",
        }
        self.create_collection(client, collection_name, dim)
        collections = self.list_collections(client)[0]
        assert collection_name in collections
    ...
    def test_milvus_client_collection_add_field_with_disable_nullable(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"added field must be nullable, please check it, "
    ...
    def test_milvus_client_collection_add_field_as_partition_ley(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"not support to add partition key field, "
            f"field name  = {field_name}: invalid parameter",
        }
        self.create_collection(client, collection_name, dim)
        collections = self.list_collections(client)[0]
    ...
    def test_milvus_client_collection_add_field_as_cluster_key(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"already has another clutering key field, "
            f"field name: {field_name}: invalid parameter",
        }
        ...
        schema.add_field(
            default_vector_field_name, DataType.FLOAT_VECTOR, dim=default_dim
        )
        schema.add_field(
            default_string_field_name,
            DataType.VARCHAR,
    ...
    def test_milvus_client_collection_add_field_same_other_name(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"duplicate field name: {default_string_field_name}: invalid parameter",
        }
        schema = self.create_schema(client)[0]
        schema.add_field(
            default_primary_key_field_name,
    ...
    def test_milvus_client_collection_add_field_exceed_max_field_number(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: "The number of fields has reached the maximum value 64: "
            "invalid parameter",
        }
        self.create_collection(client, collection_name, dim)
        collections = self.list_collections(client)[0]
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_milvus_client_collection_add_vector_field_exceed_max_vector_field_number(
        self,
    ):
        """
    ...
    def test_milvus_client_add_field_with_reranker_unsupported(self):
        ...
        expected: raise exception because decay ranker requires non-nullable fields but add_collection_field
                  only supports nullable fields
        note: on master, the Go-side validation was removed (PR #47919). This 3.0 test retains the
              negative assertion until the corresponding code fix is cherry-picked to 3.0.
        """
        client = self._client()
        collection_name = cf.gen_collection_name_by_testcase_name()
        dim = 8
