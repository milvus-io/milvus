...
class TestMilvusClientAddFieldFeature(TestMilvusClientV2Base):
    ...
    def test_milvus_client_add_nullable_vector_field_hybrid_search(self):
        ...
        old_rows = [{default_primary_key_field_name: i, base_vec_field: base_vecs[i]} for i in range(default_nb)]
        self.insert(client, collection_name, old_rows)

        self.add_collection_field(
            client, collection_name, field_name=new_vec_field, data_type=DataType.FLOAT_VECTOR, dim=dim, nullable=True
        )
...
class TestMilvusClientAddFieldFeatureInvalid(TestMilvusClientV2Base):
    ...
    def test_milvus_client_collection_add_vector_field_nullable_false(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"not support to add vector field, field name = {field_name}: invalid parameter",
        }
    ...
    def test_milvus_client_collection_add_field_as_cluster_key(self):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"already has another clutering key field, field name: {field_name}: invalid parameter",
        }
    ...
    def test_milvus_client_add_field_with_reranker_unsupported(self):
        ...
        self.search(
            client, collection_name, [vectors[0]], ranker=my_rerank_fn, check_task=CheckTasks.err_res, check_items=error
        )
