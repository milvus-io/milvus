...
class TestMilvusClientQueryInvalid(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    def test_milvus_client_query_expr_none(self, filter):
        ...
        self.query(client, collection_name, filter=filter, check_task=CheckTasks.err_res, check_items=error)
        # 4. query with offset but no limit
        self.query(client, collection_name, filter=filter, offset=1, check_task=CheckTasks.err_res, check_items=error)
        self.drop_collection(client, collection_name)
    ...
    def test_milvus_client_query_expr_inconsistent_mix_term_array(self):
        ...
        # 1. create collection
        self.create_collection(
            client, collection_name, default_dim, consistency_level="Strong", enable_dynamic_field=False, auto_id=False
        )
        # 2. insert data
        schema_info = self.describe_collection(client, collection_name)[0]
        rows = cf.gen_row_data_by_schema(nb=default_nb, schema=schema_info)
    ...
    def test_milvus_client_query_expr_with_limit_offset_out_of_range(self):
        ...
        self.query(
            client, collection_name, filter="", limit=1, offset=16384, check_task=CheckTasks.err_res, check_items=error
        )
        self.query(
            client, collection_name, filter="", limit=16384, offset=1, check_task=CheckTasks.err_res, check_items=error
        )
        ...
        self.query(
            client, collection_name, filter="", limit=2, offset=-1, check_task=CheckTasks.err_res, check_items=error
        )
        # 7. clean up
        self.drop_collection(client, collection_name)
...
class TestMilvusClientQueryValid(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    def test_milvus_client_query_expr_not_in_term(self, field):
        ...
        values = [float(row[field]) if field == ct.default_float_field_name else row[field] for row in rows]
        pos = 100
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    @pytest.mark.parametrize("expr_prefix", ["json_contains", "JSON_CONTAINS", "array_contains", "ARRAY_CONTAINS"])
    ...
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    @pytest.mark.parametrize(
        "expr_prefix", ["json_contains_all", "JSON_CONTAINS_ALL", "array_contains_all", "ARRAY_CONTAINS_ALL"]
    )
    ...
    def test_milvus_client_query_expr_out_of_range(self, expression):
        ...
        for i in range(default_nb):
            row = {
                ...
                ct.default_int32_field_name: ((start + i) * 2200000) % int32_bound,
                ct.default_int16_field_name: ((start + i) * 40) % int16_bound,
        ...
        for i in range(default_nb):
            ...
            int16 = ((start + i) * 40) % int16_bound
            int32 = ((start + i) * 2200000) % int32_bound
            _ = int8, int16, int32
    ...
    def test_milvus_client_mmap_query_string_expr_with_prefixes_and_suffix(self, vachar_expression):
        ...
        normal_res = self.query(
            client,
            collection_name,
            filter=vachar_expression,
            check_task=CheckTasks.check_query_results,
...
class TestQueryString(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize(
        "expression", ['TEXT_MATCH(varchar, "0")', 'TEXT_MATCH(varchar, "1")', 'TEXT_MATCH(varchar, "test")']
    )
...
class TestQueryArray(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    @pytest.mark.parametrize(
        "array_expression", ['string_array[0] like "0%"', 'string_array[0] like "%0"', 'string_array[0] like "%0%"']
    )
...
class TestQueryCount(TestMilvusClientV2Base):
    ...
    def test_milvus_client_count_invalid_output_field(self, invalid_output_field):
        ...
        # 1. create collection
        self.create_collection(
            client, collection_name, default_dim, consistency_level="Strong", auto_id=False, enable_dynamic_field=False
        )
        self.release_collection(client, collection_name)
        self.drop_index(client, collection_name, default_vector_field_name)
        # 2. create index and load collection
        ...
        self.query(
            ...
            check_task=CheckTasks.err_res,
            check_items={"err_code": 1, "err_msg": f"field {invalid_output_field} not exist"},
        )
        self.drop_collection(client, collection_name)
