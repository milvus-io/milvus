...
class TestMilvusClientQueryOrderValid(TestMilvusClientV2Base):
    ...
    def prepare_valid_collection(self, request):
        ...
        for i in range(3):
            ...
            if odd_data:
                client.insert(collection_name=collection_name, data=odd_data,
        ...
        if odd_growing:
            client.insert(collection_name=collection_name, data=odd_growing,
                          partition_name=CUSTOM_PARTITION_NAME)

        # Wait until all 3200 rows are visible
        expected = 3 * default_batch + default_growing
        for _ in range(30):
            count = client.query(collection_name, filter="id >= 0",
                                 output_fields=["count(*)"],
    ...
    def test_query_order_by_single_field_desc(self):
        ...
        for i in range(len(prices) - 1):
            assert prices[i] >= prices[i + 1], \
                f"price not descending at index {i}: {prices[i]} < {prices[i + 1]}"
        ...
        for i in range(len(res) - 1):
            ...
            if v1 == v2:
                assert res[i]["rating"] >= res[i + 1]["rating"], \
                    f"rating not descending within same int32 at index {i}"
                secondary_sort_tested = True
        assert secondary_sort_tested, \
            "No duplicate int32_field values found — secondary sort not tested"
    ...
    def test_query_order_by_different_limit(self, limit):
        ...
        assert prices[0] == PRICE_MIN, \
            f"Expected min price {PRICE_MIN}, got {prices[0]}"
    ...
    def test_query_order_by_raw_string_format(self):
        ...
        assert len(res) == 20
        for i in range(len(res) - 1):
            assert res[i]["price"] <= res[i + 1]["price"], \
                f"price not ascending at index {i}"
    ...
    def test_query_order_by_pagination(self):
        ...
        page1 = self.query(client, VALID_COLLECTION_NAME,
                           filter="",
                           output_fields=["id", "price"],
                           limit=10, offset=0,
                           order_by_fields=order_by,
                           consistency_level="Strong")[0]
        page2 = self.query(client, VALID_COLLECTION_NAME,
                           filter="",
                           output_fields=["id", "price"],
                           limit=10, offset=10,
                           order_by_fields=order_by,
        ...
        offset = 0
        while True:
            page = self.query(client, VALID_COLLECTION_NAME,
                              filter="",
                              output_fields=["id", "price"],
            ...
            if offset > default_nb + 1000:
                break
        assert len(all_ids) == default_nb, \
            f"Expected {default_nb} total rows, got {len(all_ids)}"
    ...
    def test_query_order_by_all_null_values(self):
        ...
        for i in range(len(ids) - 1):
            assert ids[i] < ids[i + 1], \
                f"id not ascending among NULL rows at index {i}: {ids[i]} >= {ids[i + 1]}"
    ...
    def test_query_order_by_sort_field_not_in_output(self):
        ...
        ids = [r["id"] for r in res]
        verify = self.query(client, VALID_COLLECTION_NAME,
                            filter=f"id in {ids}",
                            output_fields=["id", "price"],
                            consistency_level="Strong")[0]
    ...
    def test_query_order_by_within_partition(self):
        ...
        for r in res:
            assert r["id"] % 2 == 1, \
                f"Expected odd id from {CUSTOM_PARTITION_NAME}, got {r['id']}"
        prices = [r["price"] for r in res]
        for i in range(len(prices) - 1):
            assert prices[i] <= prices[i + 1], \
                f"price not ascending at index {i}: {prices[i]} > {prices[i + 1]}"
    ...
    def test_query_order_by_multiple_sealed_segments(self):
        ...
        for i in range(len(prices) - 1):
            assert prices[i] <= prices[i + 1], \
                f"Cross-segment sort broken at index {i}: {prices[i]} > {prices[i + 1]}"
    ...
    def test_query_order_by_sealed_and_growing_mixed(self):
        ...
        for i in range(len(prices) - 1):
            assert prices[i] <= prices[i + 1], \
                f"Sealed+growing sort broken at index {i}: {prices[i]} > {prices[i + 1]}"
        # Verify growing segment data (id >= 3000) is included
        ids = {r["id"] for r in res}
        growing_ids = set(range(3 * default_batch, 3 * default_batch + default_growing))
        assert growing_ids.issubset(ids), \
            f"Growing segment data missing: {growing_ids - ids}"
    ...
    def test_query_order_by_indexed_vs_non_indexed(self):
        ...
        res_indexed = self.query(client, VALID_COLLECTION_NAME,
                                 filter="",
                                 output_fields=["id", "int32_field"],
                                 limit=100,
                                 order_by_fields=[{"field": "int32_field", "order": "asc"}],
                                 consistency_level="Strong")[0]
        # price has no index
        res_no_index = self.query(client, VALID_COLLECTION_NAME,
                                  filter="",
                                  output_fields=["id", "price"],
                                  limit=100,
                                  order_by_fields=[{"field": "price", "order": "asc"}],
        ...
        for i in range(len(int32_vals) - 1):
            assert int32_vals[i] <= int32_vals[i + 1], \
                f"Indexed int32_field not ascending at index {i}"

        prices = [r["price"] for r in res_no_index]
        for i in range(len(prices) - 1):
            assert prices[i] <= prices[i + 1], \
                f"Non-indexed price not ascending at index {i}"
    ...
    def test_query_order_by_after_upsert(self):
        ...
        try:
            ...
            for i in range(len(prices) - 1):
                assert prices[i] >= prices[i + 1], \
                    f"price not descending at index {i}"
            # Upserted rows (id 0~9) should be at the top with highest prices
            top_ids = {r["id"] for r in res[:10]}
            assert top_ids == set(range(10)), \
                f"Upserted rows not at top: expected {{0..9}}, got {top_ids}"
            for r in res[:10]:
                assert r["price"] == 999.0 + r["id"], \
                    f"Upserted row {r['id']} has wrong price: {r['price']}"
        finally:
            if client.has_collection(collection_name):
                client.drop_collection(collection_name)
    ...
    def test_query_order_by_json_field(self):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "order_by field 'json_field' has type JSON which is not sortable"}
        self.query(client, VALID_COLLECTION_NAME,
                   filter="",
                   output_fields=["id"],
                   limit=10,
    ...
    def test_query_order_by_nonexistent_field(self):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "order_by field 'no_such_field' does not exist in collection schema"}
        self.query(client, VALID_COLLECTION_NAME,
                   filter="",
                   output_fields=["id"],
                   limit=10,
    ...
    def test_query_order_by_missing_limit(self):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "ORDER BY requires explicit limit"}
        self.query(client, VALID_COLLECTION_NAME,
                   filter="",
                   output_fields=["id"],
                   order_by_fields=[{"field": "price", "order": "asc"}],
    ...
    def test_query_order_by_array_field(self):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "order_by field 'array_field' has type Array which is not sortable"}
        self.query(client, VALID_COLLECTION_NAME,
                   filter="",
                   output_fields=["id"],
                   limit=10,
    ...
    def test_query_order_by_three_sort_fields(self):
        ...
        for i in range(len(res) - 1):
            ...
            if a["int32_field"] < b["int32_field"]:
                continue
            assert a["int32_field"] == b["int32_field"], \
                f"int32_field not ascending at index {i}"
            # Same int32_field, check price
            second_level_tested = True
            if a["price"] < b["price"]:
                continue
            assert a["price"] == b["price"], \
                f"price not ascending at index {i} within same int32"
            # Same int32_field + price, check rating
            third_level_tested = True
            assert a["rating"] >= b["rating"], \
                f"rating not descending at index {i} within same int32+price"
        assert second_level_tested, \
            "No duplicate int32_field values found — second level sort not tested"
        assert third_level_tested, \
            "No duplicate int32_field+price values found — third level sort not tested"
        ...
        for attempt in range(10):
            ...
            if first_ids is None:
                ...
                assert ids == first_ids, \
                    f"Result changed on attempt {attempt + 1}"

    # ==================== 5.13 L2: Feature Interaction ====================

    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("agg_field", [
        "count(*)",
        "sum(price)",
        "avg(price)",
        "min(price)",
    ...
    def test_query_order_by_with_aggregation(self, agg_field):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "ORDER BY can only reference GROUP BY columns"}
        self.query(client, VALID_COLLECTION_NAME,
                   filter="id < 200",
    ...
    def test_query_order_by_dynamic_field(self):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "order_by field 'dyn_age' does not exist in collection schema"}
        self.query(client, VALID_COLLECTION_NAME,
                   filter="",
    ...
    def test_query_order_by_with_complex_filter(self):
        ...
        assert len(res) > 0
        for r in res:
            assert r["price"] in [10.0, 20.0, 30.0], \
                f"price {r['price']} not in filter list"
            assert r["category"].startswith("elec"), \
                f"category '{r['category']}' does not match LIKE 'elec%'"
        prices = [r["price"] for r in res]
        for i in range(len(prices) - 1):
            assert prices[i] >= prices[i + 1], \
                f"price not descending at index {i}: {prices[i]} < {prices[i + 1]}"
    ...
    def test_query_order_by_with_iterator(self):
        ...
        client = self._client()
        error = {ct.err_code: 1100,
                 ct.err_msg: "ORDER BY with iterator is not supported"}
        self.query_iterator(client, VALID_COLLECTION_NAME,
                            filter="",
                            output_fields=["id", "price"],
                            batch_size=10, limit=50,
                            order_by_fields=[{"field": "price", "order": "asc"}],
                            check_task=CheckTasks.err_res,
                            check_items=error)
