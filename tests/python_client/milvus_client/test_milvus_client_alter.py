...
class TestMilvusClientAlterCollection(TestMilvusClientV2Base):
    ...
    def test_milvus_client_alter_collection_default(self):
        ...
        self.alter_collection_properties(
            client,
            collection_name,
            properties={"lazyload.enabled": True},
            check_task=CheckTasks.err_res,
            check_items=error,
        )
        ...
        error = {ct.err_code: 999, ct.err_msg: "can not delete lazyload.enabled properties if collection loaded"}
        self.drop_collection_properties(
            client,
            collection_name,
            property_keys=["lazyload.enabled"],
            check_task=CheckTasks.err_res,
            check_items=error,
        )
        # TODO
        ...
        self.alter_collection_properties(
            client, collection_name, properties={"collection.ttl.seconds": 100, "lazyload.enabled": True}
        )
        res2 = self.describe_collection(client, collection_name)[0]
        assert {
            "mmap.enabled": "True",
            "collection.ttl.seconds": "100",
            "lazyload.enabled": "True",
        }.items() <= res2.get("properties", {}).items()
        self.drop_collection_properties(
            client, collection_name, property_keys=["mmap.enabled", "lazyload.enabled", "collection.ttl.seconds"]
        )
        self.describe_collection(client, collection_name)[0]
        assert len(res1.get("properties", {})) == 1
