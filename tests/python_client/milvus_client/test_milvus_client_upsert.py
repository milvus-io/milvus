...
class TestMilvusClientUpsertValid(TestMilvusClientV2Base):
    ...
    def gen_default_rows_data_for_upsert(self, nb=ct.default_nb, dim=ct.default_dim, start=0, size=10000,
                                         primary_field=ct.default_int64_field_name, auto_id=False,
                                         is_binary=False, with_json=False):
        """
        Generate row data (list of dicts) for upsert operations using MilvusClient API.
        """
        float_values = [np.float32(i + size) for i in range(start, start + nb)]
        string_values = [str(i + size) for i in range(start, start + nb)]
        json_values = [{"number": i, "string": str(i)} for i in range(start, start + nb)]
        if is_binary:
            _, vec_values = gen_binary_vectors(nb, dim)
            vec_field_name = ct.default_binary_vec_field_name
        else:
            vec_values = gen_vectors(nb, dim)
            vec_field_name = ct.default_float_vec_field_name
        rows = []
        for i in range(nb):
            row = {}
            if not auto_id:
                if primary_field == ct.default_int64_field_name:
                    row[primary_field] = start + i
                else:
                    row[primary_field] = str(start + i)
            row[ct.default_float_field_name] = float_values[i]
            if primary_field != ct.default_string_field_name:
                row[ct.default_string_field_name] = string_values[i]
            if with_json:
                row[ct.default_json_field_name] = json_values[i]
            row[vec_field_name] = vec_values[i]
            rows.append(row)
        return rows, float_values, vec_values
    ...
    ******************************************************************
    ...
    def test_milvus_client_upsert_data_pk_exist(self, start):
        ...
        # 2. Upsert data whose pk exists
        upsert_rows, float_values, _ = self.gen_default_rows_data_for_upsert(upsert_nb, start=start, with_json=True)
    ...
    def test_milvus_client_upsert_with_auto_id(self):
        ...
        upsert_rows1 = self.gen_default_rows_data_for_upsert(nb=nb, start=start, with_json=True)[0]
        ...
        upsert_rows2 = self.gen_default_rows_data_for_upsert(nb=nb, start=start, with_json=True)[0]
    ...
    def test_milvus_client_upsert_binary_data(self):
        ...
        upsert_rows, _, binary_vectors = self.gen_default_rows_data_for_upsert(nb=nb, is_binary=True)
        results = self.upsert(client, collection_name, upsert_rows)[0]
        assert results['upsert_count'] == nb
        self.flush(client, collection_name)
    ...
    def test_milvus_client_upsert_in_specific_partition(self):
        ...
        upsert_rows, float_values, _ = self.gen_default_rows_data_for_upsert(nb=upsert_nb)
    ...
    def test_milvus_client_upsert_in_mismatched_partitions(self):
        ...
        upsert_rows, float_values, _ = self.gen_default_rows_data_for_upsert(upsert_nb)
