...
class TestMilvusClientIndexInvalid(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    def test_milvus_client_index_invalid_collection_name(self, name):
        ...
        self.create_index(client, name, index_params, check_task=CheckTasks.err_res, check_items=error)
    ...
    def test_milvus_client_index_not_exist_collection_name(self):
        ...
        self.create_index(
            client, not_existed_collection_name, index_params, check_task=CheckTasks.err_res, check_items=error
        )
        self.drop_collection(client, collection_name)
    ...
    def test_milvus_client_int8_vector_create_not_supported_GPU_index(self, not_supported_index):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"invalid parameter[expected=valid index][actual=invalid index type: {not_supported_index}",
        }
        self.create_index(client, collection_name, index_params, check_task=CheckTasks.err_res, check_items=error)
        self.drop_collection(client, collection_name)
    ...
    def test_milvus_client_scalar_auto_index(self, add_field):
        ...
        if add_field:
            self.add_collection_field(
                client, collection_name, field_name="field_int", data_type=DataType.INT32, nullable=True
            )
            self.add_collection_field(
                client,
                collection_name,
                field_name="field_varchar",
        ...
        index_params.add_index(
            ...
            index_type=ct.numeric_supported_index_types[0],
        ...
        if len(numeric_fields) >= len(ct.numeric_supported_index_types):
            ...
            for i in range(len(numeric_fields)):
                ...
                index_params.add_index(
                    ...
                    index_type=ct.numeric_supported_index_types[k],
                    metric_type=metric_type,
                )
                k += 1
        else:
            ...
            for i in range(len(ct.numeric_supported_index_types)):
                ...
                index_params.add_index(
                    ...
                    index_type=ct.numeric_supported_index_types[i],
                    metric_type=metric_type,
                )
                k += 1
        ...
        for index in ct.varchar_supported_index_types:
            index_params.add_index(
                field_name=f"{default_string_field_name}_{index}", index_type=index, metric_type=metric_type
            )

        if add_field:
            index_params.add_index(
                field_name="field_int", index_type=ct.numeric_supported_index_types[0], metric_type=metric_type
            )
            index_params.add_index(
                field_name="field_varchar", index_type=ct.varchar_supported_index_types[0], metric_type=metric_type
            )
        # 3. create index
        self.create_index(client, collection_name, index_params)
        # 4. drop index
        ...
        rows = [
            {
                default_primary_key_field_name: i,
                default_vector_field_name: list(rng.random((1, default_dim))[0]),
                int64_field_name: np.random.randint(0, 99),
                default_string_field_name: str(np.random.randint(0, 99)),
            }
            for i in range(default_nb)
        ]
        self.insert(client, collection_name, rows)
        ...
        rows = [
            {
                ...
                ct.default_int8_field_name: i % 128,
                default_string_field_name: str(np.random.randint(0, 99)),
            }
            for i in range(default_nb)
        ]
        ...
        rows = [
            {
                ...
                default_float_field_name: i * 1.0,
                default_string_field_name: str(i),
                default_multiple_vector_field_name: list(rng.random((1, default_dim))[0]),
            }
            for i in range(default_nb)
        ]
        self.insert(client, collection_name, rows)
...
class TestMilvusClientJsonPathIndexInvalid(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    def test_milvus_client_json_path_index_invalid_index_type(self, invalid_index_type):
        ...
        error = {
            ct.err_code: 1100,
            ct.err_msg: f"invalid parameter[expected=valid index][actual=invalid index type: {invalid_index_type}]",
        }
        self.create_index(client, collection_name, index_params, check_task=CheckTasks.err_res, check_items=error)

    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_json_path_index_not_support_index_type(
        self, enable_dynamic_field, not_supported_varchar_scalar_index
    ):
        """
        ...
        index_params.add_index(
            ...
            index_type=not_supported_varchar_scalar_index,
            params={"json_cast_type": "DOUBLE", "json_path": "my_json['a']['b']"},
        )
        # 3. create index
    ...
    def test_milvus_client_json_path_index_invalid_json_cast_type(
        self, enable_dynamic_field, invalid_json_cast_type, supported_varchar_scalar_index
    ):
        """
    ...
    def test_milvus_client_json_path_index_not_supported_json_cast_type(
        self, enable_dynamic_field, supported_varchar_scalar_index
    ):
        """
    ...
    def test_milvus_client_json_path_index_invalid_json_path(
        self, enable_dynamic_field, invalid_json_path, supported_varchar_scalar_index
    ):
        """
        target: test json path index with invalid json_cast_type
        method: create json path index with invalid json_cast_type
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_different_index_name_same_json_path(
        self, enable_dynamic_field, supported_varchar_scalar_index
    ):
        """
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_different_json_path_index_same_field_same_index_name(
...
class TestMilvusClientJsonPathIndexValid(TestMilvusClientV2Base):
    ...
    ******************************************************************
    ...
    @pytest.mark.tags(CaseLabel.L0)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_json_path_index_default(
        self, enable_dynamic_field, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_json_path_index_default_index_name(
        self, enable_dynamic_field, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
        target: test json path index with not supported json_cast_type
    ...
    def test_milvus_client_json_path_index_on_non_json_field(
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_different_json_path_index_same_field_different_index_name(
        self, enable_dynamic_field, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
    ...
    @pytest.mark.tags(CaseLabel.L1)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_diff_index_same_field_diff_index_name_diff_index_params(
        self, enable_dynamic_field, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
        target: test different json path index with different default index name at the same time
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_json_index_same_json_path_diff_field(
        self, enable_dynamic_field, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
        target: test different json path index with different default index name at the same time
    ...
    @pytest.mark.tags(CaseLabel.L2)
    @pytest.mark.parametrize("enable_dynamic_field", [True, False])
    def test_milvus_client_json_path_index_before_load(
        self, enable_dynamic_field, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
        target: test json path index with not supported json_cast_type
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_milvus_client_json_path_index_after_enable_dynamic_field(
        self, supported_json_cast_type, supported_varchar_scalar_index
    ):
        """
        ...
        expected: json path index works with dynamic field data
        """
        client = self._client()
        collection_name = cf.gen_unique_str(prefix)
        # 1. create collection
        json_field_name = "dynamic_field"
        schema = self.create_schema(client, enable_dynamic_field=False)[0]
        schema.add_field(default_primary_key_field_name, DataType.INT64, is_primary=True, auto_id=False)
        schema.add_field(default_vector_field_name, DataType.FLOAT_VECTOR, dim=default_dim)
        schema.add_field(default_string_field_name, DataType.VARCHAR, max_length=64)
        ...
        index_params.add_index(default_vector_field_name, metric_type="COSINE")
        self.create_collection(client, collection_name, schema=schema, index_params=index_params)
        # 2. alter collection dynamic field enable
        ...
        # 6. create same json index twice
        self.create_index(client, collection_name, index_params)
        self.describe_index(
            client,
            collection_name,
