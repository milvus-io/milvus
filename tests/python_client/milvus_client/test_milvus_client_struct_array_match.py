...
- cross-container NULL parity: the same logical data in a scalar array, a JSON
  path array and a struct array must answer the same MATCH query identically
"""
...
class TestMilvusClientCrossContainerNullMatch(TestMilvusClientV2Base):
    """The same logical data stored in a scalar array, a JSON path array and a
    struct array must answer the same MATCH query with identical row sets —
    including the NULL row and the empty-[] row."""
    ...
    def _build_collection(self, client):
        ...
        schema.add_field(field_name=self.JSON_FIELD, datatype=DataType.JSON, nullable=True)
            ...
            rows.append(
                {
                    ...
                    self.JSON_FIELD: None if values is None else {"a": values},
    ...
    @pytest.mark.tags(CaseLabel.L1)
    def test_cross_container_null_and_empty_parity(self):
        """
        target: verify NULL / empty-[] MATCH semantics are identical across scalar
                array, JSON path array and struct array containers
        method: store the same logical arrays in all three containers, run the same
                logical MATCH query against each, compare the three row sets
        expected: all three containers return the same pks; the NULL row (pk 2) never
                  appears (even under `not`), the [] row (pk 1) follows vacuous rules
        """
        client = self._client()
        collection_name = self._build_collection(client)

        # (description, scalar-array expr, JSON expr, struct expr, expected pks)
        cases = [
            (
                "MATCH_ANY $ > 0: only the real non-empty array",
                f"MATCH_ANY({self.ARRAY_FIELD}, $ > 0)",
                f'MATCH_ANY({self.JSON_FIELD}["a"], $ > 0)',
            ...
            (
                ...
                f'MATCH_ALL({self.JSON_FIELD}["a"], $ >= 1)',
            ...
            (
                ...
                f'not MATCH_ANY({self.JSON_FIELD}["a"], $ > 5)',
            ...
            (
                ...
                f'MATCH_MOST({self.JSON_FIELD}["a"], $ > 0, threshold=0)',
                f"MATCH_MOST({STRUCT_FIELD}, $[{INT_SUBFIELD}] > 0, threshold=0)",
                [1],
            ),
        ]

        for description, array_expr, json_expr, struct_expr, expected in cases:
            array_pks = self._query_pks(client, collection_name, array_expr)
            json_pks = self._query_pks(client, collection_name, json_expr)
            struct_pks = self._query_pks(client, collection_name, struct_expr)
            assert array_pks == json_pks == struct_pks == expected, (
                f"{description}: scalar={array_pks} json={json_pks} struct={struct_pks} expected={expected}"
            )
            log.info(f"cross-container parity ok: {description} -> {expected}")
