...
class TestCacheConfig:
    ...
    ******************************************************************
    ...
    def get_memory_available(self, connect):
        mem_info = connect._cmd("get_system_info")
        mem_available = int(mem_info["memory_total"]) - int(mem_info["memory_used"])
    ...
    def test_set_cache_config_out_of_memory_value_A(self, connect, table):
        ...
        status, reply = connect.set_config("cache_config", "cpu_cache_capacity", mem_available + 2)
        assert not status.OK()
        status, reply = connect.set_config("cache_config", "insert_buffer_size", mem_available + 2)
        assert not status.OK()
    ...
    def test_set_cache_config_out_of_memory_value_B(self, connect, table):
        ...
        assert status.OK()
        status, reply = connect.set_config("cache_config", "cpu_cache_capacity", mem_available - insert_buffer_size + 2)
        assert not status.OK()
        status, reply = connect.set_config("cache_config", "insert_buffer_size", mem_available - cpu_cache_capacity + 2)
        assert not status.OK()
    ...
    def test_set_cache_config_out_of_memory_value_C(self, connect, table):
        ...
        status, insert_buffer_size = connect.get_config("cache_config", "insert_buffer_size")
        assert status.OK()
        if mem_available >= insert_buffer_size + 20:
            status, reply = connect.set_config("cache_config", "cpu_cache_capacity", mem_available - insert_buffer_size - 10)
            assert status.OK()
            status, reply = connect.set_config("cache_config", "insert_buffer_size", insert_buffer_size + 20)
            assert not status.OK()
