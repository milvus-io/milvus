...
func (s *SqlQuerySuite) sqlQuery(sql string) sqlResp {
	...
	mlog.Info(context.Background(), "sql_query", zap.String("sql", sql), zap.Int("code", result.Code), zap.String("msg", result.Message))
	return result
}
...
func (s *SqlQuerySuite) prepareCollection() {
	...
	mlog.Info(context.Background(), "inserted rows", zap.Int("count", sqlRowNum))
	...
	mlog.Info(context.Background(), "collection ready", zap.String("collection", collName))
}
