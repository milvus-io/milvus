...
func (s *BulkInsertSuite) PrepareCollectionWithL0(dim, rowNum, delNum, delBatch int) (int64, int64, *schemapb.IDs) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute*5)
	defer cancel()
	c := s.Cluster

	collectionName := "TestL0Import_A_" + funcutil.GenRandomStr()
...
func (s *BulkInsertSuite) TestL0Import() {
	const (
		dim      = 128
		rowNum   = 50000
		delNum   = 30000
	...
	collectionName := "TestL0Import_B_" + funcutil.GenRandomStr()
