...
func (s *Suite) TestGeoAndTimeExpressions() {
	...
	for _, tc := range cases {
		s.Run(tc.counter, func() {
			...
			requireOnlyDelta(s.T(), before, after, map[string]int64{tc.counter: 1, queryOnAggregation: 1})
		})
	}
	...
	s.Run("timestamptz_compare", func() {
		...
		after := counters(s.report(ctx), typeutil.ProxyRole)
		requireOnlyDelta(s.T(), before, after, map[string]int64{"timestamptz_compare": 1, queryOnAggregation: 1})
	})
}
...
func (s *Suite) TestStructArrayExpressions() {
	...
	for _, tc := range []struct {
		...
		{"struct_array_match", fmt.Sprintf(`MATCH_ANY(%s, $[%s] > 1)`, integration.StructArrayField, sub)},
	} {
		s.Run(tc.counter, func() {
			before := counters(s.report(ctx), typeutil.ProxyRole)
			s.queryOn(ctx, name, tc.expr)
			after := counters(s.report(ctx), typeutil.ProxyRole)
			// The element predicate `> 1` is itself a comparison.
			requireOnlyDelta(s.T(), before, after, map[string]int64{tc.counter: 1, "comparison_operators=relational": 1, queryOnAggregation: 1})
		})
	}

	// A search on the struct's vector sub-field is an embedding-list search
	// when the placeholder carries a list per query, and an element-level
	// search when it carries one vector per query.
	for _, tc := range []struct {
		retrieval string
		req       *milvuspb.SearchRequest
	}{
		{"embedding_list", integration.ConstructEmbeddingListSearchRequest("", name, "", subVec,
			schemapb.DataType_FloatVector, nil, metric.MaxSimL2, map[string]any{}, 1, dim, 5, -1)},
		{"element_level", integration.ConstructElementLevelSearchRequest("", name, "", subVec,
			schemapb.DataType_FloatVector, nil, metric.L2, map[string]any{}, 1, dim, 5, -1)},
	} {
		s.Run("retrieval="+tc.retrieval, func() {
			before := units(s.report(ctx))
			tc.req.UseDefaultConsistency = true
			_, err := s.Cluster.MilvusClient.Search(ctx, tc.req)
			s.Require().NoError(err, "transport error")
			requireOnlyDelta(s.T(), before, units(s.report(ctx)), searchUnits("omitted", "omitted", "<=10", "1", tc.retrieval))
		})
	}
}

// queryOnAggregation is the counter every queryOn moves besides the one a
// case is about: it asks for count(*).
const queryOnAggregation = "query_aggregation=count"
