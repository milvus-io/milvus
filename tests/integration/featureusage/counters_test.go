...
func (s *Suite) TestSearchCounters() {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: map[string]int64{"grouping_search": 1},
		},
		{
			...
			want: map[string]int64{"range_search": 1},
		},
		{
		...
		},
		{
			...
			want: map[string]int64{"grouping_search": 1, "group_size": 1, "strict_group_size": 1},
		},
		{
			name: "hints",
			run:  func() { s.searchWith(ctx, withSearchParam(common.HintsKey, "iterative_filter")) },
			want: map[string]int64{"hints=iterative_filter": 1},
		},
		{
			name: "an undocumented hint folds into _other",
			run:  func() { s.searchWith(ctx, withSearchParam(common.HintsKey, "a_hint_that_does_not_exist")) },
			want: map[string]int64{"hints=_other": 1},
		},
		{
			name: "namespace",
			run: func() {
		...
		{
			// A real search by primary key, not the deprecated
			// search_by_primary_keys bool: nothing sets that field, and the
			// Proxy decides on the ids carried in search_input.
			name: "search_by_primary_keys",
			run: func() {
				s.searchWith(ctx, func(r *milvuspb.SearchRequest) {
					r.SearchInput = &milvuspb.SearchRequest_Ids{
						Ids: &schemapb.IDs{
			...
			want: map[string]int64{"primary_key_search": 1},
		},
		{
			name: "analyzer_name",
		...
		},
		{
			...
			want: map[string]int64{"grouping_search": 1, "rank_group_scorer": 1},
		},
		{
			name: "old iterator protocol",
			run:  func() { s.searchWith(ctx, withSearchParam("iterator", "true")) },
			want: map[string]int64{"search_iterator=v1": 1},
		},
		{
			...
			want: map[string]int64{"search_iterator=v2": 1},
		},
		{
		...
		},
		{
			...
			want: map[string]int64{"reranker=decay": 1},
		},
		{
			...
			want: map[string]int64{"reranker=_other": 1},
		},
		{
		...
		{
			name: "output_vector_field on the search path",
			run: func() {
				s.searchWith(ctx, func(r *milvuspb.SearchRequest) {
					r.OutputFields = []string{vectorField}
				})
			},
			want: map[string]int64{"output_fields=vector": 1},
		},
		{
			name: "a scalar output field does not count as a vector one",
			run: func() {
				s.searchWith(ctx, func(r *milvuspb.SearchRequest) {
					r.OutputFields = []string{textField}
				})
			},
			want: map[string]int64{},
		},
...
func (s *Suite) TestQueryCounters() {
	...
	cases := []struct {
		...
		{
			name: "query iterator has its own counter",
			run: func() {
			...
			want: map[string]int64{"query_iterator": 1},
		},
		{
		...
		},
		{
			...
			want: map[string]int64{"output_fields=dynamic": 1, "comparison_operators=relational": 1},
		},
		{
			name: "output_vector_field on the query path",
			run: func() {
				s.queryWith(ctx, func(r *milvuspb.QueryRequest) {
					r.Expr = pkField + " >= 0"
					r.OutputFields = []string{vectorField}
			...
			want: map[string]int64{"output_fields=vector": 1, "comparison_operators=relational": 1},
		},
		{
			name: "a wildcard output field counts the vector it expands to",
			run: func() {
				s.queryWith(ctx, func(r *milvuspb.QueryRequest) {
					r.Expr = pkField + " >= 0"
		...
		},
		{
			...
			want: map[string]int64{"filter_templating": 1, "comparison_operators=relational": 1},
		},
		{
			...
			want: map[string]int64{"consistency_level=Bounded": 1, "comparison_operators=relational": 1},
		},
	}
	for _, tc := range cases {
		s.Run(tc.name, func() {
...
func (s *Suite) TestExpressionCounters() {
	...
	cases := []struct {
		...
		{"json_path", fmt.Sprintf(`%s["k"] > 0`, jsonField), map[string]int64{"json_path": 1, "comparison_operators=relational": 1}},
		{"json_contains", fmt.Sprintf(`json_contains(%s["arr"], 1)`, jsonField), map[string]int64{"json_contains": 1, "json_path": 1}},
		{"array_contains", fmt.Sprintf(`array_contains(%s, 1)`, arrayField), map[string]int64{"array_contains": 1}},
		{"array_length", fmt.Sprintf(`array_length(%s) == 2`, arrayField), map[string]int64{"array_length": 1, "comparison_operators=equality": 1}},
		{"random_sample", `random_sample(0.5)`, map[string]int64{"random_sample": 1}},
		{"exists", fmt.Sprintf(`exists %s["k"]`, jsonField), map[string]int64{"exists": 1, "json_path": 1}},
		{"comparison_operators=equality", fmt.Sprintf(`%s == "milvus feature usage row 1"`, textField), map[string]int64{"comparison_operators=equality": 1}},
		{"comparison_operators=relational", fmt.Sprintf(`%s < 100`, pkField), map[string]int64{"comparison_operators=relational": 1}},
		{"in_operator", fmt.Sprintf(`%s in [1, 2, 3]`, pkField), map[string]int64{"in_operator": 1}},
		{"is_not_null", fmt.Sprintf(`%s is not null`, textField), map[string]int64{"is_not_null": 1}},
		// A LIKE pattern with a wildcard in the middle is still like, not
		// regex_match; the regex operator is driven in TestMoreSearchCounters.
		{"an infix like is not regex_match", fmt.Sprintf(`%s like "mil%%us%%"`, textField), map[string]int64{"like": 1}},
	}
	for _, tc := range cases {
		s.Run(tc.name, func() {
			before := counters(s.report(ctx), typeutil.ProxyRole)
