...
// notDrivable are the counters no request can move in this suite, each with
// the reason. They are the only permitted gaps: anything else missing from the
// report at the end of the run fails the coverage check.
//
// Keep this list short and justified. A counter parked here is a counter the
// test team cannot accept, so it should either be driven, driven in a sibling
// suite named in its reason, or deleted.
var notDrivable = map[string]string{
	"proxy\tauth_method=password": "the authentication interceptor only runs when " +
		"common.security.authorizationEnabled is on, which would make every other request in this " +
		"suite need a credential; driven end to end in tests/integration/featureusageauth",
	"proxy\tauth_method=api_key": "VerifyAPIKey delegates to the hook extension and the built-in " +
		"DefaultHook rejects every key, so no request in this tree can authenticate with one; " +
		"tests/integration/featureusageauth pins that it stays at zero without a hook",

	"proxy\tfield_ops=_other": "the fold slot for a partial-update operator this build does not know; " +
		"resolveFieldPartialUpdateOps rejects such an operator before the upsert is counted, so no request can produce one",
	"mixcoord\tcompaction=_other": "the fold slot for an unrecognized CompactionType; no request can produce one",
	"mixcoord\tcompaction=partition_key_sort": "no DataCoord path constructs this type today: " +
	...
	"mixcoord\tcompaction=clustering_partition_key_sort": "same as partition_key_sort: declared in the " +
	...
	for _, node := range report.GetNodes() {
		for _, e := range node.GetEntries() {
			if e.GetGroup() == featureusage.GroupRequest && e.GetValue() > 0 {
				hit[node.GetRole()+"\t"+entryKey(e)] = true
			}
		}
	}
