...
func (s *Suite) TestStaticGroupsAndSanitization() {
	...
	s.GreaterOrEqual(dist["num_partitions|2-16"], int64(1))
...
func (s *Suite) TestQueryNodeGroups() {
	...
	s.NotContains(before, "brute_force_search",
		"dropped: a segment without Go-side index metadata may still use an interim index")
	...
	s.Greater(after["two_stage_search"], before["two_stage_search"],
...
func (s *Suite) TestImportFileTypesAreCounted() {
	...
	for _, tc := range []struct {
		...
		{"import_file_type=JSONLine", func() string { return s.writeJSONImportFile(ctx, schema, dim, "jsonl") }},
...
func (s *Suite) TestProvidersAndCustomResourceGroup() {
	...
	// A database carrying a property is what puts the database_properties group in
