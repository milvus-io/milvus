...
// compactHighContainerBlob builds a legal MRB1 body whose wire size is small
// relative to its decoded allocation shape: every high-32 key owns one
// one-value Roaring32 child. This is the adversarial distribution needed to
// prove decoded-memory admission rather than merely testing blob-byte limits.
func compactHighContainerBlob(count uint64) []byte {
	child := make([]byte, 11)
	...
	blob := make([]byte, clientroaring.HeaderSize+len(body))
	copy(blob[:4], clientroaring.Magic)
	binary.LittleEndian.PutUint16(blob[4:6], clientroaring.Version)
	binary.LittleEndian.PutUint16(blob[6:8], clientroaring.FormatPortableRoaring64)
	binary.LittleEndian.PutUint64(blob[8:16], count)
	binary.LittleEndian.PutUint64(blob[16:24], uint64(len(body)))
	copy(blob[clientroaring.HeaderSize:], body)
	return blob
}
...
func (s *RoaringMatchTestSuite) TestVectorAndHybridSearchApplyExactFilter() {
	...
	// The same bitmap appears in the main predicate and a boost scorer filter.
	// QueryNode admits both occurrences, decodes the distinct blob once while
	// constructing the C++ plan, and segment rerank calls reference scorer 0.
	scoredReq := integration.ConstructSearchRequest(s.dbName, collectionName, expr,
		integration.FloatVecField, schemapb.DataType_FloatVector, []string{creatorIDField},
		metric.L2, integration.GetSearchParams(integration.IndexFaissIvfFlat, metric.L2),
		1, s.dim, 100, -1)
	scoredReq.ExprTemplateValues = rbParam(blob)
	scoredReq.ConsistencyLevel = commonpb.ConsistencyLevel_Strong
	scoredReq.FunctionScore = &schemapb.FunctionScore{
		Functions: []*schemapb.FunctionSchema{{
			Name: "roaring_boost",
			Type: schemapb.FunctionType_Rerank,
			Params: []*commonpb.KeyValuePair{
				{Key: "reranker", Value: "boost"},
				{Key: "filter", Value: expr},
				{Key: "weight", Value: "2"},
			},
		}},
	}
	scored, err := s.Cluster.MilvusClient.Search(context.Background(), scoredReq)
	s.Require().NoError(err)
	assertFiltered(scored, "Search+boost scorer")
...
// TestAggregateDecodedMemoryAdmission proves both Proxy aggregation points:
// repeated AST occurrences in one Query and one admitted occurrence in each of
// two HybridSearch sub-plans. Each bitmap is valid by itself and the wire bytes
// stay well below the serialized-plan budget; only the aggregate decoded shape
// exceeds 64 MiB.
func (s *RoaringMatchTestSuite) TestAggregateDecodedMemoryAdmission() {
	collectionName := "test_roaring_aggregate_" + funcutil.GenRandomStr()
	s.setupCollection(collectionName)

	blob := compactHighContainerBlob(170_000)
	summary, err := clientroaring.Validate(blob)
	s.Require().NoError(err)
	s.LessOrEqual(summary.EstimatedDecodedBytes, uint64(clientroaring.MaxEstimatedDecodedBytes))
	s.Greater(summary.EstimatedDecodedBytes*2, uint64(clientroaring.MaxEstimatedDecodedBytes))

	queryResult := s.query(collectionName,
		fmt.Sprintf("roaring_match(%s, {rb}) and roaring_match(%s, {rb})",
			creatorIDField, creatorIDField), nil, rbParam(blob))
	s.ErrorIs(merr.Error(queryResult.GetStatus()), merr.ErrParameterTooLarge)
	...
	s.Require().NoError(err)
	s.ErrorIs(merr.Error(hybrid.GetStatus()), merr.ErrParameterTooLarge)
}
