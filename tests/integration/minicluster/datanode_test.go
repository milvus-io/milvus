...
const (
	...
	maxGoRoutineNum      = 100
	balanceTimeInSeconds = 1
)

var prefix = ""

func (s *DataNodeSuite) loadCollection(collectionName string, dim int) {
	c := s.Cluster
	dbName := ""
...
func (s *DataNodeSuite) search(collectionName string, dim int) {
	c := s.Cluster
	var err error
	// Query
...
func (s *DataNodeSuite) insertBatchCollections(prefix string, collectionBatchSize, idxStart, dim int, wg *sync.WaitGroup) {
...
func (s *DataNodeSuite) TestSwapQN() {
	...
	s.Cluster.AddDataNode()
	time.Sleep(time.Second * balanceTimeInSeconds)
	cn := fmt.Sprintf("new_collection_a")
	s.loadCollection(cn, Dim)
	s.search(cn, Dim)
