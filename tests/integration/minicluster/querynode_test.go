...
const (
	Dim                  = 128
	numCollections       = 4
	rowsPerCollection    = 100
	maxGoRoutineNum      = 100
	balanceTimeInSeconds = 10
)
...
func (s *CoordSwitchSuite) checkAllCollectionsReady() {
	goRoutineNum := maxGoRoutineNum
	if goRoutineNum > numCollections {
		goRoutineNum = numCollections
	}
	...
	for i := 0; i < goRoutineNum; i++ {
		for idx := 0; idx < collectionBatchSize; idx++ {
			collectionName := prefix + "_" + strconv.Itoa(i*collectionBatchSize+idx)
			s.search(collectionName, Dim)
			queryReq := &milvuspb.QueryRequest{
				CollectionName: collectionName,
				Expr:           "",
				OutputFields:   []string{"count(*)"},
...
func (s *CoordSwitchSuite) checkQNRestarts() {
	...
	time.Sleep(time.Second * balanceTimeInSeconds)
	for i := 0; i < 1000; i++ {
...
func (s *CoordSwitchSuite) TestSwapQN() {
	s.setupData()
