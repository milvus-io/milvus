...
// with Milvus's path helper to obtain the exact object keys. V3 resolves the
// equivalent base path from the segment manifest.
func LocateTextLog(rootPath string, segments []*datapb.SegmentInfo,
	...
	for _, segment := range segments {
		...
		if segment.GetStorageVersion() == storage.StorageV3 {
			resolved := packed.NewStatsResolverFromSegmentInfo(segment).TextAndJSONIndexStatsWithBasePaths()
			if err := resolved.Err(); err != nil {
				return nil, fmt.Errorf("resolve V3 text stats for segment %d: %w", segment.GetID(), err)
			}
			resolvedStats, ok := resolved.TextIndexStats[fieldID]
			if !ok || resolvedStats == nil {
				return nil, fmt.Errorf("manifest has no text stats for segment %d field %d", segment.GetID(), fieldID)
			}
			stats = resolvedStats
			files = stats.GetFiles()
			basePath = resolved.TextBasePaths[fieldID]
		} else {
