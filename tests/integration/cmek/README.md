...
| `raw_data_v3_growing_test.go` | `TestRawDataV3GrowingSuite`, `TestRawDataV3GrowingInterimSuite` | One-shard V3 Growing batches, WAL-tail crash recovery, interim raw-vector handoff, and sealed cold reads. Each background commit is checked against DataCoord SegmentInfo, WAL assignment and checkpoint metadata, its source event, and every encrypted manifest object. |
| `raw_data_v3_consumer_test.go` | `TestRawDataV3ConsumerSuite`, growing-suite index case | Canonical and growing-source encrypted V3 manifests as real HNSW build inputs, physical index files, loaded build identity, and query/search results. |
...
Segment Storage Version, index engine version and index artifact format are separate choices. The scalar-index suites currently all use Storage V2 source segments, including the packed V3 and FMINDEX suites. They do not provide Storage V3 raw-data coverage. The canonical V2/V3 raw-data campaigns and Growing batch/recovery cases keep physical indexes absent; the Growing interim variant explicitly exercises the in-memory index, and the V3 consumer cases build physical HNSW indexes from encrypted V3 source. `vector_index_test.go` covers physical HNSW from V2 source.
...
All raw-data suites disable compaction before starting the cluster. Growing-source flush is enabled in the V3 Growing and interim suites; canonical V2/V3 and canonical HNSW keep it disabled. The Growing cases inspect each background commit before final `Flush`, require the Segment to remain Growing, and compare earlier object digests across revisions. Recovery cases SIGKILL the channel owner twice, require the new process to log a successful prefix load and WAL seek, and check exact field values before sealing. Sealed cases inspect every object, release/reload, and confirm the loaded Segment identities.
