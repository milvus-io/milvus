...
func (s *PartialSearchTestSuit) TestSingleNodeDownOnSingleReplicaWithGlobalConfig() {
	...
	paramtable.Get().Save(paramtable.Get().ProxyCfg.PartialResultRequiredDataRatio.Key, "0.5")
...
// setup 5 querynode, 1 replica, 1 channel, 5 segments, 2000 rows per segment
...
func (s *PartialSearchTestSuit) TestAccessDataRatio() {
	// set segment check interval to 10000 to slow down segment recovery
	paramtable.Get().Save(paramtable.Get().QueryCoordCfg.SegmentCheckInterval.Key, "100000")
