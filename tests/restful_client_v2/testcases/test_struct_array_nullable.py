...
class TestRestfulStructArrayNullable(TestBase):
    ...
    def _vector(seed: int) -> list[float]:
        return [float(seed) for _ in range(TestRestfulStructArrayNullable.dim)]

    @staticmethod
    def _profile(row_id: int) -> list[dict]:
        return [
    ...
    @staticmethod
    def _assert_profile_equal(actual, expected):
    ...
    def test_rest_v2_struct_array_create_query_search_output(self):
        ...
        rows = [
            {"id": 1, "normal_vector": self._vector(1), "doc_tag": "row_1", "profile": self._profile(1)},
            {"id": 2, "normal_vector": self._vector(2), "doc_tag": "row_2", "profile": []},
            {"id": 3, "normal_vector": self._vector(3), "doc_tag": "row_3", "profile": self._profile(3)},
        ]
        ...
        assert {row["id"] for row in rsp["data"]} == set(source_by_id)
        for row in rsp["data"]:
            self._assert_profile_equal(row["profile"], source_by_id[row["id"]]["profile"])
        ...
        self._assert_profile_equal(rsp["data"][0]["profile"], source_by_id[3]["profile"])
        ...
        for hit in rsp["data"]:
            ...
            self._assert_profile_equal(hit["profile"], expected["profile"])
    ...
    @pytest.mark.xfail(
        strict=True,
        reason="REST v2 schema.structFields nullable=true is silently ignored instead of propagated or rejected",
    )
    def test_rest_v2_nullable_struct_array_schema_propagation(self):
        """
        target: test REST v2 nullable propagation for Struct Array
        method: create schema.structFields with nullable=true and inspect raw schema
        expected: REST either preserves nullable=true on Struct Array, or rejects unsupported nullable explicitly
        """
        name = gen_collection_name()
        self.name = name
        self._create_struct_array_collection(name, struct_nullable=True)

        client = MilvusClient(uri=self.endpoint, token=self.api_key)
        raw_describe = client._get_connection().describe_collection(name)
