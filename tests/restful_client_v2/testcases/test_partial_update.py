...
class TestPartialUpdate(TestBase):
    ...
    def test_partial_update_multiple_fields(self, id_type):
        ...
        payload = {"collectionName": name, "data": partial_update_data, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] == 0
    ...
    def test_partial_update_mixed_scenario(self):
        ...
        payload = {"collectionName": name, "data": update_data, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] == 0
        ...
        payload = {"collectionName": name, "data": new_records_data, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] == 0
    ...
    def test_partial_update_with_auto_id(self):
        ...
        rsp = self.vector_client.vector_query(
            {"collectionName": name, "filter": "user_id >= 0", "outputFields": ["book_id", "user_id", "book_describe"]}
        )
        assert rsp["code"] == 0
        assert len(rsp["data"]) == nb
    ...
    def test_partial_update_auto_id_only_specified_fields_updated(self):
        ...
        initial_data = [{"field1": "original_value1", "field2": 100, "field3": 3.14, "vector": original_vector}]
    ...
    def test_partial_update_nullable_field_scenarios(self):
        ...
        payload = {
            ...
            "schema": {
                "fields": [
                    ...
                    {"fieldName": "book_describe", "dataType": "VarChar", "elementTypeParams": {"max_length": "256"}},
                    {
                        "fieldName": "nullable_field_no_default",
                        "dataType": "VarChar",
                        "elementTypeParams": {"max_length": "256"},
                        "nullable": True,
                    },  # Nullable, no default
                    {
                        "fieldName": "nullable_field_with_default",
                        "dataType": "VarChar",
                        "elementTypeParams": {"max_length": "256"},
                        "nullable": True,
                    ...
                    {
                        "fieldName": "nullable_field_for_null_update",
                        "dataType": "VarChar",
                        "elementTypeParams": {"max_length": "256"},
                        "nullable": True,
                    },  # Nullable, no default
        ...
        partial_update_scenario1 = [{"book_id": 1, "nullable_field_no_default": "updated_value_1"}]

        payload = {"collectionName": name, "data": partial_update_scenario1, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] == 0
        assert rsp["data"]["upsertCount"] == 1
        ...
        partial_update_scenario2 = [{"book_id": 2, "nullable_field_with_default": "updated_value_2"}]

        payload = {"collectionName": name, "data": partial_update_scenario2, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] == 0
        assert rsp["data"]["upsertCount"] == 1
        ...
        partial_update_scenario3 = [{"book_id": 3, "nullable_field_for_null_update": None}]

        payload = {"collectionName": name, "data": partial_update_scenario3, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] == 0
        assert rsp["data"]["upsertCount"] == 1
        ...
        assert (
            updated_data_by_id[2]["nullable_field_no_default"] is None
            or "nullable_field_no_default" not in updated_data_by_id[2]
        )
        ...
        assert (
            updated_data_by_id[3]["nullable_field_no_default"] is None
            or "nullable_field_no_default" not in updated_data_by_id[3]
        )
...
class TestPartialUpdateNegative(TestBase):
    ...
    def test_partial_update_invalid_collection_name(self):
        ...
        """
        partial_update_data = [{"book_id": 1, "book_describe": "updated_description"}]

        payload = {"collectionName": "non_existent_collection", "data": partial_update_data, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
        assert rsp["code"] != 0
        logger.info(f"Expected failure response: {rsp}")
    ...
    def test_partial_update_non_existent_field(self):
        ...
        assert rsp["code"] != 0
        assert (
            "dynamic schema" in rsp["message"] or "not exist" in rsp["message"] or "unknown" in rsp["message"].lower()
        )
    ...
    def test_partial_update_mixed_success_failure(self):
        ...
        payload = {"collectionName": name, "data": mixed_data, "partialUpdate": True}
        rsp = self.vector_client.vector_upsert(payload)
