...
class TestQueryOrderBy(TestBase):
    ...
    @pytest.mark.tags(CaseLabel.L0)
    def test_query_group_by_fields_with_order_by(self):
        """
        target: verify REST query forwards groupByFields and orders aggregation groups
        method: group deterministic category values and order the groups by category
        expected: five ordered groups, each with the full group count
        """
        rows = self._query(
            {
                "collectionName": self._shared_collection(),
        ...
        assert [row["category"] for row in rows] == [f"category_{i}" for i in range(5)]
        assert [row["count(*)"] for row in rows] == [NB // 5] * 5

    @pytest.mark.tags(CaseLabel.L0)
    def test_query_global_count_is_not_limited_by_rest_default(self):
        """
        target: verify REST's response default / explicit limit does not cap global count(*)
        method: count more than 100 inserted rows with omitted and explicit limit
        expected: both requests return the full matching count in one row
        """
        base_payload = {
            "collectionName": self._shared_collection(),
            "filter": "id >= 0",
        ...
        default_rows = self._query(base_payload)
        explicit_rows = self._query({**base_payload, "limit": 10})

        assert len(default_rows) == 1
        assert default_rows[0]["count(*)"] == NB
        assert len(explicit_rows) == 1
        assert explicit_rows[0]["count(*)"] == NB
