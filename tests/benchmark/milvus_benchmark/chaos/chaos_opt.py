...
class ChaosOpt(object):
    def __init__(
        self,
        kind,
        group=cf.DEFAULT_GROUP,
        version=cf.DEFAULT_VERSION,
        namespace=cf.CHAOS_NAMESPACE,
    ):
        self.group = group
    ...
    def delete_chaos_object(self, metadata_name):
        ...
        try:
            config.load_kube_config()
            api_instance = client.CustomObjectsApi()
            data = api_instance.delete_namespaced_custom_object(
    ...
    def list_chaos_object(self):
        try:
            config.load_kube_config()
            api_instance = client.CustomObjectsApi()
            data = api_instance.list_namespaced_custom_object(
                self.group, self.version, self.namespace, plural=self.plural
            )
            # pprint(data)
        except ApiException as e:
            logger.error(
