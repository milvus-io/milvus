...
class Tasks(TaskSet):
    ...
    def get(self):
        ...
        self.client.get(
            self.values["get_ids"][: self.params[op]["ids_length"]], timeout=300
        )
    ...
    def scene_test(self):
        ...
        collection_name = (
            ...
            + "_"
            + str(random.randint(10001, 999999))
        )
        self.client.scene_test(
            collection_name,
            vectors=self.values["X"][:3000],
            ids=self.values["ids"][:3000],
        )
