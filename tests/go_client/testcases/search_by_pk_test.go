...
// Method:
//  1. connect and create a collection with FLAT index (for exact search)
//  2. search by primary keys float vectors
//  3. verify search by primary keys results and self-match assertion
//
// Expected: search successfully and self-match should be top result with distance ~0 (or score ~1 for COSINE)
func TestSearchByPKFloatVectors(t *testing.T) {
	ctx := hp.CreateContext(t, time.Second*common.DefaultTimeout)
	mc := hp.CreateDefaultMilvusClient(ctx, t)

	// create collection -> insert -> flush
	...
	// Create FLAT index for exact search (guarantees self-match as top result)
	flatIndex := index.NewFlatIndex(entity.COSINE)
	idxTask, err := mc.CreateIndex(ctx, client.NewCreateIndexOption(schema.CollectionName, common.DefaultFloatVecFieldName, flatIndex))
	common.CheckErr(t, err, true)
	err = idxTask.Await(ctx)
	common.CheckErr(t, err, true)

	// Load collection
	loadTask, err := mc.LoadCollection(ctx, client.NewLoadCollectionOption(schema.CollectionName))
	common.CheckErr(t, err, true)
	...
	// With FLAT index, self-match should be the top result with score ~1.0 (COSINE similarity)
	for i, resultSet := range resSearch {
		// For COSINE metric with FLAT index, self-match score should be very close to 1.0
		score := float64(resultSet.Scores[0])
		require.InDelta(t, 1.0, score, 0.001,
			"Result set %d: self-match score %.6f should be ~1.0 for COSINE with FLAT index", i, score)
	}
}
...
//  2. insert data where some vectors are null using NewNullableColumnSparseFloatVector
...
func TestSearchByPKWithEmptyIDs(t *testing.T) {
	...
	// Expect error for empty IDs - SDK validates: "IDs column for search cannot be empty"
	common.CheckErr(t, err, false, "IDs column for search cannot be empty")
}
...
//
// Expected: search should return error for duplicate IDs
func TestSearchByPKWithDuplicateIDs(t *testing.T) {
	...
	// Milvus rejects duplicate IDs in search request
	_, err = mc.Search(ctx, searchOption)
	common.CheckErr(t, err, false, "duplicate IDs found in search request")
}
...
//
// Expected: search should return error (IDs not found)
func TestSearchByPKWithInvalidIDs(t *testing.T) {
	...
	// Search by invalid IDs - should return error since vectors cannot be found
	searchOption := client.NewSearchByIDsOption(schema.CollectionName, common.DefaultLimit, idColumn).
		WithANNSField(common.DefaultFloatVecFieldName).
		WithConsistencyLevel(entity.ClStrong)

	_, err := mc.Search(ctx, searchOption)
	// Milvus returns error when searching with non-existent IDs
...
func TestSearchByPKWithRangeSearch(t *testing.T) {
	...
	// For COSINE metric: similarity scores are in [0, 1], where 1 = perfect match
	// radius = 0.5 filters out results with score <= 0.5
	// range_filter = 1.1 includes all scores up to 1.0 (perfect match)
	radius := "0.5"
	rangeFilter := "1.1"
	...
	// Verify all scores are within the range (radius, range_filter]
	radiusFloat := float32(0.5)
	rangeFilterFloat := float32(1.1)

	for i, resultSet := range resSearch {
		t.Logf("Result set %d has %d results", i, resultSet.ResultCount)
		for j, score := range resultSet.Scores {
			// All scores should be: radius < score <= range_filter
			require.Greater(t, score, radiusFloat,
				"Score %.4f should be > radius %.4f", score, radiusFloat)
			require.LessOrEqual(t, score, rangeFilterFloat,
				"Score %.4f should be <= range_filter %.4f", score, rangeFilterFloat)
			t.Logf("  Result %d: score=%.4f (within range (%.2f, %.2f])", j, score, radiusFloat, rangeFilterFloat)
		}
	}
}
...
func TestSearchByPKWithHybridSearch(t *testing.T) {
	...
	// Current error: "empty search request" - this happens because IDs are not properly handled in hybrid search
	// TODO: Consider opening an issue for a clearer error message like "search by IDs is not supported in hybrid search"
	common.CheckErr(t, err, false, "empty search request")
