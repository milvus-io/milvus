...
func TestUpdatePartialFields(t *testing.T) {
	...
	*/
	...
	for _, genColumnsFunc := range []func(*hp.GenDataOption) ([]column.Column, []column.Column){genPkWithSingleScalarField, genPkWithSinglVectorField} {
		...
		updateRes, err := mc.Update(ctx, client.NewColumnBasedInsertOption(schema.CollectionName).WithColumns(columns...).WithColumns(dynamicColumns...))
		common.CheckErr(t, err, true)
		require.EqualValues(t, updateNb, updateRes.UpdateCount)
...
func TestPartialUpdateDynamicField(t *testing.T) {
	...
	_, err = mc.Update(ctx, client.NewColumnBasedInsertOption(schema.CollectionName).WithColumns(pkColumnA, dynamicColumnA, vecColumn))
	common.CheckErr(t, err, true)
	time.Sleep(time.Second * 4)
	resSet, err = mc.Query(ctx, client.NewQueryOption(schema.CollectionName).WithFilter(fmt.Sprintf("%s == %d", common.DefaultInt64FieldName, targetPk)).
		WithOutputFields(common.DefaultDynamicFieldName).WithConsistencyLevel(entity.ClStrong))
	...
	_, err = mc.Update(ctx, client.NewColumnBasedInsertOption(schema.CollectionName).WithColumns(pkColumnA, dynamicColumnB))
	common.CheckErr(t, err, true)
	time.Sleep(time.Second * 4)
	resSet, err = mc.Query(ctx, client.NewQueryOption(schema.CollectionName).WithFilter(fmt.Sprintf("%s == %d", common.DefaultInt64FieldName, targetPk)).
		WithOutputFields(common.DefaultDynamicFieldName).WithConsistencyLevel(entity.ClStrong))
	...
	_, err = mc.Update(ctx, client.NewColumnBasedInsertOption(schema.CollectionName).WithColumns(pkColumnA, dynamicColumnA))
	common.CheckErr(t, err, true)
	time.Sleep(time.Second * 4)
	resSet, err = mc.Query(ctx, client.NewQueryOption(schema.CollectionName).WithFilter(fmt.Sprintf("%s == %d", common.DefaultInt64FieldName, targetPk)).
		WithOutputFields(common.DefaultDynamicFieldName).WithConsistencyLevel(entity.ClStrong))
...
func TestUpdateNullableFieldBehavior(t *testing.T) {
	...
	*/
	...
	updateRes, err := mc.Update(ctx, client.NewColumnBasedInsertOption(collName).WithColumns(updatePkColumn, updateVecColumn))
	common.CheckErr(t, err, true)
	require.EqualValues(t, 2, updateRes.UpdateCount)
