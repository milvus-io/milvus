...
func TestExternalTableAlterBeforeFirstRefresh(t *testing.T) {
	ctx := hp.CreateContext(t, time.Second*common.DefaultTimeout)
	mc := hp.CreateDefaultMilvusClient(ctx, t)
	...
	collName := common.GenRandomString("ext_alter_cold_cache", 6)
	extPath := fmt.Sprintf("external-e2e-test/%s", collName)
	data, err := generateTextMatchParquetBytes(128, 0)
	require.NoError(t, err)
...
func TestExternalTableLoadedAddFieldQueryAcrossRefresh(t *testing.T) {
	...
	// Keep the collection loaded so this query waits for the online Reopen.
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		res, err := mc.Query(ctx, client.NewQueryOption(collName).
			WithConsistencyLevel(entity.ClStrong).
			WithFilter("id < 5").
...
func TestExternalTableLoadedBM25QueryAcrossRefresh(t *testing.T) {
	...
	extPath := fmt.Sprintf("external-e2e-test/%s", collName)
	const rowCount int64 = 1000
	data, err := generateParquetBytes(externalDataSchemaMulti, rowCount, 0, testVecDim)
	require.NoError(t, err)
	key := fmt.Sprintf("%s/data.parquet", extPath)
	_, err = minioClient.PutObject(ctx, minioCfg.bucket, key,
		bytes.NewReader(data), int64(len(data)),
	...
	// The DDL creates the bound index metadata immediately, but the inspector
	// must defer its build until refresh materializes the function output.
	boundIdx, err := mc.DescribeIndex(ctx, client.NewDescribeIndexOption(collName, "bm25_sparse"))
	common.CheckErr(t, err, true)
	require.Equal(t, rowCount, boundIdx.TotalRows)
	require.Zero(t, boundIdx.IndexedRows)
	require.Equal(t, rowCount, boundIdx.PendingIndexRows)
	require.Never(t, func() bool {
		desc, err := mc.DescribeIndex(ctx, client.NewDescribeIndexOption(collName, "bm25_sparse"))
		return err == nil && desc.IndexedRows > 0
	}, 5*time.Second, 500*time.Millisecond, "bound index built before refresh materialized bm25_sparse")
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		desc, err := mc.DescribeIndex(ctx, client.NewDescribeIndexOption(collName, "bm25_sparse"))
		require.NoError(c, err)
		require.Equal(c, rowCount, desc.TotalRows)
		require.Equal(c, rowCount, desc.IndexedRows)
		require.Zero(c, desc.PendingIndexRows)
	}, 120*time.Second, 2*time.Second)

	// Keep the collection loaded so search validates the index/stats Reopen path.
	require.EventuallyWithT(t, func(c *assert.CollectT) {
