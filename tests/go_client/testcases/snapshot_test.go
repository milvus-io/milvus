...
// TestCreateSnapshotWithCompactionProtection verifies that snapshot can be created
// with a compaction_protection_seconds parameter (PR #48227).
func TestCreateSnapshotWithCompactionProtection(t *testing.T) {
	ctx := hp.CreateContext(t, time.Second*common.DefaultTimeout)
	mc := hp.CreateDefaultMilvusClient(ctx, t)
	...
	snapshotName := fmt.Sprintf("snapshot_%s", common.GenRandomString(snapshotPrefix, 6))
	createOpt := client.NewCreateSnapshotOption(snapshotName, collName).
		WithDescription("Test snapshot with compaction protection").
		WithCompactionProtectionSeconds(60)

	err = mc.CreateSnapshot(ctx, createOpt)
	common.CheckErr(t, err, true)

	resp, err := mc.DescribeSnapshot(ctx, client.NewDescribeSnapshotOption(snapshotName))
	common.CheckErr(t, err, true)
	require.Equal(t, snapshotName, resp.GetName())
	require.Equal(t, collName, resp.GetCollectionName())
...
// TestCreateSnapshotCompactionProtectionInvalid verifies proxy-side validation
// for compaction_protection_seconds (negative and exceeds 7 days) from PR #48227.
func TestCreateSnapshotCompactionProtectionInvalid(t *testing.T) {
	ctx := hp.CreateContext(t, time.Second*common.DefaultTimeout)
	mc := hp.CreateDefaultMilvusClient(ctx, t)
	...
	snapshotName1 := fmt.Sprintf("snapshot_%s", common.GenRandomString(snapshotPrefix, 6))
	err = mc.CreateSnapshot(ctx, client.NewCreateSnapshotOption(snapshotName1, collName).
		WithCompactionProtectionSeconds(-1))
