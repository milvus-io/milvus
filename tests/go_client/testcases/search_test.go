...
func TestSearchFullText(t *testing.T) {
	...
	// Create schema with text field and sparse vector field for BM25
	...
	// Add text field with analyzer enabled
	...
	// Create index on sparse vector field
	indexParams := index.NewGenericIndex("sparse", map[string]string{
		index.IndexTypeKey:  "SPARSE_INVERTED_INDEX",
		index.MetricTypeKey: "BM25",
		"drop_ratio_build":  "0.2",
	})
	idx, err := mc.CreateIndex(ctx, client.NewCreateIndexOption(collectionName, "sparse", indexParams))
	common.CheckErr(t, err, true)
	require.NotNil(t, idx)

	// Load collection
	_, err = mc.LoadCollection(ctx, client.NewLoadCollectionOption(collectionName))
	common.CheckErr(t, err, true)

	// Generate 3000 random texts
	texts := make([]string, 3000)
	for i := range texts {
		texts[i] = generateRandomText()
	}

	// Add some known texts for verification
	knownTexts := []string{
		"information retrieval is a field of study.",
		"information retrieval focuses on finding relevant information in large datasets.",
		"data mining and information retrieval overlap in research.",
		"modern information retrieval systems use machine learning techniques.",
		"semantic search improves information retrieval accuracy.",
	}
	texts = append(texts, knownTexts...)

	// Insert test data
	textColumn := column.NewColumnVarChar("text", texts)
	_, err = mc.Insert(ctx, client.NewColumnBasedInsertOption(collectionName, textColumn))
	common.CheckErr(t, err, true)

	// Flush to ensure data is persisted
	_, err = mc.Flush(ctx, client.NewFlushOption(collectionName))
	common.CheckErr(t, err, true)

	// Search with text query
	searchParams := client.NewSearchOption(collectionName, 5, []entity.Vector{
		entity.Text("what is information retrieval and its applications?"),
	}).
		WithANNSField("sparse").
		WithOutputFields([]string{"id", "text"}...).
		WithSearchParam("drop_ratio_search", "0.2")

	results, err := mc.Search(ctx, searchParams)
	common.CheckErr(t, err, true)
	require.Equal(t, 1, len(results))
	require.Equal(t, 5, results[0].IDs.Len())
}