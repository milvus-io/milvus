...
// parameterized test for addfield invalid cases
...
func TestIndexAddedField(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			name:        "InvertedIndex",
			indexType:   "INVERTED",
			createIndex: func() index.Index { return index.NewInvertedIndex() },
		},
		{
			name:          "SortedIndex",
			indexType:     "STL_SORT",
			createIndex:   func() index.Index { return index.NewSortedIndex() },
			expectedError: "STL_SORT are only supported on numeric field",
		},
		{
			name:        "TrieIndex",
			indexType:   "TRIE",
			createIndex: func() index.Index { return index.NewTrieIndex() },
		},
	}
