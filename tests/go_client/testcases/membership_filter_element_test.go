...
func TestMembershipFilterRejectedInElementPredicates(t *testing.T) {
	...
	t.Run("bloom_match inside element_filter rejected", func(t *testing.T) {
		err := query(`element_filter(structA, bloom_match(doc_int, {bf}) && $[int_val] > 0)`, "bf", bloomBlob)
		common.CheckErr(t, err, false, "bloom_match is not supported inside element_filter")
	})

	t.Run("roaring_match inside element_filter rejected", func(t *testing.T) {
		err := query(`element_filter(structA, roaring_match(doc_int, {rb}) && $[int_val] > 0)`, "rb", roaringBlob)
		common.CheckErr(t, err, false, "roaring_match is not supported inside element_filter")
	})

	t.Run("bloom_match inside MATCH_ANY rejected", func(t *testing.T) {
		err := query(`MATCH_ANY(structA, bloom_match(doc_int, {bf}) && $[int_val] > 0)`, "bf", bloomBlob)
	...
	t.Run("bloom_match on struct sub-field rejected", func(t *testing.T) {
		...
		err := query(`bloom_match(structA[int_val], {bf})`, "bf", bloomBlob)
