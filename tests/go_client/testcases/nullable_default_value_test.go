...
func VerifyNullableVectorData(t *testing.T, vt NullableVectorType, queryResult client.ResultSet, pkToVecIdx map[int64]int, originalVectors interface{}, context string) {
	...
	for i := 0; i < queryResult.ResultCount; i++ {
		...
		var score float32 = -1
		if i < len(queryResult.Scores) {
			score = queryResult.Scores[i]
		}
		...
		if origIdx, ok := pkToVecIdx[pk]; ok {
			...
			case entity.FieldTypeFloatVector:
				...
				for j := range origVec {
					if math.Abs(float64(origVec[j]-queriedVec[j])) > 1e-6 {
						t.Logf("=== MISMATCH DEBUG INFO ===")
						t.Logf("Context: %s", context)
						t.Logf("Result index: %d, PK: %d, origIdx: %d, isNull: %v, score: %f", i, pk, origIdx, isNull, score)
						t.Logf("Mismatch at element %d: expected %f, got %f, diff %f", j, origVec[j], queriedVec[j], origVec[j]-queriedVec[j])
						maxPrint := 10
						if len(origVec) < maxPrint {
							maxPrint = len(origVec)
						}
						t.Logf("Original vector (first %d): %v", maxPrint, origVec[:maxPrint])
						t.Logf("Queried vector (first %d): %v", maxPrint, queriedVec[:maxPrint])
						zeroCount := 0
						for _, v := range queriedVec {
							if v == 0 {
								zeroCount++
							}
						}
						t.Logf("Queried vector: total elements=%d, zero elements=%d", len(queriedVec), zeroCount)
						t.Logf("=== END DEBUG INFO ===")
					}
			...
			case entity.FieldTypeInt8Vector:
				...
				origVec := vectors[origIdx]
				for j := range origVec {
					if origVec[j] != queriedVec[j] {
						t.Logf("=== MISMATCH DEBUG INFO ===")
						t.Logf("Context: %s", context)
						t.Logf("Result index: %d, PK: %d, origIdx: %d, isNull: %v, score: %f", i, pk, origIdx, isNull, score)
						t.Logf("Mismatch at element %d: expected %d, got %d", j, origVec[j], queriedVec[j])
						maxPrint := 10
						if len(origVec) < maxPrint {
							maxPrint = len(origVec)
						}
			...
			case entity.FieldTypeBinaryVector:
				...
				for j := range origVec {
					if origVec[j] != queriedVec[j] {
						...
						t.Logf("Context: %s", context)
						t.Logf("Result index: %d, PK: %d, origIdx: %d, isNull: %v, score: %f", i, pk, origIdx, isNull, score)
						t.Logf("Mismatch at byte %d: expected %d, got %d", j, origVec[j], queriedVec[j])
						maxPrint := 10
						if len(origVec) < maxPrint {
							maxPrint = len(origVec)
						}
						t.Logf("Original vector (first %d bytes): %v", maxPrint, origVec[:maxPrint])
						t.Logf("Queried vector (first %d bytes): %v", maxPrint, queriedVec[:maxPrint])
						t.Logf("=== END DEBUG INFO ===")
					}
