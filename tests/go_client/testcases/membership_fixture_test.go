...
// Shared int64 fixture for both membership_match kinds: a
...
func insertIntMembershipRows(t *testing.T, ctx CtxT, mc MC, collectionName string, nullable bool) {
	insertIntMembershipRowsWithDomain(t, ctx, mc, collectionName, nullable, membershipDomain)
}

func insertIntMembershipRowsWithDomain(
	t *testing.T,
	ctx CtxT,
	mc MC,
	...
	t.Helper()
	require.Positive(t, creatorDomain)
	...
	if nullable {
		...
		for i := 0; i < membershipTotalRows; i++ {
			...
			valid[i] = true
			values = append(values, int64(i%creatorDomain))
		}
		...
		for i := 0; i < membershipTotalRows; i++ {
			creators[i] = int64(i % creatorDomain)
		}
...
// The template key differs per filter blob ("bf" or "rb"), so it is passed
// explicitly.
...
func expectedMembershipIDs(totalRows, domain int, members map[int64]struct{}) []int64 {
	ids := make([]int64, 0, totalRows)
	for i := 0; i < totalRows; i++ {
		if _, ok := members[int64(i%domain)]; ok {
...
func int64IDSet(values []int64) map[int64]struct{} {
...
// requireBloomResult verifies both sides of Bloom's approximate contract:
// every true member must be present, while false positives stay far below an
// ignored-filter/full-scan result. The 30% ceiling is deliberately much looser
// than the builders' configured 0.1%/5% FPR to avoid statistical flakiness.
func requireBloomResult(
	t *testing.T,
	...
	t.Helper()
	require.LessOrEqual(t, len(expected), universeSize, "invalid Bloom ground truth: %s", context)
	for pk := range expected {
		require.Containsf(t, got, pk, "Bloom result dropped true member id=%d: %s", pk, context)
	}

	falsePositives := len(got) - len(expected)
	nonMembers := universeSize - len(expected)
	if nonMembers == 0 {
		require.Zero(t, falsePositives, "all rows are true members: %s", context)
		return
	}
	require.Lessf(t, falsePositives*10, nonMembers*3,
		"Bloom result has %d false positives over %d non-members; filter may be ignored: %s",
		falsePositives, nonMembers, context)
}

func searchMembershipIDs(t *testing.T, ctx CtxT, mc MC, collectionName, expr, templateKey string, blob any) []int64 {
	t.Helper()

	opt := client.NewSearchOption(collectionName, membershipTotalRows,
		[]entity.Vector{entity.FloatVector(make([]float32, membershipVectorDim))}).
		WithANNSField(membershipVectorField).
		WithFilter(expr).
		WithOutputFields(membershipCreatorField).
		WithConsistencyLevel(entity.ClStrong)
	if blob != nil {
		opt.WithTemplateParam(templateKey, blob)
	}
	results, err := mc.Search(ctx, opt)
	...
	creatorColumn, ok := result.GetColumn(membershipCreatorField).(*column.ColumnInt64)
	require.True(t, ok, "requested creator_id output field must be INT64")
	require.Equal(t, result.ResultCount, creatorColumn.Len(), "output field and result counts differ")

	ids := make([]int64, 0, result.ResultCount)
	for i := 0; i < pkColumn.Len(); i++ {
		id, err := pkColumn.GetAsInt64(i)
		require.NoError(t, err)
		creator, err := creatorColumn.GetAsInt64(i)
		require.NoError(t, err)
		require.Equal(t, id%membershipDomain, creator, "creator_id projection does not match PK %d", id)
		if i > 0 {
