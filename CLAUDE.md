...
Go tests MUST use `-tags dynamic,test` and `-gcflags="./...=-N -l"` (disable optimizations/inlining) or they won't compile / mockey-based monkey patching will fail:

```bash
go test -tags dynamic,test -gcflags="./...=-N -l" -count=1 ./internal/querycoordv2/...
go test -tags dynamic,test -gcflags="./...=-N -l" -count=1 ./internal/proxy/... -run TestXxx
```
