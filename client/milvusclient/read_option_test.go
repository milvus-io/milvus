...
func (s *SearchOptionSuite) TestMinShouldMatchForTextSearch() {
	collName := "search_opt_min_should_match"
	topK := 10

	// Test SearchOption WithMinShouldMatch with valid value
	opt := NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})})
	opt = opt.WithMinShouldMatch(5)

	req, err := opt.Request()
	s.Require().NoError(err)

	searchParams := entity.KvPairsMap(req.GetSearchParams())
	minShouldMatch, ok := searchParams[spMinShouldMatch]
	s.Require().True(ok)
	s.Equal("5", minShouldMatch)

	// Test validation: negative value should be corrected to 1
	opt = NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})})
	opt = opt.WithMinShouldMatch(-5)
	s.Equal(1, opt.minShouldMatch, "Negative min_should_match should be corrected to 1")
	...
	minShouldMatch, ok = searchParams[spMinShouldMatch]
	s.Require().True(ok)
	s.Equal("1", minShouldMatch)

	// Test validation: zero value should be corrected to 1
	opt = NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})})
	opt = opt.WithMinShouldMatch(0)
	s.Equal(1, opt.minShouldMatch, "Zero min_should_match should be corrected to 1")
	...
	// Test validation: value too large should be capped at 1000
	opt = NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})})
	opt = opt.WithMinShouldMatch(5000)
	s.Equal(1000, opt.minShouldMatch, "Large min_should_match should be capped at 1000")
	...
	minShouldMatch, ok = searchParams[spMinShouldMatch]
	s.Require().True(ok)
	s.Equal("1000", minShouldMatch)

	// Test that min_should_match is not included when not set
	opt = NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})})
	// Don't call WithMinShouldMatch

	req, err = opt.Request()
	s.Require().NoError(err)

	searchParams = entity.KvPairsMap(req.GetSearchParams())
	_, ok = searchParams[spMinShouldMatch]
	s.False(ok, "min_should_match should not be included when not set")
}

func (s *SearchOptionSuite) TestMinShouldMatchChaining() {
	collName := "search_opt_chaining"
	topK := 10

	// Test method chaining with min_should_match
	opt := NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})}).
		WithANNSField("vector_field").
		WithMinShouldMatch(2).
		WithFilter("id > 100").
	...
	s.Equal(collName, req.GetCollectionName())
	s.Equal("id > 100", req.GetDsl())
	s.ElementsMatch([]string{"id", "text"}, req.GetOutputFields())

	searchParams := entity.KvPairsMap(req.GetSearchParams())

	annField, ok := searchParams[spAnnsField]
	s.Require().True(ok)
	s.Equal("vector_field", annField)

	minShouldMatch, ok := searchParams[spMinShouldMatch]
	s.Require().True(ok)
	s.Equal("2", minShouldMatch)
}

func (s *SearchOptionSuite) TestMinShouldMatchWithOtherParams() {
	collName := "search_opt_combined"
	topK := 10

	// Test min_should_match combined with other search parameters
	opt := NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})}).
		WithMinShouldMatch(4).
	...
	// Verify min_should_match
	minShouldMatch, ok := searchParams[spMinShouldMatch]
	s.Require().True(ok)
	s.Equal("4", minShouldMatch)

	// Verify other parameters are still working
	groupBy, ok := searchParams[spGroupBy]
	s.Require().True(ok)
	s.Equal("category", groupBy)

	groupSize, ok := searchParams[spGroupSize]
	s.Require().True(ok)
	s.Equal("5", groupSize)

	strictGroupSize, ok := searchParams[spStrictGroupSize]
	s.Require().True(ok)
	s.Equal("true", strictGroupSize)

	offset, ok := searchParams[spOffset]
	s.Require().True(ok)
	s.Equal("10", offset)
}

func (s *SearchOptionSuite) TestMinShouldMatchEdgeCases() {
	collName := "search_opt_edge_cases"
	topK := 10

	// Test with very large min_should_match value
	opt := NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})}).
		WithMinShouldMatch(1000)
	...
	// Test overriding min_should_match value
	opt = NewSearchOption(collName, topK, []entity.Vector{entity.FloatVector([]float32{0.1, 0.2})}).
		WithMinShouldMatch(5).
		WithMinShouldMatch(10) // Override previous value
	...
	minShouldMatch, ok = searchParams[spMinShouldMatch]
	s.Require().True(ok)
	s.Equal("10", minShouldMatch, "Should use the last set value")
}
