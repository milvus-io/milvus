...
//
// Build the blob from the SAME value domain as the target field: integer fields
// hash int64, VARCHAR fields hash UTF-8. The envelope records which domains were
...
// aggregate serialized membership-plan bytes must fit
// proxy.maxMembershipFilterPlanSize (128 MiB by default). If the request also
// carries Roaring membership filters, their aggregate estimated decoded bytes
// are independently checked against the same request budget. The complete
