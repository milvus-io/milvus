...
// BuildTLSConfig creates a tls.Config from certificate file paths.
//
// caPemPath is required — the CA certificate used to verify the server.
// clientPemPath and clientKeyPath are an optional pair — when both are provided,
// the client presents its own certificate for mutual TLS (mTLS).
