...
type ClientTelemetryManager struct {
	...
	// Key: command ID, Value: true if executed
...
func (m *ClientTelemetryManager) heartbeatLoop() {
	...
	ticker := time.NewTicker(m.getHeartbeatInterval())
	defer ticker.Stop()
...
// Commands are only executed once - duplicate commands are skipped based on command ID
func (m *ClientTelemetryManager) processProtoCommands(commands []*commonpb.ClientCommand) {
	hasPersistent := false
	maxCommandTS := m.lastCommandTimestamp.Load()
	...
	for _, cmd := range commands {
		...
		alreadyExecuted := m.executedCommands[localCmd.CommandId]
		...
		m.executedCommands[localCmd.CommandId] = true
...
func (m *ClientTelemetryManager) processCommands(commands []*ClientCommand) {
	...
	for _, cmd := range commands {
		...
		// Check if command has already been executed
		m.executedCommandsMu.RLock()
		alreadyExecuted := m.executedCommands[cmd.CommandId]
		m.executedCommandsMu.RUnlock()
		...
		// Mark command as executed
		m.executedCommandsMu.Lock()
		m.executedCommands[cmd.CommandId] = true
		m.executedCommandsMu.Unlock()
