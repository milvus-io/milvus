...
const (
	...
	// MaxEstimatedDecodedBytes bounds one bitmap. The server also uses the same
	// value as the aggregate roaring_match decoded-memory budget for one logical
	// request/plan. The estimate includes the portable body plus conservative
	// fixed overhead for every high-32 child and every Roaring32 container.
	...
	portableRoaring64MinEntryBytes = 15
)
...
func prevalidatePortableRoaring64Body(body []byte) error {
	...
	// Every valid, non-empty high container needs a 4-byte high key and at
	// least an 11-byte portable Roaring32 child (one array value under a run
	// cookie with an all-zero run bitmap). This necessary bound prevents
	// ReadFrom from allocating its three top-level slices from an
	// attacker-controlled count that the body cannot possibly contain.
