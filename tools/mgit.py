...
class GitOperations:
    ...
    def commit(message: str):
        ...
        msg_file = '.git/COMMIT_EDITMSG_TMP'
    ...
    @staticmethod
    def push(branch: str, force: bool = False):
    ...
    @staticmethod
    def get_all_changes_diff(base: str = 'origin/master') -> str:
        """Get diff of all changes from base"""
        return GitOperations.run_command(['git', 'diff', base])

    @staticmethod
    def get_all_changes_stat(base: str = 'origin/master') -> str:
        """Get diff statistics from base"""
        return GitOperations.run_command(['git', 'diff', '--stat', base])

    @staticmethod
    def get_commit_messages(base: str = 'origin/master') -> List[str]:
...
"""
    ...
    def _build_issue_prompt(self, diff: str, files: List[str], stats: str, issue_type: str) -> str:
        ...
        type_desc = type_descriptions.get(issue_type, 'a change')
...
1. A clear, concise issue title (≤80 characters)
...
Return ONLY a JSON object with this exact format:
{{"title": "Issue title here", "body": "Detailed issue description here"}}
...
class GitHubOperations:
    ...
    def create_pr(title: str, body: str, branch: str, issue_type: str) -> str:
        ...
        Returns: PR URL
        """
        # Get current user (fork owner)
        user = GitOperations.run_command(['gh', 'api', 'user', '-q', '.login'])
...
def workflow_rebase():
    ...
    # 4. Fetch latest master
    print_info("Fetching latest master...")
    try:
        GitOperations.fetch('origin', 'master')
        print_success("Fetched origin/master")
    except Exception as e:
    ...
    success, error = GitOperations.rebase('origin/master')
    ...
    if new_commit_count <= 1:
        ...
        if new_commit_count == 1:
            ...
            if UserInteraction.confirm("\nForce push to origin?", default=True):
                try:
                    GitOperations.push(branch, force=True)
                    print_success(f"Force pushed to origin/{branch}")
                except Exception as e:
    ...
    diff = GitOperations.get_all_changes_diff('origin/master')
    stats = GitOperations.get_all_changes_stat('origin/master')
    ...
    try:
        GitOperations.reset_soft('origin/master')
...
def workflow_commit():
    ...
    if UserInteraction.confirm("Run code formatting tools (make fmt)?", default=True):
        ...
        try:
            ...
            result = subprocess.run(
                ['make', 'fmt'],
                cwd='/Users/xiaofanluan/code/milvus',
                capture_output=True,
                text=True,
                timeout=120
            )
...
def workflow_pr():
    ...
    if commit_count > 1:
        ...
        print_info(f"Consider running: git rebase -i origin/master")
    ...
    if branch != 'master':
        ...
        if commit_count > 0:
            ...
            if behind_count and int(behind_count) > 0:
                print_warning(f"Branch is {behind_count} commit(s) behind master")
