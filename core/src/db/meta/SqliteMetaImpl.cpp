...
SqliteMetaImpl::GetCollectionFiles(const std::string& collection_id, const std::vector<size_t>& ids,
    ...
    try {
        ...
        ENGINE_LOG_DEBUG << "Get " << selected.size() << " files by id from collection " << collection_id;
        return result;
    } catch (std::exception& e) {
...
SqliteMetaImpl::GetCollectionFilesBySegmentId(const std::string& segment_id,
    ...
    try {
        ...
        ENGINE_LOG_DEBUG << "Get " << selected.size() << " files by segment id" << segment_id;
        return Status::OK();
    } catch (std::exception& e) {
...
SqliteMetaImpl::FilesToSearch(const std::string& collection_id, FilesHolder& files_holder) {
    ...
    try {
        ...
        if (files_count == 0) {
            ...
            ENGINE_LOG_DEBUG << "Collect " << files_count << " to-search files";
        }
        return ret;
    } catch (std::exception& e) {
...
SqliteMetaImpl::FilesToMerge(const std::string& collection_id, FilesHolder& files_holder) {
    try {
        ...
        if (files_count > 0) {
            ENGINE_LOG_TRACE << "Collect " << files_count << " to-merge files";
        }
        return result;
    } catch (std::exception& e) {
