...
FPGAIVFPQ::QueryImpl(int64_t n, const float* data, int64_t k, float* distances, int64_t* labels, const Config& config,
    ...
    try {
        ...
        auto Fpga = Fpga::FpgaInst::GetInstance();
        std::vector<faiss::Index::idx_t> vlabels(n * k), filterlabels(n * k);

        std::vector<float> vdistances(n * k), filterdistances(n * k);
        auto t0 = Elapsed();
        // do query
        ...
        int sumelem = elems;
        for (int i = 0; i < n * k; i++) {
            if (bitset != nullptr && bitset->test((faiss::ConcurrentBitset::id_type_t)vlabels[i])) {
                sumelem = sumelem - 1;
                continue;
            }
            filterlabels.push_back(vlabels[i]);
            filterdistances.push_back(vdistances[i]);
        }
        //
        size_t p_id_size = sizeof(int64_t) * sumelem;
        size_t p_dist_size = sizeof(float) * sumelem;
        memcpy(distances, filterdistances.data(), p_dist_size);
        memcpy(labels, filterlabels.data(), p_id_size);
    } catch (...) {
    }
}
}  // namespace knowhere
