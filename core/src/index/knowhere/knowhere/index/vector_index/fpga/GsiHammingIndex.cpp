...
void
GsiHammingIndex::CopyIndexToFpga(uint32_t row_count, const std::string& location, std::string collection_name) {
    num_bfeatures_ = Dim();
    num_bytes_in_rec_ = num_bfeatures_ / CHAR_BITS;
    index_size_ = row_count * num_bytes_in_rec_;

    auto apu = Fpga::ApuInst::getInstance();
    apu->load(num_bfeatures_, row_count, location, Fpga::APU_METRIC_TYPE::HAMMING, collection_name);
}
...
GsiHammingIndex::Query(const DatasetPtr& dataset, const Config& config, faiss::ConcurrentBitsetPtr blacklist) {
    ...
    apu->Query(indices_, distances_, queries_, Fpga::APU_METRIC_TYPE::HAMMING, topK_);
