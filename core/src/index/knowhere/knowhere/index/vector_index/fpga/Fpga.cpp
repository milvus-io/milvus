...
void FpgaInterface::GetDevAddr()
{
    if (verbose){
        printf("Cal device addr\n");
    }
    ...
    if (verbose){
        printf("ci_s: %lu, tm2_s: %lu, db_s: %lu, pq_s: %lu, que_s: %lu\n", ci_s, tm2_s, db_s, pq_s, que_s);
    }

}

void FpgaInterface::GetConfigTable(idx_t   nprobe)
{
    step1_s = que_s + 1020;    
...
void FpgaInterface::TransFile2FPGA(const char *filename, loff_t base, size_t offset)
{
    uint32_t file_size;
...
char * FpgaInterface::LoadFile2Mem(const char *filename, size_t &file_size, char *buffer)
{
    // size_t file_size;
    loff_t base_addr;
...
void FpgaInterface::TransMem2FPGA(char *buffer, size_t size, loff_t offset)
{
    char *buffer0 = NULL;
    char *buffer1 = NULL;
    char *buffer2 = NULL;
...
void FpgaInterface::WriteFileFromFPGA(const char *filename, size_t size, loff_t base, size_t offset)
{
...
char * FpgaInterface::LoadFPGA2Mem(loff_t fpga_addr, char *buffer, size_t size)
{
    // char *buffer = NULL;
...
void FpgaInterface::SaveMem2File(char *buffer, size_t size, const char *filename)
{
...
int FpgaInterface::OpenCtrlDev()
{
    return open_dev((char *)DEVICE_CTRL);
}

int FpgaInterface::CloseCtrlDev(int fd)
{
    return close_dev(fd);
}

int FpgaInterface::WritePara(int fd, uint32_t addr, uint32_t val)
{
    return reg_rw(fd, PARA_BASE_ADDR + addr, (char *)"write", 'w', val);
}

int FpgaInterface::ReadPara(int fd, uint32_t addr)
{
    return reg_rw(fd, PARA_BASE_ADDR + addr, (char *)"read", 'w', 0);
}

int FpgaInterface::WriteReg(int fd, uint32_t addr, uint32_t val)
{
    return reg_rw(fd, REG_BASE_ADDR + addr, (char *)"write", 'w', val);
}

int FpgaInterface::ReadReg(int fd, uint32_t addr)
{
    return reg_rw(fd, REG_BASE_ADDR + addr, (char *)"read", 'w', 0);
}

void FpgaInterface::TriggerSearch(int fd)
{
    WritePara(fd, TRIGGER_REG_ADDR, 0);
    WriteReg(fd, 0, (0x1 << 6));
}

uint32_t FpgaInterface::GetSearchTime(int fd)
{
    int index;
    vector<uint32_t> vec(8, 0);
    ...
    if(verbose){
        printf("Search in FPGA time is: %d (ms)\n", search_time);
    }
...
template <typename T>
static arma::Cube<T> CubeTranspose (arma::Cube<T>& cube){
...
static void cal_term2(faiss::IndexIVFPQ *index, std::vector<double> *dis_term2, vector<double> *all_codes, vector<int> *all_list_size, vector<idx_t> *all_ids) {
    ...
    for (size_t i = 0; i < index->nlist; i++) {
        ...
        for (size_t j = 0; j < index->invlists->list_size(i); j++) {
            ...
            for (size_t m = 0; m < index->pq.M; m++) {
                ...
                dis_term2->at(term2_count) += index->precomputed_table[ i * index->pq.M * index->pq.ksub + m * index->pq.ksub + codes[m] ];
            }
...
static void cal_ci(faiss::IndexIVFPQ *index, vector<float> &ci_vec)
{
...
void FpgaInterface::CopyIndexToFPGA()
{
    int dim = d / m;

    {
        // Step1: Transfer coarse_centroids
        vector<float> ci(nlist*d, 0);
        cal_ci(index_ptr, ci);
        
        TransMem2FPGA((char *)ci.data(), ci.size()*sizeof(float), ci_s * 64);
        if(verbose){
            printf("Transfer CI to FPGA, size: %lu bytes, addr: 0x%lx\n", ci.size()*sizeof(float), ci_s * 64);
        }
...

    {
        ...
        if (verbose){
            printf("Transfer PQ quantizer to FPGA, size: %llu bytes, addr: 0x%lx\n", PQ.size()*sizeof(float), pq_s*64);
        }
...

    {
        ...
        vector<double> dis_term2(nb, 0);
        vector<double> all_codes(nb*m, 0);
        ...
        vector<uint8_t> database_u8(database.size());
        for(size_t i = 0; i < database.size(); i++){
            database_u8[i] = (uint8_t) database[i];
        }
        TransMem2FPGA((char *)database_u8.data(), database_u8.size(), db_s * 64);
        if (verbose){
            printf("Transfer database, size: %lu, addr: 0x%lx\n", database_u8.size(), db_s*64);
        }

        // term2
        index_base = 0;
        vector<double> term2_temp;
        for (size_t i = 0; i < all_list_size.size(); i++){
            int current_size = all_list_size[i];
        ...
        vector<float> term2_flt(term2_temp.size());
        ...
        if (verbose){
            printf("Transfer term2, size: %lu, addr: 0x%lx\n", term2_flt.size(), tm2_s*64);
        }
    }
}

int FpgaInterface::GetDatabaseListAlign(vector<int> &data_list, uint32_t align, vector<int> &align_list)
{
    auto iter = data_list.begin();
    int temp;

    while(iter != data_list.end()){
        temp = (int) (ceil((float) *iter / align) * align);     // round up the value
...
void FpgaInterface::GetDatabaseListSum(vector<int> &data_vec, vector<int> &sum_vec)
{
    int temp = 0;
...
void FpgaInterface::GetProbeDatabaseAddressList(uint32_t base_addr, vector<int> &data_vec, int align, vector<uint32_t> &addr_vec, uint32_t m)
{
    uint32_t addr;
    ...
    while(iter != sum_vec.end()){
        addr = base_addr + ((*iter) * m / 256);
        addr_vec.push_back(addr);
        iter++;
    }
}

void FpgaInterface::GetProbeTerm2AddressList(uint32_t base_addr, vector<int> &data_vec, int align, vector<uint32_t> &addr_vec)
{
    vector<int> offset_vec;
    vector<int> sum_vec;
    uint32_t addr;
...
int FpgaInterface::CfgDataBaseNumReal(int fd)
{
    int index = 0;
    vector<uint32_t> database_vec;
    vector<uint32_t> term2_vec;   
...
void FpgaInterface::TransferCfg()
{
    int index = 0;
    int fd;
    ...
    fd = OpenCtrlDev();
    if (fd < 0){
        printf("Error, open /dev/xdma0_bypass fail\n");
    }
...
void FpgaInterface::FlushResultMem()
{
...
void FpgaInterface::StartFPGASearch()
{
    int index = 0;
    int fd = -1;
    int value = 0;
   ...
   // milvus::LOG_ENGINE_DEBUG_ << " Trigger FPGA search";
    if(index == 10000){
        printf("Error Trigger FPGA search fail\n");
    }
...
void ReadSingleBlock(char *buffer, vector<uint32_t> &label_vec, vector<float> &dis_vec)
{
    int index;
    uint32_t *label = NULL;
    float *distance = NULL;
    ...
    for(index = 0; index < 8; index++){
        ...
        distance = (float *) (buffer+index*8+4);
        label_vec.push_back(*label);
        dis_vec.push_back(*distance);
    }
}

void FpgaInterface::ReadResultFile(vector<idx_t> &id_vec, vector<float> &dis_vec)
{
    uint32_t index;
    uint32_t offset = step3_s * 64;
    ...
    vector<uint32_t> id_vec_0;
    vector<uint32_t> id_vec_1;
    vector<float> dis_vec_0;
    vector<float> dis_vec_1;
...
bool FpgaInterface::BatchAnnQuery(idx_t   nprobe,idx_t batch_size, float *items, idx_t expect_k, vector<idx_t> &id_vec, vector<float> &dis_vec)
{
    topk = expect_k;
