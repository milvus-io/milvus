...
BinaryIVF::Search(const DatasetPtr& dataset, const Config& config) {
    ...
    try {
        ...
        search_impl(rows, (uint8_t*)p_data, config[meta::TOPK], p_dist, p_id, config);
...
BinaryIVF::Train(const DatasetPtr& dataset, const Config& config) {
    ...
    faiss::IndexBinary* coarse_quantizer = new faiss::IndexBinaryFlat(dim, GetMetricType(config[Metric::TYPE]));
...
BinaryIVF::SearchById(const DatasetPtr& dataset, const Config& config) {
    ...
    try {
        ...
        index_->search_by_id(rows, p_data, config[meta::TOPK], pdistances, p_id, bitset_);
