...
INSTANTIATE_TEST_CASE_P(METRICParameters, BinaryIDMAPTest,
                        Values(std::string("HAMMING"), std::string("JACCARD"), std::string("TANIMOTO"), std::string("SUPERSTRUCTURE"), std::string("SUBSTRUCTURE")));
        ...
        for (auto i = 0; i < nq; ++ i) {
            ...
            for (auto j = 0; j < nb; ++ j) {
                ...
                if (dist < jaccard_radius) {
                    idmap[i][j] = true;
                    bf_cnt[i] ++;
//                    std::cout << "query_id = " << i << ", ans_j = " << j << ", dis_j = " << dist << std::endl;
                }
            }
        }
    };
...
//                    std::cout << "query_id = " << i << ", ans_j = " << j << ", dis_j = " << dist << std::endl;
    ...
    auto superstructure_dis = [] (const int64_t *pa, const int64_t *pb) -> bool {
        return ((*pa) & (*pb)) == (*pb);
    };
    auto bruteforce_sup = [&] () {
        for (auto i = 0; i < nq; ++ i) {
    ...
    auto substructure_dis = [] (const int64_t *pa, const int64_t *pb) -> bool {
        return ((*pa) & (*pb)) == (*pa);
    };
    auto bruteforce_sub = [&] () {
        for (auto i = 0; i < nq; ++ i) {
...
//    std::cout << "current radius = " << conf[milvus::knowhere::IndexParams::range_search_radius].get<float>() << std::endl;
...
//                    std::cout << "query_id = " << i << ", ans_j = " << res_space->buffers[bno].ids[pos] << ", dis_j = " << res_space->buffers[bno].dis[pos] << std::endl;
