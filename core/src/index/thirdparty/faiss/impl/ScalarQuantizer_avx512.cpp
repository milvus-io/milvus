...
typedef ScalarQuantizer_avx512::QuantizerType QuantizerType;
typedef ScalarQuantizer_avx512::RangeStat RangeStat;
using SQDistanceComputer = ScalarQuantizer_avx512::SQDistanceComputer;
...
struct QuantizerTemplate<Codec, true, 1>: ScalarQuantizer_avx512::Quantizer {
    const size_t d;
...
template<class Codec>
struct QuantizerTemplate<Codec, false, 1>: ScalarQuantizer_avx512::Quantizer {
    const size_t d;
    const float *vmin, *vdiff;
...
template<>
struct QuantizerFP16<1>: ScalarQuantizer_avx512::Quantizer {
    const size_t d;
...
template<>
struct Quantizer8bitDirect<1>: ScalarQuantizer_avx512::Quantizer {
    const size_t d;
                   ...
                   std::vector<float> & trained)
{
    ...
    if (rs == ScalarQuantizer_avx512::RS_minmax) {
        ...
        vmin -= vexp;
        vmax += vexp;
    } else if (rs == ScalarQuantizer_avx512::RS_meanstd) {
        double sum = 0, sum2 = 0;
        for (size_t i = 0; i < n; i++) {
            sum += x[i];
            sum2 += x[i] * x[i];
    ...
    } else if (rs == ScalarQuantizer_avx512::RS_quantiles) {
    ...
    } else if (rs == ScalarQuantizer_avx512::RS_optim) {
        float a, b;
        float sx = 0;
        {
            vmin = HUGE_VAL, vmax = -HUGE_VAL;
...
ScalarQuantizer_avx512::ScalarQuantizer_avx512
...
void ScalarQuantizer_avx512::train (size_t n, const float *x)
{
    int bit_per_dim =
...
void ScalarQuantizer_avx512::train_residual(size_t n,
                                     const float *x,
                                     Index *quantizer,
                                     bool by_residual,
                                     bool verbose)
...
ScalarQuantizer_avx512::Quantizer *ScalarQuantizer_avx512::select_quantizer () const
{
#ifdef USE_AVX_512
    if (d % 16 == 0) {
...
void ScalarQuantizer_avx512::compute_codes (const float * x,
                                     uint8_t * codes,
                                     size_t n) const
{
...
void ScalarQuantizer_avx512::decode (const uint8_t *codes, float *x, size_t n) const
{
    std::unique_ptr<Quantizer> squant(select_quantizer ());
...
SQDistanceComputer *
ScalarQuantizer_avx512::get_distance_computer (MetricType metric) const
{
...
InvertedListScanner* sel2_InvertedListScanner
      (const ScalarQuantizer_avx512 *sq,
...
InvertedListScanner* sel1_InvertedListScanner
        (const ScalarQuantizer_avx512 *sq, const Index *quantizer,
...
InvertedListScanner* sel0_InvertedListScanner
        (MetricType mt, const ScalarQuantizer_avx512 *sq,
...
InvertedListScanner* ScalarQuantizer_avx512::select_InvertedListScanner
