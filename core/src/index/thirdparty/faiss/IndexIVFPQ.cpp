...
struct KnnSearchResults {
    ...
    inline void add (idx_t j, float dis, const BitsetViewPtr bitset) {
...
struct RangeSearchResults {
    ...
    inline void add (idx_t j, float dis, const faiss::BitsetViewPtr bitset) {
        if (C::cmp (radius, dis)) {
            idx_t id = ids ? ids[j] : lo_build (key, j);
            rres.add (dis, id);
        }
    ...
    InvertedListScanner
{
    ...
    size_t scan_codes (size_t ncode,
                       ...
                       size_t k,
                       const faiss::BitsetViewPtr bitset) const override
    {
