...
#pragma omp parallel reduction(+: nadd)
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (list_no >= 0 && list_no % nt == rank) {
                ...
                invlists->add_entry_without_resize(list_no, id, one_code.data(), hist[list_no] ++);
    ...
    if (DirectMap::Type::NoMap != dm_add.direct_map.type) {
        for (auto i = 0; i < nlist; i ++) {
            auto idsi = invlists->get_ids(i);
            auto sizei = invlists->list_size(i);
            for (auto j = 0; j < sizei; j ++)
                dm_add.direct_map.add_single_id(idsi[j], i, j);
        }
    }
