...
public:
    ...
    // for milvus
    virtual void search(NGT::SearchContainer & sc, const faiss::BitsetViewPtr bitset) { getIndex().search(sc, bitset); }
...
public:
    ...
    // for milvus
    virtual void search(NGT::SearchContainer & sc, const faiss::BitsetViewPtr bitset)
    {
        sc.distanceComputationCount = 0;
        sc.visitCount = 0;
        ObjectDistances seeds;
...
protected:
    ...
    // for milvus
    virtual void search(NGT::SearchContainer & sc, ObjectDistances & seeds, const faiss::BitsetViewPtr bitset)
    {
        if (sc.size == 0)
        {
            while (!sc.workingResult.empty())
...
public:
    ...
    // for milvus
    void
    getSeedsFromTree(NGT::SearchContainer& sc, ObjectDistances& seeds, const faiss::BitsetViewPtr bitset) {
        DVPTree::SearchContainer tso(sc.object);
        tso.mode = DVPTree::SearchContainer::SearchLeaf;
        tso.radius = 0.0;
        tso.size = 1;
    ...
    // for milvus
    void search(NGT::SearchContainer & sc, const faiss::BitsetViewPtr bitset)
    {
        sc.distanceComputationCount = 0;
        sc.visitCount = 0;
        ObjectDistances seeds;
