...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
    searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef, const faiss::BitsetViewPtr bitset, StatisticsInfo &stats) const {
        VisitedList *vl = visited_list_pool_->getFreeVisitedList();
        vl_type *visited_array = vl->mass;
        vl_type visited_array_tag = vl->curV;
...
//                        if (!has_deletions || !isMarkedDeleted(candidate_id))
    ...
    searchKnn(const void *query_data, size_t k, const faiss::BitsetViewPtr bitset, StatisticsInfo &stats) const {
    ...
    searchKnn(const void* query_data, size_t k, Comp comp, const faiss::BitsetViewPtr bitset, StatisticsInfo &stats) {
