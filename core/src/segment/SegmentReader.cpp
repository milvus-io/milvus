...
SegmentReader::Load() {
    ...
    try {
        ...
        default_codec.GetVectorsFormat()->read(operation_ptr_, segment_ptr_->vectors_ptr_);
        default_codec.GetDeletedDocsFormat()->read(operation_ptr_, segment_ptr_->deleted_docs_ptr_);
...
SegmentReader::LoadVectors(off_t offset, size_t num_bytes, std::vector<uint8_t>& raw_vectors) {
    ...
    try {
        operation_ptr_->CreateDirectory();
        default_codec.GetVectorsFormat()->read_vectors(operation_ptr_, offset, num_bytes, raw_vectors);
    } catch (std::exception& e) {
...
SegmentReader::LoadUids(std::vector<doc_id_t>& uids) {
    ...
    try {
        operation_ptr_->CreateDirectory();
        default_codec.GetVectorsFormat()->read_uids(operation_ptr_, uids);
    } catch (std::exception& e) {
...
SegmentReader::LoadBloomFilter(segment::IdBloomFilterPtr& id_bloom_filter_ptr) {
    ...
    try {
        operation_ptr_->CreateDirectory();
        default_codec.GetIdBloomFilterFormat()->read(operation_ptr_, id_bloom_filter_ptr);
    } catch (std::exception& e) {
...
SegmentReader::LoadDeletedDocs(segment::DeletedDocsPtr& deleted_docs_ptr) {
    ...
    try {
        operation_ptr_->CreateDirectory();
        default_codec.GetDeletedDocsFormat()->read(operation_ptr_, deleted_docs_ptr);
    } catch (std::exception& e) {
