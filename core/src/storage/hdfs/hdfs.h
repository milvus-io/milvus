...
LIBHDFS_EXTERNAL
int
hdfsFileIsOpenForRead(hdfsFile file);
...
LIBHDFS_EXTERNAL
int
hdfsFileIsOpenForWrite(hdfsFile file);
...
LIBHDFS_EXTERNAL
int
hdfsFileGetReadStatistics(hdfsFile file, struct hdfsReadStatistics** stats);
...
LIBHDFS_EXTERNAL
int
hdfsFileClearReadStatistics(hdfsFile file);
...
LIBHDFS_EXTERNAL
void
hdfsFileFreeReadStatistics(struct hdfsReadStatistics* stats);
...
LIBHDFS_EXTERNAL
int
hdfsGetHedgedReadMetrics(hdfsFS fs, struct hdfsHedgedReadMetrics** metrics);
...
LIBHDFS_EXTERNAL
void
hdfsFreeHedgedReadMetrics(struct hdfsHedgedReadMetrics* metrics);
...
LIBHDFS_EXTERNAL
hdfsFS
hdfsConnectAsUser(const char* nn, tPort port, const char* user);
...
LIBHDFS_EXTERNAL
hdfsFS
hdfsConnect(const char* nn, tPort port);
...
LIBHDFS_EXTERNAL
hdfsFS
hdfsConnectNewInstance(const char* nn, tPort port);
...
LIBHDFS_EXTERNAL
hdfsFS
hdfsBuilderConnect(struct hdfsBuilder* bld);
...
LIBHDFS_EXTERNAL
void
hdfsBuilderSetForceNewInstance(struct hdfsBuilder* bld);
...
LIBHDFS_EXTERNAL
void
hdfsBuilderSetNameNode(struct hdfsBuilder* bld, const char* nn);
...
LIBHDFS_EXTERNAL
void
hdfsBuilderSetNameNodePort(struct hdfsBuilder* bld, tPort port);
...
LIBHDFS_EXTERNAL
void
hdfsBuilderSetUserName(struct hdfsBuilder* bld, const char* userName);
...
LIBHDFS_EXTERNAL
void
hdfsBuilderSetKerbTicketCachePath(struct hdfsBuilder* bld, const char* kerbTicketCachePath);
...
LIBHDFS_EXTERNAL
void
hdfsFreeBuilder(struct hdfsBuilder* bld);
...
LIBHDFS_EXTERNAL
int
hdfsBuilderConfSetStr(struct hdfsBuilder* bld, const char* key, const char* val);
...
LIBHDFS_EXTERNAL
int
hdfsConfGetStr(const char* key, char** val);
...
LIBHDFS_EXTERNAL
int
hdfsConfGetInt(const char* key, int32_t* val);
 ...
 * @param flags - an | of bits/fcntl.h file flags - supported flags are O_RDONLY, O_WRONLY (meaning create or overwrite
 * i.e., implies O_TRUNCAT), O_WRONLY|O_APPEND. Other flags are generally ignored other than (O_RDWR || (O_EXCL &
...
LIBHDFS_EXTERNAL
hdfsFile
hdfsOpenFile(hdfsFS fs, const char* path, int flags, int bufferSize, short replication, tSize blocksize);
...
LIBHDFS_EXTERNAL
void
hdfsStreamBuilderFree(struct hdfsStreamBuilder* bld);
...
LIBHDFS_EXTERNAL
int
hdfsStreamBuilderSetBufferSize(struct hdfsStreamBuilder* bld, int32_t bufferSize);
...
LIBHDFS_EXTERNAL
int
hdfsStreamBuilderSetReplication(struct hdfsStreamBuilder* bld, int16_t replication);
...
LIBHDFS_EXTERNAL
int
hdfsStreamBuilderSetDefaultBlockSize(struct hdfsStreamBuilder* bld, int64_t defaultBlockSize);
...
LIBHDFS_EXTERNAL
hdfsFile
hdfsStreamBuilderBuild(struct hdfsStreamBuilder* bld);
...
LIBHDFS_EXTERNAL
int
hdfsTruncateFile(hdfsFS fs, const char* path, tOffset newlength);
...
LIBHDFS_EXTERNAL
int
hdfsUnbufferFile(hdfsFile file);
...
LIBHDFS_EXTERNAL
int
hdfsHFlush(hdfsFS fs, hdfsFile file);
...
LIBHDFS_EXTERNAL
int
hdfsHSync(hdfsFS fs, hdfsFile file);
...
LIBHDFS_EXTERNAL
int
hdfsMove(hdfsFS srcFS, const char* src, hdfsFS dstFS, const char* dst);
...
LIBHDFS_EXTERNAL
int
hdfsDelete(hdfsFS fs, const char* path, int recursive);
...
LIBHDFS_EXTERNAL
int
hdfsFileIsEncrypted(hdfsFileInfo* hdfsFileInfo);
...
LIBHDFS_EXTERNAL
tOffset
hdfsGetDefaultBlockSizeAtPath(hdfsFS fs, const char* path);
...
LIBHDFS_EXTERNAL
int
hadoopRzOptionsSetSkipChecksum(struct hadoopRzOptions* opts, int skip);
...
LIBHDFS_EXTERNAL
int
hadoopRzOptionsSetByteBufferPool(struct hadoopRzOptions* opts, const char* className);
...
LIBHDFS_EXTERNAL
void
hadoopRzOptionsFree(struct hadoopRzOptions* opts);
...
LIBHDFS_EXTERNAL
int32_t
hadoopRzBufferLength(const struct hadoopRzBuffer* buffer);
...
LIBHDFS_EXTERNAL
const void*
hadoopRzBufferGet(const struct hadoopRzBuffer* buffer);
...
LIBHDFS_EXTERNAL
void
hadoopRzBufferFree(hdfsFile file, struct hadoopRzBuffer* buffer);
