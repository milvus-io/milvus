...
SSBuildIndexTask::SSBuildIndexTask(const std::string& collection_name, engine::snapshot::ID_TYPE segment_id,
                                   TaskLabelPtr label)
    : Task(TaskType::BuildIndexTask, std::move(label)), collection_name_(collection_name), segment_id_(segment_id) {
}
...
SSBuildIndexTask::Load(milvus::scheduler::LoadType type, uint8_t device_id) {
    ...
    //            stat = Status(SERVER_UNEXPECTED_ERROR, error_msg);
    //        }
    //        fiu_do_on("XSSBuildIndexTask.Load.out_of_memory", stat = Status(SERVER_UNEXPECTED_ERROR, "out of
    //        memory")); if (!stat.ok()) {
    //            Status s;
    ...
    //        //
    //        //        std::string info = "Build index task load file id:" + std::to_string(file_->id_) + " " +
    //        type_str +
    //        //                           " file type:" + std::to_string(file_->file_type_) + " size:" +
    //        //                           std::to_string(file_size) + " bytes from location: " + file_->location_ + "
    //        totally
    //        //                           cost";
...
SSBuildIndexTask::Execute() {
    ...
    //        //            index = to_index_engine_->BuildIndex(table_file.location_,
    //        (EngineType)table_file.engine_type_);
    //        //            fiu_do_on("XSSBuildIndexTask.Execute.build_index_fail", index = nullptr);
    //        //            if (index == nullptr) {
    //        //                std::string log_msg = "Failed to build index " + table_file.file_id_ + ", reason: source
    //        index
    ...
    //        //
    //        //        if (!has_collection) {
    //        //            std::string msg = "Failed to build index " + table_file.file_id_ + ", reason: collection has
    //        been
    ...
    //        //            if (!status.ok()) {
    //        //                std::string msg =
    //        //                    "Failed to persist index file: " + table_file.location_ + ", reason: " +
    //        status.message();
    ...
    //        //        }
    //        //
    //        //        fiu_do_on("XSSBuildIndexTask.Execute.update_table_file_fail", status =
