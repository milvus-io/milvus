...
ResourcePtr
PickResource(const TaskPtr& task,
             const std::vector<int64_t>& device_ids,
             int64_t idx, std::string name)
{
    ...
    if (did < 0) {
        ...
        LOG_SERVER_DEBUG_ << LogOut("[%s][%d] %s: nq >= gpu_search_threshold, specify gpu %d to search!",
                                    "search", 0, name.c_str(), device_ids[idx]);
