...
// source /internal/mq/msgstream/msg.go
...
func (it *InsertMsg) IsRowBased() bool {
	return it.GetVersion() == pb.InsertDataVersion_RowBased
}

func (it *InsertMsg) IsColumnBased() bool {
	return it.GetVersion() == pb.InsertDataVersion_ColumnBased
}
...
func (it *InsertMsg) CheckAligned() error {
	...
	if it.IsColumnBased() {
		for _, field := range it.FieldsData {
			fieldNumRows, err := util.GetNumRowOfFieldData(field)
			if err != nil {
				return err
			}
...
func (it *InsertMsg) rowBasedIndexRequest(index int) pb.InsertRequest {
	return pb.InsertRequest{
		Base: util.NewMsgBase(
			util.WithMsgType(commonpb.MsgType_Insert),
			util.WithMsgID(it.Base.MsgID),
			util.WithTimeStamp(it.Timestamps[index]),
			util.WithSourceID(it.Base.SourceID),
		),
		DbID:           it.DbID,
		...
		Version:        pb.InsertDataVersion_RowBased,
	}
}

func (it *InsertMsg) columnBasedIndexRequest(index int) pb.InsertRequest {
	colNum := len(it.GetFieldsData())
	fieldsData := make([]*schemapb.FieldData, colNum)
	util.AppendFieldData(fieldsData, it.GetFieldsData(), int64(index))
	return pb.InsertRequest{
		Base: util.NewMsgBase(
			util.WithMsgType(commonpb.MsgType_Insert),
			util.WithMsgID(it.Base.MsgID),
		...
		NumRows:        1,
		Version:        pb.InsertDataVersion_ColumnBased,
	}
}

func (it *InsertMsg) IndexRequest(index int) pb.InsertRequest {
	if it.IsRowBased() {
...
func (dt *DeleteMsg) CheckAligned() error {
	...
	numPks := int64(util.GetSizeOfIDs(dt.PrimaryKeys))
	if numRows != numPks {
...
func (dp *DropPartitionMsg) Unmarshal(input MarshalType) (TsMsg, error) {
	dropPartitionRequest := pb.DropPartitionRequest{}
	in, err := convertToByteArray(input)
	if err != nil {
		return nil, err
	}
