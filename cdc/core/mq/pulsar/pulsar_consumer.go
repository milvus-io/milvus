...
func (pc *Consumer) Chan() <-chan api.Message {
	if pc.msgChannel == nil {
		pc.once.Do(func() {
			pc.msgChannel = make(chan api.Message, 256)
			...
			go func() {
				for { //nolint:gosimple
					...
					case msg, ok := <-pc.c.Chan():
						if !ok {
							util.Log.Debug("pulsar consumer channel closed")
							return
						}
						if !pc.skip {
...
func (pc *Consumer) Seek(id api.MessageID, inclusive bool) error {
...
func (pc *Consumer) Ack(message api.Message) {
...
func (pc *Consumer) Close() {
	pc.closeOnce.Do(func() {
		...
		fn := func() error {
			...
			if err != nil {
				...
				if strings.Contains(err.Error(), "Consumer not found") {
					util.Log.Warn("failed to find consumer, skip unsubscribe",
		...
		if err != nil {
			util.Log.Error("failed to unsubscribe", zap.String("subscription", pc.Subscription()), zap.Error(err))
			panic(err)
		}
		close(pc.closeCh)
	})
...
func (pc *Consumer) GetLatestMsgID() (api.MessageID, error) {
	msgID, err := pc.c.GetLastMessageID(pc.c.Name(), api.DefaultPartitionIdx)
