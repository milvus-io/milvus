...
func main() {
	...
	if idx > 0 {
		...
		log.Println("run subprocess with cmd:", args) //nolint:gosec // G706 logging command-line args is intentional

		/* #nosec G204 */
		cmd := exec.Command(args[0], args[1:]...) //nolint:gosec // G702 command args come from os.Args, not user input
