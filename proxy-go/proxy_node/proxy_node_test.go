...
func TestProxyNode(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			if uint64(curValue[0]) < value {
				t.Fatalf("current time stamp = %d less than saved time stamp = %d", curTime, value)
			}
			if value < startTime {
				t.Fatalf("saved time stamp = %d less than start time stamp = %d", value, startTime)
			}
	...
	func() {
		...
		for {
			...
			case cm, ok := <-tickComsumer.Chan():
				if !ok {
					t.Fatal("time tick consumer topic has closed")
				}
				tickComsumer.AckID(cm.ID())
			case cm, ok := <-reader.Chan():
				if !ok {
					t.Fatal("reader comsumer topic has closed")
				}
				reader.AckID(cm.ID())
			case cm, ok := <-deleteC.Chan():
				if !ok {
					t.Fatalf("delete topic has closed")
				}
				deleteC.AckID(cm.ID())
			case cm, ok := <-query.Chan():
				if !ok {
					t.Fatal("query topic has closed")
				}
				query.AckID(cm.ID())
			}
		}
	...
	go func() {
		...
		for {
			...
			if curT <= lastT {
				t.Fatalf("current time tick = %d, last time tick = %d", curT, lastT)
			}
			lastT = curT
		}
	}()
	...
	go func() {
		cm, ok := <-query.Chan()
		if !ok {
			t.Fatalf("query topic has closed")
		}
		query.AckID(cm.ID())
		...
		if qm.ProxyId != testOpt.proxyId {
			t.Fatalf("query id incorrect")
		}
		if qm.CollectionName != "cm100" {
			t.Fatalf("collection name incorrect")
		}
		t.Logf("query time stamp = %d", ToPhysicalTime(qm.Timestamp))
		if ToPhysicalTime(qm.Timestamp) < startTime {
			t.Fatalf("query time stamp incorrect")
		}
	...
	if insertR.Status.ErrorCode != pb.ErrorCode_SUCCESS {
		t.Fatal(insertR.Status.Reason)
	}
	if len(insertR.EntityIdArray) != 6 {
	...
	for i := 0; i < len(insertR.EntityIdArray); i++ {
		if insertR.EntityIdArray[i] != int64(i+10) {
			t.Fatal("insert error")
		}
	}
	...
	if m1.CollectionName != "cm100" {
		t.Fatalf("insert failed, collection name = %s", m1.CollectionName)
	}
	if len(m1.PrimaryKeys) != len(m1.RowsData) {
		t.Fatalf("insert failed")
	}
	t.Logf("reader time stamp = %d", ToPhysicalTime(m1.Timestamp))
	if ToPhysicalTime(m1.Timestamp) < startTime {
		t.Fatalf("insert failed")
	}
	...
	if m2.CollectionName != "cm100" {
		t.Fatalf("insert failed, collection name = %s", m2.CollectionName)
	}
	if len(m2.PrimaryKeys) != len(m2.RowsData) {
		t.Fatalf("insert failed")
	}
	t.Logf("read time stamp = %d", ToPhysicalTime(m2.Timestamp))
	if ToPhysicalTime(m2.Timestamp) < startTime {
		t.Fatalf("insert failed")
	}
	...
	if len(insertPrimaryKey) != 6 {
		t.Fatal("insert failed")
	}
	for i := 0; i < len(insertPrimaryKey); i++ {
		if insertPrimaryKey[i] != uint64(i+10) {
			t.Fatal("insert failed")
		}
	}
	...
	if deleteR.ErrorCode != pb.ErrorCode_SUCCESS {
		t.Fatal(deleteR.Reason)
	}
	deleteM, ok := <-deleteC.Chan()
	if !ok {
		t.Fatal("delete topic close")
	}
	...
	if dm.CollectionName != "cm100" {
		t.Fatal("delete failed")
	}
	if len(dm.PrimaryKeys) != 2 {
		t.Fatal("delete failed")
	}
	t.Logf("delete time stamp = %d", ToPhysicalTime(dm.Timestamp))
	if ToPhysicalTime(dm.Timestamp) < startTime {
		t.Fatalf("delete failed")
	}
	for i := 0; i < len(dm.PrimaryKeys); i++ {
		if dm.PrimaryKeys[i] != uint64(i+20) {
			t.Fatal("delete failed")
		}
	}
	...
	if searchR.Status.ErrorCode != pb.ErrorCode_SUCCESS {
		t.Fatal(searchR.Status.Reason)
	}
	if searchR.Entities.Status.ErrorCode != pb.ErrorCode_SUCCESS {
		t.Fatalf(searchR.Entities.Status.Reason)
	}

	if len(searchR.Entities.Ids) != 3 {
		t.Fatalf("query failed")
	}
	if searchR.Entities.Ids[0] != 16 {
		t.Fatalf("query failed")
	}
	if searchR.Entities.Ids[1] != 15 {
		t.Fatalf("query failed")
	}
	if searchR.Entities.Ids[2] != 13 {
		t.Fatalf("query failed")
	}

	if len(searchR.Entities.ValidRow) != 3 {
		t.Fatal("query failed")
	}
	if searchR.Entities.ValidRow[0] != true {
		t.Fatal("query failed")
	}
	if searchR.Entities.ValidRow[1] != true {
		t.Fatal("query failed")
	}
	if searchR.Entities.ValidRow[2] != true {
		t.Fatal("query failed")
	}

	if len(searchR.Entities.RowsData) != 3 {
		t.Fatal("query failed")
	}
	if binary.LittleEndian.Uint64(searchR.Entities.RowsData[0].Blob) != 16 {
		t.Fatal("query failed")
	}
	if binary.LittleEndian.Uint64(searchR.Entities.RowsData[1].Blob) != 15 {
		t.Fatal("query failed")
	}
	if binary.LittleEndian.Uint64(searchR.Entities.RowsData[2].Blob) != 13 {
		t.Fatal("query failed")
	}

	if len(searchR.Scores) != 3 {
		t.Fatal("query failed")
	}
	if searchR.Scores[0] != 16 {
		t.Fatal("query failed")
	}
	if searchR.Scores[1] != 15 {
		t.Fatal("query failed")
	}
	if searchR.Scores[2] != 13 {
		t.Fatal("query failed")
	}

	if len(searchR.Distances) != 3 {
		t.Fatal("query failed")
	}
	if searchR.Distances[0] != 16 {
		t.Fatal("query failed")
	}
	if searchR.Distances[1] != 15 {
		t.Fatal("query failed")
	}
	if searchR.Distances[2] != 13 {
		t.Fatal("query failed")
	}
