...
func NewWriteBuffer(channel string, meta meta.MetaCache, syncMgr syncmgr.SyncManager, opts ...WriteBufferOption) (WriteBuffer, error) {
	option := defaultWBOption(meta)
	...
	case DeletePolicyBFPkOracle:
		return NewBFWriteBuffer(channel, meta, syncMgr, option)
	case DeletePolicyL0Delta:
		return NewL0WriteBuffer(channel, meta, syncMgr, option)
	default:
...
func newWriteBufferBase(channel string, meta meta.MetaCache, syncMgr syncmgr.SyncManager, option *writeBufferOption) (*writeBufferBase, error) {
	flushTs := atomic.NewUint64(nonFlushTS)
	flushTsPolicy := GetFlushTsPolicy(flushTs, meta)
...
func (wb *writeBufferBase) cleanupCompactedSegments() {
	segmentIDs := wb.metaCache.GetSegmentIDsBy(
		meta.WithSegmentState(commonpb.SegmentState_Dropped),
		meta.WithCompacted(),
		meta.WithNoSyncingTask())
	...
	removed := wb.metaCache.RemoveSegments(meta.WithSegmentIDs(targetIDs...))
	if len(removed) > 0 {
...
func (wb *writeBufferBase) sealSegments(_ context.Context, segmentIDs []int64) error {
	...
	wb.metaCache.UpdateSegments(meta.UpdateState(commonpb.SegmentState_Sealed),
		meta.WithSegmentIDs(segmentIDs...),
		meta.WithSegmentState(commonpb.SegmentState_Growing))
	return nil
}
...
func (wb *writeBufferBase) bufferInsert(inData *inData, startPos, endPos *msgpb.MsgPosition) error {
	...
	if !ok {
		...
		}, func(_ *datapb.SegmentInfo) *meta.BloomFilterSet {
			return meta.NewBloomFilterSetWithBatchSize(wb.getEstBatchSize())
		}, meta.SetStartPosRecorded(false))
	}

	segBuf := wb.getOrCreateBuffer(inData.segmentID)

	totalMemSize := segBuf.insertBuffer.Buffer(inData, startPos, endPos)
	wb.metaCache.UpdateSegments(meta.UpdateBufferedRows(segBuf.insertBuffer.rows),
...
func (wb *writeBufferBase) getSyncTask(ctx context.Context, segmentID int64) (syncmgr.Task, error) {
	...
	segmentInfo, ok := wb.metaCache.GetSegmentByID(segmentID) // wb.metaCache.GetSegmentsBy(meta.WithSegmentIDs(segmentID))
	if !ok {
	...
	actions = append(actions, meta.StartSyncing(batchSize))
	wb.metaCache.UpdateSegments(meta.MergeSegmentAction(actions...), meta.WithSegmentIDs(segmentID))
