...
func NewL0WriteBuffer(channel string, metacache meta.MetaCache, syncMgr syncmgr.SyncManager, option *writeBufferOption) (WriteBuffer, error) {
...
func (wb *l0WriteBuffer) BufferData(insertMsgs []*msgstream.InsertMsg, deleteMsgs []*msgstream.DeleteMsg, startPos, endPos *msgpb.MsgPosition) error {
	...
	for _, inData := range groups {
		...
		segments := wb.metaCache.GetSegmentsBy(meta.WithSegmentIDs(inData.segmentID))
		for _, segment := range segments {
...
func (wb *l0WriteBuffer) getL0SegmentID(partitionID int64, startPos *msgpb.MsgPosition) int64 {
	...
	if !ok {
		...
		}, func(_ *datapb.SegmentInfo) *meta.BloomFilterSet { return meta.NewBloomFilterSet() }, meta.SetStartPosRecorded(false))
