...
func TestDataSyncService_newDataSyncService(t *testing.T) {
	...
	node := datanode.newIDLEDataNodeMock(ctx, schemapb.DataType_Int64)
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			ds, err := newDataSyncService(ctx, node, getWatchInfo(test))
...
func TestDataSyncService_newDataSyncService_DuplicatedChannel(t *testing.T) {
	...
	ds, err := newDataSyncService(ctx, node, watchInfo)
...
func TestGetChannelWithTickler(t *testing.T) {
	...
	node := datanode.newIDLEDataNodeMock(context.Background(), schemapb.DataType_Int64)
	...
	meta := datanode.NewMetaFactory().GetCollectionMeta(1, "test_collection", schemapb.DataType_Int64)
	...
	metaCache, err := getMetaCache(context.TODO(), node, info, unflushed, flushed, nil)
	assert.NoError(t, err)
...
type DataSyncServiceSuite struct {
	suite.Suite
	datanode.MockDataSuiteBase

	node         *server.DataNode // node param
...
func (s *DataSyncServiceSuite) SetupTest() {
	...
	s.node.channelCheckpointUpdater = datanode.newChannelCheckpointUpdater(s.node.broker)
...
func (s *DataSyncServiceSuite) TestStartStop() {
	...
	sync, err := newDataSyncService(ctx, s.node, watchInfo)
	...
	dataFactory := datanode.NewDataFactory()
