...
func (t *SyncTask) Run() (err error) {
	...
	t.metacache.UpdateSegments(meta.MergeSegmentAction(actions...), meta.WithSegmentIDs(t.segment.SegmentID()))
...
func (t *SyncTask) prefetchIDs() error {
	...
	for i := 0; i < totalIDCount; i++ {
		id, err := t.allocator.Allocate(ctx)
		if err != nil {
			return err
		}
		t.ids = append(t.ids, int64(id))
	}
	return nil
}
