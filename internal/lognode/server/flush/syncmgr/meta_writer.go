...
func (b *brokerMetaWriter) UpdateSync(pack *SyncTask) error {
	...
	startPos := lo.Map(pack.metacache.GetSegmentsBy(meta.WithSegmentState(commonpb.SegmentState_Growing, commonpb.SegmentState_Sealed, commonpb.SegmentState_Flushing),
		meta.WithStartPosNotRecorded()), func(info *meta.SegmentInfo, _ int) *datapb.SegmentStartPosition {
	...
	pack.metacache.UpdateSegments(meta.SetStartPosRecorded(true), meta.WithSegmentIDs(lo.Map(startPos, func(pos *datapb.SegmentStartPosition, _ int) int64 { return pos.GetSegmentID() })...))
...
func (b *brokerMetaWriter) UpdateSyncV2(pack *SyncTaskV2) error {
	...
	startPos := lo.Map(pack.metacache.GetSegmentsBy(meta.WithSegmentState(commonpb.SegmentState_Growing, commonpb.SegmentState_Flushing),
		meta.WithStartPosNotRecorded()), func(info *meta.SegmentInfo, _ int) *datapb.SegmentStartPosition {
		return &datapb.SegmentStartPosition{
			SegmentID:     info.SegmentID(),
			StartPosition: info.StartPosition(),
