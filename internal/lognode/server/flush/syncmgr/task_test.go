...
func (s *SyncTaskSuite) SetupTest() {
	...
	s.metacache = meta.NewMockMetaCache(s.T())
}
...
func (s *SyncTaskSuite) TestRunNormal() {
	...
	s.metacache.EXPECT().GetSegmentsBy(mock.Anything, mock.Anything).Return([]*meta.SegmentInfo{seg})
...
func (s *SyncTaskSuite) TestRunL0Segment() {
	s.broker.EXPECT().SaveBinlogPaths(mock.Anything, mock.Anything).Return(nil)
	bfs := meta.NewBloomFilterSet()
	seg := meta.NewSegmentInfo(&datapb.SegmentInfo{Level: datapb.SegmentLevel_L0}, bfs)
	s.metacache.EXPECT().GetSegmentByID(s.segmentID).Return(seg, true)
	s.metacache.EXPECT().GetSegmentsBy(mock.Anything, mock.Anything).Return([]*meta.SegmentInfo{seg})
	s.metacache.EXPECT().UpdateSegments(mock.Anything, mock.Anything).Return()
...
func (s *SyncTaskSuite) TestCompactToNull() {
	...
	seg := meta.NewSegmentInfo(&datapb.SegmentInfo{}, bfs)
	meta.UpdateNumOfRows(1000)(seg)
	meta.CompactTo(metacache.NullSegment)(seg)
	seg.GetBloomFilterSet().Roll()
	s.metacache.EXPECT().GetSegmentByID(s.segmentID).Return(seg, true)
...
func (s *SyncTaskSuite) TestRunError() {
	s.Run("target_segment_not_match", func() {
		flag := false
		seg := meta.NewSegmentInfo(&datapb.SegmentInfo{
			ID: s.segmentID,
		}, meta.NewBloomFilterSet())
		meta.CompactTo(s.segmentID + 1)(seg)
	...
	seg := meta.NewSegmentInfo(&datapb.SegmentInfo{}, meta.NewBloomFilterSet())
	meta.UpdateNumOfRows(1000)(seg)
	s.metacache.EXPECT().GetSegmentByID(s.segmentID).Return(seg, true)
	s.metacache.EXPECT().GetSegmentsBy(mock.Anything, mock.Anything).Return([]*meta.SegmentInfo{seg})
...
func (s *SyncTaskSuite) TestCalcTargetID() {
	...
	s.Run("normal_calc_segment", func() {
		s.Run("not_compacted", func() {
			segment := meta.NewSegmentInfo(&datapb.SegmentInfo{
				ID:           s.segmentID,
		...
		s.Run("compacted_null", func() {
			...
			meta.CompactTo(metacache.NullSegment)(segment)
			s.metacache.EXPECT().GetSegmentByID(s.segmentID).Return(segment, true).Once()
