...
func (t *SyncTaskV2) Run() error {
	...
	actions := []meta.SegmentAction{meta.FinishSyncing(t.batchSize)}
	switch {
	case t.isDrop:
		actions = append(actions, meta.UpdateState(commonpb.SegmentState_Dropped))
	case t.isFlush:
		actions = append(actions, meta.UpdateState(commonpb.SegmentState_Flushed))
	}

	t.metacache.UpdateSegments(meta.MergeSegmentAction(actions...), meta.WithSegmentIDs(t.segmentID))
...
func (t *SyncTaskV2) WithAllocator(allocator timestamp.Allocator) *SyncTaskV2 {
...
func (t *SyncTaskV2) WithMetaCache(metacache meta.MetaCache) *SyncTaskV2 {
