...
func NewStorageSerializer(metacache meta.MetaCache, metaWriter MetaWriter) (*storageV1Serializer, error) {
...
func (s *storageV1Serializer) EncodeBuffer(ctx context.Context, pack *SyncPack) (Task, error) {
	...
	if pack.insertData != nil {
		...
		s.metacache.UpdateSegments(meta.RollStats(singlePKStats), meta.WithSegmentIDs(pack.segmentID))
	}
