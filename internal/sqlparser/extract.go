...
var vectorDistFuncs = map[string]VectorDistOp{
	...
	"hamming_distance": VectorDistHamming,
	"jaccard_distance": VectorDistJaccard,
}
...
func extractVectorDistFunc(fc *pg_query.FuncCall, op VectorDistOp) (*VectorDistExpr, error) {
	...
	if pr := arg.GetParamRef(); pr != nil {
		...
		vector, literal, ok, err := extractVectorLiteral(arg)
		if err != nil {
			return nil, err
		}
		if ok {
...
func extractVectorDistOp(ae *pg_query.A_Expr, op VectorDistOp) (*VectorDistExpr, error) {
	...
	if pr := rexpr.GetParamRef(); pr != nil {
		...
		vector, literal, ok, err := extractVectorLiteral(rexpr)
		if err != nil {
			return nil, err
		}
		if ok {
...
// '[1, 2, 3]' or '[1, 2, 3]'::vector into float32 values. For typed literals
// that are not float arrays, it preserves the raw literal so the proxy can
// build a field-type-specific PlaceholderGroup after schema resolution.
func extractVectorLiteral(node *pg_query.Node) ([]float32, *VectorLiteral, bool, error) {
	return extractVectorLiteralWithType(node, "")
}

func extractVectorLiteralWithType(node *pg_query.Node, typeName string) ([]float32, *VectorLiteral, bool, error) {
	if node == nil {
		return nil, nil, false, nil
	}

	if tc := node.GetTypeCast(); tc != nil {
		return extractVectorLiteralWithType(tc.Arg, extractTypeName(tc.TypeName))
	}
	...
	if err != nil {
		trimmed := strings.TrimSpace(sv.Sval)
		if typeName == "" || strings.EqualFold(typeName, "vector") || strings.HasPrefix(trimmed, "[") {
			return nil, nil, true, err
		}
		return nil, &VectorLiteral{Raw: sv.Sval, TypeName: typeName}, true, nil
	}
	return vector, &VectorLiteral{Raw: sv.Sval, TypeName: typeName}, true, nil
}
