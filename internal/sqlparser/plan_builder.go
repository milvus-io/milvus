...
func BuildQueryPlan(comp *SqlComponents, schema *typeutil.SchemaHelper) (*planpb.PlanNode, error) {
	return BuildQueryPlanWithTemplateValues(comp, schema, nil)
}

// BuildQueryPlanWithTemplateValues converts SqlComponents IR into a Milvus
// PlanNode and binds SQL WHERE template values produced from ParamRef nodes.
func BuildQueryPlanWithTemplateValues(comp *SqlComponents, schema *typeutil.SchemaHelper, templateValues map[string]*schemapb.TemplateValue) (*planpb.PlanNode, error) {
	// 1. Parse WHERE -> Expr proto via planparserv2
	var predicates *planpb.Expr
	if comp.Where != nil && comp.Where.MilvusExpr != "" {
		plan, err := planparserv2.CreateRetrievePlan(schema, comp.Where.MilvusExpr, templateValues)
		if err != nil {
...
func BuildSearchPlan(comp *SqlComponents, schema *typeutil.SchemaHelper) (*planpb.PlanNode, error) {
	return BuildSearchPlanWithTemplateValues(comp, schema, nil)
}

// BuildSearchPlanWithTemplateValues converts SqlComponents with a
// VectorSearchDef into a search PlanNode and binds SQL WHERE template values.
func BuildSearchPlanWithTemplateValues(comp *SqlComponents, schema *typeutil.SchemaHelper, templateValues map[string]*schemapb.TemplateValue) (*planpb.PlanNode, error) {
