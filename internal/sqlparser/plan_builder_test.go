...
func buildParamTestSchema(t *testing.T) *typeutil.SchemaHelper {
	fields := []*schemapb.FieldSchema{
		{FieldID: 1, Name: "id", DataType: schemapb.DataType_Int64, IsPrimaryKey: true},
		{FieldID: 2, Name: "age", DataType: schemapb.DataType_Int64},
		{FieldID: 3, Name: "name", DataType: schemapb.DataType_VarChar},
	}
	schema := &schemapb.CollectionSchema{
		Name:   "users",
		AutoID: false,
...
func TestBuildQueryPlan_WithSQLParams(t *testing.T) {
	schema := buildParamTestSchema(t)
	comp := &SqlComponents{
		Collection: "users",
		SelectItems: []SelectItem{
		...
		Where: &WhereClause{
			MilvusExpr: `(age > {sql_param_1}) and (name == {sql_param_2})`,
		},
	}

	templateValues, err := BuildSQLParamTemplateValues(map[string]*schemapb.TemplateValue{
		"$1": {Val: &schemapb.TemplateValue_Int64Val{Int64Val: 18}},
		"$2": {Val: &schemapb.TemplateValue_StringVal{StringVal: "alice"}},
	})
	require.NoError(t, err)

	plan, err := BuildQueryPlanWithTemplateValues(comp, schema, templateValues)
	require.NoError(t, err)
	require.NotNil(t, plan.GetQuery().GetPredicates())
}
...
func TestBuildTypedPlaceholderGroupFromRaw_Binary(t *testing.T) {
	...
	data, err := BuildTypedPlaceholderGroupFromRaw("10100001", field)
	require.NoError(t, err)

	var pg commonpb.PlaceholderGroup
	require.NoError(t, proto.Unmarshal(data, &pg))
	require.Len(t, pg.Placeholders, 1)
	assert.Equal(t, commonpb.PlaceholderType_BinaryVector, pg.Placeholders[0].Type)
	assert.Equal(t, []byte{0xa1}, pg.Placeholders[0].Values[0])
}

func TestBuildTypedPlaceholderGroupFromRaw_Sparse(t *testing.T) {
	field := &schemapb.FieldSchema{Name: "sparse", DataType: schemapb.DataType_SparseFloatVector}

	data, err := BuildTypedPlaceholderGroupFromRaw("{3:0.8,1:0.5}/1000", field)
	require.NoError(t, err)

	var pg commonpb.PlaceholderGroup
	require.NoError(t, proto.Unmarshal(data, &pg))
	require.Len(t, pg.Placeholders, 1)
	assert.Equal(t, commonpb.PlaceholderType_SparseFloatVector, pg.Placeholders[0].Type)
	require.Len(t, pg.Placeholders[0].Values, 1)

	value := pg.Placeholders[0].Values[0]
	require.Len(t, value, 16)
	assert.Equal(t, uint32(1), binary.LittleEndian.Uint32(value[0:4]))
	assert.Equal(t, float32(0.5), math.Float32frombits(binary.LittleEndian.Uint32(value[4:8])))
	assert.Equal(t, uint32(3), binary.LittleEndian.Uint32(value[8:12]))
	assert.Equal(t, float32(0.8), math.Float32frombits(binary.LittleEndian.Uint32(value[12:16])))
}
