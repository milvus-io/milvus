...
const (
	...
	VectorDistHamming                     // <~>  Hamming distance
	VectorDistJaccard                     // <%>  Jaccard distance
)
...
func (op VectorDistOp) MetricType() string {
	...
	case VectorDistIP:
		return "IP"
	case VectorDistHamming:
		return "HAMMING"
	case VectorDistJaccard:
		return "JACCARD"
	default:
		return "L2"
	}
...
// VectorLiteral stores a vector literal whose concrete placeholder type must be
// resolved later from the target field schema.
type VectorLiteral struct {
	Raw      string // Original string literal without SQL quotes/cast syntax
	TypeName string // Optional cast type, e.g. vector, bit, sparsevec
}
