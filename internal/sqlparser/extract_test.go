...
func TestExtractWhereEscapesStringLiteral(t *testing.T) {
	comp, err := ExtractSqlComponents(`SELECT id FROM tbl WHERE name = 'a"b'`)
	require.NoError(t, err)
	require.NotNil(t, comp.Where)
	assert.Equal(t, "name == "+strconv.Quote(`a"b`), comp.Where.MilvusExpr)
}

func TestExtractWhereParamRef(t *testing.T) {
	comp, err := ExtractSqlComponents("SELECT id FROM tbl WHERE age > $1 AND name = $2")
	require.NoError(t, err)
	require.NotNil(t, comp.Where)
	assert.Equal(t, "(age > {sql_param_1}) and (name == {sql_param_2})", comp.Where.MilvusExpr)
}
...
func TestExtractVectorDistBinaryOperators(t *testing.T) {
	sql := `SELECT id, embedding <~> '01010101'::bit AS distance FROM articles ORDER BY distance LIMIT 10`
	comp, err := ExtractSqlComponents(sql)
	require.NoError(t, err)

	assert.Equal(t, QueryTypeSearch, comp.QueryType)
	require.NotNil(t, comp.VectorSearch)
	assert.Equal(t, "HAMMING", comp.VectorSearch.MetricType)
	require.NotNil(t, comp.VectorSearch.Literal)
	assert.Equal(t, "01010101", comp.VectorSearch.Literal.Raw)
	assert.Equal(t, "bit", comp.VectorSearch.Literal.TypeName)

	sql = `SELECT id, embedding <%> '11110000'::bit AS distance FROM articles LIMIT 5`
	comp, err = ExtractSqlComponents(sql)
	require.NoError(t, err)
	require.NotNil(t, comp.VectorSearch)
	assert.Equal(t, "JACCARD", comp.VectorSearch.MetricType)
}

func TestExtractVectorDistSparseLiteral(t *testing.T) {
	sql := `SELECT id, embedding <#> '{1:0.5,3:0.8}/1000'::sparsevec AS score FROM articles LIMIT 10`
	comp, err := ExtractSqlComponents(sql)
	require.NoError(t, err)
	...
	assert.Equal(t, "IP", comp.VectorSearch.MetricType)
	require.NotNil(t, comp.VectorSearch.Literal)
	assert.Equal(t, "{1:0.5,3:0.8}/1000", comp.VectorSearch.Literal.Raw)
	assert.Equal(t, "sparsevec", comp.VectorSearch.Literal.TypeName)
	assert.Empty(t, comp.VectorSearch.Vector)
}
