...
func (s *CommonSuite) TestIsStorageV2() {
	tests := []struct {
		name     string
		deltalog *datapb.FieldBinlog
		want     bool
	}{
		{
			name: "storage v1 - no child fields",
			deltalog: &datapb.FieldBinlog{
				FieldID: 1,
		...
		{
			name: "storage v2 - has child fields",
			deltalog: &datapb.FieldBinlog{
				FieldID: 1,
				Binlogs: []*datapb.Binlog{
					{LogPath: "/path/to/log"},
...
func (s *CommonSuite) TestComposeDeleteFromDeltalogs() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			pk2Ts, err := ComposeDeleteFromDeltalogs(ctx, tt.pkType, deltalogs, options...)
			s.NoError(err)
			s.NotNil(pk2Ts)
