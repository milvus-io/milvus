...
func TestHandlerClient(t *testing.T) {
	...
	producer, err := handler.CreateProducer(ctx, &options.ProducerOptions{VChannel: "pchannel_dml"})
	assert.NoError(t, err)
	assert.NotNil(t, producer)
	producer2, err := handler.CreateProducer(ctx, &options.ProducerOptions{VChannel: "pchannel_dml"})
	assert.NoError(t, err)
	assert.NotNil(t, producer)
	p.EXPECT().IsAvailable().Unset()
	p.EXPECT().IsAvailable().Return(false)
	producer3, err := handler.CreateProducer(ctx, &options.ProducerOptions{VChannel: "pchannel_dml"})
	assert.NoError(t, err)
	...
	producer4, err := handler.CreateProducer(ctx, &options.ProducerOptions{VChannel: "pchannel_dml"})
	assert.NoError(t, err)
	assert.NotNil(t, producer4)
	producer4.Close()

	consumer, err := handler.CreateConsumer(ctx, &options.ConsumerOptions{
		VChannel:      "pchannel_dml",
