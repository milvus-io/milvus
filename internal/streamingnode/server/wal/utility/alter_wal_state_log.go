...
func AlterWALStateLogFields(state *streamingpb.AlterWALState) []zap.Field {
	if state == nil {
		return nil
	}
	return []zap.Field{
		zap.String("alterWALStage", state.GetStage().String()),
		zap.String("targetWALName", state.GetTargetWalName().String()),
		zap.Int("alterWALConfigCount", len(state.GetConfigs())),
	}
}
