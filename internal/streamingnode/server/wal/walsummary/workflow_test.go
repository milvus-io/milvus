...
func testPosition(tt uint64) *streamingpb.PChannelSummaryPosition {
	return &streamingpb.PChannelSummaryPosition{TimeTick: tt, MessageId: walimplstest.NewTestMessageID(int64(tt)).IntoProto()}
}
...
func TestConcurrentCompletionAndFirstPublication(t *testing.T) {
	...
	require.Equal(t, uint64(99), m.LastAcked().TimeTick)
	require.NoError(t, first.Execute(ctx))
	require.Len(t, m.Manifest().Chunks, 3)
	require.Equal(t, uint64(99), m.LastAcked().TimeTick, "first manifest must be discoverable")
	...
	require.Equal(t, uint64(300), m.LastAcked().TimeTick)
...
func TestPublishedTermConfirmsAndRecoversUnpublishedTail(t *testing.T) {
	...
	require.Equal(t, uint64(100), m.LastAcked().TimeTick)
	...
	puts.UnPatch()
	require.Equal(t, uint64(100), successor.LastAcked().TimeTick)
...
func TestManifestPublicationRetainsConcurrentUpdates(t *testing.T) {
	...
	disk, _, err := store.ReadManifest(ctx)
	require.NoError(t, err)
	require.Equal(t, uint64(200), disk.CoveredPosition.TimeTick)
}
...
func TestNodeSchedulerUploadsInParallel(t *testing.T) {
	...
	patch := mockey.Mock((*Store).WriteChunk).To(func(s *Store, ctx context.Context, gen uint64, sections map[string]*ChunkSections, positions ...*streamingpb.PChannelSummaryPosition) (*streamingpb.PChannelSummaryChunkFooter, uint64, error) {
		...
		payload, footer, err := marshalChunk(s.PChannel(), gen, s.Term(), sections, positions...)
		if err != nil {
			return nil, 0, err
		}
		if gen == 0 {
	...
	require.Empty(t, m.Manifest().Chunks)
	close(release)
	require.Eventually(t, func() bool { return !m.HasPendingWork() && m.LastAcked().TimeTick == 200 }, 3*time.Second, time.Millisecond)
}
...
func TestRecoveryPrefixBoundaryAndTermFilter(t *testing.T) {
	...
	for _, gen := range []uint64{98, 99, 100, 102} {
		_, _, err := store.WriteChunk(ctx, gen, writeSections(map[string][]uint64{"v1": {gen + 1}}), testPosition(gen+1))
		require.NoError(t, err)
	}
	foreign := nextTermStore(store)
	_, _, err := foreign.WriteChunk(ctx, 101, writeSections(map[string][]uint64{"v1": {102}}), testPosition(102))
	require.NoError(t, err)
	var prefixes []string
...
func TestRecoveryEmptyManifestAndGenerationGap(t *testing.T) {
	...
	_, _, err := store.WriteChunk(ctx, 2, writeSections(map[string][]uint64{"v1": {300}}), testPosition(300))
	require.NoError(t, err)
	successor := newTestManager(t, nextTermStore(store), 1<<30)
	require.NoError(t, successor.Restore(ctx))
	...
	require.Equal(t, uint64(200), latest.LastAcked().TimeTick)
...
func TestRecoveryRejectsNewerOwnerAndSameTermKeyReuse(t *testing.T) {
	...
	require.Equal(t, uint64(100), reopened.LastAcked().TimeTick)
...
func TestRecoveryDoesNotAdoptUndiscoverableTerm(t *testing.T) {
	ctx := context.Background()
	store := newTestStore(t)
	_, _, err := store.WriteChunk(ctx, 0, writeSections(map[string][]uint64{"v1": {100}}), testPosition(100))
	require.NoError(t, err)
	m := newTestManager(t, nextTermStore(store), 1<<30)
	require.NoError(t, m.Restore(ctx))
	require.Empty(t, m.Manifest().Chunks)
	require.Nil(t, m.LastAcked())
	require.NoError(t, drainSummary(ctx, m))
}
...
func TestManifestValidationAndGenerationExhaustion(t *testing.T) {
	for _, bad := range []*streamingpb.PChannelSummaryManifest{
		{CoveredPosition: testPosition(1)},
		{LastChunk: &streamingpb.PChannelSummaryChunkRef{}},
		{Chunks: []*streamingpb.PChannelSummaryChunkIndexEntry{{Generation: 1}}},
		{LastChunk: &streamingpb.PChannelSummaryChunkRef{Generation: 2}, CoveredPosition: testPosition(100), Chunks: []*streamingpb.PChannelSummaryChunkIndexEntry{{Generation: 1, StartTimetick: 100, EndTimetick: 99}}},
	} {
		requireSummaryError(t, validateManifest(bad), ErrStoreCorrupted)
	}
	badPosition := testPosition(1)
	badPosition.MessageId.WALName = -123
	requireSummaryError(t, validateSummaryPosition(badPosition), ErrStoreCorrupted)
...
func TestTailTransientFailureAndWrongIdentity(t *testing.T) {
	...
	patch.UnPatch()
	payload, _, err := marshalChunk("wrong-channel", 0, store.Term(), writeSections(map[string][]uint64{"v1": {100}}), testPosition(100))
	require.NoError(t, err)
...
func TestSweepProtectsFutureTermAndCurrentTail(t *testing.T) {
	...
	for _, s := range []*Store{store, nextTermStore(store)} {
		for _, gen := range []uint64{0, 1} {
			_, _, err := s.WriteChunk(ctx, gen, writeSections(map[string][]uint64{"v1": {gen + 1}}))
			require.NoError(t, err)
		}
	}
	n, finished, err := store.sweepGarbage(ctx, store.Term(), &streamingpb.PChannelSummaryChunkRef{Generation: 0, Term: store.Term()}, nil, 1)
	require.NoError(t, err)
	require.Equal(t, 1, n)
...
func TestGCRetainsWorkArrivingDuringSweep(t *testing.T) {
	...
	require.NoError(t, m.GCOnce(ctx))
	entered, release := make(chan struct{}), make(chan struct{})
	var original func(*Store, context.Context, int64, *streamingpb.PChannelSummaryChunkRef, map[ChunkRef]struct{}, int) (int, bool, error)
	patch := mockey.Mock((*Store).sweepGarbage).Origin(&original).To(func(s *Store, ctx context.Context, term int64, last *streamingpb.PChannelSummaryChunkRef, refs map[ChunkRef]struct{}, budget int) (int, bool, error) {
