...
// produces a record. *finalized is set on return: the summary keeps no
...
// TestPersistSurfacesStoreCorruption covers what a terminal store error does: it
// fails the persist.
//
...
// keyed writes seals a small chunk per persist cycle, so the byte budget can
...
// the given vchannel: it produces no record, so observing it stages nothing.
...
func newTestIdempotentInsertMessage(
	...
	if key != "" {
		builder = builder.WithIdempotencyKey(message.IdempotencyKey(key))
	}
...
// sub-message. Reading only Insert/CommitTxn would stage nothing here, so the
// key would be absent from the window after a restart.
