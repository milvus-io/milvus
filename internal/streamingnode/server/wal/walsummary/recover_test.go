...
func TestRestoreInheritsPreviousTermChunks(t *testing.T) {
	...
	// The durable backlog through the inherited chunk is readable: recovery
	// rebuilds the idempotency window from it.
...
func TestRestoreInheritsSkipsBurnedIntermediateTerm(t *testing.T) {
	...
	manager3 := newTestManager(t, store3, 1<<30)
	require.NoError(t, manager3.Restore(ctx))
	...
	sections, err := manager3.ReadIdempotencyEntries(ctx, "v1", 0, 1000)
	require.NoError(t, err)
	require.Len(t, sections.Inserts, 1)
	assert.Equal(t, uint64(100), sections.Inserts[0].GetSourceTimetick())
}
