...
func TestMarshalUnmarshalChunkRoundTrip(t *testing.T) {
	...
	payload, footer, err := marshalChunk("p1", 7, 1, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
	assert.NotNil(t, footer)
	assert.Equal(t, uint64(7), footer.GetGeneration())
	assert.Equal(t, int64(1), footer.GetTerm())
	assert.Equal(t, uint64(99), footer.GetStartTimetick())
...
func TestUnmarshalChunkCorrupted(t *testing.T) {
	...
	payload, _, err := marshalChunk("p1", 1, 1, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
...
func TestStoreWriteReadChunk(t *testing.T) {
	...
	footer, _, err := store.WriteChunk(ctx, 0, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
	...
	_, _, err = store.WriteChunk(ctx, 0, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)

	// different content at the same generation is corruption.
	_, _, err = store.WriteChunk(ctx, 0, writeSections(map[string][]uint64{
		"v1": {101},
	}), testRecordRange(writeSections(map[string][]uint64{
		"v1": {101},
	})))
	assert.Error(t, err)
	assert.True(t, errors.Is(err, ErrStoreCorrupted))
...
func TestStoreWriteChunkRewriteIsIdempotent(t *testing.T) {
	...
	payload, _, err := marshalChunk("p1", 7, 3, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
	payload[11] ^= 0xff
	assert.NoError(t, cm.Write(ctx, NewStore(cm, "p1", 3).ChunkKey(7), payload))
	_, _, err = NewStore(cm, "p1", 3).WriteChunk(ctx, 7, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
	...
	foreign, _, err := marshalChunk("p1", 7, 5, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
...
func TestStoreProbeChunkForward(t *testing.T) {
	...
	_, _, err := store.WriteChunk(ctx, 1, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
	_, _, err = store.WriteChunk(ctx, 2, writeSections(records), testRecordRange(writeSections(records)))
	assert.NoError(t, err)
...
func TestMarshalUnmarshalIdempotencySectionsRoundTrip(t *testing.T) {
	...
	payload, footer, err := marshalChunk("p1", 7, 1, sections, testRecordRange(sections))
	require.NoError(t, err)
...
func TestMarshalIdempotencySectionOmittedWithoutKeys(t *testing.T) {
	...
	payload, footer, err := marshalChunk("p1", 1, 1, sections, testRecordRange(sections))
	require.NoError(t, err)
...
func TestProbeChunkForwardStopsAtUnreadableTail(t *testing.T) {
	...
	for _, gen := range []uint64{0, 1} {
		_, _, err := store.WriteChunk(ctx, gen, writeSections(map[string][]uint64{"v1": {100 + gen}}), testRecordRange(writeSections(map[string][]uint64{"v1": {100 + gen}})))
		require.NoError(t, err)
	}
	// Generation 2 is garbage; 3 is valid but sits behind it.
	require.NoError(t, cm.Write(ctx, store.ChunkKey(2), []byte("not a chunk")))
	_, _, err := store.WriteChunk(ctx, 3, writeSections(map[string][]uint64{"v1": {103}}), testRecordRange(writeSections(map[string][]uint64{"v1": {103}})))
	require.NoError(t, err)
