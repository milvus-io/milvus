...
func newGrowingSegmentFromVisible(ctx context.Context, collection *segcore.CCollection, visible walview.VisibleSegment) (*growingSegment, error) {
	...
	segment.replaySkipThrough = visible.Assignment.GetCheckpointTimeTick()
...
func (s *growingSegment) loadPersisted(ctx context.Context, visible walview.VisibleSegment) error {
	...
	defer s.mu.Unlock()
	if s.released {
		return merr.WrapErrServiceInternalMsg("growing segment %d released", s.segmentID)
	}
	if s.segment != nil {
		return nil
	}
...
func (s *growingSegment) ensureCSegment(timetick uint64) error {
	...
	if s.flushed {
		...
		return merr.WrapErrServiceInternalMsg("growing segment %d already flushed at timetick %d", s.segmentID, s.flushTimeTick)
	}
	if s.segment != nil {
		return nil
	}
