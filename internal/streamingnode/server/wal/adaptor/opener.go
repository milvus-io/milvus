...
func (o *openerAdaptorImpl) handleAlterWAL(ctx context.Context, l walimpls.WALImpls, opt *wal.OpenOption,
	...
	logger := log.Ctx(ctx).With(
		...
		zap.Bool("foundAlterWAL", snapshot.FoundAlterWALMsg),
	...
	if snapshot.Checkpoint.HasAlterWal && snapshot.Checkpoint.AlterWalState.Stage == streamingpb.AlterWALStage_FLUSHING {
	...
	if snapshot.Checkpoint.HasAlterWal && snapshot.Checkpoint.AlterWalState.Stage == streamingpb.AlterWALStage_ADVANCE_CHECKPOINT {
	...
	return nil, errors.Errorf("WAL switch success: %s switch to %s finish, re-opening required", opt.Channel.Name, snapshot.TargetWALName)
}

func (o *openerAdaptorImpl) handleAlterWALFlushingStage(ctx context.Context, opt *wal.OpenOption, roWAL *roWALAdaptorImpl,
	param *interceptors.InterceptorBuildParam, rs recovery.RecoveryStorage, snapshot *recovery.RecoverySnapshot) error {
	...
	targetTimeTick := snapshot.AlterWALTs
	...
	for flusherCP == nil || flusherCP.TimeTick < targetTimeTick {
		...
		case <-checkCtx.Done():
			...
			return errors.Wrap(checkCtx.Err(), "timeout waiting for flush completion during WAL switch")
		case <-ctx.Done():
...
func (o *openerAdaptorImpl) handleAlterWALAdvanceCheckpointsStage(ctx context.Context, opt *wal.OpenOption, snapshot *recovery.RecoverySnapshot) error {
	...
	finalCheckpoint.HasAlterWal = false
