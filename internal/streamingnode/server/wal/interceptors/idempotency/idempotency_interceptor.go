...
func (impl *idempotencyInterceptor) appendIdempotentMessage(ctx context.Context, msg message.MutableMessage, key IdempotencyKey, append interceptors.Append) (message.MessageID, error) {
	...
	case BeginDecisionWait:
		...
		log.Ctx(ctx).Debug("idempotency duplicate hit",
			zap.String("vchannel", msg.VChannel()),
			zap.String("idempotency_key", string(key)))
...
func getIdempotencyKey(msg message.MutableMessage, config WindowConfig) (key IdempotencyKey, hasIdempotencyKey bool, err error) {
	...
	case message.MessageTypeInsert:
		...
		if err != nil {
			return "", false, status.NewInvaildArgument("malformed insert message header")
		}
	...
	case message.MessageTypeCommitTxn:
		...
		if err != nil {
			return "", false, status.NewInvaildArgument("malformed commit txn message header")
		}
	...
	if rawKey == "" {
		if hasInsertResult {
			return "", false, status.NewInvaildArgument("idempotency insert result header requires idempotency key")
		}
		return "", false, nil
	}
	if config.MaxKeyLength > 0 && len(rawKey) > config.MaxKeyLength {
		return "", false, status.NewInvaildArgument("idempotency key length %d exceeds limit %d", len(rawKey), config.MaxKeyLength)
	}
...
func getInsertResultFromInsertMessage(msg message.MutableMessage) (*messagespb.IdempotentInsertResult, error) {
	...
	insertMsg, err := message.AsMutableInsertMessageV1(msg)
	if err != nil {
		return nil, status.NewInvaildArgument("malformed insert message header")
	}
	...
	if err := message.ValidateIdempotentInsertResult(result); err != nil {
		return nil, status.NewInvaildArgument("malformed idempotency insert result header")
	}
	return result, nil
}
