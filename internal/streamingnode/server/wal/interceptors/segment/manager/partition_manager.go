...
// SealAndFenceSegmentUntil seal all segment that contains the message or create segment message is less than the incoming timetick.
...
func (m *partitionSegmentManager) checkIfPendingSegmentReady() {
	...
	if m.pendingAllocSegmentRequest != nil && m.pendingAllocSegmentRequest.Ready() {
		...
		if newSegmentManager.CreateSegmentTimeTick() < m.fencedAssignTimeTick {
			// The new incoming segment is fenced, so we need to seal it to avoid any insert operation on it.
			// If new insert operation is applied on it, it will be lost,
			// because the new segment is flushed by manual flush message.
			newSegmentManager.WithSealPolicy(policy.PolicyFenced(m.fencedAssignTimeTick))
			m.logger.Info(
				"new incoming segment is fenced",
				...
				zap.Uint64("fencedAssignTimeTick", m.fencedAssignTimeTick))
