...
func TestStatsManager(t *testing.T) {
	...
	sealOperator1.EXPECT().AsyncMustSealSegments(mock.Anything).Return().Maybe()
	...
	sealOperator2.EXPECT().AsyncMustSealSegments(mock.Anything).Return().Maybe()
...
func TestConcurrentStasManager(t *testing.T) {
	...
	params.Save(params.StreamingCfg.NodeMemoryUsageThreshold.Key, "0.000003")
	params.Save(params.StreamingCfg.NodeGrowingSegmentBytesHwmThreshold.Key, "0.000002")
	params.Save(params.StreamingCfg.NodeGrowingSegmentBytesLwmThreshold.Key, "0.000001")
	...
	for i := 0; i < pchannelCount; i++ {
		...
		sealOperator.EXPECT().AsyncMustSealSegments(mock.Anything).Run(func(info utils.SealSegmentSignal) {
