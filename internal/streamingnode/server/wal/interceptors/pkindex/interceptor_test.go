...
// appendPanicOnInsert panics instead of appending an insert, as a bug below the
// interceptor would.
func (f *fakeAppender) appendPanicOnInsert(ctx context.Context, msg message.MutableMessage) (message.MessageID, error) {
	if msg.MessageType() == message.MessageTypeInsert {
		panic("the append of an insert panicked")
	}
...
func newTestInterceptorOfSchema(t *testing.T, sessions txnSessions, schema *schemapb.CollectionSchema) (*appendInterceptor, *testEngines) {
	...
	i.registry.add(context.Background(), testVChannel, testCollectionID, schema)
...
func TestBuildIsPassthroughWithoutEngineFactory(t *testing.T) {
...
func TestPanicBetweenDecideAndApplyReleasesTheStripes(t *testing.T) {
	i, _ := newTestInterceptor(t, nil)
	f := newFakeAppender()

	func() {
		defer func() {
			require.NotNil(t, recover(), "the panic must keep unwinding out of the interceptor")
		}()
		_, _ = i.DoAppend(newTestAppendContext(), newInsert(testVChannel, 1), f.appendPanicOnInsert)
	}()
...
func TestDeleteByExpressionPassesThrough(t *testing.T) {
	i, engines := newTestInterceptor(t, nil)
	engine := engines.of(t, testVChannel)

	// a delete by keys probes the index, so the counter below is not always equal.
	_, err := i.DoAppend(newTestAppendContext(), newDelete(testVChannel, 1), newFakeAppender().append)
	require.NoError(t, err)
	before := engine.probes()
	require.Positive(t, before)

	f := newFakeAppender()
	_, err = i.DoAppend(newTestAppendContext(), newDeleteByExpression(testVChannel), f.append)
	require.NoError(t, err)
	require.Equal(t, []message.MessageType{message.MessageTypeDelete}, f.types)
	require.Equal(t, before, engine.probes(), "a delete by expression carries no keys to probe")
}
...
// fadingSessions reports the session gone after goneAfter lookups. That is what
// CleanupTxnUntil and RollbackAllInFlightTransactions look like from the
// interceptor: they take no vchannel lock, so they can end a session at any
// point of an append.
type fadingSessions struct {
	inner     txnSessions
	goneAfter int
	lookups   int
}

func (s *fadingSessions) GetSessionOfTxn(id message.TxnID) (*txn.TxnSession, error) {
	s.lookups++
	if s.lookups > s.goneAfter {
		return nil, status.NewTransactionExpired("txn %d not found in manager", id)
	}
	return s.inner.GetSessionOfTxn(id)
}

// TestTxnSessionEndedAtRegisterCleanupDropsThePendingWrites covers the session
// that ends between the lookup and RegisterCleanup. The callback is then
// registered on a session that will never run it.
func TestTxnSessionEndedAtRegisterCleanupDropsThePendingWrites(t *testing.T) {
	manager, session := newTestTxnSession(t)
	txnCtx := session.TxnContext()
	sessions := &fadingSessions{inner: manager, goneAfter: 1}

	i, engines := newTestInterceptor(t, sessions)
	engine := engines.of(t, testVChannel)

	f := newFakeAppender()
	_, err := i.DoAppend(newTestAppendContext(), newInsert(testVChannel, 12).WithTxnContext(txnCtx), f.append)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, []message.MessageType{message.MessageTypeCommitTxn}, f2.types)
	_, ok := engine.segmentOf(t, 12)
	require.False(t, ok, "the pending writes of an ended transaction must be dropped")
}

// TestTxnSessionEndedBeforeALaterBodyDropsThePendingWrites covers the body that
// arrives after another body already claimed the cleanup. Its claim fails, so
// only the session lookup can still find the ended transaction.
func TestTxnSessionEndedBeforeALaterBodyDropsThePendingWrites(t *testing.T) {
	manager, session := newTestTxnSession(t)
	txnCtx := session.TxnContext()
	// the first body looks the session up twice, the second body finds it gone.
	sessions := &fadingSessions{inner: manager, goneAfter: 2}
	...
	f := newFakeAppender()
	_, err := i.DoAppend(newTestAppendContext(), newInsert(testVChannel, 13).WithTxnContext(txnCtx), f.append)
	require.NoError(t, err)
	require.False(t, target.decider.ClaimTxnCleanup(int64(txnCtx.TxnID)),
		"the first body must have claimed the cleanup")

	f2 := newFakeAppender()
	_, err = i.DoAppend(newTestAppendContext(), newInsert(testVChannel, 14).WithTxnContext(txnCtx), f2.append)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, []message.MessageType{message.MessageTypeCommitTxn}, f3.types)
	for _, pk := range []int64{13, 14} {
		_, ok := engine.segmentOf(t, pk)
		require.False(t, ok, "the pending writes of an ended transaction must be dropped")
	}
}

func TestTxnSessionCleanupDropsThePendingWrites(t *testing.T) {
	manager, session := newTestTxnSession(t)
	txnCtx := session.TxnContext()
	...
	f := newFakeAppender()
	_, err := i.DoAppend(newTestAppendContext(), newInsert(testVChannel, 7).WithTxnContext(txnCtx), f.append)
	require.NoError(t, err)
	require.False(t, target.decider.ClaimTxnCleanup(int64(txnCtx.TxnID)),
		"the interceptor must have claimed the cleanup of the transaction exactly once")

	// a second body of the same transaction must not claim the cleanup again.
	f2 := newFakeAppender()
	_, err = i.DoAppend(newTestAppendContext(), newInsert(testVChannel, 8).WithTxnContext(txnCtx), f2.append)
	require.NoError(t, err)
