...
func NewTxnManager(pchannel types.PChannelInfo, buffer *utility.TxnBuffer) *TxnManager {
	...
	recoveredSessions := make(map[message.TxnID]struct{}, len(builders))
	sessionIDs := make([]int64, 0, len(builders))
	...
	txnManager.logger.Info(
		"txn manager created",
		zap.Int64s("recoverdTxns", sessionIDs),
	)
	return txnManager
}
