...
// newTestSplitShardImmutableMessage is the source replica of a split as the
// shard interceptor hands it to SplitShard: mutable, with its tick assigned,
// before the append.
func newTestSplitShardImmutableMessage(vchannel string, collectionID int64, timetick uint64) message.MutableSplitShardMessageV2 {
	return newTestSplitShardImmutableMessageOfTask(vchannel, collectionID, 100, timetick)
}

func newTestSplitShardImmutableMessageOfTask(vchannel string, collectionID int64, splitTaskID int64, timetick uint64) message.MutableSplitShardMessageV2 {
	msg := message.NewSplitShardMessageBuilderV2().
		...
		WithHeader(&message.SplitShardMessageHeader{
			...
			TargetVchannels: []string{vchannel + "-target1", vchannel + "-target2"},
		}).
		WithBody(&message.SplitShardMessageBody{}).
		MustBuildMutable().
		WithTimeTick(timetick).
		WithLastConfirmedUseMessageID()
	return message.MustAsMutableSplitShardMessageV2(msg)
}
...
func newTestSplitShardGenesisImmutableMessageWithBody(vchannel string, collectionID int64, partitionIDs []int64, timetick uint64, genesis *message.CreateCollectionRequest) message.ImmutableSplitShardMessageV2 {
	msg := message.NewSplitShardMessageBuilderV2().
		...
		WithHeader(&message.SplitShardMessageHeader{
			...
			SplitTaskId:     100,
			SourceVchannel:  "v1",
			TargetVchannels: []string{vchannel},
		}).
...
// TestShardManagerSplitShardAgainKeepsTheFirstFenceTick pins T_switch's
// definition: the tick of the FIRST fence record of the task.
//
...
// of them seals the same data, and the append path reports the recorded tick
// back so DataCoord records the first one too. Moving it to a later record's
// tick is what could never drain once the source's data sync service has closed
// past the first fence.
func TestShardManagerSplitShardAgainKeepsTheFirstFenceTick(t *testing.T) {
	m := newTestShardManagerWithVChannelState(t, streamingpb.VChannelState_VCHANNEL_STATE_NORMAL, 0)

	m.SplitShard(newTestSplitShardImmutableMessage("v1", 1, 2000))
	assert.Equal(t, SplitFence{TimeTick: 2000, TaskID: 100}, m.GetSplitFence(1, "v1"))

	// the same task fences again at a later tick: T_switch does not move.
	m.SplitShard(newTestSplitShardImmutableMessage("v1", 1, 3000))
	assert.Equal(t, SplitFence{TimeTick: 2000, TaskID: 100}, m.GetSplitFence(1, "v1"))

	// nor at an earlier one.
	m.SplitShard(newTestSplitShardImmutableMessage("v1", 1, 1500))
	assert.Equal(t, SplitFence{TimeTick: 2000, TaskID: 100}, m.GetSplitFence(1, "v1"))
