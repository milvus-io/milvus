...
// fence. Without it a second split task landing on a source another task
// already fenced would read the rejection as "my own fence holds", roll
// forward, and give two tasks the same source.
...
//     It becomes SHARD_FENCED; the proxy is meant to refresh and retry, which
...
// registration slot. The shard interceptor calls it for a first fence BEFORE
// it appends the fence record, under the vchannel-exclusive lock, and never
// rolls it back: an append that reports an error may still have persisted, so
// the vchannel must refuse DML from T_switch on whatever the append's outcome.
// The message is therefore the mutable one, with its time tick assigned.
//
...
//     answers a stale proxy route afterwards -- SHARD_FENCED, meant to make
//     the proxy refresh and retry (not implemented on this branch) -- and
//     what returns T_switch to a re-sent fence.
...
//
// A second fence of the SAME task changes nothing: T_switch is the tick of the
// task's FIRST fence and stays there. The broadcaster re-drives a split whose
// source replica was not acknowledged; the re-driven record seals the same data
// (the vchannel took no DML in between), and the append path reports the
// recorded tick back, so DataCoord records the first one. Raising it could
// never drain once the source's data sync service has closed past the first
// fence. A fence record of ANOTHER task is refused on the append path; it must
// never move a fence it did not place.
func (m *shardManagerImpl) SplitShard(msg message.MutableSplitShardMessageV2) {
	...
	if fence, ok := m.fencedVChannels[vchannel]; ok {
		if fence.TaskID != taskID {
			logger.Warn(m.ctx, "split shard skipped: the vchannel is already fenced by another task",
				mlog.Int64("collectionID", collectionID),
				mlog.Int64("fencedByTaskID", fence.TaskID),
				mlog.Int64("splitTaskID", taskID))
			return
		}
		logger.Info(m.ctx, "split shard re-fence of the same task keeps the first fence tick",
			mlog.Int64("collectionID", collectionID),
			mlog.Int64("splitTaskID", taskID),
			mlog.Uint64("fencedTimeTick", fence.TimeTick),
	...
	collectionInfo, ok := m.collections[collectionID]
	if !ok {
		logger.Warn(m.ctx, "collection not exists when splitting shard", mlog.Int64("collectionID", collectionID))
		return
	}
	if collectionInfo.VChannel != vchannel {
		...
		logger.Warn(m.ctx, "split shard skipped: this pchannel now hosts another vchannel of the collection",
	...
	logger.Info(m.ctx, "vchannel is fenced by shard split, its registration is released",
		mlog.Int64("collectionID", collectionID),
		mlog.Int64("splitTaskID", taskID),
		mlog.Uint64("timetick", msg.TimeTick()),
...
func (m *shardManagerImpl) CreateVChannel(msg message.ImmutableSplitShardMessageV2) {
	...
	if err := m.checkIfVChannelCanBeCreated(collectionID, msg.VChannel()); err != nil {
		if errors.Is(err, ErrVChannelConflict) {
			...
			logger.Error(m.ctx, "cannot register vchannel, another vchannel of the collection holds this pchannel", mlog.Err(err))
			return
		}
		// ErrCollectionExists: the same vchannel is already registered, an
		// ordinary idempotent replay.
		logger.Info(m.ctx, "vchannel already registered, skip the genesis")
		return
	}
