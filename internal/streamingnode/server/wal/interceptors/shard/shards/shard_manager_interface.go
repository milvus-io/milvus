...
type ShardManager interface {
	...
	// SplitShard fences the source vchannel of a split and releases its
	// registration on this pchannel: any new DML on the vchannel is rejected
	// forever, and the slot is free for a successor vchannel of the same
	// collection. The fence is kept as a name-keyed tombstone, which is what
	// answers a stale route afterwards. It takes the mutable message because
	// the interceptor fences BEFORE appending the record and never rolls the
	// fence back on an append error (an errored append may have persisted);
	// the message's time tick, T_switch, is already assigned. A second fence
	// of the same task changes nothing: T_switch stays the tick of the task's
	// first fence.
	SplitShard(msg message.MutableSplitShardMessageV2)
