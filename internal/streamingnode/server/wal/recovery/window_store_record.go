...
func newCommittedWriteRecordFromTxnMessage(pchannel string, msg message.ImmutableTxnMessage) (*committedWriteRecord, bool) {
	...
	if err != nil {
		...
		log.Warn("failed to merge idempotent insert results for committed write record",
			zap.String("pchannel", record.SourcePChannel),
			zap.String("vchannel", record.VChannel),
			zap.Error(err))
	} else if hadAny {
