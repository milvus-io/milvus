...
func (m *windowManager) handleWindowRecoveryError(err error, checkpoint *WALCheckpoint, vchannels map[string]*vchannelRecoveryInfo) error {
	...
	m.Logger().Warn("idempotency window store corrupted; dropping window cache and continuing WAL recovery", zap.Error(err))
...
func (m *windowManager) recoverWindowInfoFromMeta(ctx context.Context, pchannel string, checkpoint *WALCheckpoint, vchannels map[string]*vchannelRecoveryInfo) (*windowStoreRecoveryInfo, error) {
	...
	m.Logger().Info("recover idempotency window meta done",
		zap.Int("windows", len(info.windowMetas)),
		zap.Bool("hasPChannelWindowMeta", info.storeMeta != nil),
	)
	return info, nil
}
...
func (m *windowManager) recoverWindowStoreFromSnapshot(ctx context.Context, info *windowStoreRecoveryInfo, checkpoint *WALCheckpoint, vchannels map[string]*vchannelRecoveryInfo) (*WALCheckpoint, error) {
	...
	m.Logger().Info("recover idempotency window info done",
		zap.Int("windows", len(m.idempotencyWindows())),
		zap.String("storage", "pchannel-window-store"),
	)
	return rewound, nil
}
...
func (m *windowManager) bootstrapPChannelWindowStore(ctx context.Context, pchannel string, sourceCheckpoint *WALCheckpoint) (*pchannelWindowStoreMeta, error) {
	...
	logger := m.Logger().With(zap.String("op", "bootstrapPChannelWindowStore"), zap.Uint64("generation", footer.Generation))
	...
	m.Logger().Info("bootstrap pchannel window store done",
		zap.String("pchannel", pchannel),
		zap.String("chunkKey", chunkKey),
		zap.Uint64("sourceTimeTick", sourceCheckpoint.TimeTick),
	)
	...
	for generation := meta.LatestGeneration + 1; ; generation++ {
		...
		if err != nil {
			if errors.Is(err, ErrPChannelWindowStoreCorrupted) {
				...
				m.Logger().Warn("dropping corrupt orphan pchannel window chunk above latest generation",
					zap.String("chunkKey", chunkKey), zap.Uint64("generation", generation), zap.Error(err))
...
func (m *windowManager) repairPChannelWindowMeta(ctx context.Context, storeMeta *pchannelWindowStoreMeta) error {
	return retryOperationWithBackoff(ctx,
		m.Logger().With(zap.String("op", "repairPChannelWindowMeta")),
