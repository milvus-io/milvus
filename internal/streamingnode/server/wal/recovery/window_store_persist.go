...
func (m *windowManager) persistPChannelWindowChunk(
	...
	if err := retryOperationWithBackoff(ctx,
		logger.With(zap.String("op", "getPChannelWindowMeta")),
	...
	if err := retryOperationWithBackoff(ctx,
		logger.With(zap.String("op", "persistPChannelWindowChunk"), zap.Uint64("generation", nextGeneration)),
		func(ctx context.Context) error {
...
func (m *windowManager) persistPChannelWindowMeta(
	...
	return retryOperationWithBackoff(ctx,
		logger.With(zap.String("op", "persistPChannelWindowMeta")),
		func(ctx context.Context) error {
			return resource.Resource().StreamingNodeCatalog().SavePChannelWindowMeta(ctx, m.pchannel, storeMeta.intoCatalogMeta())
		})
}

func (m *windowManager) persistIdempotencyWindowMetas(ctx context.Context, logger *log.MLogger, metas map[string]*streamingpb.VChannelWindowMeta) error {
	if len(metas) == 0 {
		return nil
	}
	return retryOperationWithBackoff(ctx,
		logger.With(zap.String("op", "persistIdempotencyWindowMetas")),
		func(ctx context.Context) error {
...
func (m *windowManager) rewindCheckpointForPChannelWindowReplay(sourceCheckpoint, checkpoint *WALCheckpoint, vchannels map[string]*vchannelRecoveryInfo) *WALCheckpoint {
	...
	m.Logger().Info("rewind recovery checkpoint for pchannel window replay",
		zap.String("fromMessageID", checkpointMessageIDString(checkpoint)),
		zap.Uint64("fromTimeTick", checkpointTimeTick(checkpoint)),
		zap.String("toMessageID", checkpointMessageIDString(replayStart)),
		zap.Uint64("toTimeTick", replayStart.TimeTick),
	)
