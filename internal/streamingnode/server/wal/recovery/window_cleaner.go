...
func (m *windowManager) cleanPChannelWindow(ctx context.Context, logger *log.MLogger) error {
	if logger == nil {
		logger = m.Logger()
	}
	...
	return retryOperationWithBackoff(ctx,
		logger.With(
			zap.String("op", "cleanPChannelWindowMeta"),
			zap.Uint64("oldMinAvailableGeneration", meta.MinAvailableGeneration),
			zap.Uint64("newMinAvailableGeneration", updatedMeta.MinAvailableGeneration),
			zap.Uint64("minInUseGeneration", updatedMeta.MinInUseGeneration),
			zap.Bool("hasActiveViewMinBoundary", boundary.hasActiveViewMinBoundary),
		),
...
func (m *windowManager) deletePChannelWindowChunks(ctx context.Context, logger *log.MLogger, startGeneration uint64, endGeneration uint64, latestGeneration uint64) error {
	...
	for generation := startGeneration; generation < endGeneration; generation++ {
		...
		logger.Debug("clean pchannel window chunk",
