...
// The compound SaveRecoverySnapshot already gathers the whole snapshot into one
// catalog call, paving the way for a future single-point CAS commit.
...
func (rs *recoveryStorageImpl) persistDirtySnapshot(ctx context.Context, lvl mlog.Level) (err error) {
	...
	// The catalog persists the snapshot parts in order and writes the consume
	// checkpoint strictly last as the commit point, so a whole-snapshot retry
	// is always safe (all parts are idempotent puts).
