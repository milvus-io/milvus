...
func (impl *flusherComponents) spawnGenesisDataSyncService(genesisMsg message.ImmutableMessage, collectionID int64) {
	...
	if _, ok := impl.dataServices[genesisMsg.VChannel()]; ok {
		impl.logger.Info("the data sync service of current vchannel is built, skip it", zap.String("vchannel", genesisMsg.VChannel()))
	...
	if genesisMsg.TimeTick() <= impl.recoveryCheckPointTimeTick {
		...
		// so we can skip it.
		impl.logger.Info("the vchannel genesis message is older than the recovery checkpoint, skip it",
			zap.String("vchannel", genesisMsg.VChannel()),
			zap.Uint64("timeTick", genesisMsg.TimeTick()),
...
func (impl *flusherComponents) addNewDataSyncService(
	...
	impl.logger.Info("create data sync service done", zap.String("vchannel", genesisMsg.VChannel()))
}
