...
func (impl *msgHandlerImpl) HandleAlterWAL(ctx context.Context, alterWALMsg message.ImmutableAlterWALMessageV1, currentVChannel string) error {
	...
	logger := log.With(
		...
		zap.String("targetWALName", alterWALMsg.Header().TargetWalName))
	...
	if err := impl.wbMgr.FlushChannel(context.Background(), vchannel, alterWALMsg.TimeTick()); err != nil {
