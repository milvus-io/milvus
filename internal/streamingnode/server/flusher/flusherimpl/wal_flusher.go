...
// onCheckpointUpdated is the checkpoint-updater callback: it is invoked, on
// the checkpoint updater's own goroutine, once DataCoord has acked a
// vchannel's advanced checkpoint. It persists the acked position as the
// recovered checkpoint and records it on the vchannel's data sync service.
// It closes nothing: a fenced source whose acked checkpoint has passed its
// fence is closed by the dispatch goroutine on the next dispatched message
// (flusherComponents.closeDrainedFencedSources).
func (impl *WALFlusherImpl) onCheckpointUpdated(mp *msgpb.MsgPosition) {
	...
	impl.flusherComponents.ObserveCheckpointAck(impl.notifier.Context(), mp.GetChannelName(), mp.GetTimestamp())
}

// dataCoordPersistedCheckpoint is the flusher's persistedCheckpointFunc: the
// seek position of DataCoord's recovery info, which is the stored channel
// checkpoint whenever there is one and an earlier position otherwise.
func dataCoordPersistedCheckpoint(ctx context.Context, vchannel string) (uint64, error) {
	ctx, cancel := context.WithTimeout(ctx, paramtable.Get().DataNodeCfg.UpdateChannelCheckpointRPCTimeout.GetAsDuration(time.Second))
	defer cancel()
	mixc, err := resource.Resource().MixCoordClient().GetWithContext(ctx)
	if err != nil {
		return 0, err
	}
	resp, err := mixc.GetChannelRecoveryInfo(ctx, &datapb.GetChannelRecoveryInfoRequest{Vchannel: vchannel})
	if err := merr.CheckRPCCall(resp, err); err != nil {
		return 0, err
	}
	return resp.GetInfo().GetSeekPosition().GetTimestamp(), nil
}
...
func (impl *WALFlusherImpl) buildFlusherComponents(ctx context.Context, l wal.WAL, snapshot *recovery.RecoverySnapshot) (*flusherComponents, message.MessageID, error) {
	...
	recoverInfos, checkpoint, err := impl.getRecoveryInfos(ctx, vchannels, snapshot.VChannels)
	if err != nil {
	...
	fc := &flusherComponents{
		...
		persistedCheckpoint:        dataCoordPersistedCheckpoint,
		logger:                     impl.logger,
...
func (impl *WALFlusherImpl) dispatch(msg message.ImmutableMessage) (err error) {
	...
	case message.MessageTypeSplitShard:
		...
		case message.SplitShardRoleSource:
			...
			// Record this record's own tick as the close gate before forwarding:
			// the dispatch goroutine closes the data sync service once its acked
			// checkpoint proves the dd_node has processed this seal record
			// (flusherComponents.closeDrainedFencedSources), rather than it being
			// torn down by the AlterCollection replica that later retires this
			// vchannel. The record's own tick, not the T_switch it reports: see
			// flusherComponents.fenced.
			impl.flusherComponents.RecordFence(msg.VChannel(), splitShardMsg.TimeTick())
		default:
			// The broadcast reaches only the source, the targets and the
			// control channel, so a replica landing on any other vchannel --
			// another shard of the same collection included -- is a misroute
			// with no data sync service action to take: there is nothing to
			// spawn and nothing for the dd_node to seal, so it must not be
			// forwarded either.
	...
	case message.MessageTypeAlterCollection:
		...
		if messageutil.RetiresVChannel(alterMsg.Header(), alterMsg.MustBody().GetUpdates(), msg.VChannel()) {
			...
			// not trigger. The data sync service is closed on the dispatch
			// goroutine once its acked checkpoint passes the fence tick recorded
			// when the source replica of SplitShard was dispatched
			// (flusherComponents.closeDrainedFencedSources).
