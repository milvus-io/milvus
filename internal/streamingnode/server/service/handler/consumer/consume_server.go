...
func CreateConsumeServer(walManager walmanager.Manager, streamServer streamingpb.StreamingNodeHandlerService_ConsumeServer) (*ConsumeServer, error) {
	...
	l, err := walManager.GetAvailableWAL(createReq.Pchannel, createReq.Term)
	if err != nil {
	...
	scanner, err := l.Read(streamServer.Context(), wal.ReadOption{
		...
		MessageFilter: newMessageFilter(createReq.DeliverFilters),
	})
	if err != nil {
...
func newMessageFilter(filters []*streamingpb.DeliverFilter) wal.MessageFilter {
	fs := make([]func(message.ImmutableMessage) bool, 0, len(filters))
	for _, filter := range filters {
		filter := filter
		switch filter.Filter.(type) {
		case *streamingpb.DeliverFilter_TimeTickGt:
			fs = append(fs, func(msg message.ImmutableMessage) bool {
				return msg.TimeTick() > filter.GetTimeTickGt().GetTimeTick()
			})
		case *streamingpb.DeliverFilter_TimeTickGte:
			fs = append(fs, func(msg message.ImmutableMessage) bool {
				return msg.TimeTick() >= filter.GetTimeTickGte().GetTimeTick()
			})
		case *streamingpb.DeliverFilter_Vchannel:
			fs = append(fs, func(msg message.ImmutableMessage) bool {
				return msg.VChannel() == filter.GetVchannel().GetVchannel()
			})
		}
	}
