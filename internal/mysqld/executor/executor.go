...
func (e *defaultExecutor) Run(plan *planner.PhysicalPlan) (*sqltypes.Result, error) {
	...
	return e.dispatch(statements.Statements[0])
}

func (e *defaultExecutor) dispatch(n *planner.NodeSqlStatement) (*sqltypes.Result, error) {
	if n.DmlStatement.IsSome() {
		return e.dispatchDmlStatement(n.DmlStatement.Unwrap())
	}
	return nil, fmt.Errorf("invalid sql statement, only dml statement is supported")
}

func (e *defaultExecutor) dispatchDmlStatement(n *planner.NodeDmlStatement) (*sqltypes.Result, error) {
	if n.SelectStatement.IsSome() {
		return e.execSelect(n.SelectStatement.Unwrap())
	}
	return nil, fmt.Errorf("invalid dml statement, only select statement is supported")
}

func (e *defaultExecutor) execSelect(n *planner.NodeSelectStatement) (*sqltypes.Result, error) {
	...
	if match && !from.Where.IsSome() { // count without filter.
		rowCnt, err := e.execCountWithoutFilter(tableName)
		if err != nil {
			return nil, err
		}
	...
	if match && from.Where.IsSome() { // count with filter.
		...
		return e.execCountWithFilter(tableName, filter)
	}
	...
	filter := planner.NewExprTextRestorer().RestoreExprText(from.Where.Unwrap())
	res, err := e.execQuery(tableName, filter, outputFields)
	if err != nil {
		return nil, err
	}
...
func (e *defaultExecutor) execCountWithFilter(tableName string, filter string) (*sqltypes.Result, error) {
	// TODO: check if `*` match vector field.
	outputs := []string{"*"}

	res, err := e.execQuery(tableName, filter, outputs)
	if err != nil {
		return nil, err
	}
...
func (e *defaultExecutor) execCountWithoutFilter(tableName string) (int, error) {
...
func (e *defaultExecutor) execQuery(tableName string, filter string, outputs []string) (*milvuspb.QueryResults, error) {
