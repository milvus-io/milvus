...
func (d *reorderDispatcher[K]) Submit(ctx context.Context, key K, task Task, callbacks ...func(error) error) *conc.Future[struct{}] {
	...
	if state.aborted {
		...
		// the same retry round and must not start.
		err := state.abortErr
		if err == nil {
			err = context.Canceled
		}
		d.mu.Unlock()
		d.finish(entry, err, nil)
		return future
	}
	entry.queued = true
...
func (d *reorderDispatcher[K]) dispatchCommit(key K, entry *reorderEntry) {
	go func() {
		future := d.commitPool.Submit(func() (struct{}, error) {
			return struct{}{}, runPhase(entry.ctx, entry.task.Commit)
		})
		<-future.Inner()
...
// dropEmptyKeyLocked forgets a key that holds no entries, and reports whether it
// did so.
