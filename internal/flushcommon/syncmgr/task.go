...
type SyncTask struct {
	...
	// ioRetryAttempts is this task's budget for the retries inside the writers.
	// 0 means unlimited; see DefaultIORetryAttempts for why it differs by caller.
...
func (t *SyncTask) Prepare(ctx context.Context) (err error) {
	...
	if !t.dataWritten {
		if !t.columnsFrozen {
			columnGroups = t.resolveSharedColumnGroups(segmentInfo)
		}
		...
		case storage.StorageV2:
			...
			writer := NewBulkPackWriterV2(t.metacache, t.schema, t.chunkManager, t.allocator, 0,
				packed.DefaultMultiPartUploadSize, t.storageConfig, columnGroups, t.ioRetryAttempts)
		...
		case storage.StorageV3:
			writer := NewBulkPackWriterV3(t.metacache, t.schema, t.chunkManager, t.allocator, 0,
				packed.DefaultMultiPartUploadSize, t.storageConfig, columnGroups, segmentInfo.ManifestPath(), t.ioRetryAttempts)
			...
			t.v3Prepared = true
		default:
			writer, writerErr := NewBulkPackWriter(t.metacache, t.schema, t.chunkManager, t.allocator, t.ioRetryAttempts)
			if writerErr != nil {
				return writerErr
			}
...
func (t *SyncTask) Commit(ctx context.Context) (err error) {
	...
	if t.preparedV3 != nil {
		...
		// Commit tolerates re-runs — a landed-but-unacknowledged attempt is
		// adopted via the commitAttempted gate instead of committed twice.
		var manifest string
		if err := retry.Do(ctx, func() error {
			var commitErr error
			manifest, commitErr = t.preparedV3.Commit(ctx)
			return commitErr
		}, ioRetryOptions(t.ioRetryAttempts)...); err != nil {
			return err
		}
		t.preparedV3.Destroy()
		t.preparedV3 = nil
...
// forbidden.
func (t *SyncTask) resolveSharedColumnGroups(segmentInfo *metacache.SegmentInfo) []storagecommon.ColumnGroup {
	...
	if current, ok := t.metacache.GetSegmentByID(t.segmentID); ok {
...
func (t *SyncTask) ResolveColumnGroups(segmentInfo *metacache.SegmentInfo) []storagecommon.ColumnGroup {
	return cloneColumnGroups(t.getColumnGroups(segmentInfo))
}
