...
func (s *StorageV2SerializerSuite) SetupTest() {
	...
	var err error
	s.serializer, err = NewStorageV2Serializer(s.mockCache)
...
func (s *StorageV2SerializerSuite) getDeleteBuffer(rows int) *storage.DeleteData {
...
func (s *StorageV2SerializerSuite) getVarcharDeleteBuffer(rows int) *storage.DeleteData {
	buf := &storage.DeleteData{}
	for i := 0; i < rows; i++ {
		pk := storage.NewVarCharPrimaryKey(strconv.Itoa(i + 1))
		ts := tsoutil.ComposeTSByTime(time.Now(), 0)
		buf.Append(pk, ts)
	}
	return buf
...
func (s *StorageV2SerializerSuite) TestSerializeInsert() {
	...
	s.Run("with_empty_data", func() {
		...
		rec, err := s.serializer.serializeBinlog(ctx, pack)
		s.NoError(err)
		s.Equal(rec.NumRows(), int64(0))
	})
...
func (s *StorageV2SerializerSuite) TestSerializeDelete() {
	...
	s.Run("serialize_failed", func() {
		...
		_, err := s.serializer.serializeDeltalog(ctx, pack)
		s.Error(err)
	})
	...
	s.Run("varchar_pk", func() {
		...
		pack := s.getBasicPack()
		pack.WithDeleteData(s.getVarcharDeleteBuffer(10))
		pack.WithTimeRange(50, 100)
	...
	s.Run("with_empty_data", func() {
		pack := s.getBasicPack()
		pack.WithDeleteData(s.getDeleteBuffer(0))
		pack.WithTimeRange(50, 100)
...
func (s *StorageV2SerializerSuite) TestBadSchema() {
	...
	_, err := NewStorageV2Serializer(mockCache)
	s.Error(err)
}
