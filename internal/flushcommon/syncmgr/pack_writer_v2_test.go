...
func (s *PackWriterV2Suite) TestPackWriterV2_Write() {
	...
	bw := NewBulkPackWriterV2(mc, s.schema, s.cm, s.logIDAlloc, pack, packed.DefaultWriteBufferSize, 0, nil, s.currentSplit)

	gotInserts, _, _, _, _, _, err := bw.Write(context.Background())
	s.NoError(err)
