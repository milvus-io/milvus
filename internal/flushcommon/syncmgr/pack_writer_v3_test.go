...
func (s *PackWriterV3Suite) TestPackWriterV3_Write() {
	...
	bw := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, 0, s.storageConfig, parquetSplit, manifestPath, DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestResolveInsertWriterFormatsUsesColumnGroupFormatsWithoutReadingManifest() {
	...
	bw := NewBulkPackWriterV3(nil, s.schema, s.cm, s.logIDAlloc,
		...
		packed.MarshalManifestPath("files/missing-manifest-segment", 42), DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestResolveInsertWriterFormatsRequiresFormatsForExistingManifest() {
	...
	bw := NewBulkPackWriterV3(nil, s.schema, s.cm, s.logIDAlloc,
		packed.DefaultWriteBufferSize, 0, s.storageConfig, columnGroups,
		packed.MarshalManifestPath("files/existing-manifest-segment", 42), DefaultIORetryAttempts)
	bw.initialManifestPath = bw.manifestPath
...
func (s *PackWriterV3Suite) TestWriteEmptyInsertData() {
	...
	bw := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, 0, s.storageConfig, s.currentSplit, manifestPath, DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestInvalidManifestPath() {
	...
	bw := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, 0, s.storageConfig, s.currentSplit, invalidManifestPath, DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestV3InheritsV2Fields() {
	...
	bw := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, packed.DefaultMultiPartUploadSize, s.storageConfig, s.currentSplit, manifestPath, DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestMultiBatchStatsAccumulation() {
	...
	bw1 := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, 0, s.storageConfig, s.currentSplit, manifestPath, DefaultIORetryAttempts)
	...
	bw2 := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, 0, s.storageConfig, currentSplit, manifest1, DefaultIORetryAttempts)
	...
	bw3 := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc, packed.DefaultWriteBufferSize, 0, s.storageConfig, currentSplit, manifest2, DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestWrite_SingleVersionBumpAcrossSections() {
	...
	bw := NewBulkPackWriterV3(mc, s.schema, s.cm, s.logIDAlloc,
		packed.DefaultWriteBufferSize, 0, s.storageConfig, s.currentSplit, manifestPath, DefaultIORetryAttempts)
...
func (s *PackWriterV3Suite) TestWrite_TransientCommitFailureIsReported() {
	...
	var calls int32
	patched := mockey.Mock(packed.CommitManifestUpdatesStrict).
...
// TestPreparedV3CommitAdoptionGate verifies that a landed-but-unacknowledged
// base+1 manifest version may only be adopted by the handle that actually ran
// the lost commit. A fresh handle (a task rebuilt after restart / WAL replay)
// seeing base+1 must refuse: that version belongs to a previous incarnation
// whose batch boundary may differ, and adopting it would advance the
// checkpoint past data the manifest does not cover.
func (s *PackWriterV3Suite) TestPreparedV3CommitAdoptionGate() {
	segmentID := int64(789)
	basePath := "files/adoption-gate-segment"
	baseVersion := int64(7)
	...
	var commits int32
	patchLatest := mockey.Mock(packed.GetLatestManifestVersion).
		To(func(string, *indexpb.StorageConfig) (int64, error) { return latestVersion, nil }).
		Build()
	defer patchLatest.UnPatch()
	patchStats := mockey.Mock(packed.GetManifestStats).
		To(func(string, *indexpb.StorageConfig) (map[string]packed.ManifestStat, error) {
			return map[string]packed.ManifestStat{}, nil
		}).
		Build()
	defer patchStats.UnPatch()
	patchCommit := mockey.Mock(packed.CommitManifestUpdatesStrict).
		To(func(string, int64, *indexpb.StorageConfig, *packed.ManifestUpdates) (string, error) {
	...
	// A fresh handle that never attempted a commit must NOT adopt base+1.
	fresh := newHandle()
	_, err := fresh.Commit(context.Background())
	s.Require().Error(err)
	s.ErrorIs(err, merr.ErrDataIntegrity)
	s.Equal(int32(0), atomic.LoadInt32(&commits), "refusal must happen before any commit is issued")

	// The lost-answer sequence on ONE handle: the first attempt fails
	// transiently (but actually landed), the retry sees base+1 and adopts it
	// without issuing a second commit.
	retried := newHandle()
	latestVersion = baseVersion // nothing landed yet at the first attempt
	_, err = retried.Commit(context.Background())
	s.Require().Error(err)
	s.Require().True(retried.commitAttempted)
	s.Equal(int32(1), atomic.LoadInt32(&commits))

	latestVersion = baseVersion + 1 // the "failed" commit actually landed
	manifest, err := retried.Commit(context.Background())
	s.Require().NoError(err)
	s.Equal(packed.MarshalManifestPath(basePath, baseVersion+1), manifest)
	s.Equal(int32(1), atomic.LoadInt32(&commits), "the landed attempt is adopted, not re-committed")
}
