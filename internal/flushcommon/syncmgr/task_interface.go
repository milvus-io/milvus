...
type Task interface {
	...
	// PayloadBytes / InsertPayloadBytes / DeletePayloadBytes report the
	// resident row payload this task still retains, for backpressure and
	// memory accounting. 0 means nothing resident: a growing-source task's
	// rows stay pinned in segcore, and an owned task reports 0 once object
	// storage accepted its data.
	PayloadBytes() int64
	InsertPayloadBytes() int64
	DeletePayloadBytes() int64
