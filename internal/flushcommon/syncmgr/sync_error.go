...
func ClassifySyncError(ctx context.Context, err error) SyncErrorDecision {
	...
	case errors.Is(err, context.Canceled):
		// Always ours: the dispatcher cancels queued tasks with context.Canceled
		// when it is closing or abandoning a key, and nothing below it manufactures
		// one from an unrelated context.
		return SyncCanceled
...
func ioRetryOptions(ctx context.Context, attempts uint) []retry.Option {
	return []retry.Option{
