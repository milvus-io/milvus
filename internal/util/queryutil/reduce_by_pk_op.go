...
//   - failOnDuplicate: controls duplicate PK handling.
//     At QN/Delegator level, duplicate PKs across segments are expected (growing+sealed).
//     At proxy level, cross-shard duplicates indicate data corruption.
type ReduceByPKOperator struct {
	reduceType      reduce.IReduceType
	failOnDuplicate bool
}

// NewReduceByPKOperator creates a reduce-by-PK operator with silent deduplication.
// Use this at QN/Delegator level where duplicate PKs across segments are expected.
func NewReduceByPKOperator(reduceType reduce.IReduceType) *ReduceByPKOperator {
	return &ReduceByPKOperator{reduceType: reduceType, failOnDuplicate: false}
}

// NewSortAndCheckPKOperator creates a reduce-by-PK operator that fails on duplicates.
// Use this at proxy level where cross-shard PK duplication indicates a data integrity issue.
...
func NewSortAndCheckPKOperator(reduceType reduce.IReduceType) *ReduceByPKOperator {
	return &ReduceByPKOperator{reduceType: reduceType, failOnDuplicate: true}
}
