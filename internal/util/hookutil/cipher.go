...
func initCipher() error {
	...
	paramtable.GetCipherParams().WatchCipherWithPrefix("kmsKeyArns", "", func(event *config.Event) {
		log.Info("receive the cipher refresh event", zap.Any("event", event))
		go func() {
			cipherVal := GetCipher()
			soConfig := paramtable.GetCipherParams().SoConfigs.GetValue()
			log.Info("refresh cipher configs", zap.Any("config", soConfig))
			if err = cipherVal.Init(soConfig); err != nil {
				log.Panic("fail to init configs for the cipher when refreshing", zap.Error(err))
			}
			storeCipher(cipherVal)
		}()
	})
