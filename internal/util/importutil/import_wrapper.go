...
func (p *ImportWrapper) fileValidation(filePaths []string) (bool, error) {
	...
	for i := 0; i < len(filePaths); i++ {
		...
		if fileType != JSONFileExt && fileType != NumpyFileExt {
			...
			return false, fmt.Errorf("unsupportted file type: '%s'", filePath)
		}
		...
		if err != nil {
			...
			return rowBased, fmt.Errorf("failed to get file size of '%s', make sure the input path is related path, error:%w", filePath, err)
		}
