...
type nodeCtxManager struct {
	...
	ddNodeCloseCh    chan struct{} // notify ddnode and downstream node work to exit
}
...
func NewNodeCtxManager(nodeCtx *nodeCtx, closeWg *sync.WaitGroup) *nodeCtxManager {
	return &nodeCtxManager{
		...
		ddNodeCloseCh:    make(chan struct{}),
	}
}
...
func (nodeCtxManager *nodeCtxManager) Start() {
	...
	go nodeCtxManager.inputNodeWork()
	go nodeCtxManager.ddNodeWork()
}

func (nodeCtxManager *nodeCtxManager) inputNodeWork() {
	...
	for {
		...
		case <-nodeCtxManager.inputNodeCloseCh:
			log.Info("lxg test input node closech")
			return
		...
		default:
			...
			log.Info("lxg test inputNode receive in", zap.Any("node name", inputNode.node.Name()), zap.Any("input", input))
			if !n.IsValidInMsg(input) {
				log.Info("lxg test input is invalid", zap.Any("node name", inputNode.node.Name()), zap.Any("input", input))
				inputNode.blockMutex.RUnlock()
				continue
			}
			log.Info("lxg test operate before", zap.Any("node name", inputNode.node.Name()), zap.Any("input", input))
			output = n.Operate(input)
			inputNode.blockMutex.RUnlock()
			// the output decide whether the node should be closed.
			log.Info("lxg test input node output", zap.Any("node name", inputNode.node.Name()), zap.Any("output", output))
			if isCloseMsg(output) {
				log.Info("lxg debug", zap.Any("node close name", inputNode.node.Name()))
...
func (nodeCtxManager *nodeCtxManager) ddNodeWork() {
	defer nodeCtxManager.closeWg.Done()
	...
	for {
		select {
		case <-nodeCtxManager.ddNodeCloseCh:
			log.Info("lxg test node closech")
			return
		// handles node work spinning
		// 1. collectMessage from upstream or just produce Msg from InputNode
		// 2. invoke node.Operate
		...
		default:
			...
			for curNode != nil {
				...
				// the input message decides whether the operate method is executed
				n := curNode.node
				log.Info("lxg test receive in", zap.Any("node name", curNode.node.Name()), zap.Any("input", input))
				curNode.blockMutex.RLock()
				if !n.IsValidInMsg(input) {
					log.Info("lxg input is invalid", zap.Any("node close name", curNode.node.Name()), zap.Any("input", input))
					curNode.blockMutex.RUnlock()
					curNode = ddNode
					continue
				}
				log.Info("lxg test operate before", zap.Any("node name", curNode.node.Name()), zap.Any("input", input))
				output = n.Operate(input)
				curNode.blockMutex.RUnlock()
				log.Info("lxg test output", zap.Any("node name", curNode.node.Name()), zap.Any("output", output))
				// the output decide whether the node should be closed.
				if isCloseMsg(output) {
					log.Info("lxg debug", zap.Any("node close name", curNode.node.Name()))
					nodeCtxManager.closeOnce.Do(func() {
						close(nodeCtxManager.ddNodeCloseCh)
					})
