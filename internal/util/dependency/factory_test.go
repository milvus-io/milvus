...
func TestValidateMQType(t *testing.T) {
	...
	assert.Error(t, validateMQType(false, mqTypePebblemq))
}
...
func TestSelectMQType(t *testing.T) {
	...
	assert.Equal(t, mustSelectMQType(true, mqTypeDefault, mqEnable{false, true, true, true, true}), mqTypePebblemq)
	assert.Equal(t, mustSelectMQType(true, mqTypeDefault, mqEnable{false, false, true, true, true}), mqTypePulsar)
	assert.Equal(t, mustSelectMQType(true, mqTypeDefault, mqEnable{false, false, false, true, true}), mqTypePulsar)
	assert.Equal(t, mustSelectMQType(true, mqTypeDefault, mqEnable{false, false, false, false, true}), mqTypeKafka)
	...
	assert.Equal(t, mustSelectMQType(false, mqTypeDefault, mqEnable{false, false, false, true, true}), mqTypePulsar)
	assert.Equal(t, mustSelectMQType(false, mqTypeDefault, mqEnable{false, false, false, false, true}), mqTypeKafka)
	...
	assert.Equal(t, mustSelectMQType(true, mqTypePebblemq, mqEnable{true, true, true, true, true}), mqTypePebblemq)
	...
	assert.Panics(t, func() { mustSelectMQType(false, mqTypePebblemq, mqEnable{true, true, true, true, true}) })
