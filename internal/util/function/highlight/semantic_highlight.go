...
type SemanticHighlight struct {
	...
	fieldIDs          []int64          // schema field IDs
	dynamicFieldNames []string         // dynamic field names list (set via SetDynamicFieldNames)
	dynamicFieldID    int64            // $meta field's FieldID
	inputFields       []string         // original input_fields from params
...
func NewSemanticHighlight(collSchema *schemapb.CollectionSchema, params []*commonpb.KeyValuePair, conf map[string]string, extraInfo *models.ModelExtraInfo) (*SemanticHighlight, error) {
	...
	for _, fieldName := range inputFields {
		...
		if ok {
			...
			// Non-schema fields will be treated as dynamic fields, processed later via SetDynamicFieldNames
		}
	}
	...
	return &SemanticHighlight{
		...
		fieldIDs:          fieldIDs,
		dynamicFieldNames: nil, // will be set later via SetDynamicFieldNames
		dynamicFieldID:    dynamicFieldID,
		inputFields:       inputFields, // store original input_fields for later processing
		provider:          provider,
		queries:           queries,
	}, nil
}
...
// SetDynamicFieldNames sets the dynamic field names (parsed by translateOutputFields)
func (highlight *SemanticHighlight) SetDynamicFieldNames(names []string) {
	highlight.dynamicFieldNames = names
}
...
// InputFields returns the original input_fields from params
func (highlight *SemanticHighlight) InputFields() []string {
	return highlight.inputFields
}
