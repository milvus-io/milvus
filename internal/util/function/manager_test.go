...
func TestFunctionRunnerManagerMaterializeNilSchemaWithoutAllocationIsNoop(t *testing.T) {
	manager, _ := newMockFunctionRunnerManager(t)
	t.Cleanup(manager.Close)

	changed, err := manager.Materialize(context.Background(), 1, nil, newBM25InsertRequest("message"))
	require.NoError(t, err)
	require.False(t, changed)
	requireFunctionRunnerEntryRemoved(t, manager, 1)
}

func TestFunctionRunnerManagerMaterializeNilSchemaUsesLatestVersionSnapshot(t *testing.T) {
	manager, factory := newMockFunctionRunnerManager(t)
	t.Cleanup(manager.Close)
...
func TestFunctionRunnerManagerMaterializeNilSchemaRetriesWithAllocatedSchema(t *testing.T) {
	manager := newFunctionRunnerManager()
	t.Cleanup(manager.Close)
