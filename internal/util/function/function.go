...
func NewFunctionRunner(coll *schemapb.CollectionSchema, schema *schemapb.FunctionSchema) (FunctionRunner, error) {
	...
	case schemapb.FunctionType_TextEmbedding:
		return nil, nil
	default:
		return nil, fmt.Errorf("unknown functionRunner type %s %s", schema.GetType().String(), schemapb.FunctionType_MinHash)
	}
}

func InjectFunctionInternalParams(coll *schemapb.CollectionSchema, schema *schemapb.FunctionSchema) error {
	switch schema.GetType() {
	case schemapb.FunctionType_MinHash:
		return InjectMinHashFunctionInternalParams(coll, schema)
	default:
		return nil
	}
}
