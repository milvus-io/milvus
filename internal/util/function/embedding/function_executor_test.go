...
func (s *FunctionExecutorSuite) TestExecutor() {
	...
	exec, err := NewFunctionExecutor(schema, nil, map[string]string{models.ClusterIDKey: "test-cluster"})
	s.NoError(err)
...
func (s *FunctionExecutorSuite) TestErrorSchema() {
	...
	_, err := NewFunctionExecutor(schema, nil, map[string]string{models.ClusterIDKey: "test-cluster"})
	s.Error(err)
}
