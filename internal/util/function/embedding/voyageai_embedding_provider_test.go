...
func createVoyageAIProvider(url string, schema *schemapb.FieldSchema, providerName string) (textEmbeddingProvider, error) {
	...
	case voyageAIProvider:
		return NewVoyageAIEmbeddingProvider(schema, functionSchema, map[string]string{models.URLParamKey: url}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
	default:
...
func (s *VoyageAITextEmbeddingProviderSuite) TestEmbedding() {
	...
	for _, providerName := range s.providers {
		...
		s.Equal(models.Float32Embd, provider.(*VoyageAIEmbeddingProvider).embdType)
...
func (s *VoyageAITextEmbeddingProviderSuite) TestEmbeddingIn8() {
	...
	for _, providerName := range s.providers {
		...
		s.NoError(err)
		s.Equal(models.Int8Embd, provider.(*VoyageAIEmbeddingProvider).embdType)
...
func (s *VoyageAITextEmbeddingProviderSuite) TestNewVoyageAIEmbeddingProvider() {
	...
	provider, err := NewVoyageAIEmbeddingProvider(s.schema.Fields[2], functionSchema, map[string]string{models.URLParamKey: "mock"}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
	s.NoError(err)
	...
	// Invalid truncation
	{
		...
		_, err := NewVoyageAIEmbeddingProvider(s.schema.Fields[2], functionSchema, map[string]string{}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
		s.Error(err)
