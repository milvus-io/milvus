...
func createTEIEmbeddingClient(apiKey string, endpoint string) (*tei.TEIClient, error) {
	enable := os.Getenv(models.EnableTeiEnvStr)
	if strings.ToLower(enable) == "false" {
		return nil, errors.New("TEI model serving is not enabled")
	}

	return tei.NewTEIClient(apiKey, endpoint)
}
