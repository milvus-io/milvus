...
func (s *TextEmbeddingFunctionSuite) TestProcessInsert() {
	...
	defer ts.Close()
	{
		...
		}, map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
...
func (s *TextEmbeddingFunctionSuite) TestNewTextEmbeddings() {
	{
		...
		_, err := NewTextEmbeddingFunction(s.schema, fSchema, map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
		fSchema.Params = []*commonpb.KeyValuePair{}
		_, err = NewTextEmbeddingFunction(s.schema, fSchema, map[string]string{models.ClusterIDKey: "test-cluster"})
		s.Error(err)
...
func (s *TextEmbeddingFunctionSuite) TestParseCredentail() {
	{
		...
		ak, url, err := models.ParseAKAndURL(cred, []*commonpb.KeyValuePair{}, map[string]string{}, "", "test-cluster")
		s.Equal(ak, "")
		s.Equal(url, "")
		s.NoError(err)
	}
	{
		cred := credentials.NewCredentials(map[string]string{})
		_, _, err := models.ParseAKAndURL(cred, []*commonpb.KeyValuePair{}, map[string]string{"credential": "NotExist"}, "", "test-cluster")
	...
	}
	{
		...
		_, _, err := models.ParseAKAndURL(cred, []*commonpb.KeyValuePair{}, map[string]string{"credential": "mock"}, "", "test-cluster")
		s.NoError(err)
	}
}
