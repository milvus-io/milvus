...
func createVertexAIProvider(url string, schema *schemapb.FieldSchema) (textEmbeddingProvider, error) {
	...
	return NewVertexAIEmbeddingProvider(schema, functionSchema, mockClient, map[string]string{}, credentials.NewCredentials(map[string]string{"mock.credential_json": "mock"}), map[string]string{})
}
...
func (s *VertexAITextEmbeddingProviderSuite) TestGetTaskType() {
	...
	{
		provider, err := NewVertexAIEmbeddingProvider(s.schema.Fields[2], functionSchema, mockClient, map[string]string{}, credentials.NewCredentials(map[string]string{"mock.credential_json": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
