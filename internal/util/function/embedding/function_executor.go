...
func createFunction(coll *schemapb.CollectionSchema, schema *schemapb.FunctionSchema, extralInfo map[string]string) (Runner, error) {
	switch schema.GetType() {
	case schemapb.FunctionType_BM25: // ignore bm25 function
		return nil, nil
	case schemapb.FunctionType_TextEmbedding:
		f, err := NewTextEmbeddingFunction(coll, schema, extralInfo)
		if err != nil {
			return nil, err
		}
		return f, nil
...
func ValidateFunctions(schema *schemapb.CollectionSchema, extralInfo map[string]string) error {
	for _, fSchema := range schema.Functions {
		f, err := createFunction(schema, fSchema, extralInfo)
		if err != nil {
			return err
		}
...
func NewFunctionExecutor(schema *schemapb.CollectionSchema, functions []*schemapb.FunctionSchema, extralInfo map[string]string) (*FunctionExecutor, error) {
	...
	for _, fSchema := range functions {
		runner, err := createFunction(schema, fSchema, extralInfo)
		if err != nil {
			return nil, err
		}
		if runner != nil {
