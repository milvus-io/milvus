...
const (
	...
	SeedKey        = "seed"
	// internal  parameters
	PermutationsKey    = "permutations" //  Serialized (a,b) parameters
...
func NewMinHashFunctionRunner(
	...
	for _, param := range funSchema.GetParams() {
		...
		case PermutationsKey:
			...
			if err != nil {
				return nil, fmt.Errorf("Failed to deserialize permutations(a, b): %w", err)
			}
			hasPerms = true
		}
	}
	if !hasPerms {
		return nil, fmt.Errorf("Permutations(a, b) not provided and failed to generate.")
	}
...
func InjectMinHashFunctionInternalParams(collSchema *schemapb.CollectionSchema, funSchema *schemapb.FunctionSchema) error {
	// asume ValidateMinHashFunctions has been called before, all params is valid
	numHashes := 0
	...
	// fill numHashes by output field dim if not specified
	if numHashes == 0 {
	...
	// extract params
	permA, permB := InitializePermutations(numHashes, seed)
...
func (m *MinHashFunctionRunner) run(data []string, dst [][]byte) error {
	...
	m.batchSignatureToBinaryVector(allSignatures, dst)
...
func (m *MinHashFunctionRunner) batchSignatureToBinaryVector(signatures [][]uint32, dst [][]byte) {
