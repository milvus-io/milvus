...
func (d *DecayExpr) Execute(ctx *types.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	if len(inputs) != 2 {
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("decay: expected 2 input columns, got %d", len(inputs)))
	}
	...
	if len(scoreCol.Chunks()) != numChunks {
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("decay: input column has %d chunks but score column has %d chunks", numChunks, len(scoreCol.Chunks())))
	}
	...
	for chunkIdx := 0; chunkIdx < numChunks; chunkIdx++ {
		...
		if !ok {
			return nil, merr.WrapErrServiceInternal(fmt.Sprintf("decay: score column chunk %d must be Float32, got %T", chunkIdx, scoreCol.Chunk(chunkIdx)))
		}
