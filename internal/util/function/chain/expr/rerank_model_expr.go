...
// ModelExpr implements FunctionExpr for model-based reranking.
...
// NewModelExpr creates a new ModelExpr with the given provider and queries.
func NewModelExpr(provider rerank.ModelProvider, queries []string) (*ModelExpr, error) {
	if provider == nil {
	...
	return &ModelExpr{
		BaseExpr: *NewBaseExpr("model", types.AllStages),
		provider: provider,
		queries:  queries,
	}, nil
}
...
// ModelExpr outputs a single Float32 column (the model score).
func (m *ModelExpr) OutputDataTypes() []arrow.DataType {
...
func (m *ModelExpr) Execute(ctx *types.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	if len(inputs) != 1 {
...
func (m *ModelExpr) processChunk(ctx *types.FuncContext, chunk arrow.Array, query string) (arrow.Array, error) {
...
func (m *ModelExpr) rerankBatch(ctx context.Context, query string, texts []string) ([]float32, error) {
