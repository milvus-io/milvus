...
type ModelExprTestSuite struct {
	suite.Suite
	pool *memory.CheckedAllocator
}

func (s *ModelExprTestSuite) SetupTest() {
	s.pool = memory.NewCheckedAllocator(memory.NewGoAllocator())
}

func (s *ModelExprTestSuite) TearDownTest() {
	s.pool.AssertSize(s.T(), 0)
}

func TestModelExprTestSuite(t *testing.T) {
	suite.Run(t, new(ModelExprTestSuite))
}
...
func (s *ModelExprTestSuite) createStringChunked(chunks ...[]string) *arrow.Chunked {
...
func (s *ModelExprTestSuite) createStringChunkedWithNulls(chunks ...[]*string) *arrow.Chunked {
	arrs := make([]arrow.Array, len(chunks))
	for i, chunk := range chunks {
		builder := array.NewStringBuilder(s.pool)
		for _, v := range chunk {
...
func (s *ModelExprTestSuite) TestNewModelExpr_Valid() {
	provider := &mockModelProvider{maxBatch: 10}
	expr, err := NewModelExpr(provider, []string{"query1", "query2"})
	s.Require().NoError(err)
	s.NotNil(expr)
	s.Equal("model", expr.Name())
}

func (s *ModelExprTestSuite) TestNewModelExpr_NilProvider() {
	_, err := NewModelExpr(nil, []string{"query1"})
	s.Error(err)
	s.Contains(err.Error(), "provider is nil")
}

func (s *ModelExprTestSuite) TestNewModelExpr_EmptyQueries() {
	provider := &mockModelProvider{maxBatch: 10}
	_, err := NewModelExpr(provider, []string{})
	s.Error(err)
	s.Contains(err.Error(), "queries must not be empty")
}

func (s *ModelExprTestSuite) TestNewModelExpr_NilQueries() {
	provider := &mockModelProvider{maxBatch: 10}
	_, err := NewModelExpr(provider, nil)
	s.Error(err)
	s.Contains(err.Error(), "queries must not be empty")
}
...
func (s *ModelExprTestSuite) TestOutputDataTypes() {
	provider := &mockModelProvider{maxBatch: 10}
	expr, err := NewModelExpr(provider, []string{"query"})
	s.Require().NoError(err)
...
func (s *ModelExprTestSuite) TestIsRunnable() {
	provider := &mockModelProvider{maxBatch: 10}
	expr, err := NewModelExpr(provider, []string{"query"})
	s.Require().NoError(err)

	// AllStages should include these
	...
	s.True(expr.IsRunnable(types.StageIngestion))
	s.True(expr.IsRunnable(types.StagePreProcess))
	s.True(expr.IsRunnable(types.StagePostProcess))
...
func (s *ModelExprTestSuite) TestExecute_SingleChunk() {
	...
	expr, err := NewModelExpr(provider, []string{"test query"})
	s.Require().NoError(err)
...
func (s *ModelExprTestSuite) TestExecute_MultipleChunks() {
	callCount := 0
	provider := &mockModelProvider{
		maxBatch: 100,
		rerankFunc: func(ctx context.Context, query string, docs []string) ([]float32, error) {
...
func (s *ModelExprTestSuite) TestExecute_EmptyChunk() {
...
func (s *ModelExprTestSuite) TestExecute_WithNulls() {
...
func (s *ModelExprTestSuite) TestExecute_WrongInputCount() {
	provider := &mockModelProvider{maxBatch: 100}
	expr, err := NewModelExpr(provider, []string{"query"})
	s.Require().NoError(err)
...
func (s *ModelExprTestSuite) TestExecute_QueryCountMismatch() {
	provider := &mockModelProvider{maxBatch: 100}
	// 1 query but 2 chunks
	expr, err := NewModelExpr(provider, []string{"only_one_query"})
	s.Require().NoError(err)
...
func (s *ModelExprTestSuite) TestExecute_ProviderError() {
	provider := &mockModelProvider{
		maxBatch: 100,
		rerankFunc: func(ctx context.Context, query string, docs []string) ([]float32, error) {
...
func (s *ModelExprTestSuite) TestExecute_ProviderErrorOnSecondChunk() {
	...
	expr, err := NewModelExpr(provider, []string{"q1", "q2"})
	s.Require().NoError(err)
...
func (s *ModelExprTestSuite) TestExecute_WrongScoreCount() {
	provider := &mockModelProvider{
		maxBatch: 100,
		rerankFunc: func(ctx context.Context, query string, docs []string) ([]float32, error) {
...
func (s *ModelExprTestSuite) TestBatching_MaxBatch2With5Docs() {
	batchCalls := 0
	var batchSizes []int
	provider := &mockModelProvider{
		maxBatch: 2,
...
func (s *ModelExprTestSuite) TestBatching_ZeroMaxBatch() {
	batchCalls := 0
	provider := &mockModelProvider{
...
func (s *ModelExprTestSuite) TestBatching_ErrorMidway() {
	batchCalls := 0
	provider := &mockModelProvider{
		maxBatch: 2,
		rerankFunc: func(ctx context.Context, query string, docs []string) ([]float32, error) {
...
func (s *ModelExprTestSuite) TestMemoryLeak_RepeatedExecution() {
	provider := &mockModelProvider{
		maxBatch: 100,
		rerankFunc: func(ctx context.Context, query string, docs []string) ([]float32, error) {
			scores := make([]float32, len(docs))
...
func (s *ModelExprTestSuite) TestMemoryLeak_OnError() {
	provider := &mockModelProvider{
		maxBatch: 100,
		rerankFunc: func(ctx context.Context, query string, docs []string) ([]float32, error) {
...
func (s *ModelExprTestSuite) TestExecute_NonStringInput() {
	provider := &mockModelProvider{maxBatch: 100}
	expr, err := NewModelExpr(provider, []string{"query"})
	s.Require().NoError(err)
