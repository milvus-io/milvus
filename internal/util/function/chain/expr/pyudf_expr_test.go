...
func setPyUDFEnabledForTest(t *testing.T, value string) {
	...
	oldValue := item.SwapTempValue(value)
	t.Cleanup(func() { item.SwapTempValue(oldValue) })
}

func TestNewPyUDFExprDisabled(t *testing.T) {
	setPyUDFEnabledForTest(t, "false")

	runtime := &fakePyUDFRuntime{acquire: func(context.Context, string, string) (pyudf.Lease, error) {
		t.Fatal("disabled PyUDF must not reach the selected runtime")
		return nil, nil
	}}
...
func TestNewPyUDFExprFromParams(t *testing.T) {
	...
	globalRuntime, err := pyudf.GetGlobalRuntime()
	require.NoError(t, err)
	assert.Same(t, globalRuntime, expr.runtime)
