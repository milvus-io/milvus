...
func NewScoreCombineExpr(mode string, inputCount int, weights []float64) (*ScoreCombineExpr, error) {
	if inputCount < 2 {
		return nil, merr.WrapErrParameterInvalidMsg("score_combine: at least 2 input columns required, got %d", inputCount)
	}
	...
	if mode == ModeWeighted {
		if len(weights) != inputCount {
			return nil, merr.WrapErrParameterInvalidMsg("score_combine: weighted mode requires %d weights, got %d",
...
func NewScoreCombineExprFromParams(params map[string]interface{}) (types.FunctionExpr, error) {
	...
	if float64(inputCount) != inputCountFloat {
		return nil, merr.WrapErrParameterInvalidMsg("score_combine: %s must be a whole number, got %f", InputCountKey, inputCountFloat)
	}
...
func (s *ScoreCombineExpr) Execute(ctx *types.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	if len(inputs) != s.inputCount {
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("score_combine: expected %d input columns, got %d", s.inputCount, len(inputs)))
	}
