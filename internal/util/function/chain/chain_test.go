...
func (s *MergeOpTestSuite) TestMergeOpWeighted_AllL2_NoNormalize() {
	...
	// Sorted descending: ID2 (0.4) > ID1 (0.15)
	// Note: for all-L2 no-normalize, sort is ascending (smaller = better),
	// but MergeOp's internal sort is always descending. The rerank_builder sets Sort direction.
	idCol := result.Column(types.IDFieldName)
	scoreCol := result.Column(types.ScoreFieldName)
	s.Equal(2, idCol.Chunk(0).Len())

	// Raw L2 sums, no conversion
	scoreChunk := scoreCol.Chunk(0).(*array.Float32)
	s.InDelta(0.4, float64(scoreChunk.Value(0)), 0.01)
	s.InDelta(0.15, float64(scoreChunk.Value(1)), 0.01)
}
