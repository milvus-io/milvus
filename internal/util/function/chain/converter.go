...
func exportScores(df *DataFrame) ([]float32, error) {
	...
	for chunkIdx, chunk := range col.Chunks() {
		scores := chunk.(*array.Float32)
		for rowIdx, score := range scores.Float32Values() {
			if scores.IsNull(rowIdx) {
				return nil, merr.WrapErrFunctionFailedMsg("exportScores: $score contains null in chunk %d at row %d", chunkIdx, rowIdx)
			}
			if math.IsNaN(float64(score)) || math.IsInf(float64(score), 0) {
				return nil, merr.WrapErrFunctionFailedMsg("exportScores: $score contains non-finite value %v in chunk %d at row %d", score, chunkIdx, rowIdx)
			}
