...
func functionScoreToRepr(funcScore *schemapb.FunctionScore) (*ChainRepr, error) {
	if funcScore == nil {
		return nil, merr.WrapErrServiceInternal("funcScore is nil")
	}
	...
	for i, funcSchema := range funcScore.Functions {
		...
		if err != nil {
			return nil, merr.WrapErrServiceInternal(fmt.Sprintf("function[%d]: %v", i, err))
		}
...
func functionSchemaToFunctionReprWithInputs(funcSchema *schemapb.FunctionSchema) (*FunctionRepr, []string, error) {
	if funcSchema == nil {
		return nil, nil, merr.WrapErrServiceInternal("funcSchema is nil")
	}
	...
	if funcName == "" {
		return nil, nil, merr.WrapErrParameterInvalidMsg("function name (reranker) not found in params")
	}

	if len(inputCols) == 0 {
		return nil, nil, merr.WrapErrParameterInvalidMsg("input columns not specified for function %s", funcName)
	}
