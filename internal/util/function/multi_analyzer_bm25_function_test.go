...
func (s *MultiAnalyzerBM25FunctionSuite) TestAnalyzerAliasResolvedOnce() {
	runner, err := NewMultiAnalyzerBM25FunctionRunner(
		...
		`{"by_field":"analyzer","alias":{"requested":"english","english":"default"},"analyzers":{"default":{"type":"standard"},"english":{"type":"english"}}}`,
	)
	s.Require().NoError(err)
	defer runner.Close()

	_, batches, err := runner.BatchRunWithTextTerms(
		[]string{"test of analyzer"},
		[]string{"requested"},
	)
	s.Require().NoError(err)
	s.ElementsMatch([][]byte{[]byte("analyz"), []byte("test")}, batches[0].Terms)
}
