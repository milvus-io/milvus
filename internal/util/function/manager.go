...
func functionRunnerSchema(schema *schemapb.CollectionSchema) *schemapb.CollectionSchema {
	if schema == nil {
		return nil
	}

	// 2.6 does not support BM25 function schema changes. Keep the manager
	// interface unchanged for the cherry-pick, but collapse managed runners to a
...
func FillFunctionData(ctx context.Context, collectionID int64, schema *schemapb.CollectionSchema, body *msgpb.InsertRequest) (bool, error) {
	return defaultFunctionRunnerManager.Materialize(ctx, collectionID, functionRunnerSchema(schema), body)
}
...
func AllocFunctionRunners(collectionID int64, vchannel string, schema *schemapb.CollectionSchema) <-chan error {
	return defaultFunctionRunnerManager.Alloc(collectionID, vchannel, functionRunnerSchema(schema))
}

func UpdateFunctionRunners(collectionID int64, vchannel string, schema *schemapb.CollectionSchema) <-chan error {
	return defaultFunctionRunnerManager.Update(collectionID, vchannel, functionRunnerSchema(schema))
}
