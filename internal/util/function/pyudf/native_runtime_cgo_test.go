...
// TODO: Re-enable the native runtime tests after the custom CI entrypoints
// install the trusted PyUDF runtime and pyarrow before starting Go tests.
const nativeRuntimeTestSkipReason = "custom CI entrypoints do not install the trusted PyUDF runtime and pyarrow yet"
...
def create_udf(context):
    ...
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_round_trip", wheelPath)
	require.NoError(t, err)
...
`)
	resource, err := loadNativeRuntimeTestResource("native_factory_failure", wheelPath)
	require.Error(t, err)
	assert.Nil(t, resource)
...
def create_udf(context):
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_close_failure", wheelPath)
	require.NoError(t, err)
	require.NotNil(t, resource)
...
func loadNativeRuntimeTestResource(resourceName, wheelPath string) (*nativeResource, error) {
	request, err := NewLoadRequest(fileresource.ResolvedFileResource{
		ID:        11,
		Name:      resourceName,
...
func writeNativeRuntimeTestWheel(t *testing.T, packageName, module string) string {
	...
	writeFile(packageName+"-0.0.1.dist-info/entry_points.txt", "[milvus.pyudf]\nmain = "+packageName+":create_udf\n")
