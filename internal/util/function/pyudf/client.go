...
func (c *Client) Execute(ctx context.Context, request ExecuteRequest) (outputs []*arrow.Chunked, err error) {
	...
	defer func() {
		if ctxErr := ctx.Err(); ctxErr != nil {
			// Keep cancellation/deadline as the final cause without discarding
			// an error already returned by the worker or IPC conversion.
			err = merr.Wrap(merr.Combine(err, ctxErr), "py_udf: Execute")
		}
		if err != nil {
