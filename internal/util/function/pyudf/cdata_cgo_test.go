...
func TestNativeResourceIdentityRoundTrip(t *testing.T) {
	t.Skip(nativeRuntimeTestSkipReason)
...
def create_udf(context):
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_resource_identity", wheel)
	require.NoError(t, err)
...
def create_udf(context):
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_transform_query", wheel)
	require.NoError(t, err)
	defer func() { require.NoError(t, resource.Close()) }()
...
def create_udf(context):
    return UDF()
`)
		resource, err := loadNativeRuntimeTestResource("native_one_to_two_outputs", wheel)
		require.NoError(t, err)
		defer func() { require.NoError(t, resource.Close()) }()
...
def create_udf(context):
    return UDF()
`)
		resource, err := loadNativeRuntimeTestResource("native_two_to_one_output", wheel)
		require.NoError(t, err)
		defer func() { require.NoError(t, resource.Close()) }()
...
`,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			resource, err := loadNativeRuntimeTestResource(test.packageName, wheel)
			require.NoError(t, err)
			defer func() { require.NoError(t, resource.Close()) }()
...
def create_udf(context):
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_zero_row_chunk", wheel)
	require.NoError(t, err)
	defer func() { require.NoError(t, resource.Close()) }()
...
def create_udf(context):
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_dynamic_import_failure", wheel)
	require.NoError(t, err)
	defer func() { require.NoError(t, resource.Close()) }()
...
`,
		...
		{
			name:        "base exception",
			packageName: "native_transform_base_exception",
			module: `
class UDF:
    def transform_query(self, params, columns):
        raise SystemExit("transform exit")

def create_udf(context):
    return UDF()
`,
			match: "transform exit",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
	...
	t.Run("runtime remains usable after function failures", func(t *testing.T) {
		const packageName = "native_after_transform_failure"
		wheel := writeNativeRuntimeTestWheel(t, packageName, `
class UDF:
    def transform_query(self, params, columns):
        return [columns[0]]
...
def create_udf(context):
    return UDF()
`)
		resource, err := loadNativeRuntimeTestResource(packageName, wheel)
		require.NoError(t, err)
		defer func() { require.NoError(t, resource.Close()) }()

		allocator := memory.NewCheckedAllocator(mallocator.NewMallocator())
		defer allocator.AssertSize(t, 0)
		inputArray := newPyUDFTestInt64Values(allocator, []int64{7}, nil)
		input := newPyUDFTestChunked(arrow.PrimitiveTypes.Int64, inputArray)
		inputArray.Release()
		defer input.Release()
		params, err := NewRunParams(packageName, "L2_rerank", nil)
		require.NoError(t, err)
		serializedParams, err := MarshalRunParams(params)
		require.NoError(t, err)
		outputs, err := resource.run([]*arrow.Chunked{input}, serializedParams)
		require.NoError(t, err)
		defer releasePyUDFChunked(outputs)
		require.Len(t, outputs, 1)
		assert.Equal(t, "7", outputs[0].Chunk(0).ValueStr(0))
	})
}
...
def create_udf(context):
    return UDF()
`)
	resource, err := loadNativeRuntimeTestResource("native_transform_unsupported", wheel)
	require.NoError(t, err)
	defer func() { require.NoError(t, resource.Close()) }()
