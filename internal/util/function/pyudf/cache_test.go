...
func newTestCache(t *testing.T, loader ResourceLoader) *Cache {
	t.Helper()
	cache, err := newCache(context.Background(), newResourceInfo(), loader)
...
func syncCacheResource(t *testing.T, cache *Cache, resources ...*fileresource.ResolvedFileResource) {
	t.Helper()
	source := cache.source.(*resourceInfo)
...
func TestCacheSnapshotReadinessAndVersion(t *testing.T) {
	...
	require.NoError(t, cache.source.(*resourceInfo).OnFileResourceSync(fileresource.SyncEvent{Version: 1}))
	_, err = cache.Acquire(context.Background(), "rank_udf", "L2_rerank")
	assert.ErrorIs(t, err, merr.ErrParameterInvalid)

	newResource := testWheelResource(2, "rank_udf")
	require.NoError(t, cache.source.(*resourceInfo).OnFileResourceSync(fileresource.SyncEvent{Version: 3, Resources: []*fileresource.ResolvedFileResource{newResource}}))
	oldResource := testWheelResource(1, "rank_udf")
	require.NoError(t, cache.source.(*resourceInfo).OnFileResourceSync(fileresource.SyncEvent{Version: 1, Resources: []*fileresource.ResolvedFileResource{oldResource}}))
	resolved, err := cache.resolveResource("rank_udf")
	require.NoError(t, err)
	assert.Equal(t, int64(2), resolved.ID)
}

func TestCacheReadsSnapshotSynchronizedBeforeCreation(t *testing.T) {
	...
	cache, err := newCache(
		context.Background(),
		source,
		&fakeResourceLoader{load: func(context.Context, fileresource.ResolvedFileResource, string) (LoadedResource, error) {
	...
	lease, err := cache.Acquire(context.Background(), wheel.Name, "L2_rerank")
	require.NoError(t, err)
	lease.Release()
}
...
func TestCacheEvictionWaitsForLeases(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return resource.closeCount.Load() == 1
	}, time.Second, time.Millisecond)
	second.Release()
	assert.Equal(t, int64(1), resource.closeCount.Load())
}

func TestCacheEvictionDoesNotWaitForResourceClose(t *testing.T) {
	closeStarted := make(chan struct{})
	allowClose := make(chan struct{})
	defer close(allowClose)
	resource := &fakeLoadedResource{close: func() error {
		close(closeStarted)
		<-allowClose
		return nil
	}}
	...
	go func() {
		syncDone <- cache.source.(*resourceInfo).OnFileResourceSync(fileresource.SyncEvent{Version: 2})
	}()
	...
	case err := <-syncDone:
		require.NoError(t, err)
	case <-time.After(time.Second):
		t.Fatal("resource close blocked the FileResource listener")
	}
}
...
func TestCacheRetriesWhenStaleLoadFails(t *testing.T) {
	...
	cache = newTestCache(t, &fakeResourceLoader{load: func(_ context.Context, resource fileresource.ResolvedFileResource, _ string) (LoadedResource, error) {
		if loadCount.Add(1) == 1 {
			...
			syncCacheResource(t, cache, resourceV2)
...
func TestCacheRetriesAfterSyncDuringLoad(t *testing.T) {
	...
	assert.Equal(t, int64(2), loadCount.Load())
	require.Eventually(t, func() bool {
		return oldResource.closeCount.Load() == 1
	}, time.Second, time.Millisecond)
...
func TestCacheCloseCancelsSharedLoad(t *testing.T) {
	loadStarted := make(chan struct{})
	cache, err := newCache(context.Background(), newResourceInfo(), &fakeResourceLoader{load: func(ctx context.Context, _ fileresource.ResolvedFileResource, _ string) (LoadedResource, error) {
		close(loadStarted)
		<-ctx.Done()
		return nil, ctx.Err()
	}})
...
func TestCacheClose(t *testing.T) {
	...
	cache.Close()
	source.mu.Lock()
	assert.Empty(t, source.observers)
	source.mu.Unlock()
	require.NoError(t, source.OnFileResourceSync(fileresource.SyncEvent{Version: 2}))
	assert.Equal(t, int64(0), resource.closeCount.Load())
	_, err = cache.Acquire(context.Background(), "rank_udf", "L2_rerank")
	assert.ErrorIs(t, err, merr.ErrServiceInternal)
	lease.Release()
...
func TestNewCacheValidation(t *testing.T) {
	_, err := newCache(context.Background(), nil, nil)
	assert.ErrorIs(t, err, merr.ErrServiceInternal)
	cache, err := newCache(context.TODO(), newResourceInfo(), nil)
	require.NoError(t, err)
	cache.Close()
}
