...
func TestEmbeddingClientCheck(t *testing.T) {
	{
		c := NewSiliconflowEmbeddingClient("", "mock_uri")
		err := c.Check()
	...
	{
		c := NewSiliconflowEmbeddingClient("mock_key", "")
		err := c.Check()
		assert.True(t, err != nil)
		fmt.Println(err)
	}
	...
	{
		c := NewSiliconflowEmbeddingClient("mock_key", "mock_uri")
		err := c.Check()
		assert.True(t, err == nil)
	}
}
...
func TestEmbeddingOK(t *testing.T) {
  ...
  "usage": {
	...
	{
		c := NewSiliconflowEmbeddingClient("mock_key", url)
		err := c.Check()
		assert.True(t, err == nil)
		ret, err := c.Embedding("BAAI/bge-large-zh-v1.5", []string{"sentence"}, "float", 0)
		assert.True(t, err == nil)
...
func TestEmbeddingFailed(t *testing.T) {
...

	{
		...
		err := c.Check()
		assert.True(t, err == nil)
		_, err = c.Embedding("BAAI/bge-large-zh-v1.5", []string{"sentence"}, "float", 0)
		assert.True(t, err != nil)
	}
}
