...
func newWeightedFunction(collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema, pkTypeOverride ...schemapb.DataType) (Reranker, error) {
	...
	if len(base.GetInputFieldNames()) != 0 {
		return nil, merr.WrapErrParameterInvalidMsg("the weighted function does not support input parameters, but got %s", base.GetInputFieldNames())
	}
	...
	if len(weights) == 0 {
		return nil, merr.WrapErrParameterInvalidMsg(WeightsParamsKey + " not found")
	}
