...
func (s *FunctionScoreSuite) TestNewFunctionScore() {
	...
	f, err := NewFunctionScore(schema, funcScores, map[string]string{models.ClusterIDKey: "test-cluster"})
	s.NoError(err)
	...
	// two ranker but one was boost scorer
	{
		...
		_, err := NewFunctionScore(schema, funcScores, map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
