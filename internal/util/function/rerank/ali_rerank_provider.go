...
func newAliProvider(params []*commonpb.KeyValuePair, conf map[string]string, credentials *credentials.Credentials, extralInfo map[string]string) (modelProvider, error) {
	apiKey, url, err := models.ParseAKAndURL(credentials, params, conf, models.DashscopeAKEnvStr, extralInfo[models.ClusterIDKey])
	if err != nil {
		return nil, err
	}
	if url == "" {
