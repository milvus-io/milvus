...
func newModelFunction(collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema, extraInfo *models.ModelExtraInfo, pkTypeOverride ...schemapb.DataType) (Reranker, error) {
	...
	if len(base.GetInputFieldNames()) != 1 {
		return nil, merr.WrapErrParameterInvalidMsg("rerank model only supports single input, but gets [%s] input", base.GetInputFieldNames())
	}

	if base.GetInputFieldTypes()[0] != schemapb.DataType_VarChar {
		return nil, merr.WrapErrParameterInvalidMsg("rerank model only support varchar, bug got [%s]", base.GetInputFieldTypes()[0].String())
	}
	...
	for _, param := range funcSchema.Params {
		if param.Key == queryKeyName {
			if err := json.Unmarshal([]byte(param.Value), &queries); err != nil {
				return nil, merr.WrapErrParameterInvalidMsg("parse rerank params [queries] failed, err: %v", err)
			}
		}
	}
	if len(queries) == 0 {
		return nil, merr.WrapErrParameterInvalidMsg("rerank function lost params queries")
	}
...
func (model *ModelFunction[T]) processOneSearchData(ctx context.Context, searchParams *SearchParams, query string, cols []*columns, idGroup map[any]any) (*IDScores[T], error) {
	...
	for i := 0; i < len(texts); i += model.provider.maxBatch() {
		...
		if len(newScores) != end-i {
			return nil, merr.WrapErrServiceInternalMsg("call rerank service failed, %d docs but got %d scores", end-i, len(newScores))
		}
...
func (model *ModelFunction[T]) Process(ctx context.Context, searchParams *SearchParams, inputs *rerankInputs) (*rerankOutputs, error) {
	if len(model.queries) != int(searchParams.nq) {
		return nil, merr.WrapErrParameterInvalidMsg("nq must equal to queries size, but got nq [%d], queries size [%d], queries: [%v]", searchParams.nq, len(model.queries), model.queries)
	}
