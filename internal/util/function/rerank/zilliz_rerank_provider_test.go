...
func (s *ZillizRerankProviderSuite) TestNewZillizProvider_Success() {
	tests := []struct {
	...
	}{
		{
			...
			extralInfo: map[string]string{
				models.ClusterIDKey: "test-cluster",
			},
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			provider, err := newZillizProvider(tt.params, tt.conf, tt.extralInfo)
...
func (s *ZillizRerankProviderSuite) TestNewZillizProvider_InvalidBatchSize() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			provider, err := newZillizProvider(params, conf, extralInfo)
			s.Error(err)
			s.Nil(provider)
...
func (s *ZillizRerankProviderSuite) TestNewZillizProvider_MissingConfig() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			provider, err := newZillizProvider(params, tt.conf, extralInfo)
			s.Error(err)
			s.Nil(provider)
			s.Contains(err.Error(), tt.expectedErr)
		})
...
func TestZillizProviderIntegration(t *testing.T) {
	...
	_, err := newProvider(params, extralInfo)
	assert.Error(t, err)
