...
func (s *RerankModelSuite) TestNewProvider() {
	{
		...
		_, err := newProvider(params, map[string]string{models.ClusterIDKey: "test-cluster"})
		s.ErrorContains(err, "Unknow rerank model provider")
	}

	{
		_, err := newProvider([]*commonpb.KeyValuePair{}, map[string]string{models.ClusterIDKey: "test-cluster"})
...
func (s *RerankModelSuite) TestCallVllm() {
	{
		...
		provder, err := newProvider(params, map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
...
func (s *RerankModelSuite) TestCallSiliconFlow() {
	{
		...
		provder, err := newSiliconflowProvider(params, map[string]string{models.URLParamKey: ts.URL}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
...
func (s *RerankModelSuite) TestCallCohere() {
	{
		...
		provder, err := newCohereProvider(params, map[string]string{models.URLParamKey: ts.URL}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
		scores, err := provder.rerank(context.Background(), "mytest", []string{"t1", "t2"})
		s.NoError(err)
		s.Equal([]float32{0.1, 0.2}, scores)
...
func (s *RerankModelSuite) TestCallVoyageAI() {
	{
		...
		provder, err := newVoyageaiProvider(params, map[string]string{models.URLParamKey: ts.URL}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
		scores, err := provder.rerank(context.Background(), "mytest", []string{"t1", "t2", "t3"})
		s.NoError(err)
		s.Equal([]float32{0.0, 0.1, 0.2}, scores)
...
func (s *RerankModelSuite) TestCallAli() {
	{
		...
		provder, err := newAliProvider(params, map[string]string{models.URLParamKey: ts.URL}, credentials.NewCredentials(map[string]string{"mock.apikey": "mock"}), map[string]string{models.ClusterIDKey: "test-cluster"})
		s.NoError(err)
		scores, err := provder.rerank(context.Background(), "mytest", []string{"t1", "t2", "t3"})
		s.NoError(err)
		s.Equal([]float32{0.0, 0.1, 0.2}, scores)
...
func (s *RerankModelSuite) TestNewModelFunction() {
	...
	}
	{
		...
		_, err := newModelFunction(schema, functionSchema, map[string]string{models.ClusterIDKey: "test-cluster"})
...
func (s *RerankModelSuite) TestRerankProcess() {
	...
	}
	{
		...
		{
			nq := int64(1)
			f, err := newModelFunction(schema, functionSchema, map[string]string{models.ClusterIDKey: "test-cluster"})
			s.NoError(err)
