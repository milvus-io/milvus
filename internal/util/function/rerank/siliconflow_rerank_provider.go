...
func newSiliconflowProvider(params []*commonpb.KeyValuePair, conf map[string]string, credentials *credentials.Credentials, extralInfo map[string]string) (modelProvider, error) {
	apiKey, url, err := models.ParseAKAndURL(credentials, params, conf, models.SiliconflowAKEnvStr, extralInfo[models.ClusterIDKey])
	if err != nil {
		return nil, err
	}
	if url == "" {
