...
func (s *DecayFunctionSuite) TestNewDecayErrors() {
...

	{
		for i := 0; i < 4; i++ {
			...
			functionSchema.Params[i].Value = "1"
		}
	}
}
...
func (s *DecayFunctionSuite) TestAllTypesInput() {
	...
	for _, inputType := range inputTypes {
		schema.Fields[2].DataType = inputType
		_, err := newDecayFunction(schema, functionSchema)
		s.NoError(err)
	}
...
func (s *DecayFunctionSuite) TestRerankProcess() {
	...
	// nq = 1
	{
		...
		s.NoError(err)
		data := genSearchResultData(nq, 10, schemapb.DataType_Int64, "ts")
	...
	}
	{
		...
		s.NoError(err)
		// ts/id data: 0 - 9
		data1 := genSearchResultData(nq, 10, schemapb.DataType_Int64, "ts")
		// ts/id data: 0 - 3
		data2 := genSearchResultData(nq, 4, schemapb.DataType_Int64, "ts")
...
func genSearchResultData(nq int64, topk int64, dType schemapb.DataType, fieldName string) *schemapb.SearchResultData {
	tops := make([]int64, nq)
	for i := 0; i < int(nq); i++ {
		tops[i] = topk
	}
