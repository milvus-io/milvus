...
func (r *rowParser) arrayToFieldData(arr []interface{}, field *schemapb.FieldSchema) (*schemapb.ScalarField, error) {
	...
	case schemapb.DataType_Int8, schemapb.DataType_Int16, schemapb.DataType_Int32:
		...
		for i, v := range arr {
			...
			if err != nil {
				return nil, merr.WrapErrImportFailedErr(err, "failed to parse int32 for type:%s", eleType.String())
			}
			values[i] = int32(num)
		}
		return &schemapb.ScalarField{
	...
	case schemapb.DataType_Int64:
		...
		for i, v := range arr {
			...
			if err != nil {
				return nil, merr.WrapErrImportFailedErr(err, "failed to parse int64 for type:%s", eleType.String())
			}
			values[i] = num
		}
		return &schemapb.ScalarField{
	...
	case schemapb.DataType_Float:
		...
		for i, v := range arr {
			...
			if err != nil {
				return nil, merr.WrapErrImportFailedErr(err, "failed to parse float32 for type:%s", eleType.String())
			}
			values[i] = float32(num)
		}
		if err := typeutil.VerifyFloats32(values); err != nil {
			return nil, merr.WrapErrImportFailedErr(err, "float32 verification failed for type:%s", eleType.String())
		}
		return &schemapb.ScalarField{
	...
	case schemapb.DataType_Double:
		...
		for i, v := range arr {
			...
			if err != nil {
				return nil, merr.WrapErrImportFailedErr(err, "failed to parse float64 for type:%s", eleType.String())
			}
			values[i] = num
		}
		if err := typeutil.VerifyFloats64(values); err != nil {
			return nil, merr.WrapErrImportFailedErr(err, "float64 verification failed for type:%s", eleType.String())
		}
		return &schemapb.ScalarField{
	...
	case schemapb.DataType_Timestamptz:
		...
		for i, v := range arr {
			...
			num, err := strconv.ParseInt(value.String(), 10, 64)
			if err != nil {
				return nil, merr.WrapErrImportFailedErr(err, "failed to parse timesamptz for type:%s", eleType.String())
			}
			values[i] = num
		}
		return &schemapb.ScalarField{
	...
	case schemapb.DataType_FloatVector:
		...
		for _, vectorAny := range vectors {
			...
			for i, elem := range v {
				...
				num, err := strconv.ParseFloat(value.String(), 32)
				if err != nil {
					return nil, merr.WrapErrImportFailedErr(err, "failed to parse float for type:%s", elementType.String())
				}
...
func (r *rowParser) wrapTypeError(v any, field *schemapb.FieldSchema) error {
	return merr.WrapErrImportFailedMsg("expected type '%s' for field '%s', got type '%T' with value '%v'",
		field.GetDataType().String(), field.GetName(), v, v)
}

func (r *rowParser) wrapDimError(actualDim int, field *schemapb.FieldSchema) error {
	return merr.WrapErrImportFailedMsg("expected dim '%d' for field '%s' with type '%s', got dim '%d'",
...
func (r *rowParser) wrapArrayValueTypeError(v any, eleType schemapb.DataType) error {
	return merr.WrapErrImportFailedMsg("expected element type '%s' in array field, got type '%T' with value '%v'",
		eleType.String(), v, v)
}
