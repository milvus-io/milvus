...
func verify(schema *schemapb.CollectionSchema, insertLogs map[int64][]string) (map[int64][]string, *schemapb.CollectionSchema, error) {
	...
	if _, ok := insertLogs[common.TimeStampField]; !ok {
		return nil, nil, merr.WrapErrImportFailed("no binlog for TimestampField")
	}
	...
	binSchema, err := proto.Marshal(schema)
	if err != nil {
		return nil, nil, merr.WrapErrImportFailed("unable to marshar collection schema")
	}

	cloneSchema := &schemapb.CollectionSchema{}
	err = proto.Unmarshal(binSchema, cloneSchema)
	if err != nil {
		return nil, nil, merr.WrapErrImportFailed("unable to clone collection schema")
	}
