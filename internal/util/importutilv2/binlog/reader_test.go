...
func TestStorageV3Reader_BackupFieldSelection(t *testing.T) {
	...
	r, err := NewStorageV3ManifestReader(context.Background(), nil, target, &indexpb.StorageConfig{},
		manifest, 0, math.MaxUint64, 1024, "",
		&internalpb.SnapshotImportSource{Version: 1, ManifestPath: manifest, SourceCommitTimestamp: 300}, 1024)
	require.NoError(t, err)
	defer r.Close()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			r, err := NewStorageV3ManifestReader(context.Background(), nil,
				...
				nil, packed.MarshalManifestPath("snapshot/segment/10", 7), 0, math.MaxUint64, 1024, "", nil, 0)
			require.Nil(t, r)
...
func TestStorageV3Reader_CMEKWithoutTextUsesPackedReaderContext(t *testing.T) {
	...
	for _, addedText := range []bool{false, true} {
		t.Run(fmt.Sprintf("target_only_nullable_text=%t", addedText), func(t *testing.T) {
			schema := &schemapb.CollectionSchema{Fields: []*schemapb.FieldSchema{
				{FieldID: 100, Name: "pk", DataType: schemapb.DataType_Int64, IsPrimaryKey: true},
			}}
			if addedText {
			...
			r, err := NewStorageV3ManifestReader(context.Background(), nil, schema, &indexpb.StorageConfig{},
				manifestPath, 0, math.MaxUint64, 1024, importEzk, nil, 0)
			require.NoError(t, err)
			defer r.Close()
			...
			require.Equal(t, addedText, typeutil.HasTextField(schema), "keep the target schema for NULL filling")
		})
	}
}
