...
func parseSparseFloatVectorStructs(structs []map[string]arrow.Array) ([][]byte, int, error) {
	byteArr := make([][]byte, 0)
	maxDim := int(0)
	for _, st := range structs {
		...
		if indices.Len() != values.Len() {
			return nil, 0, merr.WrapErrImportFailed("Invalid parquet struct for SparseFloatVector: 'indices' or 'values' length mismatched")
		}
		...
		default:
			return nil, 0, merr.WrapErrImportFailed("Invalid parquet struct for SparseFloatVector: index type must be uint32 or int64")
		}
		...
		default:
			return nil, 0, merr.WrapErrImportFailed("Invalid parquet struct for SparseFloatVector: value type must be float32 or float64")
		}

		for i := 0; i < indicesList.Len(); i++ {
			start, end := indicesList.ValueOffsets(i)
			rowDim := (int)(end - start)
			rowVec := make([]byte, (end-start)*8)
			if rowDim > maxDim {
				maxDim = rowDim
			}
			for i := start; i < end; i++ {
				typeutil.SparseFloatRowSetAt(rowVec, (int)(i-start), getIndexFunc((int)(i)), getValueFunc((int)(i)))
			}
