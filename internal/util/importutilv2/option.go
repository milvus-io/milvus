...
func ValidateSnapshotSourceOptions(options Options) error {
	contractKeys := []string{SourceType, BackupFlag, L0Import, StorageVersion, EZK, ExternalSpec, SnapshotSourceURI, PartitionMapping}
	...
	for _, option := range options {
		...
		if option.GetKey() == SourceType || option.GetKey() == ExternalSpec || option.GetKey() == SnapshotSourceURI || option.GetKey() == PartitionMapping {
