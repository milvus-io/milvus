...
func TestHashPKs_EquivalentToHashPK2Channels(t *testing.T) {
	for _, n := range []int{1, 2, 3, 4, 8, 16} {
		...
		assert.Equal(t, legacyHash(t, ints, ch), tbl.HashPKs(ints), "int64 n=%d", n)
		strs := strIDs([]string{"", "a", "abc", "namespace-42", "long-key-aaaaaaaaaaaaaaaa", strings.Repeat("x", 101)})
		assert.Equal(t, legacyHash(t, strs, ch), tbl.HashPKs(strs), "varchar n=%d", n)

		// Empty inputs must yield nil (not an empty slice), matching legacy exactly.
		emptyInts := intIDs([]int64{})
		assert.Equal(t, legacyHash(t, emptyInts, ch), tbl.HashPKs(emptyInts), "empty int64 n=%d", n)
		emptyStrs := strIDs([]string{})
		assert.Equal(t, legacyHash(t, emptyStrs, ch), tbl.HashPKs(emptyStrs), "empty varchar n=%d", n)
	}
}
...
// legacyHash returns typeutil.HashPK2Channels' values, failing the test on
// error. HashPK2Channels gained an error return; these equivalence tests only
// use well-formed inputs, so an error is a test bug.
func legacyHash(t *testing.T, pks *schemapb.IDs, ch []string) []uint32 {
	t.Helper()
	vals, err := typeutil.HashPK2Channels(pks, ch)
