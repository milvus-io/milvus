...
func UpdateJSONStatsBuildExecutorPoolSize(threads int) error {
	if threads < 1 {
		threads = 1
	}
	status := C.executor_set_json_stats_build_thread_num(C.int(threads))
	return HandleCStatus(&status, "failed to resize json stats build executor")
}
...
func UpdateJSONStatsBuildMaxInflightBytes(maxInflightBytes int64) error {
	if maxInflightBytes < 0 {
		maxInflightBytes = paramtable.DefaultJSONStatsBuildMaxInflightBytes
	}
	status := C.executor_set_json_stats_build_max_inflight_bytes(C.int64_t(maxInflightBytes))
	return HandleCStatus(&status, "failed to update json stats build memory budget")
}
...
func RegisterJSONStatsBuildExecutorWatcher(pt *paramtable.ComponentParam, source string) {
	...
	handler := func(evt *config.Event) {
		...
		if err := UpdateJSONStatsBuildExecutorPoolSize(threads); err != nil {
			mlog.Warn(context.TODO(), "failed to resize json stats build executor",
...
func RegisterJSONStatsBuildMemoryBudgetWatcher(pt *paramtable.ComponentParam, source string) {
	...
	handler := func(evt *config.Event) {
		...
		if err := UpdateJSONStatsBuildMaxInflightBytes(maxInflightBytes); err != nil {
			mlog.Warn(context.TODO(), "failed to resize json stats build memory budget",
				mlog.String("source", source),
				mlog.String("trigger", key),
