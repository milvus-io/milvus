...
func NewSession(ctx context.Context, metaRoot string, client *clientv3.Client, opts ...SessionOption) *Session {
	session := &Session{
		...
		retryRegisterWhenKeepAliveCancel: paramtable.Get().CommonCfg.RetryRegisterWhenKeepAliveCancel.GetAsBool(),
		reuseNodeID:                      true,
	}
...
func (s *Session) registerService(retryTimes uint, reRegister bool) (<-chan *clientv3.LeaseKeepAliveResponse, error) {
	...
	log.Debug("service begin to register to etcd")
	...
	if reRegister {
		...
		err = retry.Do(s.ctx, registerFn, retry.Attempts(retryTimes), retry.FnTimeout(time.Second*time.Duration(10)), retry.TotalTimeout(time.Second*time.Duration(s.sessionTTL)))
	} else {
		err = retry.Do(s.ctx, registerFn, retry.Attempts(retryTimes))
	}
	if err != nil {
		return nil, err
	}
