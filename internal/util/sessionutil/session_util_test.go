...
// when the session context is cancelled (via Stop()).
...
func TestGetServerID(t *testing.T) {
	...
	t.Run("checkIDExist error on cancelled context", func(t *testing.T) {
...
// exits and closes the event channel when the context is cancelled.
...
// ProcessActiveStandBy when its context is cancelled.
...
func TestProcessActiveStandByContextCancel(t *testing.T) {
	...
	// The ProcessActiveStandBy should exit when context is cancelled
	select {
	case <-errCh:
...
// if the context is cancelled during this Get, the session should exit gracefully.
