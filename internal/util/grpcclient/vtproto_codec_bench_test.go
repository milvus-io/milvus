...
// --- SearchResultData benchmarks (external API type, now also has VT) ---
...
func BenchmarkSearchResultData_Marshal_VT(b *testing.B) {
	msg := makeSearchResultData(10000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = msg.MarshalVT()
...
func BenchmarkSearchResultData_Unmarshal_VT(b *testing.B) {
	msg := makeSearchResultData(10000)
	data, _ := proto.Marshal(msg)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
