	...
	configOnce.Do(func() {
		...
		if budget := encoderStateBudget / zstdEncoderStateBytes(codecLevel); codecConcurrency > budget {
			mlog.Warn(context.TODO(),
				"grpc zstd encoder concurrency clamped to stay within the state memory budget",
				mlog.String("compressionLevel", cfg.CompressionLevel.GetValue()),
				mlog.Int("requested", codecConcurrency),
				mlog.Int("allowed", budget))
