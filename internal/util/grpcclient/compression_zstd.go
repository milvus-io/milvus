...
func newEncoder(level zstd.EncoderLevel, concurrency int, crc bool) *zstd.Encoder {
	...
	if err != nil {
		...
		// the whole pool.
		mlog.Warn(context.TODO(), "failed to build the configured zstd encoder, falling back", mlog.Err(err))
...
func newZstdReader() resettableReader {
	...
	if err != nil {
		...
		mlog.Warn(context.TODO(), "failed to build the configured zstd decoder, falling back", mlog.Err(err))
