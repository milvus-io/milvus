...
func (suite *SyncManagerSuite) SetupTest() {
	...
	var err error
	suite.tempDir, err = os.MkdirTemp("", "fileresource_test_sync_*")
	suite.NoError(err)
...
func (suite *RefManagerSuite) SetupTest() {
	// Create temporary directory for tests
	var err error
	suite.tempDir, err = os.MkdirTemp("", "fileresource_test_ref_*")
	suite.NoError(err)
...
func (suite *ManagerFactorySuite) TestNewManager_RefManager() {
	manager := NewManager(suite.mockStorage, RefMode)
	suite.IsType(&RefManager{}, manager)
	suite.Equal(RefMode, manager.Mode())
}
