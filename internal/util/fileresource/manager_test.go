...
func (suite *SyncManagerSuite) SetupTest() {
	...
	suite.manager = &SyncManager{
		BaseManager: BaseManager{localPath: suite.tempDir, downloadTimeout: time.Minute},
...
func (suite *SyncManagerSuite) TestSync_Timeout() {
	resources := []*internalpb.FileResourceInfo{
		{Id: 1, Name: "slow.file", Path: "/storage/slow.file"},
	}
	suite.manager.downloadTimeout = 10 * time.Millisecond
