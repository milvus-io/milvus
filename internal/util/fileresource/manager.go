...
func newBaseManager() BaseManager {
	...
	return BaseManager{
		...
		downloadTimeout: params.CommonCfg.FileResourceDownloadTimeout.GetAsDurationByParse(),
	}
}
...
func (m *BaseManager) downloadFile(ctx context.Context, downloader storage.ChunkManager, resource *internalpb.FileResourceInfo, destination string) error {
	if downloader == nil {
...
func (m *SyncManager) Sync(ctx context.Context, version uint64, resourceList []*internalpb.FileResourceInfo) error {
	...
	for _, resource := range resourceList {
		...
		if err := m.downloadFile(ctx, m.downloader, resource, localFilePath); err != nil {
			mlog.Warn(ctx, "download file resource failed",
				...
				mlog.Duration("timeout", m.downloadTimeout),
				mlog.Err(err))
			return err
		}
...
func (m *RefManager) Download(ctx context.Context, downloader storage.ChunkManager, resources ...*internalpb.FileResourceInfo) error {
	...
	for _, r := range resources {
		...
		_, err, _ := m.sf.Do(key, func() (interface{}, error) {
			...
			if err := m.downloadFile(ctx, downloader, resource, fileName); err != nil {
				mlog.Warn(ctx, "download file resource failed",
					mlog.String("name", resource.GetName()),
					mlog.String("path", resource.GetPath()),
					mlog.Int64("resourceID", resource.GetId()),
