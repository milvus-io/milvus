...
func (s *scheduler) handleAddTaskRequest(req addTaskReq, maxWaitTaskNum int64, now time.Time) bool {
	class := s.policy.Classify(req.task)
	...
	// The final capacity check is this request's admission snapshot; later
	// config changes apply to subsequent admissions. Keep the lane stamp fixed.
	capacity, available := s.waitingTaskCapacityAvailable(class, maxWaitTaskNum)
	if !available {
...
func (s *scheduler) waitingTaskCapacityAvailable(class taskClass, maxRegular int64) (int64, bool) {
	if class == taskClassPriorityLane {
