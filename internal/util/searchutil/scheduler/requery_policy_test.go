...
func TestRequeryPriorityPolicyBoundedBurst(t *testing.T) {
	...
	for _, expectedTask := range expected {
		assert.Equal(t, expectedTask, policy.Pop(now).Task)
	}
...
func TestRequeryPriorityPolicyCountsFallbackInNextBurst(t *testing.T) {
	...
	assert.Equal(t, laneTaskValues[0], policy.Pop(now).Task)
	assert.Equal(t, laneTaskValues[1], policy.Pop(now).Task)
	assert.Equal(t, laneTaskValues[2], policy.Pop(now).Task)
	...
	assert.Equal(t, laneTaskValues[3], policy.Pop(now).Task)
	...
	assert.Equal(t, laneTaskValues[4], policy.Pop(now).Task)
	assert.Equal(t, laneTaskValues[5], policy.Pop(now).Task)
	assert.Equal(t, regularTask, policy.Pop(now).Task)
	assert.Equal(t, laneTaskValues[6], policy.Pop(now).Task)
}
...
func TestRequeryPriorityPolicyCanceledRegularDoesNotRefreshCredit(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		assert.Equal(t, laneTaskValues[i], policy.Pop(now).Task)
	}
	assert.Equal(t, canceledTask, policy.Pop(now).Task)
	assert.Equal(t, liveTask, policy.Pop(now).Task)
	assert.Equal(t, laneTaskValues[3], policy.Pop(now).Task)
}
...
func TestRequeryPriorityPolicyUsesMergedRequestCountAsCredit(t *testing.T) {
	...
	assert.Equal(t, mergedRegularTask, policy.Pop(now).Task)
	for i := 3; i < 8; i++ {
		assert.Equal(t, laneTaskValues[i], policy.Pop(now).Task)
	}
	assert.Equal(t, nextRegularTask, policy.Pop(now).Task)
	assert.Equal(t, laneTaskValues[8], policy.Pop(now).Task)
}
