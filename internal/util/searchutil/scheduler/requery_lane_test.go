...
func TestRequeryLaneCleanupRetry(t *testing.T) {
	...
	expired.class = taskClassPriorityLane
	...
	assert.Equal(t, 1, policy.lane.len())
	assert.Equal(t, int64(1), s.getRequeryWaitingTaskTotal())
	assert.Equal(t, int64(3), s.GetWaitingTaskTotalNQ())
...
func TestRequeryLaneStagedTaskCountsAgainstCapacity(t *testing.T) {
	...
	capacity, available := s.waitingTaskCapacityAvailable(taskClassPriorityLane, 1024)
	assert.Equal(t, int64(2), capacity)
	assert.True(t, available)

	pt.QueryNodeCfg.RequeryUnsolvedQueueSize.SwapTempValue("1")
	errCh := make(chan error, 1)
	s.handleAddTaskRequest(addTaskReq{task: second, err: errCh}, 1024, now)
	err := <-errCh
	assert.ErrorIs(t, err, merr.ErrServiceTooManyRequests)
	assert.ErrorContains(t, err, pt.QueryNodeCfg.RequeryUnsolvedQueueSize.Key)
...
type classificationCallbackPolicy struct {
	*requeryPriorityPolicy
	afterClassify func()
}

func (p *classificationCallbackPolicy) Classify(task Task) taskClass {
	class := p.requeryPriorityPolicy.Classify(task)
	p.afterClassify()
	return class
}

func TestRequeryLaneCapacityChangesAfterClassification(t *testing.T) {
	paramtable.Init()
	pt := paramtable.Get()
	for _, capacity := range []string{"1", "0"} {
		t.Run(capacity, func(t *testing.T) {
			oldLane := pt.QueryNodeCfg.RequeryUnsolvedQueueSize.SwapTempValue("2")
			defer pt.QueryNodeCfg.RequeryUnsolvedQueueSize.SwapTempValue(oldLane)
			s := newUnstartedScheduler()
			now := time.Now()
			require.NoError(t, addTask(s, newMockTask(mockTaskConfig{nq: 3}), now))
			require.NoError(t, addTask(s, newMockTask(mockTaskConfig{ctx: requeryCtx(), nq: 2}), now))
			staged, _, _ := s.setupExecListener(nil, now)
			require.Equal(t, taskClassPriorityLane, staged.class)
			s.policy = &classificationCallbackPolicy{
				requeryPriorityPolicy: s.policy.(*requeryPriorityPolicy),
				afterClassify: func() {
					pt.QueryNodeCfg.RequeryUnsolvedQueueSize.SwapTempValue(capacity)
				},
			}

			errCh := make(chan error, 1)
			more := s.handleAddTaskRequest(addTaskReq{
				task: newMockTask(mockTaskConfig{ctx: requeryCtx()}), err: errCh,
			}, 1, now)
			err := <-errCh
			assert.False(t, more)
			assert.ErrorIs(t, err, merr.ErrServiceTooManyRequests)
			// A request already classified as lane must not fall back to regular.
			assert.ErrorContains(t, err, pt.QueryNodeCfg.RequeryUnsolvedQueueSize.Key)
			assert.True(t, staged.valid())
			assert.EqualValues(t, 1, s.getRegularWaitingTaskTotal())
			assert.EqualValues(t, 1, s.getRequeryWaitingTaskTotal())
			assert.EqualValues(t, 5, s.GetWaitingTaskTotalNQ())
		})
	}
}

func TestRequeryLaneRechecksAfterCleanup(t *testing.T) {
	paramtable.Init()
	pt := paramtable.Get()
	for _, change := range []string{"shrink", "disable", "cancel"} {
		t.Run(change, func(t *testing.T) {
			...
			require.NoError(t, addTask(s, newMockTask(mockTaskConfig{ctx: requeryCtx(), nq: 2}), now))
			staged, _, _ := s.setupExecListener(nil, now)
			expiredCtx, cancelExpired := context.WithCancel(requeryCtx())
			defer cancelExpired()
			incomingCtx, cancelIncoming := context.WithCancel(requeryCtx())
			defer cancelIncoming()
			cleanupCalls := 0
			expired := &doneCallbackTask{
				Task: newMockTask(mockTaskConfig{ctx: expiredCtx, nq: 3}),
				callback: func() {
					cleanupCalls++
					switch change {
					case "shrink":
			...
			require.NoError(t, addTask(s, expired, now))
			cancelExpired()
			errCh := make(chan error, 1)
			s.handleAddTaskRequest(addTaskReq{
				task: newMockTask(mockTaskConfig{ctx: incomingCtx}), err: errCh,
			}, 1024, now)
			err := <-errCh
			if change == "cancel" {
			...
			assert.True(t, staged.valid())
			assert.Zero(t, s.policy.Len())
			assert.EqualValues(t, 1, s.GetWaitingTaskTotal())
			assert.EqualValues(t, 2, s.GetWaitingTaskTotalNQ())
		})
	}
}

func TestRequeryLaneMergedRegularCountsAndClear(t *testing.T) {
	paramtable.Init()
	s := newUnstartedScheduler()
	now := time.Now()
	require.NoError(t, addTask(s, newMockTask(mockTaskConfig{mergeAble: true, nq: 3}), now))
	require.NoError(t, addTask(s, newMockTask(mockTaskConfig{mergeAble: true, nq: 5}), now))
	require.NoError(t, addTask(s, newMockTask(mockTaskConfig{ctx: requeryCtx(), nq: 7}), now))
	assert.EqualValues(t, 1, s.getRegularWaitingTaskTotal())
	assert.EqualValues(t, 1, s.getRequeryWaitingTaskTotal())
	assert.EqualValues(t, 2, s.GetWaitingTaskTotal())
	assert.EqualValues(t, 15, s.GetWaitingTaskTotalNQ())

	result, remaining := s.clearQueuedTasks(nil, "test clear", nil, now)
	assert.Equal(t, ClearResult{QueuedCleared: 2, QueuedNQCleared: 15}, result)
...
func TestUserTaskPollingQuotaRejectDoesNotTriggerGlobalCleanup(t *testing.T) {
	...
	errCh := make(chan error, 1)
	s.handleAddTaskRequest(addTaskReq{task: fresh, err: errCh}, 10, now)
...
func TestRequeryLaneDynamicDisableDrainsOldTasksWithoutCapBypass(t *testing.T) {
	...
	assert.Equal(t, taskClassPriorityLane, popped.class)
	assert.Equal(t, int64(1), s.getRequeryWaitingTaskTotal())
	...
	assert.Equal(t, taskClassRegular, popped.class)
}
...
func TestRequeryLaneClearPoppedCurrentSettlesCounters(t *testing.T) {
	...
	assert.Equal(t, taskClassPriorityLane, current.class)
...
func TestRequeryLaneWiringDisablesLaneForUserTaskPolling(t *testing.T) {
	...
	assert.True(t, ok)
	assert.Equal(t, taskClassRegular, utpScheduler.policy.Classify(newMockTask(mockTaskConfig{ctx: requeryCtx()})))
...
func TestRequeryLaneShutdownDrainsCounters(t *testing.T) {
	...
	lane1 := newMockTask(mockTaskConfig{ctx: requeryCtx(), nq: 2, executeCost: time.Millisecond})
	lane2 := newMockTask(mockTaskConfig{ctx: requeryCtx(), nq: 3, executeCost: time.Millisecond})
	lane3 := newMockTask(mockTaskConfig{ctx: requeryCtx(), nq: 4, executeCost: time.Millisecond})
	regular := newMockTask(mockTaskConfig{nq: 5, executeCost: time.Millisecond})
	...
	require.Eventually(t, func() bool {
		// One requery can be staged for execChan handoff, so two scheduler-owned
		// requeries prove that the physical lane still has work without reading
		// the scheduler-goroutine-owned lane directly.
		return s.getRequeryWaitingTaskTotal() >= 2
