...
func refreshRLSOperationTestMetadata(t *testing.T, collectionID int64, policies []*rlsutil.RowPolicy) {
	...
	rls.InvalidatePolicies(collectionID)
	t.Cleanup(func() {
		rls.InvalidatePolicies(collectionID)
	})
...
func TestHybridSearchResolvesRLSPredicateOnce(t *testing.T) {
	mockey.PatchConvey("hybrid search resolves one RLS snapshot", t, func() {
		...
		firstPlan, _, _, _, _, _, err := task.tryGeneratePlan(subSearchParams, "", nil, nil)
		require.NoError(t, err)
		secondPlan, _, _, _, _, _, err := task.tryGeneratePlan(subSearchParams, "", nil, nil)
		require.NoError(t, err)
...
func TestUpsertUsingPolicyOnlyAppliesToExistingRows(t *testing.T) {
	mockey.PatchConvey("upsert with only new rows does not require a using policy", t, func() {
		const collectionID = int64(991006)
		ctx := context.Background()
		task := createTestUpdateTask()
		...
		require.NoError(t, task.queryPreExecute(ctx, false))
	})
}
