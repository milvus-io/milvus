...
func (node *Proxy) InvalidateCollectionMetaCache(ctx context.Context, request *proxypb.InvalidateCollMetaCacheRequest) (*commonpb.Status, error) {
	...
	case commonpb.MsgType_SetRLSPrincipalTags,
		...
		rls.InvalidatePrincipalTags(collectionID, principalName)
...
func (node *Proxy) handleIfSearchByPK(ctx context.Context, request *milvuspb.SearchRequest) ([]bool, error) {
	...
	if enforceRLS {
		predicate, err := rls.ResolveUsingPredicate(ctx, collectionInfo.CollID, principalName, rls.SearchAction(false, isIterator), collectionInfo.Schema.SchemaHelper)
		if err != nil {
			return nil, err
		}
		if err := rls.MergePredicateToPlan(plan, predicate); err != nil {
			return nil, err
		}
	}
