...
func (node *Proxy) SqlQuery(ctx context.Context, request *milvuspb.SqlQueryRequest) (*milvuspb.SqlQueryResults, error) {
	...
	case sqlparser.QueryTypeSearch, sqlparser.QueryTypeHybrid:
		...
		collectionInfo, err := globalMetaCache.GetCollectionInfo(ctx, request.GetDbName(), comp.Collection, 0)
		if err != nil {
			return &milvuspb.SqlQueryResults{
				Status: merr.Status(fmt.Errorf("failed to get collection schema: %w", err)),
			}, nil
		}
		vectorField := typeutil.GetFieldByName(collectionInfo.schema.CollectionSchema, comp.VectorSearch.FieldName)
		if vectorField == nil {
			return &milvuspb.SqlQueryResults{
				Status: merr.Status(fmt.Errorf("vector field %s not found in collection %s", comp.VectorSearch.FieldName, comp.Collection)),
			}, nil
		}

		searchReq, err := sqlComponentsToSearchRequest(comp, request, vectorField)
		if err != nil {
			return &milvuspb.SqlQueryResults{
