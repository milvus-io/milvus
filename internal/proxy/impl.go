...
func (node *Proxy) checkSchemaVersionConsistency(ctx context.Context, dbName, collectionName string) error {
		...
		if stat.GetKey() == common.SchemaVersionConsistencyProportionKey {
			...
			if err != nil {
				log.Warn("failed to parse schema_version_consistency_proportion",
					...
					zap.Error(err))
				return merr.WrapErrParameterInvalidMsg(
					"invalid schema_version_consistency_proportion value: %s", value)
			}
	...
	if consistencyProportion < 0 {
		// schema_version_consistency_proportion not reported by DataCoord yet,
		// treat as fully consistent (no backfill pending).
		return nil
	}
	if consistencyProportion < 100 {
		return merr.WrapErrParameterInvalidMsg(
			"schema version consistency check failed: proportion is %.2f%%, required 100%%", consistencyProportion)
	}
	return nil
}
