...
func (it *upsertTask) queryPreExecute(ctx context.Context) error {
	...
	if len(updateIdxInUpsert) > 0 {
		...
		for fieldIdx, existField := range existFieldData {
			...
			if upsertField != nil {
				...
				if isNullableVector {
					// For COMPRESSED FORMAT nullable vector: only copy data for non-null rows
	...
	if len(insertIdxInUpsert) > 0 {
		...
		idxComputer := typeutil.NewFieldDataIdxComputer(insertWithNullField)
		for _, idx := range insertIdxInUpsert {
			fieldIdxs := idxComputer.Compute(int64(idx))
			typeutil.AppendFieldData(it.insertFieldData, insertWithNullField, int64(idx), fieldIdxs...)
		}
	}
