...
func retrieveByPKs(ctx context.Context, t *upsertTask, ids *schemapb.IDs, outputFields []string) (*milvuspb.QueryResults, segcore.StorageCost, error) {
	log := mlog.With(mlog.FieldCollectionName(t.req.GetCollectionName()))
	var err error
	...
	if t.partitionKeyMode {
		...
		if err := validatePartitionTag(partName, true); err != nil {
			log.Warn(ctx, "Invalid partition name", mlog.FieldPartitionName(partName), mlog.Err(err))
			return nil, segcore.StorageCost{}, err
		}
		partID, err := globalMetaCache.GetPartitionID(ctx, t.req.GetDbName(), t.req.GetCollectionName(), partName)
		if err != nil {
			log.Warn(ctx, "Failed to get partition id", mlog.FieldPartitionName(partName), mlog.Err(err))
			return nil, segcore.StorageCost{}, err
		}
...
func (it *upsertTask) queryPreExecute(ctx context.Context) error {
	log := mlog.With(mlog.FieldCollectionName(it.req.CollectionName))
...
func (it *upsertTask) insertPreExecute(ctx context.Context) error {
	...
	if err := validateCollectionName(collectionName); err != nil {
		mlog.Error(ctx, "valid collection name failed", mlog.FieldCollectionName(collectionName), mlog.Err(err))
		return err
	}
	...
	if allocateErr != nil {
		mlog.Warn(ctx, "failed to allocate auto id for upsert",
			...
			mlog.FieldCollectionID(it.upsertMsg.InsertMsg.CollectionID),
	...
	log := mlog.With(mlog.FieldCollectionName(it.upsertMsg.InsertMsg.CollectionName))
	if err != nil {
...
func (it *upsertTask) deletePreExecute(ctx context.Context) error {
	...
	if it.partitionKeyMode {
		...
		if err != nil {
			log.Warn(ctx, "Failed to get partition id", mlog.FieldCollectionName(collName), mlog.FieldPartitionName(partName), mlog.Err(err))
			return err
		}
...
func (it *upsertTask) PreExecute(ctx context.Context) error {
	...
	if it.schemaTimestamp != 0 {
		if it.schemaTimestamp != colInfo.updateTimestamp {
			...
			log.Info(ctx, "collection schema mismatch", mlog.FieldCollectionName(collectionName),
