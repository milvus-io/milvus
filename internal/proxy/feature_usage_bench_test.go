...
// feature". These benchmarks measure the three places where that claim is
// checkable: the per-request mark-and-flush the search path pays, the search
// parameter parse, which gained a handful of branches, and the expression
// walk, which is the one counted class whose cost is more than a branch. Run:
//
