...
func (t *sqlQueryTask) PreExecute(ctx context.Context) error {
	...
	templateValues, err := sqlparser.BuildSQLParamTemplateValues(t.sqlRequest.GetParams())
	if err != nil {
		return merr.WrapErrAsInputError(merr.WrapErrParameterInvalidMsg("failed to build SQL params: %v", err))
	}

	// Build plan directly from SqlComponents IR
	t.plan, err = sqlparser.BuildQueryPlanWithTemplateValues(t.comp, schema.schemaHelper, templateValues)
	if err != nil {
...
func sqlComponentsToSearchRequest(comp *sqlparser.SqlComponents, sqlReq *milvuspb.SqlQueryRequest, vectorField *schemapb.FieldSchema) (*milvuspb.SearchRequest, error) {
	...
	templateValues, err := sqlparser.BuildSQLParamTemplateValues(sqlReq.GetParams())
	if err != nil {
		return nil, fmt.Errorf("failed to build SQL params: %w", err)
	}
	...
	if len(vs.Vector) > 0 {
		...
		var err error
		if vs.Literal != nil {
			pg, err = sqlparser.BuildTypedPlaceholderGroupFromRaw(vs.Literal.Raw, vectorField)
		} else {
		...
		rawValue, ok := sqlReq.GetSearchParams()[vs.ParamRef]
		if !ok {
			return nil, fmt.Errorf("vector parameter %s not found in searchParams", vs.ParamRef)
		}
		pg, err := sqlparser.BuildTypedPlaceholderGroupFromRaw(rawValue, vectorField)
		if err != nil {
			return nil, fmt.Errorf("failed to build placeholder group for %s: %w", vs.ParamRef, err)
		}
		placeholderGroupBytes = pg
	} else {
