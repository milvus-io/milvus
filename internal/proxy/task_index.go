...
func checkTrain(field *schemapb.FieldSchema, indexParams map[string]string) error {
	...
	if err != nil {
		log.Info("failed to get index checker", zap.String(common.IndexTypeKey, indexType))
...
func (dit *describeIndexTask) Execute(ctx context.Context) error {
	...
	resp, err := dit.indexCoord.DescribeIndex(ctx, &indexpb.DescribeIndexRequest{CollectionID: dit.collectionID, IndexName: dit.IndexName, Timestamp: dit.Timestamp})
	if err != nil || resp == nil {
		return err
	}
