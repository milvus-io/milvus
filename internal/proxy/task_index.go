...
func parseIndexParams(m []*commonpb.KeyValuePair) (map[string]string, error) {
	...
	specifyIndexType, exist := indexParams[common.IndexTypeKey]
	if Params.AutoIndexConfig.Enable {
		if exist {
			if specifyIndexType != AutoIndexKey {
				return nil, fmt.Errorf("AutoIndex should not specify index type")
			}
		}
		// override params
...
func (cit *createIndexTask) PreExecute(ctx context.Context) error {
	...
	indexParams, err2 := parseIndexParams(cit.GetExtraParams())
	if err2 != nil {
		log.Info("failed to parse index params", zap.Error(err2))
		return fmt.Errorf("failed to parse index params: %s", err2)
	}
...
func (cit *createIndexTask) Execute(ctx context.Context) error {
	...
	if !typeutil.IsVectorType(cit.fieldSchema.DataType) {
		if cit.fieldSchema.DataType == schemapb.DataType_VarChar {
			indexParams = []*commonpb.KeyValuePair{{Key: common.IndexTypeKey, Value: DefaultStringIndexType}}
		} else {
			indexParams = []*commonpb.KeyValuePair{{Key: common.IndexTypeKey, Value: DefaultIndexType}}
		}
	}
	if cit.IndexName == "" {
