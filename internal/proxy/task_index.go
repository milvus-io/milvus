...
func (cit *createIndexTask) parseIndexParams() error {
	indexParams := cit.req.GetExtraParams()
	isVecIndex := typeutil.IsVectorType(cit.fieldSchema.DataType)
	if !isVecIndex {
		if cit.fieldSchema.DataType == schemapb.DataType_VarChar {
			indexParams = append(indexParams, &commonpb.KeyValuePair{Key: common.IndexTypeKey, Value: DefaultStringIndexType})
		} else {
			indexParams = append(indexParams, &commonpb.KeyValuePair{Key: common.IndexTypeKey, Value: DefaultIndexType})
		}
	}
	...
	if isVecIndex {
		...
		if Params.AutoIndexConfig.Enable {
			...
			cit.userIndexParams = append(cit.userIndexParams, indexParams...)
			// override params
	...
	var newIndexParams []*commonpb.KeyValuePair
	...
	cit.req.ExtraParams = newIndexParams
...
func (cit *createIndexTask) Execute(ctx context.Context) error {
	...
	req := &indexpb.CreateIndexRequest{
		...
		UserIndexParams: cit.userIndexParams,
