...
func Test_isPredicateDeleteCompareOp(t *testing.T) {
	...
	for _, op := range supported {
		assert.True(t, isPredicateDeleteCompareOp(op), op.String())
	}
	...
	for _, op := range unsupported {
		assert.False(t, isPredicateDeleteCompareOp(op), op.String())
	}
}
