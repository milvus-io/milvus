...
var initProxyFileResourceManager func(storage.ChunkManager, fileresource.Mode) error = fileresource.InitManager

func (node *Proxy) initFileResourceManager() error {
	mode := fileresource.GetLocalMode(fileresource.GetProxyMode())
	if mode != fileresource.SyncMode {
		return initProxyFileResourceManager(nil, mode)
	}
	...
	return initProxyFileResourceManager(chunkManager, mode)
}
...
func (node *Proxy) SyncFileResource(ctx context.Context, req *internalpb.SyncFileResourceRequest) (*commonpb.Status, error) {
	...
	if err != nil {
		log.Warn(ctx, "failed to sync file resource",
			mlog.Int("resourceCount", len(req.GetResources())),
			mlog.Err(err))
		return merr.Status(err), nil
	}
	log.Info(ctx, "file resource sync completed", mlog.Int("resourceCount", len(req.GetResources())))
