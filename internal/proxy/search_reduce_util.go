		...
		for _, groupVal := range groupByValList {
			if groupVal != nil {
				...
				for _, groupEntity := range groupEntities {
					...
					appendSize, _ := typeutil.AppendFieldData(ret.Results.FieldsData, subResData.FieldsData, groupEntity.resultIdx, nil)
					retSize += appendSize
	...
	if subSearchNum == 1 && offset == 0 {
		...
		for i := int64(0); i < nq; i++ {
			...
			for j = 0; j < limit; j++ {
				...
				appendSize, _ := typeutil.AppendFieldData(ret.Results.FieldsData, subSearchResultData[subSearchIdx].FieldsData, resultDataIdx, nil)
				retSize += appendSize
