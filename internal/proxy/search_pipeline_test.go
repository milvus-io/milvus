...
func (s *SearchPipelineSuite) TestParseOrderByFieldsEdgeCases() {
	...
	for _, fieldName := range []string{"f_bool", "f_int8", "f_int16", "f_int32", "f_int64", "f_float", "f_double", "f_string", "f_varchar", "f_json"} {
	...
	for _, fieldName := range []string{"f_float_vector", "f_binary_vector", "f_array"} {
		params = []*commonpb.KeyValuePair{{Key: OrderByFieldsKey, Value: fieldName}}
...
func (s *SearchPipelineSuite) TestIsSortableFieldType() {
	...
	s.True(isSortableFieldType(schemapb.DataType_JSON))
