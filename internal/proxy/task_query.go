...
func createCntPlan(expr string, schemaHelper *typeutil.SchemaHelper, expressionValues []*schemapb.FieldData) (*planpb.PlanNode, error) {
	if expr == "" {
	...
	plan, err := planparserv2.CreateRetrievePlan(schemaHelper, expr, expressionValues...)
	if err != nil {
...
func (t *queryTask) createPlan(ctx context.Context) error {
	...
	if cntMatch {
		var err error
		t.plan, err = createCntPlan(t.request.GetExpr(), schema.schemaHelper, t.request.GetExpressionValues())
