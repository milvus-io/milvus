...
func (t *queryTask) PreExecute(ctx context.Context) error {
	...
	if err != nil {
		log.Warn(ctx, "Failed to get collection id.", zap.String("collectionName", collectionName), zap.Error(err))
	...
	if err != nil {
		log.Warn(ctx, "Failed to get collection info.", zap.String("collectionName", collectionName),
			zap.Int64("collectionID", t.CollectionID), zap.Error(err))
		return merr.WrapErrAsInputErrorWhen(err, merr.ErrCollectionNotFound, merr.ErrDatabaseNotFound)
	}
	log.Debug(ctx, "Get collection ID by name", zap.Int64("collectionID", t.CollectionID))
	...
	if err != nil {
		log.Warn(ctx, "check partition key mode failed", zap.Int64("collectionID", t.CollectionID), zap.Error(err))
		return err
	}
	...
	for _, tag := range t.request.PartitionNames {
		if err := validatePartitionTag(tag, false); err != nil {
			log.Warn(ctx, "invalid partition name", zap.String("partition name", tag))
			return err
		}
	}
	...
	if t.queryParams.timezone != "" {
		...
		log.Debug(ctx, "determine timezone from request", zap.String("user defined timezone", t.resolvedTimezoneStr))
	} else {
		t.resolvedTimezoneStr = getColTimezone(colInfo)
		log.Debug(ctx, "determine timezone from collection", zap.Any("collection timezone", t.resolvedTimezoneStr))
	}
...
func (t *queryTask) Execute(ctx context.Context) error {
	...
	if err != nil {
		log.Warn(ctx, "fail to execute query", zap.Error(err))
...
func (t *queryTask) PostExecute(ctx context.Context) error {
	...
	case <-t.TraceCtx().Done():
		log.Warn(ctx, "proxy", zap.Int64("Query: wait to finish failed, timeout!, msgID:", t.ID()))
		return nil
	default:
		...
		t.resultBuf.Range(func(res *internalpb.RetrieveResults) bool {
			...
			log.Debug(ctx, "proxy receives one query result", zap.Int64("sourceID", res.GetBase().GetSourceID()))
			return true
		})
	}
	...
	if len(t.queryParams.orderByFields) > 0 {
		...
		if err != nil {
			log.Warn(ctx, "fail to parse order by fields", zap.Error(err))
			return err
		}
	}
	...
	if err != nil {
		log.Warn(ctx, "failed to get primary field schema", zap.Error(err))
		return err
	}
	...
	if err != nil {
		log.Warn(ctx, "fail to create query pipeline", zap.Error(err))
		return err
	}
	t.result, err = pipeline.Execute(ctx, toReduceResults)
	if err != nil {
		log.Warn(ctx, "fail to reduce query result", zap.Error(err))
		return err
	}
	...
	if !t.reQuery {
		if len(t.queryParams.extractTimeFields) > 0 {
			log.Debug(ctx, "extracting fields for timestamptz", zap.Strings("fields", t.queryParams.extractTimeFields))
			...
			log.Debug(ctx, "translate timestamp to ISO string", zap.String("user define timezone", t.queryParams.timezone))
...
func (t *queryTask) queryShard(ctx context.Context, nodeID int64, qn types.QueryNodeClient, channel string) error {
	...
	if err != nil {
		log.Warn(ctx, "QueryNode query return error", zap.Error(err))
	...
	if result.GetStatus().GetErrorCode() != commonpb.ErrorCode_Success {
		log.Warn(ctx, "QueryNode query result error", zap.Any("errorCode", result.GetStatus().GetErrorCode()), zap.String("reason", result.GetStatus().GetReason()))
...
func reduceRetrieveResults(ctx context.Context, retrieveResults []*internalpb.RetrieveResults, queryParams *queryParams) (*milvuspb.QueryResults, error) {
	mlog.Debug(ctx, "reduceInternalRetrieveResults", zap.Int("len(retrieveResults)", len(retrieveResults)))
	var (
