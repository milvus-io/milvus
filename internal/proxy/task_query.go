...
func (t *queryTask) PreExecute(ctx context.Context) error {
	...
	if err != nil {
		log.Warn(ctx, "Failed to get collection id.", mlog.FieldCollectionName(collectionName), mlog.Err(err))
	...
	if err != nil {
		log.Warn(ctx, "Failed to get collection info.", mlog.FieldCollectionName(collectionName),
			mlog.FieldCollectionID(t.CollectionID), mlog.Err(err))
		return merr.WrapErrAsInputErrorWhen(err, merr.ErrCollectionNotFound, merr.ErrDatabaseNotFound)
	}
	log.Debug(ctx, "Get collection ID by name", mlog.FieldCollectionID(t.CollectionID))
	...
	if err != nil {
		log.Warn(ctx, "check partition key mode failed", mlog.FieldCollectionID(t.CollectionID), mlog.Err(err))
		return err
	}
	...
	if err2 != nil {
		log.Warn(ctx, "Proxy::queryTask::PreExecute failed to GetCollectionInfo from cache",
			mlog.FieldCollectionName(collectionName), mlog.FieldCollectionID(t.CollectionID),
			mlog.Err(err2))
		return err2
	}
