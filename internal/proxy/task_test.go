...
func TestValidateAddFieldRequest(t *testing.T) {
	...
	t.Run("nested array", func(t *testing.T) {
		...
		require.NoError(t, validateAddFieldRequest(schema, newField))
	})
...
func TestAlterCollectionSchemaTask(t *testing.T) {
	...
	externalParquetTextOldSchema := proto.Clone(externalOldSchema).(*schemapb.CollectionSchema)
	externalParquetTextOldSchema.ExternalSource = "s3://bucket/source"
	externalParquetTextOldSchema.ExternalSpec = `{"format":"parquet"}`
	externalParquetTextOldSchema.Fields[1].DataType = schemapb.DataType_Text
	externalMilvusTableTextOldSchema := proto.Clone(externalParquetTextOldSchema).(*schemapb.CollectionSchema)
	externalMilvusTableTextOldSchema.ExternalSpec = `{"format":"milvus-table"}`
	...
	buildTextEmbeddingRequest := func() *milvuspb.AlterCollectionSchemaRequest {
		req := buildValidRequest()
		addRequest := req.GetAction().GetAddRequest()
		addRequest.FieldInfos[0].FieldSchema = &schemapb.FieldSchema{
			Name:     "embedding",
	...
	t.Run("External parquet PreExecute allows BM25 with TEXT input", func(t *testing.T) {
		task := buildTask(buildValidRequest(), externalParquetTextOldSchema)
		err := task.PreExecute(ctx)
		assert.NoError(t, err)
	})
	...
	t.Run("External PreExecute supports TextEmbedding function with new output field", func(t *testing.T) {
		...
		task := buildTask(buildTextEmbeddingRequest(), externalOldSchema)
		err := task.PreExecute(ctx)
		assert.NoError(t, err)
	})

	t.Run("External milvus-table PreExecute rejects TextEmbedding with TEXT input", func(t *testing.T) {
		task := buildTask(buildTextEmbeddingRequest(), externalMilvusTableTextOldSchema)
		err := task.PreExecute(ctx)
		assert.Error(t, err)
		assert.ErrorIs(t, err, merr.ErrParameterInvalid)
