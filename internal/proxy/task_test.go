...
func TestTransferReplicaTask(t *testing.T) {
	...
	req := &milvuspb.TransferReplicaRequest{
		...
		SourceResourceGroup:  "rg1",
		TargetResourceGroups: []string{"rg2"},
		NumReplica:           1,
	}
...
func TestListResourceGroupTask(t *testing.T) {
	rc := &MockRootCoordClientInterface{}
	qc := NewQueryCoordMock()
	qc.Start()
	defer qc.Stop()
	...
	req := &milvuspb.ListResourceGroupRequest{
		Base: &commonpb.MsgBase{
			MsgID:     1,
			Timestamp: 2,
			TargetID:  3,
	...
	task := &ListResourceGroupTask{
		ListResourceGroupRequest: req,
		ctx:                      ctx,
		queryCoord:               qc,
	}
	task.PreExecute(ctx)

	assert.Equal(t, commonpb.MsgType_ListResourceGroup, task.Type())
	assert.Equal(t, UniqueID(1), task.ID())
	assert.Equal(t, Timestamp(2), task.BeginTs())
	assert.Equal(t, Timestamp(2), task.EndTs())
	assert.Equal(t, paramtable.GetNodeID(), task.Base.GetSourceID())
