...
func getPlaceholderType(phgBytes []byte) commonpb.PlaceholderType {
	holder := &commonpb.PlaceholderGroup{}
	if err := proto.Unmarshal(phgBytes, holder); err != nil || len(holder.GetPlaceholders()) == 0 {
		return 0
	}
	return holder.GetPlaceholders()[0].GetType()
}
