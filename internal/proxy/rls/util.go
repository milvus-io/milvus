...
func preparePolicyExprTemplates(policies []*rlsutil.RowPolicy, action rlsutil.PolicyAction, kind exprKind) ([]policyExprTemplate, string) {
...
func compileExprTemplates(schemaHelper *typeutil.SchemaHelper, templates []policyExprTemplate, timezone string) (*compiledExpression, error) {
	if len(templates) == 0 {
		return nil, nil
	}
	...
	for _, template := range templates {
		policyExpr, err := compilePolicyExprTemplate(schemaHelper, template, visitorArgs)
		if err != nil {
			return nil, err
		}
		if policyExpr == nil {
...
func compilePolicyExprTemplate(schemaHelper *typeutil.SchemaHelper, template policyExprTemplate, visitorArgs *planparserv2.ParserVisitorArgs) (*compiledPolicyExpression, error) {
...
func ValidateCheckForWrite(ctx context.Context, collectionID UniqueID, principalName string, action rlsutil.PolicyAction, fieldsData []*schemapb.FieldData, schemaHelper *typeutil.SchemaHelper, rowNum int, operation string) error {
	return validateCheckForWrite(defaultManager, ctx, collectionID, principalName, action, fieldsData, schemaHelper, rowNum, operation)
}

func validateCheckForWrite(m *manager, ctx context.Context, collectionID UniqueID, principalName string, action rlsutil.PolicyAction, fieldsData []*schemapb.FieldData, schemaHelper *typeutil.SchemaHelper, rowNum int, operation string) error {
		...
		if isRLSScalarType(fieldData.GetType()) {
			...
			if len(validData) > 0 {
				...
				for rowIdx, valid := range validData {
					if !valid {
						reader.arrayDataIndices[rowIdx] = -1
					} else if dataLen == len(validData) {
						reader.arrayDataIndices[rowIdx] = rowIdx
					} else {
...
func arrayValue(reader *fieldReader, rowIdx int) (*schemapb.ScalarField, error) {
	...
	if len(reader.arrayDataIndices) > 0 {
		if rowIdx >= len(reader.arrayDataIndices) {
			return nil, merr.WrapErrServiceInternalMsg("RLS row index %d exceeds valid data length %d", rowIdx, len(reader.arrayDataIndices))
		}
...
func scalarArrayContains(arrayValue *schemapb.ScalarField, target *planpb.GenericValue) (bool, error) {
	...
	case *schemapb.ScalarField_BoolData:
		...
		return containsValidArrayElement(data.BoolData.GetData(), validData, func(value bool) bool {
			return value == targetValue.BoolVal
		})
	case *schemapb.ScalarField_IntData:
		...
		return containsValidArrayElement(data.IntData.GetData(), validData, func(value int32) bool {
			return int64(value) == targetValue.Int64Val
		})
	case *schemapb.ScalarField_LongData:
		...
		return containsValidArrayElement(data.LongData.GetData(), validData, func(value int64) bool {
			return value == targetValue.Int64Val
		})
	case *schemapb.ScalarField_FloatData:
		...
		return containsValidArrayElement(data.FloatData.GetData(), validData, func(value float32) bool {
	...
	case *schemapb.ScalarField_DoubleData:
		...
		return containsValidArrayElement(data.DoubleData.GetData(), validData, func(value float64) bool {
	...
	case *schemapb.ScalarField_StringData:
		...
		return containsValidArrayElement(data.StringData.GetData(), validData, func(value string) bool {
			return value == targetValue.StringVal
		})
	default:
...
func containsValidArrayElement[T any](values []T, validData []bool, matches func(T) bool) (bool, error) {
	if len(validData) > 0 && len(validData) != len(values) {
		return false, merr.WrapErrServiceInternalMsg(
			"RLS array element valid data length %d does not match data length %d",
			len(validData), len(values),
		)
	}
