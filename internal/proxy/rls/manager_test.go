...
func TestManagerRejectsTagUsedWithMixedFieldTypes(t *testing.T) {
	...
	require.NotNil(t, expr)
	require.ErrorIs(t,
		ValidateRowsByPredicate(ctx, managerTestFieldsDataWithAgeAndScore("sales", 18, 0), 1, expr, "query", "using"),
...
func TestValidateCheckForWriteUsesSchemaTimezone(t *testing.T) {
	...
	require.NoError(t, validateCheckForWrite(manager, ctx, collectionID, "alice", rlsutil.PolicyActionInsert, fieldsData, helper, 1, "insert"))
}
...
func TestValidateWritePredicatesUseThreeValuedLogic(t *testing.T) {
	...
	for _, fullSize := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			err := validateCheckForWrite(manager, ctx, collectionID, "alice", rlsutil.PolicyActionInsert, fieldsData, helper, 1, "insert")
			require.ErrorIs(t, err, merr.ErrPrivilegeNotPermitted)
		})
	}
	...
	fieldsData := []*schemapb.FieldData{{
		...
		Field: &schemapb.FieldData_Scalars{Scalars: &schemapb.ScalarField{
			...
			Data: &schemapb.ScalarField_ArrayData{ArrayData: &schemapb.ArrayArray{
				...
				Data: []*schemapb.ScalarField{
					{Data: &schemapb.ScalarField_StringData{StringData: &schemapb.StringArray{Data: []string{"red"}}}},
					{Data: &schemapb.ScalarField_StringData{StringData: &schemapb.StringArray{Data: []string{"blue"}}}},
				},
			}},
		}},
	}}
...
func TestScalarArrayContainsSkipsNullElements(t *testing.T) {
	tests := []struct {
		name        string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			contains, err := scalarArrayContains(test.array, test.nullTarget)
			require.NoError(t, err)
			require.False(t, contains)

			contains, err = scalarArrayContains(test.array, test.validTarget)
			require.NoError(t, err)
	...
	_, err := scalarArrayContains(tests[0].array, tests[0].validTarget)
	...
	rows := newRowData([]*schemapb.FieldData{{
		...
		Field: &schemapb.FieldData_Scalars{Scalars: &schemapb.ScalarField{Data: &schemapb.ScalarField_ArrayData{ArrayData: &schemapb.ArrayArray{
			...
			Data: []*schemapb.ScalarField{{
				...
				Data:      &schemapb.ScalarField_LongData{LongData: &schemapb.LongArray{Data: []int64{0, 7}}},
			}},
		}}}},
	}})
