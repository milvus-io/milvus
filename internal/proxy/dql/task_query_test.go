...
func TestQueryShardCollectsOnlySuccessfulSnapshots(t *testing.T) {
	...
	lb := &lbTarget{}
	patchCost := mockTest(t, (*lbTarget).UpdateCostMetrics)
	defer patchCost.UnPatch()
	type managerTarget struct{ shardclient.ShardClientMgr }
	manager := &managerTarget{}
	patchInvalidate := mockTest(t, (*managerTarget).InvalidateShardLeaderCache)
...
func TestQueryTaskPreExecuteUsesStrongVisibility(t *testing.T) {
	...
	metaCache := newTestCache()
	collectionIDMocker := mockTest(t, (*metacache.MetaCache).GetCollectionID, collectionID, nil)
	defer collectionIDMocker.UnPatch()
	collectionInfoMocker := mockTest(t, (*metacache.MetaCache).GetCollectionInfo, collectionInfo, nil)
	defer collectionInfoMocker.UnPatch()
	collectionSchemaMocker := mockTest(t, (*metacache.MetaCache).GetCollectionSchema, schema, nil)
	defer collectionSchemaMocker.UnPatch()
	partitionsMocker := mockey.Mock((*metacache.MetaCache).GetPartitions).Return(map[string]UniqueID{
		paramtable.Get().CommonCfg.DefaultPartitionName.GetValue(): partitionID,
	}, nil).Build()
...
func TestQueryTask_CanSkipAllocTimestamp(t *testing.T) {
	...
	t.Run("failed", func(t *testing.T) {
		// GetCollectionInfo returns an error on the first call, then succeeds;
		// GetCollectionID errors on the second call.
		infoCalls := 0
		idCalls := 0
