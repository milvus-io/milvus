		...
		for _, returnRoots := range []bool{false, true} {
			for _, inputKind := range []string{"json", "dynamic", "both"} {
				t.Run(fmt.Sprintf("direct=%v/roots=%v/%s", direct, returnRoots, inputKind), func(t *testing.T) {
					...
					queryMock := mockTestTo(t, (*requeryOperator).requery, func(_ *requeryOperator, _ context.Context, _ trace.Span, ids *schemapb.IDs, fields []string) (*milvuspb.QueryResults, segcore.StorageCost, error) {
