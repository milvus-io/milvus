...
func parseSearchInfo(searchParamsPair []*commonpb.KeyValuePair, schema *schemapb.CollectionSchema, rankParams *rankParams, largeTopKEnabled bool) (*SearchInfo, error) {
	...
	if err := validateLimit(topK, largeTopKEnabled); err != nil {
		if isIterator {
			...
			if largeTopKEnabled {
				topK = paramtable.Get().QuotaConfig.LargeTopKLimit.GetAsInt64()
			} else {
				topK = paramtable.Get().QuotaConfig.TopKLimit.GetAsInt64()
			}
		} else {
...
func GetPartitionIDs(ctx context.Context, metaCache Cache, dbName string, collectionName string, partitionNames []string) (partitionIDs []UniqueID, err error) {
	...
	useRegexp := paramtable.Get().ProxyCfg.PartitionNameRegexp.GetAsBool()
...
func parseGroupByInfo(searchParamsPair []*commonpb.KeyValuePair, schema *schemapb.CollectionSchema) (*groupByInfo, error) {
	...
	if groupSize > paramtable.Get().QuotaConfig.MaxGroupSize.GetAsInt64() {
		return nil, merr.WrapErrParameterInvalidMsg(
			fmt.Sprintf("input group size:%d exceeds configured max group size:%d", groupSize, paramtable.Get().QuotaConfig.MaxGroupSize.GetAsInt64()))
	}
