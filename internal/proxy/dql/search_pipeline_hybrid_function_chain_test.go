...
func (s *SearchPipelineSuite) TestHybridFunctionChainRequestLimitFallback() {
	originalReduceFactory := opFactory[hybridSearchReduceOp]
	defer func() { opFactory[hybridSearchReduceOp] = originalReduceFactory }()

	makeResult := func(ids []int64, scores []float32, topks []int64) *milvuspb.SearchResults {
	...
	reduced := []*milvuspb.SearchResults{
		makeResult([]int64{1, 2, 5}, []float32{0.9, 0.7, 0.5}, []int64{2, 1}),
		makeResult([]int64{3, 4}, []float32{0.8, 0.6}, []int64{2, 0}),
	}

	sortAscending := &schemapb.FunctionChainOp{
		Op:     types.OpTypeSort,
	...
	for _, test := range []struct {
		...
		{
			name: "empty results", limit: 2, empty: true,
			wantIDs: nil, wantScores: []float32{}, wantTopks: []int64{0, 0},
		},
		{
			name: "merge only uses request limit per query", limit: 2,
			wantIDs: []int64{1, 3, 5}, wantScores: []float32{0.9, 0.8, 0.5}, wantTopks: []int64{2, 1},
		},
		{
			name: "fewer candidates than request limit", limit: 10,
			wantIDs: []int64{1, 3, 2, 4, 5}, wantScores: []float32{0.9, 0.8, 0.7, 0.6, 0.5}, wantTopks: []int64{4, 1},
		},
		{
			name: "request offset applies per query", limit: 2, offset: 2,
			wantIDs: []int64{2, 4}, wantScores: []float32{0.7, 0.6}, wantTopks: []int64{2, 0},
		},
		{
			name: "offset beyond candidates returns empty results", limit: 2, offset: 5,
			wantTopks: []int64{0, 0},
		},
		{
			name: "fallback follows all user operators", limit: 2,
			ops:     []*schemapb.FunctionChainOp{sortAscending},
			wantIDs: []int64{4, 2, 5}, wantScores: []float32{0.6, 0.7, 0.5}, wantTopks: []int64{2, 1},
		},
		{
			name: "offset follows all user operators", limit: 2, offset: 1,
			ops:     []*schemapb.FunctionChainOp{sortAscending},
			wantIDs: []int64{2, 3}, wantScores: []float32{0.7, 0.8}, wantTopks: []int64{2, 0},
		},
		{
			name: "explicit limit overrides request limit and offset", limit: 1, offset: 2,
			ops:     []*schemapb.FunctionChainOp{l2LimitFunctionChain(3).GetOps()[0]},
			wantIDs: []int64{1, 3, 2, 5}, wantScores: []float32{0.9, 0.8, 0.7, 0.5}, wantTopks: []int64{3, 1},
		},
		{
			name: "intermediate limit also overrides request limit and offset", limit: 1, offset: 2,
			ops:     []*schemapb.FunctionChainOp{l2LimitFunctionChain(3).GetOps()[0], sortAscending},
			wantIDs: []int64{2, 3, 1, 5}, wantScores: []float32{0.7, 0.8, 0.9, 0.5}, wantTopks: []int64{3, 1},
		},
	} {
		s.Run(test.name, func() {
			queryResults := reduced
			if test.empty {
				queryResults = []*milvuspb.SearchResults{
					makeResult(nil, nil, []int64{0, 0}),
					makeResult(nil, nil, []int64{0, 0}),
				}
			}
			opFactory[hybridSearchReduceOp] = func(_ *SearchTask, _ map[string]any) (operator, error) {
				return searchPipelineTestOperator(func(context.Context, trace.Span, ...any) ([]any, error) {
					return []any{queryResults, []string{metric.IP, metric.IP}}, nil
				}), nil
			}
			ops := append([]*schemapb.FunctionChainOp{{
				Op: types.OpTypeMerge,
				Params: map[string]*schemapb.FunctionParamValue{
					chain.MergeParamStrategy: chainStringParam(string(chain.MergeStrategyMax)),
				},
			}}, test.ops...)
			chainPB := l2FunctionChain(ops...)
			task := getHybridSearchTask("test_collection", [][]string{{"1", "2"}, {"3", "4"}}, nil)
			// Exercise the wire conversion and request parser used for SDK-supplied limits.
			// A nested L1 Limit must not suppress the top-level L2 fallback.
			task.request = ConvertHybridSearchToSearch(&milvuspb.HybridSearchRequest{
				CollectionName: "test_collection",
				Requests: []*milvuspb.SearchRequest{
					{FunctionChains: []*schemapb.FunctionChain{l1LimitFunctionChain(1)}},
					{},
				},
				FunctionChains: []*schemapb.FunctionChain{chainPB},
				RankParams: []*commonpb.KeyValuePair{
					{Key: LimitKey, Value: strconv.FormatInt(test.limit, 10)},
					{Key: OffsetKey, Value: strconv.FormatInt(test.offset, 10)},
					{Key: RoundDecimalKey, Value: "0"},
				},
			})
			var err error
			task.rankParams, err = parseRankParams(task.request.GetSearchParams(), task.schema.CollectionSchema, false)
			s.Require().NoError(err)
			task.rerankMeta, err = selectHybridRerankMeta(task.request, task.schema)
			s.Require().NoError(err)
			for _, subReq := range task.SubReqs {
				subReq.Topk = 2
			}
			...
			result := results.GetResults()
			s.Equal(test.wantTopks, result.GetTopks())
			if len(test.wantIDs) == 0 {
				s.Empty(result.GetIds().GetIntId().GetData())
				s.Empty(result.GetScores())
			} else {
				s.Equal(test.wantIDs, result.GetIds().GetIntId().GetData())
				// Request rounding remains unapplied when falling back to request pagination.
				s.Equal(test.wantScores, result.GetScores())
			}
			s.Len(chainPB.GetOps(), len(ops))
			s.Len(task.rerankMeta.(*functionChainRerankMeta).repr.Operators, len(ops))
		})
	}
}
...
func (s *SearchPipelineSuite) TestHybridFunctionChainRequeryInputDoesNotLeak() {
	...
	pkField.FieldId = 100
	requeryMock := mockTest(s.T(), (*requeryOperator).requery, &milvuspb.QueryResults{
