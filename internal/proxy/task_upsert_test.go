		...
		for _, mode := range []string{"required", "nullable", "default"} {
			t.Run(fmt.Sprintf("autoID=%t/%s", autoID, mode), func(t *testing.T) {
				...
				if mode == "default" {
					...
					require.Equal(t, []bool{false, false}, value.GetValidData())
				}
			})
		}
	}
...
func TestPartialUpdateAutoIDPreservesOmittedFields(t *testing.T) {
	for _, mode := range []string{"required", "nullable", "default"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "nullable" {
				oldFields[1].ValidData = []bool{true, true}
			}
...
func TestPartialUpdateAutoIDReusesUnchangedFunctionOutputs(t *testing.T) {
	for _, scenario := range []struct {
		...
		t.Run(scenario.name, func(t *testing.T) {
			...
			valueField.ValidData = []bool{false, true}
...
func TestUpsertTask_queryPreExecute_DefaultValueWithValidData(t *testing.T) {
	...
	require.Equal(t, []bool{true, true, true}, defaultColField.GetValidData())
	...
	require.Empty(t, defaultColField.GetValidData())
...
func TestUpdateTaskPreExecuteStopsRejectedRequestsBeforeWriting(t *testing.T) {
	for _, stage := range []string{"collection_info", "schema", "text_storage", "field_ops", "namespace", "partition_mode", "partition_info", "primary_key", "pk_parse", "duplicate_pk", "functions", "insert", "delete", "implicit_partial_namespace"} {
		t.Run(stage, func(t *testing.T) {
			task := createTestUpdateTask()
			task.req.PartialUpdate = true
			setPartialUpdateCASTestChannels(task, partialUpdateCASTestVChannels)
