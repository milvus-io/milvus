...
func (cst *createSnapshotTask) PreExecute(ctx context.Context) error {
	...
	// Validate compaction protection duration (max 7 days = 604800 seconds)
	const maxCompactionProtectionSeconds = 7 * 24 * 3600 // 7 days
	...
	if cst.req.GetCompactionProtectionSeconds() > maxCompactionProtectionSeconds {
		return merr.WrapErrParameterInvalidMsg("compaction_protection_seconds must not exceed %d (7 days)", maxCompactionProtectionSeconds)
	}
