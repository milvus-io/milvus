...
func (node *Proxy) HandleStreamingNodes(w http.ResponseWriter, req *http.Request) {
	...
	for _, info := range streamingNodes {
		// Assume streaming nodes are "Normal".
...
// HandleStreamingNodeStatus is the main handler for the unified endpoint.
func (node *Proxy) HandleStreamingNodeStatus(w http.ResponseWriter, req *http.Request) {
	...
	// Convert the single nodeID to a slice of one element.
	...
	case "suspended":
		...
		errMsg = "failed to freeze streaming node"
	case "active":
		err = streaming.WAL().Balancer().DefreezeNodeIDs(req.Context(), nodeIDs)
		errMsg = "failed to defreeze streaming node"
	default:
...
// HandleBatchNodeStatus is the main handler for the unified endpoint.
func (node *Proxy) HandleBatchNodeStatus(w http.ResponseWriter, req *http.Request) {
	switch req.Method {
	case http.MethodPut:
		node.controlBatchNodeStatus(w, req)
	default:
		http.Error(w, `{"msg": "Method not allowed"}`, http.StatusMethodNotAllowed)
	}
}

// controlBatchNodeStatus handles PUT requests to change the node status.
func (node *Proxy) controlBatchNodeStatus(w http.ResponseWriter, req *http.Request) {
	// Define the request body struct with both node_id and status.
	var requestBody struct {
		NodeID int64  `json:"node_id"`
		Status string `json:"status"`
	}
