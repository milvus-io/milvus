...
func validateMaxLengthPerRow(collectionName string, field *schemapb.FieldSchema) error {
	...
	for _, param := range field.TypeParams {
		...
		if maxLengthPerRow > defaultMaxLength || maxLengthPerRow <= 0 {
			return merr.WrapErrParameterInvalidMsg("the maximum length specified for a field(%s) should be in (0, %d], but got %d instead", field.GetName(), defaultMaxLength, maxLengthPerRow)
		}
		exist = true
	}
	// if not exist type params max_length, return error
	if !exist {
		return fmt.Errorf("type param(max_length) should be specified for field(%s) of collection %s", field.GetName(), collectionName)
	}
