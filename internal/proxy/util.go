...
func ValidateResourceName(entity string) error {
	entity = strings.TrimSpace(entity)
	if entity == "" {
		return fmt.Errorf("resourceName should not be empty")
	}

	if int64(len(entity)) > Params.ProxyCfg.MaxNameLength {
		msg := fmt.Sprintf("invalid resource name: %s. The length of resource name must be less than %s characters",
...
func ValidateResourceType(entity string) error {
	return validateName(entity, "ResourceType")
}
...
func GetCurUserFromContext(ctx context.Context) (string, error) {
	...
	if len(authorization) < 1 {
		return "", fmt.Errorf("fail to get authorization from the md")
	}
...
func ValidateRootPermission(ctx context.Context) error {
	curUser, err := GetCurUserFromContext(ctx)
	if err != nil {
		return fmt.Errorf("fail to get current user info from the context, err: %w", err)
	}
	if curUser != util.UserRoot {
		return fmt.Errorf("the current user isn't the root, can't execute this api")
	}
