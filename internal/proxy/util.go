...
func checkFieldsDataBySchema(allFields []*schemapb.FieldSchema, schema *schemapb.CollectionSchema, insertMsg *msgstream.InsertMsg, inInsert bool) error {
	...
	primaryField := lo.Filter(allFields, func(fieldSchema *schemapb.FieldSchema, _ int) bool {
		return fieldSchema.IsPrimaryKey
	})[0]
	allowInsertAutoID, _ := common.IsAllowInsertAutoID(schema.GetProperties()...)
	hasPkData := dataNameSet.Contain(primaryField.GetName())
	needAutoGenPk := primaryField.IsPrimaryKey && primaryField.AutoID && (!allowInsertAutoID || !hasPkData)
