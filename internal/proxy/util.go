...
func validateMaxCapacityPerRow(collectionName string, field *schemapb.FieldSchema) error {
	...
	// if not exist type params max_length, return error
	if !exist {
		return merr.WrapErrParameterMissingMsg("type param(max_capacity) should be specified for array field %s of collection %s", field.GetName(), collectionName)
	}
	return nil
}
...
func CheckDuplicatePkExist(primaryFieldSchema *schemapb.FieldSchema, fieldsData []*schemapb.FieldData) (bool, error) {
	...
	if primaryFieldData == nil {
		return false, merr.WrapErrParameterInvalidMsg("must assign pk when checking duplicates, primary field: %v", primaryFieldSchema.GetName())
	}
	...
	for _, outputFieldName := range outputFields {
		...
		if outputFieldName == "*" {
			...
			if field, ok := allFieldNameMap[outputFieldName]; ok {
				if !schema.CanRetrieveRawFieldData(field) {
					return nil, nil, nil, false, merr.WrapErrParameterInvalidMsg("not allowed to retrieve raw data of field %s", outputFieldName)
				}
				...
				if schema.EnableDynamicField {
					...
					if err != nil {
						...
						return nil, nil, nil, false, merr.WrapErrParameterInvalidMsg("parse output field name failed: %s", outputFieldName)
					}
					resultFieldNameMap[common.MetaFieldName] = true
					userOutputFieldsMap[outputFieldName] = true
					userDynamicFieldsMap[dynamicNestedPath] = true
				} else {
					return nil, nil, nil, false, merr.WrapErrParameterInvalidMsg("field %s not exist", outputFieldName)
				}
			}
		}
	}
...
func checkAndFlattenStructFieldData(schema *schemapb.CollectionSchema, insertMsg *msgstream.InsertMsg) error {
	...
	if len(schema.GetStructArrayFields()) != structFieldCount {
		return merr.WrapErrParameterInvalidMsg("the number of struct array fields is not the same as needed, expected: %d, actual: %d",
