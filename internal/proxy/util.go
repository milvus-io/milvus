...
func validateFunction(coll *schemapb.CollectionSchema) error {
	...
	if err := embedding.ValidateFunctions(coll, map[string]string{models.ClusterIDKey: paramtable.Get().CommonCfg.ClusterPrefix.GetValue()}); err != nil {
		return err
	}
	return nil
}
...
func genFunctionFields(ctx context.Context, insertMsg *msgstream.InsertMsg, schema *schemaInfo, partialUpdate bool) error {
	...
	if embedding.HasNonBM25Functions(schema.CollectionSchema.Functions, []int64{}) {
		...
		defer sp.End()
		exec, err := embedding.NewFunctionExecutor(schema.CollectionSchema, needProcessFunctions, map[string]string{models.ClusterIDKey: paramtable.Get().CommonCfg.ClusterPrefix.GetValue()})
		if err != nil {
			return err
		}
