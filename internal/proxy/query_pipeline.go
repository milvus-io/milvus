...
func NewQueryPipeline(
	...
	if hasAggregation {
		...
		// ORDER BY: concat + check for cross-shard PK duplicates (fail on duplicate)
		builder.Add(queryutil.OpConcatAndCheckPK,
			[]string{chanInput},
			[]string{chanReduced},
			queryutil.NewConcatAndCheckPKOperator(),
		)
	} else {
		// Plain query: k-way merge by PK with duplicate check
		builder.Add(queryutil.OpReduceByPK,
			[]string{chanInput},
			[]string{chanReduced},
			queryutil.NewSortAndCheckPKOperator(reduceType),
		)
	}

	// Step 2: Optionally add order+topK operator
	sliceInput := chanReduced
	if hasOrderBy {
		builder.Add(queryutil.OpOrderByLimit,
			[]string{chanReduced},
			[]string{chanSorted},
			queryutil.NewOrderByLimitOperator(orderByFields, offset+limit),
		)
