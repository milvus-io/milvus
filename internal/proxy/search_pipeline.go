...
func newLambdaOperator(_ *searchTask, params map[string]any) (operator, error) {
	return &lambdaOperator{
		f: params["lambda"].(func(ctx context.Context, span trace.Span, inputs ...any) ([]any, error)),
	}, nil
}
...
func (p *pipeline) Run(ctx context.Context, span trace.Span, toReduceResults []*internalpb.SearchResults) (*milvuspb.SearchResults, error) {
	log.Ctx(ctx).Info("SearchPipeline run", zap.String("pipeline", p.String()))
	msg := opMsg{}
	msg["input"] = toReduceResults
	for _, node := range p.nodes {
		var err error
		log.Ctx(ctx).Info("SearchPipeline run node", zap.String("node", node.name))
...
var searchPipe = &pipelineDef{
	...
	nodes: []*nodeDef{
		...
		},
		{
			...
			params: map[string]any{
				"lambda": func(ctx context.Context, span trace.Span, inputs ...any) ([]any, error) {
