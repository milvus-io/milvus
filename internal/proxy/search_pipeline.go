...
func (op *orderByOperator) reorderResults(result *milvuspb.SearchResults, indices []int) error {
	...
	for newIdx, oldIdx := range indices {
		...
		if newDistances != nil {
			if oldIdx < 0 || oldIdx >= len(results.Distances) {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderResults: index %d out of bounds for distances (len=%d)", oldIdx, len(results.Distances)))
			}
			newDistances[newIdx] = results.Distances[oldIdx]
		}
		if newRecalls != nil {
			if oldIdx < 0 || oldIdx >= len(results.Recalls) {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderResults: index %d out of bounds for recalls (len=%d)", oldIdx, len(results.Recalls)))
			}
			newRecalls[newIdx] = results.Recalls[oldIdx]
		}
		if newElementIndices != nil {
			if oldIdx < 0 || oldIdx >= len(elemIndices.GetData()) {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderResults: index %d out of bounds for element indices (len=%d)", oldIdx, len(elemIndices.GetData())))
			}
	...
	if gbv := results.GetGroupByFieldValue(); gbv != nil {
		if err := reorderFieldData(gbv, indices); err != nil {
			return err
		}
	}
...
func validateReorderIndex(oldIdx, originalRows int, field *schemapb.FieldData) error {
	if oldIdx < 0 || oldIdx >= originalRows {
		return merr.WrapErrServiceInternal(fmt.Sprintf("reorderFieldData: index %d out of bounds for %s field %s (rows=%d)", oldIdx, field.GetType().String(), field.GetFieldName(), originalRows))
	}
	return nil
}
...
func reorderFieldData(field *schemapb.FieldData, indices []int) error {
	...
	case schemapb.DataType_FloatVector:
		...
		if vectors != nil && (newValidData != nil || vectors.GetFloatVector() != nil) {
			...
			if len(data)%dim != 0 {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderFieldData: FloatVector field %s has %d elements not divisible by dim %d", field.GetFieldName(), len(data), dim))
			}
	...
	case schemapb.DataType_BinaryVector:
		...
		if vectors != nil && (newValidData != nil || len(vectors.GetBinaryVector()) > 0) {
			...
			if len(data)%bytesPerVector != 0 {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderFieldData: BinaryVector field %s has %d bytes not divisible by width %d", field.GetFieldName(), len(data), bytesPerVector))
			}
	...
	case schemapb.DataType_Float16Vector:
		...
		if vectors != nil && (newValidData != nil || len(vectors.GetFloat16Vector()) > 0) {
			...
			if len(data)%bytesPerVector != 0 {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderFieldData: Float16Vector field %s has %d bytes not divisible by width %d", field.GetFieldName(), len(data), bytesPerVector))
			}
			originalRows := len(data) / bytesPerVector
			newData := make([]byte, n*bytesPerVector)
			for newIdx, oldIdx := range indices {
	...
	case schemapb.DataType_BFloat16Vector:
		...
		if vectors != nil && (newValidData != nil || len(vectors.GetBfloat16Vector()) > 0) {
			...
			if len(data)%bytesPerVector != 0 {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderFieldData: BFloat16Vector field %s has %d bytes not divisible by width %d", field.GetFieldName(), len(data), bytesPerVector))
			}
			originalRows := len(data) / bytesPerVector
			newData := make([]byte, n*bytesPerVector)
			for newIdx, oldIdx := range indices {
	...
	case schemapb.DataType_Int8Vector:
		...
		if vectors != nil && (newValidData != nil || len(vectors.GetInt8Vector()) > 0) {
			...
			if len(data)%dim != 0 {
				return merr.WrapErrServiceInternal(fmt.Sprintf("reorderFieldData: Int8Vector field %s has %d bytes not divisible by dim %d", field.GetFieldName(), len(data), dim))
			}
			originalRows := len(data) / dim
