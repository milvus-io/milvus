...
// hybridSearchWithOrderByPipe: reduce → rerank → requery → organize → order_by
// For hybrid search with order_by_fields
var hybridSearchWithOrderByPipe = &pipelineDef{
	name: "hybridSearchWithOrderBy",
	nodes: []*nodeDef{
		{
			name:    "reduce",
			inputs:  []string{pipelineInput, pipelineStorageCost},
...
func newBuiltInPipeline(t *searchTask) (*pipeline, error) {
	...
	// Reject conflicting sort criteria: order_by and function_score cannot be used together
	if hasOrderBy && t.functionScore != nil {
		return nil, fmt.Errorf("order_by and function_score cannot be used together: they specify conflicting sort criteria")
	}
	...
	// Hybrid search with order_by: reduce → rerank → requery → order_by
	if t.SearchRequest.GetIsAdvanced() && hasOrderBy {
		return newPipeline(hybridSearchWithOrderByPipe, t)
	}
