...
func (t *searchTask) initAdvancedSearchRequest(ctx context.Context) error {
	...
	for index, subReq := range t.request.GetSubReqs() {
		...
		t.addHighlightTask(subReq.GetSearchParams(), internalSubReq.GetMetricType(), internalSubReq.FieldId, subReq.GetPlaceholderGroup(), internalSubReq.GetAnalyzerName())
...
func (t *searchTask) addHighlightTask(params []*commonpb.KeyValuePair, metric_type string, annsField int64, placeholder []byte, analyzerName string) error {
	// appen highlight fields
	if value, err := funcutil.GetAttrByKeyFromRepeatedKV(HighlightKey, params); err == nil {
		params := map[string]any{}
		if err := json.Unmarshal([]byte(value), &params); err == nil {
			if metric_type != metric.BM25 {
			...
			if !ok {
				return merr.WrapErrServiceInternal(`get highlight failed, input field of BM25 annsField not found`)
			}
...
func (t *searchTask) initSearchRequest(ctx context.Context) error {
	...
	t.addHighlightTask(t.request.GetSearchParams(), t.SearchRequest.MetricType, t.SearchRequest.FieldId, t.request.GetPlaceholderGroup(), t.SearchRequest.GetAnalyzerName())
