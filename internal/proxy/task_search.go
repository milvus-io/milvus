...
func (t *searchTask) initAdvancedSearchRequest(ctx context.Context) error {
	...
	for index, subReq := range t.request.GetSubReqs() {
		...
		if paramtable.Get().ProxyCfg.SearchPlaceholderLogEnabled.GetAsBool() {
			log.RatedInfo(paramtable.Get().ProxyCfg.SearchPlaceholderLogRateLimit.GetAsFloat(), "proxy search request placeholder",
				zap.Stringer("placeholder", placeholderStringer{placeholder: subReq.GetPlaceholderGroup()}))
		}
...
func (t *searchTask) initSearchRequest(ctx context.Context) error {
	...
	if paramtable.Get().ProxyCfg.SearchPlaceholderLogEnabled.GetAsBool() {
		log.RatedInfo(paramtable.Get().ProxyCfg.SearchPlaceholderLogRateLimit.GetAsFloat(), "proxy search request placeholder",
			zap.Stringer("placeholder", placeholderStringer{placeholder: t.request.GetPlaceholderGroup()}))
	}
	log.Debug("proxy init search request",
		zap.Int64s("plan.OutputFieldIds", plan.GetOutputFieldIds()),
		zap.Stringer("plan", plan)) // may be very large if large term passed.
