...
func (t *searchTask) initAdvancedSearchRequest(ctx context.Context) error {
	...
	if t.request.FunctionScore != nil {
		if t.functionScore, err = rerank.NewFunctionScore(t.schema.CollectionSchema, t.request.FunctionScore, map[string]string{models.ClusterIDKey: paramtable.Get().CommonCfg.ClusterPrefix.GetValue()}); err != nil {
	...
	if embedding.HasNonBM25Functions(t.schema.CollectionSchema.Functions, queryFieldIDs) {
		...
		defer sp.End()
		exec, err := embedding.NewFunctionExecutor(t.schema.CollectionSchema, nil, map[string]string{models.ClusterIDKey: paramtable.Get().CommonCfg.ClusterPrefix.GetValue()})
		if err != nil {
			return err
		}
