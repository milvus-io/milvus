...
type searchTask struct {
	...
	*internalpb.SearchRequest
	...
	requery func(t *searchTask, span trace.Span, ids *schemapb.IDs, outputFields []string) (*milvuspb.QueryResults, error)
}
...
func (t *searchTask) initAdvancedSearchRequest(ctx context.Context) error {
	...
	if t.functionScore.IsSupportGroup() && t.rankParams.GetGroupByFieldId() >= 0 {
		merr.WrapErrParameterInvalidMsg("Current rerank does not support grouping search")
	}
...
func (t *searchTask) rank(ctx context.Context, span trace.Span, multipleMilvusResults []*milvuspb.SearchResults) error {
	...
	if t.needRequery {
		...
		queryResult, err := t.requery(t, span, t.result.Results.Ids, t.translatedOutputFields)
		if err != nil {
...
func (t *searchTask) hybridSearchRank(ctx context.Context, span trace.Span, multipleMilvusResults []*milvuspb.SearchResults) error {
	...
	if t.needRequery {
		...
		queryResult, err := t.requery(t, span, allIDs, allNames.Collect())
		if err != nil {
			log.Warn("failed to requery", zap.Error(err))
			return err
		}
...
func requery(t *searchTask, span trace.Span, ids *schemapb.IDs, outputFields []string) (*milvuspb.QueryResults, error) {
...
func (t *searchTask) fillInFieldInfo() {
	for _, retField := range t.result.Results.FieldsData {
		for _, schemaField := range t.schema.Fields {
			if retField != nil && retField.FieldName == schemaField.Name {
				retField.FieldId = schemaField.FieldID
