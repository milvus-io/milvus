...
func TestHistorical_Search(t *testing.T) {
	...
	t.Run("test search", func(t *testing.T) {
		...
		plan, req, err := genSimpleSearchPlanAndRequests()
		assert.NoError(t, err)

		_, _, _, err = his.search(req, defaultCollectionID, []UniqueID{defaultPartitionID}, plan, Timestamp(0))
		assert.NoError(t, err)
	})
	...
	t.Run("test no collection - search partitions", func(t *testing.T) {
		...
		_, _, _, err = his.search(req, defaultCollectionID, []UniqueID{}, plan, Timestamp(0))
		assert.Error(t, err)
	})
	...
	t.Run("test no partition in collection", func(t *testing.T) {
		...
		res, ids, _, err := his.search(req, defaultCollectionID, []UniqueID{}, plan, Timestamp(0))
	...
	t.Run("test load collection partition released in collection", func(t *testing.T) {
		...
		res, ids, _, err := his.search(req, defaultCollectionID, []UniqueID{defaultPartitionID}, plan, Timestamp(0))
		assert.Equal(t, 0, len(res))
		assert.Equal(t, 0, len(ids))
		assert.Error(t, err)
	})
