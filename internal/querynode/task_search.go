...
func (s *searchTask) searchOnStreaming() error {
	...
	if _, released := s.QS.collection.getReleaseTime(); released {
		log.Ctx(ctx).Info("collection release before search", zap.Int64("msgID", s.ID()),
	...
	if sErr != nil {
		log.Ctx(ctx).Info("failed to search streaming data", zap.Int64("msgID", s.ID()),
...
func (s *searchTask) estimateCPUUsage() {
	...
	if s.DataScope == querypb.DataScope_Streaming {
		...
		if err != nil {
			log.Info("searchTask estimateCPUUsage", zap.Error(err))
		}
...
func (s *searchTask) reduceResults(ctx context.Context, searchReq *searchRequest, results []*SearchResult) error {
	...
	if !isEmpty {
		...
		if err != nil {
			log.Ctx(ctx).Info("marshal for historical results error", zap.Int64("msgID", s.ID()), zap.Error(err))
			return err
		}
		...
		for i := 0; i < cnt; i++ {
			...
			if err != nil {
				log.Ctx(ctx).Info("getSearchResultDataBlob for historical results error", zap.Int64("msgID", s.ID()),
					zap.Error(err))
				return err
			}
