...
// BM25 and cluster memory_size while excluding text/JSON index stats.
func TestStatsBinlogSizeFromManifest(t *testing.T) {
	...
	clusterPath := path.Join(basePath, "_stats/cluster_stats.103/0.keys")
	...
	require.NoError(t, WriteFile(cfg, clusterPath, []byte("cluster-blob")))
	...
	got, err := CommitManifestUpdates(basePath, ManifestEarliest, cfg, &ManifestUpdates{
		Stats: []StatEntry{
			...
			{Key: "cluster_stats.103", Files: []string{clusterPath}, Metadata: map[string]string{"memory_size": "30"}},
	...
	require.NoError(t, err)
	require.Equal(t, int64(60), size, "should sum bloom_filter(10)+bm25(20)+cluster_stats(30), excluding text(5)")
}
