...
// TestStatsBinlogSizeFromManifest verifies the aggregate sums bloom-filter,
// BM25, and Text Log V2 memory_size while excluding text/JSON index stats.
func TestStatsBinlogSizeFromManifest(t *testing.T) {
	...
	textLogV2Path := path.Join(cfg.RootPath, basePath, "_stats/text_log_v2.102/1.fst")
	...
	require.NoError(t, WriteFile(cfg, textLogV2Path, []byte("text-log-v2")))
	...
	got, err := CommitManifestUpdates(basePath, ManifestEarliest, cfg, &ManifestUpdates{
		Stats: []StatEntry{
			...
			{Key: "text_log_v2.102", Files: []string{textLogV2Path}, Metadata: map[string]string{"memory_size": "7"}},
			// text index stats must NOT count toward StatsBinlogSize.
			{Key: "text.103", Files: []string{textPath}, Metadata: map[string]string{"memory_size": "5"}},
		},
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, int64(37), size, "should sum bloom_filter(10)+bm25(20)+text_log_v2(7), excluding text(5)")
}
