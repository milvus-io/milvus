...
func (r *StatsResolver) loadManifest() error {
	...
	basePath, _, err := UnmarshalManfestPath(r.manifestPath)
	if err != nil {
		r.manifestErr = fmt.Errorf("failed to parse manifest path: %w", err)
		return r.manifestErr
	}
	r.basePath = basePath
	r.rootPath = r.storageConfig.GetRootPath()
...
// resolveStatPaths joins rootPath, basePath, and relative paths into full storage paths.
func (r *StatsResolver) resolveStatPaths(relPaths []string) []string {
	resolved := make([]string, len(relPaths))
	for i, p := range relPaths {
		resolved[i] = path.Join(r.rootPath, r.basePath, p)
	}
	return resolved
}
