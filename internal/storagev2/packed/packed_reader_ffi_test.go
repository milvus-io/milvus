...
func TestPackedFFIReaderCloseIdempotent(t *testing.T) {
	paramtable.Init()
	pt := paramtable.Get()
	pt.Save(pt.CommonCfg.StorageType.Key, "local")
	dir := t.TempDir()
	...
	basePath := "files/packed_reader_close_test/1"
	version := int64(0)
	...
	pw, err := NewFFIPackedWriter(basePath, version, schema, columnGroups, nil, nil)
	require.NoError(t, err)
	require.NoError(t, pw.WriteRecordBatch(rec))
	manifest, err := pw.Close()
	require.NoError(t, err)
	...
	reader, err := NewFFIPackedReader(manifest, schema, []string{"pk", "vector"}, 8192, storageConfig, nil)
	require.NoError(t, err)
	...
	// Second close should be a no-op, not panic or double-free.
	err = reader.Close()
	require.NoError(t, err)

	// Release after close should also be safe (Release calls Close internally).
	reader.Release()
}

func TestPackedFFIReaderReleaseIdempotent(t *testing.T) {
	paramtable.Init()
	pt := paramtable.Get()
	pt.Save(pt.CommonCfg.StorageType.Key, "local")
	dir := t.TempDir()
	...
	basePath := "files/packed_reader_release_test/1"
	version := int64(0)
	...
	// Release calls Close internally. Calling it twice should be safe.
	reader.Release()
	reader.Release()
}
