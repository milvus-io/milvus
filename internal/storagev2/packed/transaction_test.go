...
func TestExternalFilePropertiesRoundTrip(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.format, func(t *testing.T) {
			...
			exploreManifest, err := CreateManifestForSegment(exploreBase, []string{"id"}, tc.format,
				[]Fragment{{FilePath: filePath, StartRow: 0, EndRow: 4, RowCount: 4, Properties: tc.properties}}, config)
