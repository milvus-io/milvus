...
func TestStatsResolverManifest(t *testing.T) {
	...
	jsonPath := filepath.Join(bp, "_stats/json_stats.300/7000/shared_key_index/.managed.json_0")
	...
	t.Run("TextAndJSONIndexStatsWithBasePaths returns json stats with metadata placeholder", func(t *testing.T) {
		...
		assert.Equal(t, []string{"7000/shared_key_index/.managed.json_0"}, result.JSONKeyStats[300].GetFiles())
