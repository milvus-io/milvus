...
func (suite *InsertNodeSuite) TestBasic() {
	...
	suite.delegator.EXPECT().ProcessInsert(mock.Anything).Run(func(insertRecords map[int64]*delegator.InsertData) {
		for segID := range insertRecords {
			suite.Contains(suite.insertSegmentIDs, segID)
		}
	})
...
func expectInsertSchemaState(t *testing.T, mockDelegator *delegator.MockShardDelegator, collection *segments.Collection) {
	t.Helper()
	mockDelegator.On("WithInsertSchemaState", mock.Anything).
		Run(func(args mock.Arguments) {
			state, err := collection.CaptureSchemaState()
			require.NoError(t, err)
			defer state.Release()
			args.Get(0).(func(*segments.CollectionSchemaState))(state)
		}).
		Return(nil).
		Once()
}
