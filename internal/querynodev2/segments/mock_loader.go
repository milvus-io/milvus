...
// LoadIndex provides a mock function with given fields: ctx, segment, info, version, schema
func (_m *MockLoader) LoadIndex(ctx context.Context, segment *LocalSegment, info *querypb.SegmentLoadInfo, version int64, schema *schemapb.CollectionSchema) error {
	ret := _m.Called(ctx, segment, info, version, schema)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *LocalSegment, *querypb.SegmentLoadInfo, int64, *schemapb.CollectionSchema) error); ok {
		r0 = rf(ctx, segment, info, version, schema)
	} else {
		r0 = ret.Error(0)
	}
...
func (_e *MockLoader_Expecter) LoadIndex(ctx interface{}, segment interface{}, info interface{}, version interface{}, schema interface{}) *MockLoader_LoadIndex_Call {
	return &MockLoader_LoadIndex_Call{Call: _e.mock.On("LoadIndex", ctx, segment, info, version, schema)}
}

func (_c *MockLoader_LoadIndex_Call) Run(run func(ctx context.Context, segment *LocalSegment, info *querypb.SegmentLoadInfo, version int64, schema *schemapb.CollectionSchema)) *MockLoader_LoadIndex_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(*LocalSegment), args[2].(*querypb.SegmentLoadInfo), args[3].(int64), args[4].(*schemapb.CollectionSchema))
	})
	return _c
}
...
func (_c *MockLoader_LoadIndex_Call) RunAndReturn(run func(context.Context, *LocalSegment, *querypb.SegmentLoadInfo, int64, *schemapb.CollectionSchema) error) *MockLoader_LoadIndex_Call {
	_c.Call.Return(run)
	return _c
}
