...
func NewManager() *Manager {
	...
	manager.DiskCache = cache.NewCacheBuilder[int64, Segment]().WithLazyScavenger(func(key int64) int64 {
		...
		return int64(segment.ResourceUsageEstimateOfLoad().Predict.DiskSize)
		...
		_, err, _ := sf.Do(fmt.Sprint(segment.ID()), func() (nop interface{}, err error) {
			...
			cacheLoadRecord.WithBytes(segment.ResourceUsageEstimateOfLoad().Predict.DiskSize)
			defer func() {
		...
		cacheEvictRecord.WithBytes(segment.ResourceUsageEstimateOfLoad().Predict.DiskSize)
