...
func (loader *segmentLoader) Load(ctx context.Context,
	...
	for _, info := range infos {
		...
		// prepareIndexLoadParams applies DiskANN/bitmap load params and re-runs
		// AppendPrepareLoadParams (idempotent; also ran before resource
		// estimation above) so those params are included at segment creation.
