...
func (suite *TextSegmentSuite) SaveTextBinLog(ctx context.Context,
	...
	// Blobs are returned in schema field order
	// Schema fields: RowID(0), Timestamp(1), PK(100), Vector(101), TEXT(103)
	// So blobs[0]=RowID, blobs[1]=Timestamp, blobs[2]=PK, blobs[3]=Vector, blobs[4]=TEXT
	// We only want to load the user fields (PK, Vector, TEXT)
	binlogs := make([]*datapb.FieldBinlog, 0)

	// Map blob index to field ID (skip system fields)
	blobsToLoad := []struct {
		blobIndex int
		fieldID   int64
	}{
