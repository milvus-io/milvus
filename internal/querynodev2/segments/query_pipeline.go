...
func buildStandardQNPipeline(
	...
	orderByFields, err := orderby.ConvertFromPlanOrderByFields(
		plan.GetQuery().GetOrderByFields(), schema,
	)
	if err != nil {
	...
	reduceType := reduce.ToReduceType(req.GetReq().GetReduceType())
	pipeline := queryutil.BuildQueryReducePipeline(
...
func RunDelegatorQueryPipeline(
	...
	// Unmarshal plan to extract ORDER BY fields
	var planNode planpb.PlanNode
	if err := proto.Unmarshal(req.Req.GetSerializedExprPlan(), &planNode); err != nil {
		return nil, fmt.Errorf("failed to unmarshal plan for query pipeline: %w", err)
	}
	orderByFields, err := orderby.ConvertFromPlanOrderByFields(
		planNode.GetQuery().GetOrderByFields(), schema,
	)
	if err != nil {
