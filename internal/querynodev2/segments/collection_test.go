...
func (s *CollectionManagerSuite) SetupTest() {
	...
	err := s.cm.PutOrRef(1, schema, nil, &querypb.LoadMetaInfo{
...
func (s *CollectionManagerSuite) TestUpdateSchema() {
	...
	s.Run("stale_schema_version_cannot_roll_back_fields", func() {
		...
		err := cm.PutOrRef(10, baseSchema, nil, &querypb.LoadMetaInfo{
			LoadType: querypb.LoadType_LoadCollection,
		})
...
func (s *CollectionManagerSuite) TestPutOrRefKeepsFreshCollectionInSchemaVersionDomain() {
	...
	err := cm.PutOrRef(10, initialSchema, nil, &querypb.LoadMetaInfo{
		LoadType: querypb.LoadType_LoadCollection,
	})
...
func (s *CollectionManagerSuite) TestFullLoadStillAdvancesServedSchema() {
	...
	s.Require().NoError(cm.PutOrRef(collID, baseSchema, nil, &querypb.LoadMetaInfo{
		LoadType: querypb.LoadType_LoadCollection,
	}))
	...
	s.Require().NoError(cm.PutOrRef(collID, v2Schema, nil, &querypb.LoadMetaInfo{
		LoadType: querypb.LoadType_LoadCollection,
	}))
	cm.Unref(collID, 1)
...
func (s *CollectionManagerSuite) TestPutOrRefUsesSchemaVersion() {
	s.Run("schema_version_seeds_served_version", func() {
		...
		schema.Version = 7
		s.Require().NoError(cm.PutOrRef(11, schema, nil, &querypb.LoadMetaInfo{
			LoadType: querypb.LoadType_LoadCollection,
		}))
		defer cm.Unref(11, 1)
	...
	s.Run("zero_schema_version_for_new_collection", func() {
		...
		s.Require().NoError(cm.PutOrRef(12, schema, nil, &querypb.LoadMetaInfo{
			LoadType: querypb.LoadType_LoadCollection,
		}))
		defer cm.Unref(12, 1)
...
func (s *CollectionManagerSuite) TestGpuIndexFlagWithCagraAdaptForCPU() {
	...
	for _, test := range tests {
		s.Run(test.name, func() {
			...
			collection, err := NewCollection(10, schema, indexInfos, &querypb.LoadMetaInfo{
				LoadType: querypb.LoadType_LoadCollection,
			})
...
func (s *CollectionManagerSuite) TestUnref() {
	...
	s.Run("unref_with_release", func() {
		...
		err := cm.PutOrRef(2, schema, nil, &querypb.LoadMetaInfo{
			LoadType: querypb.LoadType_LoadCollection,
		})
...
func (s *CollectionManagerSuite) TestPutOrRef() {
	s.Run("put_new_collection", func() {
		...
		err := cm.PutOrRef(100, schema, nil, &querypb.LoadMetaInfo{
			LoadType: querypb.LoadType_LoadCollection,
		})
