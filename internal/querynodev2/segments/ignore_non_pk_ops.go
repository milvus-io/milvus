...
//
// This replaces the standard path of MergeSegcoreRetrieveResults for IgnoreNonPk=true.
// Uses SelectMinPKWithTimestamp for k-way merge by PK order.
//
...
func NewMergeByPKWithOffsetsOperator(
	...
	return queryutil.NewLambdaOperator(queryutil.OpMergeByPKOffsets, func(ctx context.Context, span trace.Span, inputs ...any) ([]any, error) {
		...
		for j := 0; j < loopEnd && (limit == -1 || availableCount < limit); j++ {
			...
			if _, ok := idTsMap[pk]; !ok {
				...
				if ts != 0 && ts > idTsMap[pk] {
					idTsMap[pk] = ts
					// Find and replace existing selection
					idx := len(selections) - 1
					for ; idx >= 0; idx-- {
						pkValue := typeutil.GetPK(ids, int64(idx))
						if pk == pkValue {
							break
						}
					}
					if idx >= 0 {
						selections[idx] = OffsetSelection{
							SegmentIndex: sel,
							Offset:       offset,
						}
					}
		...
		if skipDupCnt > 0 {
			log.Ctx(ctx).Debug("skip duplicated result while merging segcore results with offsets", zap.Int64("dupCount", skipDupCnt))
		}
