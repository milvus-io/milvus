...
func MergeInternalRetrieveResult(ctx context.Context, retrieveResults []*internalpb.RetrieveResults, param *mergeParam) (*internalpb.RetrieveResults, error) {
	...
	for j := 0; j < loopEnd; {
		...
		if _, ok := idTsMap[pk]; !ok {
			...
			appendSize, _ := typeutil.AppendFieldData(ret.FieldsData, validRetrieveResults[sel].Result.GetFieldsData(), cursors[sel], nil)
			retSize += appendSize
			idTsMap[pk] = ts
			j++
		} else {
...
func MergeSegcoreRetrieveResults(ctx context.Context, retrieveResults []*segcorepb.RetrieveResults, param *mergeParam, segments []Segment, plan *RetrievePlan, manager *Manager) (*segcorepb.RetrieveResults, error) {
	...
	if !plan.IsIgnoreNonPk() {
		...
		for _, selection := range selections {
			...
			appendSize, _ := typeutil.AppendFieldData(ret.FieldsData, validRetrieveResults[selection.batchIndex].Result.GetFieldsData(), selection.resultIndex, nil)
			retSize += appendSize
			// limit retrieve result to avoid oom
			if retSize > maxOutputSize {
				return nil, fmt.Errorf("query results exceed the maxOutputSize Limit %d", maxOutputSize)
		...
		for _, selection := range selections {
			appendSize, _ := typeutil.AppendFieldData(ret.FieldsData, segmentResults[selection.batchIndex].GetFieldsData(), segmentResOffset[selection.batchIndex], nil)
			retSize += appendSize
