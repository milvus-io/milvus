...
func appendDefaultSegOffsetChunk(pool memory.Allocator, collector *chain.ChunkCollector, chunkIdx, rows int) {
	...
	collector.Set(segOffsetCol, chunkIdx, offsetBuilder.NewArray())
...
func buildTestDF(pool memory.Allocator, idsPerChunk [][]int64, scoresPerChunk [][]float32) *chain.DataFrame {
	...
	colNames := []string{idFieldName, scoreFieldName, segOffsetCol}
	...
	_ = builder.AddColumnFromChunks(segOffsetCol, collector.Consume(segOffsetCol))
	collector.Release()

	return builder.Build()
}

func buildTestDFWithoutSegOffset(pool memory.Allocator, idsPerChunk [][]int64, scoresPerChunk [][]float32) *chain.DataFrame {
	numChunks := len(idsPerChunk)
	chunkSizes := make([]int64, numChunks)
...
func buildTestDFWithGroupBy(pool memory.Allocator, idsPerChunk [][]int64, scoresPerChunk [][]float32, groupByPerChunk [][]int64, groupByFieldName string) *chain.DataFrame {
	...
	colNames := []string{idFieldName, scoreFieldName, segOffsetCol, groupByFieldName}
	collector := chain.NewChunkCollector(colNames, numChunks)
...
// buildTestDFWithStringPKAndGroupBy creates a test DataFrame with varchar PKs
// and int64 group-by values.
func buildTestDFWithStringPKAndGroupBy(pool memory.Allocator, idsPerChunk [][]string, scoresPerChunk [][]float32, groupByPerChunk [][]int64, groupByFieldName string) *chain.DataFrame {
	numChunks := len(idsPerChunk)
	chunkSizes := make([]int64, numChunks)
...
func buildTestDFWithStringPK(pool memory.Allocator, idsPerChunk [][]string, scoresPerChunk [][]float32) *chain.DataFrame {
	numChunks := len(idsPerChunk)
	chunkSizes := make([]int64, numChunks)
...
func buildTestDFWithStringPKAndElementIndices(pool memory.Allocator, idsPerChunk [][]string, scoresPerChunk [][]float32, elemIdxPerChunk [][]int32) *chain.DataFrame {
	numChunks := len(idsPerChunk)
	chunkSizes := make([]int64, numChunks)

	colNames := []string{idFieldName, scoreFieldName, segOffsetCol, elementIndicesCol}
	collector := chain.NewChunkCollector(colNames, numChunks)
...
func buildTestDFWithSegOffset(pool memory.Allocator, idsPerChunk [][]int64, scoresPerChunk [][]float32, offsetsPerChunk [][]int64) *chain.DataFrame {
	...
	for i := 0; i < numChunks; i++ {
		...
		offsetBuilder := array.NewInt64Builder(pool)
		offsetBuilder.AppendValues(offsetsPerChunk[i], nil)
		collector.Set(segOffsetCol, i, offsetBuilder.NewArray())
		offsetBuilder.Release()
	}
...
func buildTestDFWithElementIndicesAndGroupBy(
...
func TestHeapMergeReduce_StringPKStandard(t *testing.T) {
	pool := memory.NewGoAllocator()

	df0 := buildTestDFWithStringPK(pool,
		[][]string{{"b", "d"}},
		[][]float32{{0.9, 0.7}})
	defer df0.Release()

	df1 := buildTestDFWithStringPK(pool,
		[][]string{{"a", "b", "c"}},
		[][]float32{{0.9, 0.85, 0.6}})
	defer df1.Release()
	...
	require.Equal(t, 4, ids.Len())
	assert.Equal(t, []string{"a", "b", "d", "c"}, []string{
		ids.Value(0), ids.Value(1), ids.Value(2), ids.Value(3),
	})
}

func TestHeapMergeReduce_StringPKElementIndicesDedup(t *testing.T) {
	pool := memory.NewGoAllocator()

	df0 := buildTestDFWithStringPKAndElementIndices(pool,
		[][]string{{"doc", "doc", "z"}},
		[][]float32{{0.99, 0.97, 0.95}},
		[][]int32{{0, 1, 0}})
	defer df0.Release()

	df1 := buildTestDFWithStringPKAndElementIndices(pool,
		[][]string{{"doc", "a"}},
		[][]float32{{0.98, 0.96}},
		[][]int32{{1, 0}})
	defer df1.Release()
	...
	ids := result.DF.Column(idFieldName).Chunk(0).(*array.String)
	ei := result.DF.Column(elementIndicesCol).Chunk(0).(*array.Int32)
	require.Equal(t, 4, ids.Len())
	assert.Equal(t, []string{"doc", "doc", "a", "z"}, []string{
		ids.Value(0), ids.Value(1), ids.Value(2), ids.Value(3),
	})
	assert.Equal(t, []int32{0, 1, 0, 0}, []int32{
		ei.Value(0), ei.Value(1), ei.Value(2), ei.Value(3),
	})
}

func TestHeapMergeReduce_GroupByWithoutColumnsFallsBackToStandard(t *testing.T) {
	pool := memory.NewGoAllocator()
	...
	assert.False(t, result.DF.HasColumn(groupByCol))
	assert.Equal(t, []int64{2}, result.DF.ChunkSizes())
	ids := result.DF.Column(idFieldName).Chunk(0).(*array.Int64)
	assert.Equal(t, []int64{1, 2}, []int64{ids.Value(0), ids.Value(1)})
}

func TestHeapMergeReduce_SegOffsetSourceTracking(t *testing.T) {
	pool := memory.NewGoAllocator()

	df0 := buildTestDFWithSegOffset(pool,
		[][]int64{{1, 3}},
		[][]float32{{0.9, 0.7}},
		[][]int64{{101, 103}})
	defer df0.Release()

	df1 := buildTestDFWithSegOffset(pool,
		[][]int64{{2}},
		[][]float32{{0.8}},
		[][]int64{{202}})
	defer df1.Release()
	...
	require.Len(t, result.Sources[0], 3)
	assert.Equal(t, []int64{101, 202, 103}, []int64{
		result.Sources[0][0].SegOffset,
		result.Sources[0][1].SegOffset,
		result.Sources[0][2].SegOffset,
	})
	assert.Equal(t, []int{0, 0, 1}, []int{
		result.Sources[0][0].OriginalIdx,
		result.Sources[0][1].OriginalIdx,
		result.Sources[0][2].OriginalIdx,
	})
}
...
func TestHeapMergeReduce_GroupByNullKey(t *testing.T) {
	pool := memory.NewGoAllocator()

	gb0Builder := array.NewInt64Builder(pool)
	gb0Builder.AppendValues([]int64{0, 0, 10, 10}, []bool{false, false, true, true})
	gb0 := gb0Builder.NewArray()
	gb0Builder.Release()
	defer gb0.Release()

	gb1Builder := array.NewInt64Builder(pool)
	gb1Builder.AppendValues([]int64{0, 10}, []bool{false, true})
	gb1 := gb1Builder.NewArray()
	gb1Builder.Release()
	defer gb1.Release()
	...
	ids := result.DF.Column(idFieldName).Chunk(0).(*array.Int64)
	require.Equal(t, 4, ids.Len())
	assert.Equal(t, []int64{1, 5, 6, 3}, []int64{
		ids.Value(0), ids.Value(1), ids.Value(2), ids.Value(3),
	})

	gb := result.DF.Column(groupByCol).Chunk(0).(*array.Int64)
	require.Equal(t, 4, gb.Len())
	assert.True(t, gb.IsNull(0), "first null group row should be preserved")
	assert.True(t, gb.IsNull(1), "second null group row should be preserved")
	assert.False(t, gb.IsNull(2))
	assert.False(t, gb.IsNull(3))
	assert.Equal(t, int64(10), gb.Value(2))
	assert.Equal(t, int64(10), gb.Value(3))
}

func TestHeapMergeReduce_GroupByStringPK(t *testing.T) {
	pool := memory.NewGoAllocator()

	df0 := buildTestDFWithStringPKAndGroupBy(pool,
		[][]string{{"a", "b", "c", "d"}},
		[][]float32{{0.9, 0.8, 0.7, 0.6}},
		[][]int64{{1, 1, 2, 3}},
		groupByCol)
	...
	df1 := buildTestDFWithStringPKAndGroupBy(pool,
		[][]string{{"e", "f", "b"}},
		[][]float32{{0.85, 0.75, 0.65}},
		[][]int64{{2, 2, 1}},
		groupByCol)
	defer df1.Release()
	...
	ids := result.DF.Column(idFieldName).Chunk(0).(*array.String)
	require.Equal(t, 4, ids.Len())
	assert.Equal(t, []string{"a", "e", "b", "f"}, []string{
		ids.Value(0), ids.Value(1), ids.Value(2), ids.Value(3),
	})

	gb := result.DF.Column(groupByCol).Chunk(0).(*array.Int64)
	assert.Equal(t, []int64{1, 2, 1, 2}, []int64{
		gb.Value(0), gb.Value(1), gb.Value(2), gb.Value(3),
	})
}

func TestHeapMergeReduce_GroupByStringPKSkipsDuplicateAndFullGroup(t *testing.T) {
	pool := memory.NewGoAllocator()
	...
	assert.Equal(t, []string{"a", "d"}, []string{ids.Value(0), ids.Value(1)})

	gb := result.DF.Column(groupByCol).Chunk(0).(*array.Int64)
	assert.Equal(t, []int64{1, 2}, []int64{gb.Value(0), gb.Value(1)})
}

func TestMergeGroupByNoGroupFieldsFallsBackToStandard(t *testing.T) {
	pool := memory.NewGoAllocator()

	intIDBuilder := array.NewInt64Builder(pool)
	intIDBuilder.AppendValues([]int64{1, 2}, nil)
	intIDs := intIDBuilder.NewArray().(*array.Int64)
	intIDBuilder.Release()
	defer intIDs.Release()

	intScoreBuilder := array.NewFloat32Builder(pool)
	intScoreBuilder.AppendValues([]float32{0.9, 0.8}, nil)
	intScores := intScoreBuilder.NewArray().(*array.Float32)
	intScoreBuilder.Release()
	defer intScores.Release()

	intOffsetBuilder := array.NewInt64Builder(pool)
	intOffsetBuilder.AppendValues([]int64{10, 11}, nil)
	intOffsets := intOffsetBuilder.NewArray().(*array.Int64)
	intOffsetBuilder.Release()
	defer intOffsets.Release()

	intHeap := &mergeHeapInt64Pk{&mergeEntry{idInt64: intIDs, scoreArr: intScores, segOffsetArr: intOffsets}}
	heap.Init(intHeap)
	intIDsOut, _, intSources, err := mergeGroupByInt64Pk(intHeap, 2, 2, 0)
	require.NoError(t, err)
	assert.Equal(t, []int64{1, 2}, intIDsOut)
	assert.Equal(t, []segmentSource{{SegOffset: 10, OriginalIdx: 0}, {SegOffset: 11, OriginalIdx: 1}}, intSources)

	stringIDBuilder := array.NewStringBuilder(pool)
	stringIDBuilder.AppendValues([]string{"a", "b"}, nil)
	stringIDs := stringIDBuilder.NewArray().(*array.String)
	stringIDBuilder.Release()
	defer stringIDs.Release()

	stringScoreBuilder := array.NewFloat32Builder(pool)
	stringScoreBuilder.AppendValues([]float32{0.7, 0.6}, nil)
	stringScores := stringScoreBuilder.NewArray().(*array.Float32)
	stringScoreBuilder.Release()
	defer stringScores.Release()

	stringOffsetBuilder := array.NewInt64Builder(pool)
	stringOffsetBuilder.AppendValues([]int64{20, 21}, nil)
	stringOffsets := stringOffsetBuilder.NewArray().(*array.Int64)
	stringOffsetBuilder.Release()
	defer stringOffsets.Release()

	stringHeap := &mergeHeapStringPk{&mergeEntry{idString: stringIDs, scoreArr: stringScores, segOffsetArr: stringOffsets}}
	heap.Init(stringHeap)
	stringIDsOut, _, stringSources, err := mergeGroupByStringPk(stringHeap, 2, 2, 0)
	require.NoError(t, err)
	assert.Equal(t, []string{"a", "b"}, stringIDsOut)
	assert.Equal(t, []segmentSource{{SegOffset: 20, OriginalIdx: 0}, {SegOffset: 21, OriginalIdx: 1}}, stringSources)
}

func TestExtractCompositeGroupValuesMissingArray(t *testing.T) {
	pool := memory.NewGoAllocator()

	floatBuilder := array.NewFloat32Builder(pool)
	floatBuilder.AppendValues([]float32{1.25}, nil)
	floatArr := floatBuilder.NewArray()
	floatBuilder.Release()
	defer floatArr.Release()

	values, err := extractCompositeGroupValues(&mergeEntry{
		cursor:      0,
		groupByArrs: []arrow.Array{floatArr},
	}, 2)

	require.Error(t, err)
	assert.Nil(t, values)
	assert.Contains(t, err.Error(), "unsupported group-by arrow type float32")
}

func TestExtractCompositeGroupValuesMissingArrayErrors(t *testing.T) {
	pool := memory.NewGoAllocator()

	intBuilder := array.NewInt64Builder(pool)
	intBuilder.AppendValues([]int64{1}, nil)
	...
	values, err := extractCompositeGroupValues(&mergeEntry{
		cursor:      0,
		groupByArrs: []arrow.Array{intArr},
	}, 2)

	require.Error(t, err)
	assert.Nil(t, values)
	assert.Contains(t, err.Error(), "missing group-by column")
}
...
func buildTestDFWithTwoGroupByColumns(
	...
	colNames := []string{idFieldName, scoreFieldName, segOffsetCol, groupByCol0, groupByCol1}
	collector := chain.NewChunkCollector(colNames, numChunks)
...
func buildTestDFWithGroupByArr(pool memory.Allocator, idsPerChunk [][]int64, scoresPerChunk [][]float32, groupByArrPerChunk []arrow.Array) *chain.DataFrame {
	...
	colNames := []string{idFieldName, scoreFieldName, segOffsetCol, groupByCol}
	collector := chain.NewChunkCollector(colNames, numChunks)
	...
	tests := []struct {
		...
		},
		{
			...
			buildSeg0: func(pool memory.Allocator) arrow.Array {
				...
				defer b.Release()
				b.AppendValues([]bool{true, true, true, false}, nil)
				return b.NewArray()
			},
			buildSeg1: func(pool memory.Allocator) arrow.Array {
				b := array.NewBooleanBuilder(pool)
				defer b.Release()
				b.AppendValues([]bool{false, false}, nil)
				return b.NewArray()
			},
			assertCol: func(t *testing.T, arr arrow.Array) {
				a, ok := arr.(*array.Boolean)
				require.True(t, ok, "group-by column should be *array.Boolean")
				assert.Equal(t, true, a.Value(0))
				assert.Equal(t, false, a.Value(1))
				assert.Equal(t, true, a.Value(2))
				assert.Equal(t, false, a.Value(3))
			},
		},
	}
...
func TestHeapMergeReduce_MissingSegOffsetErrors(t *testing.T) {
	pool := memory.NewGoAllocator()

	df := buildTestDFWithoutSegOffset(pool, [][]int64{{1}}, [][]float32{{0.9}})
	defer df.Release()

	result, err := heapMergeReduce(pool, []*chain.DataFrame{df}, 1, nil)
	require.Error(t, err)
	assert.Nil(t, result)
	assert.Contains(t, err.Error(), "missing required $seg_offset column")
}

func TestHeapMergeReduce_InvalidGroupSizeErrors(t *testing.T) {
	pool := memory.NewGoAllocator()
	...
	result, err := heapMergeReduce(pool, []*chain.DataFrame{df}, 1, &groupByOptions{
		GroupSize: 0,
		Columns:   []string{groupByCol},
	})
	require.Error(t, err)
	assert.Nil(t, result)
	assert.Contains(t, err.Error(), "group size must be positive")
}
...
func TestHeapMergeReduce_MissingIDColumn(t *testing.T) {
	pool := memory.NewGoAllocator()
	...
	_, err := heapMergeReduce(pool, []*chain.DataFrame{df}, 2, nil)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "$id column not found")
}

func TestHeapMergeReduce_MissingScoreColumnProducesEmptyResult(t *testing.T) {
	pool := memory.NewGoAllocator()
	...
	result, err := heapMergeReduce(pool, []*chain.DataFrame{df}, 2, nil)
	require.NoError(t, err)
	defer result.DF.Release()

	assert.Equal(t, []int64{0}, result.DF.ChunkSizes())
	assert.Zero(t, result.DF.NumRows())
	require.Len(t, result.Sources, 1)
	assert.Empty(t, result.Sources[0])
}

func TestHeapMergeReduceImplDuplicateOutputColumnError(t *testing.T) {
	pool := memory.NewGoAllocator()

	df := buildTestDF(pool, [][]int64{{1}}, [][]float32{{0.9}})
	defer df.Release()

	result, err := heapMergeReduceImpl(pool, []*chain.DataFrame{df}, 1, &groupByOptions{
		GroupSize: 1,
		Columns:   []string{idFieldName},
	}, 0, 1, false)
	require.Error(t, err)
	assert.Nil(t, result)
...
func TestPickGroupByValuesEmptyAndUnsupportedArrays(t *testing.T) {
	pool := memory.NewGoAllocator()

	intBuilder := array.NewInt32Builder(pool)
	intBuilder.AppendValues([]int32{10, 20}, nil)
	intArr := intBuilder.NewArray()
	intBuilder.Release()
	defer intArr.Release()

	intDF := buildTestDFWithGroupByArr(pool,
		[][]int64{{1, 2}},
		[][]float32{{0.9, 0.8}},
		[]arrow.Array{intArr})
	defer intDF.Release()
	intCols := resolveInputCols([]*chain.DataFrame{intDF}, &groupByOptions{Columns: []string{groupByCol}}, false)

	empty, err := pickGroupByValues(pool, intCols, 0, nil, 0)
	require.NoError(t, err)
	defer empty.Release()
	assert.Equal(t, arrow.INT32, empty.DataType().ID())
	assert.Zero(t, empty.Len())

	floatBuilder := array.NewFloat32Builder(pool)
	floatBuilder.AppendValues([]float32{1.5}, nil)
	floatArr := floatBuilder.NewArray()
	floatBuilder.Release()
	defer floatArr.Release()

	floatDF := buildTestDFWithGroupByArr(pool,
		[][]int64{{1}},
		[][]float32{{0.9}},
		[]arrow.Array{floatArr})
	defer floatDF.Release()
	floatCols := resolveInputCols([]*chain.DataFrame{floatDF}, &groupByOptions{Columns: []string{groupByCol}}, false)

	unsupported, err := pickGroupByValues(pool, floatCols, 0, []segmentSource{{InputIdx: 0, OriginalIdx: 0}}, 0)
	require.Error(t, err)
	assert.Nil(t, unsupported)
	assert.Contains(t, err.Error(), "unsupported group-by arrow type float32")

	fallback := buildEmptyGroupByArray(pool, []inputCols{{}}, 0)
	defer fallback.Release()
	assert.Equal(t, arrow.INT64, fallback.DataType().ID())
	assert.Zero(t, fallback.Len())

	noArrays, err := pickGroupByValues(pool, []inputCols{{}}, 0, []segmentSource{{InputIdx: 0}}, 0)
	require.NoError(t, err)
	defer noArrays.Release()
	assert.Equal(t, arrow.INT64, noArrays.DataType().ID())
	assert.Zero(t, noArrays.Len())
}

func TestHeapMergeReduceUnsupportedGroupByTypeReleasesMemory(t *testing.T) {
	...
	df := buildTestDFWithGroupByArr(pool,
		[][]int64{{1}},
		[][]float32{{0.9}},
		[]arrow.Array{floatArr})
	defer df.Release()
...
func TestHeapMergeReduceMissingGroupByColumnErrors(t *testing.T) {
	pool := memory.NewGoAllocator()
...
func TestHeapMergeReduceElementIndicesMustBePresentOnAllInputs(t *testing.T) {
	pool := memory.NewGoAllocator()
	...
	result, err := heapMergeReduce(pool, []*chain.DataFrame{df0, df1}, 2, nil)
	require.Error(t, err)
	assert.Nil(t, result)
	assert.Contains(t, err.Error(), "missing $element_indices column")
}

func TestCompositeGroupCounterHashBucketCollision(t *testing.T) {
	values := []any{int64(2)}
	hash := reduce.HashGroupValues(values)
	counter := &compositeGroupCounter{
		groups: map[uint64][]*compositeGroup{
			hash: {
				{values: []any{int64(1)}, count: 1},
			},
		},
		distinct:       1,
		saturatedCount: 1,
	...
	require.True(t, counter.shouldAccept(values))
	require.Len(t, counter.groups[hash], 2)
	assert.Equal(t, values, counter.groups[hash][1].values)
	assert.Equal(t, int64(2), counter.distinct)
}

func TestCompositeGroupCounterAllSaturated(t *testing.T) {
	counter := newCompositeGroupCounter(2, 2)
	assert.False(t, counter.allSaturated())
	assert.Equal(t, int64(0), counter.saturatedCount)

	require.True(t, counter.shouldAccept([]any{int64(10)}))
	require.True(t, counter.shouldAccept([]any{int64(10)}))
	assert.False(t, counter.allSaturated())
	assert.Equal(t, int64(1), counter.saturatedCount)

	require.True(t, counter.shouldAccept([]any{int64(20)}))
	assert.False(t, counter.allSaturated())
	assert.Equal(t, int64(1), counter.saturatedCount)

	require.True(t, counter.shouldAccept([]any{int64(20)}))
	assert.True(t, counter.allSaturated())
	assert.Equal(t, int64(2), counter.saturatedCount)
}
...
// TestHeapMergeReduce_ElementIndicesDedup verifies that element-level search
// deduplicates by (PK, element_index): same PK with different elements is kept,
// while the same element hit is dropped.
func TestHeapMergeReduce_ElementIndicesDedup(t *testing.T) {
	...
	assert.Equal(t, int64(5), ids.Value(0))
	assert.Equal(t, int32(0), eiCol.Value(0))

	assert.Equal(t, int64(5), ids.Value(1))
	assert.Equal(t, int32(1), eiCol.Value(1))

	assert.Equal(t, int64(6), ids.Value(2))
	assert.Equal(t, int32(0), eiCol.Value(2))
...
func TestHeapMergeReduce_GroupByElementIndicesDedup(t *testing.T) {
	...
	df0 := buildTestDFWithElementIndicesAndGroupBy(pool,
		[][]int64{{5, 5, 6}},
		[][]float32{{0.99, 0.97, 0.96}},
		[][]int32{{0, 1, 0}},
		[][]int64{{5, 5, 6}},
	...
	df1 := buildTestDFWithElementIndicesAndGroupBy(pool,
		[][]int64{{5, 7}},
		[][]float32{{0.98, 0.95}},
		[][]int32{{1, 0}},
		[][]int64{{5, 7}},
	...
	assert.Equal(t, []int64{5, 5, 6}, []int64{ids.Value(0), ids.Value(1), ids.Value(2)})
	assert.Equal(t, []int32{0, 1, 0}, []int32{eiCol.Value(0), eiCol.Value(1), eiCol.Value(2)})
}
