...
// the operational metrics have to describe the same workload.
...
// sharedFilterFanOut drives executeSharedFilter with the segment search and
// per-branch reduce replaced. What remains is the fan-out's own behavior: how
// often it measures the searched segments, which context each branch receives,
// and which branches run.
...
// its size, so a large shared-filter group must not repeat that walk per
// branch.
