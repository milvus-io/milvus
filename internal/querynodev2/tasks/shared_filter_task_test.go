...
func TestSharedFilterResultPinOwnership(t *testing.T) {
	t.Run("envelope adopts branch pins", func(t *testing.T) {
		var released atomic.Int32
		branches := []*internalpb.SearchResults{
			{SlicedBlob: []byte{1}},
			{SlicedBlob: []byte{2}},
		}
		pinned := []bool{true, true}
		for _, branch := range branches {
			resource.MsgPins.Pin(branch, func() { released.Add(1) })
		}
		envelope := assembleSharedFilterEnvelope(7, branches, 0)
		require.Same(t, &branches[0].SlicedBlob[0], &envelope.SubResults[0].SlicedBlob[0],
			"the envelope must reuse the C-backed bytes without copying")
		adoptSharedFilterResultPins(envelope, branches, pinned)
		require.True(t, resource.MsgPins.HasPinned(envelope))

		runtime.GC()
		require.Zero(t, released.Load())
		resource.MsgPins.Release(envelope)
		require.EqualValues(t, 2, released.Load())
		require.Equal(t, []bool{false, false}, pinned)
		resource.MsgPins.Release(envelope)
		require.EqualValues(t, 2, released.Load(), "envelope cleanup must run exactly once")
	})

	t.Run("error path releases completed branches", func(t *testing.T) {
		var released atomic.Int32
		branch := &internalpb.SearchResults{SlicedBlob: []byte{1}}
		resource.MsgPins.Pin(branch, func() { released.Add(1) })
		pinned := []bool{true, false}
		releaseSharedFilterResultPins([]*internalpb.SearchResults{branch, nil}, pinned)
		require.EqualValues(t, 1, released.Load())
		require.Equal(t, []bool{false, false}, pinned)
	})

	t.Run("copied or empty branches leave envelope unmanaged", func(t *testing.T) {
		for _, branches := range [][]*internalpb.SearchResults{
			{{SlicedBlob: []byte{1}}, {SlicedBlob: []byte{2}}},
			{{}, {}},
		} {
			envelope := assembleSharedFilterEnvelope(7, branches, 0)
			adoptSharedFilterResultPins(envelope, branches, []bool{false, false})
			require.False(t, resource.MsgPins.HasPinned(envelope))
		}
	})
}
...
func newSharedFilterFanOut(t *testing.T, branches, segCount int, body func(ctx context.Context, branch int) error) *sharedFilterFanOut {
	...
	patches := []*mockey.Mocker{
		...
		mockey.Mock((*SearchTask).reduceSegmentResults).To(
			func(branchTask *SearchTask, searchReq *segcore.SearchRequest, _ []*segcore.SearchResult, relatedDataSize int64, _ *timerecord.TimeRecorder) error {
