...
func (t *SearchTask) executeSingle(tr *timerecord.TimeRecorder) error {
	...
	return t.reduceSegmentResults(searchReq, results, searchedSegments, relatedDataSizeOf(searchedSegments), tr)
}
...
//
// relatedDataSize is passed in rather than derived from searchedSegments,
// which is still needed for the L0 rerank: see relatedDataSizeOf.
...
func (t *SearchTask) executeSharedFilter(tr *timerecord.TimeRecorder) error {
	...
	// failure reaches sibling branch work. The fan-out is bounded, so most of a
	// large group is still queued when a branch fails; the check below keeps a
	// queued branch from starting its reduce after cancellation.
	...
	for i, branchReq := range branchReqs {
		...
		group.Go(func() error {
			...
			if err := branch.reduceSegmentResults(searchReqs[i], grouped[i], searchedSegments, relatedDataSize, branchTR); err != nil {
				return err
			}
	...
	t.result = assembleSharedFilterEnvelope(t.GetNodeID(), branchResults, tr.ElapseSpan().Milliseconds())
	return nil
}
...
// uses the fan-out's context so branch work observes sibling failure.
