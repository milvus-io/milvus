...
//
// searchedSegments is still needed for the L0 rerank; relatedDataSize is
// passed in rather than derived from it because a shared-filter group reduces
// once per branch over the same segments.
...
// the maximum over tasks merged along the NQ axis; a shared-filter group
// extends the same rule over its branches.
