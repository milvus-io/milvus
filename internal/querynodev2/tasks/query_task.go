...
func (t *QueryTask) Execute() error {
	...
	logger.Info("[sss] query task execute start")
	...
	if err != nil {
		logger.Warn("[sss] query task build retrieve plan failed",
			zap.Duration("duration", time.Since(stageStart)),
			zap.Error(err))
		return err
	}
	logger.Info("[sss] query task build retrieve plan done",
		zap.Duration("duration", time.Since(stageStart)),
		zap.Bool("planShouldIgnoreNonPk", retrievePlan.ShouldIgnoreNonPk()))
	defer retrievePlan.Delete()

	stageStart = time.Now()
	logger.Info("[sss] query task retrieve start")
	...
	if err != nil {
		logger.Warn("[sss] query task retrieve failed",
			zap.Duration("duration", time.Since(stageStart)),
			zap.Int("pinnedSegmentNum", len(pinnedSegments)),
			zap.Error(err))
		return err
	}
	logger.Info("[sss] query task retrieve done",
		zap.Duration("duration", time.Since(stageStart)),
		zap.Int("resultNum", len(results)),
		zap.Int("pinnedSegmentNum", len(pinnedSegments)))

	beforeReduce := time.Now()
	logger.Info("[sss] query task reduce start",
		zap.Int("resultNum", len(results)))
	...
	if err != nil {
		logger.Warn("[sss] query task reduce failed",
			zap.Duration("duration", time.Since(beforeReduce)),
			zap.Error(err))
		return err
	}
	logger.Info("[sss] query task reduce done",
		zap.Duration("duration", time.Since(beforeReduce)))
	...
	logger.Info("[sss] query task execute done",
