...
func (node *QueryNode) Init() error {
	...
	node.initOnce.Do(func() {
		...
		node.loadPool, err = concurrency.NewPool(cpuNum, ants.WithPreAlloc(true))
		if err != nil {
			log.Error("QueryNode init load pool failed", zap.Error(err))
			initError = err
			return
		}
		...
		if err != nil {
			log.Error("QueryNode init task pool failed", zap.Error(err))
			initError = err
			return
		}
