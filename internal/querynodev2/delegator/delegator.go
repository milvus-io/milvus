...
func (sd *shardDelegator) search(ctx context.Context, req *querypb.SearchRequest, sealed []SnapshotItem, growing []SegmentEntry, sealedRowCount map[int64]int64) ([]*internalpb.SearchResults, error) {
	...
	req, err = sd.optimizeSearchParams(ctx, req, sd.queryHook, sealedNum)
	if err != nil {
	...
	if len(req.GetExtraFilterSharingReqs()) > 0 && len(results) < len(tasks) {
		log.Warn("shared-filter group lost a worker to a partial result, retrying per branch",
		...
		for groupIdx := range groups {
			...
			future := conc.Go(func() ([]*internalpb.SearchResults, error) {
				...
				if errors.Is(err, errSharedFilterUngroupable) {
					...
					// be skipped, a worker failure was absorbed as a partial
					// result, or vector pruning was activated after grouping.
					// Redo it one branch at a time, each branch in its own future,
					// which is exactly the ungrouped path.
					log.Debug("shared-filter group not executable as a group, retrying per branch",
						zap.Int("branches", len(group)))
	...
	reduceBranch := func(reduceCtx context.Context, branchIdx, subReqIdx int) error {
		...
		branchResult, err := segments.ReduceSearchOnQueryNode(reduceCtx,
			...
			reduce.NewReduceSearchResultInfo(subReq.GetNq(), subReq.GetTopk()).
				...
				WithGroupByField(subReq.GetGroupByFieldId()).
