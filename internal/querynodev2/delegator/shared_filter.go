...
func groupSubReqsBySharedFilter(
	...
	log.Ctx(ctx).Debug("grouped hybrid sub-requests by shared filter",
		zap.Int("subRequests", len(subReqs)),
...
//   - the refreshable clustering-key config selects a vector key after the
//     group was formed. Such pruning reads only branch 0's query vector, so
//     the other branches would search the wrong segments.
//
// The retry runs only when a branch is skipped, a worker failure is accepted
// as a partial result, or key selection changes after grouping. Avoiding it
// would require per-branch status and pruning state on the wire.
...
func (sd *shardDelegator) optimizeSearchParams(
	...
	extras := req.GetExtraFilterSharingReqs()
	if len(extras) == 0 {
		return optimizers.OptimizeSearchParams(ctx, req, queryHook, numSegments)
	}
	...
	branchGroup.SetLimit(min(branchCount, hardware.GetCPUNum()))
	branchGroup.Go(func() error {
		branchOptimized, err := optimizers.OptimizeSearchParams(branchCtx, req, queryHook, numSegments)
		if err != nil {
			return err
		}
	...
	for i := range extras {
		...
		branchGroup.Go(func() error {
			...
			branchOptimized, err := optimizers.OptimizeSearchParams(
				branchCtx, branchReq, queryHook, numSegments)
			if err != nil {
				return err
			}
