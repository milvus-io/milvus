...
//   - refreshable clustering-key config selected a vector key after grouping.
//     Vector pruning reads branch 0's query vector, so the branches must retry
//     separately to retain their own segment selection.
//
// The retry costs a second round of worker calls, but it runs only when a
// branch is skipped, a worker failure is accepted as a partial result, or key
// selection changes after grouping. Avoiding it would require per-branch
// status and pruning state on the wire.
...
func (sd *shardDelegator) optimizeSearchParams(
	...
	extras := req.GetExtraFilterSharingReqs()
	if len(extras) == 0 {
		return optimizers.OptimizeSearchParams(ctx, req, queryHook, effectiveSegmentNum, branch0Stage2, dimFunc)
	}
	...
	branchGroup.Go(func() error {
		branchOptimized, err := optimizers.OptimizeSearchParams(
			branchCtx, req, queryHook, effectiveSegmentNum, branch0Stage2, dimFunc)
		if err != nil {
			return err
		}
	...
	for i := range extras {
		...
		branchGroup.Go(func() error {
			...
			branchOptimized, err := optimizers.OptimizeSearchParams(
				branchCtx, branchReq, queryHook, branchSegmentNum, branchStage2, dimFunc)
			if err != nil {
				return err
			}
