...
func TestBuildSharedFilterSearchRequest(t *testing.T) {
	...
	t.Run("the copy is shallow: it carries every field and shares the bytes", func(t *testing.T) {
		src := &internalpb.SubSearchRequest{
			...
			GroupSize: 7, FieldId: 8, IgnoreGrowing: true, AnalyzerName: "an", SearchType: 1, FilterSharingGroup: 9,
		}
...
// Stage 2 tells the query hook the filter selectivity is known
// (WithFilterKey=false). That is only right for a branch two-stage search was
// built for; a co-grouped branch that would not have qualified on its own must
// keep WithFilterKey=true even though the group as a whole is in stage 2.
//
// The extras run concurrently, so the hook records its answer into the
// branch's own search params rather than into a shared slice: that ties each
// WithFilterKey back to the branch it was asked about, whatever order the
// branches finish in.
func TestOptimizeSharedFilterBranchesStage2IsPerBranch(t *testing.T) {
	paramtable.Init()
	setParam(t, paramtable.Get().AutoIndexConfig.Enable.Key, "true")
	setParam(t, paramtable.Get().AutoIndexConfig.TwoStageSearchMinTopk.Key, "100")

	hook := &MockQueryHook{}
	hook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
		params := args.Get(0).(map[string]any)
		params[common.SearchParamKey] = fmt.Sprintf(`{"withFilter":%t}`, params[common.WithFilterKey].(bool))
	}).Return(nil)
	hook.On("CalculateEffectiveSegmentNum", mock.Anything, mock.Anything).Return(5)

	req := &querypb.SearchRequest{
		// Branch 0 is the one two-stage was built for.
		Req: &internalpb.SearchRequest{
			CollectionID: 1, Topk: 200,
			SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 200),
		},
		ExtraFilterSharingReqs: []*internalpb.SubSearchRequest{
			// A BM25 branch: large topk, but SearchType_DEFAULT -- not a pure
			// ANN search, so not what two-stage is for.
			{Topk: 200, SearchType: internalpb.SearchType_DEFAULT, SerializedExprPlan: planWithPredicate(t, 200)},
			// A dense branch that qualifies on its own.
			{Topk: 200, SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 200)},
			// A dense branch whose topk is below the two-stage threshold.
			{Topk: 50, SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 50)},
		},
	}
	dim := func(int64) int64 { return 128 }
	indexType := func(int64) string { return "" }

	sd := &shardDelegator{}
	optimized, err := sd.optimizeSearchParams(context.Background(), req, hook, []int64{1000, 1000, 1000, 1000, 1000}, 5, true, true, dim, indexType)
	require.NoError(t, err)

	withFilterOf := func(plan []byte) string {
		decoded := &planpb.PlanNode{}
		require.NoError(t, proto.Unmarshal(plan, decoded))
		return decoded.GetVectorAnns().GetQueryInfo().GetSearchParams()
	}
	// Branch 0 takes stage-2 semantics; of the extras, BM25 keeps its filter,
	// the qualifying dense branch takes stage-2 semantics, and the small-topk
	// dense branch keeps its filter too.
	got := []string{withFilterOf(optimized.GetReq().GetSerializedExprPlan())}
	for _, sub := range optimized.GetExtraFilterSharingReqs() {
		got = append(got, withFilterOf(sub.GetSerializedExprPlan()))
	}
...
func TestOptimizeSearchParamsFeedsEveryBranchTheRequestInput(t *testing.T) {
	...
	hook := &MockQueryHook{}
	hook.On("CalculateEffectiveSegmentNum", mock.Anything, mock.Anything).Return(1)
	hook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
		params := args.Get(0).(map[string]any)
		mu.Lock()
		defer mu.Unlock()
	...
	req := &querypb.SearchRequest{
		Req: &internalpb.SearchRequest{
			...
			SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 100),
		},
		ExtraFilterSharingReqs: []*internalpb.SubSearchRequest{
			{Topk: 100, SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 100)},
			{Topk: 100, SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 100)},
		},
	}
	dim := func(int64) int64 { return 128 }
	...
	sd := &shardDelegator{}
	optimized, err := sd.optimizeSearchParams(context.Background(), req, hook, []int64{1000}, 1, false, false, dim, indexType)
	require.NoError(t, err)
...
func TestOptimizeSearchParamsStartsBranchZeroWithExtras(t *testing.T) {
	...
	go func() {
		_, err := (&shardDelegator{}).optimizeSearchParams(
			context.Background(), req, hook, []int64{1}, 1, false, false,
			func(int64) int64 { return 128 }, func(int64) string { return "" })
		done <- err
	}()
...
// The effective segment count the hook is given depends on topk, so a branch
// with a different topk gets its own.
func TestOptimizeSearchParamsUsesPerBranchSegmentCount(t *testing.T) {
	paramtable.Init()
	setParam(t, paramtable.Get().AutoIndexConfig.Enable.Key, "true")

	hook := &MockQueryHook{}
	var mu sync.Mutex
	segmentNums := make(map[int64]int)
	hook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
		params := args.Get(0).(map[string]any)
		mu.Lock()
		defer mu.Unlock()
		segmentNums[params[common.TopKKey].(int64)] = params[common.SegmentNumKey].(int)
	}).Return(nil)
	// Only the extra branch's count is computed here; branch 0's is handed in
	// by the caller, exactly as before.
	hook.On("CalculateEffectiveSegmentNum", mock.Anything, int64(100)).Return(1).Once()

	req := &querypb.SearchRequest{
		Req: &internalpb.SearchRequest{
			CollectionID: 1, Topk: 300,
			SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 300),
		},
		ExtraFilterSharingReqs: []*internalpb.SubSearchRequest{
			{Topk: 100, SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 100)},
		},
	...
	sd := &shardDelegator{}
	_, err := sd.optimizeSearchParams(context.Background(), req, hook, []int64{1, 1, 1}, 3, false, false, dim, indexType)
	require.NoError(t, err)
	assert.Equal(t, map[int64]int{300: 3, 100: 1}, segmentNums)
	hook.AssertExpectations(t)
}

func TestBranchQualifiesForTwoStage(t *testing.T) {
	paramtable.Init()
	setParam(t, paramtable.Get().AutoIndexConfig.TwoStageSearchMinTopk.Key, "100")

	assert.True(t, branchQualifiesForTwoStage(100, internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER))
	assert.False(t, branchQualifiesForTwoStage(99, internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER), "topk below threshold")
	assert.False(t, branchQualifiesForTwoStage(1000, internalpb.SearchType_DEFAULT), "a BM25 branch carries SearchType_DEFAULT; not a pure ANN search")
	assert.False(t, branchQualifiesForTwoStage(1000, internalpb.SearchType_PURE_ANN_SEARCH_NO_FILTER), "no filter, nothing to stage")
}
...
func TestOptimizeSearchParamsStoresEachBranchOutputAtItsOwnIndex(t *testing.T) {
	...
	for _, topk := range extraTopks {
		req.ExtraFilterSharingReqs = append(req.ExtraFilterSharingReqs, &internalpb.SubSearchRequest{
			Topk:               topk,
			SearchType:         internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER,
			SerializedExprPlan: planWithPredicate(t, topk),
		})
	}
	dim := func(int64) int64 { return 128 }
	indexType := func(int64) string { return "" }
...
// Knowhere defaults belong to the loaded index on each vector field. A shared
// filter group may span fields with different index types, so resolving every
// branch from branch 0's field would silently apply the wrong defaults.
func TestOptimizeSearchParamsUsesEachBranchIndexType(t *testing.T) {
	paramtable.Init()
	params := paramtable.Get()
	setParam(t, params.AutoIndexConfig.Enable.Key, "false")
	setParam(t, params.KnowhereConfig.Enable.Key, "true")

	indexAKey := params.KnowhereConfig.IndexParam.KeyPrefix + "INDEX_A.search.default_param"
	indexBKey := params.KnowhereConfig.IndexParam.KeyPrefix + "INDEX_B.search.default_param"
	require.NoError(t, params.Save(indexAKey, "1"))
	require.NoError(t, params.Save(indexBKey, "2"))
	t.Cleanup(func() {
		params.Remove(indexAKey)
		params.Remove(indexBKey)
	})

	planWithParams := func(topk int64, raw string) []byte {
		plan := &planpb.PlanNode{}
		require.NoError(t, proto.Unmarshal(planWithPredicate(t, topk), plan))
		plan.GetVectorAnns().GetQueryInfo().SearchParams = raw
		blob, err := proto.Marshal(plan)
		require.NoError(t, err)
		return blob
	}
	...
	req := &querypb.SearchRequest{
		Req: &internalpb.SearchRequest{
			...
			FieldId:            101,
			Topk:               10,
			SerializedExprPlan: planWithParams(10, "{}"),
		},
		ExtraFilterSharingReqs: []*internalpb.SubSearchRequest{
			{FieldId: 102, Topk: 10, SerializedExprPlan: planWithParams(10, "{}")},
			// Request values have precedence over the selected index's default.
			{FieldId: 103, Topk: 10, SerializedExprPlan: planWithParams(10, `{"default_param":9}`)},
		},
	}
	indexType := func(fieldID int64) string {
		return map[int64]string{101: "INDEX_A", 102: "INDEX_B", 103: "INDEX_A"}[fieldID]
	}

	sd := &shardDelegator{}
	optimized, err := sd.optimizeSearchParams(
		context.Background(), req, nil, nil, 0, false, false,
		func(int64) int64 { return 0 }, indexType)
	require.NoError(t, err)

	searchParams := func(blob []byte) string {
		plan := &planpb.PlanNode{}
		require.NoError(t, proto.Unmarshal(blob, plan))
		return plan.GetVectorAnns().GetQueryInfo().GetSearchParams()
	}
	assert.JSONEq(t, `{"default_param":1}`, searchParams(optimized.GetReq().GetSerializedExprPlan()))
	assert.JSONEq(t, `{"default_param":2}`, searchParams(optimized.GetExtraFilterSharingReqs()[0].GetSerializedExprPlan()))
	assert.JSONEq(t, `{"default_param":9}`, searchParams(optimized.GetExtraFilterSharingReqs()[1].GetSerializedExprPlan()))
}
