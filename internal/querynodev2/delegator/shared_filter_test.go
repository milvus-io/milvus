...
func TestOptimizeSharedFilterBranchesStage2IsPerBranch(t *testing.T) {
	...
	// The extras run concurrently, so what the hook saw is keyed by the branch
	// it was asked about rather than by call order; every branch carries a
	// topk of its own.
	hook := &MockQueryHook{}
	var mu sync.Mutex
	withFilter := map[int64]bool{}
	hook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
		params := args.Get(0).(map[string]any)
		mu.Lock()
		defer mu.Unlock()
		withFilter[params[common.TopKKey].(int64)] = params[common.WithFilterKey].(bool)
	}).Return(nil)
	...
	req := &querypb.SearchRequest{
		...
		Req: &internalpb.SearchRequest{
			CollectionID: 1, Topk: 400,
			SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 400),
		},
		ExtraFilterSharingReqs: []*internalpb.SubSearchRequest{
			...
			{Topk: 300, SearchType: internalpb.SearchType_PURE_ANN_SEARCH_WITH_FILTER, SerializedExprPlan: planWithPredicate(t, 300)},
	...
	sd := &shardDelegator{}
	_, err := sd.optimizeSearchParams(context.Background(), req, hook, []int64{1000, 1000, 1000, 1000, 1000}, 5, true, true, dim)
	require.NoError(t, err)

	// Branch 0 and the dense branch that qualifies on its own take stage-2
	// semantics; BM25 keeps its filter, and so does the small-topk dense
	// branch.
	assert.Equal(t, map[int64]bool{400: false, 200: true, 300: false, 50: true}, withFilter)
}
...
func TestOptimizeSearchParamsFeedsEveryBranchTheRequestInput(t *testing.T) {
	...
	sd := &shardDelegator{}
	optimized, err := sd.optimizeSearchParams(context.Background(), req, hook, []int64{1000}, 1, false, false, dim)
	require.NoError(t, err)
...
func TestOptimizeSearchParamsUsesPerBranchSegmentCount(t *testing.T) {
	...
	sd := &shardDelegator{}
	_, err := sd.optimizeSearchParams(context.Background(), req, hook, []int64{1, 1, 1}, 3, false, false, dim)
	require.NoError(t, err)
...
func TestOptimizeSearchParamsStoresEachBranchOutputAtItsOwnIndex(t *testing.T) {
	...
	// Every call gets its own params map, so nothing here is shared.
	hook := &MockQueryHook{}
	hook.On("CalculateEffectiveSegmentNum", mock.Anything, mock.Anything).Return(1)
	hook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
		params := args.Get(0).(map[string]any)
