...
// up the rows once they are visible.
//
// AckedRowCount, NOT RowNum: RowNum binds to segcore get_real_count(), which
// subtracts deleted rows (and runs a count query once the segment has any
// delete). After the first delete/upsert it would sit permanently below the
// insert-derived targetOffset, leaving the source Pending forever. The acked
// count is the same monotonic insert prefix targetOffset lives in.
func growingSourceCurrentOffset(segment segments.Segment) int64 {
	if segment == nil {
		return 0
	}
	return segment.AckedRowCount()
}
