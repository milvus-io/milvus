...
func (sd *shardDelegator) ReleaseSegments(ctx context.Context, req *querypb.ReleaseSegmentsRequest, force bool) error {
	...
	// Holder-side veto: the release decision was made by QueryCoord from a leader view it pulled
	// earlier; between that heartbeat and now the delegator may have been synced to a target version
	// that pulls the segment back into its readable snapshot. Releasing it then drops loadedRatio
	// below 1.0 and makes the shard unserviceable, so the holder refuses; the checker retries once
	// the segment has genuinely left the readable set. force releases (collection drop, node
	// cleanup) bypass this -- there is no snapshot left to protect.
	if !force && req.GetScope() != querypb.DataScope_Streaming {
		...
		if len(stillServing) > 0 {
			log.Info("refusing to release segments the delegator still serves",
				zap.Int64s("stillServing", stillServing))
