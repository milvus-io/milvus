...
func (sd *shardDelegator) GetHighlight(ctx context.Context, req *querypb.GetHighlightRequest) ([]*querypb.HighlightResult, error) {
	...
	for _, task := range req.GetTasks() {
		...
		for _, text := range task.Texts {
			log.Info("test text--", zap.String("text", text))
		}
		log.Info("test--", zap.Int64("num", task.SearchTextNum+int64(len(task.Queries))))
		...
		for i := range len(topks) {
			...
			for j := 0; j < int(topks[i]); j++ {
				...
				frags := fetchFragmentsFromOffsets(task.Texts[corpusStartOffset+corpusIdx], spans, task.Options.FragmentSize, task.Options.NumOfFragments)
