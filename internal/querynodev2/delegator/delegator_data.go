...
func (sd *shardDelegator) ReleaseSegments(ctx context.Context, req *querypb.ReleaseSegmentsRequest, force bool) error {
	...
	if len(growing) > 0 && sd.useGrowingSourceFlush() && !force && req.GetCheckpoint().GetTimestamp() != typeutil.MaxTimestamp {
		checkpointTs := req.GetCheckpoint().GetTimestamp()
		unprepared := lo.Filter(growing, func(entry SegmentEntry, _ int) bool {
			return sd.growingSourceProvider == nil || !sd.growingSourceProvider.IsReleaseAllowed(entry.SegmentID, checkpointTs)
		})
		if len(unprepared) == 0 {
			log.Info("allow release growing-source segments after release handoff prepared",
			...
			log.WithRateGroup(fmt.Sprintf("delegator.release.text.%s", sd.vchannelName), 1, 60).
				RatedWarn(30, "reject release growing-source segment, waiting for WAL flusher drain",
			...
			return errors.Wrap(merr.ErrServiceTooManyRequests,
				"release growing-source segment requires WAL fence/drain before source release; checker will retry")
		}
	} else if len(growing) > 0 && sd.useGrowingSourceFlush() && !force {
		log.Info("allow release growing-source segments for dropped channel",
			zap.Int64s("segmentIDs", lo.Map(growing, func(entry SegmentEntry, _ int) int64 {
				return entry.SegmentID
			})))
	}
