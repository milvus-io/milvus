...
func (sd *shardDelegator) loadStreamDelete(ctx context.Context,
	...
	for round := 0; ; round++ {
		...
		if round >= maxLockFreeCatchUpRounds {
			log.Info("loadStreamDelete catch-up exceeded expected rounds; applying final locked barrier",
				zap.Int("round", round),
				zap.Int("remainingDeleteRecords", remaining))
			for i, info := range infos {
				if len(pending[i]) == 0 {
					continue
				}
				...
				log.Info("forward delete to worker (phase 3: final locked catch-up)...",
					zap.String("channel", info.InsertChannel),
					zap.Int64("segmentID", info.GetSegmentID()),
					zap.Int("round", round),
					zap.Int64("tsHitDeleteRowNum", tsHit),
		...
		for i, info := range infos {
			...
			log.Info("forward delete to worker (phase 3: lock-free catch-up)...",
				zap.String("channel", info.InsertChannel),
				zap.Int64("segmentID", info.GetSegmentID()),
				zap.Int("round", round),
				zap.Int64("tsHitDeleteRowNum", tsHit),
