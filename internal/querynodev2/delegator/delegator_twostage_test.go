...
func (m *MockQueryHook) CalculateEffectiveSegmentNum(rowCounts []int64, topk int64) int {
	args := m.Called(rowCounts, topk)
	return args.Int(0)
}
...
func (s *TwoStageSearchSuite) TestTwoStageSearch() {
	...
	s.Run("optimizer_path_exercised", func() {
		defer func() {
			s.workerManager.ExpectedCalls = nil
			s.delegator.queryHook = nil
		}()

		// Enable AutoIndex so OptimizeSearchParams doesn't early-return
		...
		// Wire up a real MockQueryHook on the delegator
		mockHook := &MockQueryHook{}
		hookCalled := false
		mockHook.On("CalculateEffectiveSegmentNum", mock.Anything, mock.Anything).Return(1)
		mockHook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
			hookCalled = true
			params := args.Get(0).(map[string]any)
			// In the second stage (isSecondStageSearch=true), WithFilterKey must be false
			s.False(params[common.WithFilterKey].(bool),
				"WithFilterKey should be false in second-stage search")
		}).Return(nil)
		s.delegator.queryHook = mockHook
		...
		// Build a real serialized plan with VectorAnns so the optimizer doesn't skip it
		plan := &planpb.PlanNode{
			Node: &planpb.PlanNode_VectorAnns{
				VectorAnns: &planpb.VectorANNS{
					Predicates: &planpb.Expr{}, // non-nil → withFilter=true
	...
	s.Run("optimizer_error_propagated", func() {
		defer func() {
			s.workerManager.ExpectedCalls = nil
			s.delegator.queryHook = nil
		}()
		...
		mockHook := &MockQueryHook{}
		mockHook.On("CalculateEffectiveSegmentNum", mock.Anything, mock.Anything).Return(1)
		mockHook.On("Run", mock.Anything).Return(errors.New("optimizer failed"))
		s.delegator.queryHook = mockHook
		...
		s.False(fallback)
		s.Nil(results)
		s.Contains(err.Error(), "optimizer failed")
	})
...
func (s *TwoStageSearchSuite) TestShouldUseTwoStageSearchEdgeCases() {
	...
	s.Run("zero_segments", func() {
		...
		result := optimizers.ShouldUseTwoStageSearch(req, 0)
		s.False(result, "should return false when both segments and topk are below threshold")
	})
