...
func (s *DistributionSuite) TestMarkOfflineSegments() {
	...
	for _, tc := range cases {
		s.Run(tc.tag, func() {
			...
			s.dist.SyncReadableChannelView(&querypb.SyncAction{
				TargetVersion:   1000,
				SealedInTarget:  sealedSegmentID,
				DroppedInTarget: nil,
			}, nil)
...
func TestNewChannelQueryView(t *testing.T) {
	...
	view := NewChannelQueryView(growings, sealeds, partitions, version)
	assert.NotNil(t, view)
	assert.Equal(t, growings, view.growingSegments)
	assert.Equal(t, sealeds, view.sealedSegments)
...
func TestDistribution_SyncReadableChannelView(t *testing.T) {
	...
	assert.Equal(t, action.GetGrowingInTarget(), dist.queryView.growingSegments)
	assert.Equal(t, action.GetSealedInTarget(), dist.queryView.sealedSegments)
...
func TestDistribution_MarkOfflineSegments(t *testing.T) {
	channelName := "test_channel"
	view := NewChannelQueryView([]int64{}, []int64{1, 2}, []int64{}, 0)
	dist := NewDistribution(channelName, view)
