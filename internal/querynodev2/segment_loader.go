...
func (loader *segmentLoader) loadSegment(collectionID UniqueID,
	partitionID UniqueID,
	segmentIDs []UniqueID,
	fieldIDs []int64) error {
	if len(fieldIDs) == 0 {
		var err error
		fieldIDs, err = loader.historicalReplica.getFieldIDsByCollectionID(collectionID)
		if err != nil {
			return err
		}
	}
	for _, segmentID := range segmentIDs {
		...
		err = loader.loadSegmentInternal(collectionID, segment, fieldIDs)
		if err != nil {
...
func (loader *segmentLoader) loadSegmentInternal(collectionID UniqueID, segment *Segment, fieldIDs []int64) error {
	// create segment
	statesResp, err := loader.GetSegmentStates(segment.segmentID)
	if err != nil {
		return err
	}
	...
	insertBinlogPaths, srcFieldIDs, err := loader.getInsertBinlogPaths(segment.segmentID)
	if err != nil {
		return err
	}
	...
	fieldIDs = loader.filterOutVectorFields(fieldIDs, loadIndexFieldIDs)

	//log.Debug("srcFieldIDs in internal:", srcFieldIDs)
	//log.Debug("dstFieldIDs in internal:", fieldIDs)
	targetFields, err := loader.checkTargetFields(insertBinlogPaths, srcFieldIDs, fieldIDs)
	if err != nil {
		return err
	}
	log.Debug("loading insert...")
	err = loader.loadSegmentFieldsData(segment, targetFields)
	if err != nil {
		return err
	}
