...
func TestValidateImportFilePaths(t *testing.T) {
	...
	tests := []struct {
		...
		// localStorage.path=/milvus/data this reads the insert_log dir while
		// comparing as "/data/insert_log/...".
		{"local root, relative path refused", "/milvus/data", "data/insert_log/1/2/3/100/4", nil, true, "local"},
		...
		{"absolute root, doubled slash", "/var/lib/milvus/data", "//var/lib/milvus/data/insert_log/1/2/3/100/4", nil, true, ""},
...
func TestValidateImportFilePaths_SymlinkedInternalDir(t *testing.T) {
	...
	// level below it: <root>/insert_log/449 -> <base>/nvme-insert-log.
	insertLogDir := filepath.Join(root, common.SegmentInsertLogPath)
	require.NoError(t, os.MkdirAll(insertLogDir, 0o755))
	sub := filepath.Join(base, "nvme-insert-log")
	...
	require.NoError(t, os.Symlink(sub, filepath.Join(insertLogDir, "449")))
	...
	tests := []struct {
		...
		{"symlink below the segment", filepath.Join(root, common.SegmentInsertLogPath, "449", "metadata", "12.json"), true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cm := mocks2.NewChunkManager(t)
...
func TestValidateImportFilePaths_LocalAliases(t *testing.T) {
	...
	base := t.TempDir()
	root := filepath.Join(base, "data")
	internalFile := filepath.Join(root, common.SegmentInsertLogPath, "449", "metadata", "12.json")
	require.NoError(t, os.MkdirAll(filepath.Dir(internalFile), 0o755))
	require.NoError(t, os.WriteFile(internalFile, []byte("{}"), 0o600))

	staging := filepath.Join(base, "staging")
	require.NoError(t, os.MkdirAll(staging, 0o755))
	link := filepath.Join(staging, "a.json")
	require.NoError(t, os.Symlink(internalFile, link))
	...
	tests := []struct {
		...
		{"proc self root alias", root, "/proc/self/root" + internalFile, true},
		{"staging symlink into the root", root, link, true},
		{"symlinked root, resolved path", rootLink, internalFile, true},
