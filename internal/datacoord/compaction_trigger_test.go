...
func Test_compactionTrigger_shouldDoSingleCompaction(t *testing.T) {
	...
	trigger := newCompactionTrigger(&meta{
		indexMeta:  indexMeta,
		channelCPs: newChannelCps(),
	}, &compactionInspector{}, mock0Allocator, newMockHandler())
	...
	// Version rebuilds now belong exclusively to MigrationCompactionChecker.
	indexMeta.updateSegmentIndex(&model.SegmentIndex{
		CollectionID:        2,
		SegmentID:           1,
		IndexID:             101,
	...
	info4 := &SegmentInfo{SegmentInfo: &datapb.SegmentInfo{
		ID: 1, CollectionID: 2, NumOfRows: 10000, Binlogs: binlogs2,
	}}
	for _, item := range []*paramtable.ParamItem{
		&Params.DataCoordCfg.AutoUpgradeSegmentIndex,
		&Params.DataCoordCfg.ForceRebuildSegmentIndex,
		&Params.DataCoordCfg.ForceRebuildScalarSegmentIndex,
	} {
		Params.Save(item.Key, "true")
		t.Cleanup(func() { Params.Reset(item.Key) })
	}
	Params.Save(Params.DataCoordCfg.TargetVecIndexVersion.Key, "5")
	Params.Save(Params.DataCoordCfg.TargetScalarIndexVersion.Key, "6")
	t.Cleanup(func() {
		Params.Reset(Params.DataCoordCfg.TargetVecIndexVersion.Key)
		Params.Reset(Params.DataCoordCfg.TargetScalarIndexVersion.Key)
	})
	assert.False(t, trigger.ShouldDoSingleCompaction(info4, &compactTime{expireTime: 300}))
...
func Test_compactionTrigger_new(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := newCompactionTrigger(tt.args.meta, tt.args.inspector, tt.args.allocator, newMockHandler())
...
func TestCompactionTriggerKeepsMixedSchemaVersionSegments(t *testing.T) {
	...
	trigger := newCompactionTrigger(mt, inspector, newMock0Allocator(t), newMockHandlerWithMeta(mt))
