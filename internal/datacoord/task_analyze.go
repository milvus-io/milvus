...
func (at *analyzeTask) CreateTaskOnWorker(nodeID int64, cluster session.Cluster) {
	...
	// The scheduler's task lock serializes callbacks, but the inspector may
	// have enqueued this snapshot after the previous wrapper already finished.
	task := at.meta.analyzeMeta.GetTask(at.GetTaskID())
	if task == nil || task.GetState() != indexpb.JobState_JobStateInit {
		log.Info(ctx, "analyze task is no longer dispatchable in meta")
		at.SetState(indexpb.JobState_JobStateNone, "analyze task is no longer dispatchable in meta")
		return
	}
	if at.handler == nil {
...
func (at *analyzeTask) QueryTaskOnWorker(cluster session.Cluster) {
	// Retire a stale wrapper without querying an already cleaned-up worker
	// result or changing the authoritative terminal state to Retry.
	task := at.meta.analyzeMeta.GetTask(at.GetTaskID())
	if task == nil || task.GetState() != indexpb.JobState_JobStateInProgress {
		at.SetState(indexpb.JobState_JobStateNone, "analyze task is no longer running in meta")
		return
	}
	log := mlog.With(
