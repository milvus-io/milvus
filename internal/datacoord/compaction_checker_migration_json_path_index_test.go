...
func (s *JSONPathIndexMigrationPolicySuite) SetupTest() {
	...
	s.saveParam(&paramtable.Get().DataCoordCfg.AutoUpgradeSegmentIndex, "false")
}
...
func (s *JSONPathIndexMigrationPolicySuite) TestEnableAndRegistration() {
	...
	s.True(s.policy.Enable(), "the explicit force rebuild remains enabled below the JSON migration floor")
...
func (s *JSONPathIndexMigrationPolicySuite) TestClusterVersionGates() {
	// Match real upgraded sessions: build default V5, reader/writer maximum V6.
	reader := newJSONPathMigrationQueryNode(1, common.CurrentScalarIndexEngineVersion, 1)
	reader.ScalarIndexEngineVersion.MaximumIndexVersion = common.MaximumScalarIndexEngineVersion
	writer := newJSONPathMigrationNode(2, common.CurrentScalarIndexEngineVersion)
	writer.ScalarIndexEngineVersion.MaximumIndexVersion = common.MaximumScalarIndexEngineVersion
	s.version.AddNode(reader)
	s.gate.AddDataNode(writer)
	s.saveParam(&Params.DataCoordCfg.JSONStatsFormatVersion, "auto")
	s.True(s.gate.SupportsScalarIndexVersion(6), "capability is not the write target")
	s.Equal(int32(5), s.version.ResolveScalarIndexVersion())
	s.False(s.policy.Enable(), "neither migration starts before the cluster gate")

	// The policies remain registered while disabled and observe refreshed targets.
	inspector := NewMockCompactionInspector(s.T())
	manager := NewCompactionTriggerManager(s.mockAlloc, s.handler, inspector, s.policy.meta, s.version)
	registered := manager.policies[MigrationCompactionTicker]
	s.Require().NotNil(registered)
	s.False(registered.Enable())
	Params.Save(Params.DataCoordCfg.TargetScalarIndexVersion.Key, "6")
	s.Equal(int32(6), s.version.ResolveScalarIndexVersion())
	s.True(registered.Enable(), "no coordinator restart needed after gate activation")
	targets, ok := resolveSafeSegmentRebuildTargets(s.version)
	s.Require().True(ok)
	s.True(s.policy.jsonPathIndexMigrationEnabled(targets))
	...
	// Pins stay independent; turning off new Path builds does not disable stats.
	Params.Save(Params.DataCoordCfg.TargetScalarIndexVersion.Key, "5")
	s.Equal(int32(5), s.version.ResolveScalarIndexVersion())
	s.True(registered.Enable())
...
func (s *JSONPathIndexMigrationPolicySuite) TestSelectsOnlyLegacyTypedPathIndexes() {
	...
	// Verify the production guards directly; the selected views above cover
	// the combined migration decision.
	presenceVersion := common.MinScalarIndexVersionForJsonPathPresence
	targets := safeSegmentRebuildTargets{scalarVersion: presenceVersion, vectorVersion: 1}
	activeIDs := getActiveIndexIDs(s.policy.meta.indexMeta, collection.ID)
	for _, segmentID := range []int64{209, 211, 214, 221, 222} {
		canRebuild, _ := checkSegmentIndexRebuild(
			s.policy.meta.indexMeta, segmentID, activeIDs, nil, typedIDs, nil, targets, indexRebuildOptions{})
		s.False(canRebuild, "active pending index builds must block segment rebuilds")
	}
	_, needsRebuild := checkSegmentIndexRebuild(s.policy.meta.indexMeta, 212, activeIDs, nil, typedIDs,
		nil, targets, indexRebuildOptions{})
	s.False(needsRebuild)
	canRebuild, _ := checkSegmentIndexRebuild(
		s.policy.meta.indexMeta, 216, activeIDs, nil, typedIDs, segments[216].GetTextStatsLogs(), targets, indexRebuildOptions{})
	s.False(canRebuild, "future text index artifacts must block segment rebuilds")
...
func (s *JSONPathIndexMigrationPolicySuite) TestCombinesPathIndexAndStatsMigrationIntoOneCompaction() {
	s.setQueryNodeVersions(common.MinScalarIndexVersionForJsonPathPresence, 1)
	s.gate.StartupDataNodes(map[string]*sessionutil.Session{
		"dn-1": newJSONPathMigrationNode(2, common.MinScalarIndexVersionForJsonPathPresence),
	})
	...
	collection := &collectionInfo{ID: collectionID, Schema: newJSONPathMigrationSchema()}
	segment := newJSONPathMigrationSegment(collectionID, segmentID)
	segment.JsonKeyStats = map[int64]*datapb.JsonKeyStats{
		100: {JsonKeyStatsDataFormat: common.JSONStatsDataFormatV3},
	}
	s.setPolicyMeta(
		collection,
		map[UniqueID]*SegmentInfo{segmentID: segment},
		[]*model.Index{newJSONPathMigrationIndex(collectionID, 100, typedIndexID, "DOUBLE")},
		[]*model.SegmentIndex{
			newJSONPathMigrationSegmentIndex(collectionID, segmentID, typedIndexID,
				common.MinScalarIndexVersionForJsonPathPresence-1),
		},
	)
	...
	events, err := s.policy.Trigger(context.Background())
	s.NoError(err)
	s.Equal([]int64{segmentID}, viewSegmentIDs(events[TriggerTypeSingle]))
	s.Equal(1, s.policy.currentCount, "one segment with two migration reasons must consume one rewrite token")
}
...
func (s *JSONPathIndexMigrationPolicySuite) TestFutureIndexUsesLocalWritableTarget() {
	...
	canRebuild, _ := checkSegmentIndexRebuild(
		...
		getActiveIndexIDs(s.policy.meta.indexMeta, collectionID), nil, nil, nil,
		safeSegmentRebuildTargets{scalarVersion: common.MaximumScalarIndexEngineVersion, vectorVersion: 1}, indexRebuildOptions{})
	s.False(canRebuild)
}
...
func (s *JSONPathIndexMigrationPolicySuite) TestFutureVectorIndexUsesDataNodeWriterCeilingAndSchemaType() {
	...
	canRebuild, _ := checkSegmentIndexRebuild(
		s.policy.meta.indexMeta, 201,
		getActiveIndexIDs(s.policy.meta.indexMeta, collectionID), vectorIDs, nil, nil,
		safeSegmentRebuildTargets{scalarVersion: presenceVersion, vectorVersion: writerCeiling}, indexRebuildOptions{})
	s.False(canRebuild)
}
...
func (s *JSONPathIndexMigrationPolicySuite) TestBlockedCollectionAndExternalCollection() {
	...
	views, err := s.policy.triggerOneCollection(
		...
		collectionID,
		20,
		common.JSONStatsDataFormatV3,
		safeSegmentRebuildTargets{
			scalarVersion: common.MaximumScalarIndexEngineVersion,
			vectorVersion: 1,
		},
	)
	s.NoError(err)
