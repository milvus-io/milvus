...
func (s *JSONPathIndexMigrationPolicySuite) TestSelectsOnlyLegacyTypedPathIndexes() {
	...
	for _, segmentID := range []int64{209, 211, 214, 221, 222} {
		s.False(canRebuildSegmentArtifacts(
			s.policy.meta.indexMeta, segmentID, nil, nil, presenceVersion, 1),
			"active pending index builds must block segment rebuilds")
	}
	s.False(hasLegacyTypedJSONPathIndexArtifact(s.policy.meta.indexMeta, 212, typedIDs, presenceVersion))
	s.False(canRebuildSegmentArtifacts(
		s.policy.meta.indexMeta, 216, nil, segments[216].GetTextStatsLogs(), presenceVersion, 1),
		"future text index artifacts must block segment rebuilds")
...
func (s *JSONPathIndexMigrationPolicySuite) TestFutureIndexUsesLocalWritableTarget() {
	...
	s.False(canRebuildSegmentArtifacts(
		s.policy.meta.indexMeta, 201, nil, nil, common.MaximumScalarIndexEngineVersion, 1))
}
...
func (s *JSONPathIndexMigrationPolicySuite) TestFutureVectorIndexUsesDataNodeWriterCeilingAndSchemaType() {
	...
	s.False(canRebuildSegmentArtifacts(
		s.policy.meta.indexMeta, 201, vectorIDs, nil, presenceVersion, writerCeiling))
}
