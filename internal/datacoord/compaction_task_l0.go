...
func (t *l0CompactionTask) QueryTaskOnWorker(cluster session.Cluster) {
	...
	if err != nil || result == nil {
		if errors.Is(err, merr.ErrCompactionResultNotFound) {
			// As in mix compaction, the fresh plan ID keeps replacement
			// execution independent of the old worker's cleanup.
			log.Warn(t.ctx, "l0CompactionTask result unavailable, abandoning attempt for replan", mlog.Err(err))
