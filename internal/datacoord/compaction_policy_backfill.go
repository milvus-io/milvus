			...
			for _, segment := range group.segments {
				...
				if segmentSchemaVersion < collectionSchemaVersion {
					...
					if doPhysicalBackfill {
						...
						oldSegment := policy.meta.segments.GetSegment(segmentID)
						if oldSegment == nil {
							return nil, merr.WrapErrSegmentNotFound(segmentID)
						}
						cloned := oldSegment.Clone()
						cloned.SchemaVersion = collectionSchemaVersion
						policy.meta.catalog.AlterSegments(ctx, []*datapb.SegmentInfo{cloned.SegmentInfo}, metastore.BinlogsIncrement{
							Segment: cloned.SegmentInfo,
						})
						policy.meta.segments.SetSegment(segmentID, cloned)
