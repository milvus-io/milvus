...
// SupportsScalarIndexVersion requires every QueryNode reader and discovered
// DataNode writer to advertise a complete range containing targetVersion.
// Bound workers are trusted without a writer session; missing or malformed
// QueryNode capabilities still fail closed in either mode.
func (m *versionManagerImpl) SupportsScalarIndexVersion(targetVersion int32) bool {
	m.mu.Lock()
	defer m.mu.Unlock()

	if targetVersion <= 0 || len(m.scalarIndexVersions) == 0 {
		return false
	}
	...
	if len(m.dataNodeScalarIndexVersions) == 0 {
		return false
	}
