...
func TestMeta_alterMetaStore(t *testing.T) {
	...
	newSeg := NewSegmentInfo(newSegInfo)
...
func TestMeta_GetCompleteCompactionMeta(t *testing.T) {
	...
	canCompaction := func(segment *datapb.CompactionSegmentBinlogs) bool { return true }
	beforeCompact, afterCompact, newSegment, err := m.GetCompleteCompactionMeta(inCompactionLogs, inCompactionResult, canCompaction)
	assert.NoError(t, err)
