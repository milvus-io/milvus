...
func TestMeta_Basic(t *testing.T) {
	...
	t.Run("Test segment with kv fails", func(t *testing.T) {
		...
		metakv.EXPECT().WalkAtDirectory(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		...
		metakv2.EXPECT().WalkAtDirectory(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		metakv2.EXPECT().LoadAtDirectory(mock.Anything).Return(nil, nil, nil).Maybe()
