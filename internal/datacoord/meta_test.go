...
func TestMeta_SetStatesBatch(t *testing.T) {
	newMetaWithSegments := func(catalog metastore.DataCoordCatalog, segs ...*datapb.SegmentInfo) *meta {
		m := &meta{
			catalog: catalog,
			segments: &SegmentsInfo{
		...
		for _, s := range segs {
			m.segments.SetSegment(s.GetID(), &SegmentInfo{SegmentInfo: s})
		}
		return m
	}
	...
	t.Run("empty_input_short_circuits", func(t *testing.T) {
		...
		m := newMetaWithSegments(catalog)
		assert.NoError(t, m.SetStatesBatch(context.TODO(), nil, commonpb.SegmentState_Sealed))
	})

	t.Run("happy_path_batched_alter", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		var captured []*datapb.SegmentInfo
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).Run(func(_ context.Context, infos []*datapb.SegmentInfo, _ ...metastore.BinlogsIncrement) {
			captured = infos
		}).Return(nil).Once()

		m := newMetaWithSegments(catalog,
			&datapb.SegmentInfo{ID: 1, State: commonpb.SegmentState_Growing, Level: datapb.SegmentLevel_L1},
			&datapb.SegmentInfo{ID: 2, State: commonpb.SegmentState_Growing, Level: datapb.SegmentLevel_L1},
		)

		err := m.SetStatesBatch(context.TODO(), []UniqueID{1, 2}, commonpb.SegmentState_Sealed)
		assert.NoError(t, err)
		assert.Len(t, captured, 2)
		assert.Equal(t, commonpb.SegmentState_Sealed, m.segments.GetSegment(1).GetState())
		assert.Equal(t, commonpb.SegmentState_Sealed, m.segments.GetSegment(2).GetState())
	})

	t.Run("missing_segment_non_dropped_errors", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		m := newMetaWithSegments(catalog,
			&datapb.SegmentInfo{ID: 1, State: commonpb.SegmentState_Growing, Level: datapb.SegmentLevel_L1},
		)
		err := m.SetStatesBatch(context.TODO(), []UniqueID{1, 999}, commonpb.SegmentState_Sealed)
		assert.Error(t, err)
		// state must not have been flipped because the batch aborted before AlterSegments
		assert.Equal(t, commonpb.SegmentState_Growing, m.segments.GetSegment(1).GetState())
	})

	t.Run("missing_segment_dropped_is_idempotent", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).Return(nil).Once()
		m := newMetaWithSegments(catalog,
			&datapb.SegmentInfo{ID: 1, State: commonpb.SegmentState_Flushed, Level: datapb.SegmentLevel_L1},
		)
		err := m.SetStatesBatch(context.TODO(), []UniqueID{1, 999}, commonpb.SegmentState_Dropped)
		assert.NoError(t, err)
		assert.Equal(t, commonpb.SegmentState_Dropped, m.segments.GetSegment(1).GetState())
	})

	t.Run("all_skipped_no_catalog_call", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		m := newMetaWithSegments(catalog,
			&datapb.SegmentInfo{ID: 1, State: commonpb.SegmentState_Dropped, Level: datapb.SegmentLevel_L1},
		)
		// unhealthy segments are filtered before catalog write
		assert.NoError(t, m.SetStatesBatch(context.TODO(), []UniqueID{1}, commonpb.SegmentState_Dropped))
	})

	t.Run("unhealthy_segment_filtered_out", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		var captured []*datapb.SegmentInfo
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).Run(func(_ context.Context, infos []*datapb.SegmentInfo, _ ...metastore.BinlogsIncrement) {
			captured = infos
		...
		m := newMetaWithSegments(catalog,
			&datapb.SegmentInfo{ID: 1, State: commonpb.SegmentState_Growing, Level: datapb.SegmentLevel_L1},
			&datapb.SegmentInfo{ID: 2, State: commonpb.SegmentState_Dropped, Level: datapb.SegmentLevel_L1},
		)

		err := m.SetStatesBatch(context.TODO(), []UniqueID{1, 2}, commonpb.SegmentState_Sealed)
		assert.NoError(t, err)
		assert.Len(t, captured, 1)
		assert.EqualValues(t, 1, captured[0].GetID())
		assert.Equal(t, commonpb.SegmentState_Sealed, m.segments.GetSegment(1).GetState())
		// Dropped segment is unhealthy and must not be flipped back to Sealed
		assert.Equal(t, commonpb.SegmentState_Dropped, m.segments.GetSegment(2).GetState())
	})

	t.Run("catalog_failure_propagates", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).Return(errors.New("boom")).Once()
		m := newMetaWithSegments(catalog,
			&datapb.SegmentInfo{ID: 1, State: commonpb.SegmentState_Growing, Level: datapb.SegmentLevel_L1},
		)
		err := m.SetStatesBatch(context.TODO(), []UniqueID{1}, commonpb.SegmentState_Sealed)
		assert.Error(t, err)
		// in-memory state must remain Growing when catalog write fails
		assert.Equal(t, commonpb.SegmentState_Growing, m.segments.GetSegment(1).GetState())
	})
}

func TestMeta_commitCompactionMetaMutation(t *testing.T) {
	newMeta := func(catalog metastore.DataCoordCatalog) *meta {
		return &meta{catalog: catalog, ctx: context.TODO()}
	}
	task := &datapb.CompactionTask{PlanID: 1, CollectionID: 10}
	to := []*datapb.SegmentInfo{{ID: 100, State: commonpb.SegmentState_Flushed}}
	from := []*datapb.SegmentInfo{{ID: 1, State: commonpb.SegmentState_Dropped}}

	t.Run("ok_orders_compactTo_before_compactFrom", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		var order []int64
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).Run(func(_ context.Context, infos []*datapb.SegmentInfo, _ ...metastore.BinlogsIncrement) {
			if len(infos) > 0 {
				order = append(order, infos[0].GetID())
			}
		}).Return(nil).Twice()

		err := newMeta(catalog).commitCompactionMetaMutation(task, to, from, nil)
		assert.NoError(t, err)
		// compactTo (ID=100) MUST be persisted before compactFrom (ID=1) is flipped to Dropped,
		// otherwise a crash between the two writes leaves originals dropped with no successor.
		assert.Equal(t, []int64{100, 1}, order)
	})

	t.Run("compactTo_failure_short_circuits", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).Return(errors.New("boom")).Once()
		err := newMeta(catalog).commitCompactionMetaMutation(task, to, from, nil)
		assert.Error(t, err)
		// Mock asserts no more than 1 call — if the helper accidentally swallowed the error
		// and called AlterSegments again, the mock would fail at suite teardown.
	})

	t.Run("compactFrom_failure_propagates", func(t *testing.T) {
		catalog := mocks2.NewDataCoordCatalog(t)
		var calls int
		catalog.EXPECT().AlterSegments(mock.Anything, mock.Anything).RunAndReturn(func(_ context.Context, _ []*datapb.SegmentInfo, _ ...metastore.BinlogsIncrement) error {
			calls++
			if calls == 2 {
