...
func (csm *compactionTaskMeta) GetCompactionTasks() map[int64][]*datapb.CompactionTask {
	...
	for triggerID, tasks := range csm.clusteringCompactionTasks {
...
func (csm *compactionTaskMeta) GetCompactionTasksByCollection(collectionID int64) map[int64][]*datapb.CompactionTask {
	...
	defer csm.RUnlock()
	res := make(map[int64][]*datapb.CompactionTask, 0)
	for _, tasks := range csm.clusteringCompactionTasks {
		for _, task := range tasks {
...
func (csm *compactionTaskMeta) GetCompactionTasksByTriggerID(triggerID int64) []*datapb.CompactionTask {
	...
	tasks, triggerIDExist := csm.clusteringCompactionTasks[triggerID]
	if triggerIDExist {
		for _, task := range tasks {
...
func (csm *compactionTaskMeta) saveCompactionTaskMemory(task *datapb.CompactionTask) error {
	_, triggerIDExist := csm.clusteringCompactionTasks[task.TriggerID]
	if !triggerIDExist {
		csm.clusteringCompactionTasks[task.TriggerID] = make(map[int64]*datapb.CompactionTask, 0)
	}
	csm.clusteringCompactionTasks[task.TriggerID][task.PlanID] = task
	return nil
}
...
func (csm *compactionTaskMeta) DropCompactionTask(task *datapb.CompactionTask) error {
	...
	_, triggerIDExist := csm.clusteringCompactionTasks[task.TriggerID]
	if triggerIDExist {
		delete(csm.clusteringCompactionTasks[task.TriggerID], task.PlanID)
	}
	if len(csm.clusteringCompactionTasks[task.TriggerID]) == 0 {
		delete(csm.clusteringCompactionTasks, task.TriggerID)
	}
	return nil
}
