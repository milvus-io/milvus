...
func TestMigrationCompactionIndexRebuildSingleSnapshot(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			reads := 0
			snapshot := mockey.Mock((*indexMeta).GetAllSegmentIndexes).To(func(*indexMeta, int64) []*model.SegmentIndex {
				reads++
				// Put the rebuild reason first to verify that later artifacts
				// are still checked for safety before accepting the segment.
				return []*model.SegmentIndex{model.CloneSegmentIndex(oldIndex), model.CloneSegmentIndex(otherIndex)}
			}).Build()
