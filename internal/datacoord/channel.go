...
type StateChannel struct {
	...
	assignedNode  int64
	abnormalNodes typeutil.UniqueSet // nodeID
}
...
func NewStateChannel(ch RWChannel) *StateChannel {
	c := &StateChannel{
		...
		abnormalNodes: typeutil.NewUniqueSet(),
	}
...
func (c *StateChannel) TransitionOnFailure() {
	...
	case Releasing:
		c.setState(Standby)
		c.abnormalNodes.Insert(c.assignedNode)
...
func (c *StateChannel) UpdateWatchInfo(info *datapb.ChannelWatchInfo) {
	if info.GetVchan().GetChannelName() != c.Info.GetVchan().GetChannelName() {
...
func (c *StateChannel) RemoveNode(nodeID int64) {
	c.abnormalNodes.Remove(nodeID)
}
