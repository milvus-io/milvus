...
func recordImportFileTypes(files []*internalpb.ImportFile) {
	...
	// One hit per distinct file type, not per file: the request group counts
	// requests using a feature, and recordCompactionType, its sibling on this
	// node, counts one per task. Counting per file would report a single job
	// carrying a thousand Parquet files as a thousand uses of Parquet.
	var seen featureusage.FeatureSet
