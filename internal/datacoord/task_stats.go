...
func (st *statsTask) cleanupInvalidJSONStatsResultFiles(ctx context.Context, result *workerpb.StatsResult) error {
	...
	_, local := st.meta.chunkManager.(*storage.LocalChunkManager)
	if local != path.IsAbs(basePath) {
		return nil
	}
	...
	if local {
		basePath, resultBase = path.Clean(basePath), path.Clean(resultBase)
	}
	if resultBase != basePath {
		return nil
	}
	// Match the writer's concatenation exactly. Remote keys are opaque, including
	// repeated separators and dot components in the manifest base or file name.
	statsPrefix := func(fieldID int64) string {
	...
	resolveFile := func(prefix, file string) (string, bool) {
		if file == "" {
			return "", false
		}
		if !path.IsAbs(file) && !metautil.IsJSONKeyStatsFullPath(file) {
			file = prefix + "/" + file
		}
		if local {
			file = path.Clean(file)
		}
		return file, strings.HasPrefix(file, prefix+"/")
	}
	...
	for _, stats := range segment.GetJsonKeyStats() {
		...
		prefix := statsPrefix(stats.GetFieldID())
		for _, file := range stats.GetFiles() {
			if file, ok := resolveFile(prefix, file); ok {
	...
	for _, fieldID := range st.GetJsonStatsFieldIds() {
		...
		prefix := statsPrefix(fieldID)
		files := make([]string, 0, len(stats.GetFiles()))
		for _, file := range stats.GetFiles() {
			file, ok := resolveFile(prefix, file)
			if !ok {
				continue
			}
