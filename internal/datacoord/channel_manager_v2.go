...
func (m *ChannelManagerImplV2) DeleteNode(nodeID UniqueID) error {
	...
	updates := NewChannelOpSet(
		NewDeleteOp(info.NodeID, info.Channels...),
		NewChannelOp(bufferID, Watch, info.Channels...),
	)
...
func (m *ChannelManagerImplV2) GetChannel(nodeID int64, channel string) (RWChannel, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
