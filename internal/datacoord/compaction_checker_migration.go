...
func (checker *MigrationCompactionChecker) Trigger(ctx context.Context) (map[CompactionTriggerType][]CompactionView, error) {
	...
	if maxCount <= 0 {
		// Zero tokens disable migration permanently, not just for this period,
		// and that is invisible from the outside: needDoJSONKeyIndex already
		// refuses an in-place stats task for every segment the migration policy
		// owns, so those segments keep their current artifacts forever with no
		// other signal. Say so instead of breaking out of the loop silently.
		mlog.RatedWarn(ctx, rate.Limit(1.0/300.0),
			"migration compaction is disabled by its rate limit; segments keep their current JSON stats and index artifacts",
			mlog.String("configKey", paramtable.Get().DataCoordCfg.StorageVersionCompactionRateLimitTokens.Key),
			mlog.Int("tokens", maxCount))
		return map[CompactionTriggerType][]CompactionView{TriggerTypeSingle: nil}, nil
	}
...
func (checker *MigrationCompactionChecker) triggerOneCollection(
	...
	// Resolve index liveness before entering the filter below: it runs under
	// meta.segMu and must not acquire indexMeta.fieldIndexLock.
	activeIndexIDs := getActiveIndexIDs(checker.meta.indexMeta, collectionID)

	segments := checker.meta.SelectSegmentsWithLimit(ctx, remaining, WithCollection(collectionID), SegmentFilterFunc(func(segment *SegmentInfo) bool {
...
// getActiveIndexIDs returns the live (non-deleted) index IDs of a collection.
// Resolving index liveness here, outside the segment filter, keeps
// indexMeta.fieldIndexLock out of the closure that runs under meta.segMu:
// index DDL holds that lock's write side across an etcd round trip and
// sync.RWMutex is writer-preferring, so taking it inside segMu would park the
// filter and stall every segment reader. See the lock-ordering note in
// segment_manifest_commit.go.
func getActiveIndexIDs(indexMeta *indexMeta, collectionID int64) map[int64]struct{} {
	if indexMeta == nil {
		return nil
	}

	indexIDs := make(map[int64]struct{})
	for _, index := range indexMeta.GetIndexesForCollection(collectionID, "") {
		if index != nil {
			indexIDs[index.IndexID] = struct{}{}
		}
	}
...
// so activeIndexIDs carries the collection's live index IDs resolved by the
// caller; this function must not query indexMeta for liveness itself, because
// it runs inside a segment filter that holds meta.segMu.
func checkSegmentIndexRebuild(
	indexMeta *indexMeta,
	segmentID int64,
	activeIndexIDs map[int64]struct{},
	...
	for _, segmentIndex := range indexMeta.getSegmentIndexRebuildInfo(segmentID) {
		if segmentIndex.IndexState == commonpb.IndexState_Failed {
			continue
		}
		_, isActiveIndex := activeIndexIDs[segmentIndex.IndexID]
