...
func (s *L0CompactionTaskSuite) TestSaveSegmentMetaUpdatesLatestDataViewWithoutVersionAdvance() {
	...
	task := newL0CompactionTask(ctx, &datapb.CompactionTask{
		PlanID:        1,
		TriggerID:     19530,
		CollectionID:  1,
		PartitionID:   10,
	...
	task.preparedV3Manifests[200] = packed.MarshalManifestPath(manifestBasePath, 4)
	output := []*datapb.CompactionSegment{{
		SegmentID: 200,
		Deltalogs: []*datapb.FieldBinlog{{
...
func (s *L0CompactionTaskSuite) TestPorcessStateTrans() {
	...
	s.Run("test executing with result failed", func() {
		...
		t.QueryTaskOnWorker(cluster)
		s.Equal(datapb.CompactionTaskState_retrying, t.GetTask().GetState())
		s.Equal("compaction failed in datanode", t.GetTask().GetFailReason())
	})
