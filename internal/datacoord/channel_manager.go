...
func (c *ChannelManager) AddChannel(ch *channel) error {
	...
	err := c.execute(updates, datapb.ChannelWatchState_ToWatch)
	if err != nil {
		log.Warn("fail to add channel",
			zap.String("channel", ch.String()), zap.Array("updates", updates),
			zap.Error(err))
	}
	return err
}
...
//     }
//
//     var toReleaseUpdates ChannelOpSet
//     toReleaseUpdates.Add(nodeID, []*channel{toReleaseChannel})
//     err := c.execute(toReleaseUpdates, datapb.ChannelWatchState_ToRelease)
//     if err != nil {
//         log.Warn("fail to remove channel",
//             zap.String("channel", channelName),
//             zap.Array("updates", toReleaseUpdates))
//     }
//
//     return err
// }
...
func (c *ChannelManager) execute(updates ChannelOpSet, state datapb.ChannelWatchState) error {
	...
	for _, op := range updates {
		if op.Type == Add {
			c.fillChannelWatchInfoWithState(op, state)
		}
	}
...
func (c *ChannelManager) fillChannelWatchInfoWithState(op *ChannelOp, state datapb.ChannelWatchState) {
