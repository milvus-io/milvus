...
func TestRefreshExternalCollectionTask_CreateTaskOnWorker(t *testing.T) {
	newOwnershipTask := func(
		...
		for _, segmentID := range []int64{1, 2} {
			segments.SetSegment(segmentID, &SegmentInfo{SegmentInfo: &datapb.SegmentInfo{
				...
				ManifestPath: packed.MarshalManifestPath("segment", 1),
			}})
		}
		collections := typeutil.NewConcurrentMap[UniqueID, *collectionInfo]()
	...
	t.Run("long manifest path stays out of task metadata", func(t *testing.T) {
		task, refreshMeta, cluster := newOwnershipTask(t, externalRefreshOwnershipPlanVersion, []int64{1})
		basePath := strings.Repeat("long-directory/", 1000)
		manifest := packed.MarshalManifestPath(basePath, 17)
		segment := task.mt.segments.GetSegment(1).Clone()
		segment.ManifestPath = manifest
		task.mt.segments.SetSegment(1, segment)
		task.CreateTaskOnWorker(1, cluster)
		persisted := refreshMeta.GetTask(1001)
		require.Equal(t, indexpb.JobState_JobStateInProgress, persisted.GetState())
		assert.Equal(t, map[int64]int64{1: 17}, persisted.GetBaseManifestVersions())
		wire, err := proto.Marshal(persisted)
		require.NoError(t, err)
		assert.Less(t, len(wire), 1024)
		assert.NotContains(t, string(wire), basePath)
		require.NotNil(t, cluster.refreshReq)
		assert.Equal(t, manifest, cluster.refreshReq.GetCurrentSegments()[0].GetManifestPath(),
			"worker still receives the full immutable input manifest")
	})

	t.Run("old path baseline plan is rejected", func(t *testing.T) {
		task, refreshMeta, cluster := newOwnershipTask(t, 3, []int64{1})
		task.CreateTaskOnWorker(1, cluster)
		assert.Equal(t, indexpb.JobState_JobStateFailed, refreshMeta.GetTask(1001).GetState())
		assert.Contains(t, refreshMeta.GetTask(1001).GetFailReason(), "unsupported ownership plan version 3")
		assert.Nil(t, cluster.refreshReq)
	})

	t.Run("oversized baseline fails before worker dispatch", func(t *testing.T) {
		// The ownership IDs fit before dispatch; adding one revision per segment
		// crosses the admission limit. Exercise the real size check and caller.
		ids := make([]int64, 100000)
		for i := range ids {
			ids[i] = int64(i + 1)
		}
		task, refreshMeta, cluster := newOwnershipTask(t, externalRefreshOwnershipPlanVersion, ids)
		for _, id := range ids {
			segment := newTestExternalRefreshSegment(id, 100, 1)
			segment.State = commonpb.SegmentState_Flushed
			segment.ManifestPath = packed.MarshalManifestPath("segment", 1<<60)
			task.mt.segments.SetSegment(id, NewSegmentInfo(segment))
		}
		task.CreateTaskOnWorker(1, cluster)
		failed := refreshMeta.GetTask(1001)
		assert.Equal(t, indexpb.JobState_JobStateFailed, failed.GetState())
		assert.Contains(t, failed.GetFailReason(), "metadata exceeds size limit")
		assert.Empty(t, failed.GetBaseManifestVersions())
		assert.Nil(t, cluster.refreshReq)
	})
...
func TestRefreshExternalCollectionTask_QueryTaskOnWorker(t *testing.T) {
	...
	t.Run("task_failed_on_worker", func(t *testing.T) {
		...
		// it only delays the RefreshFailed the caller is waiting for. Missing
		// explore manifests are such a failure, even when the native reader
		// classifies the underlying error as transient.
		...
		// The reason is prefixed with the spent attempt budget ("worker failure
		// 1/1: ..."), so the worker's own message is contained rather than equal.
		assert.Contains(t, metaTask.GetFailReason(), failReason)
		assert.Equal(t, indexpb.JobState_JobStateFailed, refreshMeta.GetJob(1).GetState())
		assert.Contains(t, refreshMeta.GetJob(1).GetFailReason(), failReason)
...
func TestApplyExternalCollectionSegmentUpdateForBaseline_UpsertExistingSegment(t *testing.T) {
	...
	err := applyExternalCollectionSegmentUpdateForBaseline(
		...
		collectionID,
		[]int64{segmentID},
		map[int64]int64{segmentID: testRefreshManifestVersion(t, oldSeg.GetManifestPath())},
		nil,
...
func TestApplyExternalCollectionSegmentUpdateForBaseline_RejectsUnconsumedReplay(t *testing.T) {
	...
	err := applyExternalCollectionSegmentUpdateForBaseline(
		...
		map[int64]int64{segmentID: 1}, nil, []*datapb.SegmentInfo{patch}, nil)
	assert.NoError(t, err)
	...
	err = applyExternalCollectionSegmentUpdateForBaseline(
		ctx, mt, collInfo, collectionID, []int64{segmentID},
		map[int64]int64{segmentID: 2}, nil, []*datapb.SegmentInfo{newer}, nil)
	assert.NoError(t, err)
...
func TestApplyExternalCollectionSegmentUpdateForBaseline_ManifestCAS(t *testing.T) {
	...
	conflicts := []struct {
		...
		{
			name:                 "different directory with same revision",
			current:              packed.MarshalManifestPath("other-directory", 1),
			baseManifestVersions: map[int64]int64{segmentID: 1},
		},
		{
	...
	for _, resultPath := range []string{
		packed.MarshalManifestPath(basePath, 1),
		"not-json",
		packed.MarshalManifestPath(basePath, 0),
	} {
		t.Run("reject invalid or older result "+resultPath, func(t *testing.T) {
			catalog := &stubCatalog{}
			mt := &meta{segments: NewSegmentsInfo(), catalog: catalog}
			current := newSegment(segmentID, packed.MarshalManifestPath(basePath, 2))
			mt.segments.SetSegment(segmentID, NewSegmentInfo(current))
			err := applyExternalCollectionSegmentUpdateForBaseline(ctx, mt, collInfo, collectionID,
				[]int64{segmentID}, map[int64]int64{segmentID: 2}, nil,
				[]*datapb.SegmentInfo{newSegment(segmentID, resultPath)}, nil)
			require.Error(t, err)
			assert.Empty(t, catalog.alteredSegments)
			assert.Equal(t, current.GetManifestPath(), mt.segments.GetSegment(segmentID).GetManifestPath())
		})
	}
...
func TestRefreshExternalCollectionTask_CreateTaskOnWorker_TaskNotFoundAfterVersionUpdate(t *testing.T) {
	...
	// Allow the dispatch precheck, then simulate disappearance after assignment.
	mockGetTask := mockey.Mock((*externalCollectionRefreshMeta).GetTask).
		To(func(m *externalCollectionRefreshMeta, taskID int64) *datapb.ExternalCollectionRefreshTask {
			current, _ := m.tasks.Get(taskID)
			if current.GetState() != indexpb.JobState_JobStateInit {
				return nil
			}
			return proto.Clone(current).(*datapb.ExternalCollectionRefreshTask)
		}).Build()
...
func testRefreshManifestVersion(t *testing.T, manifest string) int64 {
	t.Helper()
