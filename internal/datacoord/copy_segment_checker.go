...
func (c *copySegmentChecker) cleanupTaskForGC(job CopySegmentJob, taskID int64) bool {
	...
	cleanup := func() {
		...
		// A completed result was admitted but its task-owned files are not yet
		// reclaimed. The record carries the cleanup plan; keep it until the
		// inspector's cleanup worker has cleared the intent.
		if latest.GetCleanupRequired() {
			mlog.Info(c.ctx, "retain copy segment task until its cleanup plan is settled",
