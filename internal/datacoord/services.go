...
func (s *Server) GetCollectionStatistics(ctx context.Context, req *datapb.GetCollectionStatisticsRequest) (*datapb.GetCollectionStatisticsResponse, error) {
	...
	if collection != nil && collection.Schema != nil && collection.Schema.GetVersion() > 0 {
		...
		// The consistency gate only needs to prove that all data eligible for backfill
		// has been backfilled; see isBackfillEligibleSegment for the eligibility rules
		// (growing/L0/importing/invisible exclusions and their rationale).
		segments := s.meta.SelectSegments(ctx, WithCollection(req.CollectionID), SegmentFilterFunc(isBackfillEligibleSegment))
