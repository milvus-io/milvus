...
//
// Returns:
//   - manifestDir: Directory for segment manifest files (snapshots/{collection_id}/manifests/{snapshot_id}/)
//   - metadataPath: Full path to metadata JSON file (snapshots/{collection_id}/metadata/{snapshot_id}.json)
func GetSnapshotPaths(collectionID int64, snapshotID int64) (manifestDir, metadataPath string) {
...
func (w *SnapshotWriter) Save(ctx context.Context, snapshot *SnapshotData) (string, error) {
	...
	manifestDir, metadataPath := GetSnapshotPaths(collectionID, snapshotID)
