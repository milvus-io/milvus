...
func newReshardTask(p *datapb.ReshardTask, importMeta ImportMeta, meta *meta, alloc allocator.Allocator) *reshardTask {
	t := &reshardTask{importMeta: importMeta, meta: meta, alloc: alloc, tr: timerecord.NewTimeRecorder("reshard task"), times: taskcommon.NewTimes(), metrics: newImportTaskMetrics(ReshardTaskType)}
	t.task.Store(p)
	return t
}
...
// job is in this state: a backup import creates reshard tasks while the job is
// still Pending, every other job once it has reached Resharding.
...
// importV3PreReshardingState reports whether the job has not reached Resharding
...
func importV3PreReshardingState(state internalpb.ImportJobState) bool {
	switch state {
...
func (t *reshardTask) CreateTaskOnWorker(nodeID int64, cluster session.Cluster) {
	...
	if !reshardDispatchable(job.GetState()) {
		if importV3PreReshardingState(job.GetState()) {
			// ReshardTasks are persisted a moment before the job advances to
			// Resharding (see reshardTaskPlanner.CreateTasks). The async dispatch
	...
	plan, err := newImportV3PlanFactory(job).reshardPlan(p)
	if err != nil {
		t.fail(err.Error())
		return
	}
...
func newImportTaskV3(p *datapb.ImportTaskV3, importMeta ImportMeta, meta *meta, alloc allocator.Allocator) *importTaskV3 {
	t := &importTaskV3{importMeta: importMeta, meta: meta, alloc: alloc, tr: timerecord.NewTimeRecorder("import v3 task"), times: taskcommon.NewTimes(), metrics: newImportTaskMetrics(ImportTaskV3Type)}
	t.task.Store(p)
	return t
}
...
func (t *importTaskV3) CreateTaskOnWorker(nodeID int64, cluster session.Cluster) {
	...
	plan, err := newImportV3PlanFactory(job).importPlan(p)
	if err != nil {
		t.fail(err.Error())
		return
	}
...
func (t *importTaskV3) prepareRetry(cluster session.Cluster) {
	...
	writerSpec, err := newImportV3PlanFactory(job).writerSpec(typeutil.AppendSystemFields(job.GetSchema()))
	if err != nil {
		t.fail(err.Error())
		return
	}
