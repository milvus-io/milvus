...
func (s *Server) initDataCoord() error {
	...
	if err != nil {
		mlog.Error(s.ctx, "data coordinator id allocator initialize failed", zap.Error(err))
		return err
	}
	...
	mlog.Info(s.ctx, "init datacoord done", zap.Int64("nodeID", paramtable.GetNodeID()), zap.String("Address", s.address))
...
func (s *Server) newChunkManagerFactory() (storage.ChunkManager, error) {
	...
	if err != nil {
		mlog.Error(s.ctx, "chunk manager init failed", zap.Error(err))
		return nil, err
	}
	return cli, err
}
...
func (s *Server) initServiceDiscovery() error {
	...
	if err != nil {
		mlog.Warn(s.ctx, "DataCoord failed to init service discovery", zap.Error(err))
		return err
	}
	mlog.Info(s.ctx, "DataCoord success to get DataNode sessions", zap.Any("sessions", sessions))
	...
	if Params.DataCoordCfg.BindIndexNodeMode.GetAsBool() {
		...
		if err := s.nodeManager.AddNode(Params.DataCoordCfg.IndexNodeID.GetAsInt64(),
			...
			mlog.Warn(s.ctx, "DataCoord failed to add datanode", zap.Error(err))
			return err
		}
		...
		if err != nil {
			mlog.Warn(s.ctx, "DataCoord failed to rewatch datanode", zap.Error(err))
			return err
		}
	...
	if err != nil {
		mlog.Warn(s.ctx, "DataCoord get QueryNode sessions failed", zap.Error(err))
		return err
	}
...
func (s *Server) initKV() error {
	...
	mlog.Info(s.ctx, "data coordinator successfully connected to metadata store", zap.String("metaType", metaType))
	return nil
}
...
func (s *Server) getFlushableSegmentsInfo(ctx context.Context, flushableIDs []int64) []*SegmentInfo {
	...
	for _, id := range flushableIDs {
		...
		if sinfo == nil {
			mlog.Error(ctx, "get segment from meta error", zap.Int64("id", id))
			continue
		}
...
func (s *Server) stopServiceWatch() {
	...
	mlog.Error(s.ctx, "watch service channel closed", zap.Int64("serverID", paramtable.GetNodeID()))
	if s.ctx.Err() == nil {
			...
			if err := s.handleSessionEvent(ctx, typeutil.DataNodeRole, event); err != nil {
				go func() {
					if err := s.Stop(); err != nil {
						mlog.Warn(ctx, "DataCoord server stop error", zap.Error(err))
					}
				}()
				return
			}
...
func (s *Server) handleSessionEvent(ctx context.Context, role string, event *sessionutil.SessionEvent) error {
	...
	case typeutil.QueryNodeRole:
		...
		case sessionutil.SessionUpdateEvent:
			...
			mlog.Info(ctx, "received querynode SessionUpdateEvent", zap.Int64("serverID", serverID))
	...
	go func() {
		...
		for {
			...
			case segmentID := <-s.flushCh:
				// Ignore return error
				mlog.Info(ctx, "flush successfully", zap.Any("segmentID", segmentID))
				err := s.postFlush(ctx, segmentID)
				if err != nil {
					mlog.Warn(ctx, "failed to do post flush", zap.Int64("segmentID", segmentID), zap.Error(err))
				}
			}
		}
	}()
...
func (s *Server) postFlush(ctx context.Context, segmentID UniqueID) error {
	...
	mlog.Info(ctx, "flush segment complete", zap.Int64("id", segmentID))
	return nil
}
...
func (s *Server) handleFlushingSegments(ctx context.Context) {
	...
	for _, segment := range segments {
		...
		if err := s.flushFlushingSegment(ctx, segment.ID); err != nil {
			mlog.Warn(ctx, "flush flushing segment failed", zap.Int64("segmentID", segment.ID), zap.Error(err))
			return
		}
		mlog.Info(ctx, "flush flushing segment success", zap.Int64("segmentID", segment.ID))
		select {
		case <-ctx.Done():
			return
...
func (s *Server) flushFlushingSegment(ctx context.Context, segmentID UniqueID) error {
	return retry.Do(ctx, func() error {
		...
		if err := s.meta.UpdateSegmentsInfo(ctx, operators...); err != nil {
			mlog.Warn(ctx, "flush segment complete failed", zap.Int64("segmentID", segmentID), zap.Error(err))
			if ctx.Err() != nil {
				return ctx.Err()
			}
...
func (s *Server) CleanMeta() error {
	mlog.Debug(s.ctx, "clean meta", zap.Any("kv", s.kv))
...
func (s *Server) updateBalanceConfig() bool {
	...
	if err != nil {
		mlog.Warn(s.ctx, "check data node version occur error on etcd", zap.Error(err))
		return false
	}
	...
	mlog.RatedDebug(s.ctx, rate.Limit(10), "old data node exist", zap.Strings("sessions", lo.Keys(sessions)))
	return false
}
