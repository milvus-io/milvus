...
func (s *CopySegmentInspectorSuite) SetupTest() {
	...
	s.catalog.EXPECT().ListSegmentIndexes(mock.Anything).Return(nil, nil).Maybe()
	...
	s.catalog.EXPECT().ListFileResource(mock.Anything).Return(nil, nil).Maybe()
	s.catalog.EXPECT().ListSnapshots(mock.Anything).Return(nil, nil).Maybe()
