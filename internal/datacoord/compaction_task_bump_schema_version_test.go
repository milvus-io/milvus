...
func (s *BumpSchemaVersionCompactionTaskSuite) TestBuildCompactionRequestPreAllocateLogIDsError() {
	...
	task := newBumpSchemaVersionTask(s.generateBasicTask().GetTask(), mockAlloc, s.meta, s.ievm)
...
func (s *BumpSchemaVersionCompactionTaskSuite) TestCreateTaskOnWorker() {
	...
	s.Run("CreateTaskOnWorker fail, CreateCompaction error", func() {
		...
		task.CreateTaskOnWorker(1, cluster)
		s.Equal(datapb.CompactionTaskState_retrying, task.GetTask().GetState())
		s.True(isRetrying(task.GetTask()), "the work must be rebuilt, not re-dispatched")
	})
	...
	s.Run("CreateTaskOnWorker outcome unknown, abandon for replan", func() {
		...
		task.CreateTaskOnWorker(1, cluster)
		s.Equal(datapb.CompactionTaskState_retrying, task.GetTask().GetState())
		s.True(isRetrying(task.GetTask()), "an undecided create must be replanned, never re-dispatched")
	})
