...
func Test_assignPKRangesToFiles(t *testing.T) {
	...
	assert.Equal(t, int64(10), files[0].GetPreAllocatedAutoIds().GetEnd()-files[0].GetPreAllocatedAutoIds().GetBegin())
	assert.Equal(t, int64(20), files[1].GetPreAllocatedAutoIds().GetEnd()-files[1].GetPreAllocatedAutoIds().GetBegin())
...
func Test_reserveRanges(t *testing.T) {
	...
	widths := func(files []*internalpb.ImportFile) []int64 {
		...
		for i, f := range files {
			out[i] = f.GetPreAllocatedAutoIds().GetEnd() - f.GetPreAllocatedAutoIds().GetBegin()
		}
		return out
	}
...
func Test_assignPKRangesToFiles_largeVarcharJSONIsAccepted(t *testing.T) {
	...
	assert.Positive(t, files[0].GetPreAllocatedAutoIds().GetEnd()-files[0].GetPreAllocatedAutoIds().GetBegin())
}
