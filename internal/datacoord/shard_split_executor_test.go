...
func (f *fakeSplitPlanner) PlanTargets(ctx context.Context, collection *collectionInfo, sourceVChannel string, targetVChannels []string) ([]*datapb.SplitShardTaskTarget, uint64, error) {
	if f.planErr != nil {
	...
	return targets, uint64(len(targetVChannels)), nil
}
...
func newSplitExecutorManager(t *testing.T, m *meta, task *datapb.SplitShardTask, wal streaming.WALAccesser, vchannelAllocator splitVChannelAllocator, planner splitPlanner) (*shardSplitManager, *mocks.DataCoordCatalog) {
	...
	// The barrier must exceed every source's fence tick (the fences below land at
	// 2000), which a monotonic TSO guarantees and the fencing phase checks.
	alloc.EXPECT().AllocTimestamp(mock.Anything).Return(uint64(3000), nil).Maybe()
	if m != nil {
		// the redistribution path persists the relabeled segments through
		// the meta's own catalog.
		m.catalog = catalog
		// ...as does the write switch, which seeds each freshly created target
		// vchannel's genesis checkpoint through the same meta.
		catalog.EXPECT().SaveChannelCheckpoints(mock.Anything, mock.Anything).Return(nil).Maybe()
	}
...
// pbResidueShard builds a CollectionShardInfo owning the given residues, for
// test fixtures.
func pbResidueShard(state schemapb.ShardState, residues ...uint64) *schemapb.CollectionShardInfo {
...
func TestAdvanceFencing(t *testing.T) {
	...
	t.Run("fence and initialize targets", func(t *testing.T) {
		...
		// three persists: the fence intent (written BEFORE the append, so the task
		// can never be abandoned after a fence it cannot account for), the landed
		// T_switch, then the state transition after the routing commit.
		...
		assert.Equal(t, uint64(2000), firstSwitchTimeTick(task))
		// every target is created with the freshly allocated barrier (3000 from
		// the mocked allocator), not T_switch.
		assert.Equal(t, map[string]uint64{"v1": 3000, "v2": 3000}, initialized)
		// Each target's genesis position IS recorded as its channel checkpoint.
		// Leaving it to arrive later is what the 3 -> 8 run disproved: a target
		// that takes no live write before adoption never reports one, and the
		// seek fallback then hands out a rewrite segment's DML position, which
		// has no message id and panics the querynode that seeks from it.
		for _, vchannel := range []string{"v1", "v2"} {
			cp := m.GetChannelCheckpoint(vchannel)
			require.NotNil(t, cp, vchannel)
			assert.Equal(t, uint64(2100), cp.GetTimestamp())
	...
	t.Run("already fenced proceeds to create targets", func(t *testing.T) {
		...
		manager, catalog := newSplitExecutorManager(t, m, fencingTask(), wal, nil, &fakeSplitPlanner{})
		// three persists: the fence intent, the recovered T_switch, the transition.
		catalog.EXPECT().SaveSplitShardTask(mock.Anything, mock.Anything).Return(nil).Times(3)
		manager.advanceTasks()
		task := manager.mustGetTask(100)
		assert.Equal(t, datapb.SplitShardTaskState_SplitShardTaskRedistributing, task.GetState())
		...
		assert.Equal(t, uint64(1900), firstSwitchTimeTick(task))
	})
	...
	t.Run("resume after T_switch persisted only initializes targets", func(t *testing.T) {
		...
		task := fencingTask()
		task.Fenced = true
		// T_switch already persisted: the resume must not re-fence.
	...
	t.Run("split shard append error stays in fencing", func(t *testing.T) {
		...
		manager, catalog := newSplitExecutorManager(t, m, fencingTask(), wal, nil, &fakeSplitPlanner{})
		// only the fence intent is persisted; nothing landed, so no tick to record.
		catalog.EXPECT().SaveSplitShardTask(mock.Anything, mock.Anything).Return(nil).Once()
		manager.advanceTasks()
	...
	t.Run("create vchannel error keeps the fence and stays in fencing", func(t *testing.T) {
		...
		manager, catalog := newSplitExecutorManager(t, m, fencingTask(), wal, nil, &fakeSplitPlanner{})
		// the fence intent and the landed T_switch; the transition never happens.
	...
	t.Run("fence intent persist failure appends nothing", func(t *testing.T) {
		// The intent is written first precisely so this case is harmless: the
		// task stays abortable because no fence reached a WAL. The mock WAL has
		// no expectations, so an append here fails the test.
		m := newSplitTestMeta(true, "v0", map[int64]int64{10: 80})
		wal := mock_streaming.NewMockWALAccesser(t)
		manager, catalog := newSplitExecutorManager(t, m, fencingTask(), wal, nil, &fakeSplitPlanner{})
	...
	t.Run("T_switch persist failure keeps the task forward-only", func(t *testing.T) {
		// The mirror case: the fence DID land, so the task must stay unabortable
		// even though the tick it returned was lost. The next tick re-asks and
		// the idempotent re-fence hands back the same tick.
		m := newSplitTestMeta(true, "v0", map[int64]int64{10: 80})
		wal := mock_streaming.NewMockWALAccesser(t)
		wal.EXPECT().RawAppend(mock.Anything, mock.MatchedBy(func(msg message.MutableMessage) bool {
			return msg.MessageType() == message.MessageTypeSplitShard
		...
		task := manager.mustGetTask(100)
		assert.Equal(t, datapb.SplitShardTaskState_SplitShardTaskFencing, task.GetState())
		assert.True(t, task.GetFenced(), "a landed fence can never be abandoned")
		assert.Zero(t, firstSwitchTimeTick(task))
	})
	...
	t.Run("final persist failure stays in fencing", func(t *testing.T) {
		...
		manager, catalog := newSplitExecutorManager(t, m, fencingTask(), wal, nil, &fakeSplitPlanner{})
		// the intent and T_switch persists succeed, the state transition fails.
		catalog.EXPECT().SaveSplitShardTask(mock.Anything, mock.Anything).Return(nil).Times(2)
		catalog.EXPECT().SaveSplitShardTask(mock.Anything, mock.Anything).Return(errors.New("save failed")).Once()
		manager.advanceTasks()
		assert.Equal(t, datapb.SplitShardTaskState_SplitShardTaskFencing, manager.mustGetTask(100).GetState())
...
func TestAdvanceAdopting(t *testing.T) {
	...
	adoptingTask := func() *datapb.SplitShardTask {
		...
		task.Fenced = true
		// Past the fence, so T_switch is recorded: the routing commit refuses to
		// retire a source whose fence it cannot see.
		task.Sources[0].SwitchTimeTick = 2000
		return task
	}
	...
	t.Run("adoption flip completes the task", func(t *testing.T) {
		...
		router.EXPECT().CommitShardSplitRouting(mock.Anything, mock.MatchedBy(func(req *rootcoordpb.CommitShardSplitRoutingRequest) bool {
			...
			return req.GetRoutingModulus() > 0 &&
...
func TestCommitRoutingMultiShard(t *testing.T) {
	...
	// A two-shard collection at modulus 2: v0 owns residue 0, v3 residue 1.
	collection := &collectionInfo{
		...
		ShardInfos: map[string]*schemapb.CollectionShardInfo{
			"v0": pbResidueShard(schemapb.ShardState_ShardNormal, 0),
			"v3": pbResidueShard(schemapb.ShardState_ShardNormal, 1),
		},
	}
	// Splitting v0 -- residue 0 at modulus 2 -- doubles the modulus to 4: the
	// targets take 0 and 2, and v3 rebases from 1 to {1,3}. Reusing the
	// single-shard fixture's plan here would hand v2 residue 1, which v3 already
	// owns, and Derive would refuse the commit as an overlap.
	task := fenceSources(fencingTask(), 2000)
	task.RoutingModulus = 4
	task.Targets = []*datapb.SplitShardTaskTarget{
		{Vchannel: "v1", Buckets: []uint64{0}},
		{Vchannel: "v2", Buckets: []uint64{2}},
	}

	// past the fence: the commit refuses to retire a source whose fence it
	// cannot see.
	err := manager.commitRouting(task, collection,
	...
	assert.NotZero(t, captured.GetRoutingModulus())
	// the source is fenced.
	assert.Equal(t, schemapb.ShardState_ShardSplitting, got["v0"].GetState())
	// the unrelated shard v3 keeps its state, and its residue is re-expressed at
	// the collection's modulus after the split rather than left behind.
	assert.Equal(t, schemapb.ShardState_ShardNormal, got["v3"].GetState())
	assert.NotEmpty(t, got["v3"].GetHashRouting().GetBuckets())
	// the targets are Creating with their split residues.
	assert.Equal(t, schemapb.ShardState_ShardCreating, got["v1"].GetState())
	assert.Equal(t, []uint64{0}, got["v1"].GetHashRouting().GetBuckets())
	assert.Equal(t, []uint64{2}, got["v2"].GetHashRouting().GetBuckets())
	assert.Equal(t, []uint64{1, 3}, got["v3"].GetHashRouting().GetBuckets(),
		"the untouched shard is rebased onto the doubled modulus")

	// And the committed topology tiles the key space: that is the invariant the
	// whole commit exists to preserve, and asserting the individual residues
	// without it would pass on a plan that leaves a gap.
	writable := make([]routing.Shard, 0, len(captured.GetVirtualChannelNames()))
	for i, vch := range captured.GetVirtualChannelNames() {
		info := captured.GetShardInfos()[i]
		switch info.GetState() {
		case schemapb.ShardState_ShardNormal, schemapb.ShardState_ShardCreating:
			writable = append(writable, routing.Shard{Vchannel: vch, Buckets: info.GetHashRouting().GetBuckets()})
		}
	}
	_, deriveErr := routing.Derive(captured.GetRoutingModulus(), captured.GetVirtualChannelNames(), writable)
	require.NoError(t, deriveErr, "the committed topology must tile the key space")
}
...
func TestAdvanceRedistributing(t *testing.T) {
	...
	taskC := fencingTask()
	taskC.TaskId = 300
	taskC.Sources = []*datapb.SplitShardTaskSource{{Vchannel: "v8"}}
	...
	task2 := fencingTask()
	task2.TaskId = 200
	task2.Sources = []*datapb.SplitShardTaskSource{{Vchannel: "v9"}}
	...
	task3 := fencingTask()
	task3.TaskId = 400
	task3.Sources = []*datapb.SplitShardTaskSource{{Vchannel: "v10"}}
...
func TestAdvanceRedistributingWaitsForFlushedFence(t *testing.T) {
	...
	task.State = datapb.SplitShardTaskState_SplitShardTaskRedistributing
	task.Fenced = true
	task.Sources[0].SwitchTimeTick = 100 // T_switch
