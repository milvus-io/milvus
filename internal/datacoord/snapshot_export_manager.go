...
func (m *snapshotExportManager) Submit(
	...
	// Once pinned, finish publishing the job independently of request cancellation.
	// The catalog bounds each write with its configured metadata request timeout;
	// the manager context still cancels publication during shutdown.
	if err := m.meta.CreateJob(m.ctx, job); err != nil {
