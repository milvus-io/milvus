...
func TestExternalCollectionRefreshChecker_StaleFinalizerAfterManifestRetry(t *testing.T) {
	paramtable.Init()
	for _, waitForIndex := range []bool{false, true} {
		name, enabled := "default", "false"
		if waitForIndex {
			name, enabled = "index_wait", "true"
		}
		t.Run(name, func(t *testing.T) {
			key := Params.DataCoordCfg.RefreshWaitForIndex.Key
			previous := Params.DataCoordCfg.RefreshWaitForIndex.GetValue()
			require.NoError(t, Params.Save(key, enabled))
			defer Params.Save(key, previous)
			retryKey := Params.DataCoordCfg.ExternalCollectionMaxRetryTimes.Key
			previousRetry := Params.DataCoordCfg.ExternalCollectionMaxRetryTimes.GetValue()
			require.NoError(t, Params.Save(retryKey, "5"))
			defer Params.Save(retryKey, previousRetry)
			...
			incoming := newTestExternalRefreshSegment(10, 100, 7)
			incoming.ManifestPath = `{"base_path":"segment-10","ver":3}`
			...
			mt := &meta{ctx: ctx, catalog: catalog, segments: NewSegmentsInfo(), indexMeta: &indexMeta{}}
			collection := newTestExternalCollectionInfo(100)
			current := newTestExternalRefreshSegment(10, 100, 7)
			current.State = commonpb.SegmentState_Flushed
			current.Level = datapb.SegmentLevel_L1
			current.PartitionID = collection.Partitions[0]
			current.InsertChannel = collection.VChannelNames[0]
			current.ManifestPath = `{"base_path":"segment-10","ver":2}`
			mt.segments.SetSegment(10, NewSegmentInfo(current))
			manager := &externalCollectionRefreshManager{
				mt: mt, refreshMeta: refreshMeta,
				collectionGetter: testCollectionGetter(newTestCollections(100)),
			}
			var schemaCalls, cleanupCalls, failureCalls atomic.Int32
			checker := newRefreshChecker(ctx, mt, refreshMeta, make(chan struct{}),
				...
				manager.applyFinishedJobSegments,
				func(int64) { failureCalls.Add(1) },
				func(int64) error {
					cleanupCalls.Add(1)
					return nil
				}, nil)

			// Pause the periodic finalizer after it observes Finished. The eager
			// finalizer can then encounter a real manifest conflict and publish
			// Retry before the periodic finalizer enters the publication lock.
			observed, release, done := make(chan struct{}), make(chan struct{}), make(chan struct{})
			var calls atomic.Int32
			var originalAggregate func(*externalCollectionRefreshMeta, int64) (indexpb.JobState, int64, error)
			patch := mockey.Mock((*externalCollectionRefreshMeta).AggregateJobStateFromTasks).To(
				func(m *externalCollectionRefreshMeta, jobID int64) (indexpb.JobState, int64, error) {
					state, progress, err := originalAggregate(m, jobID)
					if calls.Add(1) == 1 {
						close(observed)
						<-release
					}
					return state, progress, err
				}).Origin(&originalAggregate).Build()
			defer patch.UnPatch()
			...
			defer func() {
				...
				case <-time.After(5 * time.Second):
					t.Error("delayed finalizer did not exit")
				}
			}()
			go func() {
				defer close(done)
			...
			case <-observed:
			case <-time.After(5 * time.Second):
				t.Fatal("finalizer did not observe Finished")
			}

			// The original attempt used revision 1, but the current manifest is
			// revision 2. Only that attempt should be rejected, leaving its job
			// active until the inspector can publish a replacement.
			checker.aggregateJobState(refreshMeta.GetJob(1))
			require.Equal(t, indexpb.JobState_JobStateRetry, refreshMeta.GetTask(1001).GetState())
			assert.Contains(t, refreshMeta.GetTask(1001).GetFailReason(), "manifest conflict")
			require.Equal(t, indexpb.JobState_JobStateInProgress, refreshMeta.GetJob(1).GetState())
			require.Empty(t, catalog.updateActions)
			releaseDelayed()
			select {
			case <-done:
			case <-time.After(5 * time.Second):
				t.Fatal("stale finalizer did not exit")
			}

			job := refreshMeta.GetJob(1)
			require.Equal(t, indexpb.JobState_JobStateInProgress, job.GetState(),
				"a stale finalizer must not permanently fail a recoverable manifest conflict")
			assert.Zero(t, job.GetIndexWaitStartedTime())
			assert.Empty(t, job.GetFailReason())
			assert.Equal(t, indexpb.JobState_JobStateRetry, refreshMeta.GetTask(1001).GetState())
			active := refreshMeta.GetActiveJobByCollectionID(100)
			require.NotNil(t, active)
			assert.Equal(t, int64(1), active.GetJobId())
			assert.Zero(t, schemaCalls.Load())
			assert.Zero(t, cleanupCalls.Load())
			assert.Zero(t, failureCalls.Load())
			assert.Empty(t, catalog.updateActions)
			assert.Equal(t, current.GetManifestPath(), mt.GetSegment(ctx, 10).GetManifestPath())

			// Resume through the real fresh-ID replacement and result publication
			// APIs. The replacement uses revision 2 and may finish normally.
			inspector := newRefreshInspector(ctx, refreshMeta, nil, make(chan struct{}))
			inspector.allocateTaskID = func(context.Context) (int64, error) { return 1002, nil }
			replacement, err := inspector.buildRetryReplacement(refreshMeta.GetTask(1001))
			require.NoError(t, err)
			replaced, err := refreshMeta.ReplaceRetryTask(1001, replacement)
			require.NoError(t, err)
			require.True(t, replaced)
			require.NoError(t, refreshMeta.StartTaskAttempt(1002, 7, map[int64]int64{10: 2}))
			require.NoError(t, refreshMeta.UpdateTaskResult(1002, indexpb.JobState_JobStateFinished,
				"", nil, []*datapb.SegmentInfo{incoming}))
			checker.aggregateJobState(refreshMeta.GetJob(1))
			...
			assert.Nil(t, refreshMeta.GetTask(1001))
			assert.True(t, isExternalRefreshTaskResultConsumed(refreshMeta.GetTask(1002)))
			assert.Equal(t, incoming.GetManifestPath(), mt.GetSegment(ctx, 10).GetManifestPath())
			assert.Equal(t, int32(1), schemaCalls.Load())
...
func TestExternalCollectionRefreshChecker_FinalizationTimeoutRace(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	timeout := Params.DataCoordCfg.ExternalCollectionJobTimeout.GetAsDuration(time.Second)

	newFixture := func(t *testing.T) (*externalCollectionRefreshMeta, *meta, *externalCollectionRefreshManager, *refreshExternalCollectionTask, *stubCatalog) {
		t.Helper()
		catalog := &stubCatalog{
			jobs: []*datapb.ExternalCollectionRefreshJob{{
				JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateInProgress,
				TaskIds: []int64{1001}, StartTime: time.Now().Add(-timeout - time.Hour).UnixMilli(),
			}},
			tasks: []*datapb.ExternalCollectionRefreshTask{{
				TaskId: 1001, JobId: 1, CollectionId: 100, NodeId: 7,
				State: indexpb.JobState_JobStateInProgress, OwnershipPlanVersion: externalRefreshOwnershipPlanVersion,
			}},
		}
		refreshMeta, err := newExternalCollectionRefreshMeta(ctx, catalog)
		require.NoError(t, err)
		...
		mt := &meta{catalog: catalog, segments: NewSegmentsInfo()}
		manager := &externalCollectionRefreshManager{
			mt: mt, refreshMeta: refreshMeta,
			collectionGetter: testCollectionGetter(newTestCollections(100)),
		}
		manager.checker = newRefreshChecker(ctx, nil, refreshMeta, make(chan struct{}), nil,
			manager.applyFinishedJobSegments, nil, nil, nil)
		return refreshMeta, mt, manager, manager.wrapTask(refreshMeta.GetTask(1001)), catalog
	}
	wait := func(t *testing.T, done <-chan struct{}, description string) {
		t.Helper()
		select {
		case <-done:
		case <-time.After(5 * time.Second):
	...
	t.Run("timeout wins before the in-flight worker query returns", func(t *testing.T) {
		refreshMeta, mt, manager, task, catalog := newFixture(t)
		queryEntered, queryRelease := make(chan struct{}), make(chan struct{})
		var releaseOnce sync.Once
		releaseQuery := func() { releaseOnce.Do(func() { close(queryRelease) }) }
		patch := mockey.Mock((*stubCluster).QueryRefreshExternalCollectionTask).To(
			func(*stubCluster, int64, int64) (*datapb.RefreshExternalCollectionTaskResponse, error) {
				close(queryEntered)
				<-queryRelease
		...
		defer func() {
			releaseQuery()
			wait(t, queryDone, "worker query cleanup")
		}()
		go func() {
			defer close(queryDone)
			task.QueryTaskOnWorker(&stubCluster{})
		}()
		wait(t, queryEntered, "worker query")

		manager.checker.tryTimeoutJob(refreshMeta.GetJob(1))
		require.Equal(t, indexpb.JobState_JobStateFailed, refreshMeta.GetJob(1).GetState())
		require.Nil(t, refreshMeta.GetActiveJobByCollectionID(100))

		releaseQuery()
		wait(t, queryDone, "late worker result")
		assert.Equal(t, indexpb.JobState_JobStateFailed, refreshMeta.GetJob(1).GetState())
		assert.Nil(t, mt.GetSegment(ctx, 10), "a late worker result must not publish segments after timeout")
		assert.Empty(t, catalog.updateActions)
	})
	...
	for _, schemaRetry := range []bool{false, true} {
		name := "finalizer wins"
		if schemaRetry {
			name = "finalizer wins but schema publication needs retry"
		}
		t.Run(name, func(t *testing.T) {
			refreshMeta, mt, manager, task, catalog := newFixture(t)
			getterEntered, getterRelease := make(chan struct{}), make(chan struct{})
			manager.collectionGetter = func(context.Context, int64) (*collectionInfo, error) {
				close(getterEntered)
				<-getterRelease
				return newTestExternalCollectionInfo(100), nil
			}
			if schemaRetry {
				manager.checker.updateJobSchema = func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
					return merr.WrapErrServiceNotReadyMsg("RootCoord temporarily unavailable")
				}
			}

			timeoutReady, timeoutRelease := make(chan struct{}), make(chan struct{})
			var originalTimeout func(*externalCollectionRefreshMeta, int64, int64, string) (bool, error)
			patch := mockey.Mock((*externalCollectionRefreshMeta).TimeoutJob).To(
				func(m *externalCollectionRefreshMeta, jobID, indexWaitStartedTime int64, reason string) (bool, error) {
					// The checker has observed an unfinished worker, but has not
					// acquired the collection lock for its terminal transition yet.
					close(timeoutReady)
					<-timeoutRelease
					return originalTimeout(m, jobID, indexWaitStartedTime, reason)
				}).Origin(&originalTimeout).Build()
			defer patch.UnPatch()
			var releaseTimeoutOnce, releaseGetterOnce sync.Once
			releaseTimeout := func() { releaseTimeoutOnce.Do(func() { close(timeoutRelease) }) }
			releaseGetter := func() { releaseGetterOnce.Do(func() { close(getterRelease) }) }
			timeoutDone, finalizerDone := make(chan struct{}), make(chan struct{})
			finalizerStarted := false
			defer func() {
				releaseTimeout()
				releaseGetter()
				wait(t, timeoutDone, "timeout cleanup")
				if finalizerStarted {
					wait(t, finalizerDone, "finalizer cleanup")
				}
			}()
			go func() {
			...
			wait(t, timeoutReady, "timeout's unfinished-task snapshot")

			var resultErr error
			finalizerStarted = true
			go func() {
				defer close(finalizerDone)
				resultErr = task.UpdateResultWithMeta(indexpb.JobState_JobStateFinished, "", nil,
					[]*datapb.SegmentInfo{newTestExternalRefreshSegment(10, 100, 7)})
			}()
			wait(t, getterEntered, "finalizer collection lookup")
			locked := refreshMeta.jobLock.TryLock(100)
			if locked {
				refreshMeta.jobLock.Unlock(100)
			}
			assert.False(t, locked, "segment publication must retain collection ownership while applying")
			require.Equal(t, indexpb.JobState_JobStateInProgress, refreshMeta.GetJob(1).GetState())

			// Do not wait for timeoutDone while the finalizer holds its lock.
			// The stale timeout must recheck the finished worker after it acquires
			// that lock, including when schema publication leaves the job active.
			releaseTimeout()
			releaseGetter()
			wait(t, finalizerDone, "segment finalization")
			wait(t, timeoutDone, "stale timeout")
			require.NoError(t, resultErr)
			if schemaRetry {
				assert.Equal(t, indexpb.JobState_JobStateInProgress, refreshMeta.GetJob(1).GetState())
				require.NotNil(t, refreshMeta.GetActiveJobByCollectionID(100))
			} else {
				assert.Equal(t, indexpb.JobState_JobStateFinished, refreshMeta.GetJob(1).GetState())
			}
			require.NotNil(t, mt.GetSegment(ctx, 10))
			assert.Equal(t, commonpb.SegmentState_Flushed, mt.GetSegment(ctx, 10).GetState())
			assert.Len(t, catalog.updateActions, 1, "publish the segment and consume its task result together")
			assert.True(t, isExternalRefreshTaskResultConsumed(refreshMeta.GetTask(1001)))
		})
	}
}
...
func TestExternalCollectionRefreshChecker_AggregateJobState(t *testing.T) {
	...
	t.Run("update_to_finished", func(t *testing.T) {
		...
		closeChan := make(chan struct{})
		cleanupCalls := 0
		checker := newRefreshChecker(ctx, nil, meta, closeChan, nil, nil, nil, func(jobID int64) error {
			cleanupCalls++
			assert.Equal(t, int64(1), jobID)
			assert.Equal(t, indexpb.JobState_JobStateInProgress, meta.GetJob(jobID).GetState(),
				"Explore cleanup must finish before Finished is published")
			return nil
		}, nil)
	...
	t.Run("finished_cleanup_failure_retries", func(t *testing.T) {
		catalog := &stubCatalog{}
		jobs := []*datapb.ExternalCollectionRefreshJob{
		...
		meta, _ := newExternalCollectionRefreshMeta(ctx, catalog)
		cleanupCalls := 0
		checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}), nil, nil, nil, func(int64) error {
			cleanupCalls++
			if cleanupCalls == 1 {
		...
		assert.Equal(t, indexpb.JobState_JobStateInProgress, meta.GetJob(1).GetState(),
			"cleanup failure must leave the job as the retry anchor")
...
func TestExternalCollectionRefreshChecker_TryTimeoutJob(t *testing.T) {
	...
	// Regression: when the checker's tryTimeoutJob races with the eager path and
	// observes a stale InProgress snapshot that a concurrent task-success path
	// has already transitioned to Finished, the UpdateJobState terminal guard
	// returns applied=false. In that case tryTimeoutJob must not run the Failed
	// callback or rewrite tasks; only the path that persisted the terminal state
	// owns those follow-up actions.
...
func TestExternalCollectionRefreshChecker_RetentionZeroDropsWorkerBeforeMeta(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	...
	assert.NoError(t, err)
	checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}), nil, nil, nil, nil, nil)
	checker.cleanupJobResources = func(jobID int64) error {
		assert.Equal(t, int64(1), jobID)
		assert.NotNil(t, meta.GetJob(jobID), "resource cleanup must run while durable meta can still anchor a retry")
		return nil
	}

	dropCalled := false
	checker.dropJobTasks = func(jobID int64) error {
		dropCalled = true
		assert.Equal(t, int64(1), jobID)
		assert.NotNil(t, meta.GetJob(1), "authoritative job meta must anchor worker cleanup")
...
func TestExternalCollectionRefreshChecker_GCWorkerDropFailureKeepsMeta(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	oldRetention := Params.DataCoordCfg.ExternalCollectionJobRetention.GetValue()
	Params.DataCoordCfg.ExternalCollectionJobRetention.SwapTempValue("0")
	t.Cleanup(func() { Params.DataCoordCfg.ExternalCollectionJobRetention.SwapTempValue(oldRetention) })
	...
	checker.dropJobTasks = func(jobID int64) error {
		dropCalls++
		assert.Equal(t, int64(1), jobID)
		assert.NotNil(t, meta.GetJob(jobID))
		if dropCalls == 1 {
			return merr.WrapErrServiceUnavailableMsg("worker temporarily unavailable")
		}
		return nil
	}
...
func TestExternalCollectionRefreshChecker_GCCleanupFailureKeepsMeta(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	oldRetention := Params.DataCoordCfg.ExternalCollectionJobRetention.GetValue()
	Params.DataCoordCfg.ExternalCollectionJobRetention.SwapTempValue("0")
	...
	assert.NoError(t, err)
	cleanupErr := errors.New("object cleanup failed")
	checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}), nil, nil, nil,
		func(int64) error { return cleanupErr }, nil)
...
func TestExternalCollectionRefreshChecker_AggregateJobState_ConcurrentFinalizationIsIdempotent(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()

	applyStarted, applyRelease := make(chan struct{}), make(chan struct{})
	var applyOnce, releaseOnce sync.Once
	releaseApply := func() { releaseOnce.Do(func() { close(applyRelease) }) }
	catalog := &controlledRefreshCatalog{
		...
		onUpdate: func() {
			applyOnce.Do(func() { close(applyStarted) })
			<-applyRelease
		},
	}
	refreshMeta, err := newExternalCollectionRefreshMeta(ctx, catalog)
	...
	addManagerOwnershipTask(t, refreshMeta, &datapb.ExternalCollectionRefreshTask{
		TaskId: 1001, JobId: 1, CollectionId: 100,
		State: indexpb.JobState_JobStateFinished, ResultReady: true,
		UpdatedSegments: []*datapb.SegmentInfo{newTestExternalRefreshSegment(10, 100, 7)},
	})
	mt := &meta{catalog: catalog, segments: NewSegmentsInfo()}
	manager := &externalCollectionRefreshManager{
	...
	checker := newRefreshChecker(ctx, nil, refreshMeta, make(chan struct{}), nil,
		manager.applyFinishedJobSegments, nil, nil, nil)
	...
	defer func() {
		...
		case <-done:
		case <-time.After(5 * time.Second):
			t.Error("concurrent finalizers did not stop")
		}
	}()
	close(start)
	select {
	case <-applyStarted:
	case <-time.After(5 * time.Second):
		t.Fatal("segment publication did not start")
	}

	// One real catalog publication is in progress. A second finalizer must
	// wait, then observe the consumed task result instead of publishing again.
	locked := refreshMeta.jobLock.TryLock(100)
	if locked {
		refreshMeta.jobLock.Unlock(100)
	}
	...
	case <-done:
	case <-time.After(5 * time.Second):
		t.Fatal("concurrent finalizers did not complete")
	}
...
func TestExternalCollectionRefreshChecker_TryTimeoutJob_CatalogErrorFailsStop(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	...
	mockSaveJob := mockey.Mock((*stubCatalog).SaveExternalCollectionRefreshJob).Return(writeErr).Build()
	defer mockSaveJob.UnPatch()
	...
	// The hooked Fatal returns in the test, but the ambiguous write must not
	// update in-memory state or continue as an ordinary retry.
	assert.Equal(t, indexpb.JobState_JobStateInProgress, meta.GetJob(1).GetState())
}
...
func TestExternalCollectionRefreshChecker_FinalizesBeforeFinished(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	...
	mockSaveJob := mockey.Mock((*stubCatalog).SaveExternalCollectionRefreshJob).
		To(func(_ context.Context, job *datapb.ExternalCollectionRefreshJob) error {
			steps = append(steps, job.GetState().String())
			return nil
		}).Build()
	defer mockSaveJob.UnPatch()

	updateSchema := func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
		steps = append(steps, "schema")
		return nil
	}
	applySegments := func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
		steps = append(steps, "segments")
		return nil
	}
	checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}), updateSchema, applySegments, nil, nil, nil)

	checker.processJobs()

	assert.Equal(t, []string{"segments", "schema", indexpb.JobState_JobStateFinished.String()}, steps)
	assert.Equal(t, indexpb.JobState_JobStateFinished, meta.GetJob(1).GetState())
}

func TestExternalCollectionRefreshChecker_FinalizationFailureRetriesBeforeFinished(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
...
func TestExternalCollectionRefreshChecker_SegmentApplyFailureRetriesBeforeSchema(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	...
	meta, err := newExternalCollectionRefreshMeta(ctx, &stubCatalog{jobs: jobs, tasks: tasks})
	require.NoError(t, err)
	mockSaveJob := mockey.Mock((*stubCatalog).SaveExternalCollectionRefreshJob).Return(nil).Build()
	defer mockSaveJob.UnPatch()
	...
	applyCalls := 0
	schemaCalls := 0
	checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}),
		func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
			schemaCalls++
			return nil
		},
		func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
			applyCalls++
			if applyCalls == 1 {
				return merr.WrapErrServiceUnavailableMsg("segment catalog temporarily unavailable")
			}
			return nil
		}, nil, nil, nil)
...
func TestExternalCollectionRefreshChecker_PermanentFinalizationFailureMarksJobFailed(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	...
	checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}),
		...
		func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
			return merr.WrapErrDataIntegrityMsg("finished task has malformed result")
		},
		func(int64) { failedCallbacks++ }, nil, nil)
	...
	assert.Contains(t, job.GetFailReason(), "malformed result")
	assert.Zero(t, schemaCalls)
	assert.Equal(t, 1, failedCallbacks)
}

func TestExternalCollectionRefreshChecker_FinishedCatalogFailureFailsStop(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	...
	mockSaveJob := mockey.Mock((*stubCatalog).SaveExternalCollectionRefreshJob).
		To(func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
			saveCalls.Add(1)
			return writeErr
		}).Build()
	defer mockSaveJob.UnPatch()
...
func TestExternalCollectionRefreshChecker_DoesNotTimeoutCompletedExecutionDuringFinalization(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	timeout := Params.DataCoordCfg.ExternalCollectionJobTimeout.GetAsDuration(time.Second)
	...
	jobs := []*datapb.ExternalCollectionRefreshJob{
		{
			JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateInProgress,
			StartTime: time.Now().Add(-timeout - time.Hour).UnixMilli(), TaskIds: []int64{1001},
		},
	}
	tasks := []*datapb.ExternalCollectionRefreshTask{
		{TaskId: 1001, JobId: 1, State: indexpb.JobState_JobStateFinished, Progress: 100},
	...
	checker := newRefreshChecker(ctx, nil, meta, make(chan struct{}), nil,
		func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
			return errors.New("segment catalog temporarily unavailable")
		}, nil, nil, nil)
	checker.processJobs()
...
func TestExternalCollectionRefreshChecker_DoesNotReplaySchemaForRetainedTerminalJobs(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()

	jobs := []*datapb.ExternalCollectionRefreshJob{
		{JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateFinished, IndexWaitStartedTime: 1},
		{JobId: 2, CollectionId: 100, State: indexpb.JobState_JobStateFailed, IndexWaitStartedTime: 1},
	}
	meta, err := newExternalCollectionRefreshMeta(ctx, &stubCatalog{jobs: jobs})
	require.NoError(t, err)
	...
	assert.Zero(t, schemaCalls,
		"retained terminal jobs must not overwrite schema published by a newer refresh")
}

func TestExternalCollectionRefreshChecker_DoesNotTimeoutCompletedIndexWaitDuringCleanupRetry(t *testing.T) {
	ctx := context.Background()
	paramtable.Init()
	pt := paramtable.Get()
	...
	jobs := []*datapb.ExternalCollectionRefreshJob{{
		...
		CollectionId:         100,
		State:                indexpb.JobState_JobStateInProgress,
		StartTime:            time.Now().Add(-timeout - time.Hour).UnixMilli(),
		TaskIds:              []int64{1001},
	...
	refreshMeta, err := newExternalCollectionRefreshMeta(ctx, &stubCatalog{jobs: jobs, tasks: tasks})
	require.NoError(t, err)
	...
	checker := newRefreshChecker(ctx, mt, refreshMeta, make(chan struct{}), nil,
		...
		func(int64) { failedCalls++ },
		func(int64) error {
			cleanupCalls++
			if cleanupCalls == 1 {
				return errors.New("temporary object storage error")
			}
			return nil
		}, nil)
...
func TestExternalCollectionRefreshChecker_IndexWait(t *testing.T) {
	...
	t.Run("the apply callback completes before the wait marker is published", func(t *testing.T) {
		// Production's apply callback atomically consumes task results with the
		// segment adoption. This checker-level test only verifies its ordering;
		// the manager/meta tests verify the composite catalog mutation itself.
		mockey.PatchConvey("apply before wait marker", t, func() {
			pt := paramtable.Get()
			pt.Save(pt.DataCoordCfg.RefreshWaitForIndex.Key, "true")
			defer pt.Reset(pt.DataCoordCfg.RefreshWaitForIndex.Key)
			...
			var markerSeenDuringApply int64
			checker := newRefreshChecker(ctx, mt, refreshMeta, make(chan struct{}), nil,
				func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
					markerSeenDuringApply = refreshMeta.GetJob(1).GetIndexWaitStartedTime()
					return nil
				},
				nil, nil, nil)
			...
			assert.Zero(t, markerSeenDuringApply)
			assert.NotZero(t, waiting.GetIndexWaitStartedTime())
		})
	})
	...
	t.Run("a refresh whose segments are all already indexed finishes in the entry pass", func(t *testing.T) {
		...
		mockey.PatchConvey("no wait when nothing is owed", t, func() {
			...
			assert.Equal(t, 1, cleanupCalls, "the index-wait finish uses the same cleanup path")
	...
	t.Run("a transient pre-apply failure stays active and retries", func(t *testing.T) {
		mockey.PatchConvey("transient entry apply failure", t, func() {
			pt := paramtable.Get()
			pt.Save(pt.DataCoordCfg.RefreshWaitForIndex.Key, "true")
			defer pt.Reset(pt.DataCoordCfg.RefreshWaitForIndex.Key)
			...
			assert.Equal(t, indexpb.JobState_JobStateInProgress, got.GetState())
			assert.Zero(t, got.GetIndexWaitStartedTime(),
				"a failed apply must not claim that index wait has started")
			assert.Empty(t, failed)

			checker.aggregateJobState(got)

			retried := refreshMeta.GetJob(1)
			assert.Equal(t, indexpb.JobState_JobStateInProgress, retried.GetState())
			assert.NotZero(t, retried.GetIndexWaitStartedTime())
			assert.Equal(t, 2, applyCalls)
			assert.Empty(t, failed)
		})
	})

	t.Run("a permanent pre-apply failure fails the job", func(t *testing.T) {
		mockey.PatchConvey("permanent entry apply failure", t, func() {
			pt := paramtable.Get()
			pt.Save(pt.DataCoordCfg.RefreshWaitForIndex.Key, "true")
			defer pt.Reset(pt.DataCoordCfg.RefreshWaitForIndex.Key)
			...
			assert.Contains(t, got.GetFailReason(), "malformed result")
			assert.Zero(t, got.GetIndexWaitStartedTime())
			assert.Equal(t, []int64{1}, failed)
		})
	})

	t.Run("failed result consumption does not enter the wait", func(t *testing.T) {
		// Result consumption is part of segment adoption, not an optimization
		// after it. If that apply callback fails, no wait marker may be published;
		// the next checker pass retries the same pending result.
		mockey.PatchConvey("release fails", t, func() {
			...
			checker := newRefreshChecker(ctx, mt, refreshMeta, make(chan struct{}), nil,
				func(context.Context, *datapb.ExternalCollectionRefreshJob) error {
					return refreshMeta.ClearTaskResultsByJobID(1)
				},
				nil, nil, nil)
			...
			held := refreshMeta.GetJob(1)
			require.Equal(t, indexpb.JobState_JobStateInProgress, held.GetState())
			require.Zero(t, held.GetIndexWaitStartedTime(), "a failed apply cannot publish the wait marker")
			...
			assert.Equal(t, indexpb.JobState_JobStateInProgress, refreshMeta.GetJob(1).GetState())
			assert.Zero(t, refreshMeta.GetJob(1).GetIndexWaitStartedTime())
			assert.Equal(t, 2, clears, "the pending apply is retried on the next checker pass")
		})
	})
...
// the SAME processJob pass: aggregateJobState applies its segments, stamps the
// marker (the job stays InProgress), and synchronously publishes the matching
// schema through syncAppliedJobSchema before index dispatch. tryTimeoutJob then
// persists Failed and fires onJobFailed. If publication were deferred until
// after the timeout transition, the collection could serve refreshed segments
// under the pre-refresh external_source/spec with no successful publish anchor.
//
...
func TestExternalCollectionRefreshChecker_IndexWait_TimeoutInEntryPassPublishes(t *testing.T) {
	...
	mockey.PatchConvey("timeout in the entry pass", t, func() {
		...
		mgr := NewExternalCollectionRefreshManager(ctx, mt, newStubScheduler(), &stubAllocator{},
			refreshMeta, nil, testCollectionGetter(collections),
		...
		checker := newRefreshChecker(ctx, mt, refreshMeta, make(chan struct{}),
			mgr.syncJobSchema,
			func(context.Context, *datapb.ExternalCollectionRefreshJob) error { return nil },
			mgr.handleJobFailed, mgr.handleJobCleanup, nil)
...
// nudge issued before syncAppliedJobSchema completes dispatches against the
// pre-refresh endpoint and credentials. Such builds fail terminally and nothing
// retries them, so the refresh would burn its whole timeout and then fail with
// its segments already serving.
func TestExternalCollectionRefreshChecker_IndexWait_NudgeWaitsForPublish(t *testing.T) {
	...
	mockey.PatchConvey("nudge waits for the publish", t, func() {
		...
		published := false
		mockey.Mock((*indexMeta).GetUnindexedSegments).To(
			func(_ *indexMeta, _ int64, _ []int64) []int64 {
				assert.True(t, published, "schema publication must complete before index debt is evaluated")
				return []int64{556}
			}).Build()
		...
		assert.Equal(t, []int64{556}, drainBuildIndexCh(),
			"the wait accelerates the build only after schema publication succeeds")
	})
}
...
func TestExternalCollectionRefreshChecker_IndexWait_DataStaysQueryVisible(t *testing.T) {
	...
	stage := func(t *testing.T) (*externalCollectionRefreshMeta, *meta, *ServerHandler) {
		...
		mockey.Mock((*Server).getCollectionFromRootCoord).Return(&collectionInfo{
			ID:     collectionID,
