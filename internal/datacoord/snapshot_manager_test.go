...
func TestSnapshotManager_BuildChannelMapping_Success(t *testing.T) {
	...
	snapshotData := &SnapshotData{
		Segments: []*datapb.SegmentDescription{
			{SegmentId: 1, ChannelName: "ch1"},
			{SegmentId: 2, ChannelName: "ch2"},
			{SegmentId: 3, ChannelName: "ch1"}, // Duplicate channel
		},
	}
	...
	getChannelsFunc := func(ctx context.Context, collectionID int64) ([]RWChannel, error) {
		return []RWChannel{
			&channelMeta{Name: "target_ch1"},
			&channelMeta{Name: "target_ch2"},
		}, nil
	}
	...
	// Sorted mapping: ch1 -> target_ch1, ch2 -> target_ch2
	assert.Equal(t, "target_ch1", mapping["ch1"])
	assert.Equal(t, "target_ch2", mapping["ch2"])
}
...
func TestSnapshotManager_RestoreSnapshot_Success(t *testing.T) {
	...
	snapshotData := &SnapshotData{
		...
		Segments: []*datapb.SegmentDescription{
			{SegmentId: 1, ChannelName: "ch1", PartitionId: 1},
			{SegmentId: 2, ChannelName: "ch2", PartitionId: 2},
		},
	}
