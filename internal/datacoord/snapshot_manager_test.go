...
func TestSnapshotManager_JSONArtifactVersions(t *testing.T) {
	...
	// Copying V4 files is a publication too: compatible readers alone cannot
	// protect the destination's V3 files from a pre-3.0.2 standby coordinator.
	require.NoError(t, Params.Save(Params.DataCoordCfg.JSONStatsFormatVersion.Key, "auto"))
	require.ErrorIs(t, sm.checkSnapshotJSONArtifactVersions(data), merr.ErrServiceNotReady)
	_, err := sm.finishRestoreSnapshot(context.Background(), nil, data, "s", 1, "target", "default", 0, false, "", "", nil, nil, nil)
	require.ErrorIs(t, err, merr.ErrServiceNotReady) // before target DDL via the nil broker
	require.NoError(t, Params.Save(Params.DataCoordCfg.JSONStatsFormatVersion.Key, "4"))
