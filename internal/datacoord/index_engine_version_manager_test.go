...
func Test_IndexEngineVersionManager_BoundWorkerPreservesScalarReaderChecks(t *testing.T) {
	params := paramtable.Get()
	previous := params.DataCoordCfg.BindIndexNodeMode.GetValue()
	params.Save(params.DataCoordCfg.BindIndexNodeMode.Key, "true")
	t.Cleanup(func() { params.Save(params.DataCoordCfg.BindIndexNodeMode.Key, previous) })

	manager := newIndexEngineVersionManager()
	gate := manager.(ScalarIndexMigrationVersionManager)
	target := common.MinScalarIndexVersionForJsonPathPresence
	assert.False(t, gate.SupportsScalarIndexVersion(target), "readers are still required")
	manager.AddNode(scalarVersionSession(1, target))
	assert.True(t, gate.SupportsScalarIndexVersion(target), "bound writers need no session")
	assert.False(t, gate.SupportsScalarIndexVersion(0))
	assert.False(t, gate.SupportsScalarIndexVersion(target+1))

	manager.AddNode(scalarVersionSession(2, target-1))
	assert.False(t, gate.SupportsScalarIndexVersion(target), "every reader must support the target")
	malformedReader := scalarVersionSession(2, target)
	malformedReader.ScalarIndexEngineVersion.MaximumIndexVersion = 0
	manager.Update(malformedReader)
	assert.False(t, gate.SupportsScalarIndexVersion(target), "missing reader capabilities are not trusted")
	manager.Update(scalarVersionSession(2, target))
	assert.True(t, gate.SupportsScalarIndexVersion(target))

	// No capabilities are fabricated: normal discovery still fails closed.
	params.Save(params.DataCoordCfg.BindIndexNodeMode.Key, "false")
	assert.False(t, gate.SupportsScalarIndexVersion(target))
}
...
func TestEmptySessionSetBoundsAnOverrideByThisBinary(t *testing.T) {
	...
	assert.Equal(t, common.MaximumScalarIndexEngineVersion, m.GetMaximumScalarIndexEngineVersion())
	...
	assert.Equal(t, common.MaximumScalarIndexEngineVersion, m.ResolveScalarIndexVersion())
}
