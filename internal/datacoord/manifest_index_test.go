...
func TestReloadSegmentIndexesFromManifests(t *testing.T) {
	...
	m.reloadSegmentIndexesFromManifests(context.TODO())
...
// An unreadable manifest degrades to "that segment looks unindexed" - the
// index inspector reissues the build - rather than failing DataCoord startup.
func TestReloadSegmentIndexesFromManifests_UnreadableManifestIsSkipped(t *testing.T) {
	withSegmentIndexEtcdWrites(t, false)
	m := setupManifestReloadMeta(t)
	...
	assert.NotPanics(t, func() { m.reloadSegmentIndexesFromManifests(context.TODO()) })
