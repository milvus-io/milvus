...
func (t *importTask) CreateTaskOnWorker(nodeID int64, cluster session.Cluster) {
	...
	if t.importMeta.GetTask(logCtx, t.GetTaskID()) == nil {
	...
	if job == nil || job.GetState() == internalpb.ImportJobState_Failed ||
		job.GetState() == internalpb.ImportJobState_Completed {
		// This wrapper is still shared with importMeta. Leave its state intact
		// so the checker can fail the task and the inspector can retire its
		// segments. The scheduler releases Pending when Create returns.
		return
	}
