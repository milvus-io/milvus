...
func (t *compactionTrigger) ShouldDoSingleCompaction(segment *SegmentInfo, isDiskIndex bool, compactTime *compactTime) bool {
	...
	if len(segment.CompactionFrom) == 0 {
		...
		var maxSize int
		if isDiskIndex {
			maxSize = int(int64(Params.DataCoordCfg.DiskSegmentMaxSize*1024*1024) / Params.DataNodeCfg.BinLogMaxSize)
		} else {
			maxSize = int(int64(Params.DataCoordCfg.SegmentMaxSize*1024*1024) / Params.DataNodeCfg.BinLogMaxSize)
		}
		...
		// By default SegmentMaxSize=512MB, BinLogMaxSize=64MB, the maxSize is 8
		// Note: 2023-7-24 change the ratio to 4.0 to avoid unnecessary compactions for segments generated by bulkinsert.
		// Typically, segment generated by bulkinsert has 20~30 statslog files for size 512MB.
		if statsLog > maxSize*4.0 {
	...
	// avoid segment has too many delta logs and the etcd meta is too large, force trigger compaction
	if deltaLog > int(Params.DataCoordCfg.SingleCompactionDeltalogMaxNum) {
