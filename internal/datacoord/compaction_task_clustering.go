...
func (t *clusteringCompactionTask) markResultSegmentsVisible() error {
	...
	for _, segID := range t.GetTaskProto().GetResultSegments() {
		mutations[segID] = []SegmentOperator{func(seg *SegmentInfo) (BinlogIncrement, bool) {
			seg.IsInvisible = false
			seg.LastPartitionStatsVersion = seg.PartitionStatsVersion
			seg.PartitionStatsVersion = planID
...
func (t *clusteringCompactionTask) doClean() error {
	...
	if t.GetTaskProto().GetState() == datapb.CompactionTaskState_completed {
		...
		if isInputDropped {
			...
			for _, segID := range t.GetTaskProto().GetInputSegments() {
				mutations[segID] = []SegmentOperator{func(seg *SegmentInfo) (BinlogIncrement, bool) {
					if seg.GetLevel() != seg.GetLastLevel() && seg.GetLastLevel() != datapb.SegmentLevel_Legacy {
						seg.Level = seg.LastLevel
					}
					return BinlogIncrement{}, true
				}}
			}
			...
			for _, segID := range t.GetTaskProto().GetResultSegments() {
				mutations[segID] = []SegmentOperator{func(seg *SegmentInfo) (BinlogIncrement, bool) {
					seg.LastLevel = seg.Level
					seg.Level = datapb.SegmentLevel_L1
					seg.LastPartitionStatsVersion = seg.PartitionStatsVersion
					seg.PartitionStatsVersion = 0
					return BinlogIncrement{}, true
				}}
			}
			...
			mutations := map[int64][]SegmentOperator{}
			dropMutate := func(seg *SegmentInfo) (BinlogIncrement, bool) {
				seg.State = commonpb.SegmentState_Dropped
				seg.DroppedAt = uint64(time.Now().UnixNano())
				return BinlogIncrement{}, true
			}
