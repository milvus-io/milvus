...
func (t *clusteringCompactionTask) completeTask() error {
	...
	err = t.meta.GetPartitionStatsMeta().SaveCurrentPartitionStatsVersion(t.GetTaskProto().GetCollectionID(), t.GetTaskProto().GetPartitionID(), t.GetTaskProto().GetChannel(), t.GetTaskProto().GetPlanID())
	if err != nil {
...
func (t *clusteringCompactionTask) processFailedOrTimeout() error {
	...
	for _, segID := range t.GetTaskProto().ResultSegments {
