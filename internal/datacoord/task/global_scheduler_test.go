...
//
// Terminal cleanup is detached too: the drops run on their own goroutines, and
// runRoundSync drains them so a test can assert on the post-drop state without
// racing the cleanup.
func runRoundSync(s *globalTaskScheduler) {
	...
	s.schedule(pending)
	s.check(running)
	waitDropsDrained(s, 10*time.Second)
}

// waitDropsDrained blocks until every queued terminal drop has finished. The
// drop goroutines are what release ownership, so tests that assert a terminal
// task is gone (or was dropped on its worker) must wait for them.
func waitDropsDrained(s *globalTaskScheduler, timeout time.Duration) {
	deadline := time.Now().Add(timeout)
	for s.droppingCount.Load() > 0 {
		if time.Now().After(deadline) {
			panic("timed out waiting for terminal drops to drain")
		}
...
// blockingDropTask is a terminal task whose DropTaskOnWorker blocks until
// released, standing in for a worker that stops answering its Drop RPC (the
// RPC is bounded only by dataCoord.requestTimeoutSeconds).
type blockingDropTask struct {
	*schedulerTestTask
	dropStarted chan struct{}
	dropRelease chan struct{}
}

func (t *blockingDropTask) DropTaskOnWorker(session.Cluster) {
	close(t.dropStarted)
	<-t.dropRelease
}

// A worker that stops answering must not hold up dispatch of unrelated tasks:
// round() must hand pending tasks to workers while a terminal task's Drop RPC
// is still hanging.
func TestGlobalScheduler_HangingDropDoesNotStallDispatch(t *testing.T) {
	cluster := &schedulerTestCluster{slots: map[int64]*session.WorkerSlots{
		1: {NodeID: 1, AvailableSlots: 10},
	}}
	scheduler := NewGlobalTaskScheduler(context.TODO(), cluster).(*globalTaskScheduler)

	// A terminal task whose Drop RPC hangs on the worker.
	ended := &blockingDropTask{
		schedulerTestTask: newSchedulerTestTask(1),
		dropStarted:       make(chan struct{}),
		dropRelease:       make(chan struct{}),
	}
	ended.state.Store(int32(taskcommon.Failed))
	ownTask(scheduler, ended)

	// A pending task that must still be dispatched while the drop hangs.
	pending := newSchedulerTestTask(2)
	ownTask(scheduler, pending)

	// A full round: it queues the ended task's drop and dispatches the pending
	// one. The drop goroutine blocks on the worker; schedule must not wait for
	// it.
	pendingTasks, _, endedTasks := scheduler.partition()
	scheduler.releaseEndedTaskOwnership(endedTasks)
	scheduler.schedule(pendingTasks)

	select {
	case <-ended.dropStarted:
	case <-time.After(5 * time.Second):
		t.Fatal("drop RPC never started")
	}

	// The pending task is dispatched even though a Drop RPC is hanging.
	assert.Equal(t, int64(1), pendingTasks[0].(*schedulerTestTask).nodeID.Load(),
		"a hanging Drop RPC must not block dispatch")
	assert.Equal(t, int32(taskcommon.InProgress), pendingTasks[0].(*schedulerTestTask).state.Load())

	// Release the drop and drain so the test does not leak the goroutine.
	close(ended.dropRelease)
	waitDropsDrained(scheduler, 5*time.Second)
}

// A delayed Drop RPC must not delete a same-ID retry that was admitted after
// ownership was released. The scheduler queues the drop and only releases
// ownership when it returns, so the retry cannot be dispatched ahead of it.
func TestGlobalScheduler_DelayedDropDoesNotDeleteSameIDRetry(t *testing.T) {
	cluster := &schedulerTestCluster{slots: map[int64]*session.WorkerSlots{
		1: {NodeID: 1, AvailableSlots: 10},
	}}
	scheduler := NewGlobalTaskScheduler(context.TODO(), cluster).(*globalTaskScheduler)

	// A terminal task whose drop is slow (in flight for a while).
	ended := &blockingDropTask{
		schedulerTestTask: newSchedulerTestTask(1),
		dropStarted:       make(chan struct{}),
		dropRelease:       make(chan struct{}),
	...
	// A retry of the same task ID, submitted while the drop is still hanging.
	retry := newSchedulerTestTask(1)

	scheduler.releaseEndedTaskOwnership([]Task{ended})

	// While the drop is in flight, ownership is retained: Enqueue of the same
	// ID is a no-op, so the retry cannot reach the worker before the Drop.
	select {
	case <-ended.dropStarted:
	case <-time.After(5 * time.Second):
		t.Fatal("drop RPC never started")
	}
	assert.True(t, scheduler.tasks.Contain(1), "ownership must be retained while the drop is in flight")
	scheduler.Enqueue(retry)
	assert.True(t, scheduler.tasks.Contain(1))
	owned, _ := scheduler.tasks.Get(1)
	assert.Equal(t, Task(ended), owned, "the retry must not be admitted while the old attempt's drop is pending")

	// After the drop returns, ownership is released and the retry can land.
	close(ended.dropRelease)
	waitDropsDrained(scheduler, 5*time.Second)
	assert.False(t, scheduler.tasks.Contain(1))
	scheduler.Enqueue(retry)
	assert.True(t, scheduler.tasks.Contain(1), "the retry must be admitted after the old drop returned")
}

// A hanging Drop must not stall the round that follows the one that queued it.
// The drop goroutine holds the key lock across its RPC; a later round must skip
// an in-flight drop instead of blocking on that lock.
func TestGlobalScheduler_HangingDropDoesNotStallNextRound(t *testing.T) {
	cluster := &schedulerTestCluster{slots: map[int64]*session.WorkerSlots{
		1: {NodeID: 1, AvailableSlots: 10},
	}}
	scheduler := NewGlobalTaskScheduler(context.TODO(), cluster).(*globalTaskScheduler)

	// A terminal task whose Drop RPC hangs.
	ended := &blockingDropTask{
		schedulerTestTask: newSchedulerTestTask(1),
		dropStarted:       make(chan struct{}),
		dropRelease:       make(chan struct{}),
	...
	// Round 1 queues the drop.
	_, _, ended1 := scheduler.partition()
	scheduler.releaseEndedTaskOwnership(ended1)
	select {
	case <-ended.dropStarted:
	case <-time.After(5 * time.Second):
		t.Fatal("drop RPC never started")
	}
	assert.True(t, scheduler.tasks.Contain(1), "ownership retained while the drop is in flight")

	// Round 2 sees the same terminal task (still owned, drop still hanging) and
	// must dispatch unrelated pending work instead of blocking on its lock.
	pending2 := newSchedulerTestTask(2)
	ownTask(scheduler, pending2)
	pending, _, ended2 := scheduler.partition()
	assert.Len(t, pending, 1, "the pending task must be schedulable while the drop hangs")
	scheduler.releaseEndedTaskOwnership(ended2)
	scheduler.schedule(pending)

	assert.Equal(t, int64(1), pending[0].(*schedulerTestTask).nodeID.Load(),
		"the round after a queued drop must not block on the in-flight drop's lock")
	assert.Equal(t, int32(taskcommon.InProgress), pending[0].(*schedulerTestTask).state.Load())
...
type statsCapabilityTestTask struct{ *schedulerTestTask }

func (t *statsCapabilityTestTask) CanRunOnNode(_ int64, slots *session.WorkerSlots) bool {
	return slots.SupportsV3StatsAttemptPath
}

func TestGlobalScheduler_StatsCapabilityDoesNotBlockOtherTasks(t *testing.T) {
	cluster := &schedulerTestCluster{slots: map[int64]*session.WorkerSlots{
		1: {AvailableSlots: 4},
	}}
	...
	ownTask(scheduler, blocked)
	ownTask(scheduler, ordinary)
	scheduler.schedule([]Task{blocked, ordinary})
	assert.Equal(t, taskcommon.Init, blocked.GetTaskState())
	assert.Equal(t, int64(1), ordinary.nodeID.Load())
	assert.Equal(t, int64(3), cluster.slots[1].AvailableSlots)
	cluster.slots[2] = &session.WorkerSlots{AvailableSlots: 2, SupportsV3StatsAttemptPath: true}
	scheduler.schedule([]Task{blocked})
	assert.Equal(t, int64(2), blocked.nodeID.Load())
	assert.Equal(t, int64(3), cluster.slots[1].AvailableSlots, "incompatible node loses no slots")
}
