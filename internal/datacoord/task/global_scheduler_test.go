...
func TestGlobalScheduler_FailedTaskBacksOffBeforeRedispatch(t *testing.T) {
	pt := paramtable.Get()
	// Keep ticks faster than the backoff so this test detects missing backoff.
	pt.Save(pt.DataCoordCfg.TaskScheduleInterval.Key, "100")
	defer pt.Reset(pt.DataCoordCfg.TaskScheduleInterval.Key)
	pt.Save(pt.DataCoordCfg.TaskRetryBackoffInterval.Key, "1")
	defer pt.Reset(pt.DataCoordCfg.TaskRetryBackoffInterval.Key)
