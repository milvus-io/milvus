...
// maxConcurrentDropRPCs bounds how many terminal worker-drop RPCs the scheduler
// keeps in flight at once. Each drop is a worker RPC bounded only by
// dataCoord.requestTimeoutSeconds, so an unbounded number of them would let a
// partially-answering fleet exhaust datanode gRPC concurrency. The bound is a
// count, not a pool: submitting to a bounded pool would block round() when it
// saturates, which is the stall this detachment exists to remove.
const maxConcurrentDropRPCs = 32
...
type globalTaskScheduler struct {
	...
	// a single map lookup, not an agreement between two containers. Keeping it
	// ordered by task ID gives older tasks priority without sorting every round.
	tasks *orderedTasks
	...
	// dropping marks task IDs whose terminal worker Drop is in flight, so a
	// round does not stack a second Drop behind the first. Ownership is
	// retained until the Drop returns, so the map key is what keeps a same-ID
	// retry fenced during cleanup. droppingCount is its concurrency limiter;
	// see maxConcurrentDropRPCs.
	dropping      sync.Map // map[int64]struct{}
	droppingCount atomic.Int32
}
...
func (s *globalTaskScheduler) createUnderTaskLock(task Task, nodeID int64) {
	...
	case taskcommon.None, taskcommon.Init, taskcommon.Retry:
		...
		s.releaseUnderTaskLock(taskID)
	default:
		// Terminal tasks are no longer useful on the worker. The drop runs
		// detached; ownership is released when it returns, not here.
		mlog.Info(s.ctx, "task ended during dispatch, scheduling worker cleanup",
			WrapTaskLog(task, mlog.String("state", task.GetTaskState().String()))...)
		task.SetTaskTime(taskcommon.TimeEnd, time.Now())
		s.submitDrop(task)
	}
...
func (s *globalTaskScheduler) queryUnderTaskLock(task Task) {
	...
	default:
		// Terminal tasks are no longer useful on the worker. The drop runs
		// detached; ownership is released when it returns, not here.
		mlog.Info(s.ctx, "task ended on its worker, scheduling worker cleanup",
			WrapTaskLog(task, mlog.String("state", task.GetTaskState().String()))...)
		task.SetTaskTime(taskcommon.TimeEnd, time.Now())
		s.submitDrop(task)
	}
...
func (s *globalTaskScheduler) pickNode(nodeSlots map[int64]*session.WorkerSlots, taskSlot int64) int64 {
	return s.pickCompatibleNode(nodeSlots, taskSlot, nil)
}

func (s *globalTaskScheduler) pickCompatibleNode(nodeSlots map[int64]*session.WorkerSlots, taskSlot int64, filter NodeFilter) int64 {
	...
	for nodeID, slots := range nodeSlots {
		if filter != nil && !filter.CanRunOnNode(nodeID, slots) {
			continue
		}
...
// partition buckets every owned task by its current state in a single pass,
// preserving ascending task-ID order within each bucket.
//
...
func (s *globalTaskScheduler) round() {
	...
	// Queue ended-task cleanup before dispatching. The drops run detached, so
	// the slots they free appear to schedule() one round later rather than in
	// this one -- a worker Drop RPC must not hold up dispatch.
...
func (s *globalTaskScheduler) releaseEndedTaskOwnership(ended []Task) {
	for _, task := range ended {
		taskID := task.GetTaskID()
		// A terminal drop is already in flight for this ID. Its goroutine holds
		// the key lock across the worker RPC and releases ownership when the
		// drop returns -- so there is nothing for this round to do, and locking
		// the task here would block on a drop that may take
		// dataCoord.requestTimeoutSeconds. Skipping without the lock keeps a
		// node that stopped answering from stalling dispatch in every
		// subsequent round, not just the one that queued the drop.
		if _, inFlight := s.dropping.Load(taskID); inFlight {
			continue
		}
		s.mu.Lock(taskID)
...
// submitDrop schedules a terminal task's worker cleanup without waiting for it.
// The Drop is a worker RPC bounded only by dataCoord.requestTimeoutSeconds, so
// running it inline would hold up dispatch for that long whenever a node stops
// answering -- the round releases ownership and dispatches pending tasks while
// the drop runs detached, exactly like the check pass. Ownership is not
// released here: the completion path releases it, so a same-ID retry cannot be
// admitted ahead of the old attempt's Drop.
//
// The dropping map both deduplicates (at most one Drop per ID in flight) and
// bounds concurrency (see maxConcurrentDropRPCs). A task left behind because a
// drop is already queued, or because the cap is reached, keeps its ownership
// and the next round retries it.
//
// submitDrop may run on a pool worker (dispatch or check), which is not itself
// a WaitGroup member. The Add is still safe because every pool worker exists
// only while the member that submitted it is alive and awaiting it: round()
// awaits the dispatch futures, and the detached check goroutine awaits the
// poll futures before Done. So the counter is never zero while a worker runs.
func (s *globalTaskScheduler) submitDrop(task Task) {
	taskID := task.GetTaskID()
	if _, loaded := s.dropping.LoadOrStore(taskID, struct{}{}); loaded {
		return
	}
	if s.droppingCount.Add(1) > maxConcurrentDropRPCs {
		s.droppingCount.Add(-1)
		s.dropping.Delete(taskID)
		return
	}
	// Counted in the same WaitGroup as the loops, so Stop() still waits for the
	// worker drops it started. Adding is safe here by the argument in the
	// function comment: the caller runs only while a WaitGroup member is alive.
	s.wg.Add(1)
	go func() {
		defer s.wg.Done()
		defer s.dropping.Delete(taskID)
		defer s.droppingCount.Add(-1)
		s.dropEndedTask(task)
	}()
}

// dropEndedTask releases a terminal task after its worker cleanup. It runs
// detached, so it re-reads ownership and state under the key lock just like any
// other callback: the task may have been aborted or finalized while the drop
// was queued, and the stale copy must not act on it.
func (s *globalTaskScheduler) dropEndedTask(task Task) {
	taskID := task.GetTaskID()
	s.mu.Lock(taskID)
	defer s.mu.Unlock(taskID)
	if !s.ownsUnderTaskLock(task) {
	...
	switch task.GetTaskState() {
	case taskcommon.Init, taskcommon.InProgress:
		// Brought back to life since the drop was queued; the next round
		// classifies it again.
	case taskcommon.None, taskcommon.Retry:
		// The business callback handled this attempt and returned it to its
		// owner. Do not issue a second Drop that can race a same-ID retry.
		mlog.Info(s.ctx, "task attempt ended, releasing scheduler ownership",
			WrapTaskLog(task, mlog.String("state", task.GetTaskState().String()))...)
		s.releaseUnderTaskLock(taskID)
	default:
...
func (s *globalTaskScheduler) schedule(pending []Task) {
	...
	// partition already puts smaller task IDs first.
	...
	for _, task := range pending {
		...
		filter, _ := task.(NodeFilter)
		nodeID := s.pickCompatibleNode(nodeSlots, taskSlot, filter)
		if nodeID == NullNodeID {
			// A blocked task must not hold up later tasks with different capabilities.
			continue
		}
