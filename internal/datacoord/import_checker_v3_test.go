...
func TestQuiesceImportJobKeepsTasksDuringRetention(t *testing.T) {
	...
	require.False(t, newImportV3JobGC(newImportV3JobContext(checker, job)).quiesce())
	...
	require.True(t, newImportV3JobGC(newImportV3JobContext(checker, job)).quiesce())
}

func TestCleanupPreparingImportV3TasksIsIdempotent(t *testing.T) {
	ctx := context.Background()
	importMeta := NewMockImportMeta(t)
	checker := &importCheckerV3{ctx: ctx, importMeta: importMeta}
	...
	require.NoError(t, newImportV3Planner(newImportV3JobContext(checker, job)).cleanupPreparingTasks(ctx, job))
	require.NoError(t, newImportV3Planner(newImportV3JobContext(checker, job)).cleanupPreparingTasks(ctx, job))
}
...
func TestCleanupPreparingImportV3TasksKeepsReadyTask(t *testing.T) {
	ctx := context.Background()
	importMeta := NewMockImportMeta(t)
	checker := &importCheckerV3{ctx: ctx, importMeta: importMeta}
	job := &importJob{ImportJob: &datapb.ImportJob{JobID: 1}}
...
func TestCleanupPreparingImportV3TaskDropsOwnedSegment(t *testing.T) {
	ctx := context.Background()
...
func TestCreateImportV3TaskPublishesPendingAfterSegments(t *testing.T) {
	...
	err = newImportV3Planner(newImportV3JobContext(checker, job)).createTask(
		ctx, job,
		&datapb.WriterSpec{StorageVersion: 1, SchemaVersion: 3},
		10, 100,
		importTaskV3Spec{channel: "v0", partitionID: 4, fragments: []*datapb.FragmentRef{{Path: "f0", RowCount: 1}}, rows: 5},
	)
	require.NoError(t, err)
...
func TestCreateReshardTasksKeepsExistingAndAddsMissingSources(t *testing.T) {
	...
	// preimport set (for the logical-size packing input); only the former has
	// a task. Probe the filter instead of relying on call order.
	...
	importMeta.EXPECT().AddTask(mock.Anything, mock.Anything).Run(func(_ context.Context, added ImportTask) {
		...
		// slots at the 512MiB node slot unit (8GiB / workerSlotUnit 16).
	...
	require.NoError(t, newReshardTaskPlanner(newImportV3JobContext(checker, job)).CreateTasks())
}
...
func TestBuildReshardTaskPlanDerivesExecutionInput(t *testing.T) {
	...
	plan, err := newImportV3PlanFactory(job).reshardPlan(task)
	require.NoError(t, err)
	...
	_, err = newImportV3PlanFactory(job).reshardPlan(task)
	require.Error(t, err)
}
...
func TestCreateReshardTaskRejectsInvalidFragmentSize(t *testing.T) {
	...
	err := newReshardTaskPlanner(newImportV3JobContext(checker, job)).createTask(context.Background(), job, 10, []reshardSource{{file: &internalpb.ImportFile{Id: 1}, size: 1}})
	require.Error(t, err)
	require.Contains(t, err.Error(), "fragmentSizeInMB")
...
func TestReshardTasksAllCompletedStates(t *testing.T) {
	...
	t.Run("failed task terminates the job", func(t *testing.T) {
		...
		completed, err := newImportV3JobContext(checker, job).reshardTasksAllCompleted()
...
func TestLoadExistingImportV3Fragments(t *testing.T) {
	...
	fragments, err := newImportV3Planner(newImportV3JobContext(checker, job)).loadExistingFragments(ctx, job)
	require.NoError(t, err)
...
func TestLoadExistingImportV3FragmentsRejectsIncompleteTask(t *testing.T) {
	...
	_, err := newImportV3Planner(newImportV3JobContext(checker, job)).loadExistingFragments(ctx, job)
	require.Error(t, err)
}
...
func TestBuildImportV3TaskPlanDerivesExecutionInput(t *testing.T) {
	...
	plan, err := newImportV3PlanFactory(job).importPlan(task)
	require.NoError(t, err)
	...
	task.Rows = 0
	_, err = newImportV3PlanFactory(job).importPlan(task)
	require.Error(t, err)
}
...
func TestImportCheckerV3PlanningQuotaFailsJob(t *testing.T) {
	...
	manifest := &datapb.ReshardManifest{Fragments: []*datapb.FragmentDescriptor{{
		Path: "frag-0-0-0.parquet", Rows: 5, LogicalBytes: 200 << 20, ChannelIndex: 0, PartitionId: 3,
	}}}
	...
	// query the ImportTaskV3 set (empty here); planImportV3Job then queries the
