...
// externalRefreshOwnershipPlanVersion identifies the persisted contract in
// which every baseline segment has one exclusive owner and every task owns one
// continuous half-open file-index range. Tasks with another version are
// rejected instead of inferring ownership during recovery.
const externalRefreshOwnershipPlanVersion = int32(1)
