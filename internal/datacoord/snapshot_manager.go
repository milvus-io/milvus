...
// Introducing V4 stats must also pass the cluster gate to protect V3 files
// from the old coordinator GC, even when all QueryNodes already support V4.
		...
		for _, stats := range segment.GetJsonKeyIndexFiles() {
			...
			if version == common.JSONStatsDataFormatV4 {
				if Params.DataCoordCfg.JSONStatsFormatVersion.GetAsInt64() != common.JSONStatsDataFormatV4 {
					return merr.WrapErrServiceNotReadyMsg("cannot restore V4 JSON stats before the JSON stats version gate opens")
				}
