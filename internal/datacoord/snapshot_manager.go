...
func (sm *snapshotManager) buildChannelMapping(
	...
	// Extract channels from snapshot segments
	channelSet := make(map[string]struct{})
	for _, seg := range snapshotData.Segments {
		channelSet[seg.GetChannelName()] = struct{}{}
	}
	...
	// Validate count
	if len(targetChannels) != len(channelsInSnapshot) {
		return nil, merr.WrapErrServiceInternal(
			fmt.Sprintf("channel count mismatch between snapshot and target collection: snapshot=%d, target=%d",
				len(channelsInSnapshot), len(targetChannels)))
	}

	// Build mapping (sorted)
	sort.Strings(channelsInSnapshot)
	sort.Slice(targetChannels, func(i, j int) bool {
		return targetChannels[i].GetName() < targetChannels[j].GetName()
	})

	for i, ch := range targetChannels {
		channelMapping[channelsInSnapshot[i]] = ch.GetName()
	}
