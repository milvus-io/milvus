...
type snapshotManager struct {
	...
	// pending flags -- which are plain set membership, not refcounts --
...
func (sm *snapshotManager) CreateSnapshot(
	...
	waitForSortedSegments = waitForSortedSegments || compactionProtectionSeconds > 0
	...
	// The WAL positions are flush watermarks, not historical read timestamps.
	// Compaction may continue while we wait; capture pins the current layout.
	if boundary == nil {
		// Before this change CreateSnapshot was CChannel-only. Replayed legacy
		// requests retain their checkpoint-based coverage without inventing a
		// historical flush watermark.
		collection, err := sm.handler.GetCollection(ctx, collectionID)
		if err != nil {
			return 0, err
		}
		if collection == nil {
		...
		for _, channel := range collection.VChannelNames {
			position := sm.meta.GetChannelCheckpoint(channel)
			if position == nil {
				return 0, merr.WrapErrServiceUnavailableMsg("snapshot channel %s has no checkpoint yet", channel)
			}
			boundary.SeekPositions = append(boundary.SeekPositions, proto.Clone(position).(*msgpb.MsgPosition))
			if boundary.SnapshotTs == 0 || position.GetTimestamp() < boundary.SnapshotTs {
				boundary.SnapshotTs = position.GetTimestamp()
			}
		}
	}
	...
	// Take the bounded capture slot before blocking compaction publication.
	...
	// Drain a commit that already passed its snapshot check. New commits see
	// pending under the same segMu, closing the check-to-publication race.
	sm.meta.segMu.Lock()
	sm.snapshotMeta.SetSnapshotPending(collectionID)
	sm.meta.segMu.Unlock()
	defer sm.snapshotMeta.ClearSnapshotPending(collectionID)

	// A clustering publication spans several catalog writes. If a prior
	// attempt published results but has not retired its inputs, let its
	// existing retry/cleanup finish before pinning either generation.
	if err := sm.checkClusteringPublication(ctx, collectionID); err != nil {
		return 0, err
	}

	if compactionProtectionSeconds > 0 {
		if err := sm.checkSnapshotBackfillTargets(ctx, collectionID, boundary); err != nil {
			return 0, err
		}
	}
...
// checkSnapshotBackfillTargets runs under capture pending. A precomputed
// compaction output must not survive a backfill and publish stale values when
// protection expires. Drain those tasks before handing their inputs to Spark.
func (sm *snapshotManager) checkSnapshotBackfillTargets(ctx context.Context, collectionID int64, boundary *SnapshotBoundary) error {
	for _, segment := range sm.meta.SelectSegments(ctx, WithCollection(collectionID)) {
		if !segment.isCompacting || !isSegmentHealthy(segment) || segment.GetLevel() == datapb.SegmentLevel_L0 {
			continue
		}
		if ts, ok := boundary.SeekTs(segment.GetInsertChannel()); ok && segmentEffectiveTs(segment.SegmentInfo) < ts {
			return merr.WrapErrServiceUnavailableMsg("snapshot waits for compaction of segment %d", segment.GetID())
		}
	}
	// On restart task metadata is loaded before the inspector restores the
	// in-memory compacting flags. Inspect durable tasks as well as segments.
	if sm.meta.compactionTaskMeta != nil {
		for _, tasks := range sm.meta.compactionTaskMeta.GetCompactionTasksByCollection(collectionID) {
			for _, task := range tasks {
				if task.GetType() == datapb.CompactionType_Level0DeleteCompaction || task.GetState() == datapb.CompactionTaskState_cleaned {
					continue
				}
				for _, id := range task.GetInputSegments() {
					segment := sm.meta.GetHealthySegment(ctx, id)
					if segment == nil {
						continue
					}
					if ts, ok := boundary.SeekTs(segment.GetInsertChannel()); ok && segmentEffectiveTs(segment.SegmentInfo) < ts {
						return merr.WrapErrServiceUnavailableMsg("snapshot waits for compaction task %d on segment %d", task.GetPlanID(), id)
					}
				}
			}
		}
...
// checkClusteringPublication runs while snapshot pending prevents publication
// and retirement. Task metadata preserves the original inputs even if GC has
// already removed intermediate sort segments from the lineage.
func (sm *snapshotManager) checkClusteringPublication(ctx context.Context, collectionID int64) error {
	if sm.meta.compactionTaskMeta == nil {
		return nil
	}
	for _, tasks := range sm.meta.compactionTaskMeta.GetCompactionTasksByCollection(collectionID) {
		for _, task := range tasks {
			if clusteringPublicationOverlaps(ctx, sm.meta, task) {
				return merr.WrapErrServiceUnavailableMsg("snapshot waits for clustering task %d to retire its published inputs", task.GetPlanID())
			}
		}
	}
	return nil
...
func clusteringPublicationOverlaps(ctx context.Context, m *meta, task *datapb.CompactionTask) bool {
	if task.GetType() != datapb.CompactionType_ClusteringCompaction {
		return false
	}
	hasLiveInput := false
	for _, id := range task.GetInputSegments() {
		if m.GetHealthySegment(ctx, id) != nil {
			hasLiveInput = true
			break
		}
	}
	...
	// Completion is persisted before retiring inputs. Results published before
	// the replacement admission check existed may already have been compacted
	// and GC'd; their absence does not prove the original generation is safe.
	if task.GetState() == datapb.CompactionTaskState_completed {
		return true
	}
	for _, id := range task.GetResultSegments() {
		segment := m.GetSegment(ctx, id)
		if task.GetState() == datapb.CompactionTaskState_cleaned && !isSegmentHealthy(segment) {
			// Failed publication is rolled back by dropping the results. Cleanup
			// preserves their old visibility bit and keeps the original inputs.
			continue
		}
		if segment != nil && !segment.GetIsInvisible() {
			return true
		}
		// Before cleanup finishes, a missing result cannot be proven to have
		// stayed invisible. Let the task finish retiring or rolling back instead
		// of relying on a lineage that GC may already have removed.
		if segment == nil && task.GetState() != datapb.CompactionTaskState_cleaned {
			return true
		}
	}
	return false
...
// checkSnapshotCompactionReachable rejects a drain that cannot run while the
// compaction subsystem is disabled. Run before broadcast: callback retries
// cannot release the collection DDL lock until the snapshot succeeds.
func checkSnapshotCompactionReachable(ctx context.Context, m *meta, collectionID int64, protected bool) error {
	if Params.DataCoordCfg.EnableCompaction.GetAsBool() {
		return nil
	}
	if protected {
		for _, segment := range m.SelectSegments(ctx, WithCollection(collectionID)) {
			if segment.isCompacting && isSegmentHealthy(segment) && segment.GetLevel() != datapb.SegmentLevel_L0 {
				return merr.WrapErrServiceUnavailableMsg("snapshot waits for compaction of segment %d; re-enable dataCoord.enableCompaction to let it finish", segment.GetID())
			}
		}
	}
	...
	for _, tasks := range m.compactionTaskMeta.GetCompactionTasksByCollection(collectionID) {
		for _, task := range tasks {
			if clusteringPublicationOverlaps(ctx, m, task) {
				return merr.WrapErrServiceUnavailableMsg("snapshot waits for clustering task %d to retire its published inputs; re-enable dataCoord.enableCompaction to let it finish", task.GetPlanID())
			}
			if !protected || task.GetType() == datapb.CompactionType_Level0DeleteCompaction || task.GetState() == datapb.CompactionTaskState_cleaned {
				continue
			}
			for _, id := range task.GetInputSegments() {
				if m.GetHealthySegment(ctx, id) != nil {
					return merr.WrapErrServiceUnavailableMsg("snapshot waits for compaction task %d; re-enable dataCoord.enableCompaction to let it finish", task.GetPlanID())
				}
			}
		}
	}
...
// segmentsAwaitingVisibility finds stream-flushed inputs selected by the flush
// watermark that still require their growing-to-sealed handoff. Unpublished
// clustering outputs are excluded because their original inputs still serve.
// External collections have no sort lifecycle and are handled separately.
