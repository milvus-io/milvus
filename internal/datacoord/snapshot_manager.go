...
func (sm *snapshotManager) ListRestoreJobs(
	...
	// Filter by collection/database and build restore info list. Jobs created
	// before DbId was persisted use the target collection as a compatibility
	// fallback so database-scoped listing continues to find them.
	restoreInfos := make([]*datapb.RestoreSnapshotInfo, 0)
	legacyCollectionDBIDs := make(map[int64]int64)
	for _, job := range jobs {
		...
		if dbID != 0 && collectionIDFilter == 0 {
			if jobDBID == 0 {
				var ok bool
				jobDBID, ok = legacyCollectionDBIDs[job.GetCollectionId()]
				if !ok {
					collection, err := sm.handler.GetCollection(ctx, job.GetCollectionId())
					if err != nil {
						if errors.Is(err, merr.ErrCollectionNotFound) {
							legacyCollectionDBIDs[job.GetCollectionId()] = 0
							continue
						}
						mlog.Warn(ctx, "failed to resolve database for legacy restore job",
					...
					jobDBID = collection.DatabaseID
					legacyCollectionDBIDs[job.GetCollectionId()] = jobDBID
				}
			}
			if jobDBID != dbID {
				continue
		...
		restoreInfo := sm.buildRestoreInfo(job)
		if restoreInfo.GetDbId() == 0 {
			restoreInfo.DbId = jobDBID
		}
