...
func (c *compactionInspector) preemptTasksByChannel(channel string) {
	logger := log.Ctx(context.TODO()).With(zap.String("channel", channel))
	...
	for _, task := range preempted {
		logger.Info("compaction task preempted by shard split",
			zap.Int64("planID", task.GetTaskProto().GetPlanID()),
			zap.String("type", task.GetTaskProto().GetType().String()),
			zap.String("state", task.GetTaskProto().GetState().String()))
		if !task.Clean() {
			logger.Warn("failed to clean the preempted compaction task, it will be retried by the clean loop",
				zap.Int64("planID", task.GetTaskProto().GetPlanID()))
...
func (c *compactionInspector) enqueueCompaction(task *datapb.CompactionTask) error {
	...
	if c.isChannelSplitting != nil && c.isChannelSplitting(task.GetChannel()) {
		...
		log.RatedInfo(60, "channel is splitting, reject compaction during the split window", zap.String("channel", task.GetChannel()))
