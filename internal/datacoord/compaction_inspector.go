...
func (c *compactionInspector) schedule() []CompactionTask {
	...
	for {
		...
		decCoordPendingCompactionTaskNum(t.GetTaskProto().GetType())
		incCoordExecutingCompactionTaskNum(t.GetTaskProto().GetType())
	}
	return selected
}
...
func (c *compactionInspector) removeTasksByChannel(channel string) {
	...
	for id, task := range c.executingTasks {
		...
		if task.GetTaskProto().GetChannel() == channel {
			...
			decNodeExecutingCompactionTaskNum(task.GetTaskProto().GetNodeID(), task.GetTaskProto().GetType())
		}
	}
	c.executingGuard.Unlock()
}
...
func (c *compactionInspector) checkCompaction() error {
	...
	for _, t := range finishedTasks {
		...
		decNodeExecutingCompactionTaskNum(t.GetTaskProto().GetNodeID(), t.GetTaskProto().GetType())
		incNodeDoneCompactionTaskNum(t.GetTaskProto().GetNodeID(), t.GetTaskProto().GetType())
	}
	c.executingGuard.Unlock()
