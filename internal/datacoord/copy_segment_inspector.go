...
func (s *copySegmentInspector) reloadFromMeta() {
	...
	for _, job := range jobs {
		...
		for _, task := range tasks {
			// A failed task still assigned to a worker may hold an admitted
			// completed result whose files need cleanup; it retains scheduler
			// polling (GetTaskState reports InProgress) whatever the job state.
