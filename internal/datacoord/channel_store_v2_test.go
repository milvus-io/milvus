...
func (s *StateChannelStoreSuite) TestGetNodeChannelsBy() {
	...
	assignments := map[int64][]string{
		bufferID: {channels[0].GetName()},
		100:      {channels[1].GetName(), channels[2].GetName()},
		101:      {channels[3].GetName(), channels[4].GetName()},
		102:      {channels[5].GetName(), channels[6].GetName()}, // legacy nodes
	}
	...
	s.Run("test GetNodeChannels", func() {
		...
		lo.ForEach(infos, func(info *NodeChannelInfo, _ int) {
			...
			gotChannels := lo.Map(info.Channels, func(ch RWChannel, _ int) string {
				return ch.GetName()
			})
...
func (s *StateChannelStoreSuite) TestUpdateMeta() {
	...
	for _, test := range tests {
		s.Run(test.description, func() {
			...
			lo.ForEach(test.channels, func(ch *StateChannel, _ int) { store.channels[ch.GetName()] = ch })
			for nodeID, channels := range test.assignments {
				lo.ForEach(channels, func(ch string, _ int) {
					store.addAssignment(nodeID, ch)
				})
			...
			for nodeID, channels := range test.outAssignments {
				...
				info := got[0]
				log.Info("info", zap.Any("info", info), zap.Any("ass", store.assignments))

				s.ElementsMatch(channels, lo.Map(info.Channels, func(ch RWChannel, _ int) string {
					return ch.GetName()
				}))
			}
		})
