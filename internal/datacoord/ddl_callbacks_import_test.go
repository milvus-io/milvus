...
func testBroadcastTargetsDataVchannels(t *testing.T, broadcastFn func(*Server, context.Context, ImportJob) error) {
	...
	assert.ElementsMatch(t, append(wantVchannels, streaming.WAL().ControlChannel()), capture.captured.BroadcastHeader().VChannels,
		"business channels supply timestamps and CChannel orders replicated callbacks")
