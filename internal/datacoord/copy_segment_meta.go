...
func (m *copySegmentMeta) ReplaceRetryTask(ctx context.Context, oldTaskID int64, task CopySegmentTask) (bool, error) {
	...
	// A checker may still hold this wrapper in an older task slice. Keep its
	// Retry state: replacement is not a task failure, and the scheduler already
	// releases Retry attempts without requiring another terminal transition.
	incoming.ctx = m.ctx
	incoming.copyMeta = m
