		...
		for _, task := range tasks {
			if task.State == datapb.CompactionTaskState_completed || task.State == datapb.CompactionTaskState_cleaned {
				...
				if duration > float64(Params.DataCoordCfg.CompactionDropToleranceInSeconds.GetAsDuration(time.Second).Seconds()) {
					...
					log.Info("drop compaction task meta", zap.Int64("planID", task.GetPlanID()))
					if err != nil {
						log.Warn("fail to drop task", zap.Int64("planID", task.GetPlanID()), zap.Error(err))
					}
				}
			}
		}
...
func (c *compactionPlanHandler) createCompactTask(pb *datapb.CompactionTask) (CompactionTask, error) {
	...
	case datapb.CompactionType_ClusteringCompaction:
		task = newClusteringCompactionTask(pbCloned, c.meta, c.sessions, c.handler, c.analyzeScheduler)
	default:
