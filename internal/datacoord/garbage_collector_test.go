...
func TestGarbageCollector_recycleUnusedNgramIndexFiles(t *testing.T) {
	t.Run("success_with_ngram_stats", func(t *testing.T) {
		cm := &mocks.ChunkManager{}
		cm.EXPECT().RootPath().Return("root")
			...
			SegmentInfo: &datapb.SegmentInfo{
				...
				NgramIndexStats: map[int64]*datapb.NgramIndexStats{
					fieldID: {
						...
						Version: 2,
						BuildID: buildID,
						Files:   []string{"ngram_file1", "ngram_file2"},
						MinGram: 2,
						MaxGram: 4,
					},
				},
		...
		meta.segments.SetSegment(segID, segment)

		cm.EXPECT().WalkWithPrefix(mock.Anything, mock.Anything, mock.Anything, mock.Anything).RunAndReturn(
			func(ctx context.Context, prefix string, recursive bool, walkFunc storage.ChunkObjectWalkFunc) error {
				walkFunc(&storage.ChunkObjectInfo{FilePath: "root/ngram_index/123/1/100/200/500/101/old_file1"})
				walkFunc(&storage.ChunkObjectInfo{FilePath: "root/ngram_index/123/1/100/200/500/101/old_file2"})
				return nil
			})
		...
		gc.recycleUnusedNgramIndexFiles(context.TODO())
	})

	t.Run("no_ngram_segments", func(t *testing.T) {
		cm := &mocks.ChunkManager{}
		cm.EXPECT().RootPath().Return("root")
	...
	t.Run("remove_file_error", func(t *testing.T) {
		cm := &mocks.ChunkManager{}
		cm.EXPECT().RootPath().Return("root")
		...
		cm.EXPECT().WalkWithPrefix(mock.Anything, mock.Anything, mock.Anything, mock.Anything).RunAndReturn(
			func(ctx context.Context, prefix string, recursive bool, walkFunc storage.ChunkObjectWalkFunc) error {
				walkFunc(&storage.ChunkObjectInfo{FilePath: "root/ngram_index/123/1/100/200/500/101/old_file1"})
				walkFunc(&storage.ChunkObjectInfo{FilePath: "root/ngram_index/123/2/100/200/500/101/old_file2"})
				return nil
			})

		cm.EXPECT().Remove(mock.Anything, mock.Anything).Return(errors.New("remove failed"))
...
func TestGarbageCollector_recycleDroppedSegments_withNgram(t *testing.T) {
	catalog := catalogmocks.NewDataCoordCatalog(t)
	catalog.On("ChannelExists", mock.Anything, mock.Anything).Return(true)
	catalog.On("DropSegment", mock.Anything, mock.Anything).Return(nil)
			...
			Binlogs: []*datapb.FieldBinlog{
				{
					FieldID: fieldID,
					Binlogs: []*datapb.Binlog{
						{LogPath: "insert_log1", LogSize: 1024},
					},
				},
			},
			Deltalogs: []*datapb.FieldBinlog{
				{
					FieldID: fieldID,
					Binlogs: []*datapb.Binlog{
						{LogPath: "delta_log1", LogSize: 512},
					},
				},
			},
			Statslogs: []*datapb.FieldBinlog{
				{
					FieldID: fieldID,
					Binlogs: []*datapb.Binlog{
						{LogPath: "stats_log1", LogSize: 256},
					},
				},
			},
			NgramIndexStats: map[int64]*datapb.NgramIndexStats{
				fieldID: {
					...
					Version: 1,
					BuildID: buildID,
					Files:   []string{"ngram_file1.idx", "ngram_file2.idx"},
					MinGram: 2,
					MaxGram: 4,
				},
				fieldID + 1: {
	...
	segments := NewSegmentsInfo()
	segments.SetSegment(segID, droppedSegment)
	...
	expectedFiles := []string{
		"insert_log1", "delta_log1", "stats_log1",
		"ngram_file1.idx", "ngram_file2.idx", "ngram_file3.idx",
	}
	cm.EXPECT().Remove(mock.Anything, mock.Anything).Return(nil).Times(len(expectedFiles))

	gc := newGarbageCollector(meta, newMockHandlerWithMeta(meta), GcOption{
		cli:           cm,
		dropTolerance: 0,
	})
	...
	deletedSegment := gc.meta.GetSegment(context.TODO(), segID)
	assert.Nil(t, deletedSegment)
}
