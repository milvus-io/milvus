...
func TestGarbageCollector_recycleDroppedSegments_SkipsSplittingChannel(t *testing.T) {
	...
	gc.recycleDroppedSegments(ctx, nil)
	assert.NotNil(t, m.GetSegment(ctx, segment.ID), "a dropped segment of a splitting channel is not collected")

	// Once the split is Done, the same segment is collected on the next round.
	gc.option.isChannelSplitting = func(channel string) bool { return false }
	gc.recycleDroppedSegments(ctx, nil)
	assert.Nil(t, m.GetSegment(ctx, segment.ID), "the segment is collected once its channel stops splitting")
}
