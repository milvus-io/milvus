...
func TestGarbageCollector_TaskRegistryAndPauseWorkers(t *testing.T) {
	...
	for _, taskName := range gcControlledTaskNames {
		assert.NotNil(t, gc.controlChannels[taskName])
	}

	tasks := gc.recycleTasks()
	taskByName := make(map[string]recycleTask, len(tasks))
	for _, task := range tasks {
		taskByName[task.name] = task
	}
	assert.Len(t, taskByName, len(gcControlledTaskNames))
	for _, taskName := range gcControlledTaskNames {
		_, ok := taskByName[taskName]
		assert.True(t, ok, taskName)
	}
	assert.Equal(t, time.Second, taskByName[gcTaskDroppedSegments].interval)
	assert.Equal(t, 2*time.Second, taskByName[gcTaskOrphan].interval)
	...
	t.Run("send timeout", func(t *testing.T) {
		...
		defer cancel()
		assert.Error(t, gc.pauseWorkers(gcCmd{timeout: ctx.Done()}))
	})
	...
	t.Run("ack timeout", func(t *testing.T) {
		...
		go func() {
			close(ready)
			<-gc.controlChannels[gcPausableTaskNames[0]]
		}()
		<-ready
		assert.Error(t, gc.pauseWorkers(gcCmd{timeout: ctx.Done()}))
	})

	t.Run("multi-task failure reports all task names", func(t *testing.T) {
		gc := newGarbageCollector(nil, nil, GcOption{})
		ctx, cancel := context.WithTimeout(context.Background(), 20*time.Millisecond)
		defer cancel()
		// No receiver on any channel: every pausable task should time out and
		// its name should be in the error message.
		err := gc.pauseWorkers(gcCmd{timeout: ctx.Done()})
		require.Error(t, err)
		for _, taskName := range gcPausableTaskNames {
			assert.Contains(t, err.Error(), taskName, "failed task %s should appear in error", taskName)
		}
	})

	t.Run("partial failure reports only the failing task", func(t *testing.T) {
		gc := newGarbageCollector(nil, nil, GcOption{})
		ctx, cancel := context.WithTimeout(context.Background(), 100*time.Millisecond)
		defer cancel()
		// Drain every channel except the last so exactly one task fails.
		failingTask := gcPausableTaskNames[len(gcPausableTaskNames)-1]
		for _, taskName := range gcPausableTaskNames {
			if taskName == failingTask {
				continue
			}
			ch := gc.controlChannels[taskName]
			go func() {
		...
		err := gc.pauseWorkers(gcCmd{timeout: ctx.Done()})
		require.Error(t, err)
		assert.Contains(t, err.Error(), failingTask)
		for _, taskName := range gcPausableTaskNames {
			if taskName == failingTask {
				continue
			}
			assert.NotContains(t, err.Error(), taskName, "successful task %s should not appear in error", taskName)
		}
	})
}

func TestGarbageCollector_DroppedSegmentWorkerSignals(t *testing.T) {
	gc := newGarbageCollector(nil, nil, GcOption{})

	t.Run("acquire slot", func(t *testing.T) {
		slots := make(chan struct{}, 1)
		assert.True(t, gc.acquireDroppedSegmentSlot(context.Background(), nil, slots))
		assert.Len(t, slots, 1)
	})

	t.Run("acquire canceled", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
		assert.False(t, gc.acquireDroppedSegmentSlot(ctx, nil, make(chan struct{})))
	})

	t.Run("acquire canceled drains pending signal", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		signal := make(chan gcCmd, 1)
		// Pre-load a pause command. With slots full (capacity 0), acquireDroppedSegmentSlot
		// would block forever; cancellation should both return false AND drain the
		// pending command so the requester is not stuck waiting for cmd.timeout.
		cmdDone := make(chan error, 1)
		signal <- gcCmd{done: cmdDone}
		cancel()
		assert.False(t, gc.acquireDroppedSegmentSlot(ctx, signal, make(chan struct{})))
		select {
		case <-cmdDone:
		case <-time.After(time.Second):
			t.Fatal("pending pause signal was not drained on ctx cancellation")
		}
	})

	t.Run("wait drains signal when wg already done", func(t *testing.T) {
		// wg starts at 0; without the deferred ackSignal the for-loop could
		// return on <-done before any tick fires, leaving the pending signal
		// stranded. Verify the defer drains it.
		...
		go func() {
			gc.waitDroppedSegmentTasks(signal, &wg)
			close(waitDone)
		}()
		select {
		case <-cmdDone:
		case <-time.After(time.Second):
			t.Fatal("waitDroppedSegmentTasks did not drain signal on early exit")
		}
		select {
		case <-waitDone:
		case <-time.After(time.Second):
			t.Fatal("waitDroppedSegmentTasks did not exit")
		}
	})
...
func TestGarbageCollector_recycleDroppedSegments_ConcurrentFanOut(t *testing.T) {
	paramtable.Get().Save(Params.DataCoordCfg.GCRemoveConcurrent.Key, "2")
	defer paramtable.Get().Reset(Params.DataCoordCfg.GCRemoveConcurrent.Key)

	ctx := context.Background()
	meta1, segment1, _, _ := setupDroppedSegmentWithIndexForGC(t)
	segment2 := &SegmentInfo{
		SegmentInfo: &datapb.SegmentInfo{
			ID:            segment1.ID + 1,
			CollectionID:  segment1.CollectionID,
			PartitionID:   segment1.PartitionID,
			InsertChannel: segment1.InsertChannel,
			State:         commonpb.SegmentState_Dropped,
			DroppedAt:     uint64(time.Now().Add(-time.Hour).UnixNano()),
			Binlogs:       []*datapb.FieldBinlog{getFieldBinlogPaths(20, "root/insert_log/100/10/1002/20/1")},
		},
	}
	require.NoError(t, meta1.AddSegment(ctx, segment2))

	gc := newGarbageCollector(meta1, newMockHandler(), GcOption{
		cli:           storage.NewLocalChunkManager(objectstorage.RootPath("/tmp/test-gc-fanout")),
		dropTolerance: 0,
	})

	// Deterministic barrier: both goroutines must arrive before either is allowed
	// to leave. If concurrency is broken, the second goroutine never starts and
	// the first one blocks on the barrier until the test deadline fires.
	const expectedConcurrent = 2
	barrier := make(chan struct{}, expectedConcurrent)
	...
	mockRecycle := mockey.Mock((*garbageCollector).recycleDroppedSegment).To(
		func(gc *garbageCollector, ctx context.Context, segmentID int64, segment *SegmentInfo) {
			...
			defer inFlight.Dec()
			for {
				curMax := maxInFlight.Load()
				if cur <= curMax || maxInFlight.CompareAndSwap(curMax, cur) {
					break
				}
			}
			barrier <- struct{}{}
			if len(barrier) >= expectedConcurrent {
				// Last arrival releases both goroutines.
				close(release)
			}
			select {
			case <-release:
			case <-time.After(5 * time.Second):
				t.Errorf("goroutine for segment %d timed out waiting for barrier", segmentID)
			}
			processed.Inc()
		}).Build()
...
func TestGarbageCollector_recycleDroppedSegments_ConcurrentEndToEnd(t *testing.T) {
	// Exercises the full recycle path (no mock on recycleDroppedSegment) for two
	// dropped segments in parallel. Catches meta concurrency regressions that
	// the dispatch-only fan-out test cannot see.
	paramtable.Get().Save(Params.DataCoordCfg.GCRemoveConcurrent.Key, "2")
	defer paramtable.Get().Reset(Params.DataCoordCfg.GCRemoveConcurrent.Key)

	ctx := context.Background()
	m, segment1, _, binlogPath1 := setupDroppedSegmentWithIndexForGC(t)
	const (
		segID2   = int64(1002)
		buildID2 = int64(41)
		fieldID2 = int64(20)
	...
	binlogPath2, err := binlog.BuildLogPath(storage.InsertBinlog,
		segment1.CollectionID, segment1.PartitionID, segID2, fieldID2, logID2)
	require.NoError(t, err)
	segment2 := &SegmentInfo{
		SegmentInfo: &datapb.SegmentInfo{
			ID:            segID2,
			...
			State:         commonpb.SegmentState_Dropped,
			DroppedAt:     uint64(time.Now().Add(-time.Hour).UnixNano()),
			Binlogs:       []*datapb.FieldBinlog{getFieldBinlogPaths(fieldID2, binlogPath2)},
		},
	}
	require.NoError(t, m.AddSegment(ctx, segment2))
	segIdx2 := &model.SegmentIndex{
		...
		IndexStorePathVersion: 0,
		IndexState:            commonpb.IndexState_Finished,
		IndexFileKeys:         []string{"idx-file-2"},
	}
	require.NoError(t, m.indexMeta.AddSegmentIndex(ctx, segIdx2))
	...
	// Both segments and their segment-index meta should be gone.
	assert.Nil(t, m.GetSegment(ctx, segment1.ID))
	assert.Nil(t, m.GetSegment(ctx, segID2))
	assert.Empty(t, m.indexMeta.GetAllSegmentIndexes(segment1.ID))
	assert.Empty(t, m.indexMeta.GetAllSegmentIndexes(segID2))

	// Binlogs and segment-index files for both segments should have been removed.
	removeMu.Lock()
	defer removeMu.Unlock()
	assert.Contains(t, removed, binlogPath1)
	assert.Contains(t, removed, binlogPath2)
}
