...
func (m *externalCollectionRefreshManager) applyLoadedFinishedJobSegments(
	...
	baseManifestVersionBySegment := make(map[int64]int64)
	...
	for _, task := range tasks {
		...
		for _, segmentID := range task.GetOwnedSegmentIds() {
			...
			if version, ok := task.GetBaseManifestVersions()[segmentID]; ok {
				baseManifestVersionBySegment[segmentID] = version
			}
			baselineSegmentIDs = append(baselineSegmentIDs, segmentID)
		}
		for segmentID := range task.GetBaseManifestVersions() {
...
func (m *externalCollectionRefreshManager) ensureTasksForInitJob(jobID int64) {
	...
	go func() {
		...
		if err != nil {
			...
			// An InputError blames the request. ErrLoonPermanent keeps system
			// blame because the shared storage boundary cannot know who owns a
			// missing path, but its source taxonomy still says replaying the same
			// operation cannot help. Both terminate this refresh immediately.
			//
			// This is merr's own Input-vs-System split, the same one the
			// DataNode uses to decide Failed vs Retry for a refresh task. The
			// Loon sentinel carries the lower layer's retry verdict without
			// relabeling a system-owned storage error as user input.
...
func (m *externalCollectionRefreshManager) createTasksForJob(
	...
	if err != nil {
		...
		//
		// HandleLoonFFIResult preserves milvus-storage's retry verdict:
		// ErrLoonTransient remains a system error and reaches the retry path;
		// ErrLoonPermanent remains a system error too, but ensureTasksForInitJob
		// catches its sentinel and terminates this job.
