...
func TestExternalCollectionRefreshMeta_TimeoutJobRechecksPhase(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "completed workers are finalizing"},
		{name: "execution timeout predates index wait", currentWait: 42},
		{name: "index wait can still time out", currentWait: 42, observedWait: 42, wantTimeout: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			meta := createMetaTestRefreshMeta(t, []*datapb.ExternalCollectionRefreshJob{{
				JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateInProgress,
				TaskIds: []int64{1001}, IndexWaitStartedTime: tc.currentWait,
			}}, []*datapb.ExternalCollectionRefreshTask{{
				TaskId: 1001, JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateFinished,
			}})

			applied, err := meta.TimeoutJob(1, tc.observedWait, "timeout")
			require.NoError(t, err)
			assert.Equal(t, tc.wantTimeout, applied)
			if tc.wantTimeout {
				assert.Equal(t, indexpb.JobState_JobStateFailed, meta.GetJob(1).GetState())
				assert.Nil(t, meta.GetActiveJobByCollectionID(100))
			} else {
				assert.Equal(t, indexpb.JobState_JobStateInProgress, meta.GetJob(1).GetState())
				assert.NotNil(t, meta.GetActiveJobByCollectionID(100))
			}
		})
	}
}

func TestExternalCollectionRefreshMeta_ApplyJobResultsRechecksTerminalState(t *testing.T) {
	meta := createMetaTestRefreshMeta(t, []*datapb.ExternalCollectionRefreshJob{{
		JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateInProgress,
	}}, nil)
	var reads atomic.Int32
	var originalGetJob func(*externalCollectionRefreshMeta, int64) *datapb.ExternalCollectionRefreshJob
	patch := mockey.Mock((*externalCollectionRefreshMeta).GetJob).To(
		func(m *externalCollectionRefreshMeta, jobID int64) *datapb.ExternalCollectionRefreshJob {
			job := originalGetJob(m, jobID)
			if reads.Add(1) == 1 {
				// Timeout commits after the finalizer reads its initial snapshot,
				// before it acquires the collection lock.
				applied, err := m.UpdateJobState(jobID, indexpb.JobState_JobStateFailed, "timeout")
				require.NoError(t, err)
				require.True(t, applied)
			}
			return job
		}).Origin(&originalGetJob).Build()
	defer patch.UnPatch()

	applied, err := meta.ApplyJobResults(1, func(*datapb.ExternalCollectionRefreshJob) error {
		t.Error("a finalizer with a stale snapshot must not publish after timeout")
		return nil
	})
	require.NoError(t, err)
	assert.False(t, applied)
...
func TestExternalCollectionRefreshMeta_FinalizationRechecksTasks(t *testing.T) {
	for _, entry := range []struct {
		name string
		call func(*externalCollectionRefreshMeta, int64, func(*datapb.ExternalCollectionRefreshJob) error) (bool, error)
	}{
		{name: "apply results", call: (*externalCollectionRefreshMeta).ApplyJobResults},
		{name: "begin index wait", call: (*externalCollectionRefreshMeta).BeginIndexWait},
	} {
		t.Run(entry.name, func(t *testing.T) {
			for _, tc := range []struct {
				...
				{name: "retry", state: indexpb.JobState_JobStateRetry},
				{name: "replacement pending", state: indexpb.JobState_JobStateInit},
				{name: "replacement running", state: indexpb.JobState_JobStateInProgress},
				{name: "failed", state: indexpb.JobState_JobStateFailed},
				{name: "no tasks", noTasks: true},
				{name: "missing task", missingTask: true},
			} {
				t.Run(tc.name, func(t *testing.T) {
					job := &datapb.ExternalCollectionRefreshJob{
						JobId: 1, CollectionId: 100, State: indexpb.JobState_JobStateInProgress,
						TaskIds: []int64{1001}, Progress: 75,
					}
					var tasks []*datapb.ExternalCollectionRefreshTask
					if tc.noTasks {
						job.TaskIds = nil
					} else if !tc.missingTask {
						tasks = []*datapb.ExternalCollectionRefreshTask{{
							TaskId: 1001, JobId: 1, CollectionId: 100, State: tc.state,
						}}
					}
					meta := createMetaTestRefreshMeta(t, []*datapb.ExternalCollectionRefreshJob{job}, tasks)
					before := meta.GetJob(1)

					applied, err := entry.call(meta, 1, func(*datapb.ExternalCollectionRefreshJob) error {
						t.Error("unfinished tasks must not reach result publication")
						return nil
					})
					if tc.missingTask {
						require.ErrorIs(t, err, merr.ErrDataIntegrity, "invalid task ownership must not be silently skipped")
					} else {
						require.NoError(t, err)
					}
					assert.False(t, applied)
					assert.True(t, proto.Equal(before, meta.GetJob(1)), "skipping publication must not change the job or index-wait marker")
					assert.NotNil(t, meta.GetActiveJobByCollectionID(100))
				})
			}
		})
...
func TestExternalCollectionRefreshMeta_ConsumeCommittedTaskResults(t *testing.T) {
	consumedTask := func(taskID int64) *datapb.ExternalCollectionRefreshTask {
		return &datapb.ExternalCollectionRefreshTask{
			...
			OwnershipPlanVersion: externalRefreshOwnershipPlanVersion,
			ResultReady:          true,
			BaseManifestVersions: map[int64]int64{taskID: 1},
		}
	}
	...
	t.Run("success_clears_persisted_and_memory_references", func(t *testing.T) {
		...
		err := fixture.meta.ConsumeCommittedTaskResults(1, func(tasks []*datapb.ExternalCollectionRefreshTask, actions []metastore.UpdateAction) error {
			...
			for i, task := range tasks {
				...
				assert.Equal(t, map[int64]int64{task.GetTaskId(): 1}, persisted.GetBaseManifestVersions())
			}
		...
		for _, taskID := range []int64{1001, 1002} {
			...
			assert.Equal(t, map[int64]int64{taskID: 1}, inMemory.GetBaseManifestVersions())
...
func TestExternalCollectionRefreshMeta_RetryFinishedTaskOnManifestConflict(t *testing.T) {
	newMeta := func(
		...
		catalog := &stubCatalog{
			...
			tasks: []*datapb.ExternalCollectionRefreshTask{{
				...
				BaseManifestVersions: map[int64]int64{10: 1},
			}},
		}
		meta, err := newExternalCollectionRefreshMeta(
			context.Background(),
	...
	t.Run("exact_token_moves_finished_to_retry", func(t *testing.T) {
		...
		assert.Empty(t, savedTask.GetBaseManifestVersions())
		...
		assert.False(t, externalRefreshTaskHasResultPayload(inMemory))
		assert.Empty(t, inMemory.GetBaseManifestVersions())
	...
	t.Run("stale_token_is_noop", func(t *testing.T) {
		...
		assert.Equal(t, map[int64]int64{10: 1}, inMemory.GetBaseManifestVersions())
...
func TestExternalCollectionRefreshMeta_StartTaskAttempt(t *testing.T) {
	t.Run("save_failed", func(t *testing.T) {
		...
		err = meta.StartTaskAttempt(1001, 10, map[int64]int64{1: 1})
		assert.Error(t, err)
		...
		assert.Empty(t, task.GetBaseManifestVersions())
	})
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, indexpb.JobState_JobStateInProgress, task.GetState())
		assert.Equal(t, map[int64]int64{1: 1}, task.GetBaseManifestVersions())
	})
...
func TestExternalCollectionRefreshMeta_BaselineVersionsRecovery(t *testing.T) {
	const taskID = int64(1001)
	catalog := &stubCatalog{tasks: []*datapb.ExternalCollectionRefreshTask{{
		TaskId: taskID, JobId: 1, CollectionId: 100, OwnershipPlanVersion: externalRefreshOwnershipPlanVersion,
	}}}
	meta, err := newExternalCollectionRefreshMeta(context.Background(), catalog)
	require.NoError(t, err)
	var wire []byte
	persist := mockey.Mock((*stubCatalog).SaveExternalCollectionRefreshTask).To(func(_ context.Context, saved *datapb.ExternalCollectionRefreshTask) error {
		var err error
		wire, err = proto.Marshal(saved)
		return err
	}).Build()
	defer persist.UnPatch()
	baseline := map[int64]int64{10: 7, 20: 123456789}
	require.NoError(t, meta.StartTaskAttempt(taskID, 11, baseline))
	// Mutating the caller's map must not alter the durable attempt baseline.
	baseline[10] = 8
	require.NotEmpty(t, wire, "recover the bytes submitted to the catalog, not the in-memory task")
	assert.Equal(t, int64(7), meta.GetTask(taskID).GetBaseManifestVersions()[10])
	recoveredTask := &datapb.ExternalCollectionRefreshTask{}
	require.NoError(t, proto.Unmarshal(wire, recoveredTask))
	recovered, err := newExternalCollectionRefreshMeta(context.Background(), &stubCatalog{
		tasks: []*datapb.ExternalCollectionRefreshTask{recoveredTask},
	})
	require.NoError(t, err)
	assert.Equal(t, map[int64]int64{10: 7, 20: 123456789}, recovered.GetTask(taskID).GetBaseManifestVersions())
	assert.Less(t, len(wire), 100, "baseline uses compact revision numbers")
}

func TestExternalCollectionRefreshMeta_RejectOversizedBaseline(t *testing.T) {
	const taskID = int64(1001)
	catalog := &stubCatalog{tasks: []*datapb.ExternalCollectionRefreshTask{{TaskId: taskID, JobId: 1}}}
	meta, err := newExternalCollectionRefreshMeta(context.Background(), catalog)
	require.NoError(t, err)
	saves := 0
	mock := mockey.Mock((*stubCatalog).SaveExternalCollectionRefreshTask).To(func(context.Context, *datapb.ExternalCollectionRefreshTask) error {
		saves++
		return nil
	}).Build()
	defer mock.UnPatch()
	baseline := make(map[int64]int64, 100000)
	for i := int64(1); i <= 100000; i++ {
		baseline[i] = 1 << 60
	}
	err = meta.StartTaskAttempt(taskID, 11, baseline)
	require.ErrorIs(t, err, errExternalRefreshTaskTooLarge)
	assert.Zero(t, saves, "reject oversized task before any catalog write")
	assert.Empty(t, meta.GetTask(taskID).GetBaseManifestVersions())
	assert.Zero(t, meta.GetTask(taskID).GetNodeId())
}
