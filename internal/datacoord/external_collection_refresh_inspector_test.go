...
func TestExternalCollectionRefreshInspector_ManifestConflictReplacementLifecycle(t *testing.T) {
	...
	finishedTask := &datapb.ExternalCollectionRefreshTask{
		...
		BaseManifestVersions: map[int64]int64{segmentID: 1},
	}
	catalog := &stubCatalog{
	...
	assert.Empty(t, retryTask.GetBaseManifestVersions())
	...
	assert.Empty(t, replacement.GetBaseManifestVersions())
	assert.False(t, replacement.GetResultReady())
	...
	assert.Equal(t, map[int64]int64{segmentID: 2}, started.GetBaseManifestVersions())
