...
func TestGenSnapshot(t *testing.T) {
	...
	mock5 := mockey.Mock((*meta).SelectSegments).To(func(m *meta, ctx context.Context, filters ...SegmentFilter) []*SegmentInfo {
		seg := NewSegmentInfo(&datapb.SegmentInfo{
			...
			Deltalogs: []*datapb.FieldBinlog{{FieldID: 0, Binlogs: []*datapb.Binlog{
				{LogID: 2, TimestampFrom: 13000, TimestampTo: 14000},
			}}},
	...
	// Capture preserves deletes after the flush watermark (12345).
	assert.Len(t, snapshotData.Segments[0].GetDeltalogs(), 1)
	assert.Equal(t, uint64(13000), snapshotData.Segments[0].GetDeltalogs()[0].GetBinlogs()[0].GetTimestampFrom())
