...
type JSONStatsMigrationPolicySuite struct {
	...
	policy    *MigrationCompactionChecker
...
func (s *JSONStatsMigrationPolicySuite) SetupTest() {
	...
	s.policy = newMigrationCompactionChecker(meta, s.mockAlloc, s.handler, s.version)
...
func (s *JSONStatsMigrationPolicySuite) TestJSONStatsMigrationEnable() {
	s.True(s.policy.Enable())
	s.True(s.policy.jsonStatsMigrationEnabled())
	...
	s.False(s.policy.jsonStatsMigrationEnabled())
...
func (s *JSONStatsMigrationPolicySuite) TestRegisteredWithTriggerManager() {
	...
	registered, ok := manager.policies[MigrationCompactionTicker]
	s.Require().True(ok)
	s.IsType(&MigrationCompactionChecker{}, registered)
}
...
func (s *JSONStatsMigrationPolicySuite) TestMigrationDoesNotDowngradeFutureStats() {
	...
	segment := newJSONStatsMigrationTestSegment(collectionID, 201, datapb.SegmentLevel_L1,
		map[int64]int64{
			100: common.JSONStatsDataFormatV3,
			101: common.JSONStatsDataFormatV4 + 1,
		})
	for _, fieldIDs := range [][]int64{{100, 101}, {101, 100}} {
		collection := &collectionInfo{ID: collectionID, Schema: newJSONStatsMigrationSchema(fieldIDs...)}
		s.setPolicyMeta(collectionID, collection, map[UniqueID]*SegmentInfo{201: segment})
		s.handler.EXPECT().GetCollection(mock.Anything, collectionID).Return(collection, nil).Once()
		events, err := s.policy.Trigger(context.Background())
		s.NoError(err)
		s.Empty(events[TriggerTypeSingle], "a future stats field blocks the whole rewrite regardless of field order")
	}
}
...
func (s *JSONStatsMigrationPolicySuite) TestDiscoveryStopsAtRemainingQuota() {
	const collID = int64(100)
	coll := &collectionInfo{ID: collID, Schema: newJSONStatsMigrationSchema(100)}
	segments := make(map[UniqueID]*SegmentInfo)
	for id := int64(201); id < 301; id++ {
	...
	s.mockAlloc.EXPECT().AllocID(mock.Anything).Return(int64(1000), nil).Once()
	visited := 0
	classification := mockey.Mock(classifyJSONStatsFormats).To(
		func(segment *SegmentInfo, fields []int64, target int64) jsonStatsFormatClassification {
			visited++
			s.Equal(common.JSONStatsDataFormatV3, segment.GetJsonKeyStats()[100].GetJsonKeyStatsDataFormat())
			return jsonStatsFormatClassification{hasOlder: true}
		}).Build()
	s.T().Cleanup(func() { classification.UnPatch() })

	views, err := s.policy.triggerOneCollection(context.Background(), collID, 3,
		common.JSONStatsDataFormatV4, s.safeRebuildTargets())
	s.NoError(err)
	s.Len(views, 2)
	s.Equal(2, visited, "discovery must stop once the remaining quota is filled")
	s.Equal(3, s.policy.currentCount)

	views, err = s.policy.triggerOneCollection(context.Background(), collID, 3,
		common.JSONStatsDataFormatV4, s.safeRebuildTargets())
	s.NoError(err)
	s.Empty(views)
	s.Equal(2, visited, "exhausted quota must skip discovery")
}
