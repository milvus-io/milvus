...
// Bounds the baseline write below etcd's default request limit, with headroom
// for the key, request envelope, and later result references. This is an
// admission guard, not a claim that arbitrarily large ownership plans fit.
const maxExternalRefreshTaskMetaBytes = 1 << 20

// Caught by CreateTaskOnWorker before dispatch and translated to a terminal
// system failure. Retrying the same oversized baseline cannot make it fit.
var errExternalRefreshTaskTooLarge = errors.New("external refresh task metadata exceeds size limit")
...
// ApplyJobResults serializes segment publication with timeout and other job
// transitions. The callback must not update job metadata or acquire a WAL
// resource lock; schema publication runs after this method releases jobLock.
func (m *externalCollectionRefreshMeta) ApplyJobResults(
	jobID int64,
	apply func(*datapb.ExternalCollectionRefreshJob) error,
) (bool, error) {
	job := m.GetJob(jobID)
	if job == nil {
		return false, merr.WrapErrServiceInternalMsg("job %d not found", jobID)
	...
	// A concurrent finalizer may have rejected a finished result and moved
	// its task to Retry while this caller waited for jobLock. Recheck under
	// that lock so the stale Finished snapshot cannot consume retrying tasks.
	state, _, err := m.AggregateJobStateFromTasks(jobID)
...
// TimeoutJob rechecks the execution phase under the same lock as result
// publication. A timeout based on an unfinished task snapshot must not fail a
// job whose workers have since finished, even while schema publication or
// terminal cleanup is still being retried outside jobLock.
func (m *externalCollectionRefreshMeta) TimeoutJob(jobID, indexWaitStartedTime int64, failReason string) (bool, error) {
	return m.updateJobStateWithPreApply(jobID, indexpb.JobState_JobStateFailed, failReason, nil,
		jobStateWriteOpts{skip: func(job *datapb.ExternalCollectionRefreshJob) (bool, error) {
			// Entering the index wait changes the timeout contract. Let the next
			// checker pass assess the new phase and its current index debt.
			if job.GetIndexWaitStartedTime() != indexWaitStartedTime {
				return true, nil
			}
			if indexWaitStartedTime == 0 {
				state, _, err := m.AggregateJobStateFromTasks(jobID)
				return err == nil && state == indexpb.JobState_JobStateFinished, nil
			}
			return false, nil
		}})
}
...
func (m *externalCollectionRefreshMeta) BeginIndexWait(
	...
	return m.updateJobStateWithPreApply(jobID, indexpb.JobState_JobStateInProgress, "", preApply,
		jobStateWriteOpts{
			...
			skip: func(job *datapb.ExternalCollectionRefreshJob) (bool, error) {
				if job.GetIndexWaitStartedTime() != 0 {
					return true, nil
				}
				// As in ApplyJobResults, a manifest conflict may have returned
				// a task to Retry after the caller observed Finished.
				state, _, err := m.AggregateJobStateFromTasks(jobID)
				return state != indexpb.JobState_JobStateFinished, err
			},
...
type jobStateWriteOpts struct {
	...
	// terminal-state check, and reports whether the transition is no longer
	// applicable. Returning true aborts the write, including preApply, and
	// reports "not applied" so the caller skips its follow-up side effects.
	// Errors abort the transition and propagate to the caller.
	skip func(*datapb.ExternalCollectionRefreshJob) (bool, error)
...
func (m *externalCollectionRefreshMeta) updateJobStateWithPreApply(
	...
	if opts.skip != nil {
		skip, err := opts.skip(job)
		if err != nil {
			return false, err
		}
		if skip {
			mlog.Info(m.ctx, "skip update job state with pre-apply, transition no longer applicable",
				mlog.FieldJobID(jobID),
				mlog.String("currentState", job.GetState().String()),
				mlog.String("requestedState", state.String()))
			return false, nil
...
func (m *externalCollectionRefreshMeta) RetryFinishedTaskOnManifestConflict(
	...
	cloneTask.BaseManifestVersions = nil
...
// baseline manifest revisions sent to the worker before crossing the Create
// RPC. A retry replacement uses a fresh task ID and takes a new snapshot.
func (m *externalCollectionRefreshMeta) StartTaskAttempt(
	...
	applied, _, err := m.mutateTask(taskID, "start task attempt", false, func(task *datapb.ExternalCollectionRefreshTask) (bool, error) {
		...
		task.FailReason = ""
		task.BaseManifestVersions = make(map[int64]int64, len(baseManifestVersions))
		for segmentID, version := range baseManifestVersions {
			task.BaseManifestVersions[segmentID] = version
		}
		if size := proto.Size(task); size > maxExternalRefreshTaskMetaBytes {
			return false, merr.Wrapf(errExternalRefreshTaskTooLarge,
				"task %d has %d bytes, limit %d", taskID, size, maxExternalRefreshTaskMetaBytes)
		}
		return false, nil
	})
	if applied {
		mlog.Info(m.ctx, "started external refresh task attempt",
			...
			mlog.Int("baseManifestCount", len(baseManifestVersions)))
	}
	return err
}
