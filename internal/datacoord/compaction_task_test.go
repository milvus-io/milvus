...
// TestCompactionTaskCleanupIsIdempotentAcrossTypes pins the cleanup contract
// shared by every compaction task type.
func TestCompactionTaskCleanupIsIdempotentAcrossTypes(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			make: func(_ *testing.T, proto *datapb.CompactionTask, meta CompactionMeta) CompactionTask {
				return newBumpSchemaVersionTask(context.TODO(), proto, nil, meta, newMockVersionManager())
			},
		},
	}
...
func TestDropCompactionTaskOnWorkerSkipsUnassignedTask(t *testing.T) {
	cluster := session.NewMockCluster(t)
	task := &datapb.CompactionTask{PlanID: 1}

	for _, nodeID := range []int64{0, NullNodeID} {
		task.NodeID = nodeID
		dropCompactionTaskOnWorker(context.TODO(), task, cluster, "testCompactionTask")
	}

	task.NodeID = 11
	cluster.EXPECT().DropCompaction(int64(11), int64(1)).Return(nil).Once()
	dropCompactionTaskOnWorker(context.TODO(), task, cluster, "testCompactionTask")
}
...
func TestMixCreateReplansOnUnknownOutcome(t *testing.T) {
	...
	t.Run("canceled assignment save failure does not send the plan", func(t *testing.T) {
		...
		// node. An active process terminates on this write failure; cancellation
		// is the only non-fatal branch that can be exercised in-process. No
		// CreateCompaction expectation -- the mock fails the test if the plan is
		// sent.
		meta := NewMockCompactionMeta(t)
		healthySegment(meta)
