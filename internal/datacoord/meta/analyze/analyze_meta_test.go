...
func (s *AnalyzeMetaSuite) Test_AnalyzeMeta() {
	...
	s.Run("GetTask", func() {
		t := am.GetAnalyzeTask(1)
		s.NotNil(t)

		t = am.GetAnalyzeTask(100)
		s.Nil(t)
	})
	...
	s.Run("UpdateVersion", func() {
		err := am.UpdateAnalyzeVersion(1)
		s.NoError(err)
		s.Equal(int64(1), am.GetAnalyzeTask(1).Version)
	})

	s.Run("BuildingTask", func() {
		err := am.BuildingAnalyzeTask(1, 1)
		s.NoError(err)
		s.Equal(indexpb.JobState_JobStateInProgress, am.GetAnalyzeTask(1).State)
	})

	s.Run("FinishTask", func() {
		err := am.FinishAnalyzeTask(1, &workerpb.AnalyzeResult{
			TaskID: 1,
			State:  indexpb.JobState_JobStateFinished,
		})
		s.NoError(err)
		s.Equal(indexpb.JobState_JobStateFinished, am.GetAnalyzeTask(1).State)
	})
}
...
func (s *AnalyzeMetaSuite) Test_failCase() {
	...
	s.Run("AddAnalyzeTask", func() {
		...
		err := am.AddAnalyzeTask(t)
		s.Error(err)
		s.Nil(am.GetAnalyzeTask(1111))
	})

	s.Run("DropAnalyzeTask", func() {
		err := am.DropAnalyzeTask(1)
		s.Error(err)
		s.NotNil(am.GetAnalyzeTask(1))
	})

	s.Run("UpdateVersion", func() {
		err := am.UpdateAnalyzeVersion(777)
		s.Error(err)

		err = am.UpdateAnalyzeVersion(1)
		s.Error(err)
		s.Equal(int64(0), am.GetAnalyzeTask(1).Version)
	})

	s.Run("BuildingTask", func() {
		err := am.BuildingAnalyzeTask(777, 1)
		s.Error(err)

		err = am.BuildingAnalyzeTask(1, 1)
		s.Error(err)
		s.Equal(int64(0), am.GetAnalyzeTask(1).NodeID)
		s.Equal(indexpb.JobState_JobStateInit, am.GetAnalyzeTask(1).State)
	})

	s.Run("FinishTask", func() {
		err := am.FinishAnalyzeTask(777, nil)
		s.Error(err)
