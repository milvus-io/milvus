...
type indexMeta struct {
	...
	indexes map[typeutil.UniqueID]map[typeutil.UniqueID]*model.Index
	// buildID2Meta records the meta information of the segment
	// buildID -> segmentIndex
	buildID2SegmentIndex map[typeutil.UniqueID]*model.SegmentIndex

	// segmentID -> indexID -> segmentIndex
	segmentIndexes map[typeutil.UniqueID]map[typeutil.UniqueID]*model.SegmentIndex
}
...
func NewIndexMeta(ctx context.Context, catalog metastore.DataCoordCatalog) (*indexMeta, error) {
	mt := &indexMeta{
		ctx:                  ctx,
		catalog:              catalog,
		indexes:              make(map[typeutil.UniqueID]map[typeutil.UniqueID]*model.Index),
		buildID2SegmentIndex: make(map[typeutil.UniqueID]*model.SegmentIndex),
		segmentIndexes:       make(map[typeutil.UniqueID]map[typeutil.UniqueID]*model.SegmentIndex),
	}
	err := mt.reloadFromKV(ctx)
	if err != nil {
		return nil, err
	}
	return mt, nil
...
func (m *indexMeta) reloadFromKV(ctx context.Context) error {
	record := timerecord.NewTimeRecorder("indexMeta-reloadFromKV")
	// load field indexes
	fieldIndexes, err := m.catalog.ListIndexes(ctx)
	if err != nil {
	...
	segmentIndexes, err := m.catalog.ListSegmentIndexes(ctx)
	if err != nil {
...
func (m *indexMeta) updateCollectionIndex(index *model.Index) {
	if _, ok := m.indexes[index.CollectionID]; !ok {
		m.indexes[index.CollectionID] = make(map[typeutil.UniqueID]*model.Index)
	}
...
func (m *indexMeta) updateSegmentIndex(segIdx *model.SegmentIndex) {
	...
	if ok {
		...
		m.segmentIndexes[segIdx.SegmentID] = make(map[typeutil.UniqueID]*model.SegmentIndex)
...
func (m *indexMeta) updateIndexTasksMetrics() {
	taskMetrics := make(map[typeutil.UniqueID]map[commonpb.IndexState]int)
...
func (m *indexMeta) CanCreateIndex(req *indexpb.CreateIndexRequest) (typeutil.UniqueID, error) {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) HasSameReq(req *indexpb.CreateIndexRequest) (bool, typeutil.UniqueID) {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetSegmentIndexState(collID, segmentID typeutil.UniqueID, indexID typeutil.UniqueID) *indexpb.SegmentIndexState {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetIndexedSegments(collectionID int64, segmentIDs, fieldIDs []typeutil.UniqueID) []int64 {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetIndexesForCollection(collID typeutil.UniqueID, indexName string) []*model.Index {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetFieldIndexes(collID, fieldID typeutil.UniqueID, indexName string) []*model.Index {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) MarkIndexAsDeleted(collID typeutil.UniqueID, indexIDs []typeutil.UniqueID) error {
...
func (m *indexMeta) IsUnIndexedSegment(collectionID typeutil.UniqueID, segID typeutil.UniqueID) bool {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) getSegmentIndexes(segID typeutil.UniqueID) map[typeutil.UniqueID]*model.SegmentIndex {
	m.RLock()
	defer m.RUnlock()

	ret := make(map[typeutil.UniqueID]*model.SegmentIndex, 0)
	segIndexInfos, ok := m.segmentIndexes[segID]
	if !ok || len(segIndexInfos) == 0 {
		return ret
	}
...
func (m *indexMeta) GetSegmentIndexes(collectionID typeutil.UniqueID, segID typeutil.UniqueID) map[typeutil.UniqueID]*model.SegmentIndex {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetFieldIDByIndexID(collID, indexID typeutil.UniqueID) typeutil.UniqueID {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetIndexNameByID(collID, indexID typeutil.UniqueID) string {
	m.RLock()
	defer m.RUnlock()
	if fieldIndexes, ok := m.indexes[collID]; ok {
		if index, ok := fieldIndexes[indexID]; ok {
...
func (m *indexMeta) GetIndexParams(collID, indexID typeutil.UniqueID) []*commonpb.KeyValuePair {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetTypeParams(collID, indexID typeutil.UniqueID) []*commonpb.KeyValuePair {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetIndexJob(buildID typeutil.UniqueID) (*model.SegmentIndex, bool) {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) IsIndexExist(collID, indexID typeutil.UniqueID) bool {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) UpdateVersion(buildID typeutil.UniqueID) error {
	m.Lock()
	defer m.Unlock()
...
func (m *indexMeta) BuildIndex(buildID, nodeID typeutil.UniqueID) error {
	m.Lock()
	defer m.Unlock()
...
func (m *indexMeta) RemoveSegmentIndex(collID, partID, segID, indexID, buildID typeutil.UniqueID) error {
	m.Lock()
	defer m.Unlock()
...
func (m *indexMeta) RemoveIndex(collID, indexID typeutil.UniqueID) error {
	m.Lock()
	defer m.Unlock()
...
func (m *indexMeta) CheckCleanSegmentIndex(buildID typeutil.UniqueID) (bool, *model.SegmentIndex) {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetMetasByNodeID(nodeID typeutil.UniqueID) []*model.SegmentIndex {
	m.RLock()
	defer m.RUnlock()
...
func (m *indexMeta) GetSegmentsIndexStates(collectionID typeutil.UniqueID, segmentIDs []typeutil.UniqueID) map[int64]map[int64]*indexpb.SegmentIndexState {
	m.RLock()
	defer m.RUnlock()
