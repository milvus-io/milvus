...
func (c *externalCollectionRefreshChecker) beginIndexWait(job *datapb.ExternalCollectionRefreshJob) bool {
	...
	if !applied {
		// The job may already be terminal or waiting for indexes, or a
		// manifest conflict may have returned a task to Retry. No follow-up
		// side effects belong to this skipped transition.
		return false
	}
...
func (c *externalCollectionRefreshChecker) aggregateJobState(job *datapb.ExternalCollectionRefreshJob) {
	...
	if state != job.GetState() {
		...
		if state == indexpb.JobState_JobStateFinished {
			var latestJob *datapb.ExternalCollectionRefreshJob
			applied, err := c.refreshMeta.ApplyJobResults(job.GetJobId(), func(current *datapb.ExternalCollectionRefreshJob) error {
				latestJob = current
				if c.applyJobInfo != nil {
					return c.applyJobInfo(c.ctx, current)
				}
				return nil
			})
			if err != nil {
			...
			// Schema publication acquires a WAL collection resource lock. Keep it
			// outside jobLock: Refresh RPCs acquire those locks in the other order.
			if c.updateJobSchema != nil {
...
func (c *externalCollectionRefreshChecker) tryTimeoutJob(job *datapb.ExternalCollectionRefreshJob) {
	...
	if age > timeout {
		...
		applied, err := c.refreshMeta.TimeoutJob(
			job.GetJobId(),
			job.GetIndexWaitStartedTime(),
			reason)
		if err != nil {
		...
		if !applied {
			// A terminal transition, worker completion, or index-wait entry won
			// the race. Do not fail its tasks or release its collection ownership.
			mlog.Info(c.ctx, "skip timeout fail path, job state changed while checking timeout",
				mlog.FieldJobID(job.GetJobId()))
			return
		}
