...
func (m *indexMeta) reloadFromKV(collectionIDs []int64) error {
	...
	g.Go(func() error {
		...
		if err != nil {
			mlog.Error(m.ctx, "indexMeta reloadFromKV load field indexes fail", zap.Error(err))
			return err
		}
	...
	mlog.Info(m.ctx, "indexMeta reloadFromKV done", zap.Duration("duration", record.ElapseSpan()))
	return nil
}
...
func (m *indexMeta) alterSegmentIndexes(segIdxes []*model.SegmentIndex) error {
	...
	if err != nil {
		mlog.Error(m.ctx, "failed to alter segments index in meta store", zap.Int("segment indexes num", len(segIdxes)),
			zap.Error(err))
		return err
	}
	for _, segIdx := range segIdxes {
...
func (m *indexMeta) updateIndexTasksMetrics() {
	...
	mlog.Info(m.ctx, "update index metric", zap.Int("collectionNum", len(taskMetrics)))
}
...
func checkParams(fieldIndex *model.Index, req *indexpb.CreateIndexRequest) bool {
	...
	mlog.Info(context.TODO(), "final request", zap.Any("create index request", req.String()))
	return !notEq
}
...
func (m *indexMeta) HasSameReq(req *indexpb.CreateIndexRequest) (bool, UniqueID) {
	...
	for _, fieldIndex := range m.indexes[req.CollectionID] {
		...
		mlog.Debug(m.ctx, "has same index", zap.Int64("collectionID", req.CollectionID),
...
func (m *indexMeta) CreateIndex(ctx context.Context, index *model.Index) error {
	...
	mlog.Info(ctx, "meta update: CreateIndex", zap.Int64("collectionID", index.CollectionID),
	...
	if err := m.catalog.CreateIndex(ctx, index); err != nil {
		mlog.Error(ctx, "meta update: CreateIndex save meta fail", zap.Int64("collectionID", index.CollectionID),
	...
	mlog.Info(ctx, "meta update: CreateIndex success", zap.Int64("collectionID", index.CollectionID),
		zap.Int64("fieldID", index.FieldID), zap.Int64("indexID", index.IndexID), zap.String("indexName", index.IndexName))
	return nil
}
...
func (m *indexMeta) AddSegmentIndex(ctx context.Context, segIndex *model.SegmentIndex) error {
	...
	mlog.Info(ctx, "meta update: adding segment index", zap.Int64("collectionID", segIndex.CollectionID),
	...
	mlog.Info(ctx, "meta update: adding segment index success", zap.Int64("collectionID", segIndex.CollectionID),
		zap.Int64("segmentID", segIndex.SegmentID), zap.Int64("indexID", segIndex.IndexID),
		zap.Int64("buildID", buildID), zap.String("indexType", segIndex.IndexType))
	return nil
}
...
func (m *indexMeta) MarkIndexAsDeleted(ctx context.Context, collID UniqueID, indexIDs []UniqueID) error {
	mlog.Info(ctx, "IndexCoord metaTable MarkIndexAsDeleted", zap.Int64("collectionID", collID),
	...
	err := m.catalog.AlterIndexes(ctx, indexes)
	if err != nil {
		mlog.Error(ctx, "failed to alter index meta in meta store", zap.Int("indexes num", len(indexes)), zap.Error(err))
		return err
	}
	...
	mlog.Info(ctx, "IndexCoord metaTable MarkIndexAsDeleted success", zap.Int64("collectionID", collID), zap.Int64s("indexIDs", indexIDs))
	return nil
}
...
func (m *indexMeta) UpdateVersion(buildID, nodeID UniqueID) error {
	...
	mlog.Info(m.ctx, "IndexCoord metaTable UpdateVersion receive", zap.Int64("buildID", buildID), zap.Int64("nodeID", nodeID))
	segIdx, ok := m.segmentBuildInfo.Get(buildID)
	if !ok {
...
func (m *indexMeta) UpdateIndexState(buildID UniqueID, state commonpb.IndexState, failReason string) error {
	...
	segIdx, ok := m.segmentBuildInfo.Get(buildID)
	if !ok {
		mlog.Warn(m.ctx, "there is no index with buildID", zap.Int64("buildID", buildID))
		return fmt.Errorf("there is no index with buildID: %d", buildID)
	}
	...
	if err := m.updateSegIndexMeta(segIdx, updateFunc); err != nil {
		mlog.Warn(m.ctx, "failed to update index meta", zap.Int64("buildID", buildID), zap.Error(err))
		return err
	}

	mlog.Info(m.ctx, "update index task state success", zap.Int64("buildID", buildID),
...
func (m *indexMeta) FinishTask(taskInfo *workerpb.IndexTaskInfo) error {
	...
	if !ok {
		mlog.Warn(m.ctx, "there is no index with buildID", zap.Int64("buildID", taskInfo.GetBuildID()))
		return nil
	}
	...
	mlog.Info(m.ctx, "finish index task success", zap.Int64("buildID", taskInfo.GetBuildID()),
...
func (m *indexMeta) DeleteTask(buildID int64) error {
	...
	mlog.Info(m.ctx, "delete index task success", zap.Int64("buildID", buildID))
	return nil
}
...
func (m *indexMeta) BuildIndex(buildID UniqueID) error {
	...
	updateFunc := func(segIdx *model.SegmentIndex) error {
		...
		if err != nil {
			mlog.Error(m.ctx, "meta Update: segment index in progress fail", zap.Int64("buildID", segIdx.BuildID), zap.Error(err))
			return err
		}
		return nil
	}
	...
	mlog.Info(m.ctx, "meta update: segment index in progress success", zap.Int64("buildID", segIdx.BuildID),
...
func (m *indexMeta) RemoveIndex(ctx context.Context, collID, indexID UniqueID) error {
	m.fieldIndexLock.Lock()
	defer m.fieldIndexLock.Unlock()
	mlog.Info(ctx, "IndexCoord meta table remove index", zap.Int64("collectionID", collID), zap.Int64("indexID", indexID))
	err := m.catalog.DropIndex(ctx, collID, indexID)
	if err != nil {
		mlog.Info(ctx, "IndexCoord meta table remove index fail", zap.Int64("collectionID", collID),
	...
	mlog.Info(ctx, "IndexCoord meta table remove index success", zap.Int64("collectionID", collID), zap.Int64("indexID", indexID))
	return nil
}
