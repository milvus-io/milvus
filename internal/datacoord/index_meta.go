...
func (m *indexMeta) reloadFromKV(collectionIDs []int64) error {
	...
	g.Go(func() error {
		...
		if err != nil {
			mlog.Error(m.ctx, "indexMeta reloadFromKV load field indexes fail", mlog.Err(err))
			return err
		}
...
func (m *indexMeta) alterSegmentIndexes(segIdxes []*model.SegmentIndex) error {
	...
	if err != nil {
		mlog.Error(m.ctx, "failed to alter segments index in meta store", mlog.Int("segment indexes num", len(segIdxes)),
			mlog.Err(err))
		return err
	}
	for _, segIdx := range segIdxes {
...
func (m *indexMeta) HasSameReq(req *indexpb.CreateIndexRequest) (bool, UniqueID) {
	...
	for _, fieldIndex := range m.indexes[req.CollectionID] {
		...
		mlog.Debug(m.ctx, "has same index", mlog.FieldCollectionID(req.CollectionID),
			mlog.FieldFieldID(req.FieldID), mlog.String("indexName", req.IndexName),
			mlog.FieldIndexID(fieldIndex.IndexID))
...
func (m *indexMeta) CreateIndex(ctx context.Context, index *model.Index) error {
	...
	mlog.Info(ctx, "meta update: CreateIndex", mlog.FieldCollectionID(index.CollectionID),
		mlog.FieldFieldID(index.FieldID), mlog.FieldIndexID(index.IndexID), mlog.String("indexName", index.IndexName))

	if err := m.catalog.CreateIndex(ctx, index); err != nil {
		mlog.Error(ctx, "meta update: CreateIndex save meta fail", mlog.FieldCollectionID(index.CollectionID),
			mlog.FieldFieldID(index.FieldID), mlog.FieldIndexID(index.IndexID),
			mlog.String("indexName", index.IndexName), mlog.Err(err))
		return err
	}

	m.updateCollectionIndex(index)
	mlog.Info(ctx, "meta update: CreateIndex success", mlog.FieldCollectionID(index.CollectionID),
		mlog.FieldFieldID(index.FieldID), mlog.FieldIndexID(index.IndexID), mlog.String("indexName", index.IndexName))
	return nil
}
...
func (m *indexMeta) AddSegmentIndex(ctx context.Context, segIndex *model.SegmentIndex) error {
	...
	mlog.Info(ctx, "meta update: adding segment index", mlog.FieldCollectionID(segIndex.CollectionID),
		mlog.FieldSegmentID(segIndex.SegmentID), mlog.FieldIndexID(segIndex.IndexID),
		mlog.FieldBuildID(buildID), mlog.String("indexType", segIndex.IndexType))
	...
	if err := m.catalog.CreateSegmentIndex(ctx, segIndex); err != nil {
		mlog.Warn(ctx, "meta update: adding segment index failed",
			mlog.FieldSegmentID(segIndex.SegmentID), mlog.FieldIndexID(segIndex.IndexID),
			mlog.FieldBuildID(segIndex.BuildID), mlog.String("indexType", segIndex.IndexType), mlog.Err(err))
		return err
	}
	...
	mlog.Info(ctx, "meta update: adding segment index success", mlog.FieldCollectionID(segIndex.CollectionID),
		mlog.FieldSegmentID(segIndex.SegmentID), mlog.FieldIndexID(segIndex.IndexID),
		mlog.FieldBuildID(buildID), mlog.String("indexType", segIndex.IndexType))
	return nil
}
...
func (m *indexMeta) MarkIndexAsDeleted(ctx context.Context, collID UniqueID, indexIDs []UniqueID) error {
	mlog.Info(ctx, "IndexCoord metaTable MarkIndexAsDeleted", mlog.FieldCollectionID(collID),
	...
	mlog.Info(ctx, "IndexCoord metaTable MarkIndexAsDeleted success", mlog.FieldCollectionID(collID), mlog.Int64s("indexIDs", indexIDs))
	return nil
}
...
func (m *indexMeta) UpdateVersion(buildID, nodeID UniqueID) error {
	...
	mlog.Info(m.ctx, "IndexCoord metaTable UpdateVersion receive", mlog.FieldBuildID(buildID), mlog.FieldNodeID(nodeID))
	segIdx, ok := m.segmentBuildInfo.Get(buildID)
	if !ok {
...
func (m *indexMeta) UpdateIndexState(buildID UniqueID, state commonpb.IndexState, failReason string) error {
	...
	segIdx, ok := m.segmentBuildInfo.Get(buildID)
	if !ok {
		mlog.Warn(m.ctx, "there is no index with buildID", mlog.FieldBuildID(buildID))
		return fmt.Errorf("there is no index with buildID: %d", buildID)
	}
	...
	if err := m.updateSegIndexMeta(segIdx, updateFunc); err != nil {
		mlog.Warn(m.ctx, "failed to update index meta", mlog.FieldBuildID(buildID), mlog.Err(err))
		return err
	}

	mlog.Info(m.ctx, "update index task state success", mlog.FieldBuildID(buildID),
...
func (m *indexMeta) FinishTask(taskInfo *workerpb.IndexTaskInfo) error {
	...
	if !ok {
		mlog.Warn(m.ctx, "there is no index with buildID", mlog.FieldBuildID(taskInfo.GetBuildID()))
		return nil
	}
	...
	if err := validateIndexStorePathVersionForFinish(taskInfo.GetBuildID(), segIdx.IndexStorePathVersion, actualPathVersion); err != nil {
		mlog.Warn(m.ctx, "invalid index store path version returned by worker",
			mlog.FieldBuildID(taskInfo.GetBuildID()),
	...
	mlog.Info(m.ctx, "finish index task success", mlog.FieldBuildID(taskInfo.GetBuildID()),
...
func (m *indexMeta) DeleteTask(buildID int64) error {
	...
	mlog.Info(m.ctx, "delete index task success", mlog.FieldBuildID(buildID))
	return nil
}
...
func (m *indexMeta) BuildIndex(buildID UniqueID) error {
	...
	updateFunc := func(segIdx *model.SegmentIndex) error {
		...
		if err != nil {
			mlog.Error(m.ctx, "meta Update: segment index in progress fail", mlog.FieldBuildID(segIdx.BuildID), mlog.Err(err))
			return err
		}
		return nil
	}
	...
	mlog.Info(m.ctx, "meta update: segment index in progress success", mlog.FieldBuildID(segIdx.BuildID),
...
func (m *indexMeta) RemoveIndex(ctx context.Context, collID, indexID UniqueID) error {
	m.fieldIndexLock.Lock()
	defer m.fieldIndexLock.Unlock()
	mlog.Info(ctx, "IndexCoord meta table remove index", mlog.FieldCollectionID(collID), mlog.FieldIndexID(indexID))
	err := m.catalog.DropIndex(ctx, collID, indexID)
	if err != nil {
		mlog.Info(ctx, "IndexCoord meta table remove index fail", mlog.FieldCollectionID(collID),
			mlog.FieldIndexID(indexID), mlog.Err(err))
		return err
	}
	...
	mlog.Info(ctx, "IndexCoord meta table remove index success", mlog.FieldCollectionID(collID), mlog.FieldIndexID(indexID))
	return nil
}
