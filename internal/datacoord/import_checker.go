...
func (c *importChecker) Start() {
	...
	for {
		...
		case <-ticker2.C:
			// TODO fubang update segment state to the dropped for the invisible segments
			ctx := context.TODO()
	...
	for channel, targetSegments := range importSegments {
		...
		for partitionID, targetSegments := range importSegmentsForPartition {
			...
			task := &datapb.CompactionTask{
				TriggerID:        job.GetJobID(), // inner trigger, use import segment id as trigger id
