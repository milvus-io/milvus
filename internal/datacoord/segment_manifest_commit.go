...
func (m *meta) commitSegmentManifestsWithActions(
	...
	for _, commit := range commits {
		...
		if len(commit.CatalogMutation.SegmentIndexes) > 0 {
			...
			return nil, nil, merr.WrapErrServiceInternalMsg("batch segment manifest commit cannot mutate a segment index, segmentID=%d", commit.SegmentID)
		}
		if manifestMutationAddsIndexEntry(commit.Mutation) {
			...
			return nil, nil, merr.WrapErrServiceInternalMsg("batch segment manifest commit cannot publish a segment index, segmentID=%d", commit.SegmentID)
		}
