...
type GcOption struct {
	...
	metaCheckInterval time.Duration        // check meta gc each interval
	fileCheckInterval time.Duration        // check file gc each interval
...
func newGarbageCollector(meta *meta, handler Handler, opt GcOption) *garbageCollector {
	log.Info("GC with option", zap.Bool("enabled", opt.enabled), zap.Duration("metaCheckInterval", opt.metaCheckInterval), zap.Duration("fileCheckInterval", opt.fileCheckInterval),
...
func (gc *garbageCollector) work() {
	...
	gc.wg.Add(1)
	go gc.runRecycleTaskWithPauser("meta", gc.option.metaCheckInterval, func() {
		gc.clearEtcd()
	...
	gc.wg.Add(1)
	go gc.runRecycleTaskWithPauser("file", gc.option.fileCheckInterval, func() {
