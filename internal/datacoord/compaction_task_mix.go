...
func (t *mixCompactionTask) CreateTaskOnWorker(nodeID int64, cluster session.Cluster) {
	...
	if err != nil {
		...
		if err != nil {
			if t.ctx.Err() == nil {
				mlog.Fatal(t.ctx, "mixCompactionTask failed to persist build failure; terminating process",
...
func (t *mixCompactionTask) QueryTaskOnWorker(cluster session.Cluster) {
	...
	if err != nil || result == nil {
		...
		if errors.Is(err, merr.ErrCompactionResultNotFound) {
			// Cleanup releases the old worker task; replacement uses a fresh
			// plan ID, so retry does not need to wait for that release.
			log.Warn(t.ctx, "mixCompactionTask result unavailable, abandoning attempt for replan", mlog.Err(err))
...
func (t *mixCompactionTask) DropTaskOnWorker(cluster session.Cluster) {
	dropCompactionTaskOnWorker(t.ctx, t.GetTask(), cluster, "mixCompactionTask")
}
...
func (t *mixCompactionTask) saveSegmentMeta(result *datapb.CompactionPlanResult) error {
	...
	// Apply metrics after the combined segment/task catalog update succeeds.
...
func (t *mixCompactionTask) doClean() error {
	...
	return finishClean(t.ctx, t, "mixCompactionTask")
}
...
func (t *mixCompactionTask) BuildCompactionRequest() (*datapb.CompactionPlan, error) {
	...
	if fileresource.IsRefMode(paramtable.Get().CommonCfg.DNFileResourceMode.GetValue()) &&
		...
		if err != nil {
			mlog.Warn(t.ctx, "get file resources for collection failed", mlog.FieldCollectionID(taskProto.GetCollectionID()), mlog.Err(err))
