...
func (m *CompactionTriggerManager) notify(ctx context.Context, eventType CompactionTriggerType, views []CompactionView) {
	...
	for _, view := range views {
		...
		if outView != nil {
			...
			case TriggerTypeSingle:
				m.SubmitSingleViewToScheduler(ctx, outView, false)
			case TriggerTypeSort:
				m.SubmitSingleViewToScheduler(ctx, outView, true)
			}
		}
	}
}
...
func (m *CompactionTriggerManager) SubmitSingleViewToScheduler(ctx context.Context, view CompactionView, isSort bool) {
	log := log.Ctx(ctx).With(zap.String("view", view.String()))
