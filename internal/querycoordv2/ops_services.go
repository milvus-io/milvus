...
func (s *Server) GetQueryNodeDistribution(ctx context.Context, req *querypb.GetQueryNodeDistributionRequest) (*querypb.GetQueryNodeDistributionResponse, error) {
	...
	segments := s.dist.SegmentDistManager.GetByNode(req.GetNodeID())
...
func (s *Server) TransferSegment(ctx context.Context, req *querypb.TransferSegmentRequest) (*commonpb.Status, error) {
	...
	for _, replica := range replicas {
		...
		segments := s.dist.SegmentDistManager.GetByCollectionAndNode(replica.GetCollectionID(), srcNode)
		...
		err := s.balanceSegments(ctx, replica.GetCollectionID(), replica.GetID(), srcNode, dstNodeSet.Collect(), toBalance.Collect(), false, req.GetCopyMode())
		if err != nil {
...
func (s *Server) TransferChannel(ctx context.Context, req *querypb.TransferChannelRequest) (*commonpb.Status, error) {
	...
	for _, replica := range replicas {
		...
		err := s.balanceChannels(ctx, replica.GetCollectionID(), replica.GetID(), srcNode, dstNodeSet.Collect(), toBalance.Collect(), false, req.GetCopyMode())
		if err != nil {
...
func (s *Server) CheckQueryNodeDistribution(ctx context.Context, req *querypb.CheckQueryNodeDistributionRequest) (*commonpb.Status, error) {
	...
	// check segment list
	segmentOnSrc := s.dist.SegmentDistManager.GetByNode(req.GetSourceNodeID())
	segmentOnDst := s.dist.SegmentDistManager.GetByNode(req.GetTargetNodeID())
