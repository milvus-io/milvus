...
func (m *ReplicaManager) SpawnWithReplicaConfig(ctx context.Context, params SpawnWithReplicaConfigParams) ([]*Replica, error) {
	...
	// Pre-compute the redundant replicas that are not in the new replica
	// config. All newly spawned replica IDs come from params.Configs, so
	// computing this before persistence is equivalent to computing it after.
	...
	// compound catalog call; on failure the in-memory state is left
	// untouched and the error propagates to the caller for retry.
	actions := make([]metastore.UpdateAction, 0, len(replicas)+len(toRemove))
	for _, replica := range replicas {
		actions = append(actions, metastore.SaveReplica(replica.replicaPB))
	}
	for _, replicaID := range toRemove {
		actions = append(actions, metastore.ReleaseReplica(params.CollectionID, replicaID))
	}
	if len(actions) > 0 {
