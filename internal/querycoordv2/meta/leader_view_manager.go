...
func WithChannelName(channelName string) LeaderViewFilter {
	return func(view *LeaderView) bool {
		return view.Channel == channelName
	}
}

func WithCollectionID(collectionID int64) LeaderViewFilter {
	return func(view *LeaderView) bool {
		return view.CollectionID == collectionID
	}
}

func WithNodeID(nodeID int64) LeaderViewFilter {
	return func(view *LeaderView) bool {
		return view.ID == nodeID
	}
}

func WithReplica(replica *Replica) LeaderViewFilter {
	return func(view *LeaderView) bool {
		return replica.GetCollectionID() == view.CollectionID && replica.Contains(view.ID)
	}
}

func WithSegment(segmentID int64, isGrowing bool) LeaderViewFilter {
	return func(view *LeaderView) bool {
		if isGrowing {
