...
// CheckOutboundNodes return all outbound node
// OutboundNode is the node in replica but not in rg of replica.
...
// OutgoingNode is the node in replica but not in rg of replica.
...
func (rm *ResourceManager) validateResourceGroupConfig(rgName string, cfg *rgpb.ResourceGroupConfig) error {
	...
	if cfg.Requests.NodeNum < 0 || cfg.Limits.NodeNum < 0 {
		return errors.Wrap(ErrIllegalRGConfig, "node num in `requests` or `limits` should not less than 0")
	}
	if cfg.Limits.NodeNum < cfg.Requests.NodeNum {
		return errors.Wrap(ErrIllegalRGConfig, "limits node num should not less than requests node num")
	}

	for _, transferCfg := range cfg.From {
		if transferCfg.ResourceGroup == rgName {
			return errors.Wrapf(ErrIllegalRGConfig, "resource group in `from` %s should not be itself", rgName)
		}
		if rm.groups[transferCfg.ResourceGroup] == nil {
			return errors.Wrapf(ErrIllegalRGConfig, "resource group in `from` %s not exist", transferCfg.ResourceGroup)
		}
	}
	for _, transferCfg := range cfg.To {
		if transferCfg.ResourceGroup == rgName {
			return errors.Wrapf(ErrIllegalRGConfig, "resource group in `to` %s should not be itself", rgName)
		}
		if rm.groups[transferCfg.ResourceGroup] == nil {
			return errors.Wrapf(ErrIllegalRGConfig, "resource group in `to` %s not exist", transferCfg.ResourceGroup)
		}
	}
	return nil
}
...
func (rm *ResourceManager) validateResourceGroupIsDeletable(rgName string) error {
	...
	if rm.groups[rgName].GetConfig().Limits.NodeNum != 0 {
		return ErrDeleteNonEmptyRG
	}

	// If rg is used by other rg, it's not deletable.
	for _, rg := range rm.groups {
		for _, transferCfg := range rg.GetConfig().From {
			if transferCfg.ResourceGroup == rgName {
		...
		for _, transferCfg := range rg.GetConfig().To {
			if transferCfg.ResourceGroup == rgName {
