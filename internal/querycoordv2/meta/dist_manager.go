...
// PatchNodeDistribution publishes a delta distribution update for a node under the
// same publish lock as PublishNodeDistribution, so a snapshot can never observe the
// segment half of an update without the channel half.
func (dm *DistributionManager) PatchNodeDistribution(
	...
	dm.segmentManager.patchLocked(nodeID, segments, removedSegmentIDs)
	if dm.publishHook != nil {
		dm.publishHook(publishStageSegmentsWritten)
	}
	return dm.channelManager.patchLocked(nodeID, channels, removedChannelNames)
}
