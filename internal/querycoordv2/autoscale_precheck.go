...
const (
	...
	queryNodeMetricsTimeout             = 10 * time.Second
...
type queryNodeResourceStatus struct {
	...
	capacity  autoscaleResourceUsage // capacity before memory and disk admission thresholds
}
...
func canExpandWithinGlobalLimit(shortage, globalCapacity, globalLimit autoscaleResourceUsage) bool {
	remainingCapacity := autoscaleResourceUsage{
		memoryBytes: max(globalLimit.memoryBytes-globalCapacity.memoryBytes, 0),
		diskBytes:   max(globalLimit.diskBytes-globalCapacity.diskBytes, 0),
	}
	availableAfterScale := usableResourceCapacity(remainingCapacity)
	return shortage.memoryBytes <= availableAfterScale.memoryBytes &&
		shortage.diskBytes <= availableAfterScale.diskBytes
}
...
func (s *Server) checkLoadResource(ctx context.Context, req *job.AlterLoadConfigRequest) (map[string]autoscaleResourceUsage, error) {
	if !paramtable.Get().QueryCoordCfg.AutoscalePrecheckEnabled.GetAsBool() {
		return nil, nil
	}

	requiredByRG, err := s.evaluateLoadResource(ctx, req)
	if err == nil {
		return requiredByRG, nil
	}
...
func (s *Server) evaluateLoadResource(ctx context.Context, req *job.AlterLoadConfigRequest) (map[string]autoscaleResourceUsage, error) {
	...
	// Physical usage includes evictable caches and cannot establish whether a
	// tiered node can admit this load. Preserve demand for the broadcast counter.
...
func recordLoadResourceDemand(requiredByRG map[string]autoscaleResourceUsage) {
	for rgName, usage := range requiredByRG {
		metrics.QueryCoordLoadDemandMemoryBytes.WithLabelValues(rgName).Add(float64(usage.memoryBytes))
		metrics.QueryCoordLoadDemandDiskBytes.WithLabelValues(rgName).Add(float64(usage.diskBytes))
	}
}
...
func (s *Server) requiredLoadResourceByRG(ctx context.Context, req *job.AlterLoadConfigRequest) (map[string]autoscaleResourceUsage, error) {
	...
	// Recovery info only carries compact segment metadata, while resource
	// estimation needs the complete binlog and statistics payload.
...
func (s *Server) currentQueryNodeResourceStatus(ctx context.Context, requiredByRG map[string]autoscaleResourceUsage, includeGlobal bool) (map[string]autoscaleResourceUsage, map[string]autoscaleResourceUsage, autoscaleResourceUsage, error) {
	...
	for rgName, nodes := range nodesByRG {
		...
		capacityByRG[rgName] = usableResourceCapacity(capacity)
	}
...
func (s *Server) onlineQueryNodes() []*session.NodeInfo {
	...
	return lo.Filter(s.nodeMgr.GetAll(), func(node *session.NodeInfo, _ int) bool {
		return node != nil && !node.IsStoppingState() && !node.IsEmbeddedQueryNodeInStreamingNode()
	})
}
...
func (s *Server) queryNodeResourceSnapshot(ctx context.Context, nodes []*session.NodeInfo) (map[int64]queryNodeResourceStatus, error) {
	...
	if s.cluster == nil {
		diskCapacityPerNode := paramtable.Get().QueryNodeCfg.DiskCapacityLimit.GetAsInt64()
		for _, node := range nodes {
			capacity := autoscaleResourceUsage{diskBytes: diskCapacityPerNode}
			if node.MemCapacity() > 0 {
				capacity.memoryBytes = int64(node.MemCapacity() * 1024 * 1024)
			}
			snapshot[node.ID()] = queryNodeResourceStatus{available: usableResourceCapacity(capacity), capacity: capacity}
		}
		return snapshot, nil
	}
	...
	metricsCtx, cancel := context.WithTimeout(ctx, queryNodeMetricsTimeout)
	defer cancel()
	metricsCtx = retry.WithMaxAttemptsContext(metricsCtx, 1)
...
func usableResourceCapacity(capacity autoscaleResourceUsage) autoscaleResourceUsage {
	return autoscaleResourceUsage{
		memoryBytes: int64(float64(capacity.memoryBytes) * paramtable.Get().QuotaConfig.QueryNodeMemoryHighWaterLevel.GetAsFloat()),
		diskBytes:   int64(float64(capacity.diskBytes) * paramtable.Get().QueryNodeCfg.MaxDiskUsagePercentage.GetAsFloat()),
	}
}
