...
// mixCoord-level bump_defence readiness provider can read the dist report. The witness
// reads per-segment loaded schema/data/manifest version; it never needs the channel dist
// or the manager's write methods, so the return is narrowed to the read interface rather
// than the whole DistributionManager. Returns nil before Init builds the dist manager.
