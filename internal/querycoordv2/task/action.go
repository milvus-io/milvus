...
func (action *SegmentAction) IsFinished(distMgr *meta.DistributionManager) bool {
	if action.Type() == ActionTypeGrow {
		views := distMgr.GetByFilter(meta.WithSegment(action.segmentID, false))
		...
		views := distMgr.LeaderViewManager.GetByFilter(meta.WithNodeID(action.Node()))
...
func (action *ChannelAction) IsFinished(distMgr *meta.DistributionManager) bool {
	views := distMgr.GetByFilter(meta.WithChannelName(action.ChannelName()))
...
func (action *LeaderAction) IsFinished(distMgr *meta.DistributionManager) bool {
	views := distMgr.LeaderViewManager.GetByFilter(meta.WithNodeID(action.leaderID), meta.WithChannelName(action.Shard()))
	if len(views) == 0 {
		return false
	}
