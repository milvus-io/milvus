...
func (ex *Executor) loadSegment(task *SegmentTask, step int) error {
	...
	view := ex.dist.ShardLeaderManager.GetShardLeader(replica.GetID(), task.Shard())
	if view == nil {
	...
	log = log.With(zap.Int64("shardLeader", view.NodeID))

	startTs := time.Now()
	log.Info("load segments...")
	status, err := ex.cluster.LoadSegments(task.Context(), view.NodeID, req)
