...
func (suite *TaskSuite) TestAutoscaleInflightLoadMetrics() {
	...
	compactSegment := &datapb.SegmentInfo{
		ID:           segmentID,
		CollectionID: collectionID,
		PartitionID:  partitionID,
		NumOfRows:    100,
	...
	suite.broker.EXPECT().GetRecoveryInfoV2(mock.Anything, collectionID).Return(nil, []*datapb.SegmentInfo{compactSegment}, nil)
	suite.NoError(suite.target.UpdateCollectionNextTarget(ctx, collectionID))
	suite.broker.EXPECT().GetSegmentInfo(mock.Anything, segmentID).Return([]*datapb.SegmentInfo{segment}, nil).Twice()
	...
	// The selected index contributes 30 bytes; the complete SegmentInfo fetched
	// by the scheduler contributes another 20 bytes of stats and 10 bytes of deletes.
...
func (suite *TaskSuite) TestAutoscaleInflightLoadMetricsSkipEstimateFailure() {
	...
	suite.NoError(suite.target.UpdateCollectionNextTarget(ctx, collectionID))
	suite.broker.EXPECT().GetSegmentInfo(mock.Anything, segmentID).Return([]*datapb.SegmentInfo{segment}, nil)
	suite.broker.EXPECT().DescribeCollection(mock.Anything, collectionID).Return(&milvuspb.DescribeCollectionResponse{
		Schema: &schemapb.CollectionSchema{
...
func (suite *TaskSuite) TestAutoscaleInflightLoadMetricsSkipSegmentInfoFailure() {
	ctx := context.Background()
	scheduler := suite.newScheduler()
	collectionID := int64(2102)
	partitionID := int64(100)
	targetNode := int64(1)
	targetNodeLabel := "1"
	targetNodeHost := "querynode-1"
	segmentIDs := []int64{3102, 3103}
	suite.nodeMgr.Add(session.NewNodeInfo(session.ImmutableNodeInfo{NodeID: targetNode, Hostname: targetNodeHost}))
	metrics.QueryCoordAutoscaleInflightLoadMemoryBytes.WithLabelValues(targetNodeLabel, targetNodeHost).Set(0)
	metrics.QueryCoordAutoscaleInflightLoadDiskBytes.WithLabelValues(targetNodeLabel, targetNodeHost).Set(0)
	replica := meta.NewReplica(&querypb.Replica{
	...
	suite.NoError(suite.meta.PutCollection(ctx, utils.CreateTestCollection(collectionID, 1)))
	suite.NoError(suite.meta.PutPartition(ctx, utils.CreateTestPartition(collectionID, partitionID)))
	suite.meta.Put(ctx, replica)

	segments := lo.Map(segmentIDs, func(segmentID int64, _ int) *datapb.SegmentInfo {
		return &datapb.SegmentInfo{
			ID:           segmentID,
			CollectionID: collectionID,
			PartitionID:  partitionID,
	...
	suite.broker.EXPECT().GetRecoveryInfoV2(mock.Anything, collectionID).Return(nil, segments, nil)
	suite.NoError(suite.target.UpdateCollectionNextTarget(ctx, collectionID))
	suite.broker.EXPECT().GetSegmentInfo(mock.Anything, segmentIDs[0]).Return(nil, errors.New("get segment info failed"))
	suite.broker.EXPECT().GetSegmentInfo(mock.Anything, segmentIDs[1]).Return(nil, nil)
	...
	suite.Equal(float64(0), testutil.ToFloat64(metrics.QueryCoordAutoscaleInflightLoadMemoryBytes.WithLabelValues(targetNodeLabel, targetNodeHost)))
	suite.Equal(float64(0), testutil.ToFloat64(metrics.QueryCoordAutoscaleInflightLoadDiskBytes.WithLabelValues(targetNodeLabel, targetNodeHost)))
	suite.broker.AssertNotCalled(suite.T(), "DescribeCollection", mock.Anything, collectionID)
	suite.broker.AssertNotCalled(suite.T(), "GetIndexInfo", mock.Anything, collectionID, mock.Anything)
}
