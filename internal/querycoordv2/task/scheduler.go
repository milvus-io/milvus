...
func (scheduler *taskScheduler) estimateAutoscaleInflightLoadResourceForTask(task Task) *autoscale.ResourceUsage {
	...
	segments, err := scheduler.broker.GetSegmentInfo(scheduler.ctx, segmentTask.SegmentID())
	if err != nil {
		mlog.RatedWarn(scheduler.ctx, rate.Limit(1), "failed to get segment info for autoscale in-flight load resource estimation",
			mlog.FieldCollectionID(segmentTask.CollectionID()),
	...
	if len(segments) == 0 || segments[0] == nil {
		mlog.RatedWarn(scheduler.ctx, rate.Limit(1), "empty segment info for autoscale in-flight load resource estimation",
			mlog.FieldCollectionID(segmentTask.CollectionID()),
			mlog.FieldSegmentID(segmentTask.SegmentID()),
		)
		return nil
	}
	return scheduler.estimateAutoscaleInflightLoadResource(segmentTask.CollectionID(), segments[:1])
}
