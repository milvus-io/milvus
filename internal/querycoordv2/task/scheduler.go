...
func (scheduler *taskScheduler) AddExecutor(nodeID int64) {
	...
	mlog.Info(scheduler.ctx, "add executor for new QueryNode", zap.Int64("nodeID", nodeID))
}
...
func (scheduler *taskScheduler) RemoveExecutor(nodeID int64) {
	...
	if ok {
		executor.Stop()
		mlog.Info(scheduler.ctx, "remove executor of offline QueryNode", zap.Int64("nodeID", nodeID))
	}
}
...
func (scheduler *taskScheduler) Add(task Task) error {
	...
	mlog.Info(task.Context(), "task added", zap.String("task", task.String()))
	task.RecordStartTs()
	return nil
}
...
func (scheduler *taskScheduler) promote(task Task) error {
	...
	if err := scheduler.check(task, true); err != nil {
		log.Info(scheduler.ctx, "failed to promote task", zap.Error(err))
		return err
	}
...
func (scheduler *taskScheduler) GetTasksJSON() string {
	...
	if err != nil {
		mlog.Warn(scheduler.ctx, "marshal tasks fail", zap.Error(err))
		return ""
	}
	return string(ret)
}
	...
	for step < len(actions) && actions[step].IsFinished(scheduler.distMgr) {
		if GetTaskType(task) == TaskTypeMove && actions[step].Type() == ActionTypeGrow {
			...
			case *ChannelAction:
				...
				mlog.Debug(scheduler.ctx, "process channelAction", zap.Bool("delegator is Nil", delegator == nil))
				if delegator != nil {
					mlog.Debug(scheduler.ctx, "process channelAction", zap.Int64("delegator node", delegator.Node),
...
func (scheduler *taskScheduler) remove(task Task) {
	...
	if errors.Is(task.Err(), merr.ErrSegmentNotFound) {
		log.Info(scheduler.ctx, "segment in target has been cleaned, trigger force update next target", zap.Int64("collectionID", task.CollectionID()))
	...
	if errors.Is(task.Err(), merr.ErrSegmentRequestResourceFailed) {
		for _, action := range task.Actions() {
			if action.Type() == ActionTypeGrow {
				...
				log.Info(scheduler.ctx, "mark resource exhaustion for node", zap.Int64("nodeID", nodeID), zap.Duration("duration", duration), zap.Error(task.Err()))
			}
		}
	}
	...
	log.Info(scheduler.ctx, "task removed")
...
func (scheduler *taskScheduler) checkStale(task Task, checkDistExist bool) error {
	...
	if task.ReplicaID() != -1 {
		...
		if replica == nil {
			mlog.Warn(scheduler.ctx, "task stale due to replica not found", zap.String("task", task.String()))
	...
	if checkDistExist {
		if segmentTask, ok := task.(*SegmentTask); ok && GetTaskType(task) == TaskTypeGrow && replica != nil {
			...
			if len(existsInDist) > 0 {
				mlog.Info(scheduler.ctx, "task stale due to segment already loaded in dist",
	...
	for _, action := range task.Actions() {
		...
		if nodeInfo == nil {
			mlog.Warn(scheduler.ctx, "task stale due to node not found", zap.String("task", task.String()), zap.Int64("nodeID", targetNode))
			return merr.WrapErrNodeNotFound(targetNode)
		}
		if action.Type() == ActionTypeGrow {
			if nodeInfo.IsStoppingState() {
				mlog.Warn(scheduler.ctx, "task stale due to node offline", zap.String("task", task.String()), zap.Int64("nodeID", targetNode))
			...
			if replica != nil && (replica.ContainRONode(targetNode) || replica.ContainROSQNode(targetNode)) {
				mlog.Warn(scheduler.ctx, "task stale due to node becomes ro node", zap.String("task", task.String()), zap.Int64("nodeID", targetNode))
