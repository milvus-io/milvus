...
func (scheduler *taskScheduler) preAdd(task Task) error {
	...
	case *SegmentTask:
		...
		if taskType == TaskTypeMove {
			leader := scheduler.distMgr.ShardLeaderManager.GetShardLeader(task.ReplicaID(), task.Shard())
			if leader == nil {
	...
	for step < len(actions) && actions[step].IsFinished(scheduler.distMgr) {
		if GetTaskType(task) == TaskTypeMove && actions[step].Type() == ActionTypeGrow {
			...
			case *ChannelAction:
				...
				delegator := scheduler.distMgr.ShardLeaderManager.GetShardLeader(task.ReplicaID(), task.Shard())
				newDelegatorReady = delegator != nil && delegator.NodeID == action.Node()
			default:
