...
func (suite *ServerSuite) TestDisableActiveStandby() {
	...
	suite.server, err = newQueryCoord()
	suite.NoError(err)
	suite.Equal(commonpb.StateCode_Initializing, suite.server.stateCode.Load().(commonpb.StateCode))
	suite.hackServer()
	...
	suite.Equal(commonpb.StateCode_Healthy, suite.server.stateCode.Load().(commonpb.StateCode))
