...
func (b *RowCountBasedBalancer) balanceReplica(replica *meta.Replica) ([]SegmentAssignPlan, []ChannelAssignPlan) {
	...
	for _, nid := range nodes {
		...
		log.Debug("simfg test node", zap.Int64("nid", nid), zap.Int("len", len(segments)))
		...
		nodesRowCnt[nid] = cnt
		log.Debug("simfg test node", zap.Int64("nid", nid), zap.Int("cnt", cnt), zap.Int("len", len(segments)))
	...
	neededRowCnt := 0
	for nodeID, _ := range nodesSegments {
		rowcnt := nodesRowCnt[nodeID]
		if rowcnt < average {
			neededRowCnt += average - rowcnt
		}
	}
	log.Debug("simfg test", zap.Int("average", average), zap.Int("need", neededRowCnt))
