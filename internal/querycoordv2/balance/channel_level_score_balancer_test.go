...
func (suite *ChannelLevelScoreBalancerTestSuite) TestExclusiveChannelBalance_ChannelOutBound() {
	...
	collection.Status = querypb.LoadStatus_Loaded
	balancer.meta.PutCollection(ctx, collection)
	balancer.meta.PutPartition(ctx, utils.CreateTestPartition(collectionID, partitionID))
	balancer.meta.Spawn(ctx, 1, map[string]int{meta.DefaultResourceGroupName: 1}, []string{"channel1", "channel2"},
	...
	for i := 0; i < nodeCount; i++ {
		...
		nodeInfo.SetState(session.NodeStateNormal)
		suite.balancer.nodeManager.Add(nodeInfo)
		suite.balancer.meta.HandleNodeUp(ctx, nodeInfo.ID())
	}
	utils.RecoverAllCollection(balancer.meta)

	replica := balancer.meta.GetByCollection(ctx, collectionID)[0]
...
func (suite *ChannelLevelScoreBalancerTestSuite) TestExclusiveChannelBalance_SegmentOutbound() {
	...
	balancer.meta.PutCollection(ctx, collection)
	balancer.meta.PutPartition(ctx, utils.CreateTestPartition(collectionID, partitionID))
	balancer.meta.Spawn(ctx, 1, map[string]int{meta.DefaultResourceGroupName: 1}, []string{"channel1", "channel2"}, commonpb.LoadPriority_LOW)
	balancer.targetMgr.UpdateCollectionNextTarget(ctx, collectionID)
	balancer.targetMgr.UpdateCollectionCurrentTarget(ctx, collectionID)
