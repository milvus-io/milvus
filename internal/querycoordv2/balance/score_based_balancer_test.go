...
func (suite *ScoreBasedBalancerTestSuite) SetupTest() {
	...
	suite.mockScheduler.EXPECT().GetNodeChannelDelta(mock.Anything).Return(0)
...
func (suite *ScoreBasedBalancerTestSuite) TestBalanceMultiRound() {
	balanceCase := struct {
		...
		distributions: []map[int64][]*meta.Segment{
			...
			},
			{
				...
				3: {
					{SegmentInfo: &datapb.SegmentInfo{ID: 1, CollectionID: 1, NumOfRows: 20}, Node: 3},
				},
			},
		},
		expectPlans: [][]SegmentAssignPlan{
	...
	for i := range balanceCase.collectionIDs {
		...
		collection.LoadType = querypb.LoadType_LoadCollection
		balancer.meta.CollectionManager.PutCollection(collection)
		balancer.meta.ReplicaManager.Put(utils.CreateTestReplica(int64(balanceCase.replicaIDs[i]), balanceCase.collectionIDs[i],
