...
func (ob *TargetObserver) shouldUpdateCurrentTarget(ctx context.Context, collectionID int64) bool {
	...
	for _, replica := range replicas {
		...
		for channel := range channelNames {
			...
			// If no ready delegator exists for this channel in this replica, mark replica as not ready
			if len(readyDelegatorsInChannel) == 0 {
				replicaReady = false
				break
			}

			readyDelegatorsInReplica = append(readyDelegatorsInReplica, delegatorList...)
		}

		// Only include delegators from replicas where all channels have at least one ready delegator
		if replicaReady {
