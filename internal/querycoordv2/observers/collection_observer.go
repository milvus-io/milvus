...
func (ob *CollectionObserver) observeTimeout() {
	...
	for _, collection := range collections {
		...
		log.Warn("[collection-observer] load collection timeout",
	...
	for collection, partitions := range partitions {
		for _, partition := range partitions {
			...
			log.Warn("[collection-observer] load partition timeout",
...
func (ob *CollectionObserver) observePartitionLoadStatus(partition *meta.Partition, replicaNum int32) {
	...
	if targetNum == 0 {
		log.Info("[collection-observer] no segment/channel in target need to be loaded")
		loadPercentage = 100
	} else {
		log.Info("[collection-observer] partition targets",
		...
		if loadedCount > 0 {
			log.Info("[collection-observer] partition load progress",
	...
	if err != nil {
		log.Warn("[collection-observer] failed to update load percentage")
		return
	}

	log.Info("[collection-observer] load status updated",
