...
func (ob *ReplicaObserver) checkNodesInReplica() {
	...
	for _, collectionID := range collections {
		...
		for _, replica := range replicas {
			outboundNodes := balance.CheckReplicaUseOutboundsNode(replica, ob.meta)
			...
			for node := range outboundNodes {
				...
				if len(channels) == 0 && len(segments) == 0 {
					...
					log.Info("all segment/channel has been removed from outboud node, remove it from replica",
						zap.Int64("collectionID", replica.GetCollectionID()),
						zap.Int64("replicaID", replica.GetCollectionID()),
