...
func (suite *ResourceObserverSuite) SetupTest() {
	...
	for i := 0; i < 10; i++ {
		...
		suite.meta.HandleNodeUp(suite.ctx, int64(i))
	}
}
...
func (suite *ResourceObserverSuite) TestObserverRecoverOperation() {
	ctx := suite.ctx
	suite.meta.AddResourceGroup(ctx, "rg", &rgpb.ResourceGroupConfig{
		Requests: &rgpb.ResourceGroupLimit{NodeNum: 4},
		Limits:   &rgpb.ResourceGroupLimit{NodeNum: 6},
	})
	suite.Error(suite.meta.MeetRequirement(ctx, "rg"))
	...
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg"))
	...
	suite.Error(suite.meta.MeetRequirement(ctx, "rg2"))
	// There's 10 exists node in cluster, new incoming resource group should get 6 nodes after recover.
	suite.observer.checkAndRecoverResourceGroup(ctx)
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg1"))
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg2"))

	suite.meta.AddResourceGroup(ctx, "rg3", &rgpb.ResourceGroupConfig{
		Requests: &rgpb.ResourceGroupLimit{NodeNum: 1},
		Limits:   &rgpb.ResourceGroupLimit{NodeNum: 1},
	})
	suite.Error(suite.meta.MeetRequirement(ctx, "rg3"))
	...
	suite.meta.HandleNodeUp(ctx, 10)
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg1"))
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg2"))
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg3"))
	// new node is down, rg3 cannot use that node anymore.
	suite.nodeMgr.Remove(10)
	suite.meta.HandleNodeDown(ctx, 10)
	suite.observer.checkAndRecoverResourceGroup(ctx)
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg1"))
	suite.NoError(suite.meta.MeetRequirement(ctx, "rg2"))
	suite.Error(suite.meta.MeetRequirement(ctx, "rg3"))
...
func (suite *ResourceObserverSuite) TestSchedule() {
	...
	check := func() {
		suite.Eventually(func() bool {
			rgs := suite.meta.ListResourceGroups(ctx)
			for _, rg := range rgs {
	...
	for i := 1; i <= 4; i++ {
		suite.meta.AddResourceGroup(ctx, fmt.Sprintf("rg%d", i), &rgpb.ResourceGroupConfig{
