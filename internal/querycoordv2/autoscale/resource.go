...
func FilterSegmentsByLoadFields(segments []*datapb.SegmentInfo, loadFields []int64) []*datapb.SegmentInfo {
	...
	return lo.Map(segments, func(segment *datapb.SegmentInfo, _ int) *datapb.SegmentInfo {
		...
		filterLoadResourceStatisticsByLoadFields(filtered.GetStats(), loadFieldSet)
		return filtered
	})
}

func filterLoadResourceStatisticsByLoadFields(stats *datapb.Statistics, loadFields typeutil.Set[int64]) {
	resource := stats.GetLoadResource()
	if resource == nil {
		return
	}
	resource.ColumnGroups = lo.Filter(resource.GetColumnGroups(), func(group *datapb.ColumnGroupStatistics, _ int) bool {
		fieldIDs := group.GetFieldIds()
		if len(fieldIDs) == 0 {
			return shouldKeepFieldForLoadConfig(group.GetGroupId(), loadFields)
		}
		return lo.SomeBy(fieldIDs, func(fieldID int64) bool {
			return shouldKeepFieldForLoadConfig(fieldID, loadFields)
		})
	})
}
...
func FilterIndexesByLoadFields(indexes map[int64][]*querypb.FieldIndexInfo, loadFields []int64) map[int64][]*querypb.FieldIndexInfo {
	if len(indexes) == 0 || len(loadFields) == 0 {
		return indexes
	}
	loadFieldSet := typeutil.NewSet(loadFields...)
	...
	for segmentID, segmentIndexes := range indexes {
		for _, index := range segmentIndexes {
			if !loadFieldSet.Contain(index.GetFieldID()) {
				continue
			}
