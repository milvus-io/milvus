...
func TestEstimateSegmentsLoadResourceIncludesTextStats(t *testing.T) {
	...
	require.NoError(t, err)
	// 1000 rows also reserve a 128-byte word-aligned Tantivy validity bitmap.
	...
	require.NoError(t, err)
	assert.Equal(t, int64(128), usage.MemoryBytes)
	assert.Equal(t, int64(2500), usage.DiskBytes)
}

func TestEstimateSegmentsLoadResourceUsesStorageV3Descriptor(t *testing.T) {
	schema := &schemapb.CollectionSchema{
...
func TestFilterSegmentsByLoadFieldsKeepsChildFieldBinlogs(t *testing.T) {
	segments := []*datapb.SegmentInfo{
		{
			...
			Stats: &datapb.Statistics{
				LoadResource: &datapb.LoadResourceStatistics{
					ColumnGroups: []*datapb.ColumnGroupStatistics{
						{GroupId: 10, FieldIds: []int64{100, 101}},
						{GroupId: 11, FieldIds: []int64{102}},
						{GroupId: 200},
						{GroupId: 201},
					},
				},
	...
	require.Len(t, filtered[0].GetStats().GetLoadResource().GetColumnGroups(), 2)
	assert.Equal(t, int64(10), filtered[0].GetStats().GetLoadResource().GetColumnGroups()[0].GetGroupId())
	assert.Equal(t, int64(200), filtered[0].GetStats().GetLoadResource().GetColumnGroups()[1].GetGroupId())
}
