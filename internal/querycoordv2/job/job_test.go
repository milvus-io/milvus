...
func (suite *JobSuite) SetupTest() {
	...
	suite.meta = meta.NewMeta(RandomIncrementIDAllocator(), suite.store, suite.nodeMgr, metacache.NewMetaStore(nil))
...
func (suite *JobSuite) TestLoadCollectionStoreFailed() {
	...
	suite.meta = meta.NewMeta(RandomIncrementIDAllocator(), store, suite.nodeMgr, metacache.NewMetaStore(nil))
...
func (suite *JobSuite) TestLoadCreateReplicaFailed() {
	...
	suite.meta = meta.NewMeta(ErrorIDAllocator(), suite.store, session.NewNodeManager(), metacache.NewMetaStore(nil))
	for _, collection := range suite.collections {
		suite.broker.EXPECT().
			GetPartitions(mock.Anything, collection).
