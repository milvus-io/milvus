...
//
// Exported because the same view is needed outside the checkers: the server
// builds it once and shares it, since two caches would disagree about when a
// split window ended.
...
// state cache supplied by the caller, so the server can share one view across
// its components.
