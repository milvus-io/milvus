...
func (suite *IndexCheckerSuite) TestExternalSchemaBumpSchedulesExistingIndexReopen() {
	...
	mockListIndexes := mockey.Mock((*indexCheckerBrokerMock).ListIndexes).Return([]*indexpb.IndexInfo{
		{
			FieldID: 101,
			IndexID: 1000,
		},
	...
	mockGetIndexInfo := mockey.Mock((*indexCheckerBrokerMock).GetIndexInfo).Return(map[int64][]*querypb.FieldIndexInfo{
		2: {{
			FieldID:        101,
			IndexID:        1000,
			EnableIndex:    true,
	...
	checker := NewIndexChecker(suite.meta, suite.checker.dist, broker, suite.nodeMgr, nil)
