...
func syncPeriodically() segmentSyncPolicy {
	var lastSyncTime = time.Now().Add(-1 * Params.DataNodeCfg.SyncPeriod)
	return func(segments []*Segment, ts Timestamp, _ *atomic.Bool) []UniqueID {
		//try to distribute the sync pressure evenly into the whole sync period
		//to avoid imposing too much stress on meta storage at single time point
		if time.Since(lastSyncTime) < Params.DataNodeCfg.SyncPeriod/40 {
			return nil
		}
		...
		for _, seg := range segments {
			...
			if len(segsToSync) >= Params.DataNodeCfg.MaxSyncBatch {
				log.Info("single period sync batch size has reached limit", zap.Int("batch size", len(segsToSync)))
				break
			}
		}
		if len(segsToSync) > 0 {
...
func syncCPLagTooBehind() segmentSyncPolicy {
	...
	return func(segments []*Segment, ts Timestamp, _ *atomic.Bool) []UniqueID {
		...
		for _, segment := range segments {
			if segment.getType() != datapb.SegmentType_Flushed {
