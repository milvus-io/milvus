...
func TestMergeExecutorHierarchicalFanIn(t *testing.T) {
	...
	executor := &MergeExecutor{
		...
		Intermediate: func(_ context.Context, round, group int) (storage.RecordWriter, func(int64) (Source, error), error) {
			intermediateCount++
			writer := &testWriter{}
...
func TestMergeExecutorRejectsInvalidFanIn(t *testing.T) {
	executor := &MergeExecutor{FanIn: 1, BatchSize: 1, Schema: testSchema(), Intermediate: func(context.Context, int, int) (storage.RecordWriter, func(int64) (Source, error), error) {
...
// TestMergeExecutorHierarchicalFanInBoundaries exercises the hierarchical
// merge round counts from the design doc (§24.4): inputs 16/17/255/256/257
// with fan-in 16. Each round must preserve the total row count and keep the
// per-group fan-in within the configured limit.
func TestMergeExecutorHierarchicalFanInBoundaries(t *testing.T) {
	for _, numInputs := range []int{16, 17, 255, 256, 257} {
		t.Run(fmt.Sprintf("inputs_%d", numInputs), func(t *testing.T) {
			inputs := make([]Source, 0, numInputs)
			expected := make([]int64, 0, numInputs)
			for i := 0; i < numInputs; i++ {
				values := []int64{int64(i) * 100, int64(i)*100 + 1}
				inputs = append(inputs, sourceFromValues(fmt.Sprintf("s%d", i), values...))
				expected = append(expected, values...)
			}
			slices.Sort(expected)

			intermediateWriters := make(map[string][]int64)
			executor := &MergeExecutor{
				FanIn:      16,
				BatchSize:  1024,
				Schema:     testSchema(),
				...
				Intermediate: func(_ context.Context, round, group int) (storage.RecordWriter, func(int64) (Source, error), error) {
					writer := &testWriter{}
					return writer, func(rows int64) (Source, error) {
						key := fmt.Sprintf("r%d-g%d", round, group)
						intermediateWriters[key] = append([]int64(nil), writer.values...)
						require.Equal(t, rows, int64(len(writer.values)))
						return sourceFromValues(key, writer.values...), nil
					}, nil
				},
			}
			finalWriter := &testWriter{}
			...
			require.NoError(t, err)
			require.Equal(t, int64(2*numInputs), rows)
			require.Equal(t, expected, finalWriter.values)
		})
	}
}
