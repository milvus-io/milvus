...
// AcquireCall is one commitment seen by a RecordingGuard.
...
// or parks on the memory safety valve -- a distinction the real guard cannot
// expose across a package boundary.
type RecordingGuard struct {
	...
	err error
	// snapshot is what Snapshot reports. SetSnapshot is for the tests that need
	// the reporting path to see a node under load.
	snapshot Snapshot
}
...
// Block holds every later Accept open until Unblock, so a test can observe what
// a caller does while the safety valve is engaged.
...
// FailAcquire makes Accept return err without recording, standing in for the
...
func (g *RecordingGuard) Accept(ctx context.Context, taskID int64, taskType taskcommon.Type, req taskresource.Requirement) error {
	g.mu.Lock()
