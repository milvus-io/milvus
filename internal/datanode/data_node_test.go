...
func TestDataNode(t *testing.T) {
	...
	t.Run("Test getSystemInfoMetrics", func(t *testing.T) {
		...
		emptyNode.channelManager = NewChannelManager(emptyNode)
	...
	t.Run("Test BackGroundGC", func(t *testing.T) {
		...
		for _, test := range testDataSyncs {
			err = node.channelManager.runningFlowgraphs.addAndStart(node, &datapb.VchannelInfo{CollectionID: 1, ChannelName: test.dmChannelName}, nil, genTestTickler())
			assert.NoError(t, err)
		...
		assert.Eventually(t, func() bool {
			for _, test := range testDataSyncs {
				if node.channelManager.runningFlowgraphs.exist(test.dmChannelName) {
					return false
				}
			}
			return true
