...
func TestExternalRefreshFailureState(t *testing.T) {
	...
	t.Run("external field type mismatch is permanent", func(t *testing.T) {
		err := merr.SegcoreError(2007, "field type mismatch, expected Arrow int64, actual Arrow string")
		assert.Equal(t, merr.InputError, merr.GetErrorType(err))
		assert.Equal(t, indexpb.JobState_JobStateFailed, externalRefreshFailureState(err))
	})

	t.Run("external vector dimension mismatch is permanent", func(t *testing.T) {
		err := merr.SegcoreError(2032, "vector list length mismatch, expected 128, actual 64")
		assert.Equal(t, merr.InputError, merr.GetErrorType(err))
		assert.Equal(t, indexpb.JobState_JobStateFailed, externalRefreshFailureState(err))
	})

	t.Run("corrupt external data is permanent after wrapping", func(t *testing.T) {
		err := merr.Wrapf(
			merr.SegcoreError(2024, "vector list contains a null child element"),
