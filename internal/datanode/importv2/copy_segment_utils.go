...
// All files are sourced from pb (2.6 has no packed/manifest segments).
...
//
// Process flow:
// 1. Collect all source files from pb
...
// collectSegmentFiles collects all files to copy from pb, organized by type.
func collectSegmentFiles(
	source *datapb.CopySegmentSource,
) *SegmentFiles {
	files := &SegmentFiles{
		InsertBinlogs:     extractFromPb(source.GetInsertBinlogs()),
		DeltaBinlogs:      extractFromPb(source.GetDeltaBinlogs()),
		StatsBinlogs:      extractFromPb(source.GetStatsBinlogs()),
		Bm25Binlogs:       extractFromPb(source.GetBm25Binlogs()),
		VectorScalarIndex: extractIndexFiles(source.GetIndexFiles()),
		TextIndex:         extractTextIndexFiles(source.GetTextIndexFiles()),
	}
...
func buildIndexInfoFromSource(
	...
	// Process text indexes: transform file paths using mappings.
	...
	// Process JSON Key indexes: transform file paths using mappings.
