...
func newCopySegmentCopierMock(
	t *testing.T,
	copyFn func(context.Context, string, string, string, string) error,
) storage.CrossBucketCopier {
	t.Helper()
	target := &copySegmentCopierTarget{}
	mockCopy := mockey.Mock((*copySegmentCopierTarget).CopyCrossBucket).To(
		func(ctx context.Context, srcBucket, srcObject, dstBucket, dstObject string) error {
			if copyFn == nil {
				return nil
			}
			return copyFn(ctx, srcBucket, srcObject, dstBucket, dstObject)
		},
	).Build()
...
func mockNoManifestLobFiles(t *testing.T) {
	t.Helper()
...
func TestCollectSegmentFiles_LocalV3UsesCompletePaths(t *testing.T) {
	root := t.TempDir()
	...
	logicalBasePath := "insert_log/100/1/10"
	manifestPath := packed.MarshalManifestPath(logicalBasePath, 2)
	source := &datapb.CopySegmentSource{
		CollectionId:   100,
		PartitionId:    1,
		SegmentId:      10,
		...
		DeltaBinlogs: []*datapb.FieldBinlog{{
			Binlogs: []*datapb.Binlog{{LogPath: "insert_log/100/1/10/_delta/7"}},
		}},
	}
	physicalBasePath := path.Join(root, logicalBasePath)

	mList := mockey.Mock(listAllFiles).To(
		func(_ context.Context, cm storage.ChunkManager, basePath string) ([]string, error) {
			assert.Same(t, sourceCM, cm)
			assert.Equal(t, physicalBasePath, basePath)
			return []string{path.Join(physicalBasePath, "_data/0")}, nil
		}).Build()
	defer mList.UnPatch()
	...
	mLob := mockey.Mock(packed.GetManifestLobFiles).To(
		func(gotManifestPath string, cfg *indexpb.StorageConfig) ([]packed.LobFileInfo, error) {
			...
			return []packed.LobFileInfo{{
				Path: "insert_log/100/1/lobs/101/_data/0",
			}}, nil
		}).Build()
	defer mLob.UnPatch()
	...
	assert.Equal(t, []string{path.Join(physicalBasePath, "_data/0")}, files.InsertBinlogs)
	assert.Equal(t, []string{path.Join(root, "insert_log/100/1/lobs/101/_data/0")}, files.LobFiles)
	assert.Equal(t, []string{path.Join(root, "insert_log/100/1/10/_delta/7")}, files.DeltaBinlogs)
}
...
func TestCompleteLocalStorageV3PathRejectsURI(t *testing.T) {
	cm := storage.NewLocalChunkManager(objectstorage.RootPath(t.TempDir()))
	_, err := completeLocalStorageV3Path(cm, "s3://source-bucket/root/object")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not a filesystem path")
}

func TestCollectSegmentFiles_LocalV3RejectsURIManifestBase(t *testing.T) {
	root := t.TempDir()
	...
	_, err := collectSegmentFiles(
		...
		&datapb.CopySegmentSource{
			SegmentId:      10,
			StorageVersion: storage.StorageV3,
			ManifestPath:   packed.MarshalManifestPath("s3://source-bucket/root/object", 1),
		},
	)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not a filesystem path")
...
func TestCollectSegmentFiles_NormalizesV3ManifestURIForListing(t *testing.T) {
	sourceCM := &struct{ storage.ChunkManager }{}
	basePath := "s3://foreign-source/foreign-root/files/insert_log/100/1/10"
	...
	mList := mockey.Mock(listAllFiles).To(
		func(_ context.Context, cm storage.ChunkManager, gotBasePath string) ([]string, error) {
			assert.Same(t, sourceCM, cm)
			assert.Equal(t, "foreign-root/files/insert_log/100/1/10", gotBasePath)
			return []string{"foreign-root/files/insert_log/100/1/10/_data/0"}, nil
		}).Build()
	defer mList.UnPatch()
	mLob := mockey.Mock(packed.GetManifestLobFiles).Return([]packed.LobFileInfo{}, nil).Build()
	defer mLob.UnPatch()

	files, err := collectSegmentFiles(
		context.Background(),
		sourceCM,
		&indexpb.StorageConfig{BucketName: "foreign-source", RootPath: "foreign-root"},
		source,
	)
...
func TestGenerateTargetPath_RemapExternalRootToLocalCompletePath(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "/var/lib/milvus/data/files/insert_log/444/555/666/100/log1.log", gotPath)
}

func TestGenerateTargetPaths_ReservedNamesInStorageRoot(t *testing.T) {
	source := &datapb.CopySegmentSource{
		CollectionId: 111,
		PartitionId:  222,
		SegmentId:    333,
	...
	t.Run("local binlog root contains stats log", func(t *testing.T) {
		target := &datapb.CopySegmentTarget{
			CollectionId:   444,
			PartitionId:    555,
			SegmentId:      666,
			TargetRootPath: "/tmp/stats_log/root",
		}
		got, err := generateTargetPath(
			"/tmp/stats_log/root/insert_log/111/222/333/_data/0",
			source,
			target,
		)
		require.NoError(t, err)
		assert.Equal(t, "/tmp/stats_log/root/insert_log/444/555/666/_data/0", got)
	})

	t.Run("remote binlog root contains insert log", func(t *testing.T) {
		target := &datapb.CopySegmentTarget{
			CollectionId:   444,
			PartitionId:    555,
			SegmentId:      666,
			TargetRootPath: "archive/insert_log/files",
		}
		got, err := generateTargetPath(
			"archive/insert_log/files/insert_log/111/222/333/_data/0",
			source,
			target,
		)
		require.NoError(t, err)
		assert.Equal(t, "archive/insert_log/files/insert_log/444/555/666/_data/0", got)
	})

	t.Run("LOB root contains insert log", func(t *testing.T) {
		target := &datapb.CopySegmentTarget{
			CollectionId:   444,
			PartitionId:    555,
			SegmentId:      666,
		...
		got, err := generateTargetLOBPath(
			"archive/insert_log/files/insert_log/111/222/lobs/100/_data/0.vx",
			source,
			target,
		)
		require.NoError(t, err)
		assert.Equal(t, "archive/insert_log/files/insert_log/444/555/lobs/100/_data/0.vx", got)
	})

	t.Run("index root contains text log", func(t *testing.T) {
		target := &datapb.CopySegmentTarget{
			CollectionId:   444,
			PartitionId:    555,
			SegmentId:      666,
			TargetRootPath: "archive/text_log/files",
		}
		got, err := generateTargetIndexPath(
			"archive/text_log/files/text_log/123/1/111/222/333/100/index",
			source,
			target,
			IndexTypeText,
		...
		require.NoError(t, err)
		assert.Equal(t, "archive/text_log/files/text_log/123/1/444/555/666/100/index", got)
	})

	t.Run("reject layout match that rewrites target root", func(t *testing.T) {
		target := &datapb.CopySegmentTarget{
			CollectionId:   444,
			PartitionId:    555,
			SegmentId:      666,
			TargetRootPath: "/tmp/insert_log/111/222/333/root",
		}
		_, err := generateTargetPath(
			"/tmp/insert_log/111/222/333/root/not-a-segment-file",
			source,
			target,
		)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "outside the configured target storage root")
	})

	t.Run("reject duplicate identity layouts", func(t *testing.T) {
		target := &datapb.CopySegmentTarget{
			CollectionId:   444,
			PartitionId:    555,
			SegmentId:      666,
		...
		_, err := generateTargetPath(
			"/tmp/insert_log/111/222/333/root/insert_log/111/222/333/_data/0",
			source,
			target,
		)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "ambiguous binlog path structure")
	})
}
...
func TestGenerateTargetPath_RemapEndpointStyleExternalRoot(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "minio endpoint",
			sourceRootPath: "minio://minio.example.com/source-bucket/source-cluster-root",
			sourcePath:     "source-cluster-root/files/insert_log/111/222/333/100/log1.log",
		},
		{
			name:           "https endpoint",
			sourceRootPath: "https://storage.example.com/source-bucket/source-cluster-root",
			sourcePath:     "source-cluster-root/files/insert_log/111/222/333/100/log1.log",
		},
		{
			name:           "s3 cloud endpoint",
			sourceRootPath: "s3://s3.amazonaws.com/source-bucket/source-cluster-root",
			sourcePath:     "source-cluster-root/files/insert_log/111/222/333/100/log1.log",
		},
		{
			name:           "endpoint source path",
			sourceRootPath: "s3://s3.amazonaws.com/source-bucket/source-cluster-root",
			sourcePath:     "s3://s3.amazonaws.com/source-bucket/source-cluster-root/files/insert_log/111/222/333/100/log1.log",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			source := &datapb.CopySegmentSource{
				...
				PartitionId:    222,
				SegmentId:      333,
				SourceRootPath: tt.sourceRootPath,
			}
			target := &datapb.CopySegmentTarget{
				CollectionId:   444,
				PartitionId:    555,
...
func TestGenerateTargetPath_RejectsPathOutsideExternalRoot(t *testing.T) {
	source := &datapb.CopySegmentSource{
		...
		PartitionId:    222,
		SegmentId:      333,
		SourceRootPath: "s3://source-bucket/source-cluster-root",
	}
	target := &datapb.CopySegmentTarget{
		CollectionId:   444,
		PartitionId:    555,
	...
	gotPath, err := generateTargetPath(
		"s3://source-bucket/source-cluster-root-v2/files/insert_log/111/222/333/100/log1.log",
		source,
		target,
	)
...
func TestGenerateTargetPath_RemapExternalBucketRoot(t *testing.T) {
	source := &datapb.CopySegmentSource{
		CollectionId:   111,
		PartitionId:    222,
		SegmentId:      333,
		SourceRootPath: "s3://source-bucket",
	}
	tests := []struct {
		name       string
		targetRoot string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target := &datapb.CopySegmentTarget{
				...
				PartitionId:    555,
				SegmentId:      666,
				TargetRootPath: tt.targetRoot,
			}

			gotPath, err := generateTargetPath(
				"s3://source-bucket/files/insert_log/111/222/333/100/log1.log",
				source,
				target,
			)
...
func TestGenerateTargetPath_AllowsEquivalentExternalSchemes(t *testing.T) {
	tests := []struct {
		name       string
		sourceRoot string
		sourcePath string
		...
		{
			name:       "GCS aliases",
			sourceRoot: "gs://source-bucket/root",
			sourcePath: "gcs://source-bucket/root/files/insert_log/111/222/333/100/log1.log",
		},
		{
			name:       "Azure aliases",
			sourceRoot: "az://blob.core.windows.net/source-container/root",
			sourcePath: "azure://blob.core.windows.net/source-container/root/files/insert_log/111/222/333/100/log1.log",
		},
	}
...
func TestGenerateTargetPath_RejectsDifferentExternalLocation(t *testing.T) {
	tests := []struct {
		name       string
		sourceRoot string
		sourcePath string
		...
		{
			name:       "bucket",
			sourceRoot: "s3://source-bucket",
			sourcePath: "s3://other-bucket/files/insert_log/111/222/333/100/log1.log",
		},
		{
			name:       "endpoint",
			sourceRoot: "minio://source.example.com/source-bucket",
			sourcePath: "minio://other.example.com/source-bucket/files/insert_log/111/222/333/100/log1.log",
		},
		{
			name:       "scheme",
			sourceRoot: "s3://source-bucket",
			sourcePath: "gs://source-bucket/files/insert_log/111/222/333/100/log1.log",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Error(t, err)
			assert.Empty(t, gotPath)
			assert.Contains(t, err.Error(), "does not match source root")
		})
	}
}
...
func TestTransformFieldBinlogs(t *testing.T) {
	...
	t.Run("count rows for insert logs", func(t *testing.T) {
		result, totalRows, err := transformFieldBinlogs(srcFieldBinlogs, mappings, true, false, false)
		assert.NoError(t, err)
	...
	t.Run("no row counting for stats logs", func(t *testing.T) {
		result, totalRows, err := transformFieldBinlogs(srcFieldBinlogs, mappings, false, false, false)
		assert.NoError(t, err)
	...
	t.Run("skip binlogs with empty path", func(t *testing.T) {
		...
		result, _, err := transformFieldBinlogs(srcWithEmpty, mappings, false, false, false)
		assert.NoError(t, err)
	...
	t.Run("error on missing mapping", func(t *testing.T) {
		...
		result, totalRows, err := transformFieldBinlogs(srcWithUnmapped, mappings, true, false, false)
		assert.Error(t, err)
...
func TestCopySegmentAndIndexFiles_ReturnsFileList(t *testing.T) {
	...
	t.Run("failure returns partial file list", func(t *testing.T) {
		callCount := 0
		copier := newCopySegmentCopierMock(t, func(_ context.Context, _, _, _, _ string) error {
			callCount++
			if callCount > 1 {
				return errors.New("copy failed")
			}
...
func TestCopySegmentAndIndexFiles_NormalizesProviderSourceURI(t *testing.T) {
	const sourceURI = "s3://source-bucket/source-root/files/insert_log/111/222/333/1/10001"
	...
	assert.NoError(t, err)
	assert.NotNil(t, result)
	assert.Equal(t, "source-root/files/insert_log/111/222/333/1/10001", copiedSource)
}
...
func TestTransformFieldBinlogs_NilInput(t *testing.T) {
	result, totalRows, err := transformFieldBinlogs(nil, map[string]string{}, true, false, false)
	assert.NoError(t, err)
	assert.Equal(t, int64(0), totalRows)
	assert.Equal(t, 0, len(result))
}
...
func TestTransformFieldBinlogs_UnmappedPath(t *testing.T) {
	...
	_, _, err := transformFieldBinlogs(srcFieldBinlogs, mappings, true, false, false)
	assert.Error(t, err)
...
func TestShortenSingleJsonStatsPath_EdgeCases(t *testing.T) {
	...
	t.Run("reserved names in root", func(t *testing.T) {
		fullPath := "archive/json_stats/shared_key_index/files/json_stats/2/123/1/444/555/666/100/shared_key_index/index"
		result := shortenSingleJSONStatsPath(fullPath)
		assert.Equal(t, "shared_key_index/index", result)
	})

	t.Run("shared key component in root", func(t *testing.T) {
		fullPath := "archive/shared_key_index/files/json_stats/2/123/1/444/555/666/100/meta.json"
		result := shortenSingleJSONStatsPath(fullPath)
		assert.Equal(t, "meta.json", result)
	})

	t.Run("shared key substring is not an anchor", func(t *testing.T) {
		fullPath := "archive/not_shared_key_index/file"
		result := shortenSingleJSONStatsPath(fullPath)
...
func TestTransformManifestPath_ExternalSourceToLocalKeepsManifestRelative(t *testing.T) {
	target := &datapb.CopySegmentTarget{
		SegmentId:      666,
		CollectionId:   444,
		PartitionId:    555,
	...
	require.NoError(t, err)
	assert.Equal(t, "/var/lib/milvus/data/files/insert_log/444/555/666/_data/0", physicalPath)
}
...
func TestTransformFieldBinlogs_SkipsPathMappingForExternalTable(t *testing.T) {
	...
	got, totalRows, err := transformFieldBinlogs(src, nil, true, true, false)
	assert.NoError(t, err)
...
func TestTransformFieldBinlogs_DropsPathlessSummaryWhenNotPreserved(t *testing.T) {
	...
	got, totalRows, err := transformFieldBinlogs(src, nil, false, false, false)
	assert.NoError(t, err)
...
func TestTransformFieldBinlogs_PreservesPathlessSummary(t *testing.T) {
	...
	got, totalRows, err := transformFieldBinlogs(src, nil, true, false, true)
	require.NoError(t, err)
	require.Len(t, got, 1)
...
func TestGenerateSegmentInfoFromSource_V3UsesDeclaredRowCount(t *testing.T) {
...
func TestCopySegmentAndIndexFiles_StorageV2CopiesManifest(t *testing.T) {
	...
	assert.Equal(t, expectedManifest, result.GetManifestPath())
	assert.Equal(t, int64(12), result.GetImportedRows())
	assert.Equal(t, expectedManifest, copied[sourceManifest])
	assert.Equal(t, "files/insert_log/444/555/666/100/1", copied[sourceBinlog])
	assert.Len(t, copiedFiles, 2)
}
...
func TestCollectSegmentFiles_ManifestLobFailure(t *testing.T) {
	manifestPath := packed.MarshalManifestPath("files/insert_log/111/222/333", 2)
	mList := mockey.Mock(listAllFiles).Return([]string{
		"files/insert_log/111/222/333/_metadata/manifest.json",
	}, nil).Build()
	defer mList.UnPatch()
	mLob := mockey.Mock(packed.GetManifestLobFiles).Return(nil, errors.New("lob manifest unavailable")).Build()
	defer mLob.UnPatch()
	...
	require.Error(t, err)
	assert.Nil(t, files)
	assert.Contains(t, err.Error(), "failed to collect LOB files")
	assert.Contains(t, err.Error(), "lob manifest unavailable")
}
...
func TestCopySegmentAndIndexFiles_WithManifest(t *testing.T) {
	...
	t.Run("successful copy with manifest", func(t *testing.T) {
		...
		copier := newCopySegmentCopierMock(t, func(_ context.Context, _, src, _, _ string) error {
...
func TestListAllFiles(t *testing.T) {
	t.Run("does not match sibling prefixes", func(t *testing.T) {
		ctx := context.Background()
		cm := storage.NewLocalChunkManager()
		basePath := path.Join(t.TempDir(), "base")
		expectedPath := path.Join(basePath, "333/manifest")
		require.NoError(t, cm.Write(ctx, expectedPath, []byte("expected")))
		require.NoError(t, cm.Write(ctx, path.Join(basePath, "3330/manifest"), []byte("sibling")))

		files, err := listAllFiles(ctx, cm, path.Join(basePath, "333"))
		require.NoError(t, err)
		assert.Equal(t, []string{expectedPath}, files)
	})
...
func TestCopySegmentAndIndexFiles_LocalStorageV3UsesCompletePaths(t *testing.T) {
	...
	ctx := context.Background()
	root := filepath.Join(t.TempDir(), "stats_log", "root")
	cm := storage.NewLocalChunkManager(objectstorage.RootPath(root))
	...
	source := &datapb.CopySegmentSource{
		...
		SegmentId:      333,
		StorageVersion: storage.StorageV3,
		ManifestPath:   packed.MarshalManifestPath("insert_log/111/222/333", 1),
		InsertBinlogs: []*datapb.FieldBinlog{{
			FieldID: 100,
			Binlogs: []*datapb.Binlog{{
				LogPath:    sourceKey,
		...
		DeltaBinlogs: []*datapb.FieldBinlog{{
			Binlogs: []*datapb.Binlog{
				{
					// Already-complete protobuf paths occur in intermediate/local
					// metadata. Step 3.5 must overwrite the physical mapping with a
					// root-relative StorageV3 metadata mapping.
	...
	// The actual ChunkManager defines the physical namespace. Leave
	// StorageType empty to verify local detection does not depend on config.
	...
	assert.Equal(t, packed.MarshalManifestPath("insert_log/444/555/666", 1), result.GetManifestPath())
