...
func TestImportTaskClonesReuseCancellationContext(t *testing.T) {
	tests := map[string]func(context.Context, context.CancelFunc) Task{
		"import": func(ctx context.Context, cancel context.CancelFunc) Task {
			return &ImportTask{
				ImportTaskV2: &datapb.ImportTaskV2{}, ctx: ctx, cancel: cancel,
				segmentsInfo: make(map[int64]*datapb.ImportSegmentInfo),
			}
		},
		"preimport": func(ctx context.Context, cancel context.CancelFunc) Task {
			return &PreImportTask{PreImportTask: &datapb.PreImportTask{}, ctx: ctx, cancel: cancel}
		},
		"l0 import": func(ctx context.Context, cancel context.CancelFunc) Task {
			return &L0ImportTask{
				ImportTaskV2: &datapb.ImportTaskV2{}, ctx: ctx, cancel: cancel,
				segmentsInfo: make(map[int64]*datapb.ImportSegmentInfo),
			}
		},
		"l0 preimport": func(ctx context.Context, cancel context.CancelFunc) Task {
			return &L0PreImportTask{PreImportTask: &datapb.PreImportTask{}, ctx: ctx, cancel: cancel}
		},
	}

	for name, newTask := range tests {
		t.Run(name, func(t *testing.T) {
			...
			case <-time.After(time.Second):
				t.Fatal("clone must reuse the task cancellation owner")
			}
		})
	}
}
...
func TestTaskManagerRemoveCancelsRunningTask(t *testing.T) {
	...
	for _, c := range cases {
		...
		manager.Update(c.task, UpdateState(datapb.ImportTaskStateV2_InProgress))
	}
