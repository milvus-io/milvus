...
func NewSyncTask(ctx context.Context,
	...
	// The AddSegment above is synchronous, so a miss here means something removed
	// the segment concurrently. Handing back a task with no frozen layout would
	// only defer the failure to Prepare — or, worse, let it write under a layout
	// nobody agreed on. Fail the ImportTask instead: DataCoord's recovery is to
	// re-read the file, which is exactly the right response.
	segmentInfo, ok := metaCache.GetSegmentByID(segmentID)
	if !ok {
		return nil, merr.WrapErrSegmentNotFound(segmentID, "segment removed while building its import sync task")
	}
	// Freeze the physical layout at construction, exactly like the write buffer
	// does. Import happens to submit one task per segment at a time today, but the
	// invariant Prepare enforces is "the layout is decided before the task runs",
	// not "the caller is single-threaded" — and two import files can target the
	// same segment, each deriving a different layout from its own batch's column
	// statistics.
