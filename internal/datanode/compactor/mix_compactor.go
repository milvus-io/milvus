...
func NewMixCompactionTask(
	...
	lobHelper := NewLOBCompactionHelper(binlogIO, compactionParams.StorageConfig)
...
func (t *mixCompactionTask) relocateLOBFiles(
	...
	lobResult, err := t.lobHelper.CompactLOBFiles(
		...
		nil, // allocator not needed for Move mode
	)
	if err != nil {
	...
	for _, sourceMeta := range t.sourceLOBMetadata {
		for fieldID, fieldMeta := range sourceMeta.LOBFields {
			...
			for _, lobFileIDStr := range fieldMeta.LOBFiles {
				...
				if err != nil {
					log.Warn("failed to parse LOB file ID, skipping",
						zap.String("fileID", lobFileIDStr),
						zap.Error(err))
					continue
				}

				// reconstruct full path using source segment metadata
				lobFilePath := metautil.BuildLOBLogPath(rootPath, sourceMeta.CollectionID, sourceMeta.PartitionID, sourceMeta.SegmentID, fieldID, int64(lobFileID))
