...
func (t *mixCompactionTask) Compact() (*datapb.CompactionPlanResult, error) {
	...
	// discarded work, and stats_inspector also skips unsorted segments in its
	// TextIndexJob filter, so the index would never be promoted to text_stats_logs
	// in datacoord either.
	...
	for _, resultSegment := range res {
		...
		if err != nil {
			log.Warn("failed to create text indexes",
				zap.Int64("targetSegmentID", resultSegment.GetSegmentID()), zap.Error(err))
			return nil, err
		}
		...
		if resultSegment.GetManifest() != "" && len(textStatsLogs) > 0 {
			...
			if mErr != nil {
				log.Warn("failed to add text index stats to manifest",
					zap.Int64("targetSegmentID", resultSegment.GetSegmentID()), zap.Error(mErr))
				return nil, mErr
			}
