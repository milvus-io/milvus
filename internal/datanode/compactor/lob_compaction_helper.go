...
func (h *LOBCompactionHelper) ExtractLOBMetadataFromSegment(
	...
	// set segment identifiers for path reconstruction
	...
	err := chunkManager.WalkWithPrefix(ctx, lobPrefix, true,
		func(chunkInfo *storage.ChunkObjectInfo) bool {
			...
			fieldID, lobFileID, ok := metautil.ParseLOBFilePath(chunkInfo.FilePath)
			if !ok || fieldID == 0 {
			...
			// store only the LOB file ID as string
			fieldMeta.LOBFiles = append(fieldMeta.LOBFiles, fmt.Sprintf("%d", lobFileID))
