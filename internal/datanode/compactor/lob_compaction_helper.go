...
func getLOBCompactionSmartRewriteThreshold() float64 {
	return paramtable.Get().DataNodeCfg.LOBCompactionSmartRewriteThreshold.GetAsFloat()
}
...
func NewLOBCompactionHelper(binlogIO io.BinlogIO) *LOBCompactionHelper {
	chunkManager := binlogIO.(storage.ChunkManager)
	return &LOBCompactionHelper{
		binlogIO: binlogIO,
		helper:   storage.NewLOBCompactionHelper(chunkManager),
...
func (h *LOBCompactionHelper) ExtractLOBMetadataFromSegment(
	...
	// TODO: Phase 1 - Read from segment binlog footer metadata (when available)
	// For now, use file scanning approach
	...
	err := chunkManager.WalkWithPrefix(ctx, lobPrefix, true,
		func(chunkInfo *storage.ChunkObjectInfo) bool {
			...
			// Parse field ID and LOB file ID from path
			fieldID, _ := parseLOBFilePath(chunkInfo.FilePath)
			if fieldID == 0 {
...
func CalculateDeleteRatio(plan *datapb.CompactionPlan, resultSegments []*datapb.CompactionSegment) float64 {
	...
	// Sum input rows - calculate from binlogs
	for _, segBinlog := range plan.GetSegmentBinlogs() {
		// Get rows from first field binlog (all fields should have same row count)
...
func (h *LOBCompactionHelper) CompactLOBFiles(
	...
	case CompactionLOBModeSmartRewrite:
		// Use SmartRewrite mode - filter unused rows and merge files
		// TODO: Collect actual used references from compacted segments
		// For now, use Relocate mode as fallback
		h.logger.Warn("SmartRewrite mode not fully implemented yet, falling back to Relocate mode",
			zap.String("reason", "reference set collection not implemented"))
		chunkManager := h.binlogIO.(storage.ChunkManager)
		return h.helper.CompactLOBFiles(
			ctx,
			sourceMetadatas,
			...
			chunkManager.RootPath(),
			storage.CompactionModeRelocate,
			nil, // TODO: pass actual reference set when implemented
		)
...
// IsLOBEnabled checks if LOB feature is enabled
func IsLOBEnabled() bool {
	return paramtable.Get().DataNodeCfg.LOBEnabled.GetAsBool()
}

// parseLOBFilePath extracts field ID and LOB file ID from a file path
// Path format: .../insert_log/{coll}/{part}/{seg}/{field}/lob/{lobfile}.parquet
func parseLOBFilePath(filePath string) (fieldID int64, lobFileID uint64) {
	...
	for i, part := range parts {
		if part == "lob" && i > 0 {
			// Field ID is before "lob"
			fieldID, _ = strconv.ParseInt(parts[i-1], 10, 64)

			// LOB file ID is after "lob"
			if i+1 < len(parts) {
				filename := parts[i+1]
				filename = strings.TrimSuffix(filename, ".parquet")
				lobFileID, _ = strconv.ParseUint(filename, 10, 64)
			}
			break
		}
	}

	return fieldID, lobFileID
}
