...
func (s *SortCompactionTaskSuite) setupTest() {
	...
	s.task = NewSortCompactionTask(context.Background(), s.mockBinlogIO, plan)
}
...
func (s *SortCompactionTaskSuite) TestNewSortCompactionTask() {
	...
	task := NewSortCompactionTask(context.Background(), s.mockBinlogIO, plan)

	s.NotNil(task)
	s.Equal(plan.GetPlanID(), task.GetPlanID())
	s.Equal(datapb.CompactionType_SortCompaction, task.GetCompactionType())
	s.Equal(paramtable.Get().DataCoordCfg.SortCompactionSlotUsage.GetAsInt64(), task.GetSlotUsage())
}
...
func (s *SortCompactionTaskSuite) TestTaskInterface() {
	...
	s.Equal(paramtable.Get().DataCoordCfg.SortCompactionSlotUsage.GetAsInt64(), s.task.GetSlotUsage())
...
func TestSortCompactionTaskBasic(t *testing.T) {
	...
	task := NewSortCompactionTask(ctx, mockBinlogIO, plan)
