...
func TestSelectedMaterializedRecordReaderFiltersBeforeMaterialization(t *testing.T) {
	...
	defer first.Release()
	added, err := storage.GenerateEmptyArrayFromSchema(schema.GetFields()[1], 2)
	require.NoError(t, err)
	defer added.Release()
	record := &materializerTestRecord{len: 2, columns: map[storage.FieldID]arrow.Array{100: first, 101: added}}
