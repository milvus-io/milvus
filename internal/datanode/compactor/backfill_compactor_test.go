...
func (s *BackfillCompactionTaskSuite) setupTest() {
	...
	s.task = NewBackfillCompactionTask(context.Background(), s.mockBinlogIO, plan, compaction.GenParams())
}
...
func TestBackfillCompactionTaskBasic(t *testing.T) {
	...
	task := NewBackfillCompactionTask(ctx, mockBinlogIO, plan, compaction.GenParams())
