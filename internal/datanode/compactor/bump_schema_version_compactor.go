...
func (t *bumpSchemaVersionCompactionTask) runFullSchemaRewrite(existingFields map[int64]struct{}) (*datapb.CompactionPlanResult, error) {
	...
	writerOpts := []storage.RwOption{
		...
		// Why: the full-rewrite path builds its own options instead of reusing
		// newV3WriterResult(); without this it would stay on DefaultWriterOptions()'s
		// 10 MiB part size even when dataNode.storage.multiPartUploadSize is raised,
		// re-exposing rewritten segments to the S3 10,000-part limit.
