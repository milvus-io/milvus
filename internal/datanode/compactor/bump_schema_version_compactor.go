...
func (t *bumpSchemaVersionCompactionTask) additiveReadSchema(diff *schemaBumpPhysicalDiff) (*schemapb.CollectionSchema, []int64, error) {
	...
	// Adds field to readFields only when it is physically present in the
	// manifest-derived existingFields, deduplicating by field ID.
	addExistingReadField := func(field *schemapb.FieldSchema) {
		if field == nil {
			return
		}
		fieldID := field.GetFieldID()
		...
		if _, ok := diff.existingFields[fieldID]; !ok {
			return
		}
		seen[fieldID] = struct{}{}
	...
	addExistingReadField(anchor)
	if len(readFields) == 0 {
		return nil, nil, merr.WrapErrDataIntegrityMsg(
			"schema bump additive reconciliation row-count anchor is not physically present")
	}
	for _, field := range inputSchema.GetFields() {
		addExistingReadField(field)
	}
