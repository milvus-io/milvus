...
func (s *SplitClusterWriterSuite) TestSplitByHash() {
	...
	splitFunc := func(value *storage.Value) (string, error) {
...
func (s *SplitClusterWriterSuite) TestSplitByRange() {
	...
	s.Equal(int64(1000), splitWriter.GetRowNum())
...
func (s *SplitClusterWriterSuite) TestConcurrentSplitByHash() {
	...
	for i := 0; i < 10; i++ {
		...
		future := pool.Submit(func() (any, error) {
			for i := j + int64(0); i < j+int64(1000); i++ {
				err := splitWriter.Write(generateInt64PKEntitiy(i))
				if i == j+100 {
					splitWriter.Flush()
				}
...
func (s *SplitClusterWriterSuite) TestFlush() {
	s.mockBinlogIO.EXPECT().Upload(mock.Anything, mock.Anything).Return(nil)
	...
	splitWriter, err := NewSplitClusterWriterBuilder().
		...
		SetChannel("ch-1").
		SetSegmentMaxRowCount(1000).
		SetSegmentMaxSize(100000).
		SetSplitKeys([]string{"0", "1"}).
		SetMemoryBufferSize(10000).
		SetSplitFunc(splitFunc).
		SetFlushPoolSize(1).
		Build()
	s.NoError(err)
	...
	for i := 0; i < 10; i++ {
		err := splitWriter.Write(generateInt64PKEntitiy(int64(i)))
		s.NoError(err)
	}
	s.Equal(int64(0), splitWriter.WrittenMemorySize())

	for i := 10; i < 1000; i++ {
		err := splitWriter.Write(generateInt64PKEntitiy(int64(i)))
		s.NoError(err)
	}
	s.Equal(int64(130000), splitWriter.WrittenMemorySize())
	err = splitWriter.Flush()
	s.NoError(err)
	s.Equal(int64(0), splitWriter.WrittenMemorySize())
}
