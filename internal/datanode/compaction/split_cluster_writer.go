...
type Writer interface {
	Write(value *storage.Value) error
	Finish() (map[string][]*datapb.CompactionSegment, error)
	GetRowNum() int64
	WrittenMemorySize() int64
	Flush() error
}

var _ Writer = (*SplitClusterWriter)(nil)
...
type SplitClusterWriter struct {
	...
	clusterNum         int32
	splitFunc          func(*storage.Value) (string, error)
...
func (c *SplitClusterWriter) Write(value *storage.Value) error {
	clusterKey, err := c.splitFunc(value)
	if err != nil {
		return err
	}
...
func (c *SplitClusterWriter) Flush() error {
	getLock := c.flushMutex.TryLock()
	if !getLock {
		return nil
	}
	defer c.flushMutex.Unlock()
	currentMemorySize := c.WrittenMemorySize()
	if currentMemorySize <= c.getMemoryBufferLowWatermark() {
		log.Info("memory low water mark", zap.Int64("memoryBufferSize", c.WrittenMemorySize()))
		return nil
	}
	...
	for _, bufferId := range bufferIDs {
		...
		if currentMemorySize <= c.getMemoryBufferLowWatermark() {
			log.Info("reach memory low water mark", zap.Int64("memoryBufferSize", c.WrittenMemorySize()))
			break
		}
	}
...
func (c *SplitClusterWriter) WrittenMemorySize() int64 {
...
// SetSplitFunc sets the splitFunc field
func (b *SplitClusterWriterBuilder) SetSplitFunc(splitFunc func(*storage.Value) (string, error)) *SplitClusterWriterBuilder {
	b.splitFunc = splitFunc
	return b
}
...
func (b *SplitClusterWriterBuilder) SetFlushPoolSize(flushPoolSize int) *SplitClusterWriterBuilder {
	b.flushPoolSize = flushPoolSize
	return b
}
...
func (b *SplitClusterWriterBuilder) Build() (*SplitClusterWriter, error) {
	writer := &SplitClusterWriter{
		...
		splitFunc:          b.splitFunc,
		flushPoolSize:      b.flushPoolSize,
		flushPool:          conc.NewPool[any](b.flushPoolSize),
