...
func (st *statsTask) createJSONKeyStats(ctx context.Context,
	...
	// (an attempt subdirectory stays inside that relative path). The loon
	// manifest always receives absolute paths, so use a separate copy here and
	// leave the original stats relative for the dual-write to etcd.
...
// V3 requests from new coordinators use task-ID subdirectories. Older
// coordinators omit the flag and keep the legacy field directory.
func computeStatsBasePath(req *workerpb.CreateStatsRequest, manifestPath string, statsType string, fieldID int64) (string, error) {
	if req.GetStorageVersion() == storage.StorageV3 {
		...
		if req.GetUseV3StatsAttemptPath() {
