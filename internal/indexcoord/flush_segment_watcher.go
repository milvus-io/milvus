...
func (fsw *flushedSegmentWatcher) internalProcess(segID UniqueID) {
	...
	log.Ctx(fsw.ctx).Debug("flushedSegmentWatcher process internal task", zap.Int64("segID", segID),
	...
	case indexTaskDone:
		if err := fsw.removeFlushedSegment(t); err != nil {
			log.Ctx(fsw.ctx).Error("IndexCoord flushSegmentWatcher removeFlushedSegment fail",
		...
		fsw.internalNotifyFunc()
	default:
		log.Ctx(fsw.ctx).Debug("IndexCoord flushedSegmentWatcher internal task get invalid state", zap.Int64("segID", segID),
			zap.String("state", t.state.String()))
	}
}
