...
func TestIndexCoord(t *testing.T) {
	...
	inm0.CallRegister = func() error {
		session := sessionutil.NewSession(context.Background(), Params.EtcdCfg.MetaRootPath, etcdCli)
		session.Init(typeutil.IndexNodeRole, Params.IndexNodeCfg.Address, false, false)
		session.Register()
		return err
	}
