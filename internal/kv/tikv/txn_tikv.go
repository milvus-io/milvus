...
const (
	...
	// to get over this we are using the reserverd keyword as placeholder.
...
// is Deferred. Deferred functions evaluate their args immidiately.
...
func isEmptyByte(value []byte) bool {
	if bytes.Equal(value, EmptyValueByte) || len(value) == 0 {
		return true
	}
	return false
}
