...
func (kv *PebbleKV) LoadWithPrefix(prefix string) ([]string, []string, error) {
	...
	// In pebble API, empty string will be used for comparison and it's smaller than any key
	// If use "", the result will be empty, so here we just ignore it
	option := pebble.IterOptions{}
	if prefix != "" {
		option.UpperBound = []byte(typeutil.AddOne(prefix))
	}
...
func (kv *PebbleKV) Has(key string) (bool, error) {
	if kv.DB == nil {
		return false, fmt.Errorf("pebbledb instance is nil when check if has %s", key)
	}

	value, closer, err := kv.DB.Get([]byte(key))
	// pebble will return ErrNotFound if there is no such key, here we ignore it for compatibility
	if err != nil && err != pebble.ErrNotFound {
		return false, err
	}
	if closer != nil {
...
func (kv *PebbleKV) HasPrefix(prefix string) (bool, error) {
	if kv.DB == nil {
