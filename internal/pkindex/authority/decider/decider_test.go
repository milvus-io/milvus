...
func TestAutocommitDelete(t *testing.T) {
	...
	t.Run("all present, no rewrite", func(t *testing.T) {
...
func TestClaimTxnCleanup(t *testing.T) {
	ctx := context.Background()
	d, _ := newTestDecider(t)
	const txnID = 41
	require.False(t, d.ClaimTxnCleanup(txnID), "unknown transaction")

	body, err := d.Decide(ctx, Request{Kind: KindInsert, VChannel: "v1", PKs: int64PKs(5), TxnID: txnID})
	require.NoError(t, err)
	body.Apply(ctx, Applied{SegmentID: 500, TimeTick: 11})
	require.True(t, d.ClaimTxnCleanup(txnID))
	require.False(t, d.ClaimTxnCleanup(txnID))
}
