	...
	return &planpb.Expr{
		Expr: &planpb.Expr_BinaryArithOpEvalRangeExpr{
			BinaryArithOpEvalRangeExpr: &planpb.BinaryArithOpEvalRangeExpr{
				...
				OperandPlaceholderName: operandExpr.GetPlaceholderName(),
				ValuePlaceholderName:   valueExpr.GetPlaceholderName(),
			},
		},
		NeedFillPlaceholder: needFillPlaceholder(operandExpr) || needFillPlaceholder(valueExpr),
	}
}
...
func handleCompareRightValue(op planpb.OpType, left *ExprWithType, right *planpb.ValueExpr) (*planpb.Expr, error) {
	...
	if !left.expr.GetNeedFillPlaceholder() && !needFillPlaceholder(right) {
	...
	expr := &planpb.Expr{
		Expr: &planpb.Expr_UnaryRangeExpr{
			UnaryRangeExpr: &planpb.UnaryRangeExpr{
				...
				PlaceholderName: right.GetPlaceholderName(),
			},
		},
		NeedFillPlaceholder: needFillPlaceholder(right),
	}
...
func handleCompare(op planpb.OpType, left *ExprWithType, right *ExprWithType) (*planpb.Expr, error) {
	...
	if left.expr.GetNeedFillPlaceholder() {
		return &planpb.Expr{
			Expr: &planpb.Expr_UnaryRangeExpr{
				UnaryRangeExpr: &planpb.UnaryRangeExpr{
					...
					PlaceholderName: left.expr.GetValueExpr().GetPlaceholderName(),
				},
			},
		}, nil
	}
...
func HandleCompare(op int, left, right *ExprWithType) (*planpb.Expr, error) {
	if !left.expr.GetNeedFillPlaceholder() && !right.expr.GetNeedFillPlaceholder() {
...
func checkRangeCompared(dataType schemapb.DataType, value *planpb.GenericValue) error {
	...
	case schemapb.DataType_Float, schemapb.DataType_Double:
		...
		if IsInteger(value) {
			value = NewFloat(float64(value.GetInt64Val()))
		}
	}
	return nil
}
