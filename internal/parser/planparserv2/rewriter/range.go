...
func mergeOrIntervals(dt schemapb.DataType, intervals []orInterval) []mergedInterval {
	...
	// Check if lower-only and upper-only together cover everything
	if mergedLowerOnly != nil && mergedUpperOnly != nil {
		cmpLU := cmpGeneric(dt, mergedLowerOnly.lower, mergedUpperOnly.upper)
		overlaps := cmpLU < 0 ||
			(cmpLU == 0 && (mergedLowerOnly.lowerInc || mergedUpperOnly.upperInc))
		if overlaps {
			// They overlap: together they cover (-∞, +∞) but we can't express that,
			// so merge into a single wider interval.
			// Actually if x > a OR x < b with a < b, this covers everything when overlapping.
			// But we have no AlwaysTrue for ranged columns... we do have newAlwaysTrueExpr
			// However, for nullable columns this is wrong. We handle nullable in tautology.
			// Here we just merge the bounds.

			// The union is: x < max(b) OR x > min(a). If they overlap, it's everything.
			// We can't express "everything" as a single range, but we can note that
			// the OR of these two is a tautology (for non-null).
			// For safety, keep them separate and let tautology detection handle it.
		}
	}
	...
	for _, b := range boundedList {
		result = append(result, mergedInterval{
			lower: b.lower, lowerInc: b.lowerInc,
			upper: b.upper, upperInc: b.upperInc,
		})
	}
	if mergedLowerOnly != nil {
		result = append(result, mergedInterval{
