...
3) Range predicate simplification (`rules/range.go`)
...
- Optional optimizer traversal remains limited to `BinaryExpr`, `UnaryExpr`, `TermExpr`, and `ValueExpr`; the always-on correctness normalizers retain their broader traversal policy.
