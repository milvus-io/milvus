...
func TestRoaringMatchPreflightBudgetsAggregateDecodedBytes(t *testing.T) {
	...
	pt.Save(pt.ProxyCfg.MaxMembershipFilterPlanSize.Key,
		strconv.Itoa(paramtable.DefaultMaxMembershipFilterPlanSize))
	defer pt.Reset(pt.ProxyCfg.MaxMembershipFilterPlanSize.Key)

	// This compact wire shape is about 40 MiB after decoding. It remains below
	// the fixed 64 MiB per-bitmap validator limit, while repeated occurrences
	// exercise the separate 128 MiB request-wide budget.
	blob := compactHighContainerRoaringBlob(200_000)
	...
	require.Greater(t, summary.EstimatedDecodedBytes, uint64(35*1024*1024))
	require.Less(t, summary.EstimatedDecodedBytes, uint64(45*1024*1024))
	require.LessOrEqual(t, summary.EstimatedDecodedBytes*2,
		uint64(paramtable.DefaultMaxMembershipFilterPlanSize))
	require.Greater(t, summary.EstimatedDecodedBytes*4,
		uint64(paramtable.DefaultMaxMembershipFilterPlanSize))
	...
	_, err = ParseExpr(helper,
		"membership_match(Int64Field, {rb}, type=roaring) and membership_match(Int64Field, {rb}, type=roaring)",
		values)
	require.NoError(t, err, "two roughly 40 MiB decoded bitmaps fit the default 128 MiB request budget")

	_, err = ParseExpr(helper,
		"membership_match(Int64Field, {rb}, type=roaring) and membership_match(Int64Field, {rb}, type=roaring) and "+
			"membership_match(Int64Field, {rb}, type=roaring) and membership_match(Int64Field, {rb}, type=roaring)",
		values)
	require.ErrorIs(t, err, merr.ErrParameterTooLarge)
...
func TestRoaringMatchPreflightDecodedBudgetUsesConfiguredPlanSizeAcrossParses(t *testing.T) {
	helper := newTestSchemaHelper(t)
	pt := paramtable.Get()
	blob := compactHighContainerRoaringBlob(200_000)
	summary, err := serverroaring.Validate(blob)
	require.NoError(t, err, "one bitmap must remain below the fixed per-bitmap validator limit")

	// The serialized bodies are much smaller than either configured limit, so
	// any failure below can only come from the decoded-memory dimension of
	// maxMembershipFilterPlanSize.
	exactLimit := summary.EstimatedDecodedBytes * 2
	body := uint64(len(blob) - roaringfilter.HeaderSize)
	require.Less(t, body*2, exactLimit-1)
	values := map[string]*schemapb.TemplateValue{"rb": bytesTemplate(blob)}
	expr := "membership_match(Int64Field, {rb}, type=roaring)"

	t.Run("exact decoded boundary passes", func(t *testing.T) {
		pt.Save(pt.ProxyCfg.MaxMembershipFilterPlanSize.Key, strconv.FormatUint(exactLimit, 10))
		defer pt.Reset(pt.ProxyCfg.MaxMembershipFilterPlanSize.Key)
		...
		require.NoError(t, err)
		_, err = parseExprInner(helper, expr, values, visitorArgs)
		require.NoError(t, err, "the configured boundary must be inclusive across shared parses")
	})

	t.Run("one byte below decoded boundary fails", func(t *testing.T) {
		planLimit := exactLimit - 1
		pt.Save(pt.ProxyCfg.MaxMembershipFilterPlanSize.Key, strconv.FormatUint(planLimit, 10))
		defer pt.Reset(pt.ProxyCfg.MaxMembershipFilterPlanSize.Key)
		visitorArgs := &ParserVisitorArgs{MembershipBudget: NewMembershipPreflightBudget()}

		_, err = parseExprInner(helper, expr, values, visitorArgs)
		require.NoError(t, err, "the first decoded occurrence fits the configured request budget")
		_, err = parseExprInner(helper, expr, values, visitorArgs)
		require.ErrorIs(t, err, merr.ErrParameterTooLarge)
		require.ErrorContains(t, err, "estimated decoded size")
		require.ErrorContains(t, err, "proxy.maxMembershipFilterPlanSize")
		require.ErrorContains(t, err, strconv.FormatUint(planLimit, 10))
	})
}
