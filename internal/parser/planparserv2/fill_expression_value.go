...
// decoded bytes. The two measurements are checked independently. Plan
// construction parses one expression per sub-request plus one per scorer, so
// threading one budget through the whole request rejects repeated blob
