...
SchemaHandle
PlanParser::RegisterSchema(const std::vector<uint8_t>& schema_proto) {
    void* proto_blob =
        const_cast<void*>(static_cast<const void*>(schema_proto.data()));
...
std::string
PlanParser::UnregisterSchema(SchemaHandle handle) {
    char* err_msg = nullptr;
...
std::vector<uint8_t>
PlanParser::Parse(SchemaHandle handle, const std::string& expr) {
...
std::vector<uint8_t>
PlanParser::ParseSearch(SchemaHandle handle,
    ...
    void* query_info_blob = query_info_proto.empty()
                                ? nullptr
                                : const_cast<void*>(static_cast<const void*>(
                                      query_info_proto.data()));
    int query_info_len = static_cast<int>(query_info_proto.size());
    char* err_msg = nullptr;
    int length = 0;

    void* result = ::ParseSearch(handle,
                                 c_expr,
                                 c_vector_field,
                                 query_info_blob,
                                 query_info_len,
    ...
    if (result == nullptr) {
        ...
        throw std::runtime_error("Failed to parse search expression: " +
                                 err_str);
    }
