...
func validateBloomFilterBlob(blob []byte) ([]byte, error) {
	...
	//
	// A separate proxy.maxMembershipFilterPlanSize request budget independently
	// bounds aggregate serialized plan bytes; Roaring occurrences additionally
	// share its aggregate estimated-decoded-bytes dimension. This per-blob gate
	// remains necessary: it rejects one oversized filter at the input boundary,
	// while the aggregate gate limits repeated otherwise-valid occurrences across
	// the request.
