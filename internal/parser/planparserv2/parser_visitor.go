...
func (v *ParserVisitor) VisitAddSub(ctx *parser.AddSubContext) interface{} {
	...
	if leftValueExpr != nil && rightValueExpr != nil {
		if needFillPlaceholder(leftValueExpr) || needFillPlaceholder(rightValueExpr) {
	...
	if leftExpr.expr.GetNeedFillPlaceholder() {
		dataType = rightExpr.dataType
	} else if rightExpr.expr.GetNeedFillPlaceholder() {
	...
	expr := &planpb.Expr{
		...
		NeedFillPlaceholder: leftExpr.expr.GetNeedFillPlaceholder() || rightExpr.expr.GetNeedFillPlaceholder(),
	}
	return &ExprWithType{
		expr:          expr,
		dataType:      dataType,
...
func (v *ParserVisitor) VisitTerm(ctx *parser.TermContext) interface{} {
	...
	expr := &planpb.Expr{
		...
		NeedFillPlaceholder: neddFillPlaceholder,
	}
	if ctx.GetOp() != nil {
		expr = &planpb.Expr{
...
func (v *ParserVisitor) VisitRange(ctx *parser.RangeContext) interface{} {
	...
	if !needFillPlaceholder(lowerValueExpr) {
		if err = checkRangeCompared(fieldDataType, lowerValue); err != nil {
			return err
		}
	}
	if !needFillPlaceholder(upperValueExpr) {
	...
	if !needFillPlaceholder(lowerValueExpr) && !needFillPlaceholder(upperValueExpr) {
	...
	expr := &planpb.Expr{
		Expr: &planpb.Expr_BinaryRangeExpr{
			BinaryRangeExpr: &planpb.BinaryRangeExpr{
				...
				LowerPlaceholderName: lowerValueExpr.GetPlaceholderName(),
				UpperPlaceholderName: upperValueExpr.GetPlaceholderName(),
			},
		},
		NeedFillPlaceholder: needFillPlaceholder(lowerValueExpr) || needFillPlaceholder(upperValueExpr),
	}
	return &ExprWithType{
		expr:     expr,
		dataType: schemapb.DataType_Bool,
...
func (v *ParserVisitor) VisitJSONContains(ctx *parser.JSONContainsContext) interface{} {
	...
	if !needFillPlaceholder(elementExpr) {
	...
	expr := &planpb.Expr{
		Expr: &planpb.Expr_JsonContainsExpr{
			JsonContainsExpr: &planpb.JSONContainsExpr{
				...
				PlaceholderName:  elementExpr.GetPlaceholderName(),
			},
		},
		NeedFillPlaceholder: needFillPlaceholder(elementExpr),
	}
	return &ExprWithType{
		expr:     expr,
		dataType: schemapb.DataType_Bool,
...
func (v *ParserVisitor) VisitJSONContainsAny(ctx *parser.JSONContainsAnyContext) interface{} {
	...
	if !needFillPlaceholder(valueExpr) {
		elementValue := valueExpr.GetValue()
	...
	expr := &planpb.Expr{
		Expr: &planpb.Expr_JsonContainsExpr{
			JsonContainsExpr: &planpb.JSONContainsExpr{
				...
				ElementsSameType: sameType,
				PlaceholderName:  valueExpr.GetPlaceholderName(),
			},
		},
		NeedFillPlaceholder: needFillPlaceholder(valueExpr),
	}
	return &ExprWithType{
		expr:     expr,
		dataType: schemapb.DataType_Bool,
