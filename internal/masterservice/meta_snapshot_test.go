...
func TestMetaSnapshot(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		err = ms.Save("abc", fmt.Sprintf("value-%d", i), typeutil.Timestamp(100+i))
		assert.Nil(t, err)
...
func TestLoad(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		err = ms.Save("key", fmt.Sprintf("value-%d", i), typeutil.Timestamp(100+i*5))
		assert.Nil(t, err)
	}
	for i := 0; i < 20; i++ {
...
func TestMultiSave(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		err = ms.MultiSave(saves, typeutil.Timestamp(100+i*5))
		assert.Nil(t, err)
	}
	for i := 0; i < 20; i++ {
...
func TestMultiSaveAndRemoveWithPrefix(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		err = ms.Save(fmt.Sprintf("kd-%d", i), fmt.Sprintf("value-%d", i), typeutil.Timestamp(100+i*5))
		assert.Nil(t, err)
	}
	for i := 20; i < 40; i++ {
		sm := map[string]string{"ks": fmt.Sprintf("value-%d", i)}
		dm := []string{fmt.Sprintf("kd-%d", i-20)}
		err := ms.MultiSaveAndRemoveWithPrefix(sm, dm, typeutil.Timestamp(100+i*5))
		assert.Nil(t, err)
	}
