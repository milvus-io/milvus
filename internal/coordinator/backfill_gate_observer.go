...
const defaultBackfillGateSweepInterval = 10 * time.Second
...
func NewBackfillAtomicGateObserver(ctx context.Context, registry backfillGateStore, readiness backfillReadiness, interval time.Duration) *BackfillAtomicGateObserver {
	if interval <= 0 {
