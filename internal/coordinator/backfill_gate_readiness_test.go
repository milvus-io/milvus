...
func (f *fakeScope) StaleSegments(collectionID int64, _ int32) []int64 { return f.stale[collectionID] }
...
func TestReadiness_WatermarkScalar(t *testing.T) {
	...
	assert.True(t, NewReadinessProvider(ready, nil).IsRoundReady(ctx, round))
	...
	assert.False(t, NewReadinessProvider(lag, nil).IsRoundReady(ctx, round))
}

func TestReadiness_WatermarkScalar_StaleNotYetInDist(t *testing.T) {
	ctx := context.Background()
	round := scalarRoundWM(1, 1, 3, 10)
	// dist reports only one ready segment, but datacoord still knows a stale one (102)
	// that has not been reported -> not satisfied.
	dist := &fakeDist{segs: map[int64][]SegmentDistInfo{1: {{SegmentID: 100, SchemaVersion: 3}}}}
	scope := &fakeScope{stale: map[int64][]int64{1: {102}}}
	assert.False(t, NewReadinessProvider(dist, scope).IsRoundReady(ctx, round))
}
...
func TestReadiness_SparkScalar(t *testing.T) {
	...
	assert.False(t, NewReadinessProvider(below, nil).IsRoundReady(ctx, round))
	...
	assert.False(t, NewReadinessProvider(missing, nil).IsRoundReady(ctx, round))
}
...
func TestReadiness_SparkScalar_V3Manifest(t *testing.T) {
	...
	assert.False(t, NewReadinessProvider(bad, nil).IsRoundReady(ctx, round))
}
...
func TestReadiness_RoundAtomicity(t *testing.T) {
	...
	assert.False(t, NewReadinessProvider(dist, nil).IsRoundReady(ctx, round))
}

func TestReadiness_EmptyRoundIsSatisfied(t *testing.T) {
	dist := &fakeDist{segs: map[int64][]SegmentDistInfo{}}
	assert.True(t, NewReadinessProvider(dist, nil).IsRoundReady(context.Background(),
