...
func vectorRound(coll, round int64, fields ...int64) *BackfillRound {
	return &BackfillRound{CollectionID: coll, RoundID: round, Fields: fields, Scope: NewWatermarkScope(2, 0)}
}
...
func TestBackfillAtomicGate_SourceDedupConcurrent(t *testing.T) {
	...
	// The batches of ONE Spark commit ack concurrently, each registering the same
	// source under a freshly-allocated roundID -- the atomic check-and-save must
	// collapse them onto a single round (no duplicates, no stray etcd entries).
	const batches = 16
...
func TestBackfillAtomicGate_SkipRegisterWhenBarrierDisabled(t *testing.T) {
	ctx := context.Background()
	key := paramtable.Get().QueryCoordCfg.UpdateTargetNeedSegmentDataReady.Key
	paramtable.Get().Save(key, "false")
	defer paramtable.Get().Reset(key)

	// With the promote barrier disabled a registered round could never be certified
	// (and thus never revoked) -- registration must stand down instead of arming an
	// un-revokable gate.
	kv := newFakeGateKV()
	reg := NewBackfillAtomicGate(kv, newRecordingPusher())
	assert.NoError(t, reg.Register(ctx, vectorRound(1, 1, 10)))
	assert.Empty(t, reg.List())
	assert.Empty(t, reg.GatedFields(1))
}
