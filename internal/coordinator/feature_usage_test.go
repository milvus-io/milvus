...
func TestMixCoord_collectProxyFeatureUsage(t *testing.T) {
	...
	t.Run("no rootcoord", func(t *testing.T) {
	...
	t.Run("no proxies", func(t *testing.T) {
		...
		coord := coordWithProxyManager(t, pcm)
...
// coordWithProxyManager returns a MixCoord whose proxy fan-out sees pcm. The
// live manager belongs to rootcoord, fed by its proxy session watcher, so that
// is where the test injects it.
func coordWithProxyManager(t *testing.T, pcm *proxyutil.MockProxyClientManager) *mixCoordImpl {
	patch := mockey.Mock((*rootcoord.Core).GetProxyClientManager).Return(pcm).Build()
	t.Cleanup(func() { patch.UnPatch() })
	return &mixCoordImpl{rootcoordServer: &rootcoord.Core{}}
}
