...
func TestBackfillAtomicGateObserver_SweepRevokesOnlySatisfied(t *testing.T) {
	...
	o := NewBackfillAtomicGateObserver(context.Background(), store, readiness, 0)
	o.sweep()
...
func TestBackfillAtomicGateObserver_SweepNoneReady(t *testing.T) {
	...
	o := NewBackfillAtomicGateObserver(context.Background(), store, satisfiedSet{ready: map[roundKey]bool{}}, 0)
	o.sweep()
	assert.Empty(t, store.revoked)
}

func TestBackfillAtomicGateObserver_StartStopIdempotent(t *testing.T) {
	o := NewBackfillAtomicGateObserver(context.Background(), &fakeStore{}, satisfiedSet{ready: map[roundKey]bool{}}, 0)
	o.Start()
	o.Start() // idempotent
	o.Stop()
	o.Stop() // idempotent
