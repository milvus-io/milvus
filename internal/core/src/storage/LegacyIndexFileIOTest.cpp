...
class LegacyIndexFileIOTest : public ::testing::Test {
 protected:
    void
    SetUp() override {
...
TEST_F(LegacyIndexFileIOTest, RawRangesAndEmptyPayload) {
...
TEST_F(LegacyIndexFileIOTest, ParquetInt8AndOldString) {
...
TEST_F(LegacyIndexFileIOTest, CancelWaitingAdmissionBeforeAnyRead) {
...
TEST_F(LegacyIndexFileIOTest, CancellationDrainsIssuedRead) {
    auto input = Open(Encode({1, 2, 3}));
    auto info = Run(InspectLegacyIndexFileAsync(*input, kPriority));
    file_->ResetCounters();
...
TEST_F(LegacyIndexFileIOTest, ReadAndTargetFailuresReleaseAdmission) {
    auto input = Open(Encode({1, 2, 3}));
    auto info = Run(InspectLegacyIndexFileAsync(*input, kPriority));
...
TEST_F(LegacyIndexFileIOTest, EncodedEventLengthValidatedBeforeDecoding) {
...
TEST_F(LegacyIndexFileIOTest, RejectMalformedDescriptorAndRawEvent) {
...
TEST_F(LegacyIndexFileIOTest, LocalSizeAndReadsRunOnLocalFilePool) {
    LocalFileIOPool::GetInstance().Configure(1);
    auto restore =
...
TEST_F(LegacyIndexFileIOTest, GenericFilesystemSizeKeepsThrownErrorCode) {
