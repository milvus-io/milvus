        ...
        for (int null_percent : {0, 20, 100}) {
            ...
            if (vec_type.is_sparse) {
                ...
                field_data = storage::CreateFieldData(
                    DataType::VECTOR_SPARSE_U32_F32, DataType::NONE, true, sparse_dim, num_rows);
                auto field_data_impl =
                    std::dynamic_pointer_cast<milvus::FieldData<milvus::SparseFloatVector>>(field_data);
                field_data_impl->FillFieldDataSparse(sparse_vecs.get(), valid_data.data(), num_rows, 0);
            } else {
                ...
                if (vec_type.data_type == DataType::VECTOR_FLOAT) {
                    auto impl = std::dynamic_pointer_cast<milvus::FieldData<milvus::FloatVector>>(field_data);
                    impl->FillFieldDataSparse(vec_data.data(), valid_data.data(), num_rows, 0);
                } else if (vec_type.data_type == DataType::VECTOR_FLOAT16) {
                    auto impl = std::dynamic_pointer_cast<milvus::FieldData<milvus::Float16Vector>>(field_data);
                    impl->FillFieldDataSparse(vec_data.data(), valid_data.data(), num_rows, 0);
                } else if (vec_type.data_type == DataType::VECTOR_BFLOAT16) {
                    auto impl = std::dynamic_pointer_cast<milvus::FieldData<milvus::BFloat16Vector>>(field_data);
                    impl->FillFieldDataSparse(vec_data.data(), valid_data.data(), num_rows, 0);
                } else if (vec_type.data_type == DataType::VECTOR_INT8) {
                    auto impl = std::dynamic_pointer_cast<milvus::FieldData<milvus::Int8Vector>>(field_data);
                    impl->FillFieldDataSparse(vec_data.data(), valid_data.data(), num_rows, 0);
                } else if (vec_type.data_type == DataType::VECTOR_BINARY) {
                    auto impl = std::dynamic_pointer_cast<milvus::FieldData<milvus::BinaryVector>>(field_data);
                    impl->FillFieldDataSparse(vec_data.data(), valid_data.data(), num_rows, 0);
                }
            }
            ...
            std::string insert_file_path = "/tmp/diskann/nullable_" + vec_type.type_name +
                                          "_" + std::to_string(null_percent);
            boost::filesystem::remove_all(insert_file_path);
            cm_->Write(insert_file_path, serialized_data.data(), serialized_data.size());
            ...
            if (vec_type.is_sparse) {
                ...
                std::shared_ptr<uint8_t[]> serialized_data_ptr(buffer.data(), [&](uint8_t*) {});
                ...
                ASSERT_TRUE(new_payload->get_data_type() == DataType::VECTOR_SPARSE_U32_F32);
                ...
                for (int i = 0; i < num_rows; ++i) {
                    if (i < valid_count) {
                        ASSERT_TRUE(new_payload->is_valid(i))
                            << "Row " << i << " should be valid for null_percent=" << null_percent;

                        auto original = &sparse_vecs[i];
                        auto new_vec = static_cast<
                            const knowhere::sparse::SparseRow<milvus::SparseValueType>*>(
                            new_payload->RawValue(i));
                        ASSERT_EQ(original->size(), new_vec->size())
                            << "Size mismatch at row " << i << " for null_percent=" << null_percent;

                        for (size_t j = 0; j < original->size(); ++j) {
                            ASSERT_EQ((*original)[j].id, (*new_vec)[j].id)
                                << "ID mismatch at row " << i << ", element " << j
                                << " for null_percent=" << null_percent;
                            ASSERT_EQ((*original)[j].val, (*new_vec)[j].val)
                                << "Value mismatch at row " << i << ", element " << j
                                << " for null_percent=" << null_percent;
                        }
                    } else {
                        ASSERT_FALSE(new_payload->is_valid(i))
                            << "Row " << i << " should be null for null_percent=" << null_percent;
                    }
                }
            } else {
                IndexMeta index_meta = {segment_id, field_id, 1000, 1, "test", "vec_field",
                                       vec_type.data_type, dim};
                ...
                if (vec_type.data_type == DataType::VECTOR_FLOAT) {
                    local_data_path = file_manager->CacheRawDataToDisk<float>(config);
                } else if (vec_type.data_type == DataType::VECTOR_INT8) {
                    local_data_path = file_manager->CacheRawDataToDisk<int8_t>(config);
                } else if (vec_type.data_type == DataType::VECTOR_FLOAT16) {
                    local_data_path = file_manager->CacheRawDataToDisk<knowhere::fp16>(config);
                } else if (vec_type.data_type == DataType::VECTOR_BFLOAT16) {
                    local_data_path = file_manager->CacheRawDataToDisk<knowhere::bf16>(config);
                } else if (vec_type.data_type == DataType::VECTOR_BINARY) {
                    local_data_path = file_manager->CacheRawDataToDisk<uint8_t>(config);
                }

                ASSERT_FALSE(local_data_path.empty()) << "Failed for " << vec_type.type_name
                                                       << " with null_percent=" << null_percent;

                auto local_chunk_manager = LocalChunkManagerSingleton::GetInstance().GetChunkManager();
                uint32_t read_num_rows = 0;
                uint32_t read_dim = 0;
                local_chunk_manager->Read(local_data_path, 0, &read_num_rows, sizeof(read_num_rows));
                local_chunk_manager->Read(local_data_path, sizeof(read_num_rows), &read_dim, sizeof(read_dim));

                EXPECT_EQ(read_num_rows, valid_count) << "Mismatch for " << vec_type.type_name
                                                       << " with null_percent=" << null_percent;
                EXPECT_EQ(read_dim, dim);

                size_t bytes_per_vector = (vec_type.data_type == DataType::VECTOR_BINARY) ?
                                          (dim / 8) : (dim * vec_type.element_size);
                ...
                EXPECT_EQ(buffer.size(), vec_data.size()) << "Data size mismatch for " << vec_type.type_name;
                for (size_t i = 0; i < std::min(buffer.size(), vec_data.size()); ++i) {
                    EXPECT_EQ(buffer[i], vec_data[i]) << "Data mismatch at byte " << i
                                                       << " for " << vec_type.type_name
                                                       << " with null_percent=" << null_percent;
                }
