...
TEST_F(DiskAnnFileManagerTest, CacheRawDataToDiskVectorArray) {
    ...
    auto field_data = milvus::storage::CreateFieldData(milvus::DataType::VECTOR_ARRAY, milvus::DataType::VECTOR_FLOAT, false, dim, num_rows);
    auto vec_array_data = std::dynamic_pointer_cast<milvus::FieldData<milvus::VectorArray>>(field_data);
    ...
    auto file_manager = std::make_shared<milvus::storage::DiskFileManagerImpl>(
        milvus::storage::FieldDataMeta{collection_id, partition_id, segment_id, field_id},
        milvus::storage::IndexMeta{segment_id, field_id, 1, 1, 1, "test_index"},
        milvus::storage::StorageConfig{});
    
    auto local_chunk_manager = milvus::storage::LocalChunkManagerSingleton::GetInstance().GetChunkManager();
    std::string local_data_path;
    bool file_created = false;
    uint32_t out_dim = dim;
    int64_t write_offset = sizeof(uint32_t) * 2;
    std::vector<size_t> offsets;
    offsets.push_back(0);

    uint32_t var_dim = dim;
    file_manager->cache_raw_data_to_disk_common<float>(
        field_data, local_chunk_manager, local_data_path, file_created, var_dim, write_offset, &offsets);
    
    ASSERT_EQ(offsets.size(), num_rows + 1);
    ASSERT_EQ(offsets.back(), num_rows * vectors_per_row); 

    uint32_t header_num_rows = offsets.back(); 
    write_offset = 0;
    local_chunk_manager->Write(local_data_path, write_offset, &header_num_rows, sizeof(uint32_t));
    write_offset += sizeof(uint32_t);
    local_chunk_manager->Write(local_data_path, write_offset, &var_dim, sizeof(uint32_t));
    ...
    local_chunk_manager->Read(local_data_path, 0, &read_num_rows, sizeof(uint32_t));
    local_chunk_manager->Read(local_data_path, sizeof(uint32_t), &read_dim, sizeof(uint32_t));
