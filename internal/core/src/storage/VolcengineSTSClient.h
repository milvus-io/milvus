 ...
 */
class AWS_CORE_API VolcengineSTSCredentialsClient
    : public AWSHttpResourceClient {
 public:
    /**
     * Initializes the provider to retrieve credentials from STS when it expires.
     */
    explicit VolcengineSTSCredentialsClient(
        const Client::ClientConfiguration& clientConfiguration);

    VolcengineSTSCredentialsClient&
    operator=(VolcengineSTSCredentialsClient& rhs) = delete;
    VolcengineSTSCredentialsClient(const VolcengineSTSCredentialsClient& rhs) =
        delete;
    VolcengineSTSCredentialsClient&
    operator=(VolcengineSTSCredentialsClient&& rhs) = delete;
    VolcengineSTSCredentialsClient(const VolcengineSTSCredentialsClient&& rhs) =
    ...
    Aws::String
    buildJwtToken(const STSAssumeRoleWithWebIdentityRequest& request);
