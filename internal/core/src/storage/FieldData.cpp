...
void
FieldDataImpl<Type, is_scalar>::FillFieldData(const void* source, ssize_t element_count) {
    if (element_count == 0) {
        return;
    }
    ...
    std::copy_n(static_cast<const Type*>(source), element_count * dim_, field_data_.data() + tell_ * dim_);
    tell_ += element_count;
}
...
template <typename Type, bool is_scalar>
void
FieldDataImpl<Type, is_scalar>::FillFieldData(const std::shared_ptr<arrow::Array> array) {
    ...
    switch (data_type_) {
        ...
        case DataType::INT8: {
            auto array_info = GetDataInfoFromArray<arrow::Int8Array, arrow::Type::type::INT8>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        case DataType::INT16: {
            auto array_info = GetDataInfoFromArray<arrow::Int16Array, arrow::Type::type::INT16>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        case DataType::INT32: {
            auto array_info = GetDataInfoFromArray<arrow::Int32Array, arrow::Type::type::INT32>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        case DataType::INT64: {
            auto array_info = GetDataInfoFromArray<arrow::Int64Array, arrow::Type::type::INT64>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        case DataType::FLOAT: {
            auto array_info = GetDataInfoFromArray<arrow::FloatArray, arrow::Type::type::FLOAT>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        case DataType::DOUBLE: {
            auto array_info = GetDataInfoFromArray<arrow::DoubleArray, arrow::Type::type::DOUBLE>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        ...
        case DataType::VECTOR_BINARY: {
            auto array_info =
                GetDataInfoFromArray<arrow::FixedSizeBinaryArray, arrow::Type::type::FIXED_SIZE_BINARY>(array);
            return FillFieldData(array_info.first, array_info.second);
        }
        default: {
