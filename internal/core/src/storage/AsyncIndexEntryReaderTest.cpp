    ...
    auto materialize_future =
        std::async(std::launch::async,
                   [reader = reader.get(), plan = std::move(plan)]() mutable {
                       return folly::coro::blockingWait(
                           reader->ReadEntriesAsync(std::move(plan)));
                   });
...
TEST_F(AsyncIndexEntryReaderTest, ReadEntriesCancelsQueuedMmapPreparation) {
    ...
    plan.materialization_context = std::shared_ptr<void>(nullptr, [&](void*) {
    ...
    auto load = std::async(std::launch::async, [&] {
        return folly::coro::blockingWait(
            reader->ReadEntriesAsync(std::move(plan), cancellation.getToken()));
    });
    ...
    for (auto outcome :
        ...
        auto load = std::async(std::launch::async, [&] {
            return folly::coro::blockingWait(reader->ReadEntriesAsync(
                std::move(plan), cancellation.getToken()));
        });
        auto drain = folly::makeGuard([&] {
            direct_file->SetAutoComplete(true);
