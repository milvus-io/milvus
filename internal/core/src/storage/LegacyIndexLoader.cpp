...
OpenLegacyIndexInputAsync(const ChunkManagerPtr& chunk_manager,
    ...
    if (!fs || milvus_storage::IsLocalFileSystem(fs)) {
        ...
        co_return std::make_shared<LegacyChunkInput>(chunk_manager,
                                                     remote_file);
    }
    co_return co_await RemoteInputStream::OpenAsync(fs, remote_file);
}
...
StreamLegacyIndexFilesAsync(std::span<const LegacyIndexFile> files,
    ...
    if (files.size() == 1 &&
        ...
        auto input = co_await OpenLegacyIndexInputAsync(
            chunk_manager, fs, files.front().path);
...
InspectLegacyIndexFileAsync(
    ...
    auto input = co_await OpenLegacyIndexInputAsync(chunk_manager, fs, path);
...
InspectLegacyIndexFilesAsync(std::span<const std::string> files,
    ...
    for (const auto& path : files) {
        ...
        if (!infos->contains(path)) {
            auto input =
                co_await OpenLegacyIndexInputAsync(chunk_manager, fs, path);
            infos->emplace(
                path,
