...
TEST(storage, InsertDataFloatVectorNullable) {
    ...
    for (int null_percent : {0, 20, 100}) {
        ...
        int valid_idx = 0;
        auto new_data = static_cast<const float*>(new_payload->Data());
        for (int i = 0; i < num_rows; ++i) {
            if (new_payload->is_valid(i)) {
                auto vec_ptr = static_cast<const float*>(new_payload->RawValue(valid_idx));
                for (int j = 0; j < DIM; ++j) {
...
TEST(storage, InsertDataSparseFloatVectorNullable) {
    ...
    for (int null_percent : {0, 20, 100}) {
        ...
        auto field_data_impl =
            std::dynamic_pointer_cast<milvus::FieldData<milvus::SparseFloatVector>>(
                field_data);
        field_data_impl->FillFieldDataSparse(vecs.get(), valid_data.data(), num_rows, 0);
        ...
        for (int i = 0; i < num_rows; ++i) {
            if (new_payload->is_valid(i)) {
                ...
                auto new_vec =  static_cast<const knowhere::sparse::SparseRow<milvus::SparseValueType>*>(new_payload->RawValue(valid_idx));
                ASSERT_EQ(original.size(), new_vec->size());
...
TEST(storage, InsertDataBinaryVectorNullable) {
    ...
    for (int null_percent : {0, 20, 100}) {
        ...
        for (int i = 0; i < num_rows; ++i) {
            if (new_payload->is_valid(i)) {
                auto vec_ptr = static_cast<const uint8_t*>(new_payload->RawValue(valid_idx));
...
TEST(storage, InsertDataFloat16VectorNullable) {
    ...
    for (int null_percent : {0, 20, 100}) {
        ...
        for (int i = 0; i < num_rows; ++i) {
            if (new_payload->is_valid(i)) {
                auto vec_ptr = static_cast<const float16*>(new_payload->RawValue(valid_idx));
                for (int j = 0; j < DIM; ++j) {
