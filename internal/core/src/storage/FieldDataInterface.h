...
class FieldDataImpl : public FieldDataBase {
 ...
 public:
    explicit FieldDataImpl(ssize_t dim, DataType data_type, int64_t buffered_num_rows = 0)
        : FieldDataBase(data_type), dim_(is_scalar ? 1 : dim), num_rows_(buffered_num_rows), tell_(0) {
    ...
    RawValue(ssize_t offset) const override {
        AssertInfo(offset < get_num_rows(), "field data subscript out of range");
        AssertInfo(offset < get_tell(), "subscript position don't has valid value");
