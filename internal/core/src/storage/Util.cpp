...
add_vector_payload(std::shared_ptr<arrow::ArrayBuilder> builder,
    ...
    AssertInfo((nullable && valid_data) || !nullable, "valid_data is required for nullable vectors");
    arrow::Status ast;
    ...
    if (nullable) {
        ...
        for (int i = 0; i < length; ++i) {
            auto bit = (valid_data[i >> 3] >> (i & 0x07)) & 1;
            if (bit) {
                ast = binary_builder->Append(values + valid_index * byte_width, byte_width);
                valid_index++;
            } else {
                ast = binary_builder->AppendNull();
            }
            AssertInfo(ast.ok(), "append value to arrow builder failed: {}", ast.ToString());
        }
    } else {
...
AddPayloadToArrowBuilder(std::shared_ptr<arrow::ArrayBuilder> builder,
    ...
    switch (data_type) {
        ...
        case DataType::VECTOR_FLOAT: {
            AssertInfo(payload.dimension.has_value(), "dimension is required for VECTOR_FLOAT");
            int byte_width = payload.dimension.value() * sizeof(float);
            add_vector_payload(builder, const_cast<uint8_t*>(raw_data), payload.valid_data, nullable, length, byte_width);
            break;
        }
        case DataType::VECTOR_BINARY: {
            AssertInfo(payload.dimension.has_value(), "dimension is required for VECTOR_BINARY");
            int byte_width = (payload.dimension.value() + 7) / 8;
            add_vector_payload(builder, const_cast<uint8_t*>(raw_data), payload.valid_data, nullable, length, byte_width);
            break;
        }
        case DataType::VECTOR_FLOAT16: {
            AssertInfo(payload.dimension.has_value(), "dimension is required for VECTOR_FLOAT16");
            int byte_width = payload.dimension.value() * 2;
            add_vector_payload(builder, const_cast<uint8_t*>(raw_data), payload.valid_data, nullable, length, byte_width);
            break;
        }
        case DataType::VECTOR_BFLOAT16: {
            AssertInfo(payload.dimension.has_value(), "dimension is required for VECTOR_BFLOAT16");
            int byte_width = payload.dimension.value() * 2;
            add_vector_payload(builder, const_cast<uint8_t*>(raw_data), payload.valid_data, nullable, length, byte_width);
            break;
        }
        case DataType::VECTOR_INT8: {
            AssertInfo(payload.dimension.has_value(), "dimension is required for VECTOR_INT8");
    ...
    switch (static_cast<DataType>(data_type)) {
        case DataType::VECTOR_FLOAT: {
            ...
            if (nullable) {
                ...
                return arrow::schema({arrow::field("val", arrow::binary(), nullable, metadata)});
            }
            return arrow::schema(
                {arrow::field("val",
                              arrow::fixed_size_binary(dim * sizeof(float)),
