...
struct IndexLoadPlan {
    ...
    // Index-specific metadata needed only by MaterializeAsync(). The reader
...
class IndexLoadArtifact {
 public:
    ...
    IndexLoadArtifact(IndexLoadArtifact&& other) noexcept
        : entries_(std::move(other.entries_)),
          materialization_context_(std::move(other.materialization_context_)),
    ...
    operator=(IndexLoadArtifact&& other) noexcept {
        if (this != &other) {
            ...
            entries_ = std::move(other.entries_);
            materialization_context_ =
                std::move(other.materialization_context_);
            cleanup_targets_ = std::move(other.cleanup_targets_);
            other.cleanup_targets_.clear();
        }
        return *this;
    ...
    MaterializationContext() {
        return std::any_cast<Context&>(materialization_context_);
    }

    template <typename Context>
    const Context&
    MaterializationContext() const {
        return std::any_cast<const Context&>(materialization_context_);
    }
 ...
 private:
    ...
    std::any materialization_context_;
