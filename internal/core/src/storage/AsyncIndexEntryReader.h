...
class AsyncIndexEntryReader {
 public:
    ...
    // Fill the plan's targets on the shared async executor. Publish the first
    ...
    ReadEntriesAsync(IndexLoadPlan plan, folly::CancellationToken token = {});
    ...
    // ReadEntriesAsync owns the lease through CRC and destination placement.
