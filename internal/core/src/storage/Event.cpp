...
DescriptorEventData::DescriptorEventData(BinlogReaderPtr reader) {
    ...
    auto ast = reader->Read(post_header_lengths.size(), post_header_lengths.data());
    assert(ast.ok());
...
BaseEventData::BaseEventData(BinlogReaderPtr reader, int event_length, DataType data_type) {
    ...
    auto payload_reader = std::make_shared<PayloadReader>(res.second.get(), payload_length, data_type);
...
BaseEventData::Serialize() {
    ...
    if (milvus::datatype_is_vector(data_type)) {
        payload_writer = std::make_unique<PayloadWriter>(data_type, field_data->get_dim());
    } else {
...
BaseEvent::BaseEvent(BinlogReaderPtr reader, DataType data_type) {
    ...
    auto event_data_length = event_header.event_length_ - GetEventHeaderSize(event_header);
...
LocalIndexEvent::LocalIndexEvent(BinlogReaderPtr reader) {
    ...
    AssertInfo(res.first.ok(), "read payload failed");
    auto payload_reader = std::make_shared<PayloadReader>(res.second.get(), index_size, DataType::INT8);
    field_data = payload_reader->get_field_data();
}
