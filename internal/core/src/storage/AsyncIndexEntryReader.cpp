...
AsyncIndexEntryReader::Open(std::shared_ptr<milvus::InputStream> input,
    ...
    auto metadata = co_await reader->ReadEntriesAsync(std::move(plan), token);
    if (!data->empty()) {
        try {
...
folly::coro::Task<void>
CleanupEntryReadAsync(
    IndexLoadPlan& plan,
    const std::vector<std::shared_ptr<MmapFileTarget>>& targets) {
    CleanupUncommittedMmapTargets(targets);
    plan.materialization_context.reset();
    co_return;
}

folly::coro::Task<void>
ReadEntrySliceAsync(AsyncIndexEntryReader* reader,
...
AsyncIndexEntryReader::ReadEntriesAsyncImpl(
    ...
    artifact.materialization_context_ = std::move(plan.materialization_context);
        ...
        co_await folly::coro::co_withCancellation(
            ...
            folly::coro::co_withExecutor(
                ResolveAsyncLoadExecutor(
                    ...
                    plan.priority),
                CleanupEntryReadAsync(plan, cleanup_targets)));
    }
