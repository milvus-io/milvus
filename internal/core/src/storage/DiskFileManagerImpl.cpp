...
DiskFileManagerImpl::CacheIndexToDiskAsync(
    ...
    try {
        ...
        for (auto& [prefix, slices] : files) {
            ...
            entries.push_back(
                {name,
                 FileEntryTarget{std::make_shared<IndexFileTarget>(
                                     local_path, file_bytes, retain_on_success),
                                 0,
                                 file_bytes}});
            ...
            co_await ReadLegacyIndexFilesAsync(
                entry_files, rcm_, fs_, entries.back().target, priority, token);
        }
        ...
        [&] {
            if (failure) {
                for (const auto& entry : entries) {
                    std::get<FileEntryTarget>(entry.target).staging->Cleanup();
                }
