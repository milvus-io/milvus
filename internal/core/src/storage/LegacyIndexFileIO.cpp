...
ReadRange(milvus::InputStream& input,
          ...
          folly::CancellationToken token,
          bool use_async = true) {
    ThrowIfCancelled(token, "LegacyIndexFileIO::Read");
    if (!use_async) {
...
WriteTargetAsync(const EntryTarget& target,
    ...
    ThrowIfCancelled(token, "LegacyIndexFileIO::WriteTarget");
...
ReadLegacyUnit(milvus::InputStream& input,
    ...
    ThrowIfCancelled(token, "LegacyIndexFileIO::ReadUnit");
    ...
    if (info.raw_payload) {
        ...
        ThrowIfCancelled(token, "LegacyIndexFileIO::Decode");
        observe_read();
        co_await WriteTargetAsync(target,
                                  base_offset,
    ...
    ThrowIfCancelled(token, "LegacyIndexFileIO::Consume");
}
...
InspectLegacyIndexFileImpl(milvus::InputStream& input,
    ...
    ThrowIfCancelled(token, "LegacyIndexFileIO::InspectSize");
    CheckFormat(
...
StreamLegacyIndexFileAsync(milvus::InputStream& input,
    ...
    token = folly::cancellation_token_merge(
        token, co_await folly::coro::co_current_cancellation_token);
    ThrowIfCancelled(token, "LegacyIndexFileIO::Stream");
    if (info.raw_payload) {
        CheckFormat(
...
StreamLegacyIndexFilesAsync(std::span<const LegacyIndexFile> files,
    ...
    token = folly::cancellation_token_merge(
        token, co_await folly::coro::co_current_cancellation_token);
    ThrowIfCancelled(token, "LegacyIndexFileIO::StreamFiles");
    if (files.empty()) {
        co_return;
    }
    ...
    try {
        for (const auto& file : files) {
            ThrowIfCancelled(effective_token, "LegacyIndexFileIO::Open");
    ...
    ThrowIfCancelled(token, "LegacyIndexFileIO::StreamFilesComplete");
}
...
InspectLegacyIndexFilesAsync(std::span<const std::string> files,
    ...
    for (const auto& path : files) {
        ThrowIfCancelled(token, "LegacyIndexFileIO::InspectFiles");
...
ReadLegacyIndexFilesAsync(std::span<const LegacyIndexFile> files,
    ...
    try {
        if (disk) {
            co_await RunLocalFileIOAsync(
                [&] {
                    ThrowIfCancelled(token, "LegacyIndexFileIO::PrepareTarget");
        ...
        if (disk) {
            co_await RunLocalFileIOAsync(
                [&] {
                    const auto start = std::chrono::steady_clock::now();
                    ThrowIfCancelled(token, "LegacyIndexFileIO::FinishTarget");
                    disk->staging->Finish();
                    monitor::internal_storage_write_disk_duration.Observe(
                        std::chrono::duration_cast<std::chrono::milliseconds>(
                            std::chrono::steady_clock::now() - start)
        ...
        ThrowIfCancelled(token, "LegacyIndexFileIO::ReadFilesComplete");
    } catch (...) {
