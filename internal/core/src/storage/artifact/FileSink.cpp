...
void
V1DiskSink::PutMeta(std::string_view, const nlohmann::json&) {
    AssertOpen(impl_->state);
    impl_->state = SinkState::Failed;
    ThrowInfo(Unsupported,
              "V1/V2 artifacts store legacy metadata as named entries");
}
...
class V3PackedSink::Impl {
 public:
    ...
    std::unique_ptr<IndexEntryWriter> writer;
    std::string file_name;
    ArtifactStoragePath storage_path;
    SinkState state{SinkState::Open};
    ...
    std::string
    PublishedLocation() const {
        auto path = RemotePath();
        if (storage_path == ArtifactStoragePath::Index) {
            return path;
        }
        const auto prefix = manager->GetRemoteTextLogPrefix() + "/";
        return path.size() > prefix.size() &&
                       path.compare(0, prefix.size(), prefix) == 0
                   ? path.substr(prefix.size())
                   : path;
...
V3PackedSink::V3PackedSink(const FileManagerContext& context,
                           std::string file_name,
                           ArtifactStoragePath storage_path)
    : impl_(
          std::make_unique<Impl>(context, std::move(file_name), storage_path)) {
}

V3PackedSink::~V3PackedSink() = default;

Generation
V3PackedSink::Gen() const {
    return Generation::V3;
}

void
V3PackedSink::WriteEntry(std::string_view name, const void* data, size_t size) {
    AssertOpen(impl_->state);
    try {
        impl_->writer->WriteEntry(std::string(name), data, size);
    } catch (...) {
        impl_->state = SinkState::Failed;
        throw;
    }
...
void
V3PackedSink::WriteEntryFromLocalFile(std::string_view name,
                                      const std::string& local_path) {
    AssertOpen(impl_->state);
    try {
        int fd = ::open(local_path.c_str(), O_RDONLY | O_CLOEXEC);
        if (fd < 0) {
            ThrowInfo(FileOpenFailed,
                      "failed to open borrowed local artifact entry {}: {}",
                      local_path,
                      std::strerror(errno));
        }
        ScopedFd scoped_fd(fd);
        ...
        if (::fstat(fd, &stat_buf) != 0) {
            ThrowInfo(FileReadFailed,
                      "failed to stat borrowed local artifact entry {}: {}",
                      local_path,
                      std::strerror(errno));
        }
        if (stat_buf.st_size < 0) {
            ThrowInfo(FileReadFailed,
                      "invalid borrowed local artifact entry size {}",
                      local_path);
        }
        impl_->writer->WriteEntry(std::string(name),
                                  scoped_fd.Get(),
...
void
V3PackedSink::WriteRawEntryFromLocalFile(std::string_view, const std::string&) {
    AssertOpen(impl_->state);
    impl_->state = SinkState::Failed;
    ThrowInfo(Unsupported,
              "V3 packed transport cannot publish a legacy raw file entry");
}

void
V3PackedSink::PutMeta(std::string_view key, const nlohmann::json& value) {
    AssertOpen(impl_->state);
    try {
        impl_->writer->PutMeta(std::string(key), value);
    } catch (...) {
        impl_->state = SinkState::Failed;
        throw;
    }
...
V3PackedSink::Finish() {
    ...
    try {
        impl_->writer->Finish();
        auto size = static_cast<int64_t>(impl_->writer->GetTotalBytesWritten());
        auto stats = ArtifactStats(
            size, {SerializedFileInfo(impl_->PublishedLocation(), size)});
        impl_->state = SinkState::Finished;
        return stats;
    } catch (...) {
        impl_->state = SinkState::Failed;
...
void
V3PackedSink::ReleaseLocalStaging() {
    // The writer streams directly to remote storage. Local paths passed to
    // WriteEntryFromLocalFile are borrowed inputs, not sink staging.
}
