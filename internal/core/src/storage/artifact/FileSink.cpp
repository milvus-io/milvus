...
RemotePath(const DiskFileManagerImpl& manager,
           ArtifactStorageNamespace storage_namespace,
           std::string_view name) {
    const auto prefix = storage_namespace == ArtifactStorageNamespace::Index
...
StreamRawFile(DiskFileManagerImpl& manager,
    ...
    auto output = manager.OpenOutputStream(
        std::string(name),
        storage_namespace == ArtifactStorageNamespace::Index);
    if (output == nullptr) {
...
AddMemoryFiles(MemFileManagerImpl& manager,
    ...
    return storage_namespace == ArtifactStorageNamespace::TextLog
...
class V1DiskSink::Impl {
 public:
    ...
    RemotePathFor(std::string_view name) const {
        return RemotePath(*disk_manager, storage_namespace, name);
    }
...
V1DiskSink::V1DiskSink(const FileManagerContext& context,
                       ArtifactStorageNamespace storage_namespace)
    : impl_(std::make_unique<Impl>(context, storage_namespace)) {
}
...
V1DiskSink::WriteRawEntryFromLocalFile(std::string_view name,
    ...
    try {
        ...
        AssertInfo(
            ...
            "artifact serialized size overflow");
        if (impl_->storage_namespace == ArtifactStorageNamespace::Index &&
...
V1DiskSink::ReleaseLocalStaging() {
    ...
    if (impl_->storage_namespace == ArtifactStorageNamespace::TextLog) {
