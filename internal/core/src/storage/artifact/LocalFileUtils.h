...
class LocalEntryGuard {
 public:
    ...
    LocalEntryGuard(LocalEntryGuard&& other) noexcept : path_(other.Release()) {
    }
...
// Stream a borrowed file without transferring ownership or staging its bytes.
inline void
WriteEntryFromLocalFile(IndexEntryWriter& writer,
    ...
    if (info.st_size < 0 || static_cast<uintmax_t>(info.st_size) >
        ...
        ThrowInfo(FileReadFailed, "invalid artifact file size: {}", path);
    }
    writer.WriteEntry(std::string(name), fd, static_cast<size_t>(info.st_size));
}

inline void
WriteEntryFromLocalFile(FileSink& sink,
                        std::string_view name,
                        const std::string& path) {
    sink.WriteEntryFromLocalFile(name, path);
}

// Keep the same ordering for legacy files and packed V3 entries.
template <typename Output>
void
WriteEntriesFromLocalFiles(Output& output,
                           std::span<const std::filesystem::path> files) {
    for (const auto& file : files) {
        WriteEntryFromLocalFile(
            output, file.filename().string(), file.string());
    }
}
