...
class FieldData : public FieldDataImpl<Type, true> {
 public:
    ...
    explicit FieldData(DataType data_type, int64_t buffered_num_rows = 0)
        : FieldDataImpl<Type, true>::FieldDataImpl(1, data_type, buffered_num_rows) {
    }
};
...
class FieldData<FloatVector> : public FieldDataImpl<float, false> {
 public:
    explicit FieldData(int64_t dim, DataType data_type, int64_t buffered_num_rows = 0)
        : FieldDataImpl<float, false>::FieldDataImpl(dim, data_type, buffered_num_rows) {
    }
};
...
class FieldData<BinaryVector> : public FieldDataImpl<uint8_t, false> {
 public:
    explicit FieldData(int64_t dim, DataType data_type, int64_t buffered_num_rows = 0)
        : binary_dim_(dim), FieldDataImpl(dim / 8, data_type, buffered_num_rows) {
        Assert(dim % 8 == 0);
    }
