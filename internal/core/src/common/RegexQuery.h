    ...
    MatchImpl(const StringType& str) const {
        ...
        for (size_t i = 0; i < segments_.size(); ++i) {
            ...
            if (is_first && !leading_wildcard_) {
                ...
                // Note: This is byte-level matching, not UTF-8 codepoint-level.
                // Incrementing by 1 byte may land in the middle of a multibyte
                // UTF-8 character. This is intentional for consistency with
                // original Boost regex behavior and SQL LIKE semantics.
                pos = found + 1;
            }
        }
        return true;
