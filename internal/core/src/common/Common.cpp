...
SetIndexSliceSize(const int64_t size) {
    FILE_SLICE_SIZE.store(size << 20, std::memory_order_release);
    LOG_INFO("set config index slice size (byte): {}",
...
SetHighPriorityThreadCoreCoefficient(const float coefficient) {
    ...
    LOG_INFO(
        ...
        HIGH_PRIORITY_THREAD_CORE_COEFFICIENT.load(std::memory_order_acquire));
}
...
SetLowPriorityThreadCoreCoefficient(const float coefficient) {
    ...
    LOG_INFO(
        ...
        LOW_PRIORITY_THREAD_CORE_COEFFICIENT.load(std::memory_order_acquire));
}

void
SetDefaultExecEvalExprBatchSize(int64_t val) {
    EXEC_EVAL_EXPR_BATCH_SIZE.store(val, std::memory_order_release);
...
SetDefaultOptimizeExprEnable(bool val) {
    OPTIMIZE_EXPR_ENABLED.store(val, std::memory_order_release);
    LOG_INFO("set default optimize expr enabled: {}",
             OPTIMIZE_EXPR_ENABLED.load(std::memory_order_acquire));
}

void
SetDefaultJSONKeyStatsCommitInterval(int64_t val) {
    JSON_KEY_STATS_COMMIT_INTERVAL.store(val, std::memory_order_release);
    LOG_INFO("set default json key Stats commit interval: {}",
             JSON_KEY_STATS_COMMIT_INTERVAL.load(std::memory_order_acquire));
}

void
SetDefaultGrowingJSONKeyStatsEnable(bool val) {
    GROWING_JSON_KEY_STATS_ENABLED.store(val, std::memory_order_release);
    LOG_INFO("set default growing json key index enable: {}",
             GROWING_JSON_KEY_STATS_ENABLED.load(std::memory_order_acquire));
}

void
SetDefaultConfigParamTypeCheck(bool val) {
    CONFIG_PARAM_TYPE_CHECK_ENABLED.store(val, std::memory_order_release);
    LOG_INFO("set default config param type check enabled: {}",
             CONFIG_PARAM_TYPE_CHECK_ENABLED.load(std::memory_order_acquire));
}
