...
// #50524: contiguous physical -> logical id window handed to knowhere as
// BitsetView::out_ids, so a brute-force scan can test a logical bitset and
// return logical ids without a per-call transform.
...
class OffsetMapping {
 public:
    ...
    // Contiguous physical -> logical id window starting at `physical_offset`.
    // May be shorter than `count` (chunked storage) and empty when the
    // implementation cannot expose a contiguous window.
...
class OffsetMappingArray {
 public:
    ...
    // Contiguous storage in both modes, so an id window can be handed out
    // without copying (#50524's BitsetView::out_ids path).
    const int32_t*
    data() const {
        return IsMmap() ? mmap_data_ : vec_.data();
    }
