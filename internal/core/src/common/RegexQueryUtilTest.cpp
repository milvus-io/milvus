...
TEST(InvalidUTF8ConsistencyTest, TruncatedMultibyteSequence) {
    ...
    // Test with percent wildcard - both matchers reject invalid UTF-8
    {
        ...
        EXPECT_EQ(re2_result, like_result)
            << "RE2/Like must agree on truncated 2-byte UTF-8 with % wildcard";
    }
}
...
TEST(InvalidUTF8ConsistencyTest, InvalidContinuationByte) {
    ...
    // Both reject: RE2's . won't match lone continuation byte,
    // Utf8WildcardCharByteLen returns 0 for 0x80 (not a valid lead byte).
    EXPECT_EQ(re2_result, like_result)
