...
inline std::string
GetBsonHex(const uint8_t* data, size_t size) {
...
inline bool
CanConvertToInt32(double x) {
    return std::trunc(x) == x && x >= std::numeric_limits<int32_t>::min() &&
...
class BsonView {
 public:
    ...
    // explicit BsonView(const std::string& json_str):
    //       data_(bsoncxx::from_json(json_str).data()),
    //       size_(bsoncxx::from_json(json_str).size()) {
    // }

    std::string
    ToString() const {
        //LOG_DEBUG("bson hex: {}", GetBsonHex(data_, size_));
    ...
    IsNullAtOffset(size_t offset) {
        ...
        if (type_tag == bsoncxx::type::k_null) {
            return true;
        }
        return false;
    }
    ...
    ParseAsValueAtOffset(size_t offset) {
        //LOG_DEBUG("bson hex: {}", GetBsonHex(data_ + offset, size_ - offset));
        const uint8_t* ptr = data_ + offset;
    ...
    ParseAsDocumentAtOffset(size_t offset) {
        if (offset == 0) {
            // if offset is 0, it means the document is the whole bson_view
            return bsoncxx::document::view(data_, size_);
        }
        ...
        // check type
        AssertInfo(
            static_cast<bsoncxx::type>(*ptr) == bsoncxx::type::k_document,
            "ParseAsDocumentAtOffset expects a document at offset {}",
            offset);
        ptr++;
        ...
        if (ptr + len > data_ + size_) {
            PanicInfo(ErrorCode::UnexpectedError,
                      "ParseAsDocumentAtOffset out of range");
        }

        return bsoncxx::document::view(view_start, len);
    }
