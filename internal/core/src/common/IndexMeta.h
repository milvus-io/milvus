...
class FieldIndexMeta {
 public:
    ...
    explicit FieldIndexMeta(const milvus::proto::segcore::FieldIndexMeta& fieldIndexMeta);
    ...
    knowhere::IndexType
    GetIndexType() const {
        return index_params_.at("index_type");
    }
...
class CollectionIndexMeta {
 public:
    // just for unittest
    CollectionIndexMeta(int64_t max_index_row_cnt, std::map<FieldId, FieldIndexMeta>&& fieldMetas);

    explicit CollectionIndexMeta(const milvus::proto::segcore::CollectionIndexMeta& collectionIndexMeta);
...
static const IndexMetaPtr empty_index_meta =
    std::make_shared<CollectionIndexMeta>(1024, std::map<FieldId, FieldIndexMeta>());
