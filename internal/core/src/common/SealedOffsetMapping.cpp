...
SealedOffsetMapping::Build(const bool* valid_data,
    ...
    // i2o holds exactly valid_count entries, so the contiguous int32 array is
    // both smaller and faster than a hash map of the same entries -- and it is
    // what GetPhysicalToLogicalIds hands to knowhere as out_ids (#50524).
    use_i2o_map_ = false;
...
SealedOffsetMapping::GetPhysicalToLogicalIds(int64_t physical_offset,
    ...
    AssertInfo(physical_offset <= valid_count_ - count,
               ...
               valid_count_);
    AssertInfo(!use_i2o_map_,
               "physical -> logical ids require contiguous i2o storage");
