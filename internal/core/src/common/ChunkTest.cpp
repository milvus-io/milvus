...
TEST(chunk, test_date_field) {
    FixedVector<int32_t> data = {1, 2, 3, 4, 5};  // Date is stored as int32
    auto field_data =
        milvus::storage::CreateFieldData(DataType::DATE, DataType::NONE);
    field_data->FillFieldData(data.data(), data.size());
    storage::InsertEventData event_data;
    auto payload_reader =
        std::make_shared<milvus::storage::PayloadReader>(field_data);
    ...
    EXPECT_EQ(span.row_count(), data.size());
    for (size_t i = 0; i < data.size(); ++i) {
        auto n = *(int32_t*)((char*)span.data() + i * span.element_sizeof());
        EXPECT_EQ(n, data[i]);
    }
}

TEST(chunk, test_time_field) {
    FixedVector<int64_t> data = {1, 2, 3, 4, 5};  // Time is stored as int64
    auto field_data =
        milvus::storage::CreateFieldData(DataType::TIME, DataType::NONE);
    field_data->FillFieldData(data.data(), data.size());
    storage::InsertEventData event_data;
    auto payload_reader =
        std::make_shared<milvus::storage::PayloadReader>(field_data);
