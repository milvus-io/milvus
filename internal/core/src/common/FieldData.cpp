...
template <typename Type, bool is_type_entire_row>
void
FieldDataImpl<Type, is_type_entire_row>::FillFieldDataSparse(
    const void* field_data,
    const uint8_t* valid_data,
    ssize_t total_element_count,
    ssize_t offset) {
    AssertInfo(nullable_, "requires nullable to be true");
    ...
    std::lock_guard lck(tell_mutex_);
    resize_field_data(length_ + total_element_count,
    ...
    if (valid_count > 0) {
        std::copy_n(static_cast<const Type*>(field_data),
                    valid_count * dim_,
                    data_.data() + valid_count_ * dim_);
        valid_count_ += valid_count;
    }

    null_count_ = total_element_count - valid_count;
    length_ += total_element_count;
}
    ...
    switch (data_type_) {
        ...
        case DataType::VECTOR_BINARY: {
            if (nullable_) {
                ...
                return FillFieldDataSparse(binary_array->value_data()->data(),
        ...
        case DataType::VECTOR_SPARSE_U32_F32: {
            ...
            if (nullable_) {
                ...
                return FillFieldDataSparse(values.data(),
