...
ArrowMemoryPool
arrow_create_memory_pool(bool enable_logging) {
...
void
arrow_release_pool(ArrowMemoryPool pool) {
    release_ref<mem_holder>(pool);
}

int
arrow_pool_allocate(ArrowMemoryPool pool, int64_t size, uint8_t** out) {
...
void
arrow_pool_free(ArrowMemoryPool pool, uint8_t* buffer, int64_t size) {
    auto holder = retrieve_instance<mem_holder>(pool);
    holder->pool->Free(buffer, size);
}

int
arrow_pool_reallocate(ArrowMemoryPool pool, int64_t old_size, int64_t new_size, uint8_t** ptr) {
    auto holder = retrieve_instance<mem_holder>(pool);
...
int64_t
arrow_pool_bytes_allocated(ArrowMemoryPool pool) {
    auto holder = retrieve_instance<mem_holder>(pool);
