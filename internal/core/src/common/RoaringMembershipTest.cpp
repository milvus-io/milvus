...
TEST(RoaringMembershipTest, ParsesGoGeneratedPortableFixture) {
    // Generated by pkg/util/roaringfilter.Build in Go from:
...
TEST(RoaringMembershipTest, RejectsStructurallyMalformedPortableBodies) {
    ...
    const auto bad_offset_child = [&](std::vector<uint32_t> declared_offsets) {
        return BuildNoRunChild({{0, 1, ArrayPayload({1})},
                                {1, 1, ArrayPayload({2})},
                                {2, 1, ArrayPayload({3})}},
                               std::move(declared_offsets));
    };
    ...
    const std::vector<MalformedCase> cases = {
        ...
        {"ignored bad offset",
         ...
         1},
        {"bad first offset",
         BuildPortableRoaring64({{0, bad_offset_child({0, 34, 36})}}),
         3},
        {"bad middle offset",
         BuildPortableRoaring64({{0, bad_offset_child({32, 0, 36})}}),
         3},
        {"bad last offset",
         BuildPortableRoaring64({{0, bad_offset_child({32, 34, 0})}}),
         3},
        {"unused run bitmap bits",
         BuildPortableRoaring64(
             {{0, BuildRunCookieChild({{0, 1, RunPayload({{0, 0}})}}, 0x81)}}),
         1},
        {"zero no-run container count",
         BuildPortableRoaring64({{0, zero_container_child}}),
         0},
