...
size_t
GetLeafRowSize(const ScalarFieldProto& row, DataType data_type) {
...
size_t
GetLeafRowSize(const ArrayValueView& row, DataType data_type) {
...
size_t
ColumnarArrayFixedWidth(DataType data_type) {
    switch (data_type) {
        case DataType::BOOL:
...
CopyLeafRow(ColumnarArrayBuildNode& node,
    ...
    const auto width = ColumnarArrayFixedWidth(data_type);
...
BuildColumnarArrayNodeImpl(const std::vector<const ScalarFieldProto*>& rows,
    ...
    if (GetColumnarArrayElementType(type) == DataType::ARRAY) {
        ...
        node->array_child = BuildColumnarArrayNodeImpl(child_rows, child_type);
        return node;
    }
    ...
    for (const auto* row : rows) {
        child_count += GetLeafRowSize(*row, node->leaf_type);
        node->offsets.push_back(child_count);
        if (is_string_leaf &&
    ...
    if (is_string_leaf) {
        ...
        const auto width = ColumnarArrayFixedWidth(node->leaf_type);
...
std::unique_ptr<ColumnarArrayBuildNode>
BuildColumnarArrayNodeImpl(const std::vector<ArrayValueView>& rows,
    ...
    if (GetColumnarArrayElementType(type) == DataType::ARRAY) {
        ...
        node->array_child = BuildColumnarArrayNodeImpl(
            child_rows,
...
BuildColumnarArrayNode(std::span<const ArrayValue> values,
    ...
    return BuildColumnarArrayNodeImpl(rows, valid_data, type);
}
...
ColumnarArraySerializedSize(int64_t row_count,
    ...
    return ColumnarArrayChunk::RootDataOffset(row_count, nullable) +
           root.offsets.size() * sizeof(ArrayOffset) +
           ColumnarArrayChildSize(root) + MMAP_ARRAY_PADDING;
}
...
MaterializeMmapColumnarArrayChunk(
    ...
    if (nullable) {
        target->write(root.validity_bitmap.data(), root.validity_bitmap.size());
    }
...
CreateMmapColumnarArrayChunkFromProtoRows(
    ...
    return MaterializeMmapColumnarArrayChunk(
        *root, row_count, type, nullable, mmap_descriptor);
}
...
CreateMmapColumnarArrayChunkFromValues(
    ...
    return MaterializeMmapColumnarArrayChunk(
        *root, row_count, type, !valid_data.empty(), mmap_descriptor);
}
