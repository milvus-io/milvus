...
constexpr size_t kPortableRoaring64MinEntryBytes = 15;
...
ValidatePortableRoaring32(std::string_view data) {
    ...
    if (has_run_bitmap) {
        ...
        const auto used_bits = container_count % 8;
        if (used_bits != 0) {
            const auto valid_mask =
                static_cast<uint8_t>((uint16_t{1} << used_bits) - 1);
            const auto last_byte = static_cast<uint8_t>(
                data[run_bitmap_start + run_bitmap_bytes - 1]);
            if ((last_byte & static_cast<uint8_t>(~valid_mask)) != 0) {
                ThrowInfo(
                    ExprInvalid,
                    "invalid portable roaring64 body: run bitmap has bits "
                    "beyond container count");
            }
        }
        ...
        if (container_count == 0 || container_count > (uint32_t{1} << 16)) {
            ThrowInfo(ExprInvalid,
    ...
    const auto offset_start = cursor;
    if (has_offsets) {
    ...
    for (uint32_t i = 0; i < container_count; ++i) {
        if (has_offsets) {
            const auto declared_offset =
                ReadLittleEndianU32(data.data() + offset_start +
                                    static_cast<size_t>(i) * sizeof(uint32_t));
            if (static_cast<uint64_t>(declared_offset) !=
                static_cast<uint64_t>(cursor)) {
                ThrowInfo(
                    ExprInvalid,
                    "invalid portable roaring64 body: container {} offset "
