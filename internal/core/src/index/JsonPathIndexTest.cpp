...
TEST(JsonPathIndexTest, LegacyPresenceMatchesForRootAndNestedValues) {
    ...
    for (int32_t version : {5, 6}) {
        const std::vector<size_t> expected =
            version == 5 ? std::vector<size_t>{8, 9, 10, 11, 12, 13, 16, 17, 18}
                         : std::vector<size_t>{8, 16};
        for (const std::string path : {"", "/a"}) {
            SCOPED_TRACE("version=" + std::to_string(version) +
                         " path=" + path);
            std::vector<std::string> rows;
            for (const auto& value : values) {
                rows.push_back(path.empty() ? value : "{\"a\":" + value + "}");
            }
            auto result = ConvertJsonToTypedFieldData<double>(
                {MakeJsonFieldData(rows)},
                MakeJsonSchema(),
                path,
                JsonCastType::FromString("DOUBLE"),
                JsonCastFunction::FromString("unknown"),
                version);
            EXPECT_EQ(result.non_exist_offsets, expected);
        }
    }
}
...
using JsonNumericArtifactTypes = testing::Types<int64_t, double>;
