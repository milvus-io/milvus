...
TEST_F(VectorMemIndexAsyncLoadTest, MinhashDiskLoadAndQueryParity) {
    ...
    for (const bool in_memory : {false, true}) {
        ...
        for (const bool enabled : {false, true}) {
            ...
            segcore::storagev2translator::SetStorageV2AsyncLoadEnabled(enabled);
            source_->expect_async = enabled;
            const auto resource = IndexFactory::GetInstance().IndexLoadResource(
                ...
                {{INDEX_TYPE, "MINHASH_LSH"},
                 ...
                 {"with_raw_data", "true"},
                 {"mh_lsh_code_in_mem", in_memory ? "true" : "false"}},
                false,
                rows + 2,
                dim,
                config.at(INDEX_FILES).get<std::vector<std::string>>(),
                Context());
            EXPECT_GT(resource.max_memory_cost, resource.final_memory_cost);
...
TEST_F(VectorMemIndexAsyncLoadTest,
       EstimateKeepsLegacyBatchAndCoversAsyncExpansion) {
    auto context = Context();
    auto infos = std::make_shared<storage::LegacyIndexFileInfos>();
    std::vector<std::string> files;
    constexpr size_t payload = DEFAULT_INDEX_FILE_SLICE_SIZE;
    const auto old_slice_size = FILE_SLICE_SIZE.exchange(payload);
    const auto restore =
        folly::makeGuard([&] { FILE_SLICE_SIZE = old_slice_size; });
    for (int i = 0; i < 12; ++i) {
        files.push_back("payload_" + std::to_string(i));
        infos->emplace(files.back(),
                       storage::LegacyIndexFileInfo{
                           payload, payload, 0, 2 * payload, true});
    }
    context.legacy_index_files = infos;
    for (const bool mmap : {false, true}) {
        for (const bool enabled : {false, true}) {
            context.use_async_load = enabled;
            const uint64_t scratch =
                enabled ? 24 * payload : DEFAULT_FIELD_MAX_MEMORY_LIMIT;
            ...
            for (const bool live : {false, true}) {
                ...
                admission.SetCapacityBytes(live ? 0 : 1 << 20);
                admission.SetCapacitySlots(live ? 0 : 1);
                const auto request =
                    IndexFactory::GetInstance().IndexLoadResource(
                        DataType::VECTOR_FLOAT,
                        DataType::NONE,
                        version_,
                        1,
                        ...
                        files,
                        context);
                EXPECT_EQ(request.max_memory_cost - request.final_memory_cost,
                          scratch + (mmap ? storage::FileWriter::MAX_BUFFER_SIZE
                                          : 12 * payload));
                if (initial_peak) {
...
TEST_F(VectorMemIndexAsyncLoadTest, CachedFileManagerKeepsLoadingMode) {
    for (const bool disk : {false, true}) {
        for (const bool enabled : {false, true}) {
            auto context = Context();
            context.use_async_load = enabled;
            ...
            const auto type = RegisterFileLoadNode(observation);
            auto config = ConfigFor(type);
            const std::vector<uint8_t> bytes(8193, 42);
            config[INDEX_FILES] = std::vector<std::string>{PersistBytes(
                disk ? "payload_0" : "payload", bytes.data(), bytes.size())};
            test::TmpPath dir;
            if (!disk) {
            ...
            for (const bool live : {!enabled, enabled}) {
                segcore::storagev2translator::SetStorageV2AsyncLoadEnabled(
                    live);
                std::unique_ptr<IndexBase> loaded;
                if (disk) {
                ...
                loaded->Load({}, config, nullptr);
                EXPECT_EQ(observation->thread.starts_with("MILVUS_ASYNC"),
                          enabled);
            }
            observation->finalize = {};
        }
...
TEST_F(VectorMemIndexAsyncLoadTest,
    ...
    for (const auto& file : files) {
        auto input = folly::coro::blockingWait(
            storage::OpenLegacyIndexInputAsync(source_, nullptr, file));
        const auto info =
        ...
        scratch += info.TotalTransientBytes();
    }
    for (const bool enabled : {false, true}) {
        ...
        EXPECT_GE(request.max_memory_cost,
                  request.final_memory_cost + retained +
                      (enabled ? scratch : DEFAULT_FIELD_MAX_MEMORY_LIMIT));
    }
}
