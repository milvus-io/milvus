...
aa("--efConstruction", default=200, type=int, help="HNSW construction factor")
...
aa("--add_bs", default=-1, type=int, help="add elements index by batches of this size")
aa(
...
aa("--train_on_gpu", default=False, action="store_true", help="do training on GPU")
aa(
    "--get_centroids_from",
    default="",
...
aa("--searchthreads", default=-1, type=int, help="nb of threads to use at search time")
aa(
    "--searchparams",
    nargs="+",
    default=["autotune"],
...
aa("--n_autotune", default=500, type=int, help="max nb of autotune experiments")
aa(
    "--autotune_max",
    default=[],
    nargs="*",
...
xt, xb, xq, gt = datasets.load_data(dataset=args.db, compute_gt=args.compute_gt)


print(
    "dataset sizes: train %s base %s query %s GT %s"
...
if args.indexfile and os.path.exists(args.indexfile):
    ...
    if args.get_centroids_from == "":
        ...
        if args.clustering_niter >= 0:
            print(("setting nb of clustering iterations to %d" % args.clustering_niter))
