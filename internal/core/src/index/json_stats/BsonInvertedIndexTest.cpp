...
class BsonInvertedIndexTest : public ::testing::Test {
 protected:
    void
    SetUp() override {
        old_enabled_ =
...
TEST_F(BsonInvertedIndexTest, HeapAndMmapRoutingAndReload) {
        ...
        for (const auto priority :
            ...
            for (const bool planning_enabled : {false, true}) {
                ...
                for (const bool enabled : {true, false, true}) {
                    ...
                    source_->expect_async = planning_enabled;
                    source_->reads = 0;
                    source_->header_reads = 0;
...
TEST_F(BsonInvertedIndexTest, CompatibilityEntryRemainsSynchronous) {
    source_->observe = true;
...
TEST_F(BsonInvertedIndexTest, AsyncLoadMultiplePersistedSlices) {
...
TEST_F(BsonInvertedIndexTest, AsyncNativePayloadReadSuspendsAndDrainsOnCancel) {
...
TEST_F(BsonInvertedIndexTest, AsyncLoadCancelWaitingForAdmission) {
    auto& admission = storage::LoadAdmissionController::GetInstance();
    ASSERT_TRUE(
        admission.TryAcquire({0, 1}, storage::LoadAdmissionPriority::High));
...
TEST_F(BsonInvertedIndexTest, AsyncLoadReadAndTantivyFailuresCleanStaging) {
    for (const auto [expected, corrupt_tantivy] :
         {std::pair{StorageError, false},
          std::pair{DataFormatBroken, false},
          std::pair{DataFormatBroken, true}}) {
        auto fs = NativeSource();
...
TEST_F(BsonInvertedIndexTest,
       AsyncLoadDestinationFailureKeepsOtherLoadDirectory) {
    auto manager = std::make_shared<storage::DiskFileManagerImpl>(Context());
...
TEST_F(BsonInvertedIndexTest, ResourceEstimateSurvivesConfigurationChanges) {
    for (const bool mmap : {false, true}) {
        ...
        const auto disabled_estimate = disabled.estimated_byte_size_of_cell(0);
        EXPECT_GE(disabled_estimate.second.memory_bytes,
                  DEFAULT_FIELD_MAX_MEMORY_LIMIT +
                      storage::FileWriter::MAX_BUFFER_SIZE);
        EXPECT_GT(disabled_estimate.second.memory_bytes,
                  estimate.second.memory_bytes);
