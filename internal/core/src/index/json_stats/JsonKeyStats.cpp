...
JsonKeyStats::GetColumnSchemaFromParquet(int64_t column_group_id,
    ...
    for (const auto& field : file_schema->fields()) {
        ...
        // auto result = metadata->Get(JSON_STATS_META_KEY_LAYOUT_TYPE_MAP);
        ...
        // field_layout_type_map_[field_name] = layout_type;
        ...
        LOG_INFO(
            ...
            field_name,
            field_id,
            shred_field_data_type_map_[field_name],
            segment_id_);
    }
}
...
JsonKeyStats::GetCommonMetaFromParquet(const std::string& file) {
    ...
    for (int i = 0; i < kv_metadata->size(); ++i) {
        ...
        if (key == JSON_STATS_META_SHREDDING_COLUMN_KEY_MAP) {
            try {
                ...
                ThrowInfo(ErrorCode::UnexpectedError,
                          "Failed to parse "
                          "JSON_STATS_META_SHREDDING_COLUMN_KEY_MAP from "
