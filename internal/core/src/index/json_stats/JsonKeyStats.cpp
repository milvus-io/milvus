...
EstimateJsonStatsMaterializeReservationBytes(const JsonStatsRowRange& range,
    ...
    reservation_bytes = SaturatingAdd(
        reservation_bytes,
        SaturatingMultiply(static_cast<size_t>(range.row_count),
                           kJsonStatsMaterializeRowOverheadBytes));
    reservation_bytes = SaturatingAdd(
        reservation_bytes,
        SaturatingMultiply(column_count,
...
JsonKeyStats::ParseJsonForBuildStats(
    const char* json_str, std::map<JsonKey, std::string>& values) const {
    jsmn_parser parser;
    jsmn_init(&parser);
...
JsonKeyStats::BuildKeyStatsParallel(
    ...
    auto submit_next = [&](bool block_for_budget) -> bool {
        ...
        try {
            ...
            reservation_bytes = EstimateJsonStatsMaterializeReservationBytes(
                range, static_cast<size_t>(schema->num_fields()));
        } catch (...) {
            remember_exception(std::current_exception());
            return false;
        }

        auto accounted_bytes =
...
JsonKeyStats::BuildWithFieldData(const std::vector<FieldDataPtr>& field_datas,
    ...
    auto infos = CollectKeyInfo(row_ranges, nullable);
...
JsonKeyStats::LoadColumnGroup(int64_t column_group_id,
    ...
    if (eager_load) {
        ...
        auto translator = std::make_unique<
            ...
            field_meta_map,
            column_names,
            /*full_projection=*/(*needed_columns == column_names),
            enable_mmap,
