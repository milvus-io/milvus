...
JsonKeyStats::JsonKeyStats(const storage::FileManagerContext& ctx,
    ...
    if (is_load) {
        ...
        path_ = std::string(TMP_JSON_STATS_LOG_PREFIX) + prefix;
        ...
        auto trueFs = milvus_storage::ArrowFileSystemSingleton::GetInstance()
                          .GetArrowFileSystem();
        if (!trueFs) {
            PanicInfo(ErrorCode::UnexpectedError, "Failed to get filesystem");
        }
        parquet_writer_ = std::make_shared<JsonStatsParquetWriter>(
            trueFs, conf, DEFAULT_BUFFER_SIZE, DEFAULT_BATCH_SIZE);
...
void
JsonKeyStats::TravelJsonForStats(const char* json,
                                 jsmntok* tokens,
                                 int& index,
                                 std::vector<std::string>& path,
                                 std::map<JsonKey, KeyStatsInfo>& infos) {
    //LOG_DEBUG("TravelJsonForStats: {} for segment {}", json, segment_id_);
    ...
    if (current.type == JSMN_OBJECT) {
        ...
        for (int i = 0; i < current.size; i++) {
            ...
            j++;
            int consumed = 0;
            TravelJsonForStats(json, tokens + j, consumed, path, infos);
            path.pop_back();
            j += consumed;
        }
        index = j;
...
JsonKeyStats::CollectSingleJsonStatsInfo(
    ...
    //LOG_DEBUG("TravelJsonForStats: {} for segment {}", json_str, segment_id_);
    TravelJsonForStats(json_str, tokens.data(), index, paths, infos);
}
...
JsonKeyStats::ClassifyJsonKeyLayoutType(
    ...
    auto ClassifyKey = [&](const JsonKey& key,
        ...
        } else if (hit_ratio >= dynamic_ratio_threshold_) {
    ...
    if (column_path_num > max_non_shared_path_) {
        ...
        size_t idx = 0;
        while (column_path_num > max_non_shared_path_ &&
...
void
JsonKeyStats::TravelJsonForBuildStats(const char* json,
                                      jsmntok* tokens,
                                      int& index,
                                      std::vector<std::string>& path,
                                      std::map<JsonKey, std::string>& values) {
    ...
    if (current.type == JSMN_OBJECT) {
        ...
        for (int i = 0; i < current.size; i++) {
            ...
            j++;
            int consumed = 0;
            TravelJsonForBuildStats(json, tokens + j, consumed, path, values);
            path.pop_back();
            j += consumed;
        }
        index = j;
...
JsonKeyStats::BuildKeyStatsForRow(const char* json_str, uint32_t row_id) {
    ...
    std::vector<std::string> paths;
    std::map<JsonKey, std::string> values;
    TravelJsonForBuildStats(json_str, tokens.data(), index, paths, values);
    DomNode root;
    ...
    for (const auto& data : field_datas) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            auto json_str = static_cast<const milvus::Json*>(data->RawValue(i))
                                ->data()
                                .data();
            BuildKeyStatsForRow(json_str, i);
        }
    }
}
...
JsonKeyStats::GetColumnSchemaFromParquet(int64_t column_group_id,
    ...
    for (const auto& field : file_schema->fields()) {
        ...
        // //LOG_DEBUG("layout_type: {} for field: {}", layout_type_str, field_name);
...
JsonKeyStats::GetCommonMetaFromParquet(const std::string& file) {
    ...
    for (int i = 0; i < kv_metadata->size(); ++i) {
        ...
        LOG_INFO("parquet metadata entry: {} = {} for segment {}",
                 key,
                 value,
                 segment_id_);
...
JsonKeyStats::LoadColumnGroup(int64_t column_group_id,
    ...
    for (const auto& file : files) {
        ...
        row_group_meta_list.push_back(row_group_meta_vector);
    }
