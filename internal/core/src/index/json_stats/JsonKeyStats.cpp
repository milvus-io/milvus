...
LoadJsonStatsMetaAsync(const storage::FileManagerContext& context,
        ...
        auto probe =
            co_await storage::LoadAdmissionController::GetInstance()
                .AcquireAsync({4096, 1},
                              ...
                              token);
        input = co_await storage::RemoteInputStream::OpenAsync(fs, remote_path);
        storage::ThrowIfCancelled(token, "JsonKeyStats::OpenMeta");
        size = input->Size();
    }
    ...
    for (size_t offset = 0; offset < size;) {
        ...
        co_await storage::ReadInputStreamExactlyAsync(
...
ReadJsonStatsParquetMetadataAsync(
    ...
    size_t footer_size;
    {
        ...
        auto file = co_await storage::RemoteInputStream::OpenAsync(fs, path);
        storage::ThrowIfCancelled(token, "JsonKeyStats::OpenParquet");
        file_size = file->Size();
