...
class JsonKeyStats : public ScalarIndex<std::string> {
 ...
 public:
    ...
    ExecuteForSharedData(
        ...
        std::function<bool(const BsonView& bson, const uint32_t offset)> func) {
        if (CanSkipShared(path)) {
            return;
        }

        // remove the first '/' in path, json path is like '/1/2/3'
        // and the bson inverted index key is '1/2/3'
        AssertInfo(path.front() == '/', "path must start with '/'");
        auto key = path.substr(1);

        bson_inverted_index_->TermQuery(
            key, [&](uint32_t row_id, uint32_t offset) {
                auto bson = shared_column_->RawBsonAt(row_id);
                //LOG_DEBUG("bson:{}", bson.ToString());
                bitset[row_id] = func(bson, offset);
            });
    }
    ...
    ExecuteExistsPathForSharedData(const std::string& path,
        ...
        auto pairs = bson_inverted_index_->TermQuery(
            path,
            [&](uint32_t row_id, uint32_t offset) { bitset[row_id] = true; });
    }
    ...
    ExecutorForGettingValid(const std::string& path,
        ...
        for (size_t i = 0; i < num_data_chunk; i++) {
            ...
            if (GetJsonType(path) == JSONType::STRING) {
    ...
    bool
    PathInShreddingColumn(const std::string& path) {
        return key_field_map_.find(path) != key_field_map_.end();
    }

    bool
    PathInSharedColumn(const std::string& path) {
        if (CanSkipShared(path)) {
            return false;
        }
        return bson_inverted_index_->KeyExists(path);
    }

    bool
    PathExists(const std::string& path) {
        return PathInShreddingColumn(path) || PathInSharedColumn(path);
    }

    bool
    CanSkipShared(const std::string& path) {
        if (key_field_map_.find(path) != key_field_map_.end()) {
            for (const auto& field_name : key_field_map_[path]) {
                if (field_key_type_map_[field_name] ==
                        JsonKeyLayoutType::TYPED ||
                    field_key_type_map_[field_name] ==
                        JsonKeyLayoutType::TYPED_NOT_ALL ||
                    field_key_type_map_[field_name] ==
                        JsonKeyLayoutType::DYNAMIC_ONLY) {
                    return true;
                }
            }
        }
    ...
    GetShreddingField(const std::string& pointer, JSONType type) {
        ...
        for (const auto& field : key_field_map_[pointer]) {
            if (field_data_type_map_[field] == type) {
                return field;
            }
        }
        return std::nullopt;
    ...
    GetShreddingFields(const std::string& pointer,
        ...
        for (const auto& field : key_field_map_[pointer]) {
            if (std::find(types.begin(),
                          types.end(),
                          field_data_type_map_[field]) != types.end()) {
    ...
    JSONType
    GetJsonType(const std::string& field_name) {
        return field_data_type_map_[field_name];
    }
 ...
 private:
    ...
    void
    TravelJsonForStats(const char* json,
                       jsmntok* tokens,
                       int& index,
    ...
    void
    TravelJsonForBuildStats(const char* json,
                            jsmntok* tokens,
                            int& index,
                            std::vector<std::string>& path,
                            std::map<JsonKey, std::string>& values);
 ...
 private:
    ...
    float dynamic_ratio_threshold_ = 0.3f;
    int64_t max_non_shared_path_ = 1024;
    ...
    // field_name -> data_type, such as json_path_int -> JSONType::INT64
    std::unordered_map<std::string, JSONType> field_data_type_map_;
    // field_name -> key_type, such as json_path_int -> JsonKeyLayoutType::TYPED
    std::unordered_map<std::string, JsonKeyLayoutType> field_key_type_map_;
