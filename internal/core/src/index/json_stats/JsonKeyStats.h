...
class JsonKeyStats : public ScalarIndex<std::string> {
 ...
 public:
    ...
    ExecuteForSharedData(
        ...
        bson_inverted_index_->TermQuery(
            key, [&](uint32_t row_id, uint32_t offset) {
                // TODO: RawBsonAt is quite slow, so we need to optimize it 
    ...
    CanSkipShared(const std::string& path,
        ...
        for (const auto& field_name : field_names) {
            if (field_key_type_map_[field_name] != JsonKeyLayoutType::SHARED) {
                continue;
            }
    ...
    GetShreddingFields(const std::string& pointer) {
        ...
        if (key_field_map_.find(pointer) != key_field_map_.end()) {
            for (const auto& field : key_field_map_[pointer]) {
                if (field_key_type_map_[field] != JsonKeyLayoutType::SHARED) {
                    fields.insert(field);
                }
            }
        }
 ...
 private:
    ...
    // key_name -> data_type, such as json_path_int -> JSONType::INT64, include shared keys
