...
class JsonKeyStats : public ScalarIndex<std::string> {
 ...
 public:
    ...
    ExecuteExistsPathForSharedData(const std::string& path,
        ...
        bson_inverted_index_->TermQuery(path,
                                        [&bitset](const uint32_t* row_id_array,
                                                  const uint32_t* offset_array,
                                                  const int64_t array_len) {
                                            for (int64_t i = 0; i < array_len;
                                                 i++) {
                                                bitset[row_id_array[i]] = true;
                                            }
                                        });
    }
    ...
    // find whether the path is in shared column with target data type
    bool
    ...
    void
    BulkGetBsonData(std::function<void(milvus::BsonView, size_t)> fn,
