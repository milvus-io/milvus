...
/// Process-wide byte budget for JSON stats ranges submitted for asynchronous
/// materialization but not yet consumed. Capacity 0 means unlimited. An
/// oversized range may run exclusively to guarantee progress.
class JsonStatsBuildMemoryBudget {
 public:
    static JsonStatsBuildMemoryBudget&
    GetInstance() {
        static JsonStatsBuildMemoryBudget instance;
        return instance;
    }
    ...
    Release(size_t bytes) {
        {
            ...
            AssertInfo(
                ...
                "JSON stats build memory budget over-release: release {}, "
                "inflight {}",
                bytes,
                inflight_bytes_);
            inflight_bytes_ -= bytes;
    ...
    ReconcileReservation(size_t reserved_bytes, size_t actual_bytes) {
        bool released_bytes = false;
        {
            ...
            AssertInfo(reserved_bytes <= inflight_bytes_,
                       "JSON stats build memory budget reconcile exceeds "
                       "inflight: "
                       "reserved {}, inflight {}",
                       reserved_bytes,
                       inflight_bytes_);
            ...
            AssertInfo(actual_bytes <= std::numeric_limits<size_t>::max() -
                       ...
                       "JSON stats build memory budget reconcile overflow: "
                       "actual "
                       "{}, other inflight {}",
                       actual_bytes,
                       other_inflight_bytes);
 ...
 private:
    JsonStatsBuildMemoryBudget() = default;
