...
CreateArrowBuilder(JSONType type) {
    ...
    switch (type) {
        ...
        case JSONType::STRING:
            ...
            break;
        case JSONType::ARRAY:
            // Store array as JSON string in a dedicated column
            builder = std::make_shared<arrow::StringBuilder>();
            break;
        default:
...
CreateArrowField(const JsonKey& key,
    ...
    switch (key.type_) {
        ...
        case JSONType::STRING:
            ...
            break;
        case JSONType::ARRAY:
            // Store array payload as string column
            field = arrow::field(field_name, arrow::utf8(), true, metadata);
            break;
        default:
            ThrowInfo(ErrorCode::Unsupported,
...
CreateParquetKVMetadata(std::map<JsonKey, JsonKeyLayoutType> column_map) {
    ...
    res.push_back(std::make_pair(JSON_STATS_META_SHREDDING_COLUMN_KEY_MAP,
                                 key_map.dump()));
    res.push_back(
