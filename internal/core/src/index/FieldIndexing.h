...
class FieldIndexing {
 public:
    explicit FieldIndexing(const milvus::base::FieldMeta& field_meta,
    ...
    BuildIndexRange(int64_t ack_beg,
                    int64_t ack_end,
                    const milvus::base::VectorBase* vec_base) = 0;
 ...
 protected:
    // additional info
    const milvus::base::FieldMeta& field_meta_;
    const milvus::base::SegcoreConfig& segcore_config_;
};
...
class ScalarFieldIndexing : public FieldIndexing {
 public:
    ...
    BuildIndexRange(int64_t ack_beg,
                    int64_t ack_end,
                    const milvus::base::VectorBase* vec_base) override;
...
class VectorFieldIndexing : public FieldIndexing {
 public:
    ...
    get_search_params(const milvus::base::SearchInfo& searchInfo) const;

 private:
    std::atomic<idx_t> index_cur_ = 0;
    std::atomic<bool> build;
    std::atomic<bool> sync_with_index;
    std::unique_ptr<milvus::base::VecIndexConfig> config_;
...
class IndexingRecord {
 public:
    explicit IndexingRecord(const milvus::base::Schema& schema,
                            const milvus::base::IndexMetaPtr& indexMetaPtr,
                            const milvus::base::SegcoreConfig& segcore_config)
        : schema_(schema),
    ...
    AppendingIndex(int64_t reserved_offset,
                   ...
                   const milvus::base::InsertRecord<is_sealed>& record) {
        if (is_in(fieldId)) {
 ...
 private:
    ...
    milvus::base::IndexMetaPtr index_meta_;
    const milvus::base::SegcoreConfig& segcore_config_;
 ...
 private:
    ...
    milvus::base::AckResponder finished_ack_;
    std::mutex mutex_;
