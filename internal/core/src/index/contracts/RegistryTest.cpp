...
struct LoaderProbeA : IndexLoader {
    ...
    static inline std::thread::id create_thread;
...
ExpectProjectedCapsRejected(std::string_view family, Config params) {
    const auto loader_entry =
        LoaderRegistry::Instance().Lookup(std::string(family));
    ASSERT_TRUE(loader_entry);
    ...
    try {
        static_cast<void>(loader_entry.derive_caps(params));
...
TEST(RegistryTest, LoaderDeriveAndCreateDispatchIndependently) {
    ...
    ASSERT_TRUE(loader_entry);
    EXPECT_TRUE(loader_entry.derive_caps({{"predicate", true}}).predicate);
    EXPECT_FALSE(loader_entry.derive_caps({{"predicate", false}}).predicate);
                ...
                .run =
                    [backend] {
                        ...
                        if (backend.Family() == families::kHybrid) {
                            EXPECT_FALSE(loader_entry);
                        } else {
                            EXPECT_TRUE(loader_entry);
                        }
                    },
            });
        }
