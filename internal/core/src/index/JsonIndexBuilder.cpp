...
// document and value expose the same inspection API. Keep document access for
// scalar roots: document.get_value() rejects those with SCALAR_DOCUMENT_AS_VALUE.
template <typename JsonValue>
bool
IsLegacyJsonPresent(JsonValue value) {
    ...
    switch (type.value()) {
        ...
        case simdjson::ondemand::json_type::object: {
            ...
            for (auto field : object) {
                if (IsLegacyJsonPresent<simdjson::ondemand::value>(
                        field.value())) {
                    return true;
                }
            }
...
HasLegacyJsonPathPresence(const Json& json, const std::string& nested_path) {
    ...
    if (nested_path.empty()) {
        return doc.error() == simdjson::SUCCESS &&
               IsLegacyJsonPresent<simdjson::ondemand::document>(
                   std::move(doc));
    }
    auto value = doc.at_pointer(nested_path);
    return value.error() == simdjson::SUCCESS &&
           IsLegacyJsonPresent(value.value());
}
...
ProcessJsonFieldData(
    ...
    // The per-row probe below cannot stand in for this: it maps
    // INVALID_JSON_POINTER to MISSING, so a malformed pointer would build a
    // silent all-MISSING index instead of failing here.
    validate_json_pointer(nested_path);
    ...
    for (const auto& data : field_datas) {
        ...
        for (int64_t i = 0; i < n; i++) {
            ...
            auto path_inspection = InspectJsonPathForTypedIndex(
                *json_column, nested_path, scalar_index_version);
