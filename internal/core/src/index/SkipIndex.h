...
class FieldSkipMetricsView {
 public:
    ...
    // Hot paths prepare values and bounds once per expression. String entries
    // may be string_views as long as their owner outlives the calls.
    bool
    CanSkipInQuery(int64_t chunk_id,
                   const index::PreparedInQuery& query) const {
        const auto* metrics = MetricsAt(chunk_id);
        if (auto decision = Precheck(metrics); decision.has_value()) {
            return *decision;
        }
        return metrics->CanSkipPreparedIn(query);
    }
