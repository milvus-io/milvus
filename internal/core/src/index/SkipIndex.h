...
class ChunkSkipIndexAppender : public milvus_storage::MetadataAppender {
 ...
 private:
    ...
    LoadNumMetrics(
        ...
        if (bf_cost > set_cost) {
            metrics.emplace_back(std::make_unique<SetFieldChunkMetric<T>>(info));
        } else {
            metrics.emplace_back(std::make_unique<BloomFilterFieldChunkMetric<T>>(info));
        }
        return metrics;
    }
    ...
    LoadStringMetrics(
        ...
        metrics.emplace_back(std::make_unique<MinMaxFieldChunkMetric<std::string>>(info));
        remaining_budget -=
        ...
        if (set_cost < bf_cost && set_cost < remaining_budget) {
            metrics.emplace_back(std::make_unique<SetFieldChunkMetric<std::string>>(info));
            remaining_budget -= set_cost;
        } else if (bf_cost < remaining_budget) {
            metrics.emplace_back(std::make_unique<BloomFilterFieldChunkMetric<std::string>>(info));
...
class SkipIndex {
 public:
    ...
    template <typename T>
    std::enable_if_t<
        std::is_same_v<T, std::string> || std::is_same_v<T, std::string_view>, bool>
    ...
    std::enable_if_t<SkipIndex::IsAllowedType<T>::value &&
                     ...
                     !std::is_same_v<T, std::string_view>, bool>
    CanSkipUnaryRange(FieldId field_id,
                      int64_t chunk_id,
                      OpType op_type,
                      const T& val) const {
