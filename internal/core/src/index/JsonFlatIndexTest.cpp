...
TEST_F(JsonFlatIndexExprTest, NestedExistsUsesRelativePath) {
    json_data_ = {
        R"({"a":{"b":1}})",
        R"({"a":{"b":"x"}})",
        R"({"a":{"b":null,"c":2}})",
    ...
    for (const auto& path : std::vector<std::vector<std::string>>{
        ...
        for (bool negate : {false, true}) {
            ...
            auto result =
                EvalExprInBatches(predicate, segment_.get(), json_data_.size())
                    .result;
            ...
            for (size_t i = 0; i < json_data_.size(); ++i) {
                ...
                EXPECT_EQ(bits[i], json.exist(pointer) != negate)
                    << "row " << i;
                EXPECT_TRUE(valid[i]) << "row " << i;
            }
        }
    }
}
