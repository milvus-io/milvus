...
namespace {
// Gate real serialized payload slice after Create has finished. Keep every
...
CheckControlledPackedLoad(bool cancel_first) {
    ...
    for (bool mmap : {false, true}) {
        ...
        auto loader = run(SortedIndexLoader::Create(
...
TEST(ScalarIndexV3AsyncTest, CancelledPayloadDrainsAndPreparedLoaderRetries) {
    CheckControlledPackedLoad(true);
}

TEST(ScalarIndexV3LoadTest, BitmapCreateDefersTargetsAndSupportsFreshLoads) {
    ...
    for (bool use_async : {false, true}) {
        for (bool mmap : {false, true}) {
            ...
            options.mmap_dir_path = staging->Path();
            auto loader =
                run(BitmapIndexLoader::Create(std::move(source), options));
            EXPECT_TRUE(std::filesystem::is_empty(staging->Path()));
...
TEST(ScalarIndexV3LoadTest, FailedCreateDetachesBorrowedSourceContext) {
    const auto& backend =
        ScalarReaderBackends().Get<std::string_view>("BitmapVarchar");
    ...
    IndexLoadRequest request{OpenedIndexSource{PackedIndexSource{source}},
                             options};
    const auto loader_entry =
        LoaderRegistry::Instance().Lookup(backend.Family());
    ExpectPackedLoadError(DataTypeInvalid, [&] {
        static_cast<void>(loader_entry.Load(std::move(request)));
    });
...
TEST(ScalarIndexV3LoadTest, LoaderEntryLoadPreservesOperationCancellation) {
    const auto& backend =
        ScalarReaderBackends().Get<std::string_view>("BitmapVarchar");
    ScalarTestData<std::string_view> data({"alpha", "beta"});
    ...
    options.op_ctx = &context;
    ExpectPackedLoadError(FollyCancel, [&] {
        static_cast<void>(loader_entry.Load(
