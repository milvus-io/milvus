...
TEST_F(RTreeIndexTest, GIS_CacheOn_CorruptRow_LoadsAndQueries) {
    ...
    // NOTE (2.6): sealed caches live in the process-global manager keyed by
    // (segment_instance_uid, segment_id, field_id); the segment accessor must
    // resolve to the same instance the load populated. (On master they
    // instead live in the segment's immutable published runtime state.)
