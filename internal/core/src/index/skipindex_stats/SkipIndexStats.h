...
// Immutable query-side state shared by every cell check. Owning strings remain
// owning in both values and bounds; string_views borrow the expression literals.
class PreparedInQuery {
 public:
    PreparedInQuery() = default;
    explicit PreparedInQuery(std::vector<Metrics> values);
    ...
    const std::optional<std::pair<Metrics, Metrics>>&
    Bounds() const {
        return bounds_;
    }

 private:
    std::vector<Metrics> values_;
    std::optional<std::pair<Metrics, Metrics>> bounds_;
};
...
class FieldChunkMetrics {
 public:
    ...
    // Bounds/bloom availability is independent of null state: an all-null
    // chunk may have no values but can still be pruned by SkipIndex.
    bool
    HasUsableStats() const {
        return has_value_;
    }
    ...
    // Providers without a range fast path retain their full-list predicate.
    virtual bool
    CanSkipPreparedIn(const PreparedInQuery& query) const {
        return CanSkipIn(query.Values());
    }
...
class FloatFieldChunkMetrics : public FieldChunkMetrics {
 public:
    ...
    bool
    CanSkipPreparedIn(const PreparedInQuery& query) const override {
        if (const auto& bounds = query.Bounds(); bounds.has_value()) {
            return CanSkipBinaryRange(
                bounds->first, bounds->second, true, true);
        }
        return CanSkipIn(query.Values());
    }
...
class IntFieldChunkMetrics : public FieldChunkMetrics {
 public:
    ...
    bool
    CanSkipPreparedIn(const PreparedInQuery& query) const override {
        if (const auto& bounds = query.Bounds();
            !bloom_filter_ && bounds.has_value()) {
            return CanSkipBinaryRange(
                bounds->first, bounds->second, true, true);
        }
        return CanSkipIn(query.Values());
 ...
 public:
    ...
    CanSkipIn(const std::vector<Metrics>& values) const override {
        ...
        if (!bloom_filter_) {
            ...
            // spans ["", "b"]). Borrow directly from the prepared values;
            // no temporary vector is needed. Expression callers use
            // CanSkipPreparedIn to reuse the hull across cell checks.
            auto first = ExtractStringView(values[0]);
            if (!first.has_value()) {
                return false;
            }
            std::string_view min = *first;
            std::string_view max = *first;
            for (size_t i = 1; i < values.size(); ++i) {
                const auto v = ExtractStringView(values[i]);
                if (!v.has_value()) {
                    return false;
                }
                if (*v < min) {
        ...
        for (const auto& value : values) {
            auto v = ExtractStringView(value);
            if (!v.has_value() || bloom_filter_->Test(*v)) {
                return false;
            }
        }
        return true;
