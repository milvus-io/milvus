    ...
    for (int workers : {0, 1}) {
        ...
        for (auto priority : {proto::common::LoadPriority::HIGH,
            ...
            for (bool file_target : {false, true}) {
                for (bool fail_materialize_from_artifact : {false, true}) {
                    SCOPED_TRACE(::testing::Message()
                                 << workers << "/" << priority << "/"
                                 << file_target << "/"
                                 << fail_materialize_from_artifact);
                    ...
                    if (fail_materialize_from_artifact) {
                        try {
                            loaded.LoadUnified(config);
                            FAIL()
                                << "expected artifact materialization failure";
                    ...
                    EXPECT_EQ(loaded.materialize_from_artifact_calls_, 1);
                    EXPECT_TRUE(
