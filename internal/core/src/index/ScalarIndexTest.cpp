...
TEST(ScalarTest, StringIndexSortBuildAndSearch) {
    // Test data
...
TEST(ScalarTest, StringIndexSortWithNulls) {
...
TEST(ScalarTest, StringIndexSortSerialization) {
    std::vector<std::string> test_data;
    for (int i = 0; i < 100; ++i) {
...
TEST(ScalarTest, StringIndexSortPerformance) {
    // Generate large dataset for performance testing
    const int large_n = 10000;
    std::vector<std::string> large_data;
    large_data.reserve(large_n);
    for (int i = 0; i < large_n; ++i) {
        large_data.push_back("test_string_" + std::to_string(i));
    }
    ...
    // Test Memory mode performance
    {
        ...
        index->Build(large_n, large_data.data());
        auto end = std::chrono::high_resolution_clock::now();
        auto build_time =
            std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
                .count();
        std::cout << "Memory mode build time for " << large_n
                  << " strings: " << build_time << " ms\n";

        // Test search performance
        std::vector<std::string> search_vals;
        for (int i = 0; i < 100; i += 10) {
            search_vals.push_back(large_data[i]);
        }
        ...
        auto search_time =
            std::chrono::duration_cast<std::chrono::microseconds>(end - start)
                .count();
        std::cout << "Memory mode search time: " << search_time << " us\n";

        ASSERT_EQ(bitset.count(), search_vals.size());
    }

    // Test Mmap mode performance
    {
        milvus::Config config;
        config["mmap_file_path"] = "/tmp/milvus_scalar_test";
        auto index = milvus::index::CreateStringIndexSort({});
        ...
        auto build_time =
            std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
                .count();
        std::cout << "Mmap mode build time for " << large_n
                  << " strings: " << build_time << " ms\n";

        // Clean up mmap files
        boost::filesystem::remove_all("/tmp/milvus_scalar_test");
    }
}
