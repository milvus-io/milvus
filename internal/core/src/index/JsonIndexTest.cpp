...
TEST(JsonIndexTest, TestJsonContains) {
    ...
    auto check_offsets = [&](const expr::TypedExprPtr& filter_expr) {
        auto ordinary = milvus::test::EvalExprInBatches(
                            filter_expr, segment.get(), json_raw_data.size())
                            .result;
        TargetBitmapView expected(ordinary->GetRawData(), ordinary->size());
        TargetBitmapView expected_valid(ordinary->GetValidRawData(),
                                        ordinary->size());
        auto query_context =
        ...
        auto compiled =
            exec::CompileExpressions({filter_expr}, &exec_context, {}, false);
        ASSERT_EQ(compiled.size(), 1);
        exec::EvalCtx eval_context(&exec_context);
        // Reuse one expression across empty, unordered, duplicate and oversized
        // offset batches. Neither empty input nor gathering may advance its
        // sequential cursor or consume the cached index bitmap.
        for (auto offsets : std::vector<exec::OffsetVector>{
                 {},
                 {26, 25, 14, 22, 25, 31, 33, 24, 17, 6},
                 {},
                 {27, 0, 18, 23, 28, 29, 30, 32, 26, 25}}) {
            // Logical NOT requires a child vector; empty input is checked on
            // the CONTAINS leaf, not passed into the composite executor.
            ...
            TargetBitmapView actual(column->GetRawData(), column->size());
            TargetBitmapView actual_valid(column->GetValidRawData(),
                                          column->size());
            for (size_t i = 0; i < offsets.size(); ++i) {
                EXPECT_EQ(actual[i], expected[offsets[i]])
                    << "offset " << offsets[i];
                EXPECT_EQ(actual_valid[i], expected_valid[offsets[i]])
                    << "offset " << offsets[i];
            }
        }
    };
    check_offsets(mixed_expr);
    ...
    auto check_numeric = [&](proto::plan::JSONContainsExpr_JSONOp op,
        ...
        check_offsets(numeric_expr);
        check_offsets(std::make_shared<expr::LogicalUnaryExpr>(
            expr::LogicalUnaryExpr::OpType::LogicalNot, numeric_expr));
    };
    ...
    // Both execution modes must keep the DOUBLE projection, with or without
    // raw JSON. Precise integer probes distinguish an index lookup from a raw
    // fallback: both 2^53 and 2^53+1 must match the indexed 2^53+1 query.
    proto::plan::GenericValue rounded_int_value;
    rounded_int_value.set_int64_val(9007199254740992LL);
    proto::plan::GenericValue missing_int_value;
    missing_int_value.set_int64_val(12345);
    for (bool index_only : {false, true}) {
        ...
        check_numeric(proto::plan::JSONContainsExpr_JSONOp_Contains,
                      {large_int_value},
                      {25, 26});
        check_numeric(proto::plan::JSONContainsExpr_JSONOp_ContainsAny,
                      {large_int_value, missing_int_value},
                      {25, 26});
        check_numeric(proto::plan::JSONContainsExpr_JSONOp_ContainsAll,
                      {rounded_int_value, large_int_value},
                      {25, 26});
        check_numeric(proto::plan::JSONContainsExpr_JSONOp_ContainsAll,
                      {large_int_value, missing_int_value},
                      {});
