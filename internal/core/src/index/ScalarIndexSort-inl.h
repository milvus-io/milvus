...
template <typename T>
inline bool
ScalarIndexSort<T>::outOfRange(const T value, const OpType op) {
    if (!data_.empty()) {
        auto lower = data_.begin();
        auto upper = data_.rbegin();
        bool outOfRange = false;
        switch (op) {
            case OpType::LessThan:
                outOfRange = lower->a_ >= value;
                break;
            case OpType::LessEqual:
                outOfRange = lower->a_ > value;
                break;
            case OpType::GreaterThan:
                outOfRange = upper->a_ <= value;
                break;
            case OpType::GreaterEqual:
                outOfRange = upper->a_ < value;
                break;
...
template <typename T>
inline bool
ScalarIndexSort<T>::overlap(const T lower_bound_value,
                            const T upper_bound_value) {
    if (!data_.empty()) {
        auto lower = data_.begin();
        auto upper = data_.rbegin();
        return !(upper->a_ < lower_bound_value ||
                 lower->a_ > upper_bound_value);
    }
    return false;
}
}  // namespace milvus::index
