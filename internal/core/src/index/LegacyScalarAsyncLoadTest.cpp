...
TEST_F(LegacyScalarAsyncLoadTest, CachedReloadKeepsEstimatedMode) {
    const int64_t values[]{10, 20, 30, 10, 50};
    const bool valid[]{true, false, true, true, false};
    ScalarIndexSort<int64_t> built(Context(proto::schema::DataType::Int64));
    built.Build(5, values, valid);
    const auto files = Persist(built.Serialize({}));
    for (const bool mmap : {false, true}) {
        for (const bool enabled : {false, true}) {
            ...
            info.mmap_dir_path = temporary_.get().string();
            info.index_params = {{INDEX_TYPE, ASCENDING_SORT},
                                 {SCALAR_INDEX_ENGINE_VERSION, "2"}};
            info.index_files = files;
            info.num_rows = 5;
            info.warmup_policy = "disable";
            CreateIndexInfo create{};
            create.field_type = DataType::INT64;
            create.index_type = ASCENDING_SORT;
            create.scalar_index_engine_version = 2;
            ...
            source_->on_read = [enabled](const auto&, uint64_t) {
                EXPECT_EQ(
                    folly::getCurrentThreadName().value_or("").starts_with(
                        "MILVUS_ASYNC"),
                    enabled);
            };
            for (const bool live : {!enabled, enabled, !enabled}) {
                segcore::storagev2translator::SetStorageV2AsyncLoadEnabled(
                    live);
                auto cells = translator.get_cells(nullptr, {0});
                ASSERT_EQ(cells.size(), 1);
                auto* loaded = dynamic_cast<ScalarIndex<int64_t>*>(
                    cells.front().second.get());
                ASSERT_NE(loaded, nullptr);
                CheckNullable(*loaded, int64_t{10});
...
TEST_F(LegacyScalarAsyncLoadTest,
    ...
    auto input = folly::coro::blockingWait(
        storage::OpenLegacyIndexInputAsync(source_, nullptr, *data_file));
    const auto info =
...
TEST_F(LegacyScalarAsyncLoadTest, CancellationDrainsQueuedLocalWrite) {
    ...
    auto input = folly::coro::blockingWait(
        storage::OpenLegacyIndexInputAsync(source_, nullptr, files.front()));
    const auto info =
        folly::coro::blockingWait(storage::InspectLegacyIndexFileAsync(
            *input, proto::common::LoadPriority::HIGH));
    folly::Baton<> release;
...
TEST_F(LegacyScalarAsyncLoadTest, LocalPoolShutdownDoesNotWaitForRemoteRead) {
    ...
    auto pending = std::async(std::launch::async, [&] {
        ...
        for (const auto& entry : entries) {
            std::get<storage::FileEntryTarget>(entry.target).staging->Commit();
        }
    });
    auto drain = folly::makeGuard([&] { release.post(); });
...
TEST_F(LegacyScalarAsyncLoadTest, DiskFailureRemovesCompletedAndPartialFiles) {
    ...
    auto input = folly::coro::blockingWait(
        storage::OpenLegacyIndexInputAsync(source_, nullptr, files.back()));
    const auto info =
        folly::coro::blockingWait(storage::InspectLegacyIndexFileAsync(
            *input, proto::common::LoadPriority::HIGH));
    source_->on_read = [&](const auto& path, uint64_t offset) {
