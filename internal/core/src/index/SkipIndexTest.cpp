...
TEST_F(SkipIndexTest, PreparedInQueryPreservesMetadataPrechecks) {
    const index::PreparedInQuery query({std::string_view("z")});
    auto metrics = std::make_shared<UnusableMetricsProbe>();
    auto column = MakeColumnWithMetrics(field_id_, metrics);
    auto view = FieldSkipMetricsView::FromProvider(column);
    EXPECT_FALSE(view.CanSkipInQuery(0, query));
    EXPECT_FALSE(view.CanSkipInQuery(1, query));
    EXPECT_FALSE(FieldSkipMetricsView().CanSkipInQuery(0, query));
    EXPECT_EQ(metrics->predicate_calls, 0);

    auto all_null_column =
        MakeColumnWithMetrics(field_id_, MakeAllNullMetrics());
    auto all_null_view = FieldSkipMetricsView::FromProvider(all_null_column);
    EXPECT_TRUE(all_null_view.CanSkipInQuery(0, query));
    EXPECT_TRUE(all_null_view.CanSkipInQuery(0, index::PreparedInQuery()));
}
