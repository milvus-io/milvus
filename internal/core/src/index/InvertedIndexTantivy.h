...
class InvertedIndexTantivy : public ScalarIndex<T> {
 ...
 protected:
    ...
    // an extra segment-sized copy when an executor is created. Set bit == row
    // is not null. Null means not finalized or growing; a non-null zero-sized
    // bitmap is the sealed all-valid sentinel.
