...
EncodeEmptyEmbeddingOffsets(int64_t dim, const std::vector<size_t>& offsets) {
    AssertInfo(detail::IsValidEmptyEmbeddingListDimension(dim),
               "empty embedding-list dimension must be positive");
    AssertInfo(detail::HasValidEmptyEmbeddingListBounds(offsets),
    ...
    detail::EncodeEmptyEmbeddingListPayload(
        bytes.data(), dim, wire_count, offsets);
    return bytes;
}
...
VectorDiskBuilder<T>::InputSpec() const {
    return vector_builder::DeriveInputSpec(build_params_,
                                           engine_->native_index);
}
