...
// Shared knowhere handle and metadata owned by readers and build artifacts.
// The nullable row mapping lives inside the native index's knowhere IdMap:
// builders and loaders publish the public-row validity bitmap into it, and
// knowhere derives both mapping directions.
...
class KnowhereEngine {
 public:
    ...
    // growing) path reads the caller's memory through `view_data` instead of
    // owning a copy, and has no file manager.
    KnowhereEngine(DataType physical_type,
                   DataType elem_type,
                   IndexType index_type,
                   MetricType metric_type,
    ...
    // Embedding-list offsets are shared with readers through an immutable
    // generation.
    bool
