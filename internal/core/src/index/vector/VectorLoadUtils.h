...
ParseNormalizedLoadMetadata(const Config& params, LoadBackend backend) {
    ...
    const auto field_type = ReadNullableDataTypeParam(params, "field_type");
    ...
    result.index_type = vector_params::ReadRequiredString(
        params, INDEX_TYPE, "normalized vector parameter");
    result.metric_type = vector_params::ReadRequiredString(
        params, METRIC_TYPE, "normalized vector parameter");
...
// Parse mmap flags after the disk loader validates DiskANN-only open parameters.
inline void
