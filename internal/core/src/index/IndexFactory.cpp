...
IndexBasePtr
IndexFactory::CreateScalarIndex(const CreateIndexInfo& create_index_info, storage::FileManagerImplPtr file_manager) {
...
IndexFactory::CreateVectorIndex(const CreateIndexInfo& create_index_info, storage::FileManagerImplPtr file_manager) {
    ...
    if (is_in_nm_list(index_type)) {
        return std::make_unique<VectorMemNMIndex>(index_type, metric_type, index_mode, file_manager);
    }
    // create mem index
    return std::make_unique<VectorMemIndex>(index_type, metric_type, index_mode, file_manager);
}
