...
IndexFactory::ScalarIndexLoadResource(
    ...
    if (index_type == milvus::index::ASCENDING_SORT) {
        ...
        if (mmap_enable) {
            ...
            // An ARRAY-field bitmap may be NESTED (element-level): its resident
            // validity bitset is sized by the ELEMENT count, not the row count,
            // so BitsetBytes(num_rows) under-counts it by the array fan-out --
            // leading the loader to over-admit and OOM on high-fan-out arrays.
            // The element count is not available here, but the serialized index
            // file already contains the element-sized validity bitset, so
            // index_size_in_bytes is a safe (slightly conservative) upper bound;
            // floor the resident estimate to it. (The element x 8-byte offset
            // cache under indexOffsetCacheEnabled is a load-time structure not
            // reflected in the file size -- its precise accounting needs the
            // element count and is a follow-up.)
