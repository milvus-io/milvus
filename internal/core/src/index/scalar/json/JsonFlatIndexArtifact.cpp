...
void
JsonFlatIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
JsonFlatIndexArtifact::SerializeImpl(Output& sink) const {
    const auto& directory = state_.directory;
    const auto& null_offsets = state_.null_offsets;
    AssertInfo(directory != nullptr, "JSON flat artifact state is incomplete");
    ...
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        ...
        sink.PutMeta(std::string(HAS_NULL),
                     nlohmann::json(!null_offsets.empty()));
    }
