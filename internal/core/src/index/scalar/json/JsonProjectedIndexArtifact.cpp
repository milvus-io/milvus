...
void
JsonProjectedIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
JsonProjectedIndexArtifact::SerializeImpl(Output& sink) const {
    AssertInfo(inner_ != nullptr,
    ...
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        ...
        sink.PutMeta(std::string(kHasNonExistMeta),
                     nlohmann::json(has_non_exist));
        if (has_non_exist) {
            sink.WriteEntry(std::string(INDEX_NON_EXIST_OFFSET_FILE_NAME),
    ...
    sink.WriteEntry(
        std::string(INDEX_NON_EXIST_OFFSET_FILE_NAME),
