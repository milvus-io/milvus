...
template <typename T>
void
SortedIndexArtifact<T>::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}
...
void
SortedIndexArtifact<T>::SerializeImpl(Output& sink) const {
    ...
    if constexpr (std::is_same_v<Output, storage::FileSink>) {
        ...
        sink.WriteEntry(
            std::string(sort_format::kIndexData), data_.data(), data_bytes);
        sink.WriteEntry(std::string(sort_format::kIndexLength),
                        &index_length,
                        sizeof(index_length));
        sink.WriteEntry(std::string(sort_format::kLegacyNumRows),
                        &total_num_rows_,
                        sizeof(total_num_rows_));
        sink.WriteEntry(
            std::string(sort_format::kLegacyNested), &nested_, sizeof(nested_));
        return;
    }

    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        sink.PutMeta(std::string(sort_format::kIndexLength),
                     nlohmann::json(data_.size()));
        sink.PutMeta(std::string(sort_format::kNumRows),
                     nlohmann::json(total_num_rows_));
        sink.PutMeta(std::string(sort_format::kNested),
                     nlohmann::json(nested_));
        sink.WriteEntry(
            std::string(sort_format::kIndexData), data_.data(), data_bytes);
        sink.WriteEntry(std::string(sort_format::kIdxToOffsets),
                        idx_to_offsets_.data(),
                        idx_to_offsets_.size() * sizeof(int32_t));
        sink.WriteEntry(std::string(sort_format::kValidBitset),
                        valid_bitset_.data(),
...
SortedStringIndexArtifact::SortedStringIndexArtifact(
    ...
    : layout_(SortedStringLayout::FromHeap(
      ...
      total_num_rows_(total_num_rows),
      nested_(nested) {
    AssertInfo(layout_ != nullptr, "sorted string artifact layout is null");
}
...
void
SortedStringIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
SortedStringIndexArtifact::SerializeImpl(Output& sink) const {
    ...
    if constexpr (std::is_same_v<Output, storage::FileSink>) {
        ...
        sink.WriteEntry(
            std::string(sort_format::kVersion), &version, sizeof(version));
        sink.WriteEntry(
            std::string(sort_format::kIndexData), packed.data(), packed.size());
        sink.WriteEntry(std::string(sort_format::kLegacyNumRows),
                        &total_num_rows_,
                        sizeof(total_num_rows_));
        sink.WriteEntry(std::string(sort_format::kValidBitset),
    ...
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        sink.PutMeta(std::string(sort_format::kVersion),
