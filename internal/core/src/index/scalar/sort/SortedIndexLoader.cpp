...
struct SortedIndexLoader::PreparedState {
    RuntimeParams params;
    NumericMeta meta;
};

SortedIndexLoader::SortedIndexLoader(OpenedIndexSource source,
...
SortedIndexLoader::PreparedState
SortedIndexLoader::PreparePackedState(const storage::IndexEntryDirectory&,
...
SortedIndexLoader::Create(OpenedIndexSource source,
                          storage::LoadOptions options) {
    auto loader = std::unique_ptr<SortedIndexLoader>(
        new SortedIndexLoader(std::move(source), std::move(options)));
    ...
    if (const auto* packed = std::get_if<PackedIndexSource>(&loader->source_)) {
        *state = PreparePackedState(
        ...
        if (IsStringDataType(state->params.value_type)) {
            static_cast<CommonMeta&>(state->meta) =
                co_await ReadStringMeta(legacy.use_async, *legacy.source);
        } else {
            state->meta =
                co_await ReadNumericMeta(legacy.use_async, *legacy.source);
        }
        ResolveNested(state->meta, state->params.nested);
        CheckCount(state->meta.count);
        CheckCount(state->meta.index_length);
