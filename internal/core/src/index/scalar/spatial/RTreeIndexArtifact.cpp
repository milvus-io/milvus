...
RTreeIndexArtifact::RTreeIndexArtifact(
    ...
    state_ = ArtifactState{
        std::move(directory), std::move(engine_files), std::move(null_offsets)};
}
...
void
RTreeIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
RTreeIndexArtifact::SerializeImpl(Output& sink) const {
    ...
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        ...
        sink.PutMeta(std::string(FILE_NAMES), file_names);
        sink.PutMeta(std::string(HAS_NULL), !null_offsets.empty());
    }
    for (const auto& [name, path] : local_files) {
        storage::WriteEntryFromLocalFile(sink, name, path);
    }
