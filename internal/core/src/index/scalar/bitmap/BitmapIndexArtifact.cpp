...
template <typename T>
void
BitmapIndexArtifact<T>::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}
...
void
BitmapIndexArtifact<T>::SerializeImpl(Output& sink) const {
    ...
    if constexpr (std::is_same_v<Output, storage::FileSink>) {
        ...
        sink.WriteEntry(
            std::string(BITMAP_INDEX_META), encoded.data(), encoded.size());
    } else {
        sink.PutMeta(std::string(BITMAP_INDEX_LENGTH),
                     nlohmann::json(postings.size()));
        sink.PutMeta(std::string(BITMAP_INDEX_NUM_ROWS),
                     nlohmann::json(state_.total_num_rows));
        sink.PutMeta(std::string(kV3NestedKey), nlohmann::json(state_.nested));
    }

    sink.WriteEntry(std::string(BITMAP_INDEX_DATA), data.data(), data.size());
    if (state_.nullable) {
        const auto validity =
            SerializeValidity(state_.valid_bitset, state_.total_num_rows);
        sink.WriteEntry(std::string(BITMAP_INDEX_VALID_BITSET),
                        validity.data(),
                        validity.size());
    }
}
