...
struct BitmapIndexLoader::PreparedState {
    RuntimeParams params;
    BitmapMeta meta;
    BitmapLayout layout;
};

BitmapIndexLoader::BitmapIndexLoader(OpenedIndexSource source,
...
BitmapIndexLoader::PreparedState
BitmapIndexLoader::PreparePackedState(
...
BitmapIndexLoader::Create(OpenedIndexSource source,
                          storage::LoadOptions options) {
    auto loader = std::unique_ptr<BitmapIndexLoader>(
        new BitmapIndexLoader(std::move(source), std::move(options)));
    auto state = std::make_unique<PreparedState>();
    if (const auto* packed = std::get_if<PackedIndexSource>(&loader->source_)) {
        *state = PreparePackedState(
            packed->Directory(), packed->Metadata(), loader->options_);
    } else {
        const auto& legacy = std::get<LegacyIndexSource>(loader->source_);
        ...
        if (!legacy.source->HasEntry(BITMAP_INDEX_DATA)) {
            ThrowInfo(DataFormatBroken, "bitmap artifact data is missing");
        }
    }
...
BitmapIndexLoader::Load(milvus::OpContext* context) {
    ...
    if (auto* packed = std::get_if<PackedIndexSource>(&source_)) {
    ...
    co_return co_await RunLegacyLoad(
        std::get<LegacyIndexSource>(source_),
        options_,
...
BitmapIndexLoader::PlanPacked(const storage::IndexEntryDirectory& directory,
                              const nlohmann::json& metadata,
                              const storage::LoadOptions& opts,
                              const PreparedState* cached) {
    std::optional<PreparedState> parsed;
    if (cached == nullptr) {
        parsed = PreparePackedState(directory, metadata, opts);
        cached = &*parsed;
    }
