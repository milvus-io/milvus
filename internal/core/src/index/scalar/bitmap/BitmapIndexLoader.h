 ...
 *
 * Owns the opened source and fixed load options; each Load creates a new
 * reader. Create retains parsed family metadata without allocating payload
 * targets.
 */
class BitmapIndexLoader final : public IndexLoader {
 ...
 private:
    ...
    BitmapIndexLoader(OpenedIndexSource source, storage::LoadOptions options);
     ...
     */
    static PreparedState
    PreparePackedState(const storage::IndexEntryDirectory&,
                       const nlohmann::json&,
    ...
    PlanPacked(const storage::IndexEntryDirectory& directory,
               ...
               const storage::LoadOptions& opts,
               const PreparedState* cached = nullptr);
