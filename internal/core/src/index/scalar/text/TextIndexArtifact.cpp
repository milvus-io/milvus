...
CreateTextIndexDirectory(const std::string& parent,
    ...
    const auto pattern = "text_" + SafePrefix(unique_id) + "_XXXXXX";
...
void
TextIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
TextIndexArtifact::SerializeImpl(Output& sink) const {
    // Sealed RAM interim indexes are not exported; the wrapper exposes no
    // RAM-directory serialization API.
    ...
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        std::vector<std::string> file_names;
        file_names.reserve(files.size());
        for (const auto& file : files) {
            file_names.push_back(file.filename().string());
        }
        // V3 text metadata has no family, type, or count markers of its own.
        sink.PutMeta(std::string(FILE_NAMES), nlohmann::json(file_names));
        sink.PutMeta(std::string(HAS_NULL),
                     nlohmann::json(!null_offsets_.empty()));
    }
    ...
    if (!null_offsets_.empty()) {
        ...
        sink.WriteEntry(std::string(INDEX_NULL_OFFSET),
                        null_offsets_.data(),
