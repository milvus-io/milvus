...
NormalizeAndValidate(TextIndexBuildParams& params) {
    ...
    // The wrapper can create a reader directly only from a V7 writer. Sealed
    // RAM builds therefore use V7; V5 is disk-only and reopens from its files.
...
TextIndexBuilder::AddBatch(const ScalarBuildBatch<std::string_view>& batch) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (batch.validity && !batch.validity[i]) {
            // Empty-array documents preserve every row coordinate, including
            // trailing and all-null input.
