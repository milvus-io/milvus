...
struct TextIndexBuildParams {
    ...
    // RAM builds use unique_id as the Tantivy schema field name. The registry
    // parser selects the appropriate spelling.
    std::string field_name;
    DataType value_type{DataType::VARCHAR};
    std::string analyzer_name{"milvus_tokenizer"};
    std::string analyzer_params{"{}"};
    // Additional analyzer metadata supplied by the build service.
...
class TextIndexBuilder final
 ...
 private:
    ...
    // Writer-mode Tantivy wrapper.
    std::shared_ptr<milvus::tantivy::TantivyIndexWrapper> engine_;

    // Row offsets whose value was null. Serialized as the `INDEX_NULL_OFFSET`
    // side entry.
