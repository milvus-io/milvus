...
void
NgramIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
NgramIndexArtifact::SerializeImpl(Output& sink) const {
    ...
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        ...
        // V3 Tantivy metadata omits min/max gram, type, path, and count; those
        // values remain runtime-only.
    ...
    // V1/V2 omit this sidecar and load with a 5000-byte default. V3 writes one
    // native size_t.
    if constexpr (std::is_same_v<Output, storage::IndexEntryWriter>) {
        sink.WriteEntry(std::string(kAvgRowSizeEntry),
                        &avg_row_size_,
                        sizeof(avg_row_size_));
    }
}
