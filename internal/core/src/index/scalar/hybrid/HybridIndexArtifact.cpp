...
void
HybridIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    SerializeImpl(sink);
}

template <typename Output>
void
HybridIndexArtifact::SerializeImpl(Output& sink) const {
    inner_->Serialize(sink);
    const auto encoded = static_cast<uint8_t>(selector_);
    if constexpr (std::is_same_v<Output, storage::FileSink>) {
        sink.WriteEntry(std::string(INDEX_TYPE), &encoded, sizeof(encoded));
    } else {
        sink.PutMeta(std::string(INDEX_TYPE), nlohmann::json(encoded));
    }
}
