...
FmIndexLoader::Create(OpenedIndexSource source, storage::LoadOptions options) {
    (void)DeriveCaps(options.params);
    if (!std::holds_alternative<PackedIndexSource>(source)) {
        ThrowInfo(DataFormatBroken,
                  "FM-index has no V1/V2 artifact representation");
    }
    auto loader = std::unique_ptr<FmIndexLoader>(new FmIndexLoader(
        std::get<PackedIndexSource>(std::move(source)), std::move(options)));
    PackedFmState metadata;
    ValidatePackedMetadata(metadata,
                           loader->source_.Directory(),
                           loader->source_.Metadata(),
                           loader->options_);
    co_return loader;
}
...
FmIndexLoader::Load(milvus::OpContext* context) {
    return RunPackedIndexLoad(
        source_, options_, &PlanPacked, &FinishPacked, context);
}
