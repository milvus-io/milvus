...
void
FmIndexArtifact::Serialize(storage::FileSink&) const {
    ThrowInfo(UnexpectedError, "FM-index has no V1/V2 artifact representation");
}

void
FmIndexArtifact::Serialize(storage::IndexEntryWriter& sink) const {
    ...
    storage::WriteEntryFromLocalFile(sink, kBlobEntry, blob_file.Path());

    if (storage_->Nullable()) {
        const auto packed =
            PackNullBitmap(storage_->NullBitmap(), storage_->Count());
        sink.WriteEntry(
            std::string(kNullBitmapEntry), packed.data(), packed.size());
    }
    sink.PutMeta(std::string(kTotalRowsMeta),
                 nlohmann::json(storage_->Count()));
    sink.PutMeta(std::string(kNullableMeta),
                 nlohmann::json(storage_->Nullable()));
}
