...
JsonKeyInvertedIndex::TravelJson(const char* json,
    ...
    if (current.type == JSMN_OBJECT) {
        ...
        if (has_escape_sequence(value)) {
            AddInvertedRecord(path,
                              0,
                              uint8_t(JSONType::STRING_ESCAPE),
                              offset,
                              current.start - 1,
