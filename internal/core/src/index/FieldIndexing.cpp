...
#include "base/IndexConfigGenerator.h"
...
VectorFieldIndexing::VectorFieldIndexing(
    ...
    const milvus::base::FieldIndexMeta& field_index_meta,
    int64_t segment_max_row_count,
    const milvus::base::SegcoreConfig& segcore_config)
    : FieldIndexing(field_meta, segcore_config),
      build(false),
      sync_with_index(false),
      config_(std::make_unique<milvus::base::VecIndexConfig>(
...
void
VectorFieldIndexing::AppendSegmentIndex(
...
milvus::base::SearchInfo
VectorFieldIndexing::get_search_params(
    const milvus::base::SearchInfo& searchInfo) const {
...
template <typename T>
void
ScalarFieldIndexing<T>::BuildIndexRange(
    int64_t ack_beg,
    int64_t ack_end,
    const milvus::base::VectorBase* vec_base) {
    auto source =
        dynamic_cast<const milvus::base::ConcurrentVector<T>*>(vec_base);
    AssertInfo(source, "vec_base can't cast to ConcurrentVector type");
    auto num_chunk = source->num_chunk();
    AssertInfo(ack_end <= num_chunk, "Ack_end is bigger than num_chunk");
    data_.grow_to_at_least(ack_end);
...
CreateIndex(const milvus::base::FieldMeta& field_meta,
            const milvus::base::FieldIndexMeta& field_index_meta,
            int64_t segment_max_row_count,
            const milvus::base::SegcoreConfig& segcore_config) {
