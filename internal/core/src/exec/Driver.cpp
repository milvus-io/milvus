...
//   - folly::FutureTimeout reaching the driver directly ->
//     ErrorCode::FollyOtherException (retriable). Only a timeout is treated as
//     retriable; other folly::FutureException subtypes (FutureNotReady,
//     BrokenPromise, ...) are programming errors and deliberately fall through
//     to the std::exception arm below as UnexpectedError, not retried.
...
#define CALL_OPERATOR(call_func, operator, method_name)                        \
    try {                                                                      \
        ...
        auto err_msg = fmt::format(                                            \
            "Operator::{} timed out for [Operator:{}, plan node id: {}] : {}", \
            method_name,                                                       \
            operator->ToString(),                                              \
            operator->get_plannode_id(),                                       \
            e.what());                                                         \
