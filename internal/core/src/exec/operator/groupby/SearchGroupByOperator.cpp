...
SearchGroupBy(const std::vector<std::shared_ptr<VectorIterator>>& iterators,
    ...
    switch (data_type) {
        case DataType::INT8: {
            auto dataGetter =
                GetDataGetter<int8_t, int8_t>(segment, group_by_field_id);
        ...
        case DataType::INT16: {
            auto dataGetter =
                GetDataGetter<int16_t, int16_t>(segment, group_by_field_id);
        ...
        case DataType::INT32: {
            auto dataGetter =
                GetDataGetter<int32_t, int32_t>(segment, group_by_field_id);
        ...
        case DataType::INT64: {
            auto dataGetter =
                GetDataGetter<int64_t, int64_t>(segment, group_by_field_id);
        ...
        case DataType::BOOL: {
            auto dataGetter =
                GetDataGetter<bool, bool>(segment, group_by_field_id);
        ...
        case DataType::VARCHAR: {
            auto dataGetter = GetDataGetter<std::string, std::string>(
                segment, group_by_field_id);
        ...
        case DataType::JSON: {
            AssertInfo(search_info.json_path_.has_value(),
                       ...
                       "search_group_by");
            AssertInfo(search_info.json_cast_type_.has_value(),
                       "json_cast_type is required for json field when doing "
                       "search_group_by");
            LOG_INFO("hc===SearchGroupBy=json_cast_type: {}, json_path: {}", search_info.json_cast_type_.value(), search_info.json_path_.value());
            switch (search_info.json_cast_type_.value()) {
                case DataType::BOOL: {
                    auto data_getter = GetDataGetter<bool, milvus::Json>(
                        segment, group_by_field_id, search_info.json_path_);
                    GroupIteratorsByType<bool>(iterators,
                                               search_info.topk_,
                                               search_info.group_size_,
                                               search_info.strict_group_size_,
                ...
                case DataType::VARCHAR: {
                    LOG_INFO("hc===SearchGroupBy=DataType::VARCHAR, json_path: {}", search_info.json_path_.value());
                ...
                default: {
                    PanicInfo(Unsupported,
                              "unsupported json cast type for search group by");
                }
            }
            break;
        }
...
GroupIteratorResult(const std::shared_ptr<VectorIterator>& iterator,
    ...
    while (iterator->HasNext() && !groupMap.IsGroupResEnough()) {
        ...
        //LOG_INFO("hc===GroupIteratorResult=offset: {}, dis: {}", offset, dis);
        //std::cout << "hc===GroupIteratorResult=before Get, offset: " << offset << std::endl;
        std::optional<T> row_data = data_getter->Get(offset);
        //std::cout << "hc===GroupIteratorResult=after Get, offset: " << offset << std::endl;
        //LOG_INFO("hc===GroupIteratorResult=row_data: {}", row_data.has_value() ? "has_value" : "null");
