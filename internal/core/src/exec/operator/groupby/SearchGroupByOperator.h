...
#define JSON_TYPE_CASE(OutputType, TargetType, JSON_INNER_TYPE, CastExpr)     \
    if constexpr (std::is_same_v<OutputType, TargetType>) {                   \
        ...
        AssertInfo(result.error() == simdjson::SUCCESS,                       \
...
#define JSON_STRING_CASE(OutputType)                                       \
    if constexpr (std::is_same_v<OutputType, std::string>) {               \
        if (this->json_cast_type_.has_value()) {                           \
            auto str_result =                                              \
                json_val.at<std::string_view>(this->json_path_.value());   \
            AssertInfo(str_result.error() == simdjson::SUCCESS,            \
...
#define JSON_TYPE_CASES(OutputType) \
    ...
    JSON_TYPE_CASE(OutputType, int8_t, int64_t, static_cast<int8_t>(result.value())) \
    JSON_TYPE_CASE(OutputType, int16_t, int64_t, static_cast<int16_t>(result.value())) \
    JSON_TYPE_CASE(OutputType, int32_t, int64_t, static_cast<int32_t>(result.value())) \
    JSON_TYPE_CASE(OutputType, int64_t, int64_t, static_cast<int64_t>(result.value()))
...
class DataGetter {
 ...
 protected:
    ...
    std::optional<DataType> json_cast_type_;
};
...
class GrowingDataGetter : public DataGetter<OutputType> {
 public:
    GrowingDataGetter(const segcore::SegmentGrowingImpl& segment,
                      ...
                      std::optional<DataType> json_cast_type) {
        growing_raw_data_ =
        ...
        this->json_cast_type_ = json_cast_type;
    }
    ...
    GrowingDataGetter(const GrowingDataGetter<OutputType, InnerRawType>& other)
        ...
        this->json_cast_type_ = other.json_cast_type_;
    }
...
GetDataGetter(const segcore::SegmentInternalInterface& segment,
              ...
              std::optional<DataType> json_cast_type = std::nullopt) {
    ...
    if (const auto* growing_segment =
        ...
        return std::make_shared<GrowingDataGetter<OutputType, InnerRawType>>(
            *growing_segment, fieldId, json_path, json_cast_type);
        ...
        return std::make_shared<SealedDataGetter<OutputType, InnerRawType>>(
            *sealed_segment, fieldId, json_path, json_cast_type);
    } else {
