        ...
        if constexpr (std::is_same_v<InnerRawType, std::string>) {
            ...
            auto parse_json_doc =
                ...
                if constexpr (std::is_same_v<OutputType, bool>) {
                    ...
                    auto result =
                        json_val.at<int64_t>(this->json_path_.value());
                    if (result.error() != simdjson::SUCCESS)
                        return std::nullopt;
                    ...
                    LOG_INFO("hc===GrowingDataGetter=int64_t, json_path: {}",
                             this->json_path_.value());
                    auto result =
                        json_val.at<int64_t>(this->json_path_.value());
                    if (result.error() != simdjson::SUCCESS)
                        return std::nullopt;
                    LOG_INFO("hc===GrowingDataGetter=result.value(): {}",
                             result.value());
                    return result.value();
                } else if constexpr (std::is_same_v<OutputType, std::string>) {
                    LOG_INFO(
                        "hc===GrowingDataGetter=std::string, json_path: {}",
                        this->json_path_.value());
                    auto str_result =
                        json_val.at<std::string_view>(this->json_path_.value());
                    if (str_result.error() != simdjson::SUCCESS)
                        return std::nullopt;
                    LOG_INFO("hc===GrowingDataGetter=str_result.value(): {}",
                             str_result.value());
 ...
 public:
    ...
    Get(int64_t idx) const {
        ...
        if (from_data_) {
            ...
            LOG_INFO(
                "hc===SealedDataGetter=Get, chunk_id: {}, inner_offset: {}",
                chunk_id,
                inner_offset);
            if constexpr (std::is_same_v<InnerRawType, std::string>) {
                ...
                LOG_INFO(
                    "hc===SealedDataGetter=Get, milvus::Json, json_path: {}",
                    this->json_path_.value());
                ...
                if constexpr (std::is_same_v<OutputType, bool>) {
                    ...
                    if (result.error() != simdjson::SUCCESS)
                        return std::nullopt;
                    LOG_INFO("hc===SealedDataGetter=result.value(): {}",
                             result.value());
                    return result.value();
                } else if constexpr (std::is_same_v<OutputType, std::string>) {
                    LOG_INFO("hc===SealedDataGetter=std::string, json_path: {}",
                             this->json_path_.value());
                    auto str_result =
                        json_val.at<std::string_view>(this->json_path_.value());
                    if (str_result.error() != simdjson::SUCCESS) {
                    ...
                    LOG_INFO("hc===SealedDataGetter=str_result.value(): {}",
                             str_result.value());
                    return std::string(str_result.value());
                }
                return std::nullopt;
...
GetDataGetter(const segcore::SegmentInternalInterface& segment,
    ...
    if (const auto* growing_segment =
        ...
        if (json_path.has_value()) {
            LOG_INFO(
                "hc===Converted DataGetter=GrowingDataGetter, json_path: {}",
                json_path.value());
        }
        ...
        if (json_path.has_value()) {
            LOG_INFO(
                "hc===Converted DataGetter=SealedDataGetter, json_path: {}",
                json_path.value());
        }
