...
// For index-backed membership when a field has no raw data, group keys come
// from `PinnedValueLookup` / `IScalarValueReader<T>`, the same source used by
// SealedDataGetter in phase one. Indexed and raw scans therefore produce the
// same group keys, including NULL rows (Lookup/Gather report a NULL row as
// invalid, which maps to `GroupKey<T>(std::nullopt)`).
//
