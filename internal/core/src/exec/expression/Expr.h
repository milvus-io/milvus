...
ApplyValidMask(const bool* valid_data,
    ...
    const bool valid_res_aliases_res = res.data() == valid_res.data() &&
...
class SegmentExpr : public Expr {
 public:
    ...
    EnsureDataScanCursor(ChunkedColumnInterface::ScanValueKind value_kind) {
        ...
        // the async PrefetchRawData path has not already warmed them.
    ...
    ApplyScanValidity(const ChunkedColumnInterface::ScanBatch& batch,
        ...
        // Delegate to the word-wise mask helper. batch.validity is already a
        // batch-relative subview, so the batch position only shifts it further.
        // ApplyValidMask handles expanded and packed validity in 64-row blocks
        // and skips the redundant second write when res and valid_res alias.
        // Batch sizes are bounded by the expression window, so the narrowing
        // to the helper's int size is safe.
        ApplyValidMask(batch.validity.Subview(batch_pos),
                       res,
                       valid_res,
                ...
                for (size_t j = 0; j < offsets.size(); j++) {
                    for (int32_t t = 0; t < run_lengths[j]; ++t, ++result_idx) {
                        if (chunk_active) {
                            ...
                            bool is_valid = valid_data.empty() || valid_data[j];
    ...
    ProcessDataChunksByScan(
        ...
        if (skip_source.has_value()) {
            ...
            process_planned_cell(ChunkedColumnInterface::PlannedCellRange{
                -1, window_start, real_batch_size, false});
        }
