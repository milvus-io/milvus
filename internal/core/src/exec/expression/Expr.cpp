...
ReorderConjunctExpr(std::shared_ptr<milvus::exec::PhyConjunctFilterExpr>& expr,
    ...
    // 5. Other expressions
    // 6. JSON column expressions (expensive to evaluate)
    // 7. Heavy conjunct expressions (conjunctions with heavy operations)
    // 8. Compare filter expressions (most expensive, comparing two columns)
