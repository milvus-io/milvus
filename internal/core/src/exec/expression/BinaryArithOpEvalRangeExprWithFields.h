...
struct ArithOpTwoFieldsElementFunc {
    ...
    operator()(const T* left,
               const U* right,
               const bool* left_valid_data,
               const bool* right_valid_data,
               size_t size,
               ResultType val,
        ...
        for (int i = 0; i < size; ++i) {
            if (!bitmap_input.empty() && !bitmap_input[i + bitmap_offset]) {
                continue;
            }
            auto offset = (offsets != nullptr) ? offsets[i] : i;
            if ((left_valid_data != nullptr && !left_valid_data[offset]) ||
                (right_valid_data != nullptr && !right_valid_data[offset])) {
            ...
            if constexpr (arith_op == proto::plan::ArithOpType::Add) {
                ...
                arith_result =
                    static_cast<ResultType>(left[offset]) / right_val;
...
class PhyBinaryArithOpEvalRangeExprWithFields : public Expr {
 public:
    PhyBinaryArithOpEvalRangeExprWithFields(
        ...
        const std::shared_ptr<
            const milvus::expr::BinaryArithOpEvalRangeExprWithFields>& expr,
 ...
 private:
    ...
    void
    MoveCursorBy(int64_t size);

    VectorPtr
    EvalForBothDataSegment(EvalCtx& context);

    template <typename T, typename U>
    VectorPtr
    ExecArithOpEvalRangeForTwoFields(EvalCtx& context);
    ...
    template <typename T,
              ...
              proto::plan::ArithOpType arith_op>
    VectorPtr
    ExecArithOpEvalRangeForTwoFieldsImpl(EvalCtx& context);
