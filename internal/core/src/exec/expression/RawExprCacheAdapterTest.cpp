...
TEST_F(RawExprCacheAdapterTest,
    ...
    ExprResCacheManager::Key hit_key{segment_->get_segment_id(), kHitSignature};
    ...
    // A lookup that cannot reserve is not treated as a capturable miss. This
    // prevents a second full pair from being accumulated under pressure.
...
TEST_F(RawExprCacheAdapterTest, RawPrefetchContributesToLatencyAdmission) {
    ...
    ExprResCacheManager::Key fast_key{segment_->get_segment_id(),
                                      "raw:prefetch-latency-fast"};
    ExprResCacheManager::Value cached;
    cached.active_count = kActiveCount;
    ...
    ExprResCacheManager::Key slow_key{segment_->get_segment_id(),
                                      "raw:prefetch-latency-slow"};
    cached.active_count = kActiveCount;
...
TEST_F(RawExprCacheAdapterTest, GrowingAdmissionSkipsCaptureForNewSnapshots) {
    ...
    auto append_rows = [&](int64_t rows) {
        auto data = segcore::DataGen(schema_, rows, 86, growing->get_row_count());
    ...
    ExprResCacheManager::Key key{growing->get_segment_id(), signature};
    ExprResCacheManager::Value cached;
...
TEST(ExprResCacheAdmissionTicketTest,
    ...
    EXPECT_FALSE(manager.ObserveMiss(key, 128).admitted);
    auto ticket = manager.ObserveMiss(key, 128);
...
TEST(ExprResCacheAdmissionTicketTest, RejectsMismatchedAndStaleTickets) {
    ...
    auto mismatch_ticket = manager.ObserveMiss(original, 64);
    ...
    auto snapshot_ticket = manager.ObserveMiss(original, 128);
    ...
    auto stale_ticket = manager.ObserveMiss(original, 64);
    ...
    auto toggled_ticket = manager.ObserveMiss(original, 64);
...
TEST(ExprResCacheAdmissionTicketTest,
    ...
    auto ticket = manager.ObserveMiss(key, 64);
    ASSERT_TRUE(ticket.admitted);
