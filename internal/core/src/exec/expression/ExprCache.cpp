...
uint64_t
AdmissionKeyHash(const ExprResCacheManager::Key& key) {
    // Frequency admission must use the same logical identity as the cache
    // entry. In particular, one query fanning the same expression out across
    // many segments must not make every per-segment entry look recurrent.
    return XXH64(key.signature.data(),
                 key.signature.size(),
...
ExprResCacheManager::ObserveMiss(const Key& key) {
    ...
    ticket.key_hash = AdmissionKeyHash(key);
...
ExprResCacheManager::PutInternal(const Key& key,
    ...
    if (ticket != nullptr) {
        const auto key_hash = AdmissionKeyHash(key);
        if (!ticket->admitted ||
            ticket->config_epoch !=
    ...
    if (config_.mode == CacheMode::Memory) {
        ...
        if (ticket == nullptr &&
            !frequency_tracker_.RecordAndCheck(AdmissionKeyHash(key),
