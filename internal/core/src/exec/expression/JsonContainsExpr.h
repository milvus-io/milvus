...
class PhyJsonContainsFilterExpr : public SegmentExpr {
 public:
    PhyJsonContainsFilterExpr(
        ...
        : SegmentExpr(std::move(input),
                      ...
                      plan_options),
          expr_(expr),
          has_array_candidates_(std::any_of(
              expr->vals_.begin(), expr->vals_.end(), [](const auto& value) {
                  return value.has_array_val();
              })) {
    }
    ...
    // Typed projections cannot preserve mixed types or nested array literals.
    // These shapes can still use stats; otherwise they run on raw JSON.
    bool
    JsonOperandShapeIsIndexable() const {
        return expr_->same_type_ && !has_array_candidates_;
    }
    ...
    DetermineExecPath() override {
        ...
        // semantics instead of declining the index. An INT64 Path index answers
        // with exact integer semantics. JsonFlatIndex keeps an exact integer
 ...
 private:
    ...
    template <typename ExprValueType>
    VectorPtr
    ExecArrayContainsForIndexSegmentImpl(const OffsetVector* input);

 private:
    std::shared_ptr<const milvus::expr::JsonContainsExpr> expr_;
    const bool has_array_candidates_;
