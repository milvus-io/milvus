...
TEST_P(ExprTest, TestMolFunction) {
    using namespace milvus;
    using namespace milvus::query;
    using namespace milvus::segcore;

    // Create schema with MOL field
    auto schema = std::make_shared<Schema>();
    auto int_fid = schema->AddDebugField("int", DataType::INT64);
    auto vec_fid = schema->AddDebugField(
        "fakevec", DataType::VECTOR_FLOAT, 16, knowhere::metric::L2);
    ...
    struct MolTestcase {
        std::string smiles;
        proto::plan::MolFunctionFilterExpr_MolOp op;
    };

    std::vector<MolTestcase> testcases = {
        {"CCO", proto::plan::MolFunctionFilterExpr_MolOp_Substructure},
        {"c1ccccc1", proto::plan::MolFunctionFilterExpr_MolOp_Substructure},
        {"C", proto::plan::MolFunctionFilterExpr_MolOp_Substructure},
        {"CCO", proto::plan::MolFunctionFilterExpr_MolOp_Superstructure},
        {"CC(=O)Oc1ccccc1C(=O)O",
         proto::plan::MolFunctionFilterExpr_MolOp_Superstructure},
    };

    for (const auto& testcase : testcases) {
        auto mol_expr = std::make_shared<milvus::expr::MolFunctionFilterExpr>(
            milvus::expr::ColumnInfo(mol_fid, DataType::MOL),
            testcase.op,
            testcase.smiles);
...
TEST(ExprTest, TestMolFunctionSealed) {
    auto schema = std::make_shared<Schema>();
    auto pk_fid = schema->AddDebugField("pk", DataType::INT64);
    auto mol_fid = schema->AddDebugField("mol", DataType::MOL);
    schema->set_primary_field_id(pk_fid);
    ...
    std::vector<
        std::pair<proto::plan::MolFunctionFilterExpr_MolOp, std::string>>
        test_cases = {
            {proto::plan::MolFunctionFilterExpr_MolOp_Substructure, "CCO"},
            {proto::plan::MolFunctionFilterExpr_MolOp_Substructure,
             "c1ccccc1"},
            {proto::plan::MolFunctionFilterExpr_MolOp_Superstructure, "CCO"},
            {proto::plan::MolFunctionFilterExpr_MolOp_Superstructure,
             "CC(=O)Oc1ccccc1C(=O)O"},
        };

    for (const auto& [op, smiles] : test_cases) {
        auto mol_expr = std::make_shared<expr::MolFunctionFilterExpr>(
            expr::ColumnInfo(mol_fid, DataType::MOL), op, smiles);
        auto plan_node = std::make_shared<plan::FilterBitsNode>(
            DEFAULT_PLANNODE_ID, mol_expr);
...
TEST_P(ExprTest, TestMolFunctionInvalidSmiles) {
    using namespace milvus;
    using namespace milvus::query;
    using namespace milvus::segcore;
    ...
    auto seg = CreateGrowingSegment(schema, empty_index_meta);
    const int64_t N = 8;
    auto raw_data = DataGen(schema, N, 0);
    seg->PreInsert(N);
    seg->Insert(0, N, raw_data.row_ids_.data(), raw_data.timestamps_.data(),
                raw_data.raw_);
...
// PLACEHOLDER_MOL_CONTROLLED
TEST_P(ExprTest, TestMolFunctionWithControlledData) {
    using namespace milvus;
    using namespace milvus::query;
    using namespace milvus::segcore;
    ...
    // Replace MOL data with controlled test data
    milvus::proto::schema::FieldData* mol_field_data = nullptr;
    for (auto& fd : *raw_data.raw_->mutable_fields_data()) {
        if (fd.field_id() == mol_fid.get()) {
            mol_field_data = &fd;
            break;
        }
    }
    assert(mol_field_data != nullptr);
    mol_field_data->mutable_scalars()->mutable_mol_data()->clear_data();

    // Known SMILES for controlled testing:
    // i%4==0: ethanol (CCO) — contains C, O substructures
    // i%4==1: benzene (c1ccccc1) — aromatic ring
    // i%4==2: aspirin (CC(=O)Oc1ccccc1C(=O)O) — contains benzene + CCO substructures
    // i%4==3: methane (C) — simplest molecule
    const char* smiles_list[] = {"CCO", "c1ccccc1",
                                  "CC(=O)Oc1ccccc1C(=O)O", "C"};
    for (int i = 0; i < N; ++i) {
        const char* smiles = smiles_list[i % 4];
        auto result = ConvertSMILESToPickle(smiles);
        assert(result.error_code == MOL_SUCCESS);
        std::string pickle(reinterpret_cast<const char*>(result.data),
                           result.size);
        FreeMolDataResult(&result);
        mol_field_data->mutable_scalars()->mutable_mol_data()->add_data(
            pickle);
    }

    seg->PreInsert(N);
    seg->Insert(0, N, raw_data.row_ids_.data(),
                raw_data.timestamps_.data(), raw_data.raw_);

    auto seg_promote = dynamic_cast<SegmentGrowingImpl*>(seg.get());

    // Test 1: Substructure query with "c1ccccc1" (benzene)
    // Should match: benzene (i%4==1) and aspirin (i%4==2) which contains benzene ring
    {
        auto mol_expr =
            std::make_shared<milvus::expr::MolFunctionFilterExpr>(
                milvus::expr::ColumnInfo(mol_fid, DataType::MOL),
                proto::plan::MolFunctionFilterExpr_MolOp_Substructure,
                "c1ccccc1");
        ...
        for (int i = 0; i < N; ++i) {
            if (final_result[i]) {
                match_count++;
                // Should be benzene (i%4==1) or aspirin (i%4==2)
                EXPECT_TRUE(i % 4 == 1 || i % 4 == 2)
                    << "Unexpected match at index " << i;
            }
        }
        EXPECT_EQ(match_count, N / 2);  // 25 benzene + 25 aspirin
    }

    // Test 2: Substructure query with "C" (single carbon)
    // Should match all molecules since they all contain carbon
    {
        auto mol_expr =
            std::make_shared<milvus::expr::MolFunctionFilterExpr>(
                milvus::expr::ColumnInfo(mol_fid, DataType::MOL),
        ...
        EXPECT_EQ(match_count, N);  // All molecules contain carbon
    }

    // Test 3: Superstructure query with "CCO" (ethanol)
    // Should match: methane (i%4==3, C is substructure of CCO)
    // and ethanol itself (i%4==0, CCO is substructure of CCO)
    {
        auto mol_expr =
            std::make_shared<milvus::expr::MolFunctionFilterExpr>(
                milvus::expr::ColumnInfo(mol_fid, DataType::MOL),
        ...
        for (int i = 0; i < N; ++i) {
            if (i % 4 == 0) {
                // ethanol: CCO is substructure of CCO (self-match)
                EXPECT_TRUE(final_result[i])
                    << "Ethanol should match superstructure CCO at index "
                    << i;
            }
            if (i % 4 == 3) {
                // methane: C is substructure of CCO
                EXPECT_TRUE(final_result[i])
                    << "Methane should match superstructure CCO at index "
                    << i;
            }
        }
    }
