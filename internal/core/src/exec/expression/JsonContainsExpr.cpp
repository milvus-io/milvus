        ...
        case DataType::ARRAY: {
            if (exec_path_ == ExprExecPath::ScalarIndex && !has_offset_input_) {
                result = EvalArrayContainsForIndexSegment(
                    expr_->column_.element_type_, input);
            } else {
...
PhyJsonContainsFilterExpr::ExecJsonContainsAllWithDiffType(EvalCtx& context) {
    ...
    auto execute_sub_batch =
        [
            ...
            &bitmap_input,
            &match_tracker,
            has_array_candidates = has_array_candidates_
...
PhyJsonContainsFilterExpr::ExecJsonContainsWithDiffType(EvalCtx& context) {
    ...
    auto execute_sub_batch =
        ...
        auto executor = [&](const size_t i) {
            ...
            for (auto&& it : array) {
                if (JsonValueMatchesAnyCandidate(it,
                                                 elements,
                                                 *number_membership_matcher,
                                                 has_array_candidates)) {
                    return std::make_pair(true, true);
...
PhyJsonContainsFilterExpr::EvalArrayContainsForIndexSegment(
    DataType data_type, const OffsetVector* input) {
    switch (data_type) {
        case DataType::BOOL: {
            return ExecArrayContainsForIndexSegmentImpl<bool>(input);
        }
        case DataType::INT8: {
            return ExecArrayContainsForIndexSegmentImpl<int8_t>(input);
        }
        case DataType::INT16: {
            return ExecArrayContainsForIndexSegmentImpl<int16_t>(input);
        }
        case DataType::INT32: {
            return ExecArrayContainsForIndexSegmentImpl<int32_t>(input);
        }
        case DataType::INT64: {
            return ExecArrayContainsForIndexSegmentImpl<int64_t>(input);
        }
        case DataType::FLOAT: {
            return ExecArrayContainsForIndexSegmentImpl<float>(input);
        }
        case DataType::DOUBLE: {
            return ExecArrayContainsForIndexSegmentImpl<double>(input);
        }
        case DataType::VARCHAR:
        case DataType::STRING: {
            return ExecArrayContainsForIndexSegmentImpl<std::string>(input);
        }
        default:
            ThrowInfo(UnexpectedError,
...
PhyJsonContainsFilterExpr::ExecArrayContainsForIndexSegmentImpl(
    ...
    // The query already returns row-level bitmaps. Cache them once, then
    // gather requested offsets or slice the next sequential batch. Both modes
    // must retain the selected JSON index's projection and validity.
    auto validity_mode = field_type_ == DataType::JSON
                             ? IndexValidityMode::JsonExactPath
                             : IndexValidityMode::Default;
    auto res = ProcessIndexChunksImpl<GetType>(execute_sub_batch,
                                               /*func_returns_row_level=*/true,
                                               validity_mode,
                                               input,
                                               elems);
