...
// Regression tests for the array/JSON element MATCH_* feature (PR #50669),
...
//   P1-(2): whole-array `arr == [...]` must fall back to brute force when the
//          array index is nested (its offsets are element, not row, offsets).
...
//   P2-(6): JSON element Term with a value > INT64_MAX must not UB and stay correct.
...
// P1-(2): whole-array equality `scores == [..]` is a ROW-level op that dispatches
// to ExecArrayEqualForIndex, which treats the index's offsets as ROW offsets. A
// nested array index stores ELEMENT offsets, so it must fall back to brute force.
// Cross-check the nested-indexed segment against the no-index segment.
...
// P2-(6): a JSON array ELEMENT whose value is a double outside int64 range must
// not trigger UB in the `static_cast<int64_t>(e.f64)` membership path. Such an
// element simply cannot equal any int64 term-set value, so its row does not
// match. Row1 below holds 1e19 (> INT64_MAX): the guard skips the cast and the
// row is correctly excluded. Uses the JSON element Term path (ExecTermJsonElement).
TEST(ScalarArrayIndexRegression, JsonElementTermHugeElementNoUB) {
    auto schema = std::make_shared<Schema>();
    ...
    const std::vector<std::string> json_rows = {
        R"({"arr": [1, 2, 3]})",
        R"({"arr": [1e19]})",  // double element far above INT64_MAX
        R"({"arr": [20, 99]})",
        R"({"arr": [-1e19]})",  // double element far below INT64_MIN
    };
    ...
    auto json_array = CreateDataArrayFrom(
        json_rows.data(), nullptr, N, schema->operator[](json_fid));
    insert->mutable_fields_data()->AddAllocated(json_array.release());
    insert->set_num_rows(N);

    auto seg = MakeSealed(schema, std::move(insert));

    // No crash / UB. Only row0 (has 1) and row2 (has 20) match; the out-of-range
    // double elements (rows 1 and 3) are guarded and match nothing.
    auto rows = RetrieveRows(
        seg.get(), *schema, schema, R"(MATCH_ANY(json["arr"], $ in [1, 20]))");
    EXPECT_EQ(rows, (std::set<int64_t>{0, 2}));
}
