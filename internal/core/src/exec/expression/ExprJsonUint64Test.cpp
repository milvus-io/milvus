...
TEST(JsonNumericTest, Uint64OutsideInt64DomainDoesNotMatchInt64Min) {
    ...
    auto comparison = CompareJsonNumberToBound(parsed.value(), int64_min);
    ASSERT_TRUE(comparison.has_value());
    EXPECT_GT(*comparison, 0);
}

TEST(JsonNumericTest, ParsedNumberComparisonPreservesNumericDomains) {
    struct TestCase {
        const char* json;
        double bound;
        int comparison;
    ...
    const std::vector<TestCase> cases = {
        {R"({"k":9007199254740993})", 0x1p53, 1},
        {R"({"k":9223372036854775807})", 0x1p63, -1},
        {R"({"k":9223372036854775809})", 0x1p63, 0},
        {R"({"k":18446744073709551615})", 0x1p64, 0},
        {R"({"k":1.5})", 1.5, 0},
    };
    for (const auto& test : cases) {
        SCOPED_TRACE(test.json);
        auto json = makeJson(test.json);
        auto parsed = json.at_numeric("/k");
        ASSERT_FALSE(parsed.error());
        proto::plan::GenericValue bound;
        bound.set_float_val(test.bound);
        auto comparison = CompareJsonNumberToBound(parsed.value(), bound);
        ASSERT_TRUE(comparison.has_value());
        EXPECT_EQ(*comparison, test.comparison);
    }
}
...
TEST(JsonNumericTest, TermComparisonWithAtNumeric) {
    auto check_in_terms =
        ...
        auto comparison = CompareJsonNumberToBound(x_num.value(), bound);
