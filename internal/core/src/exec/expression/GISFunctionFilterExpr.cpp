...
PhyGISFunctionFilterExpr::EvalForIndexSegment() {
    ...
    auto evaluate_geometry_prepared = [this,
        ...
        // `left` can be a cache-owned geometry whose stored context is shared
        // across concurrent queries, so evaluate the non-prepared fallbacks
        // (Equals/DWithin) on the per-thread context captured above rather
        // than left's own context.
