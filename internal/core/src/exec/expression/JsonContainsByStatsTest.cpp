...
class JsonInvertedIndexWithSelectablePresence
 ...
 public:
    ...
    void
    BuildWithScalarIndexVersion(const std::vector<FieldDataPtr>& field_datas,
                                int32_t scalar_index_version) {
        this->BuildInvertedWithJsonFieldData(field_datas, scalar_index_version);
    }
};
...
MakeCountedJsonStatsSlot(const BuiltJsonStatsIndex& built_index,
    ...
    *info->mutable_schema() = field_meta.field_schema;
    ...
    info->set_json_stats_data_format(built_index.json_stats_data_format);
...
TEST(JsonStatsInvalidNumberTest,
    ...
    auto make_stats_segment = [&](std::unique_ptr<JsonKeyStats> index,
                                  int64_t segment_id) {
        ...
        sealed->SetJsonStatsForTesting(json_fid,
                                       MakeTestJsonStatsSlot(std::move(index)));
    ...
    auto stats_segment = make_stats_segment(std::move(stats), 5282801);
    auto shared_stats_segment =
        make_stats_segment(std::move(shared_stats), 5282802);
...
TEST(JsonContainsNumericParityTest,
    ...
    auto cm = milvus::storage::RemoteChunkManagerSingleton::GetInstance()
                  .GetRemoteChunkManager();
    auto make_stats_segment = [&](std::unique_ptr<JsonKeyStats> stats,
                                  int64_t segment_id) {
        auto segment = segcore::CreateSealedSegment(
            schema, milvus::empty_index_meta, segment_id);
        auto* sealed =
    ...
    auto shredded_segment =
        make_stats_segment(std::move(shredded_stats), 5282811);
    auto shared_segment = make_stats_segment(std::move(shared_stats), 5282812);
    auto raw_segment =
    ...
    auto double_term = std::make_shared<expr::TermFilterExpr>(
        ...
        false);
    // JSON IN is normalized into one term per concrete numeric type.
    for (bool integer_first : {true, false}) {
        ...
        check_filter(
            split_in, expected_rows({13, 14, 15}), expected_rows({13, 14, 15}));
        check_filter(std::make_shared<expr::LogicalUnaryExpr>(
...
TEST(JsonStatsLazyInitializationTest, ConcurrentFirstUseSharesInitialization) {
    for (auto format : {JSON_STATS_DATA_FORMAT_V3, JSON_STATS_DATA_FORMAT_V4}) {
        ...
        auto schema = std::make_shared<Schema>();
        auto json_fid = schema->AddDebugField("json", DataType::JSON);
        auto built_index = BuildJsonStatsIndex(
            {R"({"a": ""})", R"({"a": "x"})", R"({"a": ""})"},
            json_fid,
            TestLocalPath,
            1403,
        ...
        EXPECT_EQ(results.front()->GetDataFormatVersion(), format);
        for (const auto& result : results) {
            EXPECT_EQ(result.get(), results.front().get());
        }
        EXPECT_EQ(counted.translator->LoadCount(), 1);

        auto field = results.front()->GetShreddingField("/a", JSONType::STRING);
        ASSERT_FALSE(field.empty());
        TargetBitmap valid(3, true);
        results.front()->ExecutorForGettingValid(
            nullptr, field, TargetBitmapView(valid));
        const bool empty_string_is_valid = format == JSON_STATS_DATA_FORMAT_V4;
        EXPECT_EQ(valid[0], empty_string_is_valid);
        EXPECT_TRUE(valid[1]);
        EXPECT_EQ(valid[2], empty_string_is_valid);
    }
}
...
TEST(JsonStatsBinaryRangeTest, ShreddingMatchesRawData) {
    ...
    auto stats_load_info = PrepareSingleFieldInsertBinlog(
        0, 0, 0, json_fid.get(), {make_json_field()}, cm);
    stats_segment->LoadFieldData(stats_load_info);
    ASSERT_TRUE(stats_segment->HasFieldData(json_fid));
    auto raw_load_info = PrepareSingleFieldInsertBinlog(
        0, 0, 0, json_fid.get(), {make_json_field()}, cm);
    raw_segment->LoadFieldData(raw_load_info);
...
TEST(JsonCrossPathContractTest, RawStatsPathAndFlatAgreeUnlessDocumented) {
    ...
    auto make_index = [&](const std::vector<std::string>& values,
        ...
        if (cast_type == "JSON") {
            ...
            auto* typed =
                static_cast<index::JsonInvertedIndex<int64_t>*>(index.get());
            typed->BuildWithFieldData({field});
            typed->finish();
            typed->create_reader(index::SetBitsetSealed);
        } else {
    ...
    auto make_double_path_index_with_presence =
        [&](const std::vector<std::string>& values,
            ...
            auto field = make_field(values);
            index->BuildWithScalarIndexVersion({field}, scalar_index_version);
            index->finish();
    ...
    // Object key lookup must keep the raw JSON spelling. In particular,
    // "n" and "\u006e" must not collapse into one stats path, in either order.
    const std::vector<std::string> escaped_key_docs = {
        R"({"\u006e":7})",
        R"({"n":8,"\u006e":7})",
        R"({"\u006e":7,"n":8})",
        R"({"n":7})",
        R"({"p":{"n":8},"\u0070":{"\u006e":7}})",
        R"({"a/b":8,"a\/b":7,"a~b":8,"a\u007eb":7})",
        R"({"中":8,"\u4e2d":7,"a\"b":7,"a\\b":8})",
        R"({})",
    };
    auto escaped_raw = make_raw(escaped_key_docs, 52829200);
    int64_t escaped_stats_id = 52829210;
    for (auto format : {JSON_STATS_DATA_FORMAT_V3, JSON_STATS_DATA_FORMAT_V4}) {
        for (int64_t max_columns : {1024, 0}) {
            auto escaped_stats = make_stats(
                escaped_key_docs, escaped_stats_id, format, true, max_columns);
            escaped_stats_id += 10;
            ASSERT_FALSE(escaped_stats->HasFieldData(json_fid));
            auto key_stats = escaped_stats->GetJsonStats(nullptr, json_fid);
            for (const auto* pointer : {"/n", R"(/\u006e)"}) {
                EXPECT_EQ(key_stats->GetShreddingFields(pointer).empty(),
                          max_columns == 0);
            }
            ...
            for (const auto& path : paths) {
                const auto name = fmt::format("escaped-key/V{}/{}/{}",
                                              format,
                                              max_columns,
                                              Json::pointer(path));
                auto column = expr::ColumnInfo(json_fid, DataType::JSON, path);
                auto equal = std::make_shared<expr::UnaryRangeFilterExpr>(
                    ...
                    i64(7),
                    std::vector<proto::plan::GenericValue>());
                const std::vector<expr::TypedExprPtr> filters = {
                    equal,
                    std::make_shared<expr::LogicalUnaryExpr>(
                        expr::LogicalUnaryExpr::OpType::LogicalNot, equal),
                    std::make_shared<expr::ExistsExpr>(column),
                };
                for (const auto& filter : filters) {
                    expect_same_filter_result(
                        name,
                        evaluate_filter(
                            filter, escaped_raw.get(), escaped_key_docs.size()),
                        evaluate_filter(filter,
                                        escaped_stats.get(),
                                        escaped_key_docs.size()));
                }
            }
        }
    }
    ...
    auto integer_term = std::make_shared<expr::TermFilterExpr>(
        number, std::vector<proto::plan::GenericValue>{i64(2)}, false);
    auto double_term = std::make_shared<expr::TermFilterExpr>(
        number, std::vector<proto::plan::GenericValue>{f64(3.5)}, false);
    const FilterResult numeric_in{
        {true, true, true, false, false, false, false, false},
        {true, true, true, true, true, true, true, false}};
    const FilterResult numeric_not_in{
        {false, false, false, true, true, true, true, false}, numeric_in.valid};
    auto integer_path = make_index(docs, 52828985, "/n", "INT64");
    for (bool integer_first : {true, false}) {
        SCOPED_TRACE(integer_first ? "integer-first" : "double-first");
        auto split_in = std::make_shared<expr::LogicalBinaryExpr>(
            expr::LogicalBinaryExpr::OpType::Or,
            integer_first ? integer_term : double_term,
            integer_first ? double_term : integer_term);
        auto split_not_in = std::make_shared<expr::LogicalUnaryExpr>(
            expr::LogicalUnaryExpr::OpType::LogicalNot, split_in);
        expect_same_filter_result(
            "mixed-numeric-in/raw",
            numeric_in,
            evaluate_filter(split_in, raw.get(), docs.size()));
        expect_same_filter_result(
            "mixed-numeric-not-in/raw",
            numeric_not_in,
            evaluate_filter(split_not_in, raw.get(), docs.size()));
        expect_cross_path_agreement(
            "mixed-numeric-in", split_in, scalar_path.get(), true, true);
        expect_cross_path_agreement("mixed-numeric-not-in",
                                    split_not_in,
                                    scalar_path.get(),
                                    true,
                                    true);

        // INT64 covers only the integer branch. A matching float resolves
        // UNKNOWN OR true (row 2), but the uint64 rows remain UNKNOWN when
        // the float branch is false. NOT preserves that validity.
        for (int layout = 0; layout < 3; ++layout) {
            SCOPED_TRACE(layout);
            auto* sealed = dynamic_cast<segcore::ChunkedSegmentSealedImpl*>(
                integer_path.get());
            ASSERT_NE(sealed, nullptr);
            std::shared_ptr<cachinglayer::CacheSlot<JsonKeyStats>> stats_slot;
            if (layout != 0) {
                auto* stats_source =
                    dynamic_cast<segcore::ChunkedSegmentSealedImpl*>(
                        (layout == 1 ? stats : shared_stats).get());
                ASSERT_NE(stats_source, nullptr);
                stats_slot = stats_source->TestGetPublishedStateSnapshot()
                                 ->runtime->json_stats.at(json_fid);
            }
            sealed->SetJsonStatsForTesting(json_fid, stats_slot);
            EXPECT_TRUE(uses_scalar_index(
                integer_term, integer_path.get(), docs.size()));
            EXPECT_FALSE(uses_scalar_index(
                double_term, integer_path.get(), docs.size()));
            EXPECT_EQ(milvus::test::CanExprExecuteAllAtOnce(
                          split_in, integer_path.get(), docs.size()),
                      layout != 0);
            const std::vector<bool> split_valid = {
                true, true, true, true, false, false, true, false};
            expect_same_filter_result(
                "mixed-numeric-in/int64-path",
                FilterResult{numeric_in.matches, split_valid},
                evaluate_filter(split_in, integer_path.get(), docs.size()));
            expect_same_filter_result(
                "mixed-numeric-not-in/int64-path",
                FilterResult{
                    {false, false, false, true, false, false, true, false},
                    split_valid},
                evaluate_filter(split_not_in, integer_path.get(), docs.size()));
        }
    }

    // Bypassing normalization must still reject a malformed mixed TermExpr
    // before any raw, stats, typed Path or Flat executor consumes its values.
    for (auto values :
         {std::vector<proto::plan::GenericValue>{i64(2), f64(3.5)},
          std::vector<proto::plan::GenericValue>{f64(3.5), i64(2)}}) {
        auto malformed =
            std::make_shared<expr::TermFilterExpr>(number, values, false);
        for (const auto* segment : {raw.get(),
                                    stats.get(),
                                    shared_stats.get(),
                                    scalar_path.get(),
                                    integer_path.get(),
                                    flat.get()}) {
            try {
                milvus::test::CanExprExecuteAllAtOnce(
                    malformed, segment, docs.size());
                FAIL() << "mixed numeric TermExpr must be rejected";
    ...
    auto cast_stats = BuildAndLoadJsonKeyStats(docs,
                                               ...
                                               52829184,
                                               1);
    auto cast_stats_slot = MakeTestJsonStatsSlot(std::move(cast_stats));
    ...
    cast_path_sealed->SetJsonStatsForTesting(json_fid, cast_stats_slot);
    ...
    typed_path_sealed->SetJsonStatsForTesting(json_fid, cast_stats_slot);
    ...
    flat_with_stats_sealed->SetJsonStatsForTesting(json_fid, cast_stats_slot);
    EXPECT_FALSE(
    ...
    mixed_path_with_stats_sealed->SetJsonStatsForTesting(json_fid,
                                                         cast_stats_slot);
    ...
    mixed_path_with_stats_and_raw_sealed->SetJsonStatsForTesting(
        json_fid, cast_stats_slot);
    ...
    const auto assert_exists_path_priority =
        [&](const std::string& name,
            ...
            auto segment = make_double_path_index_with_presence(
                docs, id, "/a", scalar_index_version);
            auto* sealed =
                dynamic_cast<segcore::ChunkedSegmentSealedImpl*>(segment.get());
            ASSERT_NE(sealed, nullptr);
            sealed->SetJsonStatsForTesting(json_fid, cast_stats_slot);
            segment->DropFieldData(json_fid);
