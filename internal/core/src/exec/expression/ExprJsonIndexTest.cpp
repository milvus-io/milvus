...
TEST(JsonIndexTest, JsonBinaryRangePathIndexMatchesRawData) {
    ...
    // Iterative-filter candidate offsets intentionally use resident raw JSON,
    // even when ordinary execution selects the Path index.
    ...
    exec::OffsetVector precise_offsets = {8, 1, 8};
    expect_same(
        evaluate(precise_expr, raw_segment.get(), &precise_offsets),
        evaluate(precise_expr, number_index_segment.get(), &precise_offsets));
}

TEST(JsonIndexTest, JsonBinaryRangeFlatIndexOffsetUsesRawJson) {
    auto schema = std::make_shared<Schema>();
    auto json_fid = schema->AddDebugField("json", DataType::JSON, true);
    ...
    auto flat_raw_load_info = PrepareSingleFieldInsertBinlog(
        1, 1, 2, json_fid.get(), {json_field}, cm);
    flat_segment->LoadFieldData(flat_raw_load_info);
    ASSERT_TRUE(flat_segment->HasFieldData(json_fid));
