...
DecorateOne(const ExprPtr& expr,
    ...
    const auto* segment = segment_expr->GetSegment();
    if (segment == nullptr ||
    ...
    std::call_once(lookup_once_, [this]() {
        ...
        try {
            if (!ExprResCacheManager::IsEnabled() ||
                ...
                input_->GetActiveCount() <= 0) {
                return;
            }

            const auto* segment = input_->GetSegment();
            auto& manager = ExprResCacheManager::Instance();
            if (segment == nullptr ||
                !manager.CanCacheSegment(segment->type()) ||
                input_->GetExecPath() != ExprExecPath::RawData) {
                return;
            }

            state_.key.emplace(ExprResCacheManager::Key{
                segment->get_segment_id(),
                input_->GetSignatureForRawExprCache()});
            ExprResCacheManager::Value cached;
            cached.active_count = input_->GetActiveCount();
            const auto lookup = manager.GetWithStatus(*state_.key, cached);
            if (lookup == ExprResCacheManager::LookupResult::Hit &&
                cached.result != nullptr && cached.valid_result != nullptr &&
                cached.result->size() ==
            ...
            if (lookup == ExprResCacheManager::LookupResult::ResourceLimit) {
                // Cache materialization is optional. Run the wrapped evaluator
                // batch by batch without turning pressure into a cache miss and
                // allocating another full-segment capture pair.
                state_.lookup_state = LookupState::Ineligible;
                return;
            }
...
RawExprCacheAdapter::TryServeCacheHit(EvalCtx& context, VectorPtr& result) {
    ...
    if (offsets != nullptr) {
        ...
        try {
            result = input_->GatherCachedResultByOffsets(
                *state_.result, *state_.valid_result, *offsets);
            return true;
        } catch (...) {
            abandon_hit();
            return false;
    ...
    const auto active_count = input_->GetActiveCount();
    const auto actual_rows =
        std::min(input_->GetNextBatchSize(),
...
RawExprCacheAdapter::EnsureAdmission(EvalCtx& context) {
    ...
    try {
        ...
        const auto active_count = input_->GetActiveCount();
        state_.admission_ticket =
            manager.ObserveMiss(*state_.key, active_count, segment->type());
        ...
        auto capture_lease = manager.TryAcquireMaterialization(active_count);
        if (!capture_lease.has_value()) {
            AbortCapture();
            return;
        }
        state_.capture_lease.emplace(std::move(*capture_lease));
...
RawExprCacheAdapter::CaptureResult(EvalCtx& context,
    ...
    if (result == nullptr) {
        if (!has_offset_input &&
            state_.sequential_pos < input_->GetActiveCount()) {
            AbortCapture();
        }
        return;
    }
...
RawExprCacheAdapter::TryPutCapturedResult() {
    ...
    try {
        ...
        value.eval_duration_us =
            state_.prefetch_duration_us + state_.eval_duration_us;
    ...
    // Release any buffers that were not moved because allocation or cache
    // insertion threw before returning the materialization reservation.
    state_.captured_result = TargetBitmap{};
    state_.captured_valid_result = TargetBitmap{};
    state_.capture_lease.reset();
}
...
RawExprCacheAdapter::MoveCursor() {
    if (!has_offset_input_) {
        const auto rows = input_->GetNextBatchSize();
        if (rows > 0) {
            const auto active_count = input_->GetActiveCount();
...
RawExprCacheAdapter::PrefetchAsync(
    ...
    prefetch_future_.emplace(folly::via(prefetch_pool.get(), [self]() {
        ...
        if (self->state_.lookup_state != LookupState::Hit) {
            const bool measure_prefetch =
                self->state_.lookup_state == LookupState::Miss;
            // Measure where the work executes: WaitPrefetch() observes only
            // whatever wait remains when the Driver eventually joins it.
            const auto prefetch_start = std::chrono::steady_clock::now();
            self->input_->PrefetchOnCurrentThread();
            if (measure_prefetch) {
                self->state_.prefetch_duration_us = std::max<int64_t>(
                    std::chrono::duration_cast<std::chrono::microseconds>(
                        std::chrono::steady_clock::now() - prefetch_start)
                        .count(),
                    1);
            }
        }
...
DecorateRawExprCache(std::vector<ExprPtr>& exprs,
                     ...
                     bool enable_cache_write) {
    for (auto& expr : exprs) {
        ExprPtr decorated = expr;
        RunExprCacheBestEffort([&]() {
            decorated = DecorateOne(expr, op_ctx, enable_cache_write);
        });
        expr = std::move(decorated);
    }
}
