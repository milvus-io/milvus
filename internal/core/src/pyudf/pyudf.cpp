...
// Descriptor shapes and ownership are internal bridge contracts, not user
// request validation. Violations are explicit internal errors.

size_t
CheckedSlotCount(int32_t outer_count, int32_t inner_count) {
    if (outer_count < 0 || inner_count < 0) {
        ThrowInfo(ErrorCode::UnexpectedError,
                  "py_udf: dimensions cannot be negative");
    }
    ...
    if (inner != 0 && outer > std::numeric_limits<size_t>::max() / inner) {
        ThrowInfo(ErrorCode::UnexpectedError,
                  "py_udf: descriptor slot count overflow");
    }
    return outer * inner;
}
...
PyUDFInvocation::PyUDFInvocation(int32_t num_inputs,
    ...
    if (num_chunks > 0 && chunk_sizes == nullptr) {
        ThrowInfo(ErrorCode::UnexpectedError, "py_udf: chunk sizes are nil");
    }
...
PyUDFResult::PyUDFResult(int32_t num_outputs, const int32_t* num_chunks) {
    if (num_outputs < 0) {
        ThrowInfo(ErrorCode::UnexpectedError,
                  "py_udf: output count cannot be negative");
    }
    if (num_outputs > 0 && num_chunks == nullptr) {
        ThrowInfo(ErrorCode::UnexpectedError,
                  "py_udf: output chunk counts are nil");
    }
