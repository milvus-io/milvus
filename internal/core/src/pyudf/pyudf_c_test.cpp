...
TEST(PyUDFHandlesCTest, InvocationRejectsInvalidArguments) {
    ...
    EXPECT_EQ(StatusCode(status), milvus::UnexpectedError);
...
TEST(PyUDFHandlesCTest, DescriptorContractViolationsCarryNativeErrorCode) {
    auto expect_internal_error = [](auto construct) {
        try {
            construct();
            FAIL() << "expected a descriptor contract failure";
        } catch (const milvus::SegcoreError& error) {
            EXPECT_EQ(error.get_error_code(), milvus::UnexpectedError);
    ...
    expect_internal_error(
        [] { milvus::pyudf::PyUDFInvocation(-1, 0, nullptr); });
    expect_internal_error(
        [] { milvus::pyudf::PyUDFInvocation(1, -1, nullptr); });
    expect_internal_error(
        [] { milvus::pyudf::PyUDFInvocation(1, 1, nullptr); });
    expect_internal_error([] {
        int64_t sizes[] = {-1};
        milvus::pyudf::PyUDFInvocation(1, 1, sizes);
    });
    expect_internal_error([] {
        int64_t sizes[] = {1};
        milvus::pyudf::PyUDFInvocation invocation(1, 1, sizes);
        invocation.ValidatePopulated();
    });
    expect_internal_error([] { milvus::pyudf::PyUDFResult(-1, nullptr); });
    expect_internal_error([] { milvus::pyudf::PyUDFResult(1, nullptr); });
    expect_internal_error([] {
        int32_t counts[] = {-1};
        milvus::pyudf::PyUDFResult(1, counts);
    });
}

TEST(PyUDFResourceRunCTest, FunctionFailureSurvivesGenericCStatusConversion) {
    try {
        throw milvus::pyudf::PyUDFFunctionError("injected function failure");
    } catch (const std::exception& error) {
        auto status = milvus::FailureCStatus(&error);
        EXPECT_EQ(StatusCode(status), PyUDFErrorCodeFunctionFailed);
        EXPECT_STREQ(StatusMessage(status), "injected function failure");
        FreeStatus(&status);
    }
}
