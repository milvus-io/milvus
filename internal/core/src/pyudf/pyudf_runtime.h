...
// Carry the existing function-pipeline code at the throw site so generic
// FailureCStatus conversion also preserves it.
class PyUDFFunctionError : public SegcoreError {
 public:
    explicit PyUDFFunctionError(const std::string& message)
        : SegcoreError(static_cast<ErrorCode>(PyUDFErrorCodeFunctionFailed),
                       message) {
    }
};
