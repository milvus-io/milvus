...
bool
HasWheelExtension(const std::filesystem::path& path) {
    ...
    return extension == ".whl";
}

constexpr int kMaxParamNestingDepth = 64;
// Protobuf recursion counts the map-entry, FunctionParamValue, and
// object/array wrapper messages in addition to the logical parameter depth.
// Keep the wire parser bounded while allowing the documented logical limit,
// plus one level so ParamObjectToPython can report the precise depth error.
constexpr int kRunParamsProtobufRecursionLimit = kMaxParamNestingDepth * 4 + 16;
...
OwnedPyObject
AdoptRequiredPyObject(PyObject* new_reference, const char* operation) {
    auto object = OwnedPyObject::Adopt(new_reference);
    if (!object) {
        ThrowPythonFailure(operation);
    }
    return object;
}
...
class RuntimeState {
 public:
    ...
    PyObject*
    load_instance() const {
        return load_instance_;
    }

    PyObject*
    close_instance() const {
        return close_instance_;
    }
 ...
 private:
    ...
    ImportTrustedWrapper() {
        ...
        auto loader = OwnedPyObject::Adopt(
            PyObject_GetAttrString(module.get(), "load_instance"));
        ...
        auto closer = OwnedPyObject::Adopt(
            PyObject_GetAttrString(module.get(), "close_instance"));
        ...
        load_instance_ = loader.release();
        close_instance_ = closer.release();
    ...
    PyObject* load_instance_ = nullptr;
    PyObject* close_instance_ = nullptr;
...
ParamObjectToPython(const milvus::proto::schema::FunctionParamObject& object,
    ...
    auto result = AdoptRequiredPyObject(PyDict_New(),
                                        "cannot allocate Python params object");
    for (const auto& [key, value] : object.fields()) {
        auto python_key = AdoptRequiredPyObject(
            PyUnicode_FromStringAndSize(key.data(),
                                        static_cast<Py_ssize_t>(key.size())),
            "cannot build Python params object key");
        auto python_value = ParamValueToPython(value, depth + 1);
        if (!python_value) {
            ThrowPythonFailure("cannot build Python params object value");
        }
        if (PyDict_SetItem(
                result.get(), python_key.get(), python_value.get()) != 0) {
...
ParamValueToPython(const milvus::proto::schema::FunctionParamValue& value,
    ...
    switch (value.value_case()) {
        ...
        case ParamValue::kInt64Value:
            return AdoptRequiredPyObject(
                PyLong_FromLongLong(value.int64_value()),
                "cannot build Python int parameter");
        case ParamValue::kDoubleValue:
            return AdoptRequiredPyObject(
                PyFloat_FromDouble(value.double_value()),
                "cannot build Python double parameter");
        case ParamValue::kStringValue:
            return AdoptRequiredPyObject(
                PyUnicode_FromStringAndSize(value.string_value().data(),
                                            value.string_value().size()),
                "cannot build Python string parameter");
        case ParamValue::kBytesValue:
            return AdoptRequiredPyObject(
                PyBytes_FromStringAndSize(value.bytes_value().data(),
                                          value.bytes_value().size()),
                "cannot build Python bytes parameter");
        case ParamValue::kArrayValue: {
            auto result = AdoptRequiredPyObject(
                PyTuple_New(value.array_value().values_size()),
                "cannot allocate Python params array");
            for (int index = 0; index < value.array_value().values_size();
                 ++index) {
                auto item = ParamValueToPython(
                    value.array_value().values(index), depth + 1);
                if (!item) {
                    ThrowPythonFailure("cannot build Python params array item");
                }
        ...
        case ParamValue::kObjectValue:
            return ParamObjectToPython(value.object_value(), depth);
...
ParseRunParams(const uint8_t* serialized_params,
    ...
    google::protobuf::io::ArrayInputStream array_stream(
        serialized_params, static_cast<int>(serialized_params_len));
    google::protobuf::io::CodedInputStream input_stream(&array_stream);
    input_stream.SetRecursionLimit(kRunParamsProtobufRecursionLimit);
    if (!params.ParseFromCodedStream(&input_stream) ||
        !input_stream.ConsumedEntireMessage()) {
        ThrowInfo(ErrorCode::UnexpectedError,
...
class CPythonPyUDFResource final : public PyUDFResource {
 public:
    // Concurrent Run calls intentionally reach the same Python instance. The
    // Go cache lease prevents Close from racing with active runs; UDF-owned
    // mutable state is the UDF author's synchronization responsibility.
    CPythonPyUDFResource(PyObject* instance,
    ...
    Close() override {
        ...
        if (instance_ != nullptr) {
            auto result = OwnedPyObject::Adopt(PyObject_CallFunctionObjArgs(
                Runtime().close_instance(), instance_, nullptr));
...
ValidateRequest(const uint8_t* serialized_request,
    ...
    if (!HasWheelExtension(local_path) ||
...
LoadResource(const uint8_t* serialized_request,
    ...
    auto resource_name = OwnedPyObject::Adopt(PyUnicode_FromStringAndSize(
        request.resource_name().data(),
        static_cast<Py_ssize_t>(request.resource_name().size())));
    auto local_path = OwnedPyObject::Adopt(PyUnicode_FromStringAndSize(
        request.local_path().data(),
        static_cast<Py_ssize_t>(request.local_path().size())));
    auto stage = OwnedPyObject::Adopt(PyUnicode_FromStringAndSize(
        request.stage().data(),
        static_cast<Py_ssize_t>(request.stage().size())));
    if (resource_name.get() == nullptr || local_path.get() == nullptr ||
        stage.get() == nullptr) {
        ThrowPythonFailure("cannot build Python load arguments");
    }
    auto instance = OwnedPyObject::Adopt(
        PyObject_CallFunctionObjArgs(Runtime().load_instance(),
                                     resource_name.get(),
                                     local_path.get(),
                                     stage.get(),
    ...
    return std::make_unique<CPythonPyUDFResource>(
        instance.release(), request.resource_name(), request.stage());
}
