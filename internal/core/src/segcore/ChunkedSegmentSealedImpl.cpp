...
ChunkedSegmentSealedImpl::PublishStateOnline(
    ...
    // Still inside the publish lease: readers are drained and none can start
    // until publish_lease is released at the end of this scope. Anything that
    // must be atomic with the swap w.r.t. reader admission runs here.
    if (while_locked) {
        while_locked();
    }
}
...
ChunkedSegmentSealedImpl::ReopenSchemaLocked(milvus::OpContext* op_ctx,
    ...
    committer.Publish(current, delta, op_ctx, publish_mode, [this, &diff] {
...
ChunkedSegmentSealedImpl::Reopen(
    ...
    delta.published_index_has_raw_data = staged->published_index_has_raw_data;
    committer.Publish(
        current, delta, op_ctx, PublishMode::Drain, [this, &diff] {
            InvalidateExprCacheAfterReopen(diff);
        });
...
void
ChunkedSegmentSealedImpl::InvalidateExprCacheAfterReopen(
    const LoadDiff& diff) const {
    if (!diff.ReplacesExistingFieldData()) {
        return;
    }
    // Erase unconditionally — do NOT gate on ExprResCacheManager::IsEnabled().
    // SetEnabled(false) only flips the switch; it leaves existing entries in
    // place. If the cache is disabled across a data-replacing reopen and later
    // re-enabled, a stale {segment_id, predicate, row_count} entry would resurface
    // for the new data. EraseSegment is safe on an empty / unconfigured cache.
    auto erased =
        milvus::exec::ExprResCacheManager::Instance().EraseSegment(id_);
    if (erased > 0) {
        LOG_INFO(
            "Reopen replaced field data for segment {}, invalidated {} cached "
            "filter bitset(s)",
            id_,
            erased);
    }
