...
ChunkedSegmentSealedImpl::ArrowToDataArray(
    ...
    switch (field_meta.get_data_type()) {
        ...
        case DataType::ARRAY: {
            ...
            // set_element_type so the client can dispatch the correct
            // ScalarField oneof decoder (IntData/LongData/StringData/...).
            // Slow path sets this via SegmentInterface::FillTargetEntry
            // after bulk_subscript; take() fast path bypasses that.
            obj->set_element_type(static_cast<proto::schema::DataType>(
