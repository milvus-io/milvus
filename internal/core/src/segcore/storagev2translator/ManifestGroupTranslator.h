...
class ManifestGroupTranslator
 ...
 private:
    ...
    // Row group stats from reader (before cell merging)
    std::vector<uint64_t> row_group_sizes_;  // size of each row group
    std::vector<uint64_t> row_group_rows_;   // row count of each row group
