...
GroupChunkTranslator::get_cells(const std::vector<cachinglayer::cid_t>& cids) {
    ...
    std::vector<cachinglayer::cid_t> sorted_cids(cids.begin(), cids.end());
    ...
    needed_row_group_indices.reserve(kRowGroupsPerCell * sorted_cids.size());
    for (auto cid : sorted_cids) {
        AssertInfo(cid < meta_.chunk_memory_size_.size(),
    ...
    // no need to sort the needed_row_group_indices because it is built from sorted_cids
    ...
    filled_cids.reserve(sorted_cids.size());
    for (auto cid : sorted_cids) {
        if (filled_cids.count(cid) > 0) {
            continue;  // Already processed this cell (handles duplicates)
        }
