...
ManifestGroupTranslator::ManifestGroupTranslator(
    ...
    auto scale_total_estimates_for_aliases =
        [&](std::vector<uint64_t>* sizes, std::string* error) {
    ...
    if (projected_estimate_available) {
        ...
        AssertInfo(fallback_base <= std::numeric_limits<uint64_t>::max() /
                   ...
                   max_materialized_field_count);
        const auto fallback =
            fallback_base * max_materialized_field_count;
    ...
    AssertInfo(positive_fallback_bytes_per_row_base <=
                   std::numeric_limits<uint64_t>::max() /
                       max_materialized_field_count,
