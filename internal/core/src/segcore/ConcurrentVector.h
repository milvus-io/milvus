    ...
    emplace_to_at_least(int64_t size, int64_t size_per_chunk) {
        ...
        while (vec_.size() < size) {
            if constexpr (IsScalar<T> && !std::is_same_v<T, PkType>) {
                vec_.emplace_back(milvus::engine::InitVectorPtr<T>(
                    size_per_chunk,
                    milvus::engine::GetDefaultMemoryPool().get()));
            }
            ++size_;
        }
    }
...
class ConcurrentVectorImpl<Type, true, true> : public VectorBase {
 ...
 public:
    explicit ConcurrentVectorImpl(int64_t size_per_chunk)
    ...
    get_span(int64_t chunk_id) const {
        auto& chunk = get_chunk(chunk_id);
        return Span<Type>(chunk->template asFlatVector<Type>()->rawValues(),
                          chunk->size());
    }
    ...
    grow_to_at_least(int64_t element_count) override {
        auto chunk_count = upper_div(element_count, size_per_chunk_);
        chunks_.template emplace_to_at_least<Type>(chunk_count,
                                                   size_per_chunk_);
    }
    ...
    grow_on_demand(int64_t element_count) {
        auto chunk_count = upper_div(element_count, size_per_chunk_);
        chunks_.template emplace_to_at_least<Type>(chunk_count, element_count);
    }
    ...
    get_chunk_data(ssize_t chunk_index) const override {
        ...
        return chunks_[chunk_index]
            ->template asFlatVector<EvalType>()
            ->rawValues();
    }
 ...
 private:
    ...
    fill_chunk(ssize_t chunk_id,
        ...
        using EvalType = typename facebook::velox::CppToType<Type>::NativeType;
        auto flat_vec =
            chunk->template as<facebook::velox::FlatVector<EvalType>>();
        if (flat_vec == NULL) {
            std::cout << "error happend" << std::endl;
        }
        EvalType* ptr = flat_vec->mutableRawValues();
        memcpy(ptr + chunk_offset * sizeof(Type),
               source,
...
class ConcurrentVector : public ConcurrentVectorImpl<Type, true, true> {
 public:
    ...
    explicit ConcurrentVector(int64_t size_per_chunk)
        : ConcurrentVectorImpl<Type, true, true>::ConcurrentVectorImpl(
              size_per_chunk) {
    }
};

template <>
class ConcurrentVector<PkType>
    : public ConcurrentVectorImpl<PkType, true, false> {
 public:
    explicit ConcurrentVector(int64_t size_per_chunk)
        : ConcurrentVectorImpl<PkType, true, false>::ConcurrentVectorImpl(
              1, size_per_chunk) {
    }
};
#else
