...
class VectorBase {
 public:
    ...
    virtual void
    set_data_raw(ssize_t element_offset, const std::vector<storage::FieldDataPtr>& data) = 0;
...
class ConcurrentVectorImpl : public VectorBase {
 ...
 public:
    ...
    void
    fill_chunk_data(const std::vector<storage::FieldDataPtr>& datas) override {  // used only for sealed segment
        ...
        for (auto& field_data : datas) {
            ...
            set_data(offset, static_cast<const Type*>(field_data->Data()), num_rows);
            offset += num_rows;
        }
    }
