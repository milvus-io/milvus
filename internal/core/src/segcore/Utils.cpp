...
int64_t
GetRawDataSizeOfDataArray(const DataArray* data, const FieldMeta& field_meta, int64_t num_rows) {
    int64_t result = 0;
    ...
    if (!datatype_is_variable(data_type)) {
        ...
        switch (data_type) {
            ...
            default: {
                PanicInfo("unsupported variable length datatype" + std::to_string((int)data_type));
            }
        }
    }
