...
SegmentInternalInterface::Retrieve(tracer::TraceContext* trace_ctx,
    ...
    // Pipeline ORDER BY path produces field_data_ directly with empty
    // result_offsets_.  Use total_data_cnt_ (set by ExecPlanNodeVisitor for
    // all pipeline paths) as the row count for the output-size guard.
    if (result_rows == 0) {
        result_rows = retrieve_results.total_data_cnt_;
    }
