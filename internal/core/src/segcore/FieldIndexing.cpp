...
void
VectorFieldIndexing::RegisterNullLogicalTail(const VectorBase* field_raw_data,
                                             int64_t logical_target) {
    if (!field_raw_data->is_mapping_storage()) {
        return;
    }
    int64_t index_logical = static_cast<int64_t>(index_->GetIdMap().OutCount());
    if (logical_target <= index_logical) {
        return;
    }
    // IdMap couples logical registration with the physical rows of the same
    // Add, so a standalone registration is only legal for a null-only range:
    // every valid logical row below logical_target maps to a physical row
    // that the caller has already drained into the index.
    int64_t count = logical_target - index_logical;
    auto validity = std::make_unique<bool[]>(count);
    field_raw_data->bulk_is_valid_range(index_logical, count, validity.get());
    auto dataset = knowhere::GenDataSet(0, get_dim(), nullptr);
    if (IsSparseFloatVectorDataType(get_data_type())) {
    ...
    dataset->SetIdMapData(knowhere::IdMapData::FromValidData(
        validity.get(), static_cast<size_t>(count)));
    index_->AddWithDataset(dataset, get_build_params(get_data_type()));
}
...
VectorFieldIndexing::BuildFirstIndexDense(const VectorBase* field_raw_data) {
    ...
    if (is_mapping_storage) {
        ...
        index_->SetIdMapType(knowhere::IdMap::Type::GROWING);
        // IdMapData only keeps a view of the buffer; keep it alive until the
        // synchronous Build call consumes the dataset.
...
VectorFieldIndexing::AppendSegmentIndexSparse(int64_t reserved_offset,
    ...
    if (!async_build_enabled_) {
        // Legacy synchronous path, matching master's sync-build semantics
        // (index_unavailable_ gating + DisableIndexAfterBuildFailure).
        if (!built_) {
            try {
...
VectorFieldIndexing::CatchUp(const VectorBase* field_raw_data) {
    ...
    for (;;) {
        ...
        bool is_mapping_storage = field_raw_data->is_mapping_storage();
        // Under IdMap the logical registration travels with each physical Add
        // (per-slice IdMapData in AddRange); only a null-only logical tail can
        // remain after the drain, and it is registered inside the lock below.
        int64_t index_logical =
            is_mapping_storage
                ? static_cast<int64_t>(index_->GetIdMap().OutCount())
                : pending;
        ...
        if (is_mapping_storage) {
            // Everything valid below `pending` is physical and drained; the
            // remaining logical tail is nulls by construction.
            RegisterNullLogicalTail(field_raw_data, pending);
            AssertInfo(
                static_cast<int64_t>(index_->GetIdMap().OutCount()) == pending,
                "id map logical count ({}) did not reach frozen watermark ({})",
                index_->GetIdMap().OutCount(),
                pending);
        }
...
// For mapping storage, attaches the IdMapData covering the logical range that
// the physical slice ending at `to_physical` completes:
// [GetIdMap().OutCount(), get_logical_offset(to_physical - 1) + 1). Returns
// the validity buffer, which must stay alive until AddWithDataset consumed
// the dataset (IdMapData only keeps a view). No-op for non-mapping storage.
std::unique_ptr<bool[]>
VectorFieldIndexing::AttachSliceIdMapData(const VectorBase* field_raw_data,
                                          knowhere::DataSet* dataset,
                                          int64_t to_physical) {
    if (!field_raw_data->is_mapping_storage()) {
        return nullptr;
    }
    int64_t logical_begin = static_cast<int64_t>(index_->GetIdMap().OutCount());
    int64_t logical_end =
        field_raw_data->get_logical_offset(to_physical - 1) + 1;
    AssertInfo(logical_end > logical_begin,
               "slice logical range is empty: [{}, {})",
               logical_begin,
               logical_end);
    int64_t logical_count = logical_end - logical_begin;
    auto validity = std::make_unique<bool[]>(logical_count);
    field_raw_data->bulk_is_valid_range(
        logical_begin, logical_count, validity.get());
    index_->SetIdMapType(knowhere::IdMap::Type::GROWING);
    dataset->SetIdMapData(knowhere::IdMapData::FromValidData(
        validity.get(), static_cast<size_t>(logical_count)));
    return validity;
}
...
VectorFieldIndexing::AddRange(const VectorBase* field_raw_data,
    ...
    if (IsSparseFloatVectorDataType(get_data_type())) {
        ...
        while (static_cast<int64_t>(index_cur_.load()) < target) {
            ...
            dataset->SetIsSparse(true);
            auto id_map_valid_data =
                AttachSliceIdMapData(field_raw_data, dataset.get(), to);
            index_->AddWithDataset(dataset, conf);
