...
std::unique_ptr<bool[]>
AttachIdMapRange(const milvus::DatasetPtr& dataset,
    ...
    // IdMapData only keeps a view of valid_data; keep this buffer alive until
    // the synchronous Build/Add call consumes the dataset.
    auto valid_data = std::make_unique<bool[]>(count);
    field_raw_data->bulk_is_valid_range(start, count, valid_data.get());
    dataset->SetIdMapData(knowhere::IdMapData::FromValidData(
        valid_data.get(), static_cast<size_t>(count)));
    return valid_data;
}
...
VectorFieldIndexing::AppendSegmentIndexSparse(int64_t reserved_offset,
    ...
    if (!built_) {
        ...
        if (is_mapping_storage) {
            ...
            id_map_valid_data =
                AttachIdMapRange(dataset, field_raw_data, 0, update_count);
        }
        try {
