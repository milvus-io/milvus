...
using LoadFieldDataInfo = milvus::segcore::LoadFieldDataInfo;
using FieldBinlogInfo = milvus::segcore::FieldBinlogInfo;
...
CStatus
AppendLoadFieldDataPath(CLoadFieldDataInfo c_load_field_data_info, int64_t field_id, const char* c_file_path) {
    try {
        auto load_field_data_info = (LoadFieldDataInfo*)c_load_field_data_info;
        auto iter = load_field_data_info->field_infos.find(field_id);
        ...
        load_field_data_info->field_infos[field_id].insert_files.emplace_back(file_path);
        return milvus::SuccessCStatus();
    } catch (std::exception& e) {
        return milvus::FailureCStatus(UnexpectedError, e.what());
    }
