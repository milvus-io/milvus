...
DefaultValueChunkTranslator::DefaultValueChunkTranslator(
    ...
    // For fixed-width types, all cells can share the same buffer regardless of
    // row count, because make_chunk_from_buffer can use row_nums_override.
    // For variable-length types (String, JSON, Array, etc.), buffer layout
    // (null bitmap size, offsets position) depends on exact row count, so we
    // need separate buffers for primary cells and tail cell (if different size).
    ...
    if (!can_share_buffer_ && num_cells() > 1) {
        ...
        if (tail_rows != primary_cell_rows_) {
            tail_buffer_ = build_buffer_for_rows(tail_rows);
...
milvus::ChunkBuffer
DefaultValueChunkTranslator::build_buffer_for_rows(int64_t num_rows) const {
    ...
    if (!use_mmap_ || mmap_dir_path_.empty()) {
        ...
        auto filepath = std::filesystem::path(mmap_dir_path_) /
                        fmt::format("seg_{}_f_{}_def", segment_id_, field_id_);
