...
ChunkTranslator::ChunkTranslator(
    ...
    meta_.avg_num_rows_per_chunk_ = field_data_info.row_count / files_and_rows_.size();
    meta_.virt_chunk_to_file_idx_.resize(field_data_info.row_count / meta_.avg_num_rows_per_chunk_);
    size_t nruc_idx = 0;
