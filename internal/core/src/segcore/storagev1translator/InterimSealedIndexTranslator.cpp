...
BuildInterimReader(
    const std::shared_ptr<std::vector<cachinglayer::PinWrapper<Chunk*>>>&
        pinned_chunks,
...
InterimSealedIndexTranslator::InterimSealedIndexTranslator(
    ...
    AssertInfo(is_sparse_ == IsSparseFloatVectorDataType(vec_data_type_),
...
InterimSealedIndexTranslator::get_cells(
    ...
    auto pinned_chunks =
        std::make_shared<std::vector<cachinglayer::PinWrapper<Chunk*>>>(
    ...
    for (int64_t chunk_id = 0; chunk_id < num_chunks; ++chunk_id) {
        const auto chunk_rows = nullable
                                    ? vec_data_->GetValidCountInChunk(chunk_id)
    ...
    ValidityView validity;
    if (nullable) {
        validity = ValidityView::FromExpanded(vec_data_->GetValidData().data());
    }
    ...
    if (is_sparse_) {
        ...
        knowhere::ViewDataOp view_data = [pinned_chunks,
                                          rows_until_chunk,
                                          num_chunks](size_t id) {
            AssertInfo(
                id <= static_cast<size_t>(std::numeric_limits<int64_t>::max()),
            ...
            AssertInfo(
                chunk != nullptr, "interim vector chunk {} is null", chunk_id);
