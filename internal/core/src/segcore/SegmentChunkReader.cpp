...
SegmentChunkReader::GetMultipleChunkScanDataAccessor(
    ...
    AssertInfo(scan_batch_size > 0,
               "column scan batch size must be positive");
...
SegmentChunkReader::GetVortexDataAccessorByOffsets(FieldId field_id,
    ...
    std::shared_ptr<TakeResult> take = column->Take(
        op_ctx_, TakeOptions{offsets, TargetTypeOf<T>()});
...
SegmentChunkReader::GetDataAccessorByOffsets(
    ...
    if (data_type == DataType::VARCHAR || data_type == DataType::STRING ||
        ...
        return GetStringDataAccessorByOffsets(
            field_id, offsets, pinned_index);
    }
