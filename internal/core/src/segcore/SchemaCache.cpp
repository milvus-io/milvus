...
struct SchemaKeyHash {
    ...
    operator()(const SchemaKey& key) const noexcept {
        ...
        const auto second = std::hash<uint64_t>{}(key.version);
...
SchemaCache::GetOrCreate(
    ...
    return GetOrCreate(collection_id,
...
SchemaCache::GetOrCreate(
    ...
    const milvus::proto::schema::CollectionSchema& schema_proto) {
    const SchemaKey key{collection_id, runtime_version};
    auto state = state_;
    Entry* entry = nullptr;
    {
        ...
        if (auto it = state->entries.find(key); it != state->entries.end()) {
            ...
            schema->set_schema_version(runtime_version);
