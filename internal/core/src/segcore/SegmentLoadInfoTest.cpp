...
TEST_F(SegmentLoadInfoTest, DiskVectorDefersFileAwareResourceEstimate) {
    auto proto = proto_;
    auto* index = proto.mutable_index_infos(0);
    for (auto& param : *index->mutable_index_params()) {
        if (param.key() == milvus::index::INDEX_TYPE) {
            param.set_value("DISKANN");
        }
    }
    SegmentLoadInfo info(proto, schema_);
    const auto indexes = info.GetFieldIndexInfos(FieldId(101));
