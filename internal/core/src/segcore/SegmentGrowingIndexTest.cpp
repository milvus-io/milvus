...
class ScopedVectorFieldMmap {
 public:
    explicit ScopedVectorFieldMmap(bool value)
        ...
        // MmapConfig's setters were removed upstream; the fields are public.
        config_.vector_field_enable_mmap = value;
    }

    ~ScopedVectorFieldMmap() {
        config_.vector_field_enable_mmap = previous_;
    }
...
TEST(GrowingIndexAsyncBuildTest, NullableVectorBuildsAsynchronously) {
    ...
    raw->bulk_is_valid_range(/*start=*/7997,
    ...
    // Logical rows: the per-slice IdMapData plus the null-tail registration
    // covered every inserted logical row, nulls included.
    const auto& indexing =
        segment_impl->get_indexing_record().get_vec_field_indexing(fixture.vec);
    auto pinned = indexing.get_segment_indexing();
    auto* vec_index = dynamic_cast<index::VectorIndex*>(pinned.get());
    ASSERT_NE(vec_index, nullptr);
    EXPECT_EQ(static_cast<int64_t>(vec_index->GetIdMap().OutCount()),
              n_batch * per_batch);
...
// from index_->GetIdMap().OutCount() vs. the incoming batch's
...
TEST(GrowingIndexAsyncBuildTest,
    ...
    InsertAsyncBatch(segment.get(), fixture.schema, 25000, 13);
    ASSERT_TRUE(WaitFlag(at_before_build, /*timeout_ms=*/60000));
    // MmapManager::GetDiskUsage() was removed upstream (dead-code sweep), so
    // the mmap-liveness evidence is now behavioral: the destructor must block
    // below while the build is parked, and once released the build reads the
    // mmap-backed chunks -- a premature unmap faults under ASAN.
