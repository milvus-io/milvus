...
void
OrderedDeletedRecord::commit() {
    std::unique_lock<std::shared_mutex> lck(mtx_);
    if (!switched_) {
        LOG_INFO("generate deleted record from buffer, len(buffered): {}",
                 buffer_.size());
        generate_unsafe();
    }
}
...
DeletedRecord&
OrderedDeletedRecord::get_deleted_record() {
    {
    ...
    LOG_WARN("deleted record not committed, len(buffered): {}", buffer_.size());

    if (switched_) {
        LOG_INFO("deleted record already generated, len(record): {}",
                 final_record_.size());
        return final_record_;
    }
...
OrderedDeletedRecord::load(const std::vector<PkType>& pks,
    ...
    std::unique_lock<std::shared_mutex> lck(mtx_);
    ExitInfo(!switched_, "[should not happen] load delete record after search");
