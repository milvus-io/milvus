        ...
        milvus::futures::Future<milvus::query::SharedFilterBitsetResult>::async(
            ...
            [c_trace,
             ...
             enable_expr_cache](folly::CancellationToken cancel_token) {
                ...
                auto read_lease =
                    AcquireSegmentReadLease(segment, cancel_token);
    ...
    auto future = milvus::futures::Future<milvus::SearchResult>::async(
        ...
        [c_trace,
         ...
         entity_ttl_physical_time_us](folly::CancellationToken cancel_token) {
            ...
            // inaccessible while its siblings proceed normally. A null
            // shared bitset (phase 1 saw no visible rows) is not shortcut
            // here: SearchWithBitset still runs this branch's own admission
            // check before it returns the empty result, exactly as a plain
            // search on an empty segment would.
            if (!internal_segment->FieldAccessible(target_vector_field_id)) {
                ...
                search_result = internal_segment->SearchWithBitset(
                    plan,
                    phg_ptr,
                    bitset_result,
                    timestamp,
