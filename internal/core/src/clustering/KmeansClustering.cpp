...
KmeansClustering::FetchSegmentViaManifest(uint8_t* buf,
    ...
    // The destination already owns the training/assignment buffer. Do not
    // retain another full segment in CacheRawDataToMemory on top of it.
    // Bound the decoded row window; the storage reader may additionally
    // retain the physical chunks intersecting that window.
    const int64_t rows_per_batch =
        std::max<int64_t>(1, DEFAULT_FIELD_MAX_MEMORY_LIMIT / dim / sizeof(T));
    for (int64_t read_rows = 0; read_rows < expected_rows;) {
        const auto rows = std::min(rows_per_batch, expected_rows - read_rows);
        config[NUM_ROWS_KEY] = rows;
        config[OFFSET_KEY] = row_offset + read_rows;
        auto field_datas = file_manager_->CacheRawDataToMemory(config);
        const auto expected_bytes = rows * dim * sizeof(T);
        int64_t source_offset = 0;
        const auto fetched = CopyFieldDatasToBuf(
            buf, expected_bytes, field_datas, source_offset, offset);
...
KmeansClustering::IsDataSkew(
    ...
    // Relative cluster-size ratios are legacy protocol fields, not skip
    // conditions. Keep only the independent absolute cluster-size safeguard.
    auto max_cluster_rows = *std::max_element(num_in_each_centroid.begin(),
                                              num_in_each_centroid.end());
    if (max_cluster_rows * dim * sizeof(T) >= max_cluster_size) {
        LOG_INFO(msg_header_ + "maximum cluster size too large: {}B",
                 max_cluster_rows * dim * sizeof(T));
        return true;
    }
    return false;
}
...
KmeansClustering::Run(const milvus::proto::clustering::AnalyzeInfo& config) {
    ...
    // Cluster layout ordering and its persisted distances always use squared
    // L2, independently of the collection metric or opaque cluster parameters.
    cluster_config[knowhere::meta::METRIC_TYPE] = knowhere::metric::L2;
