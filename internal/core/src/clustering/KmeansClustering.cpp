...
KmeansClustering::FetchDataFiles(uint8_t* buf, const DatasetPart& part) {
    ...
    for (auto& data : field_datas) {
        ...
        if (fetched >= (size_t)part.max_bytes) {
            break;
        }
    }
    return fetched;
...
KmeansClustering::CentroidIdMappingToPB(
    ...
    for (auto i = 0; i < trained_segments_num; i++) {
        ...
        for (uint64_t j = 0; j < (uint64_t)num_clusters; j++) {
...
KmeansClustering::IsDataSkew(
    ...
    if (num_in_each_centroid.back() * dim * sizeof(T) >=
...
KmeansClustering::StreamingAssignandUpload(
    ...
    // ===================================
    for (size_t i = 0; i < (size_t)trained_segments_num; i++) {
    ...
    if (total_buffer == 0) {
        // Default: use 512MB if not specified
        total_buffer = 512ULL << 20;
    }
    size_t num_threads = std::max<size_t>(pool.GetThreadNum(), 1);
    size_t remaining_segments = segment_ids.size() - trained_segments_num;
    num_threads = std::min<size_t>(num_threads, remaining_segments);
    if (num_threads > 0) {
...
KmeansClustering::Run(const milvus::proto::clustering::AnalyzeInfo& config) {
    ...
    } else if (config.storage_version() == STORAGE_V3) {
        // V3: use manifest_paths; DatasetIterator will set
        // SEGMENT_MANIFEST_KEY in the config passed to CacheRawDataToMemory
        for (const auto& [k, v] : manifest_paths) {
            insert_files[k] = {v};
        }
    } else {
        // V1: use legacy insert_files
    ...
    if (train_num < (size_t)num_clusters) {
    ...
    while (train_iter.HasNext()) {
        ...
        LOG_DEBUG(msg_header_ +
                      "Next iteration with {} max_bytes and fetched_so_far {}",
                  part.max_bytes,
                  fetched_so_far);
        ...
        if (part.max_bytes + fetched_so_far <= train_buffer_size) {
            ...
            } else if (fetched != (size_t)part.max_bytes) {
                train_iter.updateRemaining((int64_t)part.max_bytes -
                                           (int64_t)fetched);
            }
        ...
        if (fetched_so_far == 0) {
            train_iter.notifyEOF();
        } else if (fetched_so_far != (size_t)part.max_bytes) {
            train_iter.updateRemaining((int64_t)part.max_bytes -
                                       (int64_t)fetched_so_far);
        }
    }
    // flush last batch
    if (fetched_so_far > 0) {
    ...
    if (!centroids_res.has_value()) {
        ThrowInfo(
            ...
            fmt::format("failed to get centroids: {}", centroids_res.what()));
    }
