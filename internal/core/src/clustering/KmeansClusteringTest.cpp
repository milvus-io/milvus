...
transforConfigToPB(const Config& config) {
    ...
    if (info_pb.storage_version() == STORAGE_V3) {
        auto manifest_paths_map =
            config.at("manifest_paths").get<std::map<int64_t, std::string>>();
        for (auto& [segment_id, path] : manifest_paths_map) {
            (*info_pb.mutable_manifest_paths())[segment_id] = path;
        }
    }
...
template <typename T>
StoragePreparationResult
prepare_storage_v3(const std::string& root_path,
                   int64_t collection_id,
                   int64_t partition_id,
                   int64_t segment_id,
                   int64_t segment_id2,
                   ...
                   const std::shared_ptr<storage::ChunkManager>& cm,
                   bool train_full) {
    std::cout << "[DEBUG] Preparing STORAGE_V3 data\n";
    auto schema = gen_all_data_types_schema();
    int64_t batch_size = 1000;
    int64_t n_batch = nb / batch_size;
    auto fs = milvus::storage::InitArrowFileSystem(
        gen_local_storage_config(root_path));
    auto test_data_seg_1 = std::make_unique<milvus::test::V3SegmentTestData>(
        schema, n_batch, batch_size, dim, root_path, "0/seg3");
    auto test_data_seg_2 = std::make_unique<milvus::test::V3SegmentTestData>(
        schema, n_batch, batch_size, dim, root_path, "0/seg4");

    std::map<int64_t, std::vector<std::string>> flattened_insert_files = {
        {segment_id, {test_data_seg_1->BasePath()}},
        {segment_id2, {test_data_seg_2->BasePath()}}};
    std::map<int64_t, int64_t> num_rows = {{segment_id, nb}, {segment_id2, nb}};
    int64_t bytes_per_vector = dim * sizeof(T);
    int64_t train_size = 2 * nb * bytes_per_vector;
    Config config;
    config["storage_version"] = STORAGE_V3;
    config["train_size"] = train_size;
    ...
    std::map<int64_t, std::vector<std::vector<std::string>>>
        segment_insert_files = {{segment_id, {{test_data_seg_1->BasePath()}}},
                                {segment_id2, {{test_data_seg_2->BasePath()}}}};
    std::map<int64_t, std::string> manifest_paths = {
        {segment_id, test_data_seg_1->ManifestPathJson()},
        {segment_id2, test_data_seg_2->ManifestPathJson()}};
    ...
    auto loon_properties = MakeInternalPropertiesFromStorageConfig(
        ToCStorageConfig(gen_local_storage_config(root_path)));
    ctx.set_loon_ffi_properties(loon_properties);
...
test_run_unified(const std::string& storage_version, bool train_full = false) {
    ...
    if (storage_version == "STORAGE_V1") {
        ...
        prep = prepare_storage_v2<T>(root_path,
                                     ...
                                     cm,
                                     train_full);
    } else if (storage_version == "STORAGE_V3") {
        prep = prepare_storage_v3<T>(root_path,
                                     collection_id,
                                     partition_id,
                                     segment_id,
                                     segment_id2,
 ...
 *
 * STORAGE_V3:
 *   - Modern columnar storage using Apache Parquet (via milvus-storage) similar to storage v2.
 *   - The meta data of columns an column group is managed by a manifest
 *
 */
...
TEST(MajorCompaction, PartialV3) {
    test_run_unified /*v2*/<float, DataType::VECTOR_FLOAT>("STORAGE_V3");
}
...
TEST(MajorCompaction, FullV3) {
    test_run_unified /*v2*/<float, DataType::VECTOR_FLOAT>(
        "STORAGE_V3", /* full_train */ true);
}
...
TEST(MajorCompaction, NumClustersVariantsV3) {
    using T = float;
    int64_t collection_id = 1, partition_id = 2;
    int64_t segment_id = 3, segment_id2 = 4;
    int64_t index_build_id = 1000, index_version = 10000;
    ...
    auto cm = storage::CreateChunkManager(gen_local_storage_config(root_path));
    // no need to generate data_gen here because prepare_storage_v2() already does it internally
    auto prep = prepare_storage_v3<T>(root_path,
                                      collection_id,
                                      partition_id,
                                      segment_id,
                                      segment_id2,
                                      ...
                                      cm,
                                      true /* full train */);
    runNumClustersVariantsUnified<T>("STORAGE_V3", prep, dim);
}
...
//TEST(MajorCompaction, NoDataSkew_V3) {
//    using T = float;
//    int64_t collection_id = 1, partition_id = 2;
//    int64_t segment_id = 3, segment_id2 = 4;
//    int64_t index_build_id = 1000, index_version = 10000;
//    int64_t dim = 128;
//    int64_t nb = 10000;
//    std::string root_path = "/tmp/test-kmeans-nodataskew-v3/";
//    boost::filesystem::create_directories(root_path);
//    auto cm = storage::CreateChunkManager(gen_local_storage_config(root_path));
//    auto prep = prepare_storage_v3<T>(root_path,
//                                      collection_id,
...
//                                      cm,
//                                      true);
//    runNoDataSkewTestUnified<T>("STORAGE_V3", prep, dim, cm);
//}
...
TEST(MajorCompaction, DataSkew_V3) {
    using T = float;
    int64_t collection_id = 1, partition_id = 2;
    int64_t segment_id = 3, segment_id2 = 4;
    int64_t index_build_id = 1000, index_version = 10000;
    ...
    runDataSkewDetectionTestUnified<T>("STORAGE_V3", prep, dim, cm);
}
...
//TEST(MajorCompaction, DataSkew_ByMaxClusterSize_V3) {
//    using T = float;
//    int64_t collection_id = 1, partition_id = 2;
//    int64_t segment_id = 3, segment_id2 = 4;
//    int64_t index_build_id = 1000, index_version = 10000;
...
//    std::string root_path = "/tmp/test-kmeans-dataskew-maxclustersize/";
//    boost::filesystem::create_directories(root_path);
//    auto cm = storage::CreateChunkManager(gen_local_storage_config(root_path));
//    auto prep = prepare_storage_v3<T>(root_path,
//                                      collection_id,
...
//                                      cm,
//                                      true);
//    runDataSkewByMaxClusterSizeTestUnified<T>("STORAGE_V3", prep, dim, cm);
//}
