...
template <typename T>
size_t
VectorBuildMaterializer::BatchElementCount(
...
VectorBuildMaterializer::AppendBatch(TypedState<T>& state,
    ...
    if (field_type_ == DataType::VECTOR_ARRAY) {
        ...
        if (state.values.empty() && layout_.LogicalRows() == 0 &&
            !batch->IsNullable()) {
            state.values.reserve(CheckedSize(expected_rows_, "expected row"));
        }
        ...
        if (state.values.empty() && layout_.LogicalRows() == 0 &&
            ...
            const auto row_elements =
                ...
                sizeof(ValueType);
            const auto expected = CheckedSize(expected_rows_, "expected row");
            AssertInfo(
                row_elements == 0 ||
