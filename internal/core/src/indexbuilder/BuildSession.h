...
// Protobuf/default-value interpretation remains inside BuildSession.
...
// Native request assembled by the C-ABI adapter.
...
// One source-backed build lifecycle: materialize a complete input, invoke the
// selected typed builder once, retain the outcome, and publish it.
class BuildSession final {
 public:
    ...
    // Build exactly once. A failed build poisons the session; a successful
    // build retains its outcome until the session is destroyed.
    void
    BuildFromSource();

    // Publish the retained outcome. Artifact writes may upload incrementally;
    // Finish finalizes publication. A failure leaves the outcome available for
    // another Publish attempt; SkippedEmpty returns empty file stats.
