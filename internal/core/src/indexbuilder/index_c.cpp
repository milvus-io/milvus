...
BuildPrimaryIndex(ProtoLayoutInterface result,
    ...
    try {
        ...
        auto index =
            std::make_unique<index::PrimaryIndex>(fileManagerContext, false);
...
LoadPrimaryIndex(CPrimaryIndex* res_index,
    ...
    try {
        ...
        auto index =
            std::make_unique<milvus::index::PrimaryIndex>(file_ctx, true);
        index->Load(config);
...
QueryPrimaryIndex(CPrimaryIndex index_handle,
    ...
    try {
        auto index =
            reinterpret_cast<milvus::index::PrimaryIndex*>(index_handle);
        if (!index) {
            auto status = CStatus();
            status.error_code = UnexpectedError;
