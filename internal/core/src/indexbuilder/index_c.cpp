...
CreateIndex(CIndex* res_index,
    ...
    try {
        ...
        // Only this scalar/vector build path can carry the marker (JSON-key
        // and text builds never do). Emit the key ONLY for a genuine nested
        // index: the vanilla-FAISS vector build path strict-checks its build
        // params and rejects any unrecognized key, so a stray "nested_index"
        // leaking into a vector (or any non-nested) build fails knowhere with
        // segcore 2004. This mirrors the load-side guard (load_index_c.cpp /
        // SegmentLoadInfo.cpp), which also injects the key only when set.
