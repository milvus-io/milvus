...
CreateIndex(enum CDataType dtype,
    ...
    try {
        ...
        auto index =
            milvus::indexbuilder::IndexFactory::GetInstance().CreateIndex(milvus::DataType(dtype), config, nullptr);
...
CreateIndexV2(CIndex* res_index, CBuildIndexInfo c_build_index_info) {
    try {
        ...
        // get index type
        auto index_type = milvus::index::GetValueFromConfig<std::string>(config, "index_type");
        ...
        if (milvus::datatype_is_vector(field_type)) {
            auto metric_type = milvus::index::GetValueFromConfig<std::string>(config, "metric_type");
        ...
        auto index_mode = milvus::index::GetValueFromConfig<std::string>(config, "index_mode");
        if (index_mode.has_value()) {
            index_info.index_mode = milvus::index::GetIndexMode(index_mode.value());
        }

        // init file manager
        milvus::storage::FieldDataMeta field_meta{build_index_info->collection_id, build_index_info->partition_id,
                                                  build_index_info->segment_id, build_index_info->field_id};
        milvus::storage::IndexMeta index_meta{build_index_info->segment_id, build_index_info->field_id,
        ...
        auto index = milvus::indexbuilder::IndexFactory::GetInstance().CreateIndex(build_index_info->field_type, config,
                                                                                   file_manager);
        index->Build();
        *res_index = index.release();
        auto status = CStatus();
...
CStatus
NewBuildIndexInfo(CBuildIndexInfo* c_build_index_info, CStorageConfig c_storage_config) {
    try {
...
CStatus
AppendBuildIndexParam(CBuildIndexInfo c_build_index_info, const uint8_t* serialized_index_params, const uint64_t len) {
    try {
        auto build_index_info = (BuildIndexInfo*)c_build_index_info;
        auto index_params = std::make_unique<milvus::proto::indexcgo::IndexParams>();
...
CStatus
AppendBuildTypeParam(CBuildIndexInfo c_build_index_info, const uint8_t* serialized_type_params, const uint64_t len) {
    try {
        auto build_index_info = (BuildIndexInfo*)c_build_index_info;
        auto type_params = std::make_unique<milvus::proto::indexcgo::TypeParams>();
...
CStatus
AppendIndexMetaInfo(CBuildIndexInfo c_build_index_info, int64_t index_id, int64_t build_id, int64_t version) {
    try {
        auto build_index_info = (BuildIndexInfo*)c_build_index_info;
...
CStatus
AppendInsertFilePath(CBuildIndexInfo c_build_index_info, const char* c_file_path) {
    try {
        auto build_index_info = (BuildIndexInfo*)c_build_index_info;
...
SerializeIndexAndUpLoad(CIndex index, CBinarySet* c_binary_set) {
    ...
    try {
        AssertInfo(index, "failed to serialize index to binary set, passed index was null");
        auto real_index = reinterpret_cast<milvus::indexbuilder::IndexCreatorBase*>(index);
        auto binary = std::make_unique<knowhere::BinarySet>(real_index->Upload());
        *c_binary_set = binary.release();
        status.error_code = Success;
        status.error_msg = "";
    } catch (std::exception& e) {
