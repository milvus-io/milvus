...
CreateMissingFieldData(const storage::FileManagerContext& context,
    ...
    // Build the batch directly from the default value.
...
ValidateOrSetInt64(index::BuildParams& params,
    ...
    if (params.contains(key) && !params.at(key).is_null()) {
        const auto actual = index::ParseInt64Param(
            params.at(key), key, "normalized build parameter");
        if (actual != expected) {
...
BuildSession::BuildFromSource() {
    ...
    try {
        product_.emplace(RunToArtifact());
        state_ =
...
void
BuildSession::NormalizeAndValidateRequest() {
    AssertInfo(file_manager_context_.Valid(),
               "index-build session requires a valid FileManagerContext");
...
void
BuildSession::ValidateInputSpec(const index::BuilderInputSpec& spec) const {
...
BuildSession::Publish() const {
    ...
    if (req_.output.generation == storage::Generation::V3) {
        ...
        const bool is_index = req_.output.storage_namespace ==
    ...
    auto sink = std::make_unique<storage::V1DiskSink>(
        file_manager_context_, req_.output.storage_namespace);
