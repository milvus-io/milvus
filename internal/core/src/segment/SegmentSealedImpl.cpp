...
void
SegmentSealedImpl::LoadIndex(const milvus::index::LoadIndexInfo& info) {
    // print(info);
...
void
SegmentSealedImpl::LoadVecIndex(const milvus::index::LoadIndexInfo& info) {
    // NOTE: lock only when data is ready to avoid starvation
    auto field_id = FieldId(info.field_id);
    auto& field_meta = schema_->operator[](field_id);
    ...
    vector_indexings_.append_field_indexing(
        field_id,
        metric_type,
        std::move(const_cast<milvus::index::LoadIndexInfo&>(info).index));
...
void
SegmentSealedImpl::LoadScalarIndex(const milvus::index::LoadIndexInfo& info) {
    // NOTE: lock only when data is ready to avoid starvation
    auto field_id = FieldId(info.field_id);
    auto& field_meta = schema_->operator[](field_id);
...
void
SegmentSealedImpl::LoadFieldData(
    const milvus::base::LoadFieldDataInfo& load_info) {
    // NOTE: lock only when data is ready to avoid starvation
    ...
    for (auto& [id, info] : load_info.field_infos) {
        ...
        auto load_future =
            pool.Submit(milvus::storage::LoadFieldDatasFromRemote,
                        insert_files,
        ...
        LOG_INFO("segment {} submits load field {} task to thread pool",
                 ...
                 field_id.get());
        if (!info.enable_mmap ||
            milvus::base::SystemProperty::Instance().IsSystem(field_id)) {
...
void
SegmentSealedImpl::LoadFieldDataV2(
    const milvus::base::LoadFieldDataInfo& load_info) {
    // NOTE: lock only when data is ready to avoid starvation
    // only one field for now, parallel load field data in golang
    size_t num_rows = storage::GetNumRowsForLoadInfo(load_info);
    ...
    if (milvus::base::SystemProperty::Instance().IsSystem(field_id)) {
        auto system_field_type =
            milvus::base::SystemProperty::Instance().GetSystemFieldType(
                field_id);
        if (system_field_type == milvus::base::SystemFieldType::Timestamp) {
            ...
            milvus::base::TimestampIndex index;
            auto min_slice_length = num_rows < 4096 ? 1 : 4096;
            auto meta = milvus::base::GenerateFakeSlices(
            ...
            AssertInfo(
                system_field_type == milvus::base::SystemFieldType::RowId,
...
void
SegmentSealedImpl::LoadDeletedRecord(
...
SegmentSealedImpl::AddFieldDataInfoForSealed(
    const milvus::base::LoadFieldDataInfo& field_data_info) {
    // copy assignment
...
void
SegmentSealedImpl::vector_search(milvus::base::SearchInfo& search_info,
...
SegmentSealedImpl::get_vector(FieldId field_id,
    ...
    if (has_raw_data) {
        ...
        return milvus::base::CreateVectorDataArrayFrom(
...
SegmentSealedImpl::DropFieldData(const FieldId field_id) {
    if (milvus::base::SystemProperty::Instance().IsSystem(field_id)) {
        ...
        std::unique_lock lck(mutex_);
        --system_ready_count_;
        if (system_field_type == milvus::base::SystemFieldType::RowId) {
            insert_record_.row_ids_.clear();
        } else if (system_field_type ==
                   milvus::base::SystemFieldType::Timestamp) {
...
SegmentSealedImpl::DropIndex(const FieldId field_id) {
    AssertInfo(!milvus::base::SystemProperty::Instance().IsSystem(field_id),
...
SegmentSealedImpl::SegmentSealedImpl(
    ...
    milvus::base::IndexMetaPtr index_meta,
...
void
SegmentSealedImpl::bulk_subscript(milvus::base::SystemFieldType system_type,
...
SegmentSealedImpl::fill_with_empty(FieldId field_id, int64_t count) const {
    ...
    if (datatype_is_vector(field_meta.get_data_type())) {
        return milvus::base::CreateVectorDataArray(count, field_meta);
    }
    return milvus::base::CreateScalarDataArray(count, field_meta);
}
...
SegmentSealedImpl::bulk_subscript(FieldId field_id,
    ...
    if (HasIndex(field_id)) {
        ...
        if (!datatype_is_vector(field_meta.get_data_type())) {
            ...
            if (index->HasRawData()) {
                return milvus::index::ReverseDataFromIndex(
...
SegmentSealedImpl::mask_with_timestamps(BitsetType& bitset_chunk,
    ...
    auto mask = milvus::base::TimestampIndex::GenerateBitset(
...
SegmentSealedImpl::generate_binlog_index(const FieldId field_id) {
    ...
    try {
        ...
        auto field_binlog_config =
            std::unique_ptr<milvus::base::VecIndexConfig>(
                new milvus::base::VecIndexConfig(row_count,
                                                 field_index_meta,
                                                 segcore_config_,
                                                 SegmentType::Sealed));
