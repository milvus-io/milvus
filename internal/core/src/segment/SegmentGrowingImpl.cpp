...
SegmentGrowingImpl::try_remove_chunks(FieldId fieldId) {
    ...
    if (indexing_record_.SyncDataWithIndex(fieldId)) {
        auto vec_data_base =
            dynamic_cast<milvus::base::ConcurrentVector<FloatVector>*>(
...
SegmentGrowingImpl::Insert(int64_t reserved_offset,
    ...
    for (auto [field_id, field_meta] : schema_->get_fields()) {
        ...
        auto field_data_size = milvus::base::GetRawDataSizeOfDataArray(
    ...
    milvus::base::ParsePksFromFieldData(
...
void
SegmentGrowingImpl::LoadFieldData(
    const milvus::base::LoadFieldDataInfo& infos) {
    ...
    for (auto& [id, info] : infos.field_infos) {
        ...
        auto channel = std::make_shared<milvus::base::FieldDataChannel>();
        auto& pool =
        ...
        auto load_future = pool.Submit(
            milvus::storage::LoadFieldDatasFromRemote, insert_files, channel);
...
void
SegmentGrowingImpl::LoadFieldDataV2(
    const milvus::base::LoadFieldDataInfo& infos) {
    // schema don't include system field
    ...
    for (auto& [id, info] : infos.field_infos) {
        ...
        auto load_future =
            pool.Submit(milvus::storage::LoadFieldDatasFromRemote2,
                        space,
                        schema_,
                        field_data_info);
        auto field_data =
...
SegmentGrowingImpl::Delete(int64_t reserved_begin,
    ...
    milvus::base::ParsePksFromIDs(pks, field_meta.get_data_type(), *ids);
...
void
SegmentGrowingImpl::LoadDeletedRecord(
    const milvus::base::LoadDeletedRecordInfo& info) {
    ...
    std::vector<PkType> pks(size);
    milvus::base::ParsePksFromIDs(
        pks, field_meta.get_data_type(), *info.primary_keys);
...
void
SegmentGrowingImpl::vector_search(milvus::base::SearchInfo& search_info,
                                  const void* query_data,
                                  int64_t query_count,
                                  Timestamp timestamp,
                                  const BitsetView& bitset,
                                  milvus::base::SearchResult& output) const {
    auto& sealed_indexing = this->get_sealed_indexing_record();
    if (sealed_indexing.is_ready(search_info.field_id_)) {
        SearchOnSealedIndex(this->get_schema(),
                            sealed_indexing,
                            search_info,
                            query_data,
                            query_count,
...
SegmentGrowingImpl::bulk_subscript(FieldId field_id,
    ...
    if (field_meta.is_vector()) {
        auto result = milvus::base::CreateVectorDataArray(count, field_meta);
    ...
    auto result = milvus::base::CreateScalarDataArray(count, field_meta);
...
SegmentGrowingImpl::bulk_subscript_ptr_impl(
    const milvus::base::VectorBase* vec_raw,
    const int64_t* seg_offsets,
    int64_t count,
    google::protobuf::RepeatedPtrField<T>* dst) const {
    auto vec = dynamic_cast<const milvus::base::ConcurrentVector<S>*>(vec_raw);
    auto& src = *vec;
...
SegmentGrowingImpl::bulk_subscript_impl(FieldId field_id,
    ...
    auto vec_ptr =
        dynamic_cast<const milvus::base::ConcurrentVector<T>*>(vec_raw);
...
template <typename S, typename T>
void
SegmentGrowingImpl::bulk_subscript_impl(const milvus::base::VectorBase* vec_raw,
                                        const int64_t* seg_offsets,
                                        int64_t count,
                                        T* output) const {
    static_assert(IsScalar<S>);
    auto vec_ptr =
        dynamic_cast<const milvus::base::ConcurrentVector<S>*>(vec_raw);
    AssertInfo(vec_ptr, "Pointer of vec_raw is nullptr");
    auto& vec = *vec_ptr;
    for (int64_t i = 0; i < count; ++i) {
        auto offset = seg_offsets[i];
...
SegmentGrowingImpl::bulk_subscript_array_impl(
    ...
    google::protobuf::RepeatedPtrField<T>* dst) const {
    auto vec_ptr =
        dynamic_cast<const milvus::base::ConcurrentVector<Array>*>(&vec_raw);
    AssertInfo(vec_ptr, "Pointer of vec_raw is nullptr");
    auto& vec = *vec_ptr;
    for (int64_t i = 0; i < count; ++i) {
        auto offset = seg_offsets[i];
...
void
SegmentGrowingImpl::bulk_subscript(milvus::base::SystemFieldType system_type,
                                   const int64_t* seg_offsets,
                                   int64_t count,
                                   void* output) const {
    switch (system_type) {
        case milvus::base::SystemFieldType::Timestamp:
            bulk_subscript_impl<Timestamp>(&this->insert_record_.timestamps_,
                                           seg_offsets,
                                           count,
                                           static_cast<Timestamp*>(output));
            break;
        case milvus::base::SystemFieldType::RowId:
...
SegmentGrowingImpl::search_ids(const IdArray& id_array,
    ...
    auto ids_size = milvus::base::GetSizeOfIdArray(id_array);
    std::vector<PkType> pks(ids_size);
    milvus::base::ParsePksFromIDs(pks, data_type, id_array);
...
void
SegmentGrowingImpl::check_search(const query::Plan* plan) const {
    Assert(plan);
}
