...
void
FloatSegmentIndexSearch(const SegmentGrowingImpl& segment,
                        const milvus::base::SearchInfo& info,
    ...
    if (indexing_record.is_in(vecfield_id)) {
        ...
        milvus::base::SearchInfo search_conf =
            field_indexing.get_search_params(info);
...
SearchOnGrowing(const SegmentGrowingImpl& segment,
                ...
                Timestamp timestamp,
                const BitsetView& bitset,
                milvus::base::SearchResult& results) {
    auto& schema = segment.get_schema();
    auto& record = segment.get_insert_record();
    auto active_count =
