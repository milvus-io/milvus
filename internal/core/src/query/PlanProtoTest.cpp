...
TEST(PlanProto, MatchExprLegacyStructNameFallback) {
    ...
    // to struct_name (field_name = struct_name, field_id = 0, empty
    // nested_path) instead of reading an empty MatchColumnInfo.
    using namespace milvus;
    using namespace milvus::query;
    ...
    EXPECT_EQ(match_expr->get_field_id(), 0);
    EXPECT_TRUE(match_expr->get_nested_path().empty());
