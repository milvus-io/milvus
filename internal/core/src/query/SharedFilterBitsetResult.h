...
struct SharedFilterBitsetResult {
    ...
    // ElementFilterBitsNode sits inside the prefix and sets the element-level
    ...
    // bitset_is_element_level to pick between the row-level and the
    // element-level path).
    //
    // all_rows_visible is deliberately not carried. MvccNode sets it only on
    // its source-node sealed fast path, and the prefix's MvccNode is never the
    // source node here: the proxy marks a sub-request shareable only when it
    // carries a predicate, and phase 1 asserts an extractable filter prefix,
    // so a FilterBitsNode always feeds it.
    //
