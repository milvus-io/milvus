...
// The derived state that travels with the bitset is element-level and covered by
// ElementLevelMatchesIndependentSearch; a row-level predicate must report none
// of it.
//
...
// proxy only marks a sub-request shareable when it carries a predicate -- but
// the segcore API accepts it, and the result must still match an independent
// search without all_rows_visible being carried across the two phases.
...
TEST(SharedFilterBitset, FilterStorageCostReportedOnce) {
    // Byte accounting is off by default and is fixed per cache slot when the
    // slot is created, so it has to be on before the fixture loads its data.
    auto& tiered = milvus::cachinglayer::TieredStorageConfig::GetInstance();
    const bool tracking_before = tiered.storage_usage_tracking_enabled();
    tiered.SetStorageUsageTrackingEnabled(true);
    struct Restore {
        milvus::cachinglayer::TieredStorageConfig& cfg;
        bool value;
        ~Restore() {
            cfg.SetStorageUsageTrackingEnabled(value);
        }
    } restore{tiered, tracking_before};
