...
ProtoParser::ParseMatchExprs(const proto::plan::MatchExpr& expr_pb) {
    ...
    if (!expr_pb.has_column()) {
        ...
        // only target struct arrays. MatchExpr::Eval dispatches on field_name
        // first for the struct check, so field_id is unused on this path.
        return std::make_shared<expr::MatchExpr>(0,
                                                 expr_pb.struct_name(),
