...
class SparseVortexMmapFile final
 ...
 public:
    ...
    WriteAt(const uint64_t& offset,
        ...
        // Mapped writes can raise SIGBUS when the memfd cannot allocate
        // backing pages. Write through the fd so ENOSPC/quota failures are
        // returned to the Vortex loader as an Arrow status instead.
        ARROW_RETURN_NOT_OK(PWriteAll(fd_, data->data(), offset, length));
        return arrow::Status::OK();
    }
