...
class SparseVortexMmapRangeFile final : public SparseVortexRangeFileBase {
 ...
 private:
    ...
    WriteAtLocked(uint64_t offset, const arrow::Buffer& data) override {
        ...
        // MAP_SHARED writes can raise SIGBUS when either file-backed storage
        // or an anonymous memfd cannot allocate backing pages. Write through
        // the fd for every mmap backing so allocation failures are returned as
        // an Arrow status before exposing the range through the mapping.
        ARROW_RETURN_NOT_OK(PWriteAll(fd_,
                                      data.data(),
                                      offset,
