...
ThrowVortexStatus(const arrow::Status& status,
    ...
    // TODO(local-format): Preserve structured milvus-storage errors after the
    // storage translator stops stringifying Cell-load failures. This requires
    // the cross-repository construction-site audit; a local IOError mapping
    // would also misclassify Vortex decode failures.
...
VortexColumnGroup::VortexColumnGroup(
    ...
    : num_fields_(logical_field_count) {
    ...
    AssertInfo(logical_field_count > 0,
               "vortex column group has no logical fields");
    ...
    for (const auto& file : files) {
        ...
        // TODO(local-format): Each source file currently retains its sparse
        // range-file descriptor for the ColumnGroup lifetime because footer
        // planning and later Cell fills share it. Add pooling and resource
        // accounting before targeting segments with very large file counts.
        ...
        state.footer_reader =
            std::make_shared<milvus_storage::vortex::VortexFooterReader>(
                ...
                file.file_size,
                file.footer_size);
        // TODO(local-format): Thread OpContext into footer and later remote
        // range reads after milvus-storage exposes context-aware APIs. The
        // current API only permits cancellation checks around, not during, IO.
        ...
        for (const auto& field_name : field_names) {
            ...
            if (field_planner->rows() != planner->rows()) {
                ThrowInfo(ErrorCode::DataFormatBroken,
                          "vortex field {} rows {} does not match column "
                          "group rows {} for file {}",
                          field_name,
                          field_planner->rows(),
                          planner->rows(),
            ...
            if (field_cells.size() != group_cells.size()) {
                ThrowInfo(ErrorCode::DataFormatBroken,
                          "vortex field {} cells {} does not match column "
                          "group cells {} for file {}",
                          field_name,
                          field_cells.size(),
                          group_cells.size(),
                          file.path);
            ...
            for (size_t cell_id = 0; cell_id < field_cells.size(); ++cell_id) {
                if (field_cells[cell_id].row_offset !=
                        group_cells[cell_id].row_offset ||
                    field_cells[cell_id].row_count !=
                        group_cells[cell_id].row_count) {
                    ThrowInfo(ErrorCode::DataFormatBroken,
                              "vortex field {} cell {} row range [{}, {}) "
                              "does not match column group [{}, {}) for "
                              "file {}",
                              field_name,
                              cell_id,
...
size_t
VortexColumnGroup::file_size() const {
    size_t bytes = 0;
    for (const auto& file : files_) {
        bytes += file.footer_reader->file_size();
    }
    return bytes;
}
