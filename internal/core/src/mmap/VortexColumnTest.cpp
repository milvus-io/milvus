...
TEST(VortexColumnTest, StickyScanReleasesOldFileBeforePinningNextFile) {
    auto schema = MakeSchema();
    auto properties =
        std::make_shared<milvus_storage::api::Properties>(MakeProperties());
    auto dir =
        std::filesystem::temp_directory_path() /
        ("milvus_vortex_sticky_pin_transition_test_" +
...
TEST(VortexColumnTest, MixedFileRepresentationsNormalizePerFile) {
    ...
    auto normalized_file =
        WriteVortexBatches((dir / "normalized.vx").string(),
                           normalized_schema,
                           *properties,
                           {normalized_batch});
    ...
    for (int64_t i = 0; i < timestamp_take->Size(); ++i) {
        taken_timestamps.emplace_back(timestamp_take->Get<int64_t>(i));
    }
    EXPECT_EQ(taken_timestamps,
    ...
    const int64_t normalized_file_offset[]{2};
    {
        ...
        EXPECT_EQ(direct_views.get().first,
                  (std::vector<std::string_view>{geometry_wkb[0],
                                                 geometry_wkb[1]}));
        EXPECT_EQ(normalized_views.get().first,
                  (std::vector<std::string_view>{geometry_wkb[2],
                                                 geometry_wkb[3]}));
    ...
    for (int64_t i = 0; i < geometry_take->Size(); ++i) {
        taken_geometries.emplace_back(
            geometry_take->Get<std::string_view>(i));
    }
