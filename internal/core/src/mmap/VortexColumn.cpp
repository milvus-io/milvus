...
class VortexTakeResult final : public ChunkedColumnInterface::TakeResult {
 public:
    ...
    IsValid(int64_t index) const override {
        CheckIndex(index);
        return !validity_ || validity_[index];
    }
 ...
 protected:
    ...
    FixedValueAt(int64_t index) const override {
        CheckIndex(index);
        AssertInfo(values_.kind == ScanValueKind::FixedWidth &&
 ...
 private:
    ...
    ReadData(int64_t position,
        ...
        VortexDataBatchReader data_reader(
            column_, *range, mode, value_kind_, std::move(reader));
...
VortexColumn::BulkValueAt(milvus::OpContext* op_ctx,
    ...
    AssertInfo(take->Size() == count,
               ...
               count);
    auto owned = take->GetOwn();
    AssertInfo(owned.values.kind == ScanValueKind::FixedWidth,
               "vortex bulk value expected fixed-width take, got {}",
               static_cast<int>(owned.values.kind));
...
VortexColumn::Take(milvus::OpContext* op_ctx,
    ...
    auto fill_primitive =
        [&]<typename ArrowArrayT, typename ValueT, typename ContainerT>(
            ...
            for (const auto& group : groups) {
                ...
                auto table =
                    normalize && !SupportsDirectDataScan(group.file_id)
    ...
    switch (data_type_) {
        ...
        case DataType::GEOMETRY: {
            ...
            for (const auto& group : groups) {
                ...
                AssertInfo(
                    unique_views.size() == group.unique_offsets.size(),
        ...
        case DataType::ARRAY: {
            ...
            for (const auto& group : groups) {
                ...
                auto serialized =
                    BuildStringViewsFromArrow(column, std::nullopt);
                AssertInfo(
                    serialized.size() == group.unique_offsets.size(),
    ...
    if (values.kind == ScanValueKind::Default) {
        values.kind = ScanValueKind::FixedWidth;
    }
    const auto validity = IsNullable() ? owner->validity.View() : ValidityView{};
