...
ThrowVortexStatus(const arrow::Status& status,
    ...
    // TODO(local-format): Preserve the structured milvus-storage error code
    // through this boundary once the storage translator stops stringifying
    // Cell-load failures. This needs the cross-repository construction-site
    // audit; changing this fallback alone can misclassify decode IOError.
...
ScalarFieldProto::DataCase
ArrayProtoDataCase(DataType element_type) {
    switch (element_type) {
        case DataType::BOOL:
            return ScalarFieldProto::kBoolData;
        case DataType::INT8:
        case DataType::INT16:
        case DataType::INT32:
            return ScalarFieldProto::kIntData;
        case DataType::INT64:
            return ScalarFieldProto::kLongData;
        case DataType::FLOAT:
            return ScalarFieldProto::kFloatData;
        case DataType::DOUBLE:
            return ScalarFieldProto::kDoubleData;
        case DataType::STRING:
        case DataType::VARCHAR:
        case DataType::TEXT:
            return ScalarFieldProto::kStringData;
        default:
...
DataType
ArrayTypeSchemaElementType(const proto::schema::TypeSchema& type) {
    AssertInfo(type.has_array_element(),
               "ARRAY type schema is missing its element");
...
void
ValidateNestedArrayProto(const ScalarFieldProto& row,
    ...
    if (row.data_case() == ScalarFieldProto::DATA_NOT_SET) {
        if (!nullable) {
            ThrowInfo(ErrorCode::DataFormatBroken,
                      "non-nullable vortex ARRAY field {} contains a null "
                      "nested row",
                      field_id.get());
        }
        return;
    ...
    const auto element_type = ArrayTypeSchemaElementType(type);
    if (element_type != DataType::ARRAY) {
        const auto expected_case = ArrayProtoDataCase(element_type);
        if (row.data_case() != expected_case) {
            ThrowInfo(ErrorCode::DataFormatBroken,
                      "vortex ARRAY field {} expected proto case {}, got {}",
                      field_id.get(),
                      static_cast<int>(expected_case),
                      static_cast<int>(row.data_case()));
        }
        return;
    }

    if (row.data_case() != ScalarFieldProto::kArrayData) {
        ThrowInfo(ErrorCode::DataFormatBroken,
                  "vortex nested ARRAY field {} expected array proto, got "
                  "case {}",
                  field_id.get(),
                  static_cast<int>(row.data_case()));
    }
    const auto& child_type = type.array_element();
    const auto expected_child_type = static_cast<proto::schema::DataType>(
        ArrayTypeSchemaElementType(child_type));
    const auto& array_data = row.array_data();
    if (array_data.element_type() != proto::schema::DataType::None &&
        array_data.element_type() != expected_child_type) {
        ThrowInfo(ErrorCode::DataFormatBroken,
                  "vortex nested ARRAY field {} expected child type {}, got "
                  "{}",
                  field_id.get(),
                  expected_child_type,
                  array_data.element_type());
    }
    for (const auto& child : array_data.data()) {
        ValidateNestedArrayProto(
            child, child_type, child_type.nullable(), field_id);
    }
}

void
ValidateVortexArrayProto(const ScalarFieldProto& row,
                         const FieldMeta& field_meta) {
    if (field_meta.is_nested_array()) {
        ValidateNestedArrayProto(row,
    ...
    const auto expected_case =
        ArrayProtoDataCase(field_meta.get_element_type());
    if (row.data_case() != expected_case) {
        ThrowInfo(ErrorCode::DataFormatBroken,
                  "vortex ARRAY field {} expected proto case {}, got {}",
...
class VortexTakeResult final : public ChunkedColumnInterface::TakeResult {
 ...
 protected:
    ...
    JsonAt(int64_t index) const override {
        const auto& value =
            static_cast<const Json*>(values_.data)[values_.offset + index];
        return Json(static_cast<std::string_view>(value));
    }
...
class VortexDataBatchReader final {
 ...
 private:
    ...
    PrepareBatchArray(const std::shared_ptr<arrow::Array>& array) const {
        ...
        auto normalized =
            storage::NormalizeExternalArrow(array, column_->field_meta_, true);
        AssertInfo(
    ...
    FillStringLikeOutput(const std::shared_ptr<arrow::Array>& array,
        ...
        if (target_type_ == ChunkedColumnInterface::TargetType::StringView) {
            ...
            for (int64_t i = 0;
                 i < static_cast<int64_t>(owner->string_views.size());
                 ++i) {
                const auto value = owner->string_views[i];
                if (column_->IsNullable() && !string_column.IsValid(i)) {
                    owner->json_values.emplace_back();
                } else {
                    owner->json_values.emplace_back(
                        simdjson::padded_string(value.data(), value.size()));
                }
            }
    ...
    FillArrayOutput(const std::shared_ptr<arrow::Array>& array,
                    const std::shared_ptr<BatchOwner>& owner,
                    ChunkedColumnInterface::ScanBatch* out) const {
        // TODO(local-format): ARRAY Scan currently normalizes native List and
        // Milvus-written Binary to the same protobuf-per-row representation.
        // Return reader-backed native List views after writer/local-format
        // ARRAY representation is unified.
        ...
        if (target_type_ ==
            ...
            for (int64_t i = 0; i < out->size; ++i) {
                ...
                if (!proto.ParseFromArray(value.data(),
                ...
                ValidateVortexArrayProto(proto, column_->field_meta_);
            ...
            for (int64_t i = 0; i < out->size; ++i) {
                ...
                ValidateVortexArrayProto(proto, column_->field_meta_);
                owner->arrays.emplace_back(
                    proto, column_->field_meta_.is_element_nullable());
            }
            ...
            for (int64_t i = 0; i < out->size; ++i) {
                ...
                owner->array_views[i] =
                    ArrayView(const_cast<char*>(value.data()),
                              ...
                              value.get_element_valid_data(),
                              value.is_element_nullable(),
                              value.has_invalid_element());
            }
...
class VortexScanCursor final : public ChunkedColumnInterface::ScanCursor {
 public:
    VortexScanCursor(const VortexColumn* column,
        ...
        // TODO(local-format): Vortex intentionally leaves options.prefetch
        // unused in the initial integration. Add it only for scan paths whose
        // previous behavior or benchmarks justify widening the pin window.
 ...
 private:
    ...
    ReadData(int64_t position,
        ...
        // TODO(local-format): Vortex opens one range reader per Next() window
        // to preserve the current execution and pin lifetime. Reuse a reader
        // only after the cursor contract can guarantee it holds no decoded
        // data across windows and a benchmark shows the setup cost matters.
...
VortexColumn::VortexColumn(
    ...
    AssertInfo(!IsVectorDataType(data_type_),
               ...
               field_id_.get());
    AssertInfo(!field_name_.empty(),
               "vortex physical field name is empty for field {}",
               field_id_.get());
    ...
    data_byte_size_ = data_byte_size.value_or(column_group_->file_size());
...
VortexColumn::DataByteSize() const {
    // Vortex keeps the file representation and decodes on access, so this is
    // the attributed physical file size rather than decoded memory size.
    // TODO(local-format): Add a separate per-field decoded logical-size
    // estimate when storage metadata provides one; do not change this physical
    // file-size accounting contract to serve result-size estimation.
...
VortexColumn::IsValid(milvus::OpContext* op_ctx, size_t offset) const {
    ...
    const auto segment_offset = static_cast<int64_t>(offset);
    bool valid = false;
    BulkIsValid(
        op_ctx,
        [&valid](bool is_valid, size_t) { valid = is_valid; },
        &segment_offset,
        1);
    return valid;
}
...
VortexColumn::BulkIsValid(milvus::OpContext* op_ctx,
    ...
    if (offsets == nullptr) {
        // TODO(local-format): The legacy complete-column validity API still
        // materializes one temporary Chunk per file. Move its remaining
        // consumers to validity-only Scan before removing this compatibility
        // path; explicit sparse offsets already use Take below.
    ...
    for (auto& [chunk_id, indices] : indices_by_chunk) {
        std::sort(
            indices.begin(), indices.end(), [&](int64_t left, int64_t right) {
                return std::tie(offsets_in_chunk[left], left) <
                       std::tie(offsets_in_chunk[right], right);
            });
        std::vector<int64_t> unique_offsets;
        std::vector<std::vector<int64_t>> original_positions;
        unique_offsets.reserve(indices.size());
        original_positions.reserve(indices.size());
        for (const auto index : indices) {
            if (unique_offsets.empty() ||
                unique_offsets.back() != offsets[index]) {
                unique_offsets.emplace_back(offsets[index]);
                original_positions.emplace_back();
            }
            original_positions.back().emplace_back(index);
        }

        auto table = TakeArrowFromFile(op_ctx, chunk_id, unique_offsets);
        int64_t unique_index = 0;
        for (const auto& array : table->column(0)->chunks()) {
            for (int64_t i = 0; i < array->length(); ++i) {
                const auto valid = array->IsValid(i);
                for (const auto original_position :
                     original_positions[unique_index]) {
                    fn(valid, original_position);
                }
                ++unique_index;
            }
        }
        AssertInfo(unique_index == static_cast<int64_t>(unique_offsets.size()),
                   "vortex validity take returned {} rows, expected {}",
                   unique_index,
                   unique_offsets.size());
    }
}
...
VortexColumn::VectorArrayViews(
    ...
    // TODO(local-format): Vortex storage supports List<FixedSizeBinary>, but
    // vector fields are outside the initial Vortex local-format scope. Add
    // VECTOR_ARRAY together with the native ARRAY representation work.
    ThrowInfo(ErrorCode::Unsupported,
...
VortexColumn::StringViewsByOffsets(milvus::OpContext* op_ctx,
    ...
    if (data_type_ == DataType::JSON) {
        ...
        const auto* values = owned.values.data_as<Json>();
        for (size_t i = 0; i < offsets.size(); ++i) {
            const auto valid = !owned.validity || owned.validity[i];
            if (valid) {
                views.first[i] = std::string_view(values[i]);
            }
            views.second[i] = valid;
        }
        return PinWrapper<
            std::pair<std::vector<std::string_view>, FixedVector<bool>>>(
...
VortexColumn::BuildFileState(
    ...
    if (data_type_ == DataType::ARRAY && !field_meta_.is_nested_array() &&
        (arrow_field->type()->id() == arrow::Type::LIST ||
         arrow_field->type()->id() == arrow::Type::LARGE_LIST ||
         arrow_field->type()->id() == arrow::Type::LIST_VIEW)) {
        const auto list_type =
            std::static_pointer_cast<arrow::BaseListType>(arrow_field->type());
        if (!storage::IsCompatibleArrayElementArrowType(
                list_type->value_type(), field_meta_.get_element_type())) {
            ThrowInfo(ErrorCode::DataFormatBroken,
                      "vortex file {} field {} ARRAY element type {} is "
                      "incompatible with {}",
                      group_file.path,
                      field_id_.get(),
...
VortexColumn::MaterializeChunk(
    ...
    int64_t chunk_id,
    std::optional<std::pair<int64_t, int64_t>> offset_len) const {
    // TODO(local-format): This is a phase-1 compatibility bridge for sealed
    // consumers that still require Span/DataOfChunk/*Views. Delete it after
    // those callers move to Scan/Take and the public Column interface no
    // longer exposes Chunk-shaped access.
    CheckChunkId(chunk_id);
    const auto chunk_rows = Planner().CellRows(chunk_id);
    int64_t start = 0;
    ...
    for (const auto& array : arrays) {
        ...
        decoded_rows += array->length();
    }
    if (decoded_rows != length) {
        ThrowInfo(ErrorCode::DataFormatBroken,
                  "vortex materialize field {} chunk {} returned {} rows, "
                  "expected {}",
                  field_id_.get(),
                  chunk_id,
                  decoded_rows,
    ...
    if (data_type_ == DataType::ARRAY) {
        for (const auto& array : arrays) {
            auto binary = std::dynamic_pointer_cast<arrow::BinaryArray>(array);
            if (binary == nullptr) {
                ThrowInfo(ErrorCode::DataFormatBroken,
                          "vortex ARRAY field {} normalized to unexpected "
                          "Arrow type {}",
                          field_id_.get(),
                          array->type()->ToString());
            }
            for (int64_t i = 0; i < binary->length(); ++i) {
                ...
                const auto value = binary->GetView(i);
                ScalarFieldProto proto;
                if (!proto.ParseFromArray(value.data(),
                                          static_cast<int>(value.size()))) {
                    ThrowInfo(ErrorCode::DataFormatBroken,
                              "failed to parse vortex ARRAY field {} row",
                              field_id_.get());
                }
                ValidateVortexArrayProto(proto, field_meta_);
            }
        }
    }
...
VortexColumn::Take(milvus::OpContext* op_ctx, TakeOptions options) const {
    ...
    for (const auto& entry : entries) {
        ...
        AssertInfo(file_id < files.size() &&
                   ...
                   NumRows());
        if (planned_cell_skipped &&
            (!IsNullable() || !options.filter->SkippedValidityRequired())) {
            continue;
        }
        if (groups.empty() ||
            groups.back().file_id != static_cast<int64_t>(file_id) ||
            groups.back().validity_only != planned_cell_skipped) {
            groups.emplace_back(OffsetGroup{static_cast<int64_t>(file_id),
                                            planned_cell_skipped,
                                            {},
                                            {},
                                            {}});
    ...
    auto normalize_table = [&](const std::shared_ptr<arrow::Table>& table) {
        ...
        for (const auto& array : table->column(0)->chunks()) {
            arrays.emplace_back(
                storage::NormalizeExternalArrow(array, field_meta_, true));
        }
    ...
    auto fill_primitive =
        [&]<typename ArrowArrayT, typename ValueT, typename ContainerT>(
            ...
            for (const auto& group : groups) {
                ...
                if (group.validity_only) {
                    copy_validity(take, group);
                    continue;
                }
    ...
    switch (data_type_) {
        ...
        case DataType::GEOMETRY: {
            ...
            for (const auto& group : groups) {
                ...
                owner->tables.emplace_back(std::move(table));
            }
            if (data_type_ == DataType::JSON) {
                owner->json_values.reserve(owner->string_views.size());
                for (int64_t i = 0;
                     ...
                     ++i) {
                    const auto value = owner->string_views[i];
                    if ((has_data_skipped && owner->data_skipped.Get(i)) ||
                        (IsNullable() && !owner->validity.Get(i))) {
                        owner->json_values.emplace_back();
                    } else {
                        owner->json_values.emplace_back(simdjson::padded_string(
        ...
        case DataType::ARRAY: {
            // TODO(local-format): Like Scan, ARRAY Take currently parses the
            // normalized protobuf Binary rows. Replace this with owned or
            // reader-backed native List results when the writer representation
            // is unified.
                ...
                for (size_t i = 0; i < group.unique_offsets.size(); ++i) {
                    ...
                    if (valid) {
                        ...
                        if (target_type == TargetType::ArrayValueView) {
                            ...
                            owner->arrays.emplace_back(
                                proto, field_meta_.is_element_nullable());
                        }
...
VortexColumn::BulkRawJsonAt(milvus::OpContext* op_ctx,
    ...
    if (offsets == nullptr) {
        BulkStringLikeAt(
            ...
            [&](std::string_view value, size_t index, bool valid) {
                fn(valid ? Json(simdjson::padded_string(value.data(),
                                                        value.size()))
                         : Json{},
                   index,
                   valid);
    ...
    auto take =
        Take(op_ctx,
             TakeOptions{OffsetView::From(offsets, count), TargetType::Json});
    AssertInfo(take != nullptr,
               "vortex JSON take is unsupported for type {}",
               data_type_);
    ...
    if (offsets == nullptr) {
        ...
        for (int64_t chunk_id = 0; chunk_id < num_chunks(); ++chunk_id) {
            ...
            if (!SupportsDirectDataScan(chunk_id)) {
                ...
                if (row_offset != file_rows) {
                    ThrowInfo(
                        ErrorCode::DataFormatBroken,
                        "vortex string-like scan field {} chunk {} returned "
                        "{} rows, expected {}",
                        field_id_.get(),
                        chunk_id,
                        row_offset,
