...
class ChunkedColumnBase : public ChunkedColumnInterface {
 public:
    ...
    DataOfChunk(int chunk_id) const override {
        ...
        auto ca = SemiInlineGet(slot_->PinCells(ctx, {chunk_id}));
    ...
    IsValid(size_t offset) const override {
        ...
        milvus::OpContext ctx;
        auto ca =
            SemiInlineGet(slot_->PinCells(ctx, {static_cast<cid_t>(chunk_id)}));
        auto chunk = ca->get_cell_of(chunk_id);
    ...
    BulkIsValid(std::function<void(bool, size_t)> fn,
        ...
        if (offsets == nullptr) {
            milvus::OpContext ctx;
            auto ca = SemiInlineGet(slot_->PinAllCells(ctx));
            for (int64_t i = 0; i < num_rows_; i++) {
            ...
            milvus::OpContext ctx;
            auto ca = SemiInlineGet(slot_->PinCells(ctx, cids));
            for (int64_t i = 0; i < count; i++) {
...
class ChunkedColumn : public ChunkedColumnBase {
 public:
    ...
    BulkValueAt(std::function<void(const char*, size_t)> fn,
        ...
        auto ca = SemiInlineGet(slot_->PinCells(op_context, cids));
        for (int64_t i = 0; i < count; i++) {
