use tantivy_5::tokenizer::{TextAnalyzer, Tokenizer, TokenStream, Token};
...
impl<'a> AdapterTokenStream<'a> {
    fn new(token_stream: NewBoxTokenStream<'a>) -> Self {
        Self { token_stream, token: Token::default() }
    }
}
...
impl<'a> TokenStream for AdapterTokenStream<'a> {
    fn advance(&mut self) -> bool {
        if self.token_stream.advance() {
            ...
            self.token = Token { offset_from: new_token.offset_from, offset_to: new_token.offset_to, position: new_token.position, text: new_token.text.clone(), position_length: new_token.position_length };
            true
        } else {
            false
        }
