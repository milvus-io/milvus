...
use tantivy::{schema::Value, tokenizer::{Token, TokenStream, Tokenizer}};
    ...
    match params.get("dict") {
        Some(value)=>{
            ...
            for word in value.as_array().unwrap(){
                ...
                dict.push(word.as_str().unwrap().to_string());
            }
            Ok((dict, true))
         },
        _=>{
...
impl<'a> JiebaTokenizer<'a> {
    pub fn new() -> JiebaTokenizer<'a>{
        JiebaTokenizer{mode: JiebaMode::Search, hmm: true, tokenizer: Cow::Borrowed(&JIEBA)}
    }
    ...
    pub fn from_json(params: &json::Map<String, json::Value>) -> Result<JiebaTokenizer<'a>>{
        ...
        // let tokenizer = jieba_rs::Jieba::new();
        Ok(JiebaTokenizer{mode: mode, hmm: true, tokenizer: Cow::Owned(tokenizer)})
    }
    ...
    fn tokenize(&self, text: &str) -> Vec<Token>{
        ...
        let ori_tokens = match self.mode{
            JiebaMode::Exact => {
                self.tokenizer.tokenize(text, jieba_rs::TokenizeMode::Default, self.hmm)
            },
            JiebaMode::Search => {
                self.tokenizer.tokenize(text, jieba_rs::TokenizeMode::Search, self.hmm)
            },
        };
