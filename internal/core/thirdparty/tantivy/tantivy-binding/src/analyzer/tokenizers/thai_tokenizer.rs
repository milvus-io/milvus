...
impl Clone for ThaiTokenizer {
    fn clone(&self) -> Self {
        ThaiTokenizer {
            segmenter: WordSegmenter::try_new_lstm(WordBreakOptions::default()).unwrap(),
        }
    }
}
...
/// Check if a string segment contains at least one word character
/// (alphanumeric or Thai script character).
fn is_word_segment(s: &str) -> bool {
    s.chars()
        .any(|c| c.is_alphanumeric() || ('\u{0E01}'..='\u{0E5B}').contains(&c))
}
    ...
    fn tokenize(&self, text: &str) -> Vec<Token> {
        ...
        for pair in breakpoints.windows(2) {
            ...
            tokens.push(Token {
                ...
                offset_to: end,
                position,
                position_length: segment.chars().count(),
            });
            position += 1;
        }
