...
// Test 1: Basic WriteLOB functionality
TEST_F(GrowingLOBTest, WriteLOB_Basic) {
    ...
    EXPECT_FALSE(ref.is_inline());
}

// Test 2: Basic ReadLOB functionality
...
// Test 3: Multiple writes - verify offset tracking
...
// Test 5: Large TEXT (greater than inline threshold)
...
// Test 7: LOBReference encode/decode
TEST_F(GrowingLOBTest, LOBReference_EncodeDecode) {
    LOBReference original = LOBReference::ForGrowing(123456, 789);

    auto encoded = original.encode();
    EXPECT_EQ(encoded.size(), 16);

    LOBReference decoded = LOBReference::decode(encoded);
    EXPECT_EQ(decoded.GetOffset(), original.GetOffset());
    EXPECT_EQ(decoded.GetSize(), original.GetSize());
}

// Test 8: Statistics for single field
...
// Test 9: Concurrent writes (thread safety)
...
// Test 10: Concurrent reads (thread safety)
...
// Test 13: is_inline() method
TEST_F(GrowingLOBTest, IsInline) {
    LOBReference inline_ref(0, 0, 0);
    EXPECT_TRUE(inline_ref.is_inline());

    LOBReference non_inline_ref = LOBReference::ForGrowing(0, 100);
    EXPECT_FALSE(non_inline_ref.is_inline());

    LOBReference non_inline_ref2 = LOBReference::ForGrowing(100, 0);
    EXPECT_FALSE(non_inline_ref2.is_inline());
}

// Test 14: File storage tracking
...
// Test 15: LOB file path with field_id
...
// Test 16: File persistence - data appended correctly
...
// Test 17: GetOffset and GetSize helpers
...
// Test 18: Multiple TEXT fields - data isolation
...
// Test 19: Concurrent writes to multiple fields
...
// Test 20: File path for different fields
