...
CStatus
CRetrieve(CSegmentInterface c_segment, CRetrievePlan c_plan, uint64_t timestamp, CRetrieveResult* result) {
    return Retrieve(c_segment, c_plan, timestamp, result, DEFAULT_MAX_OUTPUT_SIZE);
}
...
TEST(CApiTest, DeleteTest) {
    ...
    auto del_res = Delete(segment, 0, 3, delete_data.data(), delete_data.size(), delete_timestamps);
...
TEST(CApiTest, MultiDeleteGrowingSegment) {
    ...
    auto del_res = Delete(segment, 0, 1, delete_data.data(), delete_data.size(), delete_timestamps.data());
    assert(del_res.error_code == Success);
    ...
    del_res = Delete(segment, 0, 1, delete_data.data(), delete_data.size(), delete_timestamps.data());
    assert(del_res.error_code == Success);
...
TEST(CApiTest, DeleteRepeatedPksFromGrowingSegment) {
    ...
    CRetrieveResult retrieve_result;
    res = CRetrieve(segment, plan.get(), dataset.timestamps_[N - 1], &retrieve_result);
    ASSERT_EQ(res.error_code, Success);
    auto query_result = std::make_unique<proto::segcore::RetrieveResults>();
    auto suc = query_result->ParseFromArray(retrieve_result.proto_blob, retrieve_result.proto_size);
    ASSERT_TRUE(suc);
    ...
    auto del_res = Delete(segment, 0, 3, delete_data.data(), delete_data.size(), delete_timestamps.data());
    assert(del_res.error_code == Success);
...
TEST(CApiTest, DeleteRepeatedPksFromSealedSegment) {
    ...
    CRetrieveResult retrieve_result;
    auto res = CRetrieve(segment, plan.get(), dataset.timestamps_[N - 1], &retrieve_result);
    ASSERT_EQ(res.error_code, Success);
    auto query_result = std::make_unique<proto::segcore::RetrieveResults>();
    auto suc = query_result->ParseFromArray(retrieve_result.proto_blob, retrieve_result.proto_size);
    ASSERT_TRUE(suc);
...
TEST(CApiTest, RetrieveTestWithExpr) {
    ...
    CRetrieveResult retrieve_result;
    auto res = CRetrieve(segment, plan.get(), dataset.timestamps_[0], &retrieve_result);
    ASSERT_EQ(res.error_code, Success);
...
TEST(CApiTest, RetriveScalarFieldFromSealedSegmentWithIndex) {
    ...
    CRetrieveResult retrieve_result;
    res = CRetrieve(segment, plan.get(), raw_data.timestamps_[N - 1], &retrieve_result);
    ASSERT_EQ(res.error_code, Success);
    auto query_result = std::make_unique<proto::segcore::RetrieveResults>();
    auto suc = query_result->ParseFromArray(retrieve_result.proto_blob, retrieve_result.proto_size);
    ASSERT_TRUE(suc);
