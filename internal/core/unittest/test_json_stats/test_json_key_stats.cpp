...
class JsonKeyStatsTest : public ::testing::TestWithParam<bool> {
 protected:
    ...
    Init(int64_t collection_id,
        ...
        config[STATS_BASE_PATH_KEY] = stats_base_path;
...
TEST_F(JsonKeyStatsUploadLoadTest, StatsPathsRequireExplicitBase) {
    InitContext();
    storage::FileManagerContext ctx(
        field_meta_, index_meta_, chunk_manager_, fs_);
    storage::DiskFileManagerImpl missing_base(ctx);
    EXPECT_THROW(missing_base.GetRemoteJsonStatsLogPrefix(), SegcoreError);

    for (auto format : {JSON_STATS_DATA_FORMAT_V3, JSON_STATS_DATA_FORMAT_V4}) {
        auto base = storage::GenRemoteJsonStatsPathPrefix(chunk_manager_,
                                                          index_build_id_,
                                                          index_version_,
                                                          collection_id_,
                                                          partition_id_,
                                                          ...
                                                          field_id_,
                                                          format);
        ctx.set_stats_base_path(base);
        storage::DiskFileManagerImpl manager(ctx);
        EXPECT_EQ(manager.GetRemoteJsonStatsLogPrefix(), base);
    }
}
...
TEST_F(JsonKeyStatsUploadLoadTest, V4AcceptsRepresentableSubnormal) {
    InitContext();
...
TEST_F(JsonKeyStatsUploadLoadTest, V3RejectsUnrepresentableScalarNumber) {
    InitContext();
    // std::stod rejected these before V4. Preserve rejection, but return a
    // permanent error so the legacy writer cannot occupy a worker forever.
    for (const auto* number : {"1e400", "-1.48e-309", "1e-400"}) {
        SCOPED_TRACE(number);
        PrepareData({fmt::format(R"({{"value":{}}})", number)});
        try {
            BuildAndUpload(0, 1024, JSON_STATS_DATA_FORMAT_V3);
            FAIL() << "expected the legacy scalar conversion to reject "
                   << number;
...
TEST_F(JsonKeyStatsUploadLoadTest, V3RejectsUnrepresentableArrayElement) {
    ...
    try {
        BuildAndUpload(0, 1024, JSON_STATS_DATA_FORMAT_V3);
        FAIL() << "expected the legacy array parser to reject overflow";
    } catch (const SegcoreError& error) {
        EXPECT_EQ(error.get_error_code(), ErrorCode::DataFormatBroken);
    }
}

TEST_F(JsonKeyStatsUploadLoadTest, NumericWritesFollowTheirFormatAfterReload) {
    for (auto format : {JSON_STATS_DATA_FORMAT_V3, JSON_STATS_DATA_FORMAT_V4}) {
        for (int64_t max_columns : {0, 1024}) {
            SCOPED_TRACE(
                fmt::format("format={}, columns={}", format, max_columns));
            ++index_build_id_;
            InitContext();
            PrepareData({R"({"value":18446744073709551616,"ok":7})",
                         R"({"value":1.5,"ok":8})"});
            ASSERT_NO_THROW(BuildAndUpload(0, max_columns, format));
            ASSERT_NO_THROW(Load());
            EXPECT_EQ(load_index_->GetDataFormatVersion(), format);
            const bool legacy = format == JSON_STATS_DATA_FORMAT_V3;
            const double legacy_value = std::stod("18446744073709551616");
            TargetBitmap seen(data_.size());
            if (max_columns == 0) {
                EXPECT_TRUE(load_index_->GetShreddingFields("/value").empty());
                PinWrapper<BsonInvertedIndex*> bson_index{nullptr};
                load_index_->ExecuteForSharedData(
                    nullptr,
                    bson_index,
                    "/value",
                    [&](BsonView bson, uint32_t row, uint32_t offset) {
                        auto value = bson.ParseAsValueAtOffset<double>(offset);
                        ASSERT_TRUE(value.has_value());
                        seen[row] = true;
                        EXPECT_DOUBLE_EQ(value.value(),
                                         row == 0 ? legacy_value : 1.5);
                    });
                TargetBitmap siblings(data_.size());
                load_index_->ExecuteForSharedData(
                    nullptr,
                    bson_index,
                    "/ok",
                    [&](BsonView bson, uint32_t row, uint32_t offset) {
                        ...
                        ASSERT_TRUE(value.has_value());
                        EXPECT_EQ(value.value(), 7 + row);
                        siblings[row] = true;
                    });
                EXPECT_EQ(siblings.count(), data_.size());
            } else {
                auto fields = load_index_->GetShreddingFields(
                    "/value", std::vector<JSONType>{JSONType::DOUBLE});
                ASSERT_EQ(fields.size(), 1);
                TargetBitmap matches(data_.size());
                auto check_values = [&](const double* values,
                                        ValidityView valid,
                                        int size,
                                        TargetBitmapView result,
                    ...
                    for (int i = 0; i < size; ++i) {
                        ...
                        if (valid[i]) {
                            EXPECT_DOUBLE_EQ(values[i],
                                             i == 0 ? legacy_value : 1.5);
                        }
                    }
                };
                EXPECT_EQ(load_index_->ExecutorForShreddingData<double>(
                              nullptr,
                              *fields.begin(),
                              check_values,
                              nullptr,
                              TargetBitmapView(matches),
                              TargetBitmapView(seen)),
                          data_.size());
            }
            EXPECT_EQ(seen[0], legacy);
            EXPECT_TRUE(seen[1]);
        }
    }
}
...
TEST_F(JsonKeyStatsUploadLoadTest, V4EncodesInvalidNumbersAsNull) {
    ...
    load_index_->ExecuteForSharedData(
        ...
        [&](BsonView bson, uint32_t row_id, uint32_t offset) {
            EXPECT_EQ(row_id, 0);
            EXPECT_FALSE(bson.IsBsonValuePresentForExists(offset));
