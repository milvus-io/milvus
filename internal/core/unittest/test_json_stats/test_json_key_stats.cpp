...
class JsonStatsMetaFileSystem : public arrow::fs::SubTreeFileSystem {
 public:
    ...
    OpenInputFileAsync(const std::string& path) override {
        ...
        auto pending = io_context().executor()->Submit(
            [this, path] { return OpenInputFile(path); });
        if (!pending.ok())
            return arrow::Future<std::shared_ptr<arrow::io::RandomAccessFile>>::
                MakeFinished(pending.status());
        return *pending;
    }
...
TEST(JsonStatsAsyncFileInput, GenericSizeDoesNotBlockLoadWorker) {
    ...
    class SizeFileSystem : public arrow::fs::SubTreeFileSystem {
     public:
        explicit SizeFileSystem(arrow::io::RandomAccessFile& file)
            : SubTreeFileSystem("",
                                std::make_shared<arrow::fs::LocalFileSystem>()),
              file_(&file, [](auto*) {}) {
        }
    ...
    auto fs = std::make_shared<SizeFileSystem>(file);
    ...
    auto open = [&]() -> folly::coro::Task<void> {
        co_await storage::RemoteInputStream::OpenAsync(fs, "metadata");
        storage::ThrowIfCancelled(cancel.getToken(), "test metadata open");
    };
    auto pending = std::async(std::launch::async, [&] {
        return folly::coro::blockingWait(open().scheduleOn(executor));
    });
    ...
    class FailingSizeFile : public milvus::test::AsyncTrackingRandomAccessFile {
     public:
        FailingSizeFile() : AsyncTrackingRandomAccessFile({}) {
        }
        arrow::Result<int64_t>
        GetSize() override {
            throw SegcoreError(MemAllocateFailed, "injected size exception");
        }
    } failing;
    auto failing_fs = std::make_shared<SizeFileSystem>(failing);
    try {
        folly::coro::blockingWait(
            storage::RemoteInputStream::OpenAsync(failing_fs, "metadata")
                .scheduleOn(executor));
        FAIL() << "size exception must propagate across the I/O executor";
    } catch (const SegcoreError& error) {
        ...
        if (outcome == Outcome::Stream) {
            ...
            try {
                ...
                const auto expected =
                    ...
                    : outcome == Outcome::InvalidJson   ? DataFormatBroken
                    : outcome == Outcome::ShortRead     ? FileReadFailed
                                                        : UnexpectedError;
