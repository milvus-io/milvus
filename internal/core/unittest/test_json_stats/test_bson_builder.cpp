...
TEST_F(BsonBuilderTest, CreateValueNodeTest) {
    ...
    // Test INT32 type
    auto int32_node = builder.CreateValueNode("42", JSONType::INT32);
    EXPECT_EQ(int32_node.type, DomNode::Type::VALUE);
    EXPECT_TRUE(int32_node.bson_value.value().view().type() ==
                bsoncxx::type::k_int32);
    EXPECT_EQ(int32_node.bson_value.value().view().get_int32().value, 42);
    ...
    EXPECT_EQ(array_view[0].get_int32().value, 1);
    EXPECT_EQ(array_view[1].get_int32().value, 2);
    EXPECT_EQ(array_view[2].get_int32().value, 3);
...
TEST_F(BsonBuilderTest, AppendToDomTest) {
    ...
    builder.AppendToDom(
        root, {"level1", "level2", "key2"}, "42", JSONType::INT32);
...
TEST_F(BsonBuilderTest, ConvertDomToBsonTest) {
    ...
    builder.AppendToDom(root, {"int_field"}, "42", JSONType::INT32);
    ...
    EXPECT_EQ(bson_view["int_field"].get_int32().value, 42);
...
TEST_F(BsonBuilderTest, ComplexDocumentTest) {
    ...
    builder.AppendToDom(root, {"user", "age"}, "30", JSONType::INT32);
    builder.AppendToDom(
    ...
    EXPECT_EQ(bson_view["user"]["age"].get_int32().value, 30);
    ...
    EXPECT_EQ(scores_array[0].get_int32().value, 85);
    EXPECT_EQ(scores_array[1].get_int32().value, 90);
    EXPECT_EQ(scores_array[2].get_int32().value, 95);
}
