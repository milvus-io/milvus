...
TEST_F(UtilsTest, CreateParquetKVMetadataTest) {
    ...
    EXPECT_EQ(metadata.size(), 2);  // key_map and layout_type

    // Parse and verify key_map
    auto key_map_json = nlohmann::json::parse(metadata[0].second);
    for (const auto& [key, type] : column_map) {
        std::string key_with_type = key.key_ + "_" + ToString(key.type_);
        EXPECT_TRUE(key_map_json.contains(key_with_type));
        EXPECT_EQ(key_map_json[key_with_type], key.key_);
    }

    // Parse and verify layout_type
    auto layout_type_json = nlohmann::json::parse(metadata[1].second);
    for (const auto& [key, type] : column_map) {
        std::string key_with_type = key.key_ + "_" + ToString(key.type_);
