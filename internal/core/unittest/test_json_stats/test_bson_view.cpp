...
TEST_F(BsonViewTest, ParseAsDocumentAtOffsetTest) {
    // Test case 1: offset = 0 (whole document)
    {
        bsoncxx::builder::basic::document doc;
        doc.append(bsoncxx::builder::basic::kvp("field1", "value1"));
        doc.append(bsoncxx::builder::basic::kvp("field2", 42));

        BsonView view(doc.view().data(), doc.view().length());
        auto doc_view = view.ParseAsDocumentAtOffset(0);

        // Verify document content
        EXPECT_TRUE(doc_view.find("field1") != doc_view.end());
        EXPECT_TRUE(doc_view.find("field2") != doc_view.end());
        EXPECT_STREQ(doc_view["field1"].get_string().value.data(), "value1");
        EXPECT_EQ(doc_view["field2"].get_int32(), 42);
    }

    // Test case 2: offset != 0 (nested document)
    {
        bsoncxx::builder::basic::document nested_doc;
        nested_doc.append(
            bsoncxx::builder::basic::kvp("nested_field1", "nested_value1"));
        nested_doc.append(bsoncxx::builder::basic::kvp("nested_field2", 123));

        bsoncxx::builder::basic::document doc;
        doc.append(bsoncxx::builder::basic::kvp("top_field", "top_value"));
        doc.append(bsoncxx::builder::basic::kvp("nested", nested_doc));

        BsonView view(doc.view().data(), doc.view().length());

        // Find the offset of the nested document
        size_t nested_offset = 0;
        auto offsets = BsonBuilder::ExtractBsonKeyOffsets(doc.view());
        for (const auto& offset : offsets) {
            if (offset.first == "/nested") {
                nested_offset = offset.second;
                break;
            }
        }

        std::cout << "nested_offset: " << nested_offset << std::endl;
        std::cout << "nested_view: " << view.ToString() << std::endl;
        std::cout << view.ToString() << std::endl;
        auto nested_view = view.ParseAsDocumentAtOffset(nested_offset);

        // Verify nested document content
        EXPECT_TRUE(nested_view.find("nested_field1") != nested_view.end());
        EXPECT_TRUE(nested_view.find("nested_field2") != nested_view.end());
        EXPECT_STREQ(nested_view["nested_field1"].get_string().value.data(),
                     "nested_value1");
        EXPECT_EQ(nested_view["nested_field2"].get_int32(), 123);
    }
    ...
    // Test case 4: Error cases
    {
        ...
        EXPECT_THROW(view.ParseAsDocumentAtOffset(1000), std::runtime_error);
    }
}
