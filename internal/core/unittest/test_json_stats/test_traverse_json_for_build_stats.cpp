...
TEST(TraverseJsonForBuildStatsTest, MalformedStructureRetainsParseErrorCode) {
    milvus::storage::FieldDataMeta field_meta{1, 2, 3, 100, {}};
    milvus::storage::IndexMeta index_meta{3, 100, 1, 1};
    milvus::storage::StorageConfig storage_config;
    storage_config.storage_type = "local";
    ...
    milvus::storage::FileManagerContext ctx(field_meta, index_meta, cm, fs);
    JsonKeyStats stats(ctx, true);
    stats.SetDataFormatVersion(JSON_STATS_DATA_FORMAT_V4);

    const char* json = R"({"a":{1:2}})";
    std::map<JsonKey, milvus::index::KeyStatsInfo> infos;
    try {
        CollectSingleJsonStatsInfoAccessor::Call(stats, json, infos);
        FAIL() << "expected a classified sampling error";
    } catch (const milvus::SegcoreError& error) {
        EXPECT_EQ(error.get_error_code(), milvus::ErrorCode::DataFormatBroken);
    }
    std::vector<std::string> path;
    std::map<JsonKey, JsonStatsBuildValue> values;
    try {
        TraverseJsonForBuildStatsAccessor::Call(stats, json, path, values);
        FAIL() << "expected a classified build error";
    } catch (const milvus::SegcoreError& error) {
        EXPECT_EQ(error.get_error_code(), milvus::ErrorCode::DataFormatBroken);
    }
}
...
TEST(TraverseJsonForBuildStatsTest, V3KeepsLegacyScalarNumberConversion) {
    milvus::storage::FieldDataMeta field_meta{1, 2, 3, 100, {}};
    milvus::storage::IndexMeta index_meta{3, 100, 1, 1};
    milvus::storage::StorageConfig storage_config;
    storage_config.storage_type = "local";
    ...
    ASSERT_NO_THROW(TraverseJsonForBuildStatsAccessor::Call(
        stats, R"({"number":18446744073709551616})", path, values));
    const auto& value = values.at(JsonKey{"/number", JSONType::DOUBLE});
    EXPECT_FALSE(value.IsUnrepresentableNumber());
    EXPECT_DOUBLE_EQ(value.parsed_double.value(),
                     std::stod("18446744073709551616"));

    std::map<JsonKey, JsonStatsBuildValue> root_values;
    ASSERT_NO_THROW(TraverseJsonForBuildStatsAccessor::Call(
        stats, "18446744073709551616", path, root_values));
    EXPECT_DOUBLE_EQ(
        root_values.at(JsonKey{"", JSONType::DOUBLE}).parsed_double.value(),
        value.parsed_double.value());
    std::map<JsonKey, milvus::index::KeyStatsInfo> infos;
    ASSERT_NO_THROW(CollectSingleJsonStatsInfoAccessor::Call(
        stats, R"({"number":18446744073709551616})", infos));
    EXPECT_NE(infos.find(JsonKey{"/number", JSONType::DOUBLE}), infos.end());

    for (const auto* json :
         {R"({"number":1e400})", R"({"number":-1.48e-309})"}) {
        SCOPED_TRACE(json);
        path.clear();
        values.clear();
        try {
            TraverseJsonForBuildStatsAccessor::Call(stats, json, path, values);
            FAIL() << "expected legacy numeric conversion to reject the value";
        } catch (const milvus::SegcoreError& error) {
