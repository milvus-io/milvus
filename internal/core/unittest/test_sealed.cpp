...
TEST(Sealed, LoadScalarIndex) {
    ...
    segment->LoadFieldData(RowFieldID, std::vector<milvus::storage::FieldDataPtr>{field_data});

    LoadFieldDataInfo ts_info;
    FieldMeta ts_field_meta(FieldName("Timestamp"), TimestampFieldID, DataType::INT64);
    field_data = std::make_shared<milvus::storage::FieldData<int64_t>>(DataType::INT64);
    field_data->FillFieldData(dataset.timestamps_.data(), N);
    segment->LoadFieldData(TimestampFieldID, std::vector<milvus::storage::FieldDataPtr>{field_data});
