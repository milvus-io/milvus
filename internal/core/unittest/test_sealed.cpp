...
// Regression: a reopen that replaces existing row data (compaction rewriting
// a column's parquet, an external-manifest refresh, ...) must drop this
// segment's process-level filter-bitset cache. The cache key carries no data
// generation, so without invalidation a same-id / same-row-count replacement
// would keep serving the pre-reopen generation's bitmap. A reopen that only
// adds new fields must NOT drop the still-correct cache.
TEST(SealedSegmentCowState, ReopenReplacingDataInvalidatesExprCache) {
    using milvus::exec::ExprResCacheManager;

    auto schema = std::make_shared<Schema>();
    auto pk = schema->AddDebugField("pk", DataType::INT64);
    schema->AddDebugField("payload", DataType::INT64);
    schema->set_primary_field_id(pk);

    const int64_t seg_id = 4242001;
    auto segment = CreateSealedSegment(schema, empty_index_meta, seg_id);
    auto* sealed = dynamic_cast<ChunkedSegmentSealedImpl*>(segment.get());
    ASSERT_NE(sealed, nullptr);
    ...
    auto tmpdir = std::filesystem::temp_directory_path() /
                  ("expr_cache_reopen_" + std::to_string(getpid()));
    std::filesystem::create_directories(tmpdir);
    mgr.SetDiskConfig(tmpdir.string(), 1ULL << 20, 1ULL << 20, true);

    ExprResCacheManager::Key key{seg_id, "payload > 0"};
    ExprResCacheManager::Value v;
    v.result = std::make_shared<milvus::TargetBitmap>(128);
    v.valid_result = std::make_shared<milvus::TargetBitmap>(128);
    v.active_count = 128;
    mgr.Put(key, v);
    ...
    auto is_cached = [&]() {
        ...
        return mgr.Get(key, got);
    };
    ASSERT_TRUE(is_cached());

    // Add-only reopen: cache stays (existing rows' values unchanged).
    LoadDiff add_only;
    add_only.fields_to_fill_default.push_back(FieldId(999));
    sealed->TestInvalidateExprCacheAfterReopen(add_only);
    EXPECT_TRUE(is_cached());

    // Data-replacing reopen: cache for this segment is dropped.
    LoadDiff replace;
    replace.manifest_updated = true;
    sealed->TestInvalidateExprCacheAfterReopen(replace);
...
// The invalidation must be atomic with the publication w.r.t. reader
// admission: it runs as the committer.Publish while_locked hook, after the new
// state is stored but *before* the publish lease reopens the read gate. If it
// ran after the lease released, a new query could read the freshly published
// (replaced) data and still hit the pre-reopen bitmap in the gap. This test
// pins that ordering: while the hook runs, a new read lease must be refused.
TEST(SealedSegmentCowState, ExprCacheInvalidationOrderedBeforeReaderAdmission) {
    using milvus::exec::ExprResCacheManager;
    ...
    mgr.Clear();
    auto tmpdir = std::filesystem::temp_directory_path() /
                  ("expr_cache_order_" + std::to_string(getpid()));
    std::filesystem::create_directories(tmpdir);
    mgr.SetDiskConfig(tmpdir.string(), 1ULL << 20, 1ULL << 20, true);
    ...
    LoadDiff replace;
    replace.manifest_updated = true;
    bool reader_blocked_during_erase =
        sealed->TestInvalidateExprCacheDuringPublish(replace);
    EXPECT_TRUE(reader_blocked_during_erase);
    EXPECT_FALSE(is_cached());
...
// Disabling the cache only flips the switch (SetEnabled) — it does NOT drop
// existing entries. A data-replacing reopen that happens while the cache is
// disabled must still erase the segment's stale bitsets; otherwise they
// resurface as valid once the cache is re-enabled. So the invalidation must
// NOT be gated on ExprResCacheManager::IsEnabled().
TEST(SealedSegmentCowState, ExprCacheInvalidatedEvenWhenDisabledDuringReopen) {
    using milvus::exec::ExprResCacheManager;
    ...
    mgr.Clear();
    auto tmpdir = std::filesystem::temp_directory_path() /
                  ("expr_cache_disabled_" + std::to_string(getpid()));
    std::filesystem::create_directories(tmpdir);
    mgr.SetDiskConfig(tmpdir.string(), 1ULL << 20, 1ULL << 20, true);
    ...
    // Turn the cache off (entry stays), do a data-replacing reopen, turn it
    // back on. The stale entry must be gone.
    ExprResCacheManager::SetEnabled(false);
    LoadDiff replace;
    replace.manifest_updated = true;
    sealed->TestInvalidateExprCacheAfterReopen(replace);
