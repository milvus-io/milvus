...
TEST_F(DiskAnnFileManagerTest, AddFilePositive) {
    ...
    for (auto& file : local_files) {
        ...
        auto index = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::INT8);
    ...
    auto objects = rcm->ListWithPrefix(diskAnnFileManager->GetRemoteIndexObjectPrefix());
