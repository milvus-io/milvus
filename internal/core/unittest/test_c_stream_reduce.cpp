...
TEST(CApiTest, StreamReduce) {
    ...
    auto status = NewSegment(collection, Growing, -1, &segment_1, false, nullptr);
    ASSERT_EQ(status.error_code, Success);
    CSegmentInterface segment_2;
    status = NewSegment(collection, Growing, -1, &segment_2, false, nullptr);
    ASSERT_EQ(status.error_code, Success);
...
TEST(CApiTest, StreamReduceGroupBY) {
    ...
    auto status = NewSegment(c_collection, Growing, -1, &segment, false, nullptr);
    ASSERT_EQ(status.error_code, Success);
