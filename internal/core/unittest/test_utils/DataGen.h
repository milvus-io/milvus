...
struct GeneratedData {
 ...
 private:
    ...
    friend GeneratedData
    DataGenForJsonArray(
        SchemaPtr schema, int64_t N, uint64_t seed, uint64_t ts_offset, int repeat_count, int array_len);
};
...
inline GeneratedData
DataGenForJsonArray(
    SchemaPtr schema, int64_t N, uint64_t seed = 42, uint64_t ts_offset = 0, int repeat_count = 1, int array_len = 1) {
    using std::vector;
    std::default_random_engine er(seed);
    std::normal_distribution<> distr(0, 1);
            ...
            case DataType::JSON: {
                ...
                for (int i = 0; i < N / repeat_count; i++) {
                    ...
                    for (int i = 0; i < array_len; ++i) {
                        ...
                        doubleVec.push_back(std::to_string(static_cast<double>(er())));
                        stringVec.push_back("\"" + std::to_string(er()) + "\"");
                        boolVec.push_back(i % 2 == 0 ? "true" : "false");
                    }
                    auto str = R"({"int":[)" + join(intVec, ",") + R"(],"double":[)" + join(doubleVec, ",") +
                               R"(],"string":[)" + join(stringVec, ",") + R"(],"bool": [)" + join(boolVec, ",") + "]}";
