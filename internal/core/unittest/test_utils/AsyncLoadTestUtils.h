...
class ScopedLoadTransientBudget {
 public:
    explicit ScopedLoadTransientBudget(size_t capacity_bytes)
        : budget_(storage::LoadAdmissionController::GetInstance()),
...
class AsyncTrackingRandomAccessFile : public arrow::io::RandomAccessFile {
 public:
    ...
    arrow::Result<int64_t>
    GetSize() override {
        size_calls_.fetch_add(1);
        return static_cast<int64_t>(content_.size());
    }

    size_t
    GetSizeCalls() const {
        return size_calls_.load();
    }
 ...
 private:
    ...
    int64_t position_{0};
    std::atomic<size_t> size_calls_{0};
...
class ControlledDirectReadFile final
 ...
 public:
    ...
    void
    SetSizeFuture(arrow::Future<int64_t> future) {
        size_future_ = std::move(future);
    }
    ...
    void
    ResetCounters() {
        AsyncTrackingRandomAccessFile::ResetCounters();
        std::lock_guard lock(mutex_);
...
inline std::unique_ptr<storage::AsyncIndexEntryReader>
