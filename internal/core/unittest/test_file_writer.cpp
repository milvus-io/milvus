...
TEST_F(FileWriterTest, BasicWriteWithBufferedIO) {
    FileWriterConfig::GetInstance().SetMode(FileWriterConfig::WriteMode::BUFFERED);
...
TEST_F(FileWriterTest, BasicWriteWithDirectIO) {
    FileWriterConfig::GetInstance().SetMode(FileWriterConfig::WriteMode::DIRECT);
...
TEST_F(FileWriterTest, UnalignedToBufferSizeWriteWithDirectIO) {
    ...
    std::string filename = (test_dir_ / "unaligned_to_buffer_size.txt").string();
    FileWriter writer(filename);
...
TEST_F(FileWriterTest, MultipleSmallChunksWriteWithBufferedIO) {
    ...
    std::string filename = (test_dir_ / "multiple_small_chunks_buffered.txt").string();
    FileWriter writer(filename);
...
TEST_F(FileWriterTest, MultipleSmallChunksWriteWithDirectIO) {
    ...
    std::string filename = (test_dir_ / "multiple_small_chunks_direct.txt").string();
    FileWriter writer(filename);
...
TEST_F(FileWriterTest, MemoryAddressAlignedDataWriteWithDirectIO) {
    ...
    std::vector<char> read_data((std::istreambuf_iterator<char>(file)),
                                std::istreambuf_iterator<char>());
    EXPECT_EQ(read_data, std::vector<char>(static_cast<char*>(aligned_data), static_cast<char*>(aligned_data) + kBufferSize));
...
TEST_F(FileWriterTest, UnknownModeWriteWithDirectIO) {
    ...
    EXPECT_THROW(
        {
            FileWriterConfig::GetInstance().SetMode(static_cast<FileWriterConfig::WriteMode>(mode));
            FileWriterConfig::GetInstance().SetBufferSize(kBufferSize);
        },
