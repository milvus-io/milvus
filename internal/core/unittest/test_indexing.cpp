...
#ifdef BUILD_DISK_ANN
        auto rcm =
            std::make_shared<storage::MinioChunkManager>(storage_config_);
...
TEST(Indexing, SearchDiskAnnWithInvalidParam) {
    ...
    auto file_manager = std::make_shared<milvus::storage::DiskFileManagerImpl>(
        field_data_meta, index_meta, storage_config);
    auto index = milvus::index::IndexFactory::GetInstance().CreateIndex(
        create_index_info, file_manager);
    ...
    DatasetPtr xb_dataset = knowhere::GenDataSet(NB, DIM, xb_data.data());
    ASSERT_NO_THROW(index->BuildWithDataset(xb_dataset, build_conf));
    ...
    auto new_index = milvus::index::IndexFactory::GetInstance().CreateIndex(
        create_index_info, new_file_manager);
