...
TEST(storage, InsertDataBool) {
    ...
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::BOOL);
...
TEST(storage, InsertDataInt8) {
    ...
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::INT8);
    field_data->FillFieldData(data.data(), data.size());
...
TEST(storage, InsertDataInt16) {
    ...
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::INT16);
    field_data->FillFieldData(data.data(), data.size());
...
TEST(storage, InsertDataInt32) {
    ...
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::INT32);
    field_data->FillFieldData(data.data(), data.size());
...
TEST(storage, InsertDataInt64) {
    ...
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::INT64);
    field_data->FillFieldData(data.data(), data.size());
...
TEST(storage, InsertDataString) {
    FixedVector<std::string> data = {"test1", "test2", "test3", "test4", "test5"};
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::VARCHAR);
    field_data->FillFieldData(data.data(), data.size());
...
TEST(storage, InsertDataDouble) {
    ...
    auto field_data = milvus::storage::FieldDataFactory::GetInstance().CreateFieldData(storage::DataType::DOUBLE);
    field_data->FillFieldData(data.data(), data.size());
...
TEST(storage, InsertDataFloatVector) {
    ...
    memcpy(new_data.data(), new_payload->Data(), new_payload->get_num_rows() * sizeof(float) * DIM);
    ASSERT_EQ(data, new_data);
}
...
TEST(storage, IndexData) {
    ...
    std::shared_ptr<uint8_t[]> serialized_data_ptr(serialized_bytes.data(), [&](uint8_t*) {});
    auto new_index_data = storage::DeserializeFileData(serialized_data_ptr, serialized_bytes.size());
    ASSERT_EQ(new_index_data->GetCodecType(), storage::IndexDataType);
    ASSERT_EQ(new_index_data->GetTimeRage(), std::make_pair(Timestamp(0), Timestamp(100)));
