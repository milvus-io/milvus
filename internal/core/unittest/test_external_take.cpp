...
// uses IAM), InjectExtfsProperties must NOT copy it into extfs.{cid}.use_iam.
...
TEST(InjectExtfsAllowlist, NoBaselineLeakFromFsProperties) {
    ...
    ::InjectExtfsProperties(props, coll_id, "s3://user-bucket/key", spec);
...
TEST(InjectExtfsAllowlist, InvalidJsonIsHandledGracefully) {
    ...
    EXPECT_NO_THROW(::InjectExtfsProperties(
