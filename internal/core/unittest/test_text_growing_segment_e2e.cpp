...
class TextGrowingSegmentE2ETest : public ::testing::Test {
 protected:
    void
    SetUp() override {
        // create temp directory for LOB files
        test_root_ = std::filesystem::temp_directory_path() /
                     "milvus_growing_text_e2e_test";
    ...
    InsertData(const std::vector<int64_t>& pks,
        ...
        segment_->Insert(offset, N, nullptr, timestamps.data(), insert_data.get());
    }
...
TEST_F(TextGrowingSegmentE2ETest, UnicodeAndSpecialCharacters) {
    ...
    std::vector<int64_t> pks = {1, 2, 3, 4, 5, 6};
    std::vector<std::string> expected_texts = {
        "Hello 世界 🌍",                      // unicode
        "Special: \n\t\r\\\"'",              // special chars
        GenerateString(100 * 1024, '中'),    // large unicode (LOB)
    ...
    // retrieve
    std::vector<int64_t> offsets = {0, 1, 2, 3, 4, 5};
    auto retrieved = RetrieveData(offsets);
...
TEST_F(TextGrowingSegmentE2ETest, VerifyLOBReferenceInStorage) {
    ...
    auto text_field_data = insert_record.template get_data<std::string>(text_field_id_);
...
TEST_F(TextGrowingSegmentE2ETest, LOBOffsetTracking) {
    // insert multiple large texts and verify offsets increase
    std::vector<int64_t> pks = {1, 2, 3, 4};
    std::vector<std::string> texts = {
        GenerateString(100 * 1024, 'a'),  // 100KB at offset 0
        GenerateString(50 * 1024, 'b'),   // 50KB at offset 100KB
        GenerateString(200 * 1024, 'c'),  // 200KB at offset 150KB
        GenerateString(80 * 1024, 'd'),   // 80KB at offset 350KB
    };
    ...
    size_t lob_file_size = GetLOBFileSize(text_field_id_);
    size_t total_size = (100 + 50 + 200 + 80) * 1024;
