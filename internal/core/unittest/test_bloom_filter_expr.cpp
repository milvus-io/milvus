...
std::string
SerializeMbf1(std::string bitset,
              uint64_t n_declared,
              double fpr_declared,
              uint8_t domains = milvus::bloom_envelope::kDomainInt64) {
    ...
    out[28] = static_cast<char>(domains);  // out[29..31] stay reserved
...
class TestSbbfBuilder {
 public:
    ...
    void
    AddInt64(int64_t v) {
        domains_ |= milvus::bloom_envelope::kDomainInt64;
        uint8_t buf[8];
    ...
    AddString(const std::string& s) {
        domains_ |= milvus::bloom_envelope::kDomainUtf8;
    ...
    Serialize(uint64_t n_declared = 0, double fpr_declared = 0.001) const {
        ...
        out[28] = static_cast<char>(domains_);  // out[29..31] stay reserved
...
TEST(BloomFilterExprTest, MalformedEnvelopeRejected) {
    ...
    // Non-zero reserved field (offset 28 is now the domains byte).
    {
        auto blob = good;
        blob[29] = 1;
        expect_rejected(blob, "non-zero reserved");
    }
    // Domain bits this version cannot probe.
    {
        auto blob = good;
        blob[28] = static_cast<char>(blob[28] | 0x04);
        expect_rejected(blob, "unknown value domain bit");
    }
...
// ---------------------------------------------------------------------------
// Value domains. The int64 and utf8 hash domains share one XXH64 output space:
// kCollidingInt64's 8-byte little-endian image is byte-identical to the UTF-8
// bytes of kCollidingString, so the two hash to the same value by construction.
// The envelope's domains byte is what keeps a probe from matching a filter that
// never recorded its domain — without it this alias would match with
// probability 1, regardless of the configured fpr.
// ---------------------------------------------------------------------------
namespace {
constexpr int64_t kCollidingInt64 = 0x4847464544434241LL;
constexpr const char* kCollidingString = "ABCDEFGH";
}  // namespace

TEST(BloomFilterExprTest, ProbeSkipsAbsentValueDomain) {
    // Sanity: the two hash to the same 64-bit value.
    {
        uint8_t buf[8];
        auto u = static_cast<uint64_t>(kCollidingInt64);
        for (int i = 0; i < 8; ++i) {
            buf[i] = static_cast<uint8_t>(u >> (8 * i));
        }
        ASSERT_EQ(XXH64(buf, sizeof(buf), 0),
                  XXH64(kCollidingString, std::strlen(kCollidingString), 0));
    }

    // The view aliases the blob bytes, so every blob must outlive its view —
    // hence the named strings rather than a temporary passed into Parse().
    TestSbbfBuilder int_only(4);
    int_only.AddInt64(kCollidingInt64);
    const std::string int_blob = int_only.Serialize(1, 0.001);
    const auto int_view = SplitBlockBloomFilterView::Parse(int_blob);
    EXPECT_TRUE(int_view.TestInt64(kCollidingInt64))
        << "member must not be missed";
    EXPECT_FALSE(
        int_view.TestBytes(kCollidingString, std::strlen(kCollidingString)))
        << "utf8 probe must not alias an int64-only filter";

    TestSbbfBuilder str_only(4);
    str_only.AddString(kCollidingString);
    const std::string str_blob = str_only.Serialize(1, 0.001);
    const auto str_view = SplitBlockBloomFilterView::Parse(str_blob);
    EXPECT_TRUE(
        str_view.TestBytes(kCollidingString, std::strlen(kCollidingString)))
        << "member must not be missed";
    EXPECT_FALSE(str_view.TestInt64(kCollidingInt64))
        << "int64 probe must not alias a utf8-only filter";

    // A mixed filter records both domains, so both probes stay live and the
    // collision is an honest false positive again.
    TestSbbfBuilder mixed(4);
    mixed.AddInt64(kCollidingInt64);
    mixed.AddString("unrelated");
    const std::string mixed_blob = mixed.Serialize(2, 0.001);
    const auto mixed_view = SplitBlockBloomFilterView::Parse(mixed_blob);
    EXPECT_TRUE(mixed_view.TestInt64(kCollidingInt64));
    EXPECT_TRUE(
        mixed_view.TestBytes(kCollidingString, std::strlen(kCollidingString)));

    // A filter that recorded no domain (empty membership set) matches nothing.
