...
TEST(IndexLoadTest, ScalarV3MmapTantivyUsesDownloadConcurrencyBound) {
    ...
    for (const auto* index_type : {milvus::index::INVERTED_INDEX_TYPE,
        ...
        load_index_info.index_params = std::move(index_params);
        load_index_info.index_size = kIndexSize;
        load_index_info.num_rows = 64UL * 1024 * 1024;
        load_index_info.schema.set_nullable(false);

        auto request = EstimateLoadIndexResource(&load_index_info);

        EXPECT_EQ(request.max_memory_cost, expected_download_peak);
...
TEST(IndexLoadTest, ScalarV3MmapTantivyReservesNullableOffsets) {
    constexpr uint64_t kIndexSize = 1024UL * 1024 * 1024;
    constexpr int64_t kNumRows = 32UL * 1024 * 1024;
    constexpr uint64_t kNullOffsetsSize = kNumRows * sizeof(size_t);
    const auto worker_count = std::max<size_t>(
        1,
        milvus::ThreadPools::GetThreadPool(milvus::ThreadPoolPriority::HIGH)
            .GetMaxThreadNum());
    const auto download_peak = std::min<uint64_t>(
        kIndexSize, worker_count * DEFAULT_INDEX_FILE_SLICE_SIZE);
    const auto expected_loading_peak =
        std::max<uint64_t>(download_peak, 2 * kNullOffsetsSize);
    std::map<std::string, std::string> index_params{
    ...
    EXPECT_EQ(request.max_memory_cost, expected_loading_peak);
    EXPECT_EQ(request.final_memory_cost, kNullOffsetsSize);
}

TEST(IndexLoadTest, ScalarV3MmapTypedJsonReservesBothOffsetVectors) {
    constexpr uint64_t kIndexSize = 1024UL * 1024 * 1024;
    constexpr int64_t kNumRows = 16UL * 1024 * 1024;
    constexpr uint64_t kOneOffsetVectorSize = kNumRows * sizeof(size_t);
    const auto worker_count = std::max<size_t>(
        1,
        milvus::ThreadPools::GetThreadPool(milvus::ThreadPoolPriority::HIGH)
            .GetMaxThreadNum());
    const auto download_peak = std::min<uint64_t>(
        kIndexSize, worker_count * DEFAULT_INDEX_FILE_SLICE_SIZE);
    const auto expected_loading_peak =
        std::max<uint64_t>(download_peak, 3 * kOneOffsetVectorSize);
    std::map<std::string, std::string> index_params{
        {milvus::index::INDEX_TYPE, milvus::index::INVERTED_INDEX_TYPE},
        {milvus::index::SCALAR_INDEX_ENGINE_VERSION, "3"},
    ...
    EXPECT_EQ(request.max_memory_cost, expected_loading_peak);
    EXPECT_EQ(request.final_memory_cost, 2 * kOneOffsetVectorSize);
}
