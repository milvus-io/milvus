...
var pmqPath = "/tmp/pebblemq_client"
...
func TestClient_CreateProducer(t *testing.T) {
	...
	pmqPathTest := pmqPath + "/test_client1"
...
func TestClient_Subscribe(t *testing.T) {
	...
	/////////////////////////////////////////////////
	os.MkdirAll(pmqPath, os.ModePerm)
	pmqPathTest := pmqPath + "/test_client2"
	pmq := newPebbleMQ(t, pmqPathTest)
	defer removePath(pmqPath)
	client1, err := NewClient(Options{
		Server: pmq,
...
func TestClient_SeekLatest(t *testing.T) {
	os.MkdirAll(pmqPath, os.ModePerm)
	pmqPathTest := pmqPath + "/seekLatest"
	pmq := newPebbleMQ(t, pmqPathTest)
	defer removePath(pmqPath)
	client, err := NewClient(Options{
		Server: pmq,
...
func TestClient_consume(t *testing.T) {
	os.MkdirAll(pmqPath, os.ModePerm)
	pmqPathTest := pmqPath + "/test_client3"
	pmq := newPebbleMQ(t, pmqPathTest)
	defer removePath(pmqPath)
	client, err := NewClient(Options{
		Server: pmq,
