...
func (ms *mqMsgStream) AsProducer(channels []string) {
	for _, channel := range channels {
		...
		err := retry.Do(context.TODO(), fn, retry.Attempts(10), retry.Sleep(time.Millisecond*200))
		if err != nil {
...
func (ms *MqTtMsgStream) Seek(msgPositions []*internalpb.MsgPosition) error {
	...
	for idx := range msgPositions {
		...
		if err = retry.Do(context.TODO(), fn, retry.Attempts(10), retry.Sleep(time.Millisecond*200)); err != nil {
