...
type RootCoordCatalog interface {
	...
	// CreateRole creates role by the entity for the tenant. Please make sure the tenant and entity.Name aren't empty. Empty entity.Name may end up with deleting all roles
	// Returns common.IgnorableError if the role already existed
	...
	// - if user doesn't have the role when RemoveUserFromRole
