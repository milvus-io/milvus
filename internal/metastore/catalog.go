...
type RootCoordCatalog interface {
	...
	// DeleteGrantByDatabaseID deletes all Collection grants keyed under a dropped database,
	// including specific-collection grants ({dbName}.col / dbID:X.colID:Y), wildcard grants
	// ({dbName}.* / dbID:X.*), and their grantee-id sub-keys. Grants for other databases and
	// non-Collection object types are left untouched. Called by DropDatabase to keep the RBAC
	// catalog in sync with the dropped database, symmetric with DeleteGrantByCollectionID.
	DeleteGrantByDatabaseID(ctx context.Context, tenant string, dbID int64, dbName string) error
	// DeleteStaleNameBasedGrant removes name-based grant pointer keys that exactly match
	// {dbName}.{collectionName}, without touching ID-based keys. Called by RenameCollection
	// to clean up stale pointers from the pre-rename name so that a later drop+recreate
	// with the old name does not inherit stale privileges via the name-based fallback path.
	DeleteStaleNameBasedGrant(ctx context.Context, tenant string, dbName string, collectionName string) error
