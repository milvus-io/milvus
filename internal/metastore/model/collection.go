...
func (c *Collection) ApplyUpdates(header *message.AlterCollectionMessageHeader, body *message.AlterCollectionMessageBody) {
	...
	for _, field := range updateMask.GetPaths() {
		...
		case message.FieldMaskCollectionShardSplitRouting:
			...
			// lockstep. Their shape is not re-checked here: the only producer of
			// this mask, MetaTable.ApplyShardSplitRouting, applies a post-image
			// after routing.JudgeCommit has refused one whose arrays are not
			// parallel or list a vchannel twice (routing.CheckPostImageShape), so
			// the bounds guards below never pad a persisted record.
			//
