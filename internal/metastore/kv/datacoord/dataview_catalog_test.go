...
func TestDataViewCatalogUsesTwoPartVersionKey(t *testing.T) {
	ctx := context.Background()
	store := newDataViewMetaKV()
	catalog := NewCatalog(store, "", "")
	dataView := &viewpb.DataViewOfCollection{
...
func TestDataViewCatalogSkipsUnmarshalableValues(t *testing.T) {
	ctx := context.Background()
	store := newDataViewMetaKV()
	catalog := NewCatalog(store, "", "")
	dataView := &viewpb.DataViewOfCollection{
	...
	require.NoError(t, catalog.SaveDataView(ctx, dataView))
	// A corrupt value under the DataView prefix must be skipped, not abort
	// the walk: one bad key would otherwise block Coordinator startup.
	store.values["coord/dv/999/versions/1/0"] = "not-a-proto"

	allViews, err := catalog.ListAllDataViews(ctx)
	require.NoError(t, err)
	require.Len(t, allViews, 1)
	require.True(t, proto.Equal(dataView, allViews[0]))
}
