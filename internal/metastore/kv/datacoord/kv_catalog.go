...
func (kc *Catalog) SavePreImportV2Task(ctx context.Context, task *datapb.PreImportV2Task) error {
	key := buildPreImportV2TaskKey(task.GetTaskID())
	value, err := proto.Marshal(task)
	if err != nil {
		return err
	}
...
func (kc *Catalog) ListPreImportV2Tasks(ctx context.Context) ([]*datapb.PreImportV2Task, error) {
	tasks := make([]*datapb.PreImportV2Task, 0)

	applyFn := func(key []byte, value []byte) error {
		task := &datapb.PreImportV2Task{}
		err := proto.Unmarshal(value, task)
		if err != nil {
			return err
		}
	...
	err := kc.MetaKv.WalkWithPrefix(ctx, PreImportV2TaskPrefix+"/", kc.paginationSize, applyFn)
	if err != nil {
		return nil, err
	}
	return tasks, nil
...
func (kc *Catalog) DropPreImportV2Task(ctx context.Context, taskID int64) error {
	key := buildPreImportV2TaskKey(taskID)
	return kc.MetaKv.Remove(ctx, key)
}
