...
// ListSplitShardTask reads back every persisted shard split task. The tasks are
// keyed by collection and task id, so one prefix walk recovers the whole set at
// datacoord start.
