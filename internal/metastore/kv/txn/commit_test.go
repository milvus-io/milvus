...
func TestCommit_AtomicWhenFits(t *testing.T) {
	...
	b.Save("a", "1")
	b.Save("b", "2")
	assert.NoError(t, Commit(context.Background(), tk, 64, b))
}
...
func TestCommit_FallbackOrdersCommitLast(t *testing.T) {
	...
	b.Save("c2", "y")
	b.CommitSave("coll", "v")
	assert.NoError(t, Commit(context.Background(), tk, 2, b)) // limit=2 forces fallback
}

func TestCommit_Empty(t *testing.T) {
	tk := kvmocks.NewTxnKV(t)
	assert.NoError(t, Commit(context.Background(), tk, 64, New()))
}
...
func TestCommit_FallbackPutBatchesPreserveOrder(t *testing.T) {
	...
	assert.NoError(t, Commit(context.Background(), tk, 2, b)) // limit=2 forces fallback, 2 puts per chunk
...
func TestCommit_AtomicMixedRemovalsRejected(t *testing.T) {
	...
	assert.Error(t, Commit(context.Background(), tk, 64, b))
}
...
func TestCommit_FallbackPreservesOrderAcrossKinds(t *testing.T) {
	...
	b.CommitSave("marker", "v")
	assert.NoError(t, Commit(context.Background(), tk, 1, b)) // limit=1 forces fallback, one op per batch
...
func TestCommit_FallbackExceedsCommitLimit(t *testing.T) {
	...
	b.CommitSave("m1", "v1")
	b.CommitSave("m2", "v2")
	assert.Error(t, Commit(context.Background(), tk, 1, b))
}
...
func TestCommit_NonPositiveLimit(t *testing.T) {
	...
	b := New()
	b.Save("c1", "x")
	assert.Error(t, Commit(context.Background(), tk, 0, b))
}
...
func TestCommit_ConditionalCommitFallback(t *testing.T) {
	...
	b.Save("c2", "y")
	b.CommitSaveIfValue("cp", "old", "new")
	assert.NoError(t, Commit(context.Background(), tk, 2, b))
}
...
func TestCommit_FallbackSkipsEmptyTrailingCommit(t *testing.T) {
	...
	b.Save("k2", "2")
	b.Save("k3", "3")
	assert.NoError(t, Commit(context.Background(), tk, 2, b)) // limit=2 forces fallback, no commit ops
}
