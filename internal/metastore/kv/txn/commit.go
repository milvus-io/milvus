...
// Commit applies every op recorded in b against txn, given the backend's
// per-transaction operation limit.
//
// The atomic-vs-fallback threshold is the store's own per-transaction op limit:
// etcd reports a small cap, TiKV a large one, so the same composite write
// commits atomically on TiKV where it would have to chunk on etcd. Commit is
// storage-agnostic - the caller (the catalog's composite Update) supplies the
// limit it reads from its metastore config (metastore.maxEtcdTxnNum), so this
// package never hard-codes a backend limit.
//
...
func Commit(ctx context.Context, txn kv.TxnKV, limit int, b *Builder) error {
	total := len(b.ops)
	if total == 0 {
		return nil
	}
...
func CommitWithoutFallback(ctx context.Context, txn kv.TxnKV, limit int, b *Builder) error {
	if len(b.ops) == 0 {
		return nil
	}
	if limit <= 0 {
