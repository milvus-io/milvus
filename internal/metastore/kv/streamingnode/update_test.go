...
func TestCatalog_SaveRecoverySnapshot_CheckpointOnly(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			}), mock.Anything).Return(tc.commitErr).Maybe()

			ctx := context.Background()
			if tc.commitErr != nil {
				// NewCataLog wraps the kv in a ReliableWriteMetaKv (upstream
				// #51426): a predicate-free write retries unconditionally on any
				// error, so a failing mock write only surfaces once the context
				// is canceled.
				var cancel context.CancelFunc
				ctx, cancel = context.WithCancel(ctx)
				cancel()
			}
			err := NewCataLog(kv).SaveRecoverySnapshot(ctx, "p1", &metastore.WALRecoverySnapshot{
