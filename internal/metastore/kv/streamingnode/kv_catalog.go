...
// Prefix functions - return prefixes for LoadWithPrefix operations.

// buildWALPrefix builds the prefix for a WAL pchannel directory.
func buildWALPrefix(pchannelName string) string {
	return MetaPrefix + "/" + DirectoryWAL + "/" + pchannelName + "/"
}

// buildVChannelPrefix builds the prefix for vchannel meta under a pchannel.
func buildVChannelPrefix(pChannelName string) string {
	return buildWALPrefix(pChannelName) + DirectoryVChannel + "/"
}

// buildSegmentAssignmentPrefix builds the prefix for segment assignments under a pchannel.
func buildSegmentAssignmentPrefix(pChannelName string) string {
	return buildWALPrefix(pChannelName) + DirectorySegmentAssign + "/"
}

// Key functions - return exact keys for single-key operations.

// buildVChannelKey builds the exact key for a specific vchannel meta.
func buildVChannelKey(pChannelName string, vchannelName string) string {
	return buildVChannelPrefix(pChannelName) + vchannelName
}

// buildVChannelSchemaKey builds the exact key for a specific vchannel schema version.
...
// buildSegmentAssignmentKey builds the exact key for a specific segment assignment.
func buildSegmentAssignmentKey(pChannelName string, segmentID int64) string {
	return buildWALPrefix(pChannelName) + DirectorySegmentAssign + "/" + strconv.FormatInt(segmentID, 10)
}

// buildConsumeCheckpointKey builds the exact key for the consume checkpoint.
