...
func WithLOBManagerEnabled(enabled bool) LOBManagerOption {
...
func (m *LOBManager) IsEnabled() bool {
	return m.enabled
}
...
//
// Processing order:
//  1. Write all pending LOBs (lazy mode) - with batch optimization
//  2. Flush eager mode LOB writers
func (m *LOBManager) FlushSegment(ctx context.Context, segmentID int64) error {
	if !m.enabled {
		return nil
	}
	...
	if err := m.flushPendingLOBs(ctx, segmentID); err != nil {
		m.logger.Error("failed to flush pending LOBs",
			zap.Int64("segmentID", segmentID),
			zap.Error(err))
		return errors.Wrap(err, "failed to flush pending LOBs")
	}
	...
	if len(errs) > 0 {
		return errors.Newf("failed to flush %d LOB writers for segment %d", len(errs), segmentID)
	}
...
func (m *LOBManager) Close(ctx context.Context) error {
	...
	m.pendingMu.Lock()
	pendingCount := 0
	for _, pending := range m.pendingLOBs {
		pendingCount += len(pending)
	}
	m.pendingLOBs = make(map[int64][]*PendingLOB)
	m.pendingMu.Unlock()

	if pendingCount > 0 {
		m.logger.Warn("closing LOB manager with unflushed pending LOBs",
			zap.Int("droppedCount", pendingCount))
	}
...
func (m *LOBManager) GetSegmentMetadata(segmentID int64) *LOBSegmentMetadata {
	...
	for key, writer := range m.writers {
		// Parse the key "segmentID:fieldID"
		var sid, fid int64
		...
		// Collect actual LOB file paths from writer
		fieldMeta.LOBFiles = writer.GetLOBFilePaths()
