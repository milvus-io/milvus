...
func TestInsertBinlog(t *testing.T) {
	...
	err = e1.AddDataToPayload([]int64{1, 2, 3}, valid...)
	assert.NoError(t, err)
	err = e1.AddDataToPayload([]int32{4, 5, 6}, valid...)
	assert.Error(t, err)
	err = e1.AddDataToPayload([]int64{4, 5, 6}, valid...)
	assert.NoError(t, err)
	e1.SetEventTimestamp(100, 200)

	e2, err := w.NextInsertEventWriter()
	assert.NoError(t, err)
	err = e2.AddDataToPayload([]int64{7, 8, 9}, valid...)
	assert.NoError(t, err)
	err = e2.AddDataToPayload([]bool{true, false, true}, valid...)
	assert.Error(t, err)
	err = e2.AddDataToPayload([]int64{10, 11, 12}, valid...)
	assert.NoError(t, err)
