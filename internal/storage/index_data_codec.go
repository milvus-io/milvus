...
func (codec *IndexFileBinlogCodec) DeserializeImpl(blobs []*Blob) (
	...
	if len(blobs) == 0 {
		return 0, 0, 0, 0, 0, 0, nil, "", 0, nil, merr.WrapErrStorageMsg("blobs is empty")
	}
...
func (indexCodec *IndexCodec) Deserialize(blobs []*Blob) ([]*Blob, map[string]string, string, UniqueID, error) {
	...
	if file == nil {
		return nil, nil, "", InvalidUniqueID, merr.WrapErrStorageMsg("can not find params blob")
	}
	info := struct {
		Params    map[string]string
		IndexName string
	...
	if err := json.Unmarshal(file.Value, &info); err != nil {
		return nil, nil, "", InvalidUniqueID, merr.WrapErrStorage(err, "json unmarshal error")
	}
