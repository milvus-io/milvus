...
func TestNewLOBReference(t *testing.T) {
	...
	assert.Equal(t, uint32(0), ref.Reserved)
}
...
func TestDecodeLOBReference(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.ref.Reserved, decoded.Reserved)
		})
	}
}
...
func TestLOBReference_String(t *testing.T) {
	ref := NewLOBReference(12345, 678)
	expected := "lob:12345:678"
	assert.Equal(t, expected, ref.String())
}

func TestLOBReference_IsValid(t *testing.T) {
	tests := []struct {
		name  string
		ref   *LOBReference
		valid bool
		...
		{
			name:  "zero fileID - invalid",
			ref:   NewLOBReference(0, 100),
			valid: false,
		},
		{
			name:  "valid with zero offset",
			ref:   NewLOBReference(12345, 0),
			valid: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.valid, tt.ref.IsValid())
		})
	}
}

func TestLOBReferenceFromString(t *testing.T) {
	tests := []struct {
		name    string
		input   string
		want    *LOBReference
		...
		{
			name:  "large numbers",
			input: "lob:453718835200262143:999999",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := LOBReferenceFromString(tt.input)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
