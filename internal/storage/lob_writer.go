...
func NewLOBWriter(
	...
	arrowSchema := arrow.NewSchema(
		[]arrow.Field{
			{Name: "text_data", Type: arrow.BinaryTypes.String, Metadata: fieldMetadata},
		},
		nil,
	)
...
// GetLOBFileIDs returns the list of LOB file IDs as strings
func (w *LOBWriter) GetLOBFileIDs() []string {
	ids := make([]string, 0, len(w.pathsMap))
	for id := range w.pathsMap {
