...
func newAzureObjectStorageWithConfig(ctx context.Context, c *config) (*AzureObjectStorage, error) {
	...
	if connectionString != "" {
		...
		cred, credErr := azblob.NewSharedKeyCredential(c.accessKeyID, c.secretAccessKeyID)
		if credErr != nil {
			return nil, credErr
		}
		client, err = service.NewClientWithSharedKeyCredential("https://"+cred.AccountName()+".blob."+c.address+"/", cred, &service.ClientOptions{})
	}
	if err != nil {
		return nil, err
	}
