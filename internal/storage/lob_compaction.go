...
func NewLOBCompactionHelper(chunkManager ChunkManager) *LOBCompactionHelper {
...
func (h *LOBCompactionHelper) compactFieldLOBFiles(
	...
	if mode == CompactionModeRelocate {
		...
		return h.smartRewriteFieldLOBFiles(ctx, sourceFieldMeta, targetSegmentID, partitionID, collectionID, fieldID, rootPath, targetFieldMeta, refMapping, referenceSet)
	}
...
func (h *LOBCompactionHelper) relocateFieldLOBFiles(
	...
	for _, sourceLOBFile := range sourceFieldMeta.LOBFiles {
		lobFileID, err := h.extractLOBFileID(sourceLOBFile)
		if err != nil {
		...
		targetPath := h.buildLOBFilePath(rootPath, collectionID, partitionID, targetSegmentID, fieldID, lobFileID)
...
func (h *LOBCompactionHelper) smartRewriteFieldLOBFiles(
	...
	if shouldMerge {
		...
		return h.mergeLOBFilesWithFilter(ctx, lobFiles, targetSegmentID, partitionID, collectionID, fieldID, rootPath, CompactionModeSmartRewrite, targetFieldMeta, refMapping, referenceSet)
	}
	...
	for _, fileInfo := range lobFiles {
		...
		newLobFileID := uint64(targetSegmentID)*1000000 + lobFileID
		targetPath := h.buildLOBFilePath(rootPath, collectionID, partitionID, targetSegmentID, fieldID, newLobFileID)
...
func (h *LOBCompactionHelper) transferLOBFileV2(ctx context.Context, sourcePath, targetPath string) error {
	...
	for {
		...
		// need to retain record as we'll use it after reader closes
		record.Retain()
	...
	if err := writer.Close(); err != nil {
		return errors.Wrap(err, "failed to close packed writer")
	}

	h.logger.Debug("successfully transferred LOB file using Storage V2",
		zap.String("source", sourcePath),
		zap.String("target", targetPath),
		zap.Int("recordBatches", len(allRecords)),
	)
...
func (h *LOBCompactionHelper) smartRewriteLOBFile(
	...
	h.logger.Info("smart rewrite completed",
		zap.String("source", sourcePath),
		zap.String("target", targetPath),
		zap.Uint64("oldFileID", oldLobFileID),
		zap.Uint64("newFileID", newLobFileID),
		zap.Int("originalRows", int(currentRowOffset)),
		zap.Int("filteredRows", int(newRowOffset)),
		zap.Int("referenceMappings", len(oldToNewRowOffset)),
		zap.Float64("compressionRatio", float64(currentRowOffset)/float64(newRowOffset)),
	)
...
// Path format: insert_log/{collection_id}/{partition_id}/{segment_id}/{field_id}/lob/{lob_file_id}.parquet
func (h *LOBCompactionHelper) extractLOBFileID(filePath string) (uint64, error) {
	...
	var lobFileID uint64
	if _, err := fmt.Sscanf(filename, "%d", &lobFileID); err != nil {
		return 0, errors.Wrap(err, "failed to parse LOB file ID")
	}

	return lobFileID, nil
}

func (h *LOBCompactionHelper) buildLOBFilePath(rootPath string, collectionID, partitionID, segmentID, fieldID int64, lobFileID uint64) string {
	return fmt.Sprintf("%s/insert_log/%d/%d/%d/%d/lob/%d.parquet",
		rootPath, collectionID, partitionID, segmentID, fieldID, lobFileID)
}
...
func (h *LOBCompactionHelper) mergeLOBFilesWithFilter(
	...
	for groupIdx, group := range groups {
		newLobFileID := uint64(targetSegmentID)*1000000 + uint64(fieldID)*1000 + uint64(groupIdx)
		targetPath := h.buildLOBFilePath(rootPath, collectionID, partitionID, targetSegmentID, fieldID, newLobFileID)
...
func (h *LOBCompactionHelper) mergeFileGroupWithFilter(
	...
	for _, sourceFile := range sourceFiles {
		...
		for {
			...
			numRows := int(record.NumRows())
			keepIndices := make([]int64, 0, numRows)
			oldToNewMapping := make(map[uint32]uint32) // Track mapping for this batch
