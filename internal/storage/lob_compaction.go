...
func (h *LOBCompactionHelper) compactFieldLOBFiles(
	...
	if !exists {
		...
		targetMetadata.LOBFields[fieldID] = targetFieldMeta
	}

	if mode == CompactionModeRelocate {
		return h.relocateFieldLOBFiles(ctx, sourceMeta, sourceFieldMeta, targetSegmentID, partitionID, collectionID, fieldID, rootPath, targetFieldMeta)
	} else if mode == CompactionModeSmartRewrite {
		return h.smartRewriteFieldLOBFiles(ctx, sourceMeta, sourceFieldMeta, targetSegmentID, partitionID, collectionID, fieldID, rootPath, targetFieldMeta, refMapping, referenceSet, lobFileIDAlloc)
	}
...
func (h *LOBCompactionHelper) relocateFieldLOBFiles(
	...
	for _, lobFileIDStr := range sourceFieldMeta.LOBFiles {
		lobFileID, err := strconv.ParseInt(lobFileIDStr, 10, 64)
		if err != nil {
			h.logger.Warn("failed to parse LOB file ID, skipping",
				zap.String("fileID", lobFileIDStr),
				zap.Error(err),
			)
			continue
		}

		// reconstruct source path using source segment metadata
		sourcePath := metautil.BuildLOBLogPath(rootPath, sourceMeta.CollectionID, sourceMeta.PartitionID, sourceMeta.SegmentID, fieldID, lobFileID)
		targetPath := metautil.BuildLOBLogPath(rootPath, collectionID, partitionID, targetSegmentID, fieldID, lobFileID)
		...
		// store only the file ID string
		targetFieldMeta.LOBFiles = append(targetFieldMeta.LOBFiles, lobFileIDStr)
	}
...
func (h *LOBCompactionHelper) smartRewriteFieldLOBFiles(
	...
	for _, lobFileIDStr := range sourceFieldMeta.LOBFiles {
		lobFileID, err := strconv.ParseUint(lobFileIDStr, 10, 64)
		if err != nil {
			h.logger.Warn("failed to parse LOB file ID, skipping",
				zap.String("fileID", lobFileIDStr),
				zap.Error(err),
		...
		// reconstruct source path using source segment metadata
		sourcePath := metautil.BuildLOBLogPath(rootPath, sourceMeta.CollectionID, sourceMeta.PartitionID, sourceMeta.SegmentID, fieldID, int64(lobFileID))
	...
	for _, fileInfo := range lobFiles {
		...
		// store only the new file ID string
		targetFieldMeta.LOBFiles = append(targetFieldMeta.LOBFiles, strconv.FormatUint(newLobFileID, 10))
	}
...
func (h *LOBCompactionHelper) transferLOBFileV2(ctx context.Context, sourcePath, targetPath string) error {
	...
	if storageType != "local" {
		sourceReadPath = path.Join(h.storageConfig.GetBucketName(), sourcePath)
	}

	targetWritePath := targetPath
	if storageType != "local" {
		targetWritePath = path.Join(h.storageConfig.GetBucketName(), targetPath)
	}
...
func (h *LOBCompactionHelper) smartRewriteLOBFile(
	...
	if newRowOffset > 0 {
		compressionRatio = float64(totalOriginalRows) / float64(newRowOffset)
	}
...
func (h *LOBCompactionHelper) ValidateLOBCompaction(
	...
	for fieldID, fieldMeta := range targetMetadata.LOBFields {
		for _, lobFileIDStr := range fieldMeta.LOBFiles {
			fullPath, err := targetMetadata.BuildLOBFilePath(rootPath, fieldID, lobFileIDStr)
			if err != nil {
				errs = append(errs, errors.Wrapf(err, "failed to build LOB file path for field %d", fieldID))
				continue
			}
...
func (h *LOBCompactionHelper) mergeFileGroupWithFilter(
	...
	for _, sourceFile := range sourceFiles {
		// sourceFile.path is already a full path built by BuildLOBLogPath (includes rootPath)
		...
		for {
			...
			if len(keepIndices) == numRows {
				...
				filteredArray := builder.NewStringArray()
				builder.Release() // Release builder immediately after use

				filteredRecord = array.NewRecord(h.arrowSchema, []arrow.Array{filteredArray}, int64(len(keepIndices)))
				filteredArray.Release() // Release array after record creation (record holds reference)
			}
	...
	if totalInputRows > 0 {
		filterRatio = float64(totalOutputRows) / float64(totalInputRows)
	}
