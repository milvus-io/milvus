...
func (stats *FieldStats) UnmarshalJSON(data []byte) error {
	...
	case schemapb.DataType_Int32:
		...
		isScalarField = true
	case schemapb.DataType_Int64, schemapb.DataType_Timestamptz, schemapb.DataType_Decimal:
...
func (stats *FieldStats) UpdateByMsgs(msgs FieldData) {
	...
	case schemapb.DataType_Decimal:
		// Comparing/min-maxing the unscaled int64 is numerically correct decimal
		// comparison (see EncodeUnscaledInt64) — no decimal-specific stats type needed.
		data := msgs.(*DecimalFieldData).Data
		// return error: msgs must has one element at least
		if len(data) < 1 {
			return
		}
