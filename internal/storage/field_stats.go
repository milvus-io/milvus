...
func NewPartitionStatsSnapshot() *PartitionStatsSnapshot {
	return &PartitionStatsSnapshot{
		SegmentStats: make(map[UniqueID][]FieldStats, 0),
	}
}
...
func (ps *PartitionStatsSnapshot) AddStats(segmentID UniqueID, stats []FieldStats) {
	if _, ok := ps.SegmentStats[segmentID]; ok {
		ps.SegmentStats[segmentID] = append(ps.SegmentStats[segmentID], stats...)
	} else {
		ps.SegmentStats[segmentID] = stats
	}
}
