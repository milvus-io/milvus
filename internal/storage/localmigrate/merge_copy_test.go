...
func TestCopyFilePublicationRenameFailureRetainsSourceAndRetries(t *testing.T) {
	sourceDir, targetDir := t.TempDir(), t.TempDir()
	key := "index_files/8/0/2/3/milvus_packed_inverted_index.v3"
	writeFile(t, filepath.Join(sourceDir, key), "complete-index")
	require.NoError(t, os.MkdirAll(filepath.Join(targetDir, filepath.Dir(key)), 0o755))
	...
	var originalRename func(int, string, int, string) error
	renameCalls := 0
	renamePatch := mockey.Mock(unix.Renameat).To(func(oldfd int, oldname string, newfd int, newname string) error {
		if strings.HasSuffix(oldname, ".tmp") && newname == filepath.Base(key) {
			renameCalls++
			return unix.EIO
		}
		return originalRename(oldfd, oldname, newfd, newname)
	}).Origin(&originalRename).Build()
	defer renamePatch.UnPatch()

	err = copyFile(t.Context(), source, key, target, key, &Report{})
	require.ErrorIs(t, err, unix.EIO)
	require.Equal(t, 1, renameCalls)
	require.Equal(t, "complete-index", readFile(t, filepath.Join(sourceDir, key)))
	require.NoFileExists(t, filepath.Join(targetDir, key))
	artifacts, err := filepath.Glob(filepath.Join(targetDir, filepath.Dir(key), ".milvus-local-copy-*"))
	require.NoError(t, err)
	require.Empty(t, artifacts)

	renamePatch.UnPatch()
	report := &Report{}
	require.NoError(t, copyFile(t.Context(), source, key, target, key, report))
	require.Equal(t, 1, report.Copied)
	require.Equal(t, "complete-index", readFile(t, filepath.Join(targetDir, key)))
	require.NoFileExists(t, filepath.Join(sourceDir, key))
}

func TestCopyFileCanceledBeforeReadRetainsSource(t *testing.T) {
	sourceDir, targetDir := t.TempDir(), t.TempDir()
	key := "index_files/8/0/2/3/milvus_packed_inverted_index.v3"
	writeFile(t, filepath.Join(sourceDir, key), "complete-index")
	require.NoError(t, os.MkdirAll(filepath.Join(targetDir, filepath.Dir(key)), 0o755))
	...
	cancel()
	err = copyFile(ctx, source, key, target, key, &Report{})
