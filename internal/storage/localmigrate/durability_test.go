		...
		t.Run(publication, func(t *testing.T) {
			...
			removePatch := mockey.Mock((*os.Root).Remove).Origin(&originalRemove).To(func(opened *os.Root, name string) error {
				if filepath.Join(opened.Name(), name) == filepath.Join(cwd, key) {
					require.GreaterOrEqual(t, len(synced), len(expected))
					require.Equal(t, expected, synced[len(synced)-len(expected):],
						"persist the complete destination directory chain before unlinking the source")
					removed++
				}
	...
	for _, publication := range []string{"fresh", "resumed"} {
		for _, ancestor := range []string{"index_files/8/0/2/3", "index_files/8/0/2", "index_files/8/0", "index_files/8", "index_files", "."} {
			t.Run(publication+"/"+ancestor, func(t *testing.T) {
				...
				matchedSyncs := 0
				failOnSync := 1
				if publication == "fresh" && ancestor == filepath.Dir(key) {
					failOnSync = 2
				}
				var originalSync func(*os.File) error
				syncPatch := mockey.Mock((*os.File).Sync).Origin(&originalSync).To(func(file *os.File) error {
					if filepath.Clean(file.Name()) == failedDirectory {
						matchedSyncs++
						// The leaf is synced once before the temporary file is
						// renamed and again to persist publication. Fail the
						// latter so every case exercises restart after publish.
						if matchedSyncs == failOnSync {
							failures++
							return unix.EIO
						}
					}
				...
				require.False(t, os.SameFile(markerInfo, targetInfo), "publication proof must not be a hard link")
				require.Contains(t, readFile(t, markers[0]), "milvus-local-copy-v1")
				syncPatch.UnPatch()
