...
//
// Canceling ctx stops the migration between filesystem reads and directories, which is
// safe precisely because it is resumable: the sources that have not moved yet
// are rediscovered on the next run. A partially copied temporary file is never
// published and is recopied from the intact source on the next run.
...
func collectLegacyIndexDirs(ctx context.Context, source *migrationSource, root, active, layout string, groups map[string]*migrationDir) error {
	...
	return fs.WalkDir(source.root.FS(), layout, func(name string, entry fs.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			// The CWD is already resolved and WalkDir never follows symlinks.
			physical := filepath.Join(source.physical, name)
...
func isLegacyUnifiedIndexFile(name string) bool {
	// A discovery marker, also moved last so interrupted CWD leaves stay recognizable.
...
// os.Root confines access but permits in-root symlinks. Migration must not
// follow those aliases into a different live directory, including at mutation.
