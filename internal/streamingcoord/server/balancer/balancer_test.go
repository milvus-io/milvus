...
func TestBalancer_WithRecoveryLag(t *testing.T) {
	...
	streamingNodeManager.EXPECT().CollectAllStatus(mock.Anything).RunAndReturn(func(ctx context.Context) (map[int64]*types.StreamingNodeStatus, error) {
		...
		return map[int64]*types.StreamingNodeStatus{
			1: {
				...
				BalanceAttrs: types.StreamingNodeBalanceAttrs{
					ChannelBalanceAttrs: map[types.ChannelID]types.PChannelBalanceAttrs{
						channel.ChannelID{Name: "test-channel-1"}: types.RWChannelBalanceAttrs{RecoveryLag: recoveryLag},
						channel.ChannelID{Name: "test-channel-2"}: types.RWChannelBalanceAttrs{RecoveryLag: recoveryLag},
						channel.ChannelID{Name: "test-channel-3"}: types.RWChannelBalanceAttrs{RecoveryLag: recoveryLag},
					},
				},
			},
			2: {
	...
	defer cancel()
	err = b.WatchChannelAssignments(ctx2, func(version typeutil.VersionInt64Pair, relations []types.PChannelInfoAssigned) error {
