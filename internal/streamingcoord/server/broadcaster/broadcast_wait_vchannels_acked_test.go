...
// secondary cluster's append gate makes: two named vchannels, a waiter
...
func TestWaitVChannelsAckedReturnsWhenTheNamedReplicasLand(t *testing.T) {
	...
	msg := createNewSplitShardBroadcastMsg(
		[]string{"p0_1v0", "p1_1v1", "p2_1v2"}, "p0_1v0").WithBroadcastID(broadcastID)
...
func TestWaitVChannelsAckedRejectsAVChannelOutsideTheBroadcast(t *testing.T) {
	...
	// The only waiter is a secondary's replicate stream, whose append-first
	// names come off the replicated header; a name the task does not carry is
	// the primary's replicas disagreeing about their own broadcast. That is a
	// replicate violation -- unrecoverable, and a code the streaming RPC
	// interceptor forwards intact, unlike a merr error, which it collapses to
	// STREAMING_CODE_UNKNOWN.
	streamingErr := status.AsStreamingError(err)
	assert.True(t, streamingErr.IsReplicateViolation())
	assert.True(t, streamingErr.IsUnrecoverable())
	assert.Equal(t, streamingpb.StreamingCode_STREAMING_CODE_REPLICATE_VIOLATION,
		status.AsStreamingError(status.ConvertStreamingError("WaitVChannelsAcked",
			status.NewGRPCStatusFromStreamingError(streamingErr).Err())).Code,
		"the code survives the RPC boundary")
}
...
func TestWaitVChannelsAckedIsReleasedByClose(t *testing.T) {
	...
	msg := createNewSplitShardBroadcastMsg(
		[]string{"p0_1v0", "p1_1v1", "p2_1v2"}, "p0_1v0").WithBroadcastID(arrived)
...
// TestWaitVChannelsAckedOnACollectedBroadcast: the primary retried a target
// replica whose append had landed, so the secondary receives a second copy of it
// after the broadcast completed here and its task was tombstoned and collected.
// Nothing recreates the task before this gate opens, so the gate must be opened
// by the record that the append-first replica landed here -- and only by it.
func TestWaitVChannelsAckedOnACollectedBroadcast(t *testing.T) {
	const (
		broadcastID = uint64(950)
		source      = "p0_1v0"
		target      = "p1_1v1"
	)
	collected := func(t *testing.T) *broadcastTaskManager {
		bm := newWaitTestManager(t)
		msg := createNewSplitShardBroadcastMsg([]string{source, target}, source).WithBroadcastID(broadcastID)
		for i, vchannel := range []string{source, target} {
			require.NoError(t, bm.Ack(context.Background(), replicatedReplicaOf(msg, vchannel, uint64(200+i))))
		}
		require.NoError(t, bm.WaitVChannelsAcked(context.Background(), broadcastID, []string{source}))
		bm.removeBroadcastTask(broadcastID)
		_, ok := bm.getBroadcastTaskByID(broadcastID)
		require.False(t, ok)
		return bm
	}
	waitBriefly := func(bm *broadcastTaskManager) error {
		ctx, cancel := context.WithTimeout(context.Background(), 200*time.Millisecond)
		defer cancel()
		return bm.WaitVChannelsAcked(ctx, broadcastID, []string{source})
	}

	t.Run("a duplicate replica passes once the source's fence is on record", func(t *testing.T) {
		bm := collected(t)
		defer bm.Close()
		registry.RegisterAppendFirstReplicaRecordedChecker(func(_ context.Context, vchannel string) (bool, error) {
			return vchannel == source, nil
		})
		assert.NoError(t, waitBriefly(bm))
		assert.Zero(t, waitersOf(bm, broadcastID), "an opened gate must not leave its creation waiter behind")
	})

	t.Run("without a record the gate still waits for the task", func(t *testing.T) {
		bm := collected(t)
		defer bm.Close()
		assert.ErrorIs(t, waitBriefly(bm), context.DeadlineExceeded)

		registry.RegisterAppendFirstReplicaRecordedChecker(func(context.Context, string) (bool, error) {
			return false, errors.New("mock datacoord unavailable")
		})
		assert.ErrorIs(t, waitBriefly(bm), context.DeadlineExceeded, "a checker that cannot answer must not open the gate")
		assert.Zero(t, waitersOf(bm, broadcastID))
	})

	t.Run("every named vchannel must be on record", func(t *testing.T) {
		bm := collected(t)
		defer bm.Close()
		registry.RegisterAppendFirstReplicaRecordedChecker(func(_ context.Context, vchannel string) (bool, error) {
			return vchannel == source, nil
		...
		ctx, cancel := context.WithTimeout(context.Background(), 200*time.Millisecond)
		defer cancel()
		assert.ErrorIs(t, bm.WaitVChannelsAcked(ctx, broadcastID, []string{source, "p2_1v2"}), context.DeadlineExceeded)
	})

	t.Run("a record that appears while the gate waits opens it", func(t *testing.T) {
		old := appendFirstRecordedRecheckInterval
		appendFirstRecordedRecheckInterval = 20 * time.Millisecond
		defer func() { appendFirstRecordedRecheckInterval = old }()

		bm := collected(t)
		defer bm.Close()
		done := make(chan error, 1)
		go func() { done <- bm.WaitVChannelsAcked(context.Background(), broadcastID, []string{source}) }()
		select {
		case err := <-done:
			t.Fatalf("the gate opened with nothing on record: %v", err)
		case <-time.After(100 * time.Millisecond):
		}
		// A coord restarting: DataCoord registers its checker after the replicate
		// stream has already started waiting.
		registry.RegisterAppendFirstReplicaRecordedChecker(func(context.Context, string) (bool, error) { return true, nil })
		select {
		case err := <-done:
			assert.NoError(t, err)
		case <-time.After(10 * time.Second):
			t.Fatal("the gate was not opened by a record that appeared while it waited")
		}
	})

	t.Run("a task rebuilt from a first duplicate does not hold a second one", func(t *testing.T) {
		bm := collected(t)
		defer bm.Close()
		registry.RegisterAppendFirstReplicaRecordedChecker(func(_ context.Context, vchannel string) (bool, error) {
			return vchannel == source, nil
		})
		// The first duplicate passes and is acked: the manager rebuilds the task
		// from that replica alone, with the source unacked in it.
		require.NoError(t, waitBriefly(bm))
		msg := createNewSplitShardBroadcastMsg([]string{source, target}, source).WithBroadcastID(broadcastID)
		require.NoError(t, bm.Ack(context.Background(), replicatedReplicaOf(msg, target, 400)))
		rebuilt, ok := bm.getBroadcastTaskByID(broadcastID)
		require.True(t, ok)
		require.Equal(t, []string{source}, rebuilt.UnackedVChannels([]string{source}))
		// A second duplicate still passes.
		assert.NoError(t, waitBriefly(bm))
	})

	t.Run("a record that appears while the gate waits on an existing task opens it", func(t *testing.T) {
		old := appendFirstRecordedRecheckInterval
		appendFirstRecordedRecheckInterval = 20 * time.Millisecond
		defer func() { appendFirstRecordedRecheckInterval = old }()
		...
		defer bm.Close()
		msg := createNewSplitShardBroadcastMsg([]string{source, target}, source).WithBroadcastID(broadcastID)
		require.NoError(t, bm.Ack(context.Background(), replicatedReplicaOf(msg, target, 500)))
		done := make(chan error, 1)
		go func() { done <- bm.WaitVChannelsAcked(context.Background(), broadcastID, []string{source}) }()
		select {
		case err := <-done:
			t.Fatalf("the gate opened before the source was acked or recorded: %v", err)
		case <-time.After(100 * time.Millisecond):
		}
		registry.RegisterAppendFirstReplicaRecordedChecker(func(context.Context, string) (bool, error) { return true, nil })
		select {
		case err := <-done:
			assert.NoError(t, err)
		case <-time.After(10 * time.Second):
			t.Fatal("the gate was not opened by a record that appeared while it waited on the task")
		}
	})
}
