...
func TestGetPendingBroadcastMessages(t *testing.T) {
	t.Run("empty_tasks", func(t *testing.T) {
		...
		// No pending messages for empty task manager
		msgs := bc.GetPendingBroadcastMessages()
		assert.Empty(t, msgs)

		bc.Close()
	})

	t.Run("pending_tasks_with_unacked_vchannels", func(t *testing.T) {
		registry.ResetRegistration()
		paramtable.Init()
		balance.ResetBalancer()
		...
		// Create a task with some unacked vchannels (pending state)
		pendingTask := createNewBroadcastTask(10, []string{"v1", "v2", "v3"})

		meta := mock_metastore.NewMockStreamingCoordCataLog(t)
		meta.EXPECT().ListBroadcastTask(mock.Anything).Return([]*streamingpb.BroadcastTask{pendingTask}, nil).Times(1)
		meta.EXPECT().SaveBroadcastTask(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		rc := idalloc.NewMockRootCoordClient(t)
		f := syncutil.NewFuture[internaltypes.MixCoordClient]()
		f.Set(rc)
		...
		fbc := syncutil.NewFuture[Broadcaster]()
		mw := mock_streaming.NewMockWALAccesser(t)
		mw.EXPECT().AppendMessages(mock.Anything, mock.Anything).Return(types.AppendResponses{
			Responses: []types.AppendResponse{
				{AppendResult: &types.AppendResult{MessageID: walimplstest.NewTestMessageID(1), TimeTick: 100}},
			},
		}).Maybe()
		mw.EXPECT().AppendMessages(mock.Anything, mock.Anything, mock.Anything).Return(types.AppendResponses{
			Responses: []types.AppendResponse{
				{AppendResult: &types.AppendResult{MessageID: walimplstest.NewTestMessageID(1), TimeTick: 100}},
				{AppendResult: &types.AppendResult{MessageID: walimplstest.NewTestMessageID(2), TimeTick: 200}},
			},
		}).Maybe()
		mw.EXPECT().AppendMessages(mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(types.AppendResponses{
			Responses: []types.AppendResponse{
				{AppendResult: &types.AppendResult{MessageID: walimplstest.NewTestMessageID(1), TimeTick: 100}},
				{AppendResult: &types.AppendResult{MessageID: walimplstest.NewTestMessageID(2), TimeTick: 200}},
				{AppendResult: &types.AppendResult{MessageID: walimplstest.NewTestMessageID(3), TimeTick: 300}},
			},
		}).Maybe()
		streaming.SetWALForTest(mw)
		...
		// Should return pending messages for unacked vchannels
		msgs := bc.GetPendingBroadcastMessages()
		assert.Equal(t, 3, len(msgs)) // all 3 vchannels are pending

		bc.Close()
	})

	t.Run("partially_acked_task", func(t *testing.T) {
		registry.ResetRegistration()
		paramtable.Init()
		balance.ResetBalancer()
		...
		// Create a task with some acked vchannels
		partiallyAckedTask := createNewWaitAckBroadcastTaskFromMessage(
			createNewBroadcastMsg([]string{"v1", "v2", "v3"}).WithBroadcastID(11),
			streamingpb.BroadcastTaskState_BROADCAST_TASK_STATE_PENDING,
			[]byte{0x01, 0x00, 0x01}, // v1 and v3 acked, v2 pending
		)

		meta := mock_metastore.NewMockStreamingCoordCataLog(t)
		meta.EXPECT().ListBroadcastTask(mock.Anything).Return([]*streamingpb.BroadcastTask{partiallyAckedTask}, nil).Times(1)
		meta.EXPECT().SaveBroadcastTask(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		rc := idalloc.NewMockRootCoordClient(t)
		f := syncutil.NewFuture[internaltypes.MixCoordClient]()
		f.Set(rc)
		...
		// Should only return 1 pending message (v2, since v1 and v3 are acked)
		msgs := bc.GetPendingBroadcastMessages()
	...
	t.Run("replicated_task_with_pending", func(t *testing.T) {
		registry.ResetRegistration()
		paramtable.Init()
		balance.ResetBalancer()
		...
		// Create a replicated task with unacked vchannels
		replicatedTask := createNewWaitAckBroadcastTaskFromMessage(
			createNewBroadcastMsg([]string{"v1", "v2"}).WithBroadcastID(12),
			streamingpb.BroadcastTaskState_BROADCAST_TASK_STATE_REPLICATED,
			[]byte{0x00, 0x00}, // none acked
		)

		meta := mock_metastore.NewMockStreamingCoordCataLog(t)
		meta.EXPECT().ListBroadcastTask(mock.Anything).Return([]*streamingpb.BroadcastTask{replicatedTask}, nil).Times(1)
		meta.EXPECT().SaveBroadcastTask(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		rc := idalloc.NewMockRootCoordClient(t)
		f := syncutil.NewFuture[internaltypes.MixCoordClient]()
		f.Set(rc)
		...
		// Replicated tasks with none acked should return pending messages
		msgs := bc.GetPendingBroadcastMessages()
	...
	t.Run("tombstone_task_no_pending", func(t *testing.T) {
		registry.ResetRegistration()
		paramtable.Init()
		balance.ResetBalancer()
		...
		// Create a tombstone task - should not return pending messages
		msg := createNewBroadcastMsg([]string{"v1"}).WithBroadcastID(13)
		pb := msg.IntoMessageProto()
		tombstoneTask := &streamingpb.BroadcastTask{
			Message: &messagespb.Message{
				Payload:    pb.Payload,
				Properties: pb.Properties,
			},
		...
		meta := mock_metastore.NewMockStreamingCoordCataLog(t)
		meta.EXPECT().ListBroadcastTask(mock.Anything).Return([]*streamingpb.BroadcastTask{tombstoneTask}, nil).Times(1)
		meta.EXPECT().SaveBroadcastTask(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		rc := idalloc.NewMockRootCoordClient(t)
		f := syncutil.NewFuture[internaltypes.MixCoordClient]()
		f.Set(rc)
		...
		// Tombstone tasks should not have pending messages
		msgs := bc.GetPendingBroadcastMessages()
		assert.Empty(t, msgs)
