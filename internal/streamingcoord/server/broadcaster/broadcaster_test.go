...
func createAlterReplicateConfigBroadcastTask(broadcastID uint64, vchannels []string, bitmap []byte) *streamingpb.BroadcastTask {
	msg := message.NewAlterReplicateConfigMessageBuilderV2().
		WithHeader(&message.AlterReplicateConfigMessageHeader{}).
		WithBody(&message.AlterReplicateConfigMessageBody{}).
		WithBroadcast(vchannels).
		MustBuildBroadcast()
	msg = msg.OverwriteBroadcastHeader(broadcastID)
	pb := msg.IntoMessageProto()
	acks := make([]*streamingpb.AckedCheckpoint, len(bitmap))
	for i := 0; i < len(bitmap); i++ {
		if bitmap[i] != 0 {
...
func TestFixIncompleteBroadcastsForForcePromote(t *testing.T) {
	t.Run("no_incomplete_tasks", func(t *testing.T) {
		...
		// No incomplete tasks, should return nil without error
		err = bc.FixIncompleteBroadcastsForForcePromote(context.Background())
		assert.NoError(t, err)

		bc.Close()
	})

	t.Run("mixed_incomplete_tasks", func(t *testing.T) {
		registry.ResetRegistration()
		paramtable.Init()
		balance.ResetBalancer()
		...
		// Create mixed incomplete tasks:
		// 1. AlterReplicateConfig task with pending vchannels (should be marked with ignore)
		// 2. Regular DropCollection task with pending vchannels (should NOT be marked)
		alterConfigTask := createAlterReplicateConfigBroadcastTask(10, []string{"v1", "v2"}, []byte{0x01, 0x00}) // v2 pending
		dropCollectionTask := createNewWaitAckBroadcastTaskFromMessage(
			createNewBroadcastMsg([]string{"v1", "v2", "v3"}).WithBroadcastID(11),
			streamingpb.BroadcastTaskState_BROADCAST_TASK_STATE_PENDING,
			[]byte{0x01, 0x00, 0x00}, // v2 and v3 pending
		)

		meta := mock_metastore.NewMockStreamingCoordCataLog(t)
		meta.EXPECT().ListBroadcastTask(mock.Anything).Return([]*streamingpb.BroadcastTask{
			alterConfigTask,
			dropCollectionTask,
		}, nil).Times(1)
		meta.EXPECT().SaveBroadcastTask(mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
		...
		mw := mock_streaming.NewMockWALAccesser(t)
		// Expect 3 messages to be appended (1 from alterConfig + 2 from dropCollection)
		mw.EXPECT().AppendMessages(mock.Anything, mock.Anything, mock.Anything, mock.Anything).RunAndReturn(
			func(ctx context.Context, msgs ...message.MutableMessage) types.AppendResponses {
				appendCount.Add(int32(len(msgs)))
				responses := make([]types.AppendResponse, len(msgs))
				for i := range msgs {
					responses[i] = types.AppendResponse{
						AppendResult: &types.AppendResult{
				...
				return types.AppendResponses{Responses: responses}
			}).Maybe()
		streaming.SetWALForTest(mw)
		...
		err = bc.FixIncompleteBroadcastsForForcePromote(context.Background())
		assert.NoError(t, err)
		// At least 3 messages should be appended (1 from alterConfig + 2 from dropCollection)
		// Note: Background scheduler may also append, so we check >= 3
		assert.GreaterOrEqual(t, appendCount.Load(), int32(3))
	...
	t.Run("append_failure", func(t *testing.T) {
		...
		// Create a task with pending vchannels
		dropCollectionTask := createNewWaitAckBroadcastTaskFromMessage(
			createNewBroadcastMsg([]string{"v1", "v2"}).WithBroadcastID(12),
			streamingpb.BroadcastTaskState_BROADCAST_TASK_STATE_PENDING,
			[]byte{0x01, 0x00}, // v2 pending
		)
