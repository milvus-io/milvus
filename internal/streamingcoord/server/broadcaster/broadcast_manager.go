...
func (bm *broadcastTaskManager) LegacyAck(ctx context.Context, broadcastID uint64, vchannel string) error {
	...
	if !ok {
		bm.Logger().Warn(ctx,
			...
			"broadcast task not found, it may already acked, ignore the request", mlog.Uint64("broadcastID", broadcastID), mlog.FieldVChannel(vchannel))
		return nil
	}
	...
	if msg == nil {
		task.Logger().Warn(ctx,
			...
			"vchannel is already acked, ignore the ack request", mlog.FieldVChannel(vchannel))
		return nil
	}
