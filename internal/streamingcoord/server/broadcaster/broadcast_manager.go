...
type broadcastTaskManager struct {
	...
	// closing is closed by Close() BEFORE it waits for the lifetime, so that
	// anything parked on an event that can no longer happen lets go.
	//
	// WaitVChannelsAcked is the reason it exists. It holds the lifetime across a
	// wait bounded only by its caller's context, and that caller is a DIFFERENT
	// process -- the secondary's proxy, whose replicate-stream context carries no
	// deadline. mixcoord's GracefulStop closes this broadcaster before it reaches
	// the gRPC server's GracefulStopTimeout fallback, so without this channel a
	// single gated replica keeps lifetime.Wait() blocked until the pod is
	// SIGKILLed. A waiter has nothing to gain by outliving the broadcaster: no
	// ack can arrive after it.
	closing   chan struct{}
	closeOnce sync.Once
	mu        *sync.Mutex
...
// has a recorded checkpoint in THIS cluster, or every one is on record as landed
// here (appendFirstReplicasRecorded), or the context ends.
//
...
// vchannels' replicas have landed here, and this ack state -- or, once the task
// can no longer answer, the durable record of the landing -- is the only fact a
...
func (bm *broadcastTaskManager) WaitVChannelsAcked(ctx context.Context, broadcastID uint64, vchannels []string) error {
	...
	// A gated replica blocks its ENTIRE pchannel's replicate stream on the
	// secondary -- handleReplicateMessage is called synchronously from the
	// stream's recv loop -- so a wait that does not end is indistinguishable,
	// from outside, from replication being wedged. Say which broadcast and which
	// vchannel it is waiting for; that names the stream that is actually stuck.
	observer := bm.observeAckWait(ctx, broadcastID, vchannels)
	defer observer.Release(ctx)

	// The task's ack state is not the only proof that the named replicas
	// landed, and not always an available one: see appendFirstReplicasRecorded.
	if bm.appendFirstReplicasRecorded(ctx, broadcastID, vchannels) {
		bm.logAppendGateOpenedByRecord(ctx, broadcastID, vchannels)
		return nil
	}
	...
	go func() {
		defer poller.Done()
		ticker := time.NewTicker(appendFirstRecordedRecheckInterval)
		defer ticker.Stop()
		for {
			select {
			case <-waitCtx.Done():
				return
			case <-ticker.C:
				if bm.appendFirstReplicasRecorded(waitCtx, broadcastID, vchannels) {
					recorded.Store(true)
					cancel()
					return
				}
	...
	err := bm.waitVChannelsAckedByTask(waitCtx, broadcastID, vchannels, observer)
	cancel()
	poller.Wait()
	if err != nil && recorded.Load() && ctx.Err() == nil {
		bm.logAppendGateOpenedByRecord(ctx, broadcastID, vchannels)
		return nil
	}
	return err
...
// waitVChannelsAckedByTask waits for the broadcast's task to exist here and for
// every named vchannel to be acked in it.
func (bm *broadcastTaskManager) waitVChannelsAckedByTask(ctx context.Context, broadcastID uint64, vchannels []string, observer *ackWaitObserver) error {
	t, err := bm.blockUntilBroadcastTaskCreated(ctx, broadcastID)
	if err != nil {
		return err
	}
	observer.TaskArrived(t)
	// The task pointer is resolved once and deliberately NOT re-resolved between
	// vchannels, unlike the creation wait above, which loops. Only DropTombstone
	// removes a task from the manager, and a task is tombstoned only after every
	// one of its vchannels has been acked -- so by the time this pointer could go
	// stale, every channel it hands out is already closed and the gate is open.
	// Re-resolving could only ever find the same task or nothing at all.
	for _, vchannel := range vchannels {
		if err := t.BlockUntilVChannelAcked(ctx, vchannel, bm.closing); err != nil {
			return err
		}
	}
	return nil
...
// appendFirstRecordedRecheckInterval is how often a waiting gate asks again
// whether the named replicas are on record. Normally the record is either there
// when the wait starts or the task's acks open the gate first; the recheck
// covers a record that appears while the gate waits on a task that will never
// see those acks, and a checker registered after the wait began (a coord
// restarting).
//
// A variable rather than a constant only so a test can shorten it.
var appendFirstRecordedRecheckInterval = 5 * time.Second

// appendFirstReplicasRecorded reports whether every named vchannel's replica is
// on record on this cluster (registry.IsAppendFirstReplicaRecorded). A checker
// error counts as not recorded: the wait goes on and asks again.
//
// It opens the gate on its own, ahead of the task's ack state, because the task
// can no longer answer for a replica the primary wrote twice. Its broadcaster
// retries an append whose response was an error even though it landed, so a
// second copy of a target replica -- with a later replicate time tick, which the
// replicate checkpoint does not cover -- can reach this cluster after the
// broadcast completed here:
//   - once the task has been tombstoned and collected, nothing recreates it
//     except the ack of a replica that already passed this gate;
//   - once one such copy has passed and been acked, getOrCreateBroadcastTask
//     rebuilds the task from it alone, and a second copy -- of another target, or
//     of the same one -- would wait on that task for a source ack that already
//     happened and is never delivered again.
//
// A record is sufficient on its own because a checker answers true only after
// the named replica landed in this cluster's WAL, which is exactly what the
// gate waits for.
func (bm *broadcastTaskManager) appendFirstReplicasRecorded(ctx context.Context, broadcastID uint64, vchannels []string) bool {
	if len(vchannels) == 0 {
		return false
	}
	for _, vchannel := range vchannels {
		recorded, err := registry.IsAppendFirstReplicaRecorded(ctx, vchannel)
		if err != nil {
			if ctx.Err() == nil {
				bm.Logger().Warn(ctx, "failed to ask whether a broadcast's append-first replica is on record, keep waiting",
					mlog.FieldBroadcastID(broadcastID), mlog.FieldVChannel(vchannel), mlog.Err(err))
			}
			return false
		}
		if !recorded {
...
func (bm *broadcastTaskManager) logAppendGateOpenedByRecord(ctx context.Context, broadcastID uint64, vchannels []string) {
	bm.Logger().Info(ctx, "append gate opened by the record that the append-first replicas landed on this cluster",
		mlog.FieldBroadcastID(broadcastID),
		mlog.Strings("appendFirstVChannels", vchannels))
}

// waitVChannelsAckedSlowThreshold is how long a gate may wait before the coord
// says so out loud. Chosen well above any healthy cross-cluster append: a
// replicate stream that is keeping up delivers the append-first replicas within
// the time it takes them to be consumed, milliseconds to seconds. Crossing this
// means something upstream is not moving.
//
// A variable rather than a constant only so a test can reach the slow path
// without sleeping through it; nothing in production writes it.
var waitVChannelsAckedSlowThreshold = 30 * time.Second

// ackWaitObserver reports a gate that is taking too long, exactly once, and then
// reports its release, exactly once. Silent for a wait that finishes promptly,
// which is nearly all of them.
type ackWaitObserver struct {
	logger      *mlog.Logger
	broadcastID uint64
	vchannels   []string
	start       time.Time
...
// observeAckWait starts watching a wait that has just begun.
func (bm *broadcastTaskManager) observeAckWait(ctx context.Context, broadcastID uint64, vchannels []string) *ackWaitObserver {
...
// watch fires the one Warn, if the wait lives long enough to earn it.
func (o *ackWaitObserver) watch(ctx context.Context) {
	timer := time.NewTimer(waitVChannelsAckedSlowThreshold)
	defer timer.Stop()
	select {
	case <-ctx.Done():
		return
	...
	unacked := o.vchannels
	if task != nil {
		unacked = task.UnackedVChannels(o.vchannels)
	}
	o.logger.Warn(ctx, "append gate is still waiting for a broadcast's append-first replicas",
		mlog.Uint64("broadcastID", o.broadcastID),
		mlog.Strings("appendFirstVChannels", o.vchannels),
		mlog.Strings("unackedVChannels", unacked),
		mlog.Bool("taskArrived", task != nil),
		mlog.Duration("elapsed", time.Since(o.start)))
}

// TaskArrived records the broadcast task once it exists here, so a later Warn
// can name the vchannels that are actually still unacked rather than all of them.
func (o *ackWaitObserver) TaskArrived(task *broadcastTask) {
	o.mu.Lock()
	defer o.mu.Unlock()
	o.task = task
}

// Release ends the watch, and says so only if the Warn was already spent -- an
// operator who saw the Warn needs the matching "it moved" line; nobody needs one
// for a wait that was never reported.
func (o *ackWaitObserver) Release(ctx context.Context) {
	o.stop()
	o.mu.Lock()
	warned := o.warned
	o.mu.Unlock()
	...
	o.logger.Info(ctx, "append gate stopped waiting for a broadcast's append-first replicas",
		mlog.Uint64("broadcastID", o.broadcastID),
		mlog.Strings("appendFirstVChannels", o.vchannels),
		mlog.Duration("elapsed", time.Since(o.start)))
}
...
func (bm *broadcastTaskManager) blockUntilBroadcastTaskCreated(ctx context.Context, broadcastID uint64) (*broadcastTask, error) {
	for {
		...
		case <-bm.closing:
			bm.removeTaskCreationWaiter(broadcastID, created)
			return nil, status.NewOnShutdownError("broadcaster is closing, stop waiting for broadcast %d to arrive", broadcastID)
		case <-created:
...
func (bm *broadcastTaskManager) Close() {
	...
	// Release the waits that hold the lifetime BEFORE waiting on it, or Close
	// deadlocks against them: WaitVChannelsAcked's wait ends only on its caller's
	// context, which belongs to another cluster's replicate stream and carries no
	// deadline. Guarded by a Once so that a second Close -- which the schedulers
	// below do not tolerate anyway -- cannot turn into a double-close panic here.
	bm.closeOnce.Do(func() { close(bm.closing) })
	bm.lifetime.Wait()
