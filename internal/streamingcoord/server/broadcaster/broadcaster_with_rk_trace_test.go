...
func TestBroadcasterWithRK_InjectsTraceContextBeforeTaskPersist(t *testing.T) {
	...
	mockey.Mock((*broadcastTaskManager).broadcast).To(
		func(_ *broadcastTaskManager, _ context.Context, msg message.BroadcastMutableMessage, _ uint64, _ *lockGuards, _ int) (*types.BroadcastAppendResult, *broadcastTask, error) {
			capturedMsg = msg
			return &types.BroadcastAppendResult{}, nil, nil
		}).Build()
