...
func (c *Core) initSession() error {
	c.session = sessionutil.NewSession(c.ctx, Params.BaseParams.MetaRootPath, Params.BaseParams.EtcdEndpoints)
	if c.session == nil {
...
func (c *Core) Init() error {
	...
	if c.kvBaseCreate == nil {
		c.kvBaseCreate = func(root string) (kv.TxnKV, error) {
			return etcdkv.NewEtcdKV(Params.BaseParams.EtcdEndpoints, root)
		}
	}
	c.initOnce.Do(func() {
		...
		connectEtcdFn := func() error {
			if c.etcdCli, initError = clientv3.New(clientv3.Config{Endpoints: Params.BaseParams.EtcdEndpoints, DialTimeout: 5 * time.Second}); initError != nil {
		...
		kv, initError := tsoutil.NewTSOKVBase(Params.BaseParams.EtcdEndpoints, Params.BaseParams.KvRootPath, "gid")
		if initError != nil {
			return
		}
		...
		kv, initError = tsoutil.NewTSOKVBase(Params.BaseParams.EtcdEndpoints, Params.BaseParams.KvRootPath, "tso")
		if initError != nil {
			return
		}
		...
		c.proxyManager, initError = newProxyManager(
			c.ctx,
			Params.BaseParams.EtcdEndpoints,
