...
func (c *Core) CreateRole(ctx context.Context, in *milvuspb.CreateRoleRequest) (*commonpb.Status, error) {
	...
	if err != nil {
		...
		return failStatus(commonpb.ErrorCode_CreateRoleFailure, errMsg), errorutil.ReadableError(errMsg, err, zap.String("role_name", entity.Name))
	}
...
func (c *Core) DropRole(ctx context.Context, in *milvuspb.DropRoleRequest) (*commonpb.Status, error) {
	...
	if _, err := c.MetaTable.SelectRole(util.DefaultTenant, &milvuspb.RoleEntity{Name: in.RoleName}, false); err != nil {
		...
		return failStatus(commonpb.ErrorCode_DropRoleFailure, errMsg), errorutil.ReadableError(errMsg, err, zap.String("role_name", in.RoleName))
	}
	...
	for _, roleResult := range roleResults {
		for index, userEntity := range roleResult.Users {
			if err = c.MetaTable.OperateUserRole(util.DefaultTenant,
				...
				return failStatus(commonpb.ErrorCode_OperateUserRoleFailure, errMsg),
					errorutil.ReadableError(errMsg, err, zap.String("role_name", roleResult.Role.Name), zap.String("username", userEntity.Name), zap.Int("current_index", index))
			}
		}
	}
...
func (c *Core) OperateUserRole(ctx context.Context, in *milvuspb.OperateUserRoleRequest) (*commonpb.Status, error) {
	...
	if _, err := c.MetaTable.SelectRole(util.DefaultTenant, &milvuspb.RoleEntity{Name: in.RoleName}, false); err != nil {
		...
		return failStatus(commonpb.ErrorCode_OperateUserRoleFailure, errMsg), errorutil.ReadableError(errMsg, err, zap.String("role_name", in.RoleName))
	}
	if _, err := c.MetaTable.SelectUser(util.DefaultTenant, &milvuspb.UserEntity{Name: in.Username}, false); err != nil {
		...
		return failStatus(commonpb.ErrorCode_OperateUserRoleFailure, errMsg), errorutil.ReadableError(errMsg, err, zap.String("username", in.Username))
	}
	updateCache := true
	if err := c.MetaTable.OperateUserRole(util.DefaultTenant, &milvuspb.UserEntity{Name: in.Username}, &milvuspb.RoleEntity{Name: in.RoleName}, in.Type); err != nil {
		if !common.IsIgnorableError(err) {
			...
			return failStatus(commonpb.ErrorCode_OperateUserRoleFailure, errMsg),
				errorutil.ReadableError(errMsg, err, zap.String("role_name", in.RoleName), zap.String("username", in.Username))
		}
		updateCache = false
	}
	...
	if updateCache {
		...
		default:
			...
			return failStatus(commonpb.ErrorCode_OperateUserRoleFailure, errMsg),
				errorutil.ReadableError(errMsg, nil, zap.Any("op_type", in.Type))
		}
...
func (c *Core) SelectRole(ctx context.Context, in *milvuspb.SelectRoleRequest) (*milvuspb.SelectRoleResponse, error) {
	...
	if in.Role != nil {
		if _, err := c.MetaTable.SelectRole(util.DefaultTenant, &milvuspb.RoleEntity{Name: in.Role.Name}, false); err != nil {
			...
			}, errorutil.ReadableError(errMsg, err, zap.String("role_name", in.Role.Name))
		}
	}
	...
	if err != nil {
		...
		return &milvuspb.SelectRoleResponse{
			Status: failStatus(commonpb.ErrorCode_SelectRoleFailure, errMsg),
		}, errorutil.ReadableError(errMsg, err, zap.Any("role", in.Role), zap.Bool("include_user_info", in.IncludeUserInfo))
	}
...
func (c *Core) SelectUser(ctx context.Context, in *milvuspb.SelectUserRequest) (*milvuspb.SelectUserResponse, error) {
	...
	if in.User != nil {
		if _, err := c.MetaTable.SelectUser(util.DefaultTenant, &milvuspb.UserEntity{Name: in.User.Name}, false); err != nil {
			...
			}, errorutil.ReadableError(errMsg, err, zap.String("username", in.User.Name))
		}
	}
	...
	if err != nil {
		...
		return &milvuspb.SelectUserResponse{
			Status: failStatus(commonpb.ErrorCode_SelectUserFailure, errMsg),
		}, errorutil.ReadableError(errMsg, err, zap.Any("user", in.User), zap.Bool("include_role_info", in.IncludeRoleInfo))
	}
...
func (c *Core) isValidRole(entity *milvuspb.RoleEntity) error {
	if entity == nil {
		return errorutil.ReadableError("the role entity is nil", nil)
	}
	if entity.Name == "" {
		return errorutil.ReadableError("the name in the role entity is empty", nil)
	}
	if _, err := c.MetaTable.SelectRole(util.DefaultTenant, &milvuspb.RoleEntity{Name: entity.Name}, false); err != nil {
		errMsg := "fail to select the role"
		return errorutil.ReadableError(errMsg, err, zap.String("role_name", entity.Name))
	}
	return nil
}
...
func (c *Core) isValidObject(entity *milvuspb.ObjectEntity) error {
	if entity == nil {
		return errorutil.ReadableError("the object entity is nil", nil)
	}
	if _, ok := commonpb.ObjectType_value[entity.Name]; !ok {
		return errorutil.ReadableError("the object type in the object entity is invalid", nil, zap.String("name", entity.Name))
	}
	return nil
}
...
func (c *Core) isValidGrantor(entity *milvuspb.GrantorEntity, object string) error {
	if entity == nil {
		return errorutil.ReadableError("the grantor entity is nil", nil)
	}
	if entity.User == nil {
		return errorutil.ReadableError("the user entity in the grantor entity is nil", nil)
	}
	if entity.User.Name == "" {
		return errorutil.ReadableError("the name in the user entity of the grantor entity is empty", nil)
	}
	if _, err := c.MetaTable.SelectUser(util.DefaultTenant, &milvuspb.UserEntity{Name: entity.User.Name}, false); err != nil {
		errMsg := "fail to select the user"
		return errorutil.ReadableError(errMsg, err, zap.String("user", entity.User.Name))
	}
	if entity.Privilege == nil {
		return errorutil.ReadableError("the privilege entity in the grantor entity is nil", nil)
	}
	...
	if privilegeName := util.PrivilegeNameForMetastore(entity.Privilege.Name); privilegeName == "" {
		return errorutil.ReadableError("the privilege name in the privilege entity is invalid", nil, zap.String("privilege_name", entity.Privilege.Name))
	}
	privileges, ok := util.ObjectPrivileges[object]
	if !ok {
		return errorutil.ReadableError("the object type is invalid", nil, zap.String("object", object))
	}
	...
	return errorutil.ReadableError("the privilege name is invalid", nil, zap.String("privilege_name", entity.Privilege.Name))
}
...
func (c *Core) OperatePrivilege(ctx context.Context, in *milvuspb.OperatePrivilegeRequest) (*commonpb.Status, error) {
	...
	if err := c.MetaTable.OperatePrivilege(util.DefaultTenant, in.Entity, in.Type); err != nil {
		if !common.IsIgnorableError(err) {
			...
			return failStatus(commonpb.ErrorCode_OperatePrivilegeFailure, errMsg), errorutil.ReadableError(errMsg, err, zap.Any("in", in))
		}
		updateCache = false
	}
	...
	if updateCache {
		...
		default:
			...
			return failStatus(commonpb.ErrorCode_OperatePrivilegeFailure, errMsg), errorutil.ReadableError(errMsg, nil, zap.Any("op_type", in.Type))
		}
		if err := c.proxyClientManager.RefreshPolicyInfoCache(ctx, &proxypb.RefreshPolicyInfoCacheRequest{
			OpType: opType,
...
func (c *Core) SelectGrant(ctx context.Context, in *milvuspb.SelectGrantRequest) (*milvuspb.SelectGrantResponse, error) {
	...
	if err != nil {
		...
		return &milvuspb.SelectGrantResponse{
			Status: failStatus(commonpb.ErrorCode_SelectGrantFailure, errMsg),
		}, errorutil.ReadableError(errMsg, err, zap.Any("in", in))
	}
...
func (c *Core) ListPolicy(ctx context.Context, in *internalpb.ListPolicyRequest) (*internalpb.ListPolicyResponse, error) {
	...
	if err != nil {
		...
		}, errorutil.ReadableError(errMsg, err)
	}
