...
func (c *Core) CreatePrivilegeGroup(ctx context.Context, in *milvuspb.CreatePrivilegeGroupRequest) (*commonpb.Status, error) {
	...
	if in.Privileges == nil {
		ctxLog.Warn("the privileges in the create request is nil, set it to empty")
		in.Privileges = make([]*milvuspb.PrivilegeEntity, 0)
	}
	redoTask := newBaseRedoTask(c.stepExecutor)
	redoTask.AddSyncStep(NewSimpleStep("create privilege group meta data", func(ctx context.Context) ([]nestedStep, error) {
		err := c.meta.CreatePrivilegeGroup(ctx, in.GroupName, in.Privileges)
		if err != nil {
...
func (c *Core) AddPrivilegesToGroup(ctx context.Context, in *milvuspb.AddPrivilegesToGroupRequest) (*commonpb.Status, error) {
	...
	err := c.meta.AddPrivilegesToGroup(ctx, in.GroupName, in.Privileges)
	if err != nil {
...
func (c *Core) DropPrivilegesFromGroup(ctx context.Context, in *milvuspb.DropPrivilegesFromGroupRequest) (*commonpb.Status, error) {
	...
	err := c.meta.DropPrivilegesFromGroup(ctx, in.GroupName, in.Privileges)
	if err != nil {
