...
func validateDesiredShardNum(coll *model.Collection, properties []*commonpb.KeyValuePair, deleteKeys []string) error {
	...
	// Range-routed (namespace) collections size themselves by splitting on
	// namespace boundaries; there is no shard count to set.
	if coll.RoutingMode != schemapb.RoutingMode_RoutingModeHash {
		return merr.WrapErrParameterInvalidMsg(
			"%s applies to hash-routed collections only", common.CollectionShardNum)
	}
	if desired < 2 {
		return merr.WrapErrParameterInvalidMsg(
