...
func TestRootCoord_Base(t *testing.T) {
	...
	wg.Add(1)
	t.Run("report import appendTaskSegments fail", func(t *testing.T) {
		defer wg.Done()
		assert.NoError(t, err)
		req := &rootcoordpb.ImportResult{
			TaskId:   100,
		...
		tempKV := core.importManager.taskStore
		mockKv := &kv.MockMetaKV{
			SaveWithIgnoreLeaseMockErr: true,
		}
		mockKv.InMemKv = make(map[string]string)
		core.importManager.taskStore = mockKv
		defer func() {
			core.importManager.taskStore = tempKV
		}()
		core.importManager.workingTasks[100] = &datapb.ImportTaskInfo{
			Id: 100,
