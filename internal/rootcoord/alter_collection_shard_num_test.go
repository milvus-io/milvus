...
func TestValidateDesiredShardNumAcceptsAWithdrawal(t *testing.T) {
	...
	// A collection nothing can rehash may still withdraw: a range-routed one
	// cannot be given a shard count at all, and a request to shrink is refused,
	// but neither refusal has anything to say about asking for nothing.
	rangeColl := &model.Collection{
		Name:                "ns",
		RoutingMode:         schemapb.RoutingMode_RoutingModeRange,
		VirtualChannelNames: []string{"a"},
	}
	assert.NoError(t, validateDesiredShardNum(rangeColl, nil, []string{common.CollectionShardNum}))
...
func TestValidateDesiredShardNumRejections(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// A namespace collection sizes itself by splitting on namespace
			// boundaries; there is no shard count to set.
			name: "range-routed collection",
			coll: &model.Collection{
				Name:                "ns",
				RoutingMode:         schemapb.RoutingMode_RoutingModeRange,
				VirtualChannelNames: []string{"a"},
			...
			errStr: "hash-routed collections only",
		},
		{
