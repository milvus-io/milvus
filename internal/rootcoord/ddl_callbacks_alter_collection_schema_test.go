...
func TestBroadcastAlterCollectionSchemaInternalMessageIsReplicable(t *testing.T) {
	...
	coll, err := core.meta.GetCollectionByName(ctx, dbName, collectionName, typeutil.MaxTimestamp, false)
	require.NoError(t, err)
	broadcastAPI := &alterSchemaBroadcastAPIMock{}
	require.NoError(t, core.broadcastAlterCollectionSchemaAdd(
		ctx,
		broadcastAPI,
		coll,
		buildAlterSchemaAddFieldReq(dbName, collectionName, "field2", false),
	))
	require.NotNil(t, broadcastAPI.msg)
	require.False(t, broadcastAPI.msg.IsUnreplicable())
}

func TestBroadcastLegacyAddCollectionFieldExternalMessageIsUnreplicable(t *testing.T) {
	mockAlterSchemaBroadcastAPI(t)
	core := initStreamingSystemAndCore(t)
	ctx := context.Background()
	...
	broadcastAPI := &alterSchemaBroadcastAPIMock{}
	lockMocker := mockey.Mock((*Core).startBroadcastWithAliasOrCollectionLock).
	...
	require.NoError(t, err)
	require.NotNil(t, broadcastAPI.msg)
	require.True(t, broadcastAPI.msg.IsUnreplicable())
}
