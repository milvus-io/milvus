...
func TestNewTelemetryManager(t *testing.T) {
	...
	assert.NotNil(t, mgr)
	assert.NotNil(t, mgr.clientMetrics)
}
...
func TestMatchesScopeDatabase(t *testing.T) {
	...
	t.Run("database scope does not match client with nil AccessedDatabases", func(t *testing.T) {
		...
		// Manually set AccessedDatabases to nil
		mgr.mu.Lock()
		if cache, ok := mgr.clientMetrics["db-client-3"]; ok {
			cache.AccessedDatabases = nil
		}
		mgr.mu.Unlock()
...
func TestCleanupInactiveClients(t *testing.T) {
	...
	mgr.mu.Lock()
	if cache, ok := mgr.clientMetrics["client-old"]; ok {
		cache.LastHeartbeat = time.Now().Add(-35 * time.Minute)
	}
	mgr.mu.Unlock()
...
func TestGetClientTelemetryWithDatabaseFilter(t *testing.T) {
	...
	mgr.mu.Lock()
	if cache, ok := mgr.clientMetrics["db-filter-client"]; ok {
		cache.AccessedDatabases["test_db"] = struct{}{}
	}
	mgr.mu.Unlock()
...
func TestListClientsWithDatabaseFilter(t *testing.T) {
	...
	mgr.mu.Lock()
	if cache, ok := mgr.clientMetrics["client-with-db"]; ok {
		cache.AccessedDatabases["my_database"] = struct{}{}
	}
	mgr.mu.Unlock()
...
func TestGetClientTelemetryAllFilters(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if i < 2 {
			mgr.mu.Lock()
			if cache, ok := mgr.clientMetrics[clientID]; ok {
				cache.AccessedDatabases["shared_db"] = struct{}{}
			}
			mgr.mu.Unlock()
		}
	}
...
func TestHeartbeatUpdatesConfigHash(t *testing.T) {
	...
	mgr.mu.RLock()
	cache := mgr.clientMetrics["hash-test-client"]
	mgr.mu.RUnlock()
...
func TestGetClientTelemetryReturnsStatus(t *testing.T) {
	...
	mgr.mu.Lock()
	if cache, ok := mgr.clientMetrics["inactive-client"]; ok {
		cache.LastHeartbeat = time.Now().Add(-10 * time.Minute)
	}
	mgr.mu.Unlock()
...
func TestGetClientTelemetryDatabaseList(t *testing.T) {
	...
	mgr.mu.Lock()
	if cache, ok := mgr.clientMetrics["db-list-client"]; ok {
		cache.AccessedDatabases["db1"] = struct{}{}
		cache.AccessedDatabases["db2"] = struct{}{}
	}
	mgr.mu.Unlock()
...
func TestProcessCommandRepliesEdgeCases(t *testing.T) {
	t.Run("handles empty replies", func(t *testing.T) {
		...
		mgr.processCommandReplies("test-client", nil)
		mgr.processCommandReplies("test-client", []*commonpb.CommandReply{})
	})
...
func TestGetClientTelemetryNilClientInfo(t *testing.T) {
	t.Run("handles nil ClientInfo in cache", func(t *testing.T) {
		...
		mgr.mu.Lock()
		mgr.clientMetrics["test-nil-client"] = &ClientMetricsCache{
			ClientInfo:    nil,
			LastHeartbeat: time.Now(),
		}
		mgr.mu.Unlock()
...
// TestHandleHeartbeatAccessedDatabasesNil tests edge case where AccessedDatabases is nil
func TestHandleHeartbeatAccessedDatabasesNil(t *testing.T) {
	mgr := NewTelemetryManager(nil)
	...
	// Manually set AccessedDatabases to nil to test defensive code
	mgr.mu.Lock()
	if cache, ok := mgr.clientMetrics["access-db-test"]; ok {
		cache.AccessedDatabases = nil
	}
	mgr.mu.Unlock()

	// Now send heartbeat with db_name - should not panic
	resp, err := mgr.HandleHeartbeat(&milvuspb.ClientHeartbeatRequest{
		ClientInfo: &commonpb.ClientInfo{
			SdkType: "test-sdk",
			Reserved: map[string]string{
	...
	mgr.mu.RLock()
	cache := mgr.clientMetrics["access-db-test"]
	_, exists := cache.AccessedDatabases["new_database"]
	mgr.mu.RUnlock()
...
func TestProcessCommandRepliesNilStore2(t *testing.T) {
	...
	// Should not panic
	mgr.processCommandReplies("test-client", replies)
}
...
func TestProcessCommandRepliesPayloadFormat(t *testing.T) {
	...
	mgr.processCommandReplies(clientID, []*commonpb.CommandReply{
		{
