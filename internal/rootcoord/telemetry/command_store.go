...
func (s *CommandStore) PushCommand(ctx context.Context, req *milvuspb.PushClientCommandRequest) (string, error) {
	...
	if req.Persistent {
		...
		existingIDs, existingPayloads := s.getConfigIDsAndPayloads(req.CommandType, scope)
...
func (s *CommandStore) getConfigIDsAndPayloads(configType, scope string) ([]string, [][]byte) {
	s.cacheMu.RLock()
