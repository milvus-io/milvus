...
func (mt *MetaTable) RenameCollection(ctx context.Context, dbName string, oldName string, newDBName string, newName string, ts Timestamp) error {
	...
	if newDBName != dbName {
		originalDB, ok := mt.dbName2Meta[dbName]
		if !ok {
			log.Warn("original  database not found")
			return fmt.Errorf("database:%s not found", dbName)
		}

		if hookutil.IsDBEncyptionEnabled(originalDB.Properties) || hookutil.IsDBEncyptionEnabled(targetDB.Properties) {
			log.Warn("deny to change collection databases due to at least one database enabled encryption")
