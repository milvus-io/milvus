...
// /expr must keep using the proxy's own credential check. Routing it through
// the management verifier would hand it that verifier's cached hash -- a
// rotated-away root password would keep opening /expr for as long as the cache
// held it -- and let it answer 503, on a flag /expr has nothing to do with.
func TestExprUsesDataPlaneVerifierNotManagementVerifier(t *testing.T) {
	paramtable.Init()
	params := paramtable.Get()
	require.NoError(t, params.Save(params.CommonCfg.ExprAuthMode.Key, ExprAuthModeRootOnly))
	t.Cleanup(func() { params.Reset(params.CommonCfg.ExprAuthMode.Key) })

	dataPlaneCalls, managementCalls := 0, 0
	setVerifyFunc(t, func(_ context.Context, username, password string) bool {
		dataPlaneCalls++
		return username == util.UserRoot && password == "live-password"
	})
	setManagementVerifierKeepingOthers(t, VerifierSlotProxy, func(context.Context, string, string) error {
		managementCalls++
		return nil
	})

	req := httptest.NewRequest(http.MethodGet, ExprPath, nil)
	req.SetBasicAuth(util.UserRoot, "live-password")
	assert.NoError(t, CheckExprAuth(context.Background(), req))

	req = httptest.NewRequest(http.MethodGet, ExprPath, nil)
	req.SetBasicAuth(util.UserRoot, "rotated-away")
	assert.True(t, IsAuthenticationError(CheckExprAuth(context.Background(), req)),
		"a password the data plane rejects must not be admitted by the management cache")

	assert.Equal(t, 2, dataPlaneCalls)
	assert.Zero(t, managementCalls, "/expr must not reach the management verifier")
}

// setManagementVerifierKeepingOthers installs fn in one slot without clearing
// the rest, for tests that also need setVerifyFunc's data-plane verifier.
func setManagementVerifierKeepingOthers(t *testing.T, slot ManagementVerifierSlot, fn CredentialVerifier) {
	t.Helper()
	passwordVerifyMu.Lock()
	prev := managementVerifiers[slot]
	managementVerifiers[slot] = fn
	passwordVerifyMu.Unlock()
	t.Cleanup(func() {
		passwordVerifyMu.Lock()
		defer passwordVerifyMu.Unlock()
		managementVerifiers[slot] = prev
	})
}
