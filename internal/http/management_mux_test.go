...
// The V1 registration switch must never reopen the metrics-port fallback
// when the process has opted into metrics and probes only.
func TestManagementMetricsOnlyTakesPrecedenceOverV1(t *testing.T) {
	for _, enableV1 := range []string{"true", "false"} {
		t.Run(enableV1, func(t *testing.T) {
			setupManagementMode(t, "true")
			params := paramtable.Get()
			require.NoError(t, params.Save(params.HTTPCfg.EnableV1.Key, enableV1))
			t.Cleanup(func() { params.Reset(params.HTTPCfg.EnableV1.Key) })
			registerDefaults()
			called := false
			Register(&Handler{Path: RootPath, HandlerFunc: func(w http.ResponseWriter, r *http.Request) {
				called = true
				w.WriteHeader(http.StatusOK)
			}})
			for _, path := range []string{"/api/v1/_cluster/configs", "/api/v1/health"} {
				w := httptest.NewRecorder()
				metricsServer.ServeHTTP(w, httptest.NewRequest(http.MethodGet, path, nil))
				assert.Equal(t, http.StatusNotFound, w.Code)
			}
			assert.False(t, called)
			w := httptest.NewRecorder()
			metricsServer.ServeHTTP(w, httptest.NewRequest(http.MethodGet, LivezRouterPath, nil))
			assert.Equal(t, http.StatusOK, w.Code)
		})
	}
}
