...
// check used by HTTP RBAC and /expr. This is called by the proxy package to
...
// deliberately not consulted -- that is /expr's, and mixing the two would give
// /expr this cache's staleness and let it answer 503.
...
// hasRBACPasswordVerifier reports whether the proxy-owned verifier /expr needs
// is registered. A management-plane verifier must not make /expr look
// available on a coordinator or worker node.
...
// verifyRBACPassword verifies credentials for HTTP RBAC and for /expr, using
// only the proxy-owned bool verifier: the management verifier is a root-only
// hook, and letting it stand in here would make a MixCoord registration reject
// valid non-root users in a standalone process. endpoint is accepted only so
// this matches checkRootAuth's shape.
...
// metricsPortAuthMiddleware; /expr keeps its own exprAuthMode; the probe and
// scrape surface stays open by design.
...
func registerDefaults() {
	...
	Register(&Handler{
		...
		Handler: http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			// /expr keeps its own auth mode, so it never passes through
			// wrapAdminAuth and nothing else applies the cross-site check to
			// it. It needs one once the gate is on: it executes a
			// query-string expression, so a link or an <img> is enough to
			// invoke it, and the gate teaches browsers to hold a root
			// credential for this origin.
			if AdminAuthEnabled() {
				if status, msg, ok := CheckCrossSite(req, ExprPath, false); !ok {
...
// Register() serving http.DefaultServeMux; nothing serves that mux now, so the
// only way onto this port is an explicit call with a visible auth posture. Heap
// dumps can reveal cached credentials, so these are gated.
func registerPprof() {
