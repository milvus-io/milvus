...
func updateMetaFromTask(ctx context.Context, triggerTask task, meta Meta) error {
	...
	default:
		...
		segments, err := getSegmentsFromTasks(meta, loadSegmentTasks)
		if err != nil {
			return err
		}
		...
		dmChannels, err := getDmChannelsFromTasks(meta, watchDmChannelTasks)
		if err != nil {
			return err
		}
...
func getSegmentsFromTasks(meta Meta, tasks []*loadSegmentTask) (map[UniqueID]*querypb.SegmentInfo, error) {
	var (
...
func getDmChannelsFromTasks(meta Meta, tasks []*watchDmChannelTask) (map[string]*querypb.DmChannelWatchInfo, error) {
	var (
