...
// added when namespaces are enabled. Counting them would report every
// collection as an Int64 user, and the namespace field's existence is already
// what the enable_namespace predicate reports.
func isServerField(f *model.Field) bool {
	return common.IsSystemField(f.FieldID) || f.Name == common.NamespaceFieldName
}
