...
func TestComputeCollectionEntries(t *testing.T) {
	...
	assert.Len(t, ob, 2, "databases and aliases only; RBAC objects are not counted")
...
func TestComputeCollectionEntriesEmpty(t *testing.T) {
	...
	ob := indexGroup(entries, GroupObjects)
	assert.Len(t, ob, 2, "databases and aliases")
	for _, e := range ob {
		assert.Zero(t, e.Value)
	}
}
...
// The server adds RowID and Timestamp to every collection, and a namespace
// field when namespaces are on. None of them is a user's field: a collection
// whose only user fields are VarChar and a vector must not show up as an Int64
// user, and the namespace field is reported by enable_namespace, not by type.
func TestServerFieldsAreNotCounted(t *testing.T) {
	col := &model.Collection{
		CollectionID: 1, ShardsNum: 1,
		Fields: []*model.Field{
			{FieldID: common.RowIDField, Name: common.RowIDFieldName, DataType: schemapb.DataType_Int64},
			{FieldID: common.TimeStampField, Name: common.TimeStampFieldName, DataType: schemapb.DataType_Int64},
			{FieldID: 100, Name: "pk", DataType: schemapb.DataType_VarChar, IsPrimaryKey: true},
			{FieldID: 101, Name: common.NamespaceFieldName, DataType: schemapb.DataType_Int64},
			{FieldID: 102, Name: "vec", DataType: schemapb.DataType_FloatVector, TypeParams: []*commonpb.KeyValuePair{kv(common.DimKey, "8")}},
		},
	}
	ft := indexGroup(ComputeCollectionEntries(CollectionInput{Collections: []*model.Collection{col}}), GroupFieldTypes)
	assert.EqualValues(t, 0, ft[schemapb.DataType_Int64.String()].Value)
	assert.EqualValues(t, 1, ft[schemapb.DataType_VarChar.String()].Value)
	assert.EqualValues(t, 1, ft[schemapb.DataType_FloatVector.String()].Value)
}
