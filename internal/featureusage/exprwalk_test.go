...
func TestCollectExprFeatures_ParsedExpressions(t *testing.T) {
	...
	cases := []struct {
		...
		{`Int64Field > 3 and Int64Field < 10`, []string{"comparison_operators=relational"}},
		{`Int64Field in [1, 2, 3]`, []string{"in_operator"}},
		{`VarCharField == "a" or VarCharField != "b"`, []string{"comparison_operators=equality"}},
		...
		{`exists JSONField["a"]`, []string{"exists", "json_path"}},
		{`exists $meta["dyn"]`, []string{"exists", "json_path"}},
		{`JSONField["a"] > 1`, []string{"json_path", "comparison_operators=relational"}},
		{`JSONField["a"]["b"] == "x" and Int64Field > 0`, []string{"json_path", "comparison_operators=equality", "comparison_operators=relational"}},
		{`json_contains(JSONField["tags"], 1)`, []string{"json_contains", "json_path"}},
		{`json_contains_all(JSONField["tags"], [1, 2])`, []string{"json_contains", "json_path"}},
		{`json_contains_any(JSONField["tags"], [1, 2])`, []string{"json_contains", "json_path"}},
		{`array_contains(ArrayField, 1)`, []string{"array_contains"}},
		{`array_contains_any(ArrayField, [1, 2])`, []string{"array_contains"}},
		{`array_length(ArrayField) == 3`, []string{"array_length", "comparison_operators=equality"}},
		{`ArrayField[0] > 1`, []string{"comparison_operators=relational"}},
		...
		{`TimestamptzField > ISO '2025-01-01T00:00:00Z'`, []string{"timestamptz_compare", "comparison_operators=relational"}},
		{`random_sample(0.1)`, []string{"random_sample"}},
		{`Int64Field > 5 and random_sample(0.5)`, []string{"random_sample", "comparison_operators=relational"}},
		// Several features in one predicate: each is reported once.
		{
			...
			[]string{"text_match", "exists", "json_path", "comparison_operators=relational"},
		},
	}
	for _, c := range cases {
...
func TestCollectExprFeatures_HandBuiltNodes(t *testing.T) {
	...
	assert.ElementsMatch(t, []string{"element_filter", "regex_match", "struct_array_match", "timestamptz_compare"}, names)
