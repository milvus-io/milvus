...
const (
	...
	GroupDBProperties = "database_properties"
	...
	GroupLoaded = "loaded"
	// GroupRequest: monotonic count of requests using this feature since node
	// start.
	//
	// "Request" is the user request on the Proxy and the created task on
	// MixCoord. On a QueryNode it is the search or query request that node
	// received, which is not the same thing: a delegator fans one user search
	// out to one request per shard, and to a separate request per segment group
	// (sealed and growing). A QueryNode counter is therefore a rate to compare
	// against other QueryNode counters, not a count of user requests.
