...
const (
	...
	SegmentTextStats            = "text_match_index"
	SegmentJSONKeyStats         = "json_shredding"
	SegmentBM25Stats            = "full_text_search"
)

// StorageVersionName is the documented name of a segment storage format:
// V1 (the 2.0 binlog format), V2 or V3. The legacy format is stored as 0
// (storage.StorageV1), not 1, so the stored integer is not the name.
func StorageVersionName(v int64) string {
	if v == 0 {
		return "V1"
	}
...
func ComputeSegmentEntries(segments []*datapb.SegmentInfo) []*internalpb.FeatureEntry {
	...
	for _, seg := range segments {
		...
		c.addOnce(s, GroupSegment, SegmentStorageVersionPrefix+StorageVersionName(seg.GetStorageVersion()), "")
		if seg.GetIsSorted() {
