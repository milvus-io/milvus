...
func TestComputeSegmentEntries(t *testing.T) {
	segs := []*datapb.SegmentInfo{
		// collection 1: two live segments, V2 and the legacy V1 format (stored as
		// 0, storage.StorageV1), one sorted with text stats
		{
		...
		{
			ID: 2, CollectionID: 1, State: commonpb.SegmentState_Sealed, StorageVersion: 0,
	...
	assert.EqualValues(t, 2, entries["storage_version=V2"].Value, "one per collection, two collections on v2")
	assert.EqualValues(t, 1, entries["storage_version=V1"].Value)
	assert.NotContains(t, entries, "storage_version=V0", "the legacy format is named V1, not by its stored value")
	assert.NotContains(t, entries, "storage_version=V9", "dropped / invisible segments are skipped")
...
func TestStorageVersionName(t *testing.T) {
	assert.Equal(t, "V1", StorageVersionName(0))
	assert.Equal(t, "V2", StorageVersionName(2))
	assert.Equal(t, "V3", StorageVersionName(3))
}
