...
func Test_InternalTLS(t *testing.T) {
	...
	mockGrpcClient := mocks.NewMockGrpcClient[rootcoordpb.RootCoordClient](t)

	mockGrpcClient.EXPECT().Close().Return(nil)
	mockGrpcClient.EXPECT().GetNodeID().Return(1)
	mockGrpcClient.EXPECT().ReCall(testify_mock.Anything, testify_mock.Anything).RunAndReturn(func(ctx context.Context, f func(rootcoordpb.RootCoordClient) (interface{}, error)) (interface{}, error) {
		return f(mockRC)
	})
	...
	t.Run("NoCertPool", func(t *testing.T) {
		...
		mockRC.EXPECT().GetComponentStates(testify_mock.Anything, testify_mock.Anything).Return(nil, ErrNoCertPool)
	...
	t.Run("InvalidCertPath", func(t *testing.T) {
		...
		cp, err := utils.CreateCertPoolforClient(invalidCAPath, "rootcoord")
		assert.NotNil(t, err)
		assert.Nil(t, cp)
	})
	...
	t.Run("TlsHandshakeFailed", func(t *testing.T) {
		cp, err := utils.CreateCertPoolforClient(validPath, "rootcoord")
		assert.Nil(t, err)
		mockRC.ExpectedCalls = nil

		mockGrpcClient.EXPECT().SetInternalTLSCertPool(cp).Return().Once()
		mockGrpcClient.EXPECT().GetNodeID().Return(1)
		mockRC.EXPECT().GetComponentStates(testify_mock.Anything, testify_mock.Anything).Return(nil, errors.New("TLS handshake failed"))
	...
	t.Run("TlsHandshakeSuccess", func(t *testing.T) {
		...
		mockGrpcClient.EXPECT().SetInternalTLSCertPool(cp).Return().Once()
		mockGrpcClient.EXPECT().GetNodeID().Return(1)
		mockRC.EXPECT().GetComponentStates(testify_mock.Anything, testify_mock.Anything).Return(&milvuspb.ComponentStates{}, nil)
