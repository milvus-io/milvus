...
func (s *Server) init() error {
	...
	queryService, err := qsc.NewClient(qn.Params.MetaRootPath, qn.Params.EtcdAddress, 3*time.Second)
	if err != nil {
		panic(err)
	}
	...
	masterService, err := msc.NewClient(qn.Params.MetaRootPath, qn.Params.EtcdAddress, 3*time.Second)
	if err != nil {
		panic(err)
	}
	...
	indexService := isc.NewClient(qn.Params.MetaRootPath, qn.Params.EtcdAddress, 3*time.Second)
	...
	dataService := dsc.NewClient(qn.Params.MetaRootPath, qn.Params.EtcdAddress, 3*time.Second)
