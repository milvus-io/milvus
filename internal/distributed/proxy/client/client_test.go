...
func Test_InternalTLS(t *testing.T) {
	...
	mockGrpcClient.EXPECT().ReCall(testify_mock.Anything, testify_mock.Anything).RunAndReturn(func(ctx context.Context, f func(proxypb.ProxyClient) (interface{}, error)) (interface{}, error) {
		return f(mockProxy)
	})
	...
	t.Run("NoCertPool", func(t *testing.T) {
		...
		mockProxy.EXPECT().GetComponentStates(testify_mock.Anything, testify_mock.Anything).Return(nil, ErrNoCertPool)
	...
	t.Run("InvalidCertPath", func(t *testing.T) {
		...
		cp, err := utils.CreateCertPoolforClient(invalidCAPath, "proxy")
		assert.NotNil(t, err)
		assert.Nil(t, cp)
	})
	...
	t.Run("TlsHandshakeFailed", func(t *testing.T) {
		cp, err := utils.CreateCertPoolforClient(validPath, "proxy")
		assert.Nil(t, err)
		mockProxy.ExpectedCalls = nil

		mockGrpcClient.EXPECT().SetInternalTLSCertPool(cp).Return().Once()
		mockGrpcClient.EXPECT().GetNodeID().Return(1)
		mockProxy.EXPECT().GetComponentStates(testify_mock.Anything, testify_mock.Anything).Return(nil, errors.New("TLS handshake failed"))
	...
	t.Run("TlsHandshakeSuccess", func(t *testing.T) {
		...
		mockGrpcClient.EXPECT().SetInternalTLSCertPool(cp).Return().Once()
		mockGrpcClient.EXPECT().GetNodeID().Return(1)
		mockProxy.EXPECT().GetComponentStates(testify_mock.Anything, testify_mock.Anything).Return(&milvuspb.ComponentStates{}, nil)
