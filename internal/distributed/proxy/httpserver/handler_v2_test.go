...
func TestPrewarmNamespaceV2(t *testing.T) {
	paramtable.Init()
	paramtable.Get().Save(paramtable.Get().QuotaConfig.QuotaAndLimitsEnabled.Key, "false")
	defer paramtable.Get().Reset(paramtable.Get().QuotaConfig.QuotaAndLimitsEnabled.Key)
	...
	const (
		collectionName = "docs"
		namespaceName  = "tenant_123"
		taskID         = "prewarm_10001"
	)

	mp := mocks.NewMockProxy(t)
	mp.EXPECT().Prewarm(mock.Anything, mock.MatchedBy(func(req *milvuspb.PrewarmRequest) bool {
		return req.GetDbName() == DefaultDbName &&
			req.GetCollectionName() == collectionName &&
			req.GetNamespace() == namespaceName &&
			req.GetTtlSeconds() == 3600 &&
			req.GetPriority() == "normal"
	})).Return(&milvuspb.PrewarmResponse{
		Status:    commonSuccessStatus,
		TaskID:    taskID,
	...
	testEngine := initHTTPServerV2(mp, false)
	req := httptest.NewRequest(http.MethodPost, versionalV2(NamespaceCategory, PrewarmAction), bytes.NewReader([]byte(`{
		"collectionName": "docs",
		"namespaceName": "tenant_123",
		"ttlSeconds": 3600,
		"priority": "normal"
	...
	assert.Equal(t, merr.Code(nil), body.Code)
	assert.Equal(t, taskID, body.Data.TaskID)
	assert.Equal(t, namespaceName, body.Data.NamespaceName)
}

func TestDescribePrewarmTaskV2(t *testing.T) {
	paramtable.Init()
	paramtable.Get().Save(paramtable.Get().QuotaConfig.QuotaAndLimitsEnabled.Key, "false")
	defer paramtable.Get().Reset(paramtable.Get().QuotaConfig.QuotaAndLimitsEnabled.Key)

	const taskID = "prewarm_10001"
	mp := mocks.NewMockProxy(t)
	mp.EXPECT().DescribePrewarmTask(mock.Anything, mock.MatchedBy(func(req *milvuspb.DescribePrewarmTaskRequest) bool {
		return req.GetTaskID() == taskID
	})).Return(&milvuspb.DescribePrewarmTaskResponse{
		Status:   commonSuccessStatus,
		TaskID:   taskID,
		State:    milvuspb.PrewarmTaskState_PrewarmTaskStateWarming,
		Progress: 60,
	}, nil).Once()

	testEngine := initHTTPServerV2(mp, false)
	req := httptest.NewRequest(http.MethodPost, versionalV2(NamespaceCategory, PrewarmAction+"/"+DescribeAction), bytes.NewReader([]byte(`{
		"taskId": "prewarm_10001"
	}`)))
	w := httptest.NewRecorder()
	testEngine.ServeHTTP(w, req)
	...
	assert.Equal(t, merr.Code(nil), body.Code)
	assert.Equal(t, taskID, body.Data.TaskID)
	assert.Equal(t, "Warming", body.Data.State)
	assert.EqualValues(t, 60, body.Data.Progress)
