...
type externalCollectionRESTProxy struct {
	...
	restoreReq                 *datapb.RestoreSnapshotRequest
...
func (m *externalCollectionRESTProxy) RestoreExternalSnapshotInternal(ctx context.Context, request *datapb.RestoreSnapshotRequest) (*datapb.RestoreSnapshotResponse, error) {
...
func TestRestoreExternalSnapshotRESTV2(t *testing.T) {
	...
	assert.Equal(t, http.StatusOK, w.Code)
	assert.Equal(t, "default", proxy.restoreReq.GetTargetDbName())
	assert.Equal(t, "restored_books", proxy.restoreReq.GetTargetCollectionName())
	assert.True(t, proxy.restoreReq.GetExternal())
	assert.Equal(t, "s3://bucket/files/snapshots/meta.json", proxy.restoreReq.GetSnapshotS3Location())
...
func TestRestoreExternalSnapshotRequiresGlobalPrivilege(t *testing.T) {
	paramtable.Init()
	proxy.InitEmptyGlobalCache()

	userName := "restore_external_user"
	roleName := "restore_external_role"
	dbName := "default"
	privilegeName := commonpb.ObjectPrivilege_PrivilegeRestoreExternalSnapshot.String()
	userRoles := []string{funcutil.EncodeUserRoleCache(userName, roleName)}
	defer privilege.GetPrivilegeCache().InitPolicyInfo(nil, nil)

	t.Run("collection restore privilege is not enough", func(t *testing.T) {
		privilege.GetPrivilegeCache().InitPolicyInfo(
			[]string{funcutil.PolicyForPrivilege(roleName, commonpb.ObjectType_Collection.String(), util.AnyWord, privilegeName, dbName)},
			userRoles,
		)

		ginCtx, _ := gin.CreateTestContext(httptest.NewRecorder())
		ginCtx.Set(ContextUsername, userName)
		err := checkGlobalRestoreSnapshotAuthorization(context.Background(), ginCtx, dbName)
		assert.Error(t, err)
	})

	t.Run("global restore privilege is accepted", func(t *testing.T) {
		privilege.GetPrivilegeCache().InitPolicyInfo(
			[]string{funcutil.PolicyForPrivilege(roleName, commonpb.ObjectType_Global.String(), util.AnyWord, privilegeName, dbName)},
			userRoles,
		)
