...
func TestCheckGetPrimaryKeyTyping(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"a number for a VarChar key", varcharColl, `{"id": [1]}`},
...
func TestTemplateArrayChecksEveryElement(t *testing.T) {
	t.Run("an oversized integer after a float is rejected", func(t *testing.T) {
		_, err := templateValueFromJSON("v", gjson.Parse(`[1.5, 9223372036854775809]`))
		require.Error(t, err)
		assert.ErrorIs(t, err, merr.ErrParameterInvalid)
	...
	t.Run("an oversized integer after a string is rejected", func(t *testing.T) {
		_, err := templateValueFromJSON("v", gjson.Parse(`["a", 99999999999999999999]`))
		require.Error(t, err)
		assert.ErrorIs(t, err, merr.ErrParameterInvalid)
	})

	t.Run("a mixed array of representable values still works", func(t *testing.T) {
		value, err := templateValueFromJSON("v", gjson.Parse(`[1.5, 9223372036854775807]`))
		require.NoError(t, err)
...
func TestTemplateArrayChecksNestedIntegers(t *testing.T) {
	for _, tt := range []struct {
		...
		// a fraction or an exponent asks for a double, and comparing it as one
		// is what the caller wrote
		{"a float literal is left alone", `[9223372036854775809.0, "x"]`, false},
	} {
		t.Run(tt.name, func(t *testing.T) {
			_, err := templateValueFromJSON("v", gjson.Parse(tt.value))
			if tt.rejected {
				require.Error(t, err)
				assert.ErrorIs(t, err, merr.ErrParameterInvalid)
			} else {
