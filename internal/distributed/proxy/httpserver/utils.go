		...
		for _, element := range elements {
			if element.Type != gjson.String {
				return nil, merr.WrapErrParameterInvalidMsg(
					"%s must be a string for a VarChar primary key, got: %s",
...
func checkEngineCompatible(field string, document string) error {
	...
	var walk func(node gjson.Result, depth int) error
	walk = func(node gjson.Result, depth int) error {
		// simdjson counts containers, not values: 1023 arrays holding a scalar
		// and 1024 empty arrays both parse, while 1024 arrays holding a scalar
		// do not. So the limit is on a node sitting past maxJSONDepth, not on
		// reaching it.
		...
		case node.Type == gjson.Number:
			if _, err := jsonNumberLiteral(field, node.Raw); err != nil {
				return err
			}
		}
		return nil
	...
	return walk(gjson.Parse(document), 1)
}
		...
		if data.Type == gjson.JSON {
			...
			for _, field := range collSchema.Fields {
				...
				if typeutil.IsVectorType(fieldType) {
					...
					// caught here.
					if !compatibilityMode && strings.TrimSpace(dataString) == "null" {
						return reallyDataArray, validDataMap, merr.WrapErrParameterInvalidMsg(
				...
				case schemapb.DataType_Float:
					if !compatibilityMode && fieldValue.Type == gjson.Number {
						// Rounding the literal to float64 and then to float32
						// rounds twice, which lands on a neighboring float32 for
						// a measurable share of literals. Parse to float32 once.
						parsed, err := strconv.ParseFloat(fieldValue.Raw, 32)
						if err != nil {
							return reallyDataArray, validDataMap, merr.WrapErrParameterInvalid(
								schemapb.DataType_name[int32(fieldType)], fieldValue.Raw, err.Error())
						}
...
func buildStructSubArrayScalar(sub *schemapb.FieldSchema, vals []gjson.Result, compatibilityMode bool) (*schemapb.ScalarField, error) {
	...
	case schemapb.DataType_Float:
		...
		for _, v := range vals {
			...
			// parse to float32 once; rounding via float64 rounds twice
			parsed, err := strconv.ParseFloat(v.Raw, 32)
			if err != nil {
				return nil, wrapStructSubParseError(sub, v, "expect float")
			}
...
func serializeFloatVectors(vectorStr string, dataType schemapb.DataType, dimension, bytesLen int64, fpArrayToBytesFunc func([]float32) []byte) ([][]byte, error) {
	...
	// search ran against a point the caller never asked for. This is the query
	// side of the same rule insert applies to a vector field, and like it is not
	// gated by compatibilityMode: a vector has no null coordinates to be
	// compatible with.
	var nullErr error
...
func serializeSparseFloatVectors(vectors []gjson.Result, dataType schemapb.DataType) ([][]byte, error) {
	...
	for _, vector := range vectors {
		...
		// an empty vector instead of reporting the mistake.
		if !compatibilityMode && strings.TrimSpace(string(vectorBytes)) == "null" {
...
func convertQueries2Placeholder(body string, dataType schemapb.DataType, dimension int64) (*commonpb.PlaceholderValue, error) {
	...
	case schemapb.DataType_VarChar:
		...
		for _, v := range res {
			...
			// for "", and an object searched for its own JSON text. Insert
			...
			for j := 0; j < columnNum; j++ {
				...
				case schemapb.DataType_JSON:
					...
					if ok && !fieldDataList[j].GetIsDynamic() {
						...
						// content. json.Valid restores that: it accepts trailing
...
func templateValueFromJSON(name string, value gjson.Result) (*schemapb.TemplateValue, error) {
	switch value.Type {
	case gjson.True, gjson.False:
	...
	case gjson.Number:
		...
		// An integer literal that does not fit must not become a float: comparing
		// 9223372036854775809 as a double matches ...808 instead, so the filter
		// silently returns the wrong rows.
		if !strings.ContainsAny(value.Raw, ".eE") {
			return nil, merr.WrapErrParameterInvalidMsg(
				"expression template parameter %s has an integer %s outside the int64 range", name, value.Raw)
		}
	...
	case gjson.JSON:
		if value.IsArray() {
			array, err := templateArrayFromJSON(name, value)
			if err != nil {
				return nil, err
			}
...
// checkTemplateIntegerRange rejects an integer literal an int64 cannot hold,
// anywhere inside an expression template parameter.
//
// Such a literal is only ever carried as raw JSON, which the planner compares as
// a double, so it silently matches a neighboring value instead of nothing. A
// literal written with a fraction or an exponent is left alone: the caller asked
// for a floating-point value and comparing it as a double is what that means.
func checkTemplateIntegerRange(name string, value gjson.Result) error {
	switch {
	case value.IsArray(), value.IsObject():
		var err error
		value.ForEach(func(_, element gjson.Result) bool {
			err = checkTemplateIntegerRange(name, element)
			return err == nil
		})
		return err

	case value.Type == gjson.Number && !strings.ContainsAny(value.Raw, ".eE"):
		if _, ok := parseJSONInteger(value.Raw, 64); !ok {
			return merr.WrapErrParameterInvalidMsg(
				"expression template parameter %s has an integer %s outside the int64 range", name, value.Raw)
		}
...
func templateArrayFromJSON(name string, value gjson.Result) (*schemapb.TemplateArrayValue, error) {
	...
	for _, element := range elements {
		...
		if err := checkTemplateIntegerRange(name, element); err != nil {
			return nil, err
		}
	...
	case schemapb.DataType_Array:
		...
		for i, element := range elements {
			nested, err := templateArrayFromJSON(name, element)
			if err != nil {
				return nil, err
			}
			result[i] = nested
...
func generateExpressionTemplate(params map[string]json.RawMessage) (map[string]*schemapb.TemplateValue, error) {
	...
	for name, raw := range params {
		value, err := templateValueFromJSON(name, gjson.ParseBytes(raw))
		if err != nil {
			return nil, err
		}
