...
func anyToColumns(rows []map[string]interface{}, validDataMap map[string][]bool, sch *schemapb.CollectionSchema, inInsert bool, partialUpdate bool) ([]*schemapb.FieldData, error) {
	...
	for _, row := range rows {
		...
		for _, structArrayField := range sch.StructArrayFields {
			candi, _ := set[structArrayField.Name]
			singleStructMap := candi.v.Interface().(map[string]any)
			existingStuctMap := nameColumns[structArrayField.Name].(map[string]*schemapb.FieldData)
			for name, singleFieldData := range singleStructMap {
				switch singleFieldData.(type) {
				case *schemapb.ScalarField:
					existingStuctMap[name].Field.(*schemapb.FieldData_Scalars).Scalars.Data.(*schemapb.ScalarField_ArrayData).ArrayData.Data = append(existingStuctMap[name].Field.(*schemapb.FieldData_Scalars).Scalars.Data.(*schemapb.ScalarField_ArrayData).ArrayData.Data, singleFieldData.(*schemapb.ScalarField))
				case *schemapb.VectorField:
					existingStuctMap[name].Field.(*schemapb.FieldData_Vectors).Vectors.Data.(*schemapb.VectorField_VectorArray).VectorArray.Data = append(existingStuctMap[name].Field.(*schemapb.FieldData_Vectors).Vectors.Data.(*schemapb.VectorField_VectorArray).VectorArray.Data, singleFieldData.(*schemapb.VectorField))
				default:
					return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("invalid type(%v) of field(%v) ", structArrayField.Name))
				}
			}
			nameColumns[structArrayField.Name] = existingStuctMap
		}
