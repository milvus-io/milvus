...
func (h *HandlersV2) query(ctx context.Context, c *gin.Context, anyReq any, dbName string) (interface{}, error) {
	...
	if err == nil {
		...
		if buildErr != nil {
			...
			mlog.Warn(ctx, "high level restful api, fail to deal with query result", mlog.Any("response", resp), mlog.Err(buildErr))
...
func (h *HandlersV2) get(ctx context.Context, c *gin.Context, anyReq any, dbName string) (interface{}, error) {
	...
	if err == nil {
		...
		if buildErr != nil {
			...
			queryResp.Status = merr.Status(merr.ErrInvalidSearchResult)
			mlog.Warn(ctx, "high level restful api, fail to deal with get result", mlog.Any("response", resp), mlog.Err(buildErr))
			returnInvalidSearchResult(c, buildErr)
		} else {
			scannedRemoteBytes, scannedTotalBytes, cacheHitRatio, isValid := proxy.GetStorageCost(queryResp.GetStatus())
			respBody := gin.H{
	...
	if err == nil {
		...
		if hasSearchAggregationResult(searchResp.Results) {
			...
			if buildErr != nil {
				...
				mlog.Warn(ctx, "high level restful api, fail to deal with search aggregation result", mlog.Any("result", searchResp.Results), mlog.Err(buildErr))
				returnInvalidSearchResult(c, buildErr)
			} else {
				respBody := gin.H{
					HTTPReturnCode:     merr.Code(nil),
