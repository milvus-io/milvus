...
func (h *HandlersV2) createCollection(ctx context.Context, c *gin.Context, anyReq any, dbName string) (interface{}, error) {
	...
	if len(httpReq.Schema.Fields) == 0 {
		...
		for _, structArrayField := range httpReq.Schema.StructArrayFields {
			...
			maxCapacity, hasMaxCapacity := structArrayField.TypeParams["max_capacity"]
			if !hasMaxCapacity {
				err := merr.WrapErrParameterInvalidMsg("max_capacity is required for struct array field: %s", structArrayField.Name)
			...
			for _, field := range structArrayField.Fields {
				...
				if field.ElementTypeParams == nil {
					field.ElementTypeParams = make(map[string]interface{})
				}
				field.ElementTypeParams["max_capacity"] = maxCapacity
