...
func (h *HandlersV1) describeCollection(ctx context.Context, c *gin.Context, dbName string, collectionName string) (*schemapb.CollectionSchema, error) {
	...
	if ok && primaryField.AutoID && !primaryField.AutoID {
		mlog.Warn(ctx, "primary filed autoID VS schema autoID", zap.String("collectionName", collectionName), zap.Bool("primary Field", primaryField.AutoID), zap.Bool("schema", primaryField.AutoID))
...
func (h *HandlersV1) createCollection(c *gin.Context) {
	...
	if err := c.ShouldBindWith(&httpReq, binding.JSON); err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, the parameter of create collection is incorrect", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
	...
	if httpReq.CollectionName == "" || httpReq.Dimension == 0 {
		mlog.Warn(c.Request.Context(), "high level restful api, create collection require parameters: [collectionName, dimension], but miss", zap.Any("request", httpReq))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
	...
	if err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, marshal collection schema fail", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
...
func (h *HandlersV1) getCollectionDetails(c *gin.Context) {
	...
	if collectionName == "" {
		mlog.Warn(c.Request.Context(), "high level restful api, desc collection require parameter: [collectionName], but miss")
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrMissingRequiredParameters),
	...
	if err != nil {
		mlog.Warn(ctx, "get collection load state fail",
	...
	if err != nil {
		indexDesc = []gin.H{}
		mlog.Warn(ctx, "get indexes description fail",
			zap.String("collection", collectionName),
...
func (h *HandlersV1) dropCollection(c *gin.Context) {
	...
	if err := c.ShouldBindWith(&httpReq, binding.JSON); err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, the parameter of drop collection is incorrect", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
			HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
		})
	...
	if httpReq.CollectionName == "" {
		mlog.Warn(c.Request.Context(), "high level restful api, drop collection require parameter: [collectionName], but miss")
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrMissingRequiredParameters),
			HTTPReturnMessage: merr.ErrMissingRequiredParameters.Error() + ", required parameters: [collectionName]",
		})
...
func (h *HandlersV1) query(c *gin.Context) {
	...
	if err := c.ShouldBindWith(&httpReq, binding.JSON); err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, the parameter of query is incorrect", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
			HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
		})
	...
	if httpReq.CollectionName == "" || httpReq.Filter == "" {
		mlog.Warn(c.Request.Context(), "high level restful api, query require parameter: [collectionName, filter], but miss")
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrMissingRequiredParameters),
	...
	if err != nil {
		...
		if err != nil {
			mlog.Warn(ctx, "high level restful api, fail to deal with query result", zap.Any("response", response), zap.Error(err))
...
func (h *HandlersV1) get(c *gin.Context) {
	...
	if err := c.ShouldBindBodyWith(&httpReq, binding.JSON); err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, the parameter of get is incorrect", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
			HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
		})
	...
	if httpReq.CollectionName == "" || httpReq.ID == nil {
		mlog.Warn(c.Request.Context(), "high level restful api, get require parameter: [collectionName, id], but miss")
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrMissingRequiredParameters),
	...
	if err != nil {
		...
		outputData, err := buildQueryResp(int64(0), queryResp.OutputFields, queryResp.FieldsData, nil, nil, allowJS, nil)
		if err != nil {
			mlog.Warn(ctx, "high level restful api, fail to deal with get result", zap.Any("response", response), zap.Error(err))
			HTTPReturn(c, http.StatusOK, gin.H{
				HTTPReturnCode:    merr.Code(merr.ErrInvalidSearchResult),
				HTTPReturnMessage: merr.ErrInvalidSearchResult.Error() + ", error: " + err.Error(),
			})
...
func (h *HandlersV1) delete(c *gin.Context) {
	...
	if err := c.ShouldBindBodyWith(&httpReq, binding.JSON); err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, the parameter of delete is incorrect", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
			HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
		})
	...
	if httpReq.CollectionName == "" || (httpReq.ID == nil && httpReq.Filter == "") {
		mlog.Warn(c.Request.Context(), "high level restful api, delete require parameter: [collectionName, id/filter], but miss")
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrMissingRequiredParameters),
...
func (h *HandlersV1) insert(c *gin.Context) {
	...
	if err := c.ShouldBindBodyWith(&httpReq, binding.JSON); err != nil {
		...
		if err = c.ShouldBindBodyWith(&singleInsertReq, binding.JSON); err != nil {
			mlog.Warn(c.Request.Context(), "high level restful api, the parameter of insert is incorrect", zap.Any("request", httpReq), zap.Error(err))
			HTTPAbortReturn(c, http.StatusOK, gin.H{
				HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
				HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
			})
	...
	response, err := h.executeRestRequestInterceptor(ctx, c, req, func(reqCtx context.Context, req any) (any, error) {
		...
		if err != nil {
			mlog.Warn(ctx, "high level restful api, fail to deal with insert data", zap.Any("body", body), zap.Error(err))
			HTTPAbortReturn(c, http.StatusOK, gin.H{
		...
		if err != nil {
			mlog.Warn(ctx, "high level restful api, fail to deal with insert data", zap.Any("data", httpReq.Data), zap.Error(err))
			HTTPAbortReturn(c, http.StatusOK, gin.H{
				HTTPReturnCode:    merr.Code(merr.ErrInvalidInsertData),
				HTTPReturnMessage: merr.ErrInvalidInsertData.Error() + ", error: " + err.Error(),
			})
...
func (h *HandlersV1) upsert(c *gin.Context) {
	...
	if err := c.ShouldBindBodyWith(&httpReq, binding.JSON); err != nil {
		...
		if err = c.ShouldBindBodyWith(&singleUpsertReq, binding.JSON); err != nil {
			mlog.Warn(c.Request.Context(), "high level restful api, the parameter of upsert is incorrect", zap.Any("request", httpReq), zap.Error(err))
			HTTPAbortReturn(c, http.StatusOK, gin.H{
				HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
				HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
			})
	...
	response, err := h.executeRestRequestInterceptor(ctx, c, req, func(reqCtx context.Context, req any) (any, error) {
		...
		if err != nil {
			mlog.Warn(ctx, "high level restful api, fail to deal with upsert data", zap.Any("body", body), zap.Error(err))
			HTTPAbortReturn(c, http.StatusOK, gin.H{
				HTTPReturnCode:    merr.Code(merr.ErrInvalidInsertData),
				HTTPReturnMessage: merr.ErrInvalidInsertData.Error() + ", error: " + err.Error(),
			})
		...
		if err != nil {
			mlog.Warn(ctx, "high level restful api, fail to deal with upsert data", zap.Any("data", httpReq.Data), zap.Error(err))
			HTTPAbortReturn(c, http.StatusOK, gin.H{
				HTTPReturnCode:    merr.Code(merr.ErrInvalidInsertData),
				HTTPReturnMessage: merr.ErrInvalidInsertData.Error() + ", error: " + err.Error(),
			})
...
func (h *HandlersV1) search(c *gin.Context) {
	...
	if err := c.ShouldBindWith(&httpReq, binding.JSON); err != nil {
		mlog.Warn(c.Request.Context(), "high level restful api, the parameter of search is incorrect", zap.Any("request", httpReq), zap.Error(err))
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
			HTTPReturnMessage: merr.ErrIncorrectParameterFormat.Error() + ", error: " + err.Error(),
		})
	...
	if httpReq.CollectionName == "" || httpReq.Vector == nil {
		mlog.Warn(c.Request.Context(), "high level restful api, search require parameter: [collectionName, vector], but miss")
		HTTPAbortReturn(c, http.StatusOK, gin.H{
			HTTPReturnCode:    merr.Code(merr.ErrMissingRequiredParameters),
	...
	if httpReq.Params != nil {
		...
		if rangeFilterOk {
			if !radiusOk {
				mlog.Warn(c.Request.Context(), "high level restful api, search params invalid, because only "+ParamRangeFilter)
				HTTPAbortReturn(c, http.StatusOK, gin.H{
					HTTPReturnCode:    merr.Code(merr.ErrIncorrectParameterFormat),
	...
	if err != nil {
		...
		if searchResp.Results.TopK == int64(0) {
			...
			if err != nil {
				mlog.Warn(ctx, "high level restful api, fail to deal with search result", zap.Any("result", searchResp.Results), zap.Error(err))
				HTTPReturn(c, http.StatusOK, gin.H{
					HTTPReturnCode:    merr.Code(merr.ErrInvalidSearchResult),
					HTTPReturnMessage: merr.ErrInvalidSearchResult.Error() + ", error: " + err.Error(),
				})
